...
use crate::crypto::{group_ops, twopc_mpc};
...
pub fn all_natives(silent: bool, protocol_config: &ProtocolConfig) -> NativeFunctionTable {
    let pera_framework_natives: &[(&str, &str, NativeFunction)] = &[
    ...
    let pera_framework_natives_iter =
        pera_framework_natives
            .iter()
            .cloned()
    ...
    let pera_system_natives: &[(&str, &str, NativeFunction)] = &[
        (
            "validator",
    ...
    pera_system_natives
        ...
        .chain(pera_framework_natives_iter)
