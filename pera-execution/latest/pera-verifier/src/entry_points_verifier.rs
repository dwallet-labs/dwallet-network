...
use pera_types::randomness_state::is_mutable_random;
use pera_types::{
...
fn verify_param_type(
    ...
    // Only `pera::pera_system` is allowed to expose entry functions that accept a mutable clock
    // parameter.
    ...
    // Only `pera::pera_system` is allowed to expose entry functions that accept a mutable Random
    // parameter.
