...
mod checked {
    ...
    use pera_types::execution_status::{CommandArgumentError, PackageUpgradeError};
    use pera_types::storage::{get_package_objects, PackageObject};
    use pera_types::{
        base_types::{
            MoveObjectType, ObjectID, PeraAddress, TxContext, TxContextKind, RESOLVED_ASCII_STR,
            RESOLVED_STD_OPTION, RESOLVED_UTF8_STR, TX_CONTEXT_MODULE_NAME, TX_CONTEXT_STRUCT_NAME,
        },
        coin::Coin,
        error::{command_argument_error, ExecutionError, ExecutionErrorKind},
        id::{RESOLVED_PERA_ID, UID},
    ...
    fn publish_and_verify_modules(
        ...
        // run the Pera verifier
        for module in modules {
            // Run Pera bytecode verifier, which runs some additional checks that assume the Move
            // bytecode verifier has passed.
            pera_verifier::verifier::pera_verify_module_unmetered(module, &BTreeMap::new())?;
        }
    ...
    fn check_visibility_and_signature<Mode: ExecutionMode>(
        ...
        let data_store = PeraDataStore::new(&context.linkage_view, &no_new_packages);
        let module = context
            .vm
            .get_runtime()
    ...
    fn check_private_generics(
        ...
        if module_ident == (&PERA_FRAMEWORK_ADDRESS, EVENT_MODULE) {
            return Err(ExecutionError::new_with_source(
                ExecutionErrorKind::NonEntryFunctionInvoked,
                format!("Cannot directly call functions in pera::{}", EVENT_MODULE),
            ));
        }

        if module_ident == (&PERA_FRAMEWORK_ADDRESS, TRANSFER_MODULE)
            && PRIVATE_TRANSFER_FUNCTIONS.contains(&function)
        {
            let msg = format!(
                "Cannot directly call pera::{m}::{f}. \
                Use the public variant instead, pera::{m}::public_{f}",
                m = TRANSFER_MODULE,
                f = function
            );
            return Err(ExecutionError::new_with_source(
        ...
        match value {
            ...
            Value::Receiving(_, _, assigned_type) => {
                ...
                let Some(s) = context.vm.get_runtime().get_type(*sidx) else {
                    invariant_violation!("pera::transfer::Receiving struct not found in session")
                };
    ...
    pub fn is_tx_context(
        ...
        let is_tx_context_type = module_addr == &PERA_FRAMEWORK_ADDRESS
        ...
        fn deserialize<D: serde::de::Deserializer<'d>>(
            ...
            match self {
                ...
                PrimitiveArgumentLayout::Address => {
                    PeraAddress::deserialize(deserializer)?;
                    Ok(())
                }
            }
        }
