...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn into_effects_v1(
        ...
        let updated_gas_object_info = if let Some(coin_id) = gas_charger.gas_coin() {
            ...
            (
                ...
                Owner::AddressOwner(PeraAddress::default()),
            )
        };
    ...
    pub fn conserve_unmetered_storage_rebate(&mut self, unmetered_storage_rebate: u64) {
        ...
        let mut system_state_wrapper = self
            .read_object(&PERA_SYSTEM_STATE_OBJECT_ID)
        ...
        // If not, we would be dropping PERA on the floor by overriding it.
...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn get_objects_to_authenticate(
    ...
    ) -> PeraResult<(Vec<ObjectID>, HashSet<ObjectID>)> {
        ...
        for id in &self.execution_results.modified_objects {
            ...
            match &old_obj.owner {
                ...
                Owner::Immutable => {
                    assert!(is_epoch_change, "Immutable objects cannot be written, except for Pera Framework/Move stdlib upgrades at epoch change boundaries");
...
impl<'backing> TemporaryStore<'backing> {
    ...
    /// `PeraGasStatus` `storage_rebate` and `storage_gas_units` track the transaction
...
impl<'backing> TemporaryStore<'backing> {
    pub fn advance_epoch_safe_mode(
        ...
        let wrapper = get_pera_system_state_wrapper(self.store.as_object_store())
...
impl<'backing> TemporaryStore<'backing> {
    fn get_input_pera(
        ...
        if let Some(obj) = self.input_objects.get(id) {
            ...
            obj.get_total_pera(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input PERA in PERA conservation checking for input with \
                         type {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
            ...
            let Ok(Some(obj)) = self.store.get_object_by_key(id, expected_version) else {
                invariant_violation!(
                    "Failed looking up dynamic field {id} in PERA conservation checking"
                );
            };
            obj.get_total_pera(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input PERA in PERA conservation checking for type \
                         {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
    ...
    /// Check that this transaction neither creates nor destroys PERA. This should hold for all txes
    ...
    ///
    /// 1. all PERA in storage rebate fields of input objects should flow either to the transaction
    ///    storage rebate, or the transaction non-refundable storage rebate
    /// 2. all PERA charged for storage should flow into the storage rebate field of some output
    ///    object
    ///
    ...
    pub fn check_pera_conserved(
        ...
        // total amount of PERA in storage rebate of input objects
        let mut total_input_rebate = 0;
        // total amount of PERA in storage rebate of output objects
        ...
        if gas_summary.storage_cost == 0 {
                    ...
                    + gas_summary.non_refundable_storage_fee
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "PERA conservation failed -- no storage charges in gas summary \
            ...
            // all PERA in storage rebate fields of input objects should flow either to
            ...
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "PERA conservation failed -- {} PERA in storage rebate field of input objects, \
                        {} PERA in tx storage rebate or tx non-refundable storage rebate",
                    total_input_rebate, gas_summary.non_refundable_storage_fee,
                )));
            }

            // all PERA charged for storage should flow into the storage rebate field
            // of some output object
            if gas_summary.storage_cost != total_output_rebate {
                return Err(ExecutionError::invariant_violation(format!(
                    "PERA conservation failed -- {} PERA charged for storage, \
                        {} PERA in storage rebate field of output objects",
    ...
    /// Check that this transaction neither creates nor destroys PERA.
    /// This more expensive check will check a third invariant on top of the 2 performed
    /// by `check_pera_conserved` above:
    ///
    /// * all PERA in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// advance epoch transaction would mint `epoch_fees` amount of PERA, and burn `epoch_rebates`
    /// amount of PERA. We need these information for this check.
    pub fn check_pera_conserved_expensive(
        &self,
        gas_summary: &GasCostSummary,
        advance_epoch_gas_summary: Option<(u64, u64)>,
        layout_resolver: &mut impl LayoutResolver,
    ) -> Result<(), ExecutionError> {
        // total amount of PERA in input objects, including both coins and storage rebates
        let mut total_input_pera = 0;
        // total amount of PERA in output objects, including both coins and storage rebates
        let mut total_output_pera = 0;
        for (id, input, output) in self.get_modified_objects() {
            if let Some(input) = input {
                total_input_pera += self.get_input_pera(&id, input.version, layout_resolver)?;
            }
            if let Some(object) = output {
                total_output_pera += object.get_total_pera(layout_resolver).map_err(|e| {
                    make_invariant_violation!(
                        "Failed looking up output PERA in PERA conservation checking for \
                         mutated type {:?}: {e:#?}",
                        object.struct_tag(),
                    )
                })?;
        ...
        total_output_pera += gas_summary.computation_cost + gas_summary.non_refundable_storage_fee;
        if let Some((epoch_fees, epoch_rebates)) = advance_epoch_gas_summary {
            total_input_pera += epoch_fees;
            total_output_pera += epoch_rebates;
        }
        if total_input_pera != total_output_pera {
            return Err(ExecutionError::invariant_violation(format!(
                "PERA conservation failed: input={}, output={}, \
                    this transaction either mints or burns PERA",
                total_input_pera, total_output_pera,
            )));
        }
        Ok(())
    }
...
impl<'backing> Storage for TemporaryStore<'backing> {
    ...
    fn record_execution_results(&mut self, results: ExecutionResults) {
        let ExecutionResults::V2(results) = results else {
            panic!("ExecutionResults::V2 expected in pera-execution v1 and above");
        };
    ...
    fn check_coin_deny_list(
        ...
        unreachable!("Coin denylist v2 is not supported in pera-execution v1");
    }
}
    ...
    fn get_resource(
        ...
        let object = match self.read_object(&ObjectID::from(*address)) {
            ...
            None => match self.read_object(&ObjectID::from(*address)) {
                ...
                Some(x) => {
                    if !x.is_immutable() {
                        fp_bail!(PeraError::ExecutionInvariantViolation);
                    }
                    x
                }
            },
