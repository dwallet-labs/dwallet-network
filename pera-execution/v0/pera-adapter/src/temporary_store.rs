...
use pera_types::execution::{DynamicallyLoadedObjectMetadata, ExecutionResults, SharedInput};
...
use pera_types::storage::{BackingStore, DeleteKindWithOldVersion, DenyListResult, PackageObject};
...
impl<'backing> TemporaryStore<'backing> {
    ...
    ///
    /// 3. all PERA in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// amount of PERA. We need these information for conservation check.
    pub fn check_pera_conserved(
        ...
        for (id, input, output) in self.get_modified_objects() {
            if let Some((version, storage_rebate)) = input {
                ...
                if do_expensive_checks {
                    total_input_pera += self.get_input_pera(&id, version, layout_resolver)?;
                }
            }
        ...
        if do_expensive_checks {
            ...
            if total_input_pera != total_output_pera {
                ...
                format!("PERA conservation failed: input={}, output={}, this transaction either mints or burns PERA",
                total_input_pera,
                total_output_pera))
            );
            }
        ...
        // all PERA in storage rebate fields of input objects should flow either to the transaction storage rebate, or the non-refundable
        // storage rebate pool
        if total_input_rebate != gas_summary.storage_rebate + gas_summary.non_refundable_storage_fee
        {
            ...
            /*return Err(ExecutionError::invariant_violation(
                format!("PERA conservation failed--{} PERA in storage rebate field of input objects, {} PERA in tx storage rebate or tx non-refundable storage rebate",
                total_input_rebate,
                gas_summary.non_refundable_storage_fee))
            );*/
        }

        // all PERA charged for storage should flow into the storage rebate field of some output object
        if gas_summary.storage_cost != total_output_rebate {
            // TODO: re-enable once we fix the edge case with OOG, gas smashing, and storage rebate
            /*return Err(ExecutionError::invariant_violation(
                format!("PERA conservation failed--{} PERA charged for storage, {} PERA in storage rebate field of output objects",
...
impl<'backing> Storage for TemporaryStore<'backing> {
    ...
    fn record_execution_results(&mut self, results: ExecutionResults) {
        let ExecutionResults::V1(results) = results else {
            panic!("ExecutionResults::V1 expected in pera-execution v0");
        };
    ...
    fn check_coin_deny_list(
        ...
        unreachable!("Coin denylist v2 is not supported in pera-execution v0");
    }
}
