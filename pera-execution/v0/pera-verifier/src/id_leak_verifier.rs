...
//! Objects whose struct type has key ability represent Pera objects.
//! They have unique IDs that should never be reused. This verifier makes
//! sure that the id field of Pera objects never get leaked.
//! Unpack is the only bytecode that could extract the id field out of
//! a Pera object. From there, we track the flow of the value and make
...
const PERA_SYSTEM_CREATE: FunctionIdent = (
    &PERA_SYSTEM_ADDRESS,
    PERA_SYSTEM_MODULE_NAME,
    ident_str!("create"),
);
const PERA_CLOCK_CREATE: FunctionIdent = (
    &PERA_FRAMEWORK_ADDRESS,
    CLOCK_MODULE_NAME,
    ident_str!("create"),
);
...
#[cfg(msim)]
const PERA_AUTHENTICATOR_STATE_CREATE: FunctionIdent = (
    &PERA_FRAMEWORK_ADDRESS,
    AUTHENTICATOR_STATE_MODULE_NAME,
    ident_str!("create"),
);
#[cfg(msim)]
const PERA_RANDOMNESS_STATE_CREATE: FunctionIdent = (
    &PERA_FRAMEWORK_ADDRESS,
    RANDOMNESS_MODULE_NAME,
    ident_str!("create"),
);
#[cfg(msim)]
const PERA_DENY_LIST_OBJECT_CREATE: FunctionIdent = (
    &PERA_FRAMEWORK_ADDRESS,
    COIN_MODULE_NAME,
...
#[cfg(not(msim))]
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[PERA_SYSTEM_CREATE, PERA_CLOCK_CREATE];

#[cfg(msim)]
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[
    PERA_SYSTEM_CREATE,
    PERA_CLOCK_CREATE,
    PERA_AUTHENTICATOR_STATE_CREATE,
    PERA_RANDOMNESS_STATE_CREATE,
    PERA_DENY_LIST_OBJECT_CREATE,
];
    ...
    if handle.abilities.has_key() && last_value != AbstractValue::Fresh {
        ...
        let msg = format!(
            "Invalid object creation in {cur_package}::{cur_module}::{cur_function}. \
                ...
                The UID must come directly from pera::{}::{}. \
                Or for tests, it can come from pera::{}::{}",
