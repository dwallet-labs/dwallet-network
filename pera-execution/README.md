...
The `pera-execution` crate is responsible for abstracting access to the
...
- The adapter, that integrates Move into Pera.
...
- `./pera-execution`, (latest version and cuts/copies of pera-specific
  crates).
...
code must be via `pera-execution`, and not by directly depending a
constituent crate.
...
and elsewhere, **default to accessing execution via `pera-execution`.**
...
`pera-execution tests::test_encapsulation` is a test that detects
...
`./crates/pera-types` are shared across all versions.  Changes to
...
## `pera-execution/src/lib.rs`

The entry-point to the execution crate -- `pera-execution/src/lib.rs`
...
- `pera-execution/src/lib.template.rs` -- a template file with
