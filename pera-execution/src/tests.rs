...
#[test]
/// Make sure that all accesses to execution layer crates in the `pera-node` and `pera-replay` crates
/// go via the `pera-execution` crate (in other words, the `pera-execution` crate dominates execution
/// layer crates in the dependency graphs of `pera-node` and `pera-replay`).
///
...
fn test_encapsulation() {
    ...
    let mut exec_crates: BTreeSet<_> = packages.normal_deps("pera-execution").collect();

    // Remove the crates that the execution layer depends on but which are not directly part of the
    // execution layer -- these don't need to be accessed exclusively via `pera-execution`.
    exec_crates.remove("pera-protocol-config");
    exec_crates.remove("pera-types");
    ...
    for root in ["pera-node", "pera-replay"] {
        let mut graph = packages.graph(root);

        // If we can still create a path from `root` to an execution crate after removing these
        // nodes then we know that we can potential bypass "pera-execution".
        graph.remove_node("pera-execution");
    ...
    panic!(
        "protocol-sensitive binaries depend on execution crates outside of 'pera-execution', e.g.:\n\
         \n  {}\n\
         \n\
         This can cause execution to fork by not respecting the execution layer version set in the \
         protocol config.  Fix this by depending on these crates via 'pera-execution'.\n\
         \n\
         P.S. if you believe one of these crates should not be part of 'pera-execution' then update \
         the test to exclude this crate.",
        examples.join("\n  "),
    );
}

/// Parse `cargo metadata` for the `pera` repo.
fn cargo_metadata() -> cargo_metadata::Result<Metadata> {
    let pera_execution = PathBuf::from(env!("CARGO_MANIFEST_DIR"));

    MetadataCommand::new()
        .manifest_path(pera_execution.join("../Cargo.toml"))
        .current_dir(pera_execution.join(".."))
        .no_deps()
        .exec()
}
