...
fun test_bridge_and_claim() {
    ...
    //
    // move from eth and transfer to pera account
    let transfer_id1 = env.bridge_to_pera<ETH>(
        source_chain,
        eth_address,
        pera_address,
    ...
    let transfer_id = env.bridge_to_pera<ETH>(
        source_chain,
        eth_address,
        pera_address,
        amount,
    );
    let token = env.claim_token<ETH>(pera_address, source_chain, transfer_id);
    env.send_token<ETH>(
        pera_address,
        source_chain,
        eth_address,
