...
module bridge::bridge_env {
    ...
    use bridge::message::{
        Self,
        BridgeMessage,
        create_add_tokens_on_pera_message,
    ...
    use pera::coin::{Self, Coin, CoinMetadata, TreasuryCap};
    use pera::ecdsa_k1::{KeyPair, secp256k1_keypair_from_seed, secp256k1_sign};
    use pera::event;
    use pera::package::UpgradeCap;
    use pera::test_scenario::{Self, Scenario};
    use pera::test_utils::destroy;
    ...
    public fun create_env(chain_id: u8): BridgeEnv {
        ...
        let mut clock = pera::clock::create_for_testing(ctx);
    ...
    public fun register_committee(env: &mut BridgeEnv) {
        ...
        let mut system_state = test_scenario::take_shared<PeraSystemState>(
            scenario,
        );
    ...
    fun add_default_tokens(env: &mut BridgeEnv, sender: address) {
        ...
        let add_token_message = create_add_tokens_on_pera_message(
            env.chain_id,
            bridge.get_seq_num_for(message_types::add_tokens_on_pera()),
            false,
    ...
    const PERA_MESSAGE_PREFIX: vector<u8> = b"PERA_BRIDGE_MESSAGE";

    fun sign_message(
        env: &BridgeEnv,
        message: BridgeMessage,
    ): vector<vector<u8>> {
        let mut message_bytes = PERA_MESSAGE_PREFIX;
    ...
    public fun bridge_to_pera<Token>(
        env: &mut BridgeEnv,
        source_chain: u8,
        source_address: vector<u8>,
        target_address: address,
...
module bridge::test_token {
    ...
    use std::type_name;
    use pera::address;
    use pera::coin::{CoinMetadata, TreasuryCap, create_currency};
    use pera::hex;
    use pera::package::{UpgradeCap, test_publish};
    use pera::test_utils::create_one_time_witness;
