...
module bridge::bridge {
    ...
    use pera::coin::{Coin, TreasuryCap, CoinMetadata};
    use pera::event::emit;
    use pera::linked_table::{Self, LinkedTable};
    use pera::package::UpgradeCap;
    use pera::vec_map::{Self, VecMap};
    use pera::versioned::{Self, Versioned};
    ...
    use bridge::message::{
        ...
        UpdateBridgeLimit, AddTokenOnPera, ParsedTokenTransferMessage,
    ...
    const EInvariantPeraInitializedTokenTransferShouldNotBeClaimed: u64 = 10;
    ...
    // Record bridge message approvals in Pera, called by the bridge client
    ...
    public fun approve_token_transfer(
        ...
        // retrieve pending message if source chain is Pera, the initial message
        // must exist on chain
        if (message.source_chain() == inner.chain_id) {
            ...
            assert!(!record.claimed, EInvariantPeraInitializedTokenTransferShouldNotBeClaimed);
    ...
    public fun execute_system_message(
        ...
        } else if (message_type == message_types::add_tokens_on_pera()) {
            let payload = message.extract_add_tokens_on_pera();
            inner.execute_add_tokens_on_pera(payload);
        } else {
    ...
    fun execute_add_tokens_on_pera(inner: &mut BridgeInner, payload: AddTokenOnPera) {
    ...
    #[test_only]
    public fun test_execute_add_tokens_on_pera(bridge: &mut Bridge, payload: AddTokenOnPera) {
        let inner = load_inner_mut(bridge);
        inner.execute_add_tokens_on_pera(payload);
    }
