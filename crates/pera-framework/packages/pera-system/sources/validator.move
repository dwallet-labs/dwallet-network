...
module pera_system::validator {
    ...
    use pera::pera::PERA;
    use pera_system::validator_cap::{Self, ValidatorOperationCap};
    use pera_system::staking_pool::{Self, PoolTokenExchangeRate, StakedPera, StakingPool};
    ...
    public(package) fun request_add_stake(
        ...
        let staked_pera = self.staking_pool.request_add_stake(stake, stake_epoch, ctx);
    ...
    public(package) fun request_add_stake_at_genesis(
        ...
        let staked_pera = self.staking_pool.request_add_stake(
            stake,
            0, // epoch 0 -- genesis
            ctx
        );

        transfer::public_transfer(staked_pera, staker_address);
    ...
    public(package) fun request_withdraw_stake(
        ...
        let principal_amount = staked_pera.staked_pera_amount();
        let stake_activation_epoch = staked_pera.stake_activation_epoch();
        let withdrawn_stake = self.staking_pool.request_withdraw_stake(staked_pera, ctx);
    ...
    public(package) fun request_set_gas_price(
        ...
        assert!(validator_address == self.metadata.pera_address, EInvalidCap);
    ...
    public(package) fun deposit_stake_rewards(self: &mut Validator, reward: Balance<PERA>) {
    ...
    public fun pera_address(self: &Validator): address {
    ...
    public fun total_stake_amount(self: &Validator): u64 {
        self.staking_pool.pera_balance()
    }
    ...
    public(package) fun new_unverified_validator_operation_cap_and_transfer(self: &mut Validator, ctx: &mut TxContext) {
        ...
        assert!(address == self.metadata.pera_address, ENewCapNotCreatedByValidatorItself);
    ...
    fun new_from_metadata(
        ...
        let pera_address = metadata.pera_address;

        let staking_pool = staking_pool::new(ctx);

        let operation_cap_id = validator_cap::new_unverified_validator_operation_cap_and_transfer(pera_address, ctx);
        Validator {
            metadata,
    ...
    // Note: `proof_of_possession` MUST be a valid signature using pera_address and
    ...
    public(package) fun new_for_testing(
        ...
        primary_address: vector<u8>,
        worker_address: vector<u8>,
        mut initial_stake_option: Option<Balance<PERA>>,
        gas_price: u64,
        commission_rate: u64,
        ...
        if (initial_stake_option.is_some()) {
            request_add_stake_at_genesis(
                ...
                pera_address, // give the stake to the validator
                ctx
            );
        };
