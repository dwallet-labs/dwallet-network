...
module pera_system::pera_system_state_inner {
    ...
    use pera::pera::PERA;
    use pera_system::validator::{Self, Validator};
    use pera_system::validator_set::{Self, ValidatorSet};
    use pera_system::validator_cap::{UnverifiedValidatorOperationCap, ValidatorOperationCap};
    use pera_system::stake_subsidy::StakeSubsidy;
    use pera_system::storage_fund::{Self, StorageFund};
    ...
    use pera::bag::Bag;
    use pera::bag;
    use pera_system::dwallet_network_key::{KeyType, EncryptedNetwotkDecryptionKeyShare,
    ...
    public struct PeraSystemStateInner has store {
        ...
        safe_mode: bool,
        safe_mode_storage_rewards: Balance<PERA>,
        safe_mode_computation_rewards: Balance<PERA>,
    ...
    public struct PeraSystemStateInnerV2 has store {
        /// The current epoch ID, starting from 0.
        epoch: u64,
        /// The current protocol version, starting from 1.
        protocol_version: u64,
        ...
        encrypted_decryption_key_shares: VecMap<KeyType, vector<EncryptedNetwotkDecryptionKeyShare>>,
        /// Contains all information about the validators.
        validators: ValidatorSet,
        /// The storage fund.
        storage_fund: StorageFund,
    ...
    public(package) fun v1_to_v2(self: PeraSystemStateInner): PeraSystemStateInnerV2 {
        let PeraSystemStateInner {
            epoch,
            protocol_version,
    ...
    public(package) fun request_withdraw_stake(
        ...
        self.validators.request_withdraw_stake(staked_pera, ctx)
    }
    ...
    public(package) fun report_validator(
        ...
        assert!(self.validators.is_active_validator_by_pera_address(reportee_addr), ENotValidator);
    ...
    public(package) fun advance_epoch(
        ...
        mut storage_reward: Balance<PERA>,
        mut computation_reward: Balance<PERA>,
    ...
    public(package) fun epoch(self: &PeraSystemStateInnerV2): u64 {
        self.epoch
    }

    public(package) fun protocol_version(self: &PeraSystemStateInnerV2): u64 {
        self.protocol_version
    }

    public(package) fun system_state_version(self: &PeraSystemStateInnerV2): u64 {
    ...
    public(package) fun epoch_start_timestamp_ms(self: &PeraSystemStateInnerV2): u64 {
        self.epoch_start_timestamp_ms
    }

    /// Returns the total amount staked with `validator_addr`.
    /// Aborts if `validator_addr` is not an active validator.
    public(package) fun validator_stake_amount(self: &PeraSystemStateInnerV2, validator_addr: address): u64 {
        self.validators.validator_total_stake_amount(validator_addr)
    }

    /// Returns the voting power for `validator_addr`.
    /// Aborts if `validator_addr` is not an active validator.
    public(package) fun active_validator_voting_powers(self: &PeraSystemStateInnerV2): VecMap<address, u64> {
    ...
    /// Aborts if `validator_addr` is not an active validator.
    public(package) fun validator_staking_pool_id(self: &PeraSystemStateInnerV2, validator_addr: address): ID {

        self.validators.validator_staking_pool_id(validator_addr)
    }

    /// Returns reference to the staking pool mappings that map pool ids to active validator addresses
    public(package) fun validator_staking_pool_mappings(self: &PeraSystemStateInnerV2): &Table<ID, address> {

        self.validators.staking_pool_mappings()
    }

    /// Returns all the validators who are currently reporting `addr`
    public(package) fun get_reporters_of(self: &PeraSystemStateInnerV2, addr: address): VecSet<address> {
    ...
    public(package) fun get_storage_fund_total_balance(self: &PeraSystemStateInnerV2): u64 {
        self.storage_fund.total_balance()
    }

    public(package) fun get_storage_fund_object_rebates(self: &PeraSystemStateInnerV2): u64 {
    ...
    fun extract_coin_balance(mut coins: vector<Coin<PERA>>, amount: option::Option<u64>, ctx: &mut TxContext): Balance<PERA> {
    ...
    public(package) fun validators(self: &PeraSystemStateInnerV2): &ValidatorSet {
        &self.validators
    }

    #[test_only]
    /// Return the currently active validator by address
    public(package) fun active_validator_by_address(self: &PeraSystemStateInnerV2, validator_address: address): &Validator {
        self.validators().get_active_validator_ref(validator_address)
    }

    #[test_only]
    /// Return the currently pending validator by address
    public(package) fun pending_validator_by_address(self: &PeraSystemStateInnerV2, validator_address: address): &Validator {
        self.validators().get_pending_validator_ref(validator_address)
    }

    #[test_only]
    /// Return the currently candidate validator by address
    public(package) fun candidate_validator_by_address(self: &PeraSystemStateInnerV2, validator_address: address): &Validator {
        validators(self).get_candidate_validator_ref(validator_address)
    }

    #[test_only]
    public(package) fun get_stake_subsidy_distribution_counter(self: &PeraSystemStateInnerV2): u64 {
        self.stake_subsidy.get_distribution_counter()
    }

    #[test_only]
    public(package) fun set_epoch_for_testing(self: &mut PeraSystemStateInnerV2, epoch_num: u64) {
