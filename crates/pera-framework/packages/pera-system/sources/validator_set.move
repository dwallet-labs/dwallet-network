...
module pera_system::validator_set {
    ...
    use pera::event;
    use pera::table_vec::{Self, TableVec};
    use pera_system::voting_power;
    ...
    public struct ValidatorSet has store {
        ...
        /// changes in the [`pending_removals`] and [`pending_active_validators`] until the next epoch starts, false otherwise.
        locked: bool
    }
    ...
    public struct ValidatorDataForDWalletSecretShare has copy, drop, store {
    ...
    public(package) fun new(init_active_validators: vector<Validator>, ctx: &mut TxContext): ValidatorSet {
        ...
        while (i < num_validators) {
            ...
            staking_pool_mappings.add(staking_pool_id(validator), pera_address(validator));
            i = i + 1;
        };
    ...
    public(package) fun request_add_validator_candidate(
        ...
        let validator_address = pera_address(&validator);
        assert!(
        ...
        self.validator_candidates.add(
            pera_address(&validator),
            validator_wrapper::create_v1(validator, ctx),
        );
    }

    /// Called by `pera_system` to remove a validator candidate, and move them to `inactive_validators`.
    ...
    /// Called by `pera_system`, to remove a validator.
    ...
    /// Called by `pera_system`, to add a new stake to the validator.
    ...
    public(package) fun request_add_stake(
        ...
        assert!(pera_amount >= MIN_STAKING_THRESHOLD, EStakingBelowThreshold);
    ...
    /// Called by `pera_system`, to withdraw some share of a stake from the validator. The share to withdraw
    /// is denoted by `principal_withdraw_amount`. One of two things occurs in this function:
    /// 1. If the `staked_pera` is staked with an active validator, the request is added to the validator's
    ///    staking pool's pending stake withdraw entries, processed at the end of the epoch.
    /// 2. If the `staked_pera` was staked with a validator that is no longer active,
    ...
    public(package) fun request_withdraw_stake(
        ...
        let staking_pool_id = pool_id(&staked_pera);
        let validator =
            if (self.staking_pool_mappings.contains(staking_pool_id)) { // This is an active validator.
                let validator_address = self.staking_pool_mappings[pool_id(&staked_pera)];
        ...
        validator.request_withdraw_stake(staked_pera, ctx)
    }
    ...
    public(package) fun lock_next_epoch_committee(self: &mut ValidatorSet, epoch: u64) {
        ...
        event::emit(LockedNextEpochCommitteeEvent { epoch });
        self.locked = true;
    }
    ...
    /// Maps the given `Validator` objects to the corresponding `ValidatorDataForDWalletSecretShare` objects.
    public(package) fun get_active_validators_data(
    ...
    public(package) fun advance_epoch(
        self: &mut ValidatorSet,
        computation_reward: &mut Balance<PERA>,
        storage_fund_reward: &mut Balance<PERA>,
    ...
    fun update_and_process_low_stake_departures(
        ...
        while (i > 0) {
            ...
            let validator_address = validator_ref.pera_address();
    ...
    /// Called by `pera_system` to derive reference gas price for the new epoch.
    ...
    public(package) fun is_active_validator_by_pera_address(
        self: &ValidatorSet,
        validator_address: address,
    ): bool {
    ...
    /// It differs from `is_active_validator_by_pera_address` in that the former checks
    ...
    fun find_validator(validators: &vector<Validator>, validator_address: address): Option<u64> {
        ...
        while (i < length) {
            let v = &validators[i];
            if (v.pera_address() == validator_address) {
    ...
    fun process_validator_departure(
        ...
        let new_epoch = ctx.epoch() + 1;
        let validator_address = validator.pera_address();
    ...
    fun process_pending_validators(
        ...
        while (!self.pending_active_validators.is_empty()) {
            ...
            event::emit(
                ValidatorJoinEvent {
                    epoch: new_epoch,
                    validator_address: validator.pera_address(),
                    staking_pool_id: staking_pool_id(&validator),
                }
            );
    ...
    fun compute_slashed_validators(
        ...
        while (!validator_report_records.is_empty()) {
            ...
            assert!(
                is_active_validator_by_pera_address(self, validator_address),
    ...
    fun distribute_reward(
        ...
        staking_rewards: &mut Balance<PERA>,
        storage_fund_reward: &mut Balance<PERA>,
        ctx: &mut TxContext
    ) {
        let length = validators.length();
    ...
    fun emit_validator_epoch_events(
        ...
        while (i < num_validators) {
            let v = &vs[i];
            let validator_address = v.pera_address();
    ...
    public(package) fun active_validator_addresses(self: &ValidatorSet): vector<address> {
        ...
        while (i < length) {
            let validator_address = vs[i].pera_address();
