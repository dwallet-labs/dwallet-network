...
#[test_only]
module pera_system::governance_test_utils {
    use pera::address;
    use pera::balance;
    use pera::pera::PERA;
    use pera::coin::{Self, Coin};
    use pera_system::staking_pool::{StakedPera, StakingPool};
    use pera::test_utils::assert_eq;
    use pera_system::validator::{Self, Validator};
    use pera_system::pera_system::{Self, PeraSystemState};
    use pera_system::pera_system_state_inner;
    use pera_system::stake_subsidy;
    ...
    public fun create_validator_for_testing(
        addr: address, init_stake_amount_in_pera: u64, ctx: &mut TxContext
    ): Validator {
        let validator = validator::new_for_testing(
            ...
            b"/ip4/127.0.0.1/udp/80",
            b"/ip4/127.0.0.1/udp/80",
            option::some(balance::create_for_testing<PERA>(init_stake_amount_in_pera * NPERA_PER_PERA)),
            1,
            0,
            true,
            ctx
    ...
    public fun create_pera_system_state_for_testing(
        validators: vector<Validator>, pera_supply_amount: u64, storage_fund_amount: u64, ctx: &mut TxContext
    ) {
        let system_parameters = pera_system_state_inner::create_system_parameters(
        ...
        let stake_subsidy = stake_subsidy::create(
            balance::create_for_testing<PERA>(pera_supply_amount * NPERA_PER_PERA), // pera_supply
        ...
        pera_system::create(
            object::new(ctx), // it doesn't matter what ID pera system state has in tests
            validators,
            balance::create_for_testing<PERA>(storage_fund_amount * NPERA_PER_PERA), // storage_fund
            1,   // protocol version
    ...
    public fun set_up_pera_system_state(mut addrs: vector<address>) {
    ...
    public fun advance_epoch_with_reward_amounts(
        ...
        let storage_rebate = advance_epoch_with_reward_amounts_return_rebate(storage_charge * NPERA_PER_PERA, computation_charge * NPERA_PER_PERA, 0, 0, scenario);
    ...
    public fun advance_epoch_with_reward_amounts_and_slashing_rates(
        ...
        let storage_rebate = system_state.advance_epoch_for_testing(
            new_epoch, 1, storage_charge * NPERA_PER_PERA, computation_charge * NPERA_PER_PERA, 0, 0, 0, reward_slashing_rate, 0, ctx
        );
        test_utils::destroy(storage_rebate);
        test_scenario::return_shared(system_state);
        scenario.next_epoch(@0x0);
    ...
    public fun stake_with(
        ...
        system_state.request_add_stake(coin::mint_for_testing(amount * NPERA_PER_PERA, ctx), validator, ctx);
        test_scenario::return_shared(system_state);
    }

    public fun unstake(
        staker: address, staked_pera_idx: u64, scenario: &mut Scenario
    ) {
        scenario.next_tx(staker);
        let stake_pera_ids = scenario.ids_for_sender<StakedPera>();
        let staked_pera = scenario.take_from_sender_by_id(stake_pera_ids[staked_pera_idx]);
        let mut system_state = scenario.take_shared<PeraSystemState>();

        let ctx = scenario.ctx();
        system_state.request_withdraw_stake(staked_pera, ctx);
        test_scenario::return_shared(system_state);
    }
    ...
    public fun add_validator_full_flow(validator: address, name: vector<u8>, net_addr: vector<u8>, init_stake_amount: u64, pubkey: vector<u8>, pop: vector<u8>, scenario: &mut Scenario) {
        ...
        system_state.request_add_stake(coin::mint_for_testing<PERA>(init_stake_amount * NPERA_PER_PERA, ctx), validator, ctx);
    ...
    /// Return the rewards for the validator at `addr` in terms of PERA.
    public fun stake_plus_current_rewards_for_validator(addr: address, system_state: &mut PeraSystemState, scenario: &mut Scenario): u64 {
    ...
    public fun stake_plus_current_rewards(addr: address, staking_pool: &StakingPool, scenario: &mut Scenario): u64 {
        ...
        let mut stake_ids = scenario.ids_for_sender<StakedPera>();
        let current_epoch = scenario.ctx().epoch();

        while (!stake_ids.is_empty()) {
            let staked_pera_id = stake_ids.pop_back();
            let staked_pera = scenario.take_from_sender_by_id<StakedPera>(staked_pera_id);
            sum = sum + staking_pool.calculate_rewards(&staked_pera, current_epoch);
            scenario.return_to_sender(staked_pera);
        };
        sum
    }

    public fun total_pera_balance(addr: address, scenario: &mut Scenario): u64 {
        let mut sum = 0;
        scenario.next_tx(addr);
        let coin_ids = scenario.ids_for_sender<Coin<PERA>>();
        let mut i = 0;
        while (i < coin_ids.length()) {
            let coin = scenario.take_from_sender_by_id<Coin<PERA>>(coin_ids[i]);
            sum = sum + coin.value();
