...
// This file contains tests testing functionalities in `pera_system` that are not
// already tested by the other more themed tests such as `stake_tests` or
// `rewards_distribution_tests`.

#[test_only]
module pera_system::pera_system_tests {
    use pera::test_scenario::{Self, Scenario};
    use pera::pera::PERA;
    use pera_system::governance_test_utils::{add_validator_full_flow, advance_epoch, remove_validator, set_up_pera_system_state, create_pera_system_state_for_testing, stake_with, unstake};
    ...
    use pera::balance;
    use pera::test_utils::{assert_eq, destroy};
    use pera::url;
    ...
    fun test_validator_ops_by_stakee_ok() {
        let mut scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        set_up_pera_system_state(vector[@0x1, @0x2]);
    ...
    #[test]
    #[expected_failure(abort_code = ::pera_system::validator_set::EInvalidCap)]
    ...
    #[test]
    #[expected_failure(abort_code = pera_system_state_inner::ENotValidator)]
    ...
    #[test]
    #[expected_failure(abort_code = pera_system_state_inner::ECannotReportOneself)]
    ...
    #[test]
    #[expected_failure(abort_code = pera_system_state_inner::EReportRecordNotFound)]
    ...
    fun test_staking_pool_mappings() {
        ...
        set_up_pera_system_state(vector[@0x1, @0x2, @0x3, @0x4]);
        scenario.next_tx(@0x1);
        let mut system_state = scenario.take_shared<PeraSystemState>();
    ...
    fun update_candidate(
        scenario: &mut Scenario,
        system_state: &mut PeraSystemState,
        name: vector<u8>,
    ...
    // Note: `pop` MUST be a valid signature using pera_address and protocol_pubkey_bytes.
    ...
    fun test_active_validator_update_metadata() {
        ...
        // Set up PeraSystemState with an active validator
        ...
        let validator = validator::new_for_testing(
            ...
            b"/ip4/127.0.0.1/udp/80",
            b"/ip4/127.0.0.1/udp/80",
            option::some(balance::create_for_testing<PERA>(100_000_000_000)),
            1,
            0,
            true,
            ctx
    ...
    fun test_add_validator_candidate_failure_duplicate_with_active() {
        ...
        create_pera_system_state_for_testing(vector[validator], 1000, 0, ctx);
    ...
    fun test_withdraw_inactive_stake() {
...

        {
            ...
            assert!(staking_pool.pera_balance() == 100 * 1_000_000_000, 0);
