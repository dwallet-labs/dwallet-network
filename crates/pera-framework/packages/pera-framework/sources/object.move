...
module pera::object {
    ...
    const PERA_SYSTEM_STATE_OBJECT_ID: address = @0x5;

    /// The hardcoded ID for the singleton Clock Object.
    const PERA_CLOCK_OBJECT_ID: address = @0x6;

    /// The hardcoded ID for the singleton AuthenticatorState Object.
    const PERA_AUTHENTICATOR_STATE_ID: address = @0x7;

    /// The hardcoded ID for the singleton Random Object.
    const PERA_RANDOM_ID: address = @0x8;

    /// The hardcoded ID for the singleton DenyList.
    const PERA_DENY_LIST_OBJECT_ID: address = @0x403;

    /// The hardcoded ID for the Bridge Object.
    const PERA_BRIDGE_ID: address = @0x9;
    ...
    /// Create the `UID` for the singleton `PeraSystemState` object.
    /// This should only be called once from `pera_system`.
    fun pera_system_state(ctx: &TxContext): UID {
        assert!(ctx.sender() == @0x0, ENotSystemAddress);
        UID {
            id: ID { bytes: PERA_SYSTEM_STATE_OBJECT_ID },
        }
    }
    ...
    public(package) fun clock(): UID {
        UID {
            id: ID { bytes: PERA_CLOCK_OBJECT_ID }
        }
    }
    ...
    public(package) fun authenticator_state(): UID {
        UID {
            id: ID { bytes: PERA_AUTHENTICATOR_STATE_ID }
        }
    }
    ...
    public(package) fun randomness_state(): UID {
        UID {
            id: ID { bytes: PERA_RANDOM_ID }
        }
    }

    /// Create the `UID` for the singleton `DenyList` object.
    /// This should only be called once from `deny_list`.
    public(package) fun pera_deny_list_object_id(): UID {
        UID {
            id: ID { bytes: PERA_DENY_LIST_OBJECT_ID }
        }
    }
    ...
    /// Create a new object. Returns the `UID` that must be stored in a Pera object.
    ...
    // This exists to inform Pera of object deletions. When an object
    // gets unpacked, the programmer will have to do something with its
    // `UID`. The implementation of this function emits a deleted
    // system event so Pera knows to process the object deletion
