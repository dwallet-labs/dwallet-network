...
/// of the type at once.
module pera::transfer_policy {
    use std::type_name::{Self, TypeName};
    use pera::package::{Self, Publisher};
    ...
    public struct TransferPolicy<phantom T> has key, store {
        id: UID,
        /// The Balance of the `TransferPolicy` which collects `PERA`.
    ...
    /// Add some `PERA` to the balance of a `TransferPolicy`.
    public fun add_to_balance<T, Rule: drop>(
        _: Rule, policy: &mut TransferPolicy<T>, coin: Coin<PERA>
    ) {
