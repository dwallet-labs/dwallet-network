...
/// `list_with_purchase_cap` therefore performing a trade (isperang a
...
module pera::kiosk {
    ...
    entry fun default(ctx: &mut TxContext) {
        ...
        pera::transfer::share_object(kiosk);
    }
    ...
    public fun purchase<T: key + store>(
        self: &mut Kiosk, id: ID, payment: Coin<PERA>
    ): (T, TransferRequest<T>) {
    ...
    public fun purchase_with_cap<T: key + store>(
        self: &mut Kiosk, purchase_cap: PurchaseCap<T>, payment: Coin<PERA>
    ): (T, TransferRequest<T>) {
    ...
    public fun profits_mut(self: &mut Kiosk, cap: &KioskOwnerCap): &mut Balance<PERA> {
        assert!(self.has_access(cap), ENotOwner);
        &mut self.profits
    }
