...
/// Similar to `pera::table`, an `ObjectTable<K, V>` is a map-like collection. But unlike
/// `pera::table`, the values bound to these dynamic fields _must_ be objects themselves. This allows
...
module pera::object_table {
    ...
    /// Aborts with `pera::dynamic_field::EFieldAlreadyExists` if the table already has an entry with
    /// that key `k: K`.
    ...
    /// Aborts with `pera::dynamic_field::EFieldDoesNotExist` if the table does not have an entry with
    /// that key `k: K`.
