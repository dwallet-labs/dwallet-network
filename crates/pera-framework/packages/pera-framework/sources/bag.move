...
/// A bag is a heterogeneous map-like collection. The collection is similar to `pera::table` in that
/// its keys and values are not stored within the `Bag` value, but instead are stored using Pera's
...
/// assert!(&bag1 != &bag2);
/// ```
/// At it's core, `pera::bag` is a wrapper around `UID` that allows for access to
/// `pera::dynamic_field` while preventing accidentally stranding field values. A `UID` can be
/// deleted, even if it has dynamic fields associated with it, but a bag, on the other hand, must be
/// empty to be destroyed.
module pera::bag {
    use pera::dynamic_field as field;
    ...
    /// Aborts with `pera::dynamic_field::EFieldAlreadyExists` if the bag already has an entry with
    /// that key `k: K`.
    ...
    /// Aborts with `pera::dynamic_field::EFieldDoesNotExist` if the bag does not have an entry with
    /// that key `k: K`.
    /// Aborts with `pera::dynamic_field::EFieldTypeMismatch` if the bag has an entry for the key, but
