...
#[test_only]
module pera::kiosk_marketplace_ext {
    use pera::pera::PERA;
    use pera::coin::Coin;
    use pera::kiosk_extension as ext;
    use pera::kiosk::{Self, KioskOwnerCap, Kiosk, PurchaseCap};
    use pera::transfer_policy::{Self as policy, TransferPolicy, TransferRequest};

    /// Trying to access an owner-only action.
    const ENotOwner: u64 = 0;
    /// Trying to purchase an item with an incorrect amount of PERA.
    ...
    /// Collection bidding: the Kiosk Owner offers a bid (in PERA) for an item of type `T`.
    ///
    /// There can be only one bid per type.
    public fun bid<Market, T: key + store>(
        kiosk: &mut Kiosk, cap: &KioskOwnerCap, bid: Coin<PERA>
    ) {
    ...
    public fun accept_bid<Market, T: key + store>(
        ...
        let bid: Coin<PERA> = ext::storage_mut(Ext<Market> {}, destination).remove(Bid<T> {});
...
#[test_only]
module pera::kiosk_extensions_tests {
    use pera::kiosk_test_utils::{Self as test};
    use pera::kiosk_extension as ext;
    ...
    #[test, expected_failure(abort_code = pera::kiosk_extension::EExtensionNotAllowed)]
    ...
    #[test, expected_failure(abort_code = pera::kiosk_extension::EExtensionNotInstalled)]
