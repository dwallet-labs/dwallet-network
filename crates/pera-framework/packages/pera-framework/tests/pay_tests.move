...
module pera::pay_tests {
    ...
    fun test_coin_split_n() {
        ...
        let mut coin = coin::mint_for_testing<PERA>(10, ctx);

        scenario.next_tx(TEST_SENDER_ADDR);
        coin.divide_and_keep(3, scenario.ctx());

        scenario.next_tx(TEST_SENDER_ADDR);
        let coin1 = scenario.take_from_sender<Coin<PERA>>();

        scenario.next_tx(TEST_SENDER_ADDR);
        let coin2 = scenario.take_from_sender<Coin<PERA>>();
        ...
        assert!(coin.value() == 4);
        assert!(
            !scenario.has_most_recent_for_sender<Coin<PERA>>(),
            1
        );
    ...
    fun test_split_and_transfer_fail() {
        ...
        scenario.next_tx(TEST_SENDER_ADDR);
        let coin_transfer_fail = scenario.take_from_sender<Coin<PERA>>();
