...
#[test_only]
module pera::test_scenario_tests {
    ...
    fun test_dynamic_field_still_borrowed() {
        ...
        pera::dynamic_field::add(&mut parent, b"", 10);
        let r = pera::dynamic_field::borrow<vector<u8>, u64>(&parent, b"");
        scenario.end();
        assert!(*r == 10);
        parent.delete();
    }
    ...
    fun test_dynamic_object_field_still_borrowed() {
        ...
        let mut parent = scenario.new_object();
        let id = scenario.new_object();
        pera::dynamic_object_field::add(&mut parent, b"", Object { id, value: 10});
        let obj = pera::dynamic_object_field::borrow<vector<u8>, Object>(&parent, b"");
        scenario.end();
        assert!(obj.value == 10);
        parent.delete();
    }
    ...
    fun test_dynamic_object_field_not_retrievable() {
        ...
        assert!(!test_scenario::has_most_recent_for_address<Object>(sender));
        pera::dynamic_object_field::add(&mut parent, b"", obj);
        scenario.next_tx(sender);
        assert!(!test_scenario::has_most_recent_for_address<Object>(sender));
        scenario.end();
        parent.delete();
    ...
    fun test_dynamic_field_shared_misuse() {
        ...
        assert!(object::id(&obj) == id);
        // wraps the object
        pera::dynamic_field::add(&mut parent, b"", obj);
        scenario.next_tx(sender);
        abort 42
    }
