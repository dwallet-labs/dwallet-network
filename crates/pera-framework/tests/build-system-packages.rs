...
fn build_system_packages() {
    ...
    let pera_system_path = packages_path.join("pera-system");
    let pera_framework_path = packages_path.join("pera-framework");
...
fn check_diff(checked_in: &Path, built: &Path) {
    for path in [COMPILED_PACKAGES_DIR, DOCS_DIR, PUBLISHED_API_FILE] {
        ...
        if !output.status.success() {
            let header =
                "Generated and checked-in pera-framework packages and/or docs do not match.\n\
...
fn build_packages(
    bridge_path: &Path,
    deepbook_path: &Path,
    pera_system_path: &Path,
    pera_framework_path: &Path,
    stdlib_path: &Path,
    out_dir: &Path,
) {
...
fn build_packages_with_move_config(
    ...
    let pera_system = system_pkg.get_pera_system_modules();
    let pera_framework = framework_pkg.get_pera_framework_modules();
    ...
    let pera_system_members =
        serialize_modules_to_file(pera_system, &compiled_packages_dir.join(system_dir)).unwrap();
    let pera_framework_members =
        serialize_modules_to_file(pera_framework, &compiled_packages_dir.join(framework_dir))
            .unwrap();
    ...
    let published_api = [
        pera_system_members.join("\n"),
        pera_framework_members.join("\n"),
