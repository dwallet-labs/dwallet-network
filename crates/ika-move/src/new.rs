...
const IKA_PKG_NAME: &str = "Ika";

// Use testnet by default. Probably want to add options to make this configurable later
const IKA_PKG_PATH: &str = "{ git = \"https://github.com/MystenLabs/sui.git\", subdir = \"crates/ika-framework/packages/ika-framework\", rev = \"framework/testnet\" }";

#[derive(Parser)]
#[group(id = "ika-move-new")]
pub struct New {
    #[clap(flatten)]
    pub new: new::New,
}
...
impl New {
    pub fn execute(self, path: Option<&Path>) -> anyhow::Result<()> {
        ...
        self.new
            .execute(path, [(IKA_PKG_NAME, IKA_PKG_PATH)], [(name, "0x0")], "")?;
