...
use ika_move_build::decorate_warnings;
use ika_move_natives::test_scenario::InMemoryTestStore;
    ...
    pub fn execute(
        ...
        if !cfg!(debug_assertions) && compute_coverage {
            return Err(anyhow::anyhow!(
                "The --coverage flag is currently supported only in debug builds. Please build the Ika CLI from source in debug mode."
            ));
        }
