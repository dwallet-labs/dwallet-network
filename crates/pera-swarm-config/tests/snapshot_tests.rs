...
// This file contains tests that detect changes in Pera configs.
...
// 1. Run `cargo insta test --review` under `./pera-config`.
...
fn populated_genesis_snapshot_matches() {
    ...
    let genesis = Builder::new()
        ...
        .build();
    assert_yaml_snapshot!(genesis.pera_system_wrapper_object());
