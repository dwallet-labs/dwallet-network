    ...
    pub async fn build(self) -> Result<Swarm, anyhow::Error> {
        ...
        let network_config = if let Some(network_config) = self.network_config  {
            ...
            let mut config_builder = ConfigBuilder::new(
                dir.as_ref(), sui_fullnode_rpc_url, sui_faucet_url
            );

            if let Some(epoch_duration_ms) = self.epoch_duration_ms {
                config_builder =
                    config_builder.with_epoch_duration(epoch_duration_ms);
            }

            if let Some(protocol_version) = self.protocol_version {
                config_builder =
                    config_builder.with_protocol_version(protocol_version);
            }
            ...
            {
                config_builder = config_builder
                    .with_fullnode_run_with_range(fullnode_run_with_range);
            }

            if let Some(fullnode_supported_protocol_versions_config) = self.fullnode_supported_protocol_versions_config
            {
                config_builder = config_builder
                    .with_fullnode_supported_protocol_versions_config(fullnode_supported_protocol_versions_config);
            }
        ...
        for fullnode_config in &network_config.fullnode_configs {
            ...
            nodes.insert(fullnode_config.protocol_public_key(), Node::new(fullnode_config.to_owned()));
        }
