...
pub(crate) struct Container {
    ...
    node_watch: watch::Receiver<Weak<IkaNode>>,
}
...
impl Container {
    ...
    pub async fn spawn(config: NodeConfig, _runtime: RuntimeType) -> Self {
        ...
        let node = builder
            .ip(ip)
            .name(&format!("{:?}", config.protocol_public_key().to_string()))
            .init(move || {
                info!("Node restarted");
                ...
                async move {
                    ...
                    let server = IkaNode::start(config, registry_service, None)
                        .await
                        .unwrap();
    ...
    pub fn get_node_handle(&self) -> Option<IkaNodeHandle> {
        Some(IkaNodeHandle::new(self.node_watch.borrow().upgrade()?))
    }
