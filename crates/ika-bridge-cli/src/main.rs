...
use ika_bridge::client::bridge_authority_aggregator::BridgeAuthorityAggregator;
use ika_bridge::crypto::{BridgeAuthorityPublicKey, BridgeAuthorityPublicKeyBytes};
use ika_bridge::eth_transaction_builder::build_eth_transaction;
use ika_bridge::metrics::BridgeMetrics;
use ika_bridge::ika_client::IkaClient;
use ika_bridge::ika_transaction_builder::build_ika_transaction;
use ika_bridge::types::BridgeActionType;
use ika_bridge::utils::{
...
use ika_bridge::utils::{get_eth_contracts, EthBridgeContracts};
use ika_bridge_cli::{
...
use ika_sdk::IkaClient as IkaSdkClient;
use ika_sdk::IkaClientBuilder;
use ika_types::base_types::IkaAddress;
use ika_types::bridge::BridgeChainId;
use ika_types::bridge::{MoveTypeCommitteeMember, MoveTypeCommitteeMemberRegistration};
...
async fn main() -> anyhow::Result<()> {
    ...
    match args.command {
        ...
        BridgeCommand::Governance {
            ...
            let ika_bridge_client =
                IkaClient::<IkaSdkClient>::new(&config.ika_rpc_url, metrics.clone()).await?;

            let (ika_key, ika_address, gas_object_ref) = config
                .get_ika_account_info()
                .await
                .expect("Failed to get ika account info");
            let bridge_summary = ika_bridge_client
                .get_bridge_summary()
                .await
            ...
            // Handle Ika Side
            if chain_id.is_ika_chain() {
                let ika_chain_id = BridgeChainId::try_from(bridge_summary.chain_id).unwrap();
                assert_eq!(
                    ika_chain_id, chain_id,
                    "Chain ID mismatch, expected: {:?}, got from url: {:?}",
                    chain_id, ika_chain_id
                );
                // Create BridgeAction
                let ika_action = make_action(ika_chain_id, &cmd);
                println!("Action to execute on Ika: {:?}", ika_action);
                ...
                let bridge_arg = ika_bridge_client
                    ...
                    .await;
                let rgp = ika_bridge_client
                    .get_reference_gas_price_until_success()
                    .await;
                let id_token_map = ika_bridge_client.get_token_id_map().await.unwrap();
                ...
                .expect("Failed to build ika transaction");
                let ika_sig = Signature::new_secure(
                    &IntentMessage::new(Intent::ika_transaction(), tx.clone()),
                    &ika_key,
                );
                let tx = Transaction::from_data(tx, vec![ika_sig]);
                ...
                if resp.status_ok().unwrap() {
                    println!("Ika Transaction succeeded: {:?}", resp.digest);
                } else {
                    println!(
                        "Ika Transaction failed: {:?}. Effects: {:?}",
        ...
        BridgeCommand::ViewBridgeRegistration { ika_rpc_url } => {
            let metrics = Arc::new(BridgeMetrics::new_for_testing());
            let ika_bridge_client = IkaClient::<IkaSdkClient>::new(&ika_rpc_url, metrics).await?;
            let bridge_summary = ika_bridge_client
                .get_bridge_summary()
                .await
                .map_err(|e| anyhow::anyhow!("Failed to get bridge summary: {:?}", e))?;
            let move_type_bridge_committee = bridge_summary.committee;
            let ika_client = IkaClientBuilder::default().build(ika_rpc_url).await?;
            let stakes = ika_client
                .governance_api()
                ...
                .map(|summary| {
                    let protocol_key =
                        AuthorityPublicKeyBytes::from_bytes(&summary.protocol_pubkey_bytes)
                            .unwrap();
                    (summary.ika_address, (protocol_key, summary.name))
                })
                .collect::<HashMap<_, _>>();
            let mut authorities = vec![];
            let mut output_wrapper = Output::<OutputIkaBridgeRegistration>::default();
            for (_, member) in move_type_bridge_committee.member_registration {
                ...
                let Ok(pubkey) = BridgeAuthorityPublicKey::from_bytes(&bridge_pubkey_bytes) else {
                    output_wrapper.add_error(format!(
                        ...
                        ika_address, bridge_pubkey_bytes
                    ));
                    continue;
                };
                ...
                let Ok(url) = from_utf8(&http_rest_url) else {
                    output_wrapper.add_error(format!(
                        ...
                        ika_address, http_rest_url
                    ));
                    continue;
                };
                let url = url.to_string();

                let (protocol_key, name) = names.get(&ika_address).unwrap();
                let stake = stakes.get(protocol_key).unwrap();
                authorities.push((name, ika_address, pubkey, eth_address, url, stake));
            }
            let total_stake = authorities
                .iter()
                .map(|(_, _, _, _, _, stake)| **stake)
                .sum::<u64>();
            let mut output = OutputIkaBridgeRegistration {
                total_registered_stake: total_stake as f32 / TOTAL_VOTING_POWER as f32 * 100.0,
                ..Default::default()
            };
            for (name, ika_address, pubkey, eth_address, url, stake) in authorities {
        ...
        BridgeCommand::ViewIkaBridge {
            ...
            let names = ika_client
                ...
                .active_validators
                .into_iter()
                .map(|summary| (summary.ika_address, summary.name))
                .collect::<HashMap<_, _>>();
            let mut authorities = vec![];
            ...
            let client = reqwest::Client::builder()
                ...
                .build()
                .unwrap();
            let mut output_wrapper = Output::<OutputIkaBridge>::default();
            for (_, member) in move_type_bridge_committee.members {
                ...
                let name = names.get(&ika_address).cloned().unwrap_or(url.clone());
            ...
            let total_stake = authorities
                ...
                .sum::<u64>();
            let mut output = OutputIkaBridge {
            ...
            for ((name, ika_address, pubkey, eth_address, url, stake, blocklisted), ping_resp) in
        ...
        BridgeCommand::Client { config_path, cmd } => {
            ...
            let metrics = Arc::new(BridgeMetrics::new_for_testing());
            let ika_bridge_client =
                IkaClient::<IkaSdkClient>::new(&config.ika_rpc_url, metrics).await?;
            cmd.handle(&config, ika_bridge_client).await?;
            return Ok(());
        }
    }
...
#[derive(serde::Serialize, Default)]
struct OutputIkaBridgeRegistration {
