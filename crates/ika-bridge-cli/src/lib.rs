...
use std::str::FromStr;
use std::sync::Arc;
use ika_bridge::abi::EthBridgeCommittee;
use ika_bridge::abi::{eth_ika_bridge, EthIkaBridge};
use ika_bridge::crypto::BridgeAuthorityPublicKeyBytes;
use ika_bridge::error::BridgeResult;
use ika_bridge::ika_client::IkaBridgeClient;
use ika_bridge::types::BridgeAction;
use ika_bridge::types::{
    AddTokensOnEvmAction, AddTokensOnIkaAction, AssetPriceUpdateAction, BlocklistCommitteeAction,
    BlocklistType, EmergencyAction, EmergencyActionType, EvmContractUpgradeAction,
    LimitUpdateAction,
};
use ika_bridge::utils::{get_eth_signer_client, EthSigner};
...
use ika_types::bridge::{BridgeChainId, BRIDGE_MODULE_NAME};
use ika_types::crypto::{Signature, IkaKeyPair};
...
use ika_types::{TypeTag, BRIDGE_PACKAGE_ID};
use tracing::info;
...
pub enum BridgeCommand {
    ...
    ViewBridgeRegistration {
        #[clap(long = "ika-rpc-url")]
        ika_rpc_url: String,
    },
    /// View current status of Ika bridge
    #[clap(name = "view-ika-bridge")]
    ViewIkaBridge {
        #[clap(long = "ika-rpc-url")]
        ika_rpc_url: String,
...
pub enum GovernanceClientCommands {
    ...
    AddTokensOnEvm {
        ...
        #[clap(name = "token-prices", use_value_delimiter = true, long)]
        token_prices: Vec<u64>,
        #[clap(name = "token-ika-decimals", use_value_delimiter = true, long)]
        token_ika_decimals: Vec<u8>,
    },
...
pub fn make_action(chain_id: BridgeChainId, cmd: &GovernanceClientCommands) -> BridgeAction {
    match cmd {
        ...
        GovernanceClientCommands::AddTokensOnIka {
            nonce,
            token_ids,
            token_type_names,
            token_prices,
        ...
        GovernanceClientCommands::AddTokensOnEvm {
            ...
            assert_eq!(token_ids.len(), token_prices.len());
            assert_eq!(token_ids.len(), token_ika_decimals.len());
            BridgeAction::AddTokensOnEvmAction(AddTokensOnEvmAction {
                ...
                token_prices: token_prices.clone(),
                token_ika_decimals: token_ika_decimals.clone(),
            })
        }
...
pub fn select_contract_address(
    ...
    match cmd {
        ...
        GovernanceClientCommands::AddTokensOnIka { .. } => unreachable!(),
...
pub struct BridgeCliConfig {
    ...
    /// Proxy address for IkaBridge deployed on Eth
    ...
    /// At leaset one of `ika_key_path` or `eth_key_path` must be provided.
    /// If only one is provided, it will be used for both Ika and Eth.
    pub ika_key_path: Option<PathBuf>,
    /// See `ika_key_path`. Must be Secp256k1 key.
...
impl LoadedBridgeCliConfig {
    pub async fn load(cli_config: BridgeCliConfig) -> anyhow::Result<Self> {
        if cli_config.eth_key_path.is_none() && cli_config.ika_key_path.is_none() {
            return Err(anyhow!(
                "At least one of `ika_key_path` or `eth_key_path` must be provided"
            ));
        }
        let ika_key = if let Some(ika_key_path) = &cli_config.ika_key_path {
            Some(read_key(ika_key_path, false)?)
        } else {
            None
        };
        ...
        let (eth_key, ika_key) = {
            if eth_key.is_none() {
                let ika_key = ika_key.unwrap();
                if !matches!(ika_key, IkaKeyPair::Secp256k1(_)) {
                    return Err(anyhow!("Eth key must be an ECDSA key"));
                }
                (ika_key.copy(), ika_key)
            } else if ika_key.is_none() {
                let eth_key = eth_key.unwrap();
                (eth_key.copy(), eth_key)
            } else {
                (eth_key.unwrap(), ika_key.unwrap())
            }
        };
        ...
        let ika_bridge = EthIkaBridge::new(cli_config.eth_bridge_proxy_address, provider.clone());
        let eth_bridge_committee_proxy_address: EthAddress = ika_bridge.committee().call().await?;
        let eth_bridge_limiter_proxy_address: EthAddress = ika_bridge.limiter().call().await?;
        let eth_committee =
        ...
        let ika_address = IkaAddress::from(&ika_key.public());
        println!("Using Ika address: {:?}", ika_address);
        println!("Using Eth address: {:?}", eth_address);
        println!("Using Eth chain: {:?}", eth_chain_id);

        Ok(Self {
            ika_rpc_url: cli_config.ika_rpc_url,
...
impl LoadedBridgeCliConfig {
    ...
    pub async fn get_ika_account_info(
        self: &LoadedBridgeCliConfig,
    ) -> anyhow::Result<(IkaKeyPair, IkaAddress, ObjectRef)> {
        let pubkey = self.ika_key.public();
        let ika_client_address = IkaAddress::from(&pubkey);
        let ika_sdk_client = IkaClientBuilder::default()
            .build(self.ika_rpc_url.clone())
            .await?;
        let gases = ika_sdk_client
            .coin_read_api()
            .get_coins(ika_client_address, None, None, None)
            .await?
            .data;
        // TODO: is 5 Ika a good number?
        let gas = gases
            .into_iter()
        ...
        Ok((self.ika_key.copy(), ika_client_address, gas.object_ref()))
    }
}
#[derive(Parser)]
#[clap(rename_all = "kebab-case")]
    ...
    pub async fn handle(
        ...
        match self {
            BridgeClientCommands::DepositNativeEtherOnEth {
                ...
                let eth_tx = eth_ika_bridge
                    .bridge_eth(ika_recipient_address.to_vec().into(), target_chain)
                    .value(amount);
                let pending_tx = eth_tx.send().await.unwrap();
                let tx_receipt = pending_tx.await.unwrap().unwrap();
                info!(
                    "Deposited {ether_amount} Ethers to {:?} (target chain {target_chain}). Receipt: {:?}", ika_recipient_address, tx_receipt,
                );
                Ok(())
            }
            BridgeClientCommands::ClaimOnEth { seq_num, dry_run } => {
                claim_on_eth(seq_num, config, ika_bridge_client, dry_run)
                    .await
                    .map_err(|e| anyhow!("{:?}", e))
            }
            BridgeClientCommands::DepositOnIka {
                coin_object_id,
                coin_type,
                target_chain,
                recipient_address,
...
async fn deposit_on_ika(
    ...
    let ika_client = ika_bridge_client.ika_client();
    let bridge_object_arg = ika_bridge_client
    ...
    let rgp = ika_client
        ...
        .await
        .unwrap();
    let sender = IkaAddress::from(&config.ika_key.public());
    let gas_obj_ref = ika_client
        .coin_read_api()
        ...
        .object_ref();
    let coin_obj_ref = ika_client
        .read_api()
        .get_object_with_options(coin_object_id, IkaObjectDataOptions::default())
        .await?
        .data
        .unwrap()
        .object_ref();
    ...
    info!(?tx_digest, "Sending deposit transction to Ika.");
    let resp = ika_bridge_client
...
async fn claim_on_eth(
    ...
    let ika_chain_id = ika_bridge_client.get_bridge_summary().await?.chain_id;
    let parsed_message = ika_bridge_client
        .get_parsed_token_transfer_message(ika_chain_id, seq_num)
        .await?;
    if parsed_message.is_none() {
        println!("No record found for seq_num: {seq_num}, chain id: {ika_chain_id}");
        return Ok(());
    }
    let parsed_message = parsed_message.unwrap();
    let sigs = ika_bridge_client
        .get_token_transfer_action_onchain_signatures_until_success(ika_chain_id, seq_num)
        .await;
    if sigs.is_none() {
        println!("No signatures found for seq_num: {seq_num}, chain id: {ika_chain_id}");
        return Ok(());
    }
    let signatures = sigs
        .unwrap()
    ...
    let message = eth_ika_bridge::Message::from(parsed_message);
    let tx = eth_ika_bridge.transfer_bridged_tokens_with_signatures(signatures, message);
    if dry_run {
        let tx = tx.tx;
        let resp = config.eth_signer.estimate_gas(&tx, None).await;
        println!(
            "Ika to Eth bridge transfer claim dry run result: {:?}",
            resp
        );
    } else {
        let eth_claim_tx_receipt = tx.send().await.unwrap().await.unwrap().unwrap();
        println!(
            "Ika to Eth bridge transfer claimed: {:?}",
...
mod tests {
    ...
    async fn test_encode_call_data() {
        let abi_json =
            std::fs::read_to_string("../ika-bridge/abi/tests/mock_ika_bridge_v2.json").unwrap();
