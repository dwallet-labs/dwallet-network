...
use pera_config::{Config, NodeConfig};
use pera_core::runtime::PeraRuntimes;
use pera_node::metrics;
use pera_telemetry::send_telemetry_event;
...
fn main() {
    ...
    move_vm_profiler::gas_profiler_feature_enabled! {
        panic!("Cannot run the pera-node binary with gas-profiler feature enabled");
    }
    ...
    let runtimes = PeraRuntimes::new(&config);
    ...
    info!("Pera Node version: {VERSION}");
    info!(
    ...
    // if it deadlocks.
    let node_once_cell = Arc::new(AsyncOnceCell::<Arc<pera_node::PeraNode>>::new());
    let node_once_cell_clone = node_once_cell.clone();
    let rpc_runtime = runtimes.json_rpc.handle().clone();

    // let pera-node signal main to shutdown runtimes
    let (runtime_shutdown_tx, runtime_shutdown_rx) = broadcast::channel::<()>(1);

    runtimes.pera_node.spawn(async move {
        match pera_node::PeraNode::start_async(config, registry_service, Some(rpc_runtime), VERSION).await {
            Ok(pera_node) => node_once_cell_clone
                .set(pera_node)
        ...
        // when we get a shutdown signal from pera-node, forward it on to the runtime_shutdown_channel here in
        // main to signal runtimes to all shutdown.
        tokio::select! {
           _ = shutdown_rx.recv() => {
                runtime_shutdown_tx.send(()).expect("failed to forward shutdown signal from pera-node to pera-node main");
            }
        }
    ...
    runtimes.metrics.spawn(async move {
        ...
        info!("Pera chain identifier: {chain_identifier}");
        prometheus_registry
        ...
        pera_node::admin::run_admin_server(node, admin_interface_port, filter_handle).await
    });
