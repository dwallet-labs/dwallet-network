...
use pera_core::consensus_adapter::SubmitToConsensus;
use pera_core::consensus_manager::ConsensusClient;
use pera_core::epoch::randomness::RandomnessManager;
use pera_core::execution_cache::build_execution_cache;
use pera_core::state_accumulator::StateAccumulatorMetrics;
use pera_core::storage::RestReadStore;
use pera_core::traffic_controller::metrics::TrafficControllerMetrics;
use pera_json_rpc::bridge_api::BridgeReadApi;
use pera_json_rpc_api::JsonRpcMetrics;
use pera_network::randomness;
use pera_rest_api::RestMetrics;
...
use pera_config::node::{DBCheckpointConfig, RunWithRange};
use pera_config::node_config_metrics::NodeConfigMetrics;
use pera_config::object_storage_config::{ObjectStoreConfig, ObjectStoreType};
use pera_config::{ConsensusConfig, NodeConfig};
...
use pera_core::authority::epoch_start_configuration::EpochStartConfiguration;
use pera_core::authority_aggregator::{AuthAggMetrics, AuthorityAggregator};
use pera_core::authority_server::{ValidatorService, ValidatorServiceMetrics};
use pera_core::checkpoints::checkpoint_executor::metrics::CheckpointExecutorMetrics;
use pera_core::checkpoints::checkpoint_executor::{CheckpointExecutor, StopReason};
use pera_core::checkpoints::{
    CheckpointMetrics, CheckpointService, CheckpointStore, SendCheckpointToStateSync,
    SubmitCheckpointToConsensus,
};
use pera_core::consensus_adapter::{
    CheckConnection, ConnectionMonitorStatus, ConsensusAdapter, ConsensusAdapterMetrics,
};
use pera_core::consensus_manager::{ConsensusManager, ConsensusManagerTrait};
use pera_core::consensus_throughput_calculator::{
    ConsensusThroughputCalculator, ConsensusThroughputProfiler, ThroughputProfileRanges,
};
use pera_core::consensus_validator::{PeraTxValidator, PeraTxValidatorMetrics};
use pera_core::db_checkpoint_handler::DBCheckpointHandler;
use pera_core::epoch::committee_store::CommitteeStore;
use pera_core::epoch::consensus_store_pruner::ConsensusStorePruner;
use pera_core::epoch::epoch_metrics::EpochMetrics;
use pera_core::epoch::reconfiguration::ReconfigurationInitiator;
use pera_core::module_cache_metrics::ResolverMetrics;
use pera_core::overload_monitor::overload_monitor;
use pera_core::rest_index::RestIndexStore;
use pera_core::signature_verifier::SignatureVerifierMetrics;
use pera_core::state_accumulator::StateAccumulator;
use pera_core::storage::RocksDbStore;
use pera_core::transaction_orchestrator::TransactiondOrchestrator;
use pera_core::{
    authority::{AuthorityState, AuthorityStore},
    authority_client::NetworkAuthorityClient,
};
use pera_json_rpc::coin_api::CoinReadApi;
use pera_json_rpc::governance_api::GovernanceReadApi;
use pera_json_rpc::indexer_api::IndexerApi;
use pera_json_rpc::move_utils::MoveUtils;
use pera_json_rpc::read_api::ReadApi;
use pera_json_rpc::transaction_builder_api::TransactionBuilderApi;
use pera_json_rpc::transaction_execution_api::TransactionExecutionApi;
use pera_json_rpc::JsonRpcServerBuilder;
use pera_macros::fail_point;
use pera_macros::{fail_point_async, replay_log};
use pera_network::api::ValidatorServer;
use pera_network::discovery;
use pera_network::discovery::TrustedPeerChangeEvent;
use pera_network::state_sync;
use pera_protocol_config::{Chain, ProtocolConfig};
use pera_snapshot::uploader::StateSnapshotUploader;
use pera_storage::{
    http_key_value_store::HttpKVStore,
    key_value_store::{FallbackTransactionKVStore, TransactionKeyValueStore},
    key_value_store_metrics::KeyValueStoreMetrics,
};
use pera_storage::{FileCompression, IndexStore, StorageFormat};
...
use crate::metrics::{GrpcMetrics, PeraNodeMetrics};
...
mod simulator {
    ...
    pub(super) struct SimState {
        pub sim_node: pera_simulator::runtime::NodeHandle,
        pub sim_safe_mode_expected: AtomicBool,
        _leak_detector: pera_simulator::NodeLeakDetector,
    }

    impl Default for SimState {
        fn default() -> Self {
            Self {
                sim_node: pera_simulator::runtime::NodeHandle::current(),
                sim_safe_mode_expected: AtomicBool::new(false),
                _leak_detector: pera_simulator::NodeLeakDetector::new(),
            }
        }
    }

    type JwkInjector = dyn Fn(AuthorityName, &OIDCProvider) -> PeraResult<Vec<(JwkId, JWK)>>
        + Send
        + Sync
        + 'static;
    ...
    ) -> PeraResult<Vec<(JwkId, JWK)>> {
        use fastcrypto_zkp::bn254::zk_login::parse_jwks;
        // Just load a default Twitch jwk for testing.
        parse_jwks(
            pera_types::zk_login_util::DEFAULT_JWK_BYTES,
            &OIDCProvider::Twitch,
        )
        .map_err(|_| PeraError::JWKRetrievalError)
    }
...
use pera_core::dwallet_mpc::mpc_outputs_manager::DWalletMPCOutputsVerifier;
...
pub struct PeraNode {
    ...
    end_of_epoch_channel: broadcast::Sender<PeraSystemState>,
    ...
    // Channel to allow signaling upstream to shutdown pera-node
...
impl fmt::Debug for PeraNode {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("PeraNode")
...
impl PeraNode {
    ...
    pub async fn start_async(
        ...
        info!(node =? config.protocol_public_key(),
            "Initializing pera-node listening on {}", config.network_address
        );
        ...
        if is_genesis {
            info!("checking PERA conservation at genesis");
            // When we are opening the db table, the only time when it's safe to
            // check PERA conservation is at genesis. Otherwise we may be in the middle of
            // an epoch and the PERA conservation check will fail. This also initialize
            // the expected_network_pera_amount table.
            cache_traits
                .reconfig_api
                .expensive_check_pera_conservation(&epoch_store)
                .expect("PERA conservation check cannot fail at genesis");
        }
        ...
        if epoch_store.epoch() == 0 {
            ...
            let transaction =
                pera_types::executable_transaction::VerifiedExecutableTransaction::new_unchecked(
                    pera_types::executable_transaction::ExecutableTransaction::new_from_data_and_sig(
            ...
            .enable_secondary_index_checks()
        {
            if let Some(indexes) = state.indexes.clone() {
                pera_core::verify_indexes::verify_indexes(
        ...
        let network_connection_metrics =
            NetworkConnectionMetrics::new("pera", &registry_service.default_registry());
    ...
    pub fn subscribe_to_epoch_change(&self) -> broadcast::Receiver<PeraSystemState> {
    ...
    pub async fn close_epoch(&self, epoch_store: &Arc<AuthorityPerEpochStore>) -> PeraResult {
    ...
    pub fn clear_override_protocol_upgrade_buffer_stake(&self, epoch: EpochId) -> PeraResult {
        self.state
    ...
    pub async fn close_epoch_for_testing(&self) -> PeraResult {
    ...
    fn create_p2p_network(
        ...
        let p2p_network = {
            ...
            let inbound_network_metrics =
                NetworkMetrics::new("pera", "inbound", prometheus_registry);
            let outbound_network_metrics =
                NetworkMetrics::new("pera", "outbound", prometheus_registry);
            ...
            let server_name = format!("pera-{}", chain_identifier);
    ...
    async fn construct_validator_components(
        ...
        connection_monitor_status: Arc<ConnectionMonitorStatus>,
        registry_service: &RegistryService,
        pera_node_metrics: Arc<PeraNodeMetrics>,
        ...
        let pera_tx_validator_metrics =
            PeraTxValidatorMetrics::new(&registry_service.default_registry());
        ...
        consensus_manager
            .start(
                ...
                PeraTxValidator::new(
                    ...
                    pera_tx_validator_metrics.clone(),
                ),
            )
            .await;
    ...
    pub async fn monitor_reconfiguration(self: Arc<Self>) -> Result<()> {
        ...
        loop {
            ...
            let latest_system_state = self
                ...
                .expect("Read Pera System State object cannot fail");
            ...
            let next_epoch_committee = new_epoch_start_state.get_pera_committee();
            ...
            *self.validator_components.lock().await = new_validator_components;
                ...
                )
            {
                ...
                .prune_checkpoints_for_eligible_epochs_for_testing(
                    self.config.clone(),
                    pera_core::authority::authority_store_pruner::AuthorityStorePruningMetrics::new_for_test(),
                )
                .await?;
            }
...
#[cfg(msim)]
impl PeraNode {
    pub fn get_sim_node_id(&self) -> pera_simulator::task::NodeId {
...
pub async fn build_http_server(
    ...
    let json_rpc_router = {
        ...
        let name_service_config =
            if let (Some(package_address), Some(registry_id), Some(reverse_registry_id)) = (
                ...
                pera_json_rpc::name_service::NameServiceConfig::new(
                    package_address,
                    registry_id,
                    reverse_registry_id,
                )
            } else {
                pera_json_rpc::name_service::NameServiceConfig::default()
            };
    ...
    if config.enable_experimental_rest_api {
        let mut rest_service = pera_rest_api::RestService::new(
