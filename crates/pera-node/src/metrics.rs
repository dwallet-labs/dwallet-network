...
pub struct MetricsPushClient {
    certificate: std::sync::Arc<pera_tls::SelfSignedCertificate>,
    client: reqwest::Client,
}

impl MetricsPushClient {
    pub fn new(network_key: pera_types::crypto::NetworkKeyPair) -> Self {
        use fastcrypto::traits::KeyPair;
        let certificate = std::sync::Arc::new(pera_tls::SelfSignedCertificate::new(
            network_key.private(),
            pera_tls::PERA_VALIDATOR_SERVER_NAME,
        ));
    ...
    pub fn certificate(&self) -> &pera_tls::SelfSignedCertificate {
        &self.certificate
    }
...
/// is configured.
pub fn start_metrics_push_task(config: &pera_config::NodeConfig, registry: RegistryService) {
    use fastcrypto::traits::KeyPair;
    use pera_config::node::MetricsConfig;
...
mod tests {
    ...
    pub async fn test_metrics_endpoint_with_multiple_registries_add_remove() {
        ...
        let registry_2 = Registry::new_custom(Some("pera".to_string()), None).unwrap();
        ...
        assert!(result.contains(
            "# HELP pera_counter_2 a sample counter 2
# TYPE pera_counter_2 counter
pera_counter_2 0"
        ));
