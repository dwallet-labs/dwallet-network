...
use ika_protocol_config_macros::{
...
// Version 3: gas model v2, including all ika conservation fixes. Fix for loaded child object
...
//            `max_meter_ticks_per_module` limits each from 6_000_000 to 16_000_000. ika-system
//            framework changes.
...
//             Changes to ika-system package to add PTB-friendly unstake function, and minor cleanup.
...
// Version 19: Changes to ika-system package to enable liquid staking.
...
// Version 25: Add ika::table_vec::swap and ika::table_vec::swap_remove to system packages.
// Version 26: New gas model version.
//             Add support for receiving objects off of other objects in devnet only.
// Version 28: Add ika::zklogin::verify_zklogin_id and related functions to ika framework.
//             Enable transaction effects v2 in devnet.
// Version 29: Add verify_legacy_zklogin_address flag to ika framework, this add ability to verify
...
//             Add support for getting object ID referenced by receiving object in ika framework.
//             Create new execution layer version, and preserve previous behavior in v1.
//             Update semantics of `ika::transfer::receive` and add `ika::transfer::public_receive`.
// Version 32: Add delete functions for VerifiedID and VerifiedIssuer.
//             Add ika::token module to ika framework.
...
// Version 42: Migrate ika framework and related code to Move 2024
...
//             Modified ika-system package to enable withdrawal of stake before it becomes active.
//             Enable soft bundle in devnet and testnet.
//             Core macro visibility in ika core framework.
...
pub struct ProtocolConfig {
    ...
    /// Max size of objects a transaction can write to disk after completion. Enforce by the Ika adapter.
    ...
    /// Max size of objects a system transaction can write to disk after completion. Enforce by the Ika adapter.
    ...
    /// Maximum size of the `contents` part of an object, in bytes. Enforced by the Ika adapter when effects are produced.
    ...
    /// Maximum size of a Move package object, in bytes. Enforced by the Ika adapter at the end of a publish transaction.
    ...
    // === Execution gas costs ====
    /// Base cost for any Ika transaction
    ...
    /// Per-byte cost of storing an object in the Ika global object store. Some of this cost may be refundable if the object is later freed
    obj_data_cost_refundable: Option<u64>,

    // Per-byte cost of storing an object in the Ika transaction log (e.g., in CertifiedTransactionEffects)
        ...
        for cur in 2..=version.0 {
            match cur {
                ...
                3 => {
                    ...
                    // max gas budget is in NIKA and an absolute value 50IKA
