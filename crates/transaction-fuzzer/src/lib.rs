...
use ika_types::object::{MoveObject, Object, Owner, OBJECT_START_VERSION};
use ika_types::{gas_coin::TOTAL_SUPPLY_NIKA, transaction::GasData};
...
fn generate_random_gas_data(
    ...
    let (sender, sender_key): (IkaAddress, AccountKeyPair) = get_key_pair();
    let mut rng = StdRng::from_seed(seed);
    let mut gas_objects = vec![];
    let mut object_refs = vec![];

    let max_gas_balance = TOTAL_SUPPLY_NIKA;
