...
fn create_alias_keystore_file_test() {
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("pera.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    keystore
...
fn check_reading_aliases_file_correctly() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("pera.keystore");
    let keystore_path_keep = temp_dir.path().join("pera.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    let kp = keystore
        .generate_and_add_new_key(SignatureScheme::ED25519, None, None, None)
        .unwrap();
...
fn keystore_no_aliases() {
    ...
    // pera.aliases file with the existing address in the pera.keystore,
...
/// This test confirms rust's implementation of mnemonic is the same with the Pera Wallet
#[test]
fn pera_wallet_address_mnemonic_test() -> Result<(), anyhow::Error> {
    ...
    assert_eq!(pubkey.flag(), Ed25519PeraSignature::SCHEME.flag());

    let mut hasher = DefaultHash::default();
    hasher.update([pubkey.flag()]);
    hasher.update(pubkey);
    let g_arr = hasher.finalize();
    let mut res = [0u8; PERA_ADDRESS_LENGTH];
    res.copy_from_slice(&AsRef::<[u8]>::as_ref(&g_arr)[..PERA_ADDRESS_LENGTH]);
    let address = PeraAddress::try_from(res.as_slice())?;
