...
pub trait AccountKeystore: Send + Sync {
    ...
    where
        T: Serialize;
    fn addresses(&self) -> Vec<PeraAddress> {
        self.keys().iter().map(|k| k.into()).collect()
    }
    fn addresses_with_alias(&self) -> Vec<(&PeraAddress, &Alias)>;
    ...
    fn get_alias_by_address(&self, address: &PeraAddress) -> Result<String, anyhow::Error>;
    fn get_address_by_alias(&self, alias: String) -> Result<&PeraAddress, anyhow::Error>;
    ...
    ) -> Result<(PeraAddress, String, SignatureScheme), anyhow::Error> {
...
pub struct FileBasedKeystore {
    keys: BTreeMap<PeraAddress, PeraKeyPair>,
    aliases: BTreeMap<PeraAddress, Alias>,
    path: Option<PathBuf>,
}
...
impl FileBasedKeystore {
        ...
        let aliases = if aliases_path.exists() {
            ...
            aliases
                ...
                .map(|alias| {
                    ...
                    key.map(|k| (Into::<PeraAddress>::into(&k), alias))
                })
                .collect::<Result<BTreeMap<_, _>, _>>()
                .map_err(|e| {
                    anyhow!(
    ...
    /// To see Bech32 format encoding, use `pera keytool export $PERA_ADDRESS` where
    /// $PERA_ADDRESS can be found with `pera keytool list`. Or use `pera keytool convert $BASE64_STR`
    ...
    pub fn key_pairs(&self) -> Vec<&PeraKeyPair> {
...
impl InMemKeystore {
    pub fn new_insecure_for_tests(initial_key_number: usize) -> Self {
        ...
        let keys = (0..initial_key_number)
            ...
            .map(|(ad, k)| (ad, PeraKeyPair::Ed25519(k)))
            .collect::<BTreeMap<PeraAddress, PeraKeyPair>>();
...
mod tests {
    ...
    fn validate_alias_test() {
        ...
        assert!(validate_alias("abc_123.pera").is_ok());
        // Not allowed
