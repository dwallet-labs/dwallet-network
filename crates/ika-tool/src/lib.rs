...
use std::{fs, io};
use ika_config::{genesis::Genesis, NodeConfig};
use ika_core::authority_client::{AuthorityAPI, NetworkAuthorityClient};
use ika_core::execution_cache::build_execution_cache_from_env;
...
use ika_storage::object_store::http::HttpDownloaderBuilder;
use ika_storage::object_store::util::Manifest;
use ika_storage::object_store::util::PerEpochManifest;
use ika_storage::object_store::util::MANIFEST_FILENAME;
use ika_types::accumulator::Accumulator;
use ika_types::committee::QUORUM_THRESHOLD;
use ika_types::crypto::AuthorityPublicKeyBytes;
use ika_types::messages_grpc::LayoutGenerationOption;
use ika_types::multiaddr::Multiaddr;
use ika_types::{base_types::*, object::Owner};
use tokio::sync::mpsc;
...
use ika_archival::reader::{ArchiveReader, ArchiveReaderMetrics};
use ika_archival::{verify_archive_with_checksums, verify_archive_with_genesis_config};
...
use ika_core::authority::AuthorityStore;
use ika_core::checkpoints::CheckpointStore;
use ika_core::epoch::committee_store::CommitteeStore;
use ika_core::storage::RocksDbStore;
use ika_snapshot::reader::StateSnapshotReaderV1;
use ika_snapshot::setup_db_state;
use ika_storage::object_store::util::{copy_file, exists, get_path};
use ika_storage::object_store::ObjectStoreGetExt;
use ika_storage::verify_checkpoint_range;
use ika_types::messages_checkpoint::{CheckpointCommitment, ECMHLiveObjectSetDigest};
use ika_types::messages_grpc::{
    ObjectInfoRequest, ObjectInfoRequestKind, ObjectInfoResponse, TransactionInfoRequest,
    TransactionStatus,
};

use ika_types::storage::{ReadStore, SharedInMemoryStore};
use tracing::info;
...
async fn make_clients(
    ...
    for validator in active_validators {
        ...
        //     ika_types::crypto::NetworkPublicKey::from_bytes(&validator.network_pubkey_bytes)?,
...
pub async fn get_transaction_block(
    ...
    let ika_client = Arc::new(IkaClientBuilder::default().build(fullnode_rpc).await?);
    let clients = make_clients(&ika_client).await?;
            ...
            anemo_cli::ServiceInfo::new()
                ...
                .add_method(
                    ...
                    anemo_cli::ron_method!(
                        ...
                        ika_types::messages_checkpoint::CheckpointContentsDigest
                    ),
                )
                .add_method(
