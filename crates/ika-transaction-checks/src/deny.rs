...
use ika_types::{
    ...
    error::{IkaError, IkaResult, UserInputError},
            ...
            deny_if_true!(
                filter_config.zklogin_disabled_providers().contains(
                    &OIDCProvider::from_iss(z.get_iss())
                        .map_err(|_| IkaError::UnexpectedMessage(z.get_iss().to_string()))?
                        .to_string()
                ),
...
fn check_signers(filter_config: &TransactionDenyConfig, tx_data: &TransactionData) -> IkaResult {
