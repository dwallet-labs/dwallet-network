...
mod checked {
    ...
    use ika_types::error::{IkaResult, UserInputError, UserInputResult};
    use ika_types::executable_transaction::VerifiedExecutableTransaction;
    use ika_types::metrics::BytecodeVerifierMetrics;
    ...
    use ika_types::{
        IKA_AUTHENTICATOR_STATE_OBJECT_ID, IKA_CLOCK_OBJECT_ID, IKA_CLOCK_OBJECT_SHARED_VERSION,
    ...
    ) -> IkaResult<(IkaGasStatus, CheckedInputObjects)> {
    ...
    pub fn check_dev_inspect_input(
        ...
        let mut used_objects: HashSet<IkaAddress> = HashSet::new();
    ...
    fn check_gas(
        ...
        if tx_kind.is_system_tx() {
            Ok(IkaGasStatus::new_unmetered())
        } else {
            let gas_status =
                IkaGasStatus::new(gas_budget, gas_price, reference_gas_price, protocol_config)?;
    ...
    pub fn check_non_system_packages_to_be_published(
        ...
        let mut verifier = ika_execution::verifier(protocol_config, signing_limits, metrics);
