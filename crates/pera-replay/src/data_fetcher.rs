...
use pera_json_rpc_types::PeraGetPastObjectRequest;
use pera_json_rpc_types::PeraObjectData;
use pera_json_rpc_types::PeraObjectDataOptions;
use pera_json_rpc_types::PeraObjectResponse;
use pera_json_rpc_types::PeraPastObjectResponse;
...
pub(crate) trait DataFetcher {
    ...
    ) -> Result<PeraTransactionBlockResponse, ReplayEngineError>;
    ...
    async fn get_epoch_change_events(
        &self,
        reverse: bool,
    ) -> Result<Vec<PeraEvent>, ReplayEngineError>;
...
impl RemoteFetcher {
    pub fn new(rpc_client: PeraClient) -> Self {
        Self {
            rpc_client,
...
impl DataFetcher for RemoteFetcher {
    ...
    async fn multi_get_versioned(
        ...
        let options = PeraObjectDataOptions::bcs_lossless();

        let objs: Vec<_> = to_fetch
            .iter()
            .map(|(object_id, version)| PeraGetPastObjectRequest {
                object_id: *object_id,
                version: *version,
            })
            .collect();
    ...
    async fn get_child_object(
        ...
        let response = self
            ...
            .await
            .map_err(|q| ReplayEngineError::PeraRpcError { err: q.to_string() })?;
    ...
    async fn multi_get_latest(
        ...
        join_all(objectsx)
            ...
            .iter()
            .flatten()
            .map(obj_from_pera_obj_response)
            .collect::<Result<Vec<_>, _>>()
            .map(|mut x| {
                // Add the cached objects to the result
                x.extend(cached);
    ...
    async fn get_transaction(
        ...
        let tx_fetch_opts = PeraTransactionBlockResponseOptions::full_content();
    ...
    async fn get_epoch_change_events(
        ...
        let mut epoch_change_events: Vec<PeraEvent> = vec![];
...
fn convert_past_obj_response(resp: PeraPastObjectResponse) -> Result<Object, ReplayEngineError> {
    match resp {
        PeraPastObjectResponse::VersionFound(o) => obj_from_pera_obj_data(&o),
        PeraPastObjectResponse::ObjectDeleted(r) => Err(ReplayEngineError::ObjectDeleted {
            id: r.object_id,
            version: r.version,
            digest: r.digest,
        }),
        ...
        PeraPastObjectResponse::VersionTooHigh {
            object_id,
            asked_version,
            latest_version,
...
fn obj_from_pera_obj_response(o: &PeraObjectResponse) -> Result<Object, ReplayEngineError> {
    let o = o.object().map_err(ReplayEngineError::from)?.clone();
    obj_from_pera_obj_data(&o)
}

fn obj_from_pera_obj_data(o: &PeraObjectData) -> Result<Object, ReplayEngineError> {
...
pub fn extract_epoch_and_version(ev: PeraEvent) -> Result<(u64, u64), ReplayEngineError> {
