...
use ika_sdk_types::types::{ObjectId, StructTag};
use ika_types::ika_sdk_types_conversions::struct_tag_sdk_to_core;
...
async fn get_coin_info(
    ...
    let ika_types::storage::CoinInfo {
    ...
    let metadata = if let Some(coin_metadata_object_id) = coin_metadata_object_id {
        state
            ...
            .map(ika_types::coin::CoinMetadata::try_from)
            .transpose()
            .map_err(|_| {
                RestError::new(
    ...
    let treasury = if let Some(treasury_object_id) = treasury_object_id {
        state
            ...
            .map(ika_types::coin::TreasuryCap::try_from)
            .transpose()
            .map_err(|_| {
                RestError::new(
                    axum::http::StatusCode::INTERNAL_SERVER_ERROR,
    ...
    } else if ika_types::gas_coin::GAS::is_gas(&core_coin_type) {
        Some(CoinTreasury::IKA)
    } else {
        None
    };
...
impl From<ika_types::coin::CoinMetadata> for CoinMetadata {
    fn from(value: ika_types::coin::CoinMetadata) -> Self {
        Self {
...
impl CoinTreasury {
    const IKA: Self = Self {
        id: None,
        total_supply: ika_types::gas_coin::TOTAL_SUPPLY_NIKA,
    };
}
