...
use crate::{
    ...
    types::{
        X_IKA_CHAIN, X_IKA_CHAIN_ID, X_IKA_CHECKPOINT_HEIGHT, X_IKA_EPOCH,
        X_IKA_LOWEST_AVAILABLE_CHECKPOINT, X_IKA_LOWEST_AVAILABLE_CHECKPOINT_OBJECTS,
        X_IKA_TIMESTAMP_MS,
    },
...
pub async fn append_info_headers(
    ...
    if let Ok(chain_id) = state.chain_id().to_string().try_into() {
        headers.insert(X_IKA_CHAIN_ID, chain_id);
    }

    if let Ok(chain) = state.chain_id().chain().as_str().try_into() {
        headers.insert(X_IKA_CHAIN, chain);
    }

    if let Ok(latest_checkpoint) = state.reader.inner().get_latest_checkpoint() {
        headers.insert(X_IKA_EPOCH, latest_checkpoint.epoch().into());
        headers.insert(
            X_IKA_CHECKPOINT_HEIGHT,
            latest_checkpoint.sequence_number.into(),
        );
        headers.insert(X_IKA_TIMESTAMP_MS, latest_checkpoint.timestamp_ms.into());
    }

    if let Ok(lowest_available_checkpoint) = state.reader.inner().get_lowest_available_checkpoint()
    {
        headers.insert(
            X_IKA_LOWEST_AVAILABLE_CHECKPOINT,
    ...
    {
        headers.insert(
            X_IKA_LOWEST_AVAILABLE_CHECKPOINT_OBJECTS,
