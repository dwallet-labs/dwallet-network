...
use std::sync::Arc;
use ika_sdk_types::types::framework::Coin;
...
use ika_types::transaction_executor::{SimulateTransactionResult, TransactionExecutor};
use tap::Pipe;
...
> {
    ...
    let request = ika_types::quorum_driver_types::ExecuteTransactionRequestV3 {
    ...
    let ika_types::quorum_driver_types::ExecuteTransactionResponseV3 {
        effects,
        events,
        input_objects,
        output_objects,
    ...
    let (effects, finality) = {
        let ika_types::quorum_driver_types::FinalizedEffects {
            effects,
            finality_info,
        } = effects;
        let finality = match finality_info {
            ika_types::quorum_driver_types::EffectsFinalityInfo::Certified(sig) => {
                EffectsFinality::Certified {
                    signature: sig.into(),
                }
            }
            ika_types::quorum_driver_types::EffectsFinalityInfo::Checkpointed(
                _epoch,
                checkpoint,
            ) => EffectsFinality::Checkpointed { checkpoint },
            ika_types::quorum_driver_types::EffectsFinalityInfo::QuorumExecuted(_) => {
...
enum ReadableEffectsFinality {
    ...
    Checkpointed {
        #[serde_as(as = "ika_types::ika_serde::Readable<ika_types::ika_serde::BigInt<u64>, _>")]
