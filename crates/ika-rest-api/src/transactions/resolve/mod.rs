...
use ika_sdk_types::types::Argument;
use ika_sdk_types::types::Command;
use ika_sdk_types::types::ObjectId;
use ika_sdk_types::types::Transaction;
use ika_sdk_types::types::UnresolvedInputArgument;
use ika_sdk_types::types::UnresolvedObjectReference;
use ika_sdk_types::types::UnresolvedProgrammableTransaction;
use ika_sdk_types::types::UnresolvedTransaction;
use ika_sdk_types::types::UnresolvedValue;
...
use ika_types::transaction::GasData;
    ...
    if resolved_transaction.gas_data().payment.is_empty() {
        let input_objects = resolved_transaction
            ...
            .flat_map(|obj| match obj {
                ika_types::transaction::InputObjectKind::ImmOrOwnedMoveObject((id, _, _)) => {
                    Some(*id)
                }
                _ => None,
            })
...
fn called_packages(
    ...
    let binary_config = ika_types::execution_config_utils::to_binary_config(protocol_config);
...
fn resolve_unresolved_transaction(
    ...
    Ok(TransactionData::V1(
        ika_types::transaction::TransactionDataV1 {
            kind: ika_types::transaction::TransactionKind::ProgrammableTransaction(ptb),
            sender,
            gas_data,
            expiration,
        },
...
fn resolve_object_reference_with_object(
    object: &ika_types::object::Object,
    unresolved_object_reference: UnresolvedObjectReference,
) -> Result<ObjectRef> {
    ...
    match object.owner() {
        ika_types::object::Owner::AddressOwner(_) | ika_types::object::Owner::Immutable => {}
        _ => {
...
fn resolve_arg(
    ...
    use ika_sdk_types::types::UnresolvedInputArgumentKind::*;
...
fn resolve_object(
    ...
    arg_idx: usize,
    object_id: ObjectId,
    version: Option<ika_sdk_types::types::Version>,
    digest: Option<ika_sdk_types::types::ObjectDigest>,
    ...
    match object.owner() {
        ika_types::object::Owner::Immutable => resolve_object_reference_with_object(
            &object,
            UnresolvedObjectReference {
                object_id,
                version,
        ...
        ika_types::object::Owner::AddressOwner(_) => {
        ...
        ika_types::object::Owner::Shared { .. } => {
            resolve_shared_input_with_object(called_packages, commands, arg_idx, object)
        }
        ika_types::object::Owner::ObjectOwner(_) => Err(RestError::new(
            axum::http::StatusCode::BAD_REQUEST,
...
fn is_input_argument_receiving(
    ...
    let (receiving_package, receiving_module, receiving_struct) =
        ika_types::transfer::RESOLVED_RECEIVING_STRUCT;
...
fn arg_type_of_move_call_input<'a>(
    ...
    move_call: &ika_sdk_types::types::MoveCall,
    idx: usize,
...
fn resolve_shared_input_with_object(
    ...
    let initial_shared_version = if let ika_types::object::Owner::Shared {
