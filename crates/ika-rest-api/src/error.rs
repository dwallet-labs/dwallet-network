...
impl From<ika_types::storage::error::Error> for RestError {
    fn from(value: ika_types::storage::error::Error) -> Self {
        Self {
...
impl From<ika_types::ika_sdk_types_conversions::SdkTypeConversionError> for RestError {
...
impl From<ika_types::quorum_driver_types::QuorumDriverError> for RestError {
    fn from(error: ika_types::quorum_driver_types::QuorumDriverError) -> Self {
        ...
        match error {
            InvalidUserSignature(err) => {
                let message = {
                    let err = match err {
                        IkaError::UserInputError { error } => error.to_string(),
                        _ => err.to_string(),
                    };
