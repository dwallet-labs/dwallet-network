...
impl ObjectHandler {
    ...
    async fn process_object(
        ...
        let (struct_tag, pera_move_struct) = if let Some(move_struct) = move_struct {
            match move_struct.into() {
                PeraMoveStruct::WithTypes { type_, fields } => {
                    (Some(type_), Some(PeraMoveStruct::WithFields(fields)))
                }
        ...
        let entry = ObjectEntry {
            ...
            object_json: pera_move_struct.map(|x| x.to_json_value().to_string()),
        };
        state.objects.push(entry);
        Ok(())
    }
