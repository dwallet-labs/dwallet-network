...
use crate::types::AddTokensOnIkaAction;
...
use crate::types::EthToIkaBridgeAction;
...
use ika_types::base_types::IKA_ADDRESS_LENGTH;
...
pub const ADD_TOKENS_ON_IKA_MESSAGE_VERSION: u8 = 1;
pub const ADD_TOKENS_ON_EVM_MESSAGE_VERSION: u8 = 1;

pub const BRIDGE_MESSAGE_PREFIX: &[u8] = b"IKA_BRIDGE_MESSAGE";
...
impl BridgeMessageEncoding for IkaToEthBridgeAction {
    fn as_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::new();
        let e = &self.ika_bridge_event;
        // Add message type
        ...
        // Add source chain id
        bytes.push(e.ika_chain_id as u8);
    ...
    fn as_payload_bytes(&self) -> Vec<u8> {
        ...
        bytes.push(IKA_ADDRESS_LENGTH as u8);
        // Add source address
        bytes.extend_from_slice(&e.ika_address.to_vec());
        // Add dest chain id
        ...
        // Add token amount
        bytes.extend_from_slice(&e.amount_ika_adjusted.to_be_bytes());

        bytes
    }
}

impl BridgeMessageEncoding for EthToIkaBridgeAction {
    fn as_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::new();
    ...
    fn as_payload_bytes(&self) -> Vec<u8> {
        ...
        // Add token amount
        bytes.extend_from_slice(&e.ika_adjusted_amount.to_be_bytes());
...
impl BridgeMessageEncoding for AddTokensOnIkaAction {
    fn as_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::new();
        // Add message type
        bytes.push(BridgeActionType::AddTokensOnIka as u8);
        // Add message version
        bytes.push(ADD_TOKENS_ON_IKA_MESSAGE_VERSION);
        // Add nonce
        bytes.extend_from_slice(&self.nonce.to_be_bytes());
        // Add chain id
        bytes.push(self.chain_id as u8);
...
impl BridgeMessageEncoding for AddTokensOnEvmAction {
    ...
    fn as_payload_bytes(&self) -> Vec<u8> {
        ...
        // Add token ika decimals
        // Unwrap: bcs serialization should not fail
        bytes.push(u8::try_from(self.token_ika_decimals.len()).unwrap());
        for token_ika_decimal in &self.token_ika_decimals {
            bytes.push(*token_ika_decimal);
        }
...
#[cfg(test)]
mod tests {
    use crate::abi::EthToIkaTokenBridgeV1;
    use crate::crypto::BridgeAuthorityKeyPair;
    use crate::crypto::BridgeAuthorityPublicKeyBytes;
    use crate::crypto::BridgeAuthoritySignInfo;
    use crate::events::EmittedIkaToEthTokenBridgeV1;
    ...
    use ika_types::bridge::TOKEN_ID_BTC;
    use ika_types::bridge::TOKEN_ID_USDC;
    ...
    fn test_bridge_message_encoding() -> anyhow::Result<()> {
        ...
        let ika_chain_id = BridgeChainId::IkaTestnet;
        let ika_tx_event_index = 1u16;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let ika_address = IkaAddress::random_for_testing_only();
        let eth_address = EthAddress::random();
        let token_id = TOKEN_ID_USDC;
        let amount_ika_adjusted = 1_000_000;

        let ika_bridge_event = EmittedIkaToEthTokenBridgeV1 {
            nonce,
            ika_chain_id,
            eth_chain_id,
            ika_address,
        ...
        let encoded_bytes = BridgeAction::IkaToEthBridgeAction(IkaToEthBridgeAction {
            ika_tx_digest,
            ika_tx_event_index,
            ika_bridge_event,
        })
        ...
        let source_chain_id_bytes = vec![ika_chain_id as u8]; // len: 1

        let ika_address_length_bytes = vec![IKA_ADDRESS_LENGTH as u8]; // len: 1
        let ika_address_bytes = ika_address.to_vec(); // len: 32
        ...
        let token_amount_bytes = amount_ika_adjusted.to_be_bytes().to_vec(); // len: 8
        ...
        combined_bytes.extend_from_slice(&ika_address_length_bytes);
        combined_bytes.extend_from_slice(&ika_address_bytes);
    ...
    #[test]
    fn test_bridge_message_encoding_regression_emitted_ika_to_eth_token_bridge_v1(
    ) -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        ...
        let ika_chain_id = BridgeChainId::IkaTestnet;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let ika_address = IkaAddress::from_str(
    ...
    #[test]
    fn test_bridge_message_encoding_regression_eth_to_ika_token_bridge_v1() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        ...
        let eth_address =
            EthAddress::from_str("0x00000000000000000000000000000000000000c8").unwrap();
        let token_id = TOKEN_ID_USDC;
        let ika_adjusted_amount = 12345;

        let eth_bridge_event = EthToIkaTokenBridgeV1 {
            nonce,
            ika_chain_id,
            eth_chain_id,
            ika_address,
        ...
        let encoded_bytes = BridgeAction::EthToIkaBridgeAction(EthToIkaBridgeAction {
            eth_tx_hash,
            eth_event_index,
            eth_bridge_event,
        })
    ...
    #[test]
    fn test_bridge_message_encoding_regression_add_coins_on_ika() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();

        let action = BridgeAction::AddTokensOnIkaAction(AddTokensOnIkaAction {
            nonce: 0,
            chain_id: BridgeChainId::IkaCustom,
            native: false,
