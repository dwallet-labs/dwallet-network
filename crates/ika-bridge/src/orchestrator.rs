...
//! 1. monitors Ika and Ethereum events with the help of `IkaSyncer` and `EthSyncer`
...
pub struct BridgeOrchestrator<C> {
    _ika_client: Arc<IkaClient<C>>,
    ika_events_rx: mysten_metrics::metered_channel::Receiver<(Identifier, Vec<IkaEvent>)>,
    eth_events_rx: mysten_metrics::metered_channel::Receiver<(EthAddress, u64, Vec<EthLog>)>,
    store: Arc<BridgeOrchestratorTables>,
    ika_monitor_tx: mysten_metrics::metered_channel::Sender<IkaBridgeEvent>,
    ...
    C: IkaClientInner + 'static,
{
    ...
    pub async fn run(
        ...
        task_handles.push(spawn_logged_monitored_task!(Self::run_ika_watcher(
            store_clone,
    ...
    async fn run_ika_watcher(
        store: Arc<BridgeOrchestratorTables>,
        executor_tx: mysten_metrics::metered_channel::Sender<BridgeActionExecutionWrapper>,
        mut ika_events_rx: mysten_metrics::metered_channel::Receiver<(Identifier, Vec<IkaEvent>)>,
        monitor_tx: mysten_metrics::metered_channel::Sender<IkaBridgeEvent>,
        metrics: Arc<BridgeMetrics>,
    ) {
        info!("Starting ika watcher task");
        while let Some((identifier, events)) = ika_events_rx.recv().await {
            if events.is_empty() {
                continue;
            }
            info!("Received {} Ika events: {:?}", events.len(), events);
            metrics
                .ika_watcher_received_events
                .inc_by(events.len() as u64);
            let bridge_events = events
                .iter()
                .filter_map(|ika_event| {
                    match IkaBridgeEvent::try_from_ika_event(ika_event) {
                        ...
                        Err(BridgeError::ZeroValueBridgeTransfer(_)) => {
                            error!("Zero value bridge transfer: {:?}", ika_event);
                            None
                        }
                        Err(e) => {
                            panic!(
                                "Ika Event could not be deserialzed to IkaBridgeEvent: {:?}",
                                e
                            );
                        }
                    }
            ...
            let mut actions = vec![];
            for (ika_event, opt_bridge_event) in events.iter().zip(bridge_events) {
                if opt_bridge_event.is_none() {
                    // TODO: we probably should not miss any events, log for now.
                    metrics.ika_watcher_unrecognized_events.inc();
                    error!("Ika event not recognized: {:?}", ika_event);
                    continue;
                }
                // Unwrap safe: checked above
                let bridge_event: IkaBridgeEvent = opt_bridge_event.unwrap();
                info!("Observed Ika bridge event: {:?}", bridge_event);
                ...
                if let Some(action) = bridge_event
                    .try_into_bridge_action(ika_event.id.tx_digest, ika_event.id.event_seq as u16)
                {
            ...
            if !actions.is_empty() {
                info!("Received {} actions from Ika: {:?}", actions.len(), actions);
                metrics
                    .ika_watcher_received_actions
            ...
            store
                .update_ika_event_cursor(identifier, cursor)
                .expect("Store operation should not fail");
        }
        panic!("Ika event channel was closed unexpectedly");
    }
...
mod tests {
    use crate::{
        test_utils::{get_test_eth_to_ika_bridge_action, get_test_log_and_action},
    ...
    use crate::test_utils::get_test_ika_to_eth_bridge_action;
    use crate::{events::tests::get_test_ika_event_and_action, ika_mock_client::IkaMockClient};

    #[tokio::test]
    async fn test_ika_watcher_task() {
        // Note: this test may fail because of the following reasons:
        // the IkaEvent's struct tag does not match the ones in events.rs
        ...
        let identifier = Identifier::from_str("test_ika_watcher_task").unwrap();
        let (ika_event, bridge_action) = get_test_ika_event_and_action(identifier.clone());
        ika_events_tx
            .send((identifier.clone(), vec![ika_event.clone()]))
            .await
            .unwrap();
        ...
        loop {
            ...
            assert_eq!(
                store.get_ika_event_cursors(&[identifier]).unwrap()[0].unwrap(),
                ika_event.id,
            );
            break;
        }
    ...
    async fn test_resume_actions_in_pending_logs() {
        ...
        let action1 = get_test_ika_to_eth_bridge_action(
            None,
            Some(0),
            Some(99),
            Some(10000),
        ...
        let action2 = get_test_eth_to_ika_bridge_action(None, None, None, None);
        store
    ...
    fn setup() -> (
        mysten_metrics::metered_channel::Sender<(Identifier, Vec<IkaEvent>)>,
        mysten_metrics::metered_channel::Receiver<(Identifier, Vec<IkaEvent>)>,
        ...
        let (ika_events_tx, ika_events_rx) = mysten_metrics::metered_channel::channel(
            100,
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["unit_test_ika_events_queue"]),
        );
