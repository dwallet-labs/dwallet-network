...
pub struct BridgeOrchestratorTables {
    ...
    pub(crate) ika_syncer_cursors: DBMap<Identifier, EventID>,
...
impl BridgeOrchestratorTables {
    ...
    pub(crate) fn update_ika_event_cursor(
        &self,
        module: Identifier,
        cursor: EventID,
    ) -> BridgeResult<()> {
        let mut batch = self.ika_syncer_cursors.batch();

        batch
            .insert_batch(&self.ika_syncer_cursors, [(module, cursor)])
            .map_err(|e| {
                BridgeError::StorageError(format!(
                    "Coudln't insert into ika_syncer_cursors: {:?}",
                    e
                ))
            })?;
        batch
    ...
    pub fn get_ika_event_cursors(
        &self,
        identifiers: &[Identifier],
    ) -> BridgeResult<Vec<Option<EventID>>> {
        self.ika_syncer_cursors.multi_get(identifiers).map_err(|e| {
            BridgeError::StorageError(format!("Couldn't get ika_syncer_cursors: {:?}", e))
        })
    }
...
mod tests {
    ...
    async fn test_bridge_storage_basic() {
        ...
        // update ika event cursor
        let ika_module = Identifier::from_str("test").unwrap();
        let ika_cursor = EventID {
            tx_digest: TransactionDigest::random(),
            event_seq: 1,
        };
        assert!(store.get_ika_event_cursors(&[ika_module.clone()]).unwrap()[0].is_none());
        store
            .update_ika_event_cursor(ika_module.clone(), ika_cursor)
            .unwrap();
        assert_eq!(
            store.get_ika_event_cursors(&[ika_module.clone()]).unwrap()[0].unwrap(),
            ika_cursor
        );
    }
}
