...
//! `BridgeMonitor` receives all `IkaBridgeEvent` and `EthBridgeEvent`
//! and handles them accordingly.

use crate::abi::{
    EthBridgeCommitteeEvents, EthBridgeConfigEvents, EthBridgeEvent, EthBridgeLimiterEvents,
    EthCommitteeUpgradeableContractEvents, EthIkaBridgeEvents,
};
use crate::client::bridge_authority_aggregator::BridgeAuthorityAggregator;
use crate::crypto::BridgeAuthorityPublicKeyBytes;
use crate::events::{BlocklistValidatorEvent, CommitteeMemberUrlUpdateEvent};
use crate::events::{EmergencyOpEvent, IkaBridgeEvent};
use crate::metrics::BridgeMetrics;
use crate::retry_with_max_elapsed_time;
use crate::ika_client::{IkaClient, IkaClientInner};
...
pub struct BridgeMonitor<C> {
    ...
    ika_monitor_rx: mysten_metrics::metered_channel::Receiver<IkaBridgeEvent>,
    ...
    C: IkaClientInner + 'static,
{
    ...
    pub async fn run(self) {
        ...
        let mut latest_token_config = (*ika_token_type_tags.load().clone()).clone();

        loop {
            tokio::select! {
                ika_event = ika_monitor_rx.recv() => {
                    if let Some(ika_event) = ika_event {
                        Self::handle_ika_events(
                            ika_event,
                            &ika_client,
                            &bridge_auth_agg,
                            &bridge_paused_watch_tx,
                        ...
                        panic!("BridgeMonitor ika events channel was closed unexpectedly");
                    }
                }
    ...
    async fn handle_ika_events(
        event: IkaBridgeEvent,
        ika_client: &Arc<IkaClient<C>>,
        bridge_auth_agg: &Arc<ArcSwap<BridgeAuthorityAggregator>>,
        bridge_paused_watch_tx: &tokio::sync::watch::Sender<IsBridgePaused>,
        ika_token_type_tags: &Arc<ArcSwap<HashMap<u8, TypeTag>>>,
        ...
        info!("Received IkaBridgeEvent: {:?}", event);
        macro_rules! bump_ika_counter {
            ($action:expr) => {
                bridge_metrics
                    .observed_governance_actions
                    .with_label_values(&[$action, "ika"])
                    .inc();
            };
        }

        match event {
            IkaBridgeEvent::IkaToEthTokenBridgeV1(_) => (),
            IkaBridgeEvent::TokenTransferApproved(_) => (),
            IkaBridgeEvent::TokenTransferClaimed(_) => (),
            IkaBridgeEvent::TokenTransferAlreadyApproved(_) => (),
            IkaBridgeEvent::TokenTransferAlreadyClaimed(_) => (),
            IkaBridgeEvent::TokenTransferLimitExceed(_) => {
                // TODO do we want to do anything here?
            }

            IkaBridgeEvent::EmergencyOpEvent(event) => {
                bump_ika_counter!(if event.frozen {
                    "bridge_paused"
                } else {
                    "bridge_unpaused"
                });
            ...
            IkaBridgeEvent::CommitteeMemberRegistration(_) => {
                bump_ika_counter!("committee_member_registered");
            }
            IkaBridgeEvent::CommitteeUpdateEvent(_) => {
                bump_ika_counter!("committee_updated");
            }

            IkaBridgeEvent::CommitteeMemberUrlUpdateEvent(event) => {
                bump_ika_counter!("committee_member_url_updated");
            ...
            IkaBridgeEvent::BlocklistValidatorEvent(event) => {
                bump_ika_counter!(if event.blocklisted {
            ...
            IkaBridgeEvent::TokenRegistrationEvent(_) => {
                bump_ika_counter!("new_token_registered");
            }

            IkaBridgeEvent::NewTokenEvent(event) => {
                bump_ika_counter!("new_token_added");
                    ...
                    latest_token_config.entry(event.token_id)
                {
                    ...
                    ika_token_type_tags.store(Arc::new(latest_token_config.clone()));
                } else {
                    // invariant
            ...
            IkaBridgeEvent::UpdateTokenPriceEvent(_event) => {
                bump_ika_counter!("token_price_updated");
            }

            IkaBridgeEvent::UpdateRouteLimitEvent(_event) => {
                bump_ika_counter!("limit_updated");
            }
        }
    }
    ...
    fn handle_eth_events(event: EthBridgeEvent, bridge_metrics: &Arc<BridgeMetrics>) {
        ...
        match event {
            ...
            EthBridgeEvent::EthIkaBridgeEvents(event) => match event {
                EthIkaBridgeEvents::TokensClaimedFilter(_) => (),
                EthIkaBridgeEvents::TokensDepositedFilter(_) => (),
                EthIkaBridgeEvents::PausedFilter(_) => bump_eth_counter!("bridge_paused"),
                EthIkaBridgeEvents::UnpausedFilter(_) => bump_eth_counter!("bridge_unpaused"),
                EthIkaBridgeEvents::UpgradedFilter(_) => {
                    bump_eth_counter!("bridge_contract_upgraded")
                }
                EthIkaBridgeEvents::InitializedFilter(_) => {
                    bump_eth_counter!("bridge_contract_initialized")
                }
                EthIkaBridgeEvents::ContractUpgradedFilter(_) => {
                    bump_eth_counter!("bridge_contract_upgraded")
                }
                EthIkaBridgeEvents::EmergencyOperationFilter(e) => {
                    if e.paused {
...
async fn get_latest_bridge_committee_with_url_update_event<C: IkaClientInner>(
    ika_client: Arc<IkaClient<C>>,
    ...
    loop {
        let Ok(Ok(committee)) = retry_with_max_elapsed_time!(
            ika_client.get_bridge_committee(),
...
async fn get_latest_bridge_committee_with_blocklist_event<C: IkaClientInner>(
    ika_client: Arc<IkaClient<C>>,
...
async fn get_latest_bridge_pause_status_with_emergency_event<C: IkaClientInner>(
    ika_client: Arc<IkaClient<C>>,
    ...
    loop {
        let Ok(Ok(summary)) =
            retry_with_max_elapsed_time!(ika_client.get_bridge_summary(), Duration::from_secs(600))
        else {
...
mod tests {
    ...
    use ika_types::bridge::BridgeCommitteeSummary;
    use ika_types::bridge::MoveTypeCommitteeMember;
    use ika_types::crypto::get_key_pair;

    use crate::{ika_mock_client::IkaMockClient, types::BridgeCommittee};
    ...
    async fn test_get_latest_bridge_committee_with_url_update_event() {
        ...
        ika_client_mock.set_bridge_committee(summary.clone());
        ...
        ika_client_mock.set_bridge_committee(old_summary.clone());
        let timer = std::time::Instant::now();
        // update the url to "http://new.url" in 1 second
        let ika_client_mock_clone = ika_client_mock.clone();
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            ika_client_mock_clone.set_bridge_committee(summary.clone());
        });
        let committee = get_latest_bridge_committee_with_url_update_event(
            ika_client.clone(),
            event.clone(),
        ...
        ika_client_mock.set_bridge_committee(newer_summary.clone());
        let timer = std::time::Instant::now();
        let committee = get_latest_bridge_committee_with_url_update_event(
            ika_client.clone(),
            event.clone(),
    ...
    async fn test_get_bridge_pause_status_with_emergency_event() {
        ...
        ika_client_mock.set_is_bridge_paused(BRIDGE_PAUSED);
        let timer = std::time::Instant::now();
        assert!(
        ...
        ika_client_mock.set_is_bridge_paused(BRIDGE_UNPAUSED);
        let timer = std::time::Instant::now();
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                ika_client.clone(),
        ...
        tokio::spawn(async move {
            tokio::time::sleep(Duration::from_secs(1)).await;
            ika_client_mock_clone.set_is_bridge_paused(BRIDGE_UNPAUSED);
        });
        assert!(
            !get_latest_bridge_pause_status_with_emergency_event(
                ika_client.clone(),
    ...
    async fn test_update_bridge_authority_aggregation_with_url_change_event() {
        ...
        let ika_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        ...
        ika_client_mock.set_bridge_committee(new_committee_summary.clone());
        ika_monitor_tx
            .send(IkaBridgeEvent::CommitteeMemberUrlUpdateEvent(
    ...
    async fn test_update_bridge_authority_aggregation_with_blocklist_event() {
        ...
        ika_client_mock.set_bridge_committee(new_committee_summary.clone());
        ika_monitor_tx
            .send(IkaBridgeEvent::BlocklistValidatorEvent(
    ...
    async fn test_update_bridge_pause_status_with_emergency_event() {
        ...
        ika_client_mock.set_is_bridge_paused(event.frozen);
        ika_monitor_tx
            .send(IkaBridgeEvent::EmergencyOpEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
    ...
    #[tokio::test]
    async fn test_update_ika_token_type_tags() {
        let (
            ika_monitor_tx,
            ika_monitor_rx,
            _eth_monitor_tx,
        ...
        let ika_token_type_tags = Arc::new(ArcSwap::from(Arc::new(HashMap::new())));
        let ika_token_type_tags_clone = ika_token_type_tags.clone();
        let _handle = tokio::task::spawn(
            BridgeMonitor::new(
                ika_client.clone(),
                ika_monitor_rx,
        ...
        ika_monitor_tx
            .send(IkaBridgeEvent::NewTokenEvent(event.clone()))
            .await
            .unwrap();
        // Wait for the monitor to process the event
        tokio::time::sleep(Duration::from_secs(1)).await;
        // Now expect new token type tags to appear in ika_token_type_tags
        ika_token_type_tags
            .load()
            .clone()
            .get(&event.token_id)
    ...
    fn setup() -> (
        mysten_metrics::metered_channel::Sender<IkaBridgeEvent>,
        mysten_metrics::metered_channel::Receiver<IkaBridgeEvent>,
        mysten_metrics::metered_channel::Sender<EthBridgeEvent>,
        mysten_metrics::metered_channel::Receiver<EthBridgeEvent>,
        IkaMockClient,
        Arc<IkaClient<IkaMockClient>>,
        ...
        let ika_client_mock = IkaMockClient::default();
        let ika_client = Arc::new(IkaClient::new_for_testing(ika_client_mock.clone()));
        let (ika_monitor_tx, ika_monitor_rx) = mysten_metrics::metered_channel::channel(
            10000,
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["ika_monitor_queue"]),
        );
