...
use crate::types::{
    ...
    BridgeActionType, EmergencyAction, EthLog, EthToIkaBridgeAction, EvmContractUpgradeAction,
    LimitUpdateAction, IkaToEthBridgeAction,
};
...
#[rustfmt::skip]
gen_eth_events!(
    EthIkaBridge, EthIkaBridgeEvents, "abi/ika_bridge.json",
...
impl EthBridgeEvent {
    pub fn try_into_bridge_action(
        ...
        Ok(match self {
            EthBridgeEvent::EthIkaBridgeEvents(event) => {
                match event {
                    EthIkaBridgeEvents::TokensDepositedFilter(event) => {
                        let bridge_event = match EthToIkaTokenBridgeV1::try_from(&event) {
                            Ok(bridge_event) => {
                                if bridge_event.ika_adjusted_amount == 0 {
                            ...
                            Err(e) => {
                                return Err(BridgeError::Generic(format!("Manual intervention is required. Failed to convert TokensDepositedFilter log to EthToIkaTokenBridgeV1. This indicates incorrect parameters or a bug in the code: {:?}. Err: {:?}", event, e)));
                            }
                        };

                        Some(BridgeAction::EthToIkaBridgeAction(EthToIkaBridgeAction {
                            eth_tx_hash,
                            eth_event_index,
                            eth_bridge_event: bridge_event,
                        }))
                    }
                    EthIkaBridgeEvents::TokensClaimedFilter(_event) => None,
                    EthIkaBridgeEvents::PausedFilter(_event) => None,
                    EthIkaBridgeEvents::UnpausedFilter(_event) => None,
                    EthIkaBridgeEvents::UpgradedFilter(_event) => None,
                    EthIkaBridgeEvents::InitializedFilter(_event) => None,
                    EthIkaBridgeEvents::ContractUpgradedFilter(_event) => None,
                    EthIkaBridgeEvents::EmergencyOperationFilter(_event) => None,
                }
            }
...
pub struct EthToIkaTokenBridgeV1 {
    pub nonce: u64,
    ...
    pub token_id: u8,
    pub ika_adjusted_amount: u64,
}

impl TryFrom<&TokensDepositedFilter> for EthToIkaTokenBridgeV1 {
    type Error = BridgeError;
    fn try_from(event: &TokensDepositedFilter) -> BridgeResult<Self> {
        Ok(Self {
            nonce: event.nonce,
            ika_chain_id: BridgeChainId::try_from(event.destination_chain_id)?,
            eth_chain_id: BridgeChainId::try_from(event.source_chain_id)?,
            ika_address: IkaAddress::from_bytes(event.recipient_address.as_ref())?,
            eth_address: event.sender_address,
            token_id: event.token_id,
            ika_adjusted_amount: event.ika_adjusted_amount,
        })
    }
}
...
impl From<IkaToEthBridgeAction> for eth_ika_bridge::Message {
    fn from(action: IkaToEthBridgeAction) -> Self {
        eth_ika_bridge::Message {
            message_type: BridgeActionType::TokenTransfer as u8,
            version: TOKEN_TRANSFER_MESSAGE_VERSION,
            nonce: action.ika_bridge_event.nonce,
            chain_id: action.ika_bridge_event.ika_chain_id as u8,
            payload: action.as_payload_bytes().into(),
        }
    }
}

impl From<ParsedTokenTransferMessage> for eth_ika_bridge::Message {
...
impl From<EmergencyAction> for eth_ika_bridge::Message {
...
mod tests {
    ...
    use hex_literal::hex;
    use std::str::FromStr;
    use ika_types::{bridge::TOKEN_ID_ETH, crypto::ToFromBytes};
    ...
    fn test_eth_message_conversion_emergency_action_regression() -> anyhow::Result<()> {
        ...
        let message: eth_ika_bridge::Message = action.into();
        assert_eq!(
            message,
            eth_ika_bridge::Message {
                message_type: BridgeActionType::EmergencyButton as u8,
    ...
    fn test_eth_message_conversion_update_limit_action_regression() -> anyhow::Result<()> {
        ...
        let action = LimitUpdateAction {
            nonce: 2,
            chain_id: BridgeChainId::EthSepolia,
            sending_chain_id: BridgeChainId::IkaTestnet,
    ...
    #[test]
    fn test_token_deposit_eth_log_to_ika_bridge_event_regression() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        ...
        assert_eq!(
            event,
            EthBridgeEvent::EthIkaBridgeEvents(EthIkaBridgeEvents::TokensDepositedFilter(
                TokensDepositedFilter {
                    source_chain_id: 12,
                    nonce: 0,
                    destination_chain_id: 2,
                    token_id: 2,
                    ika_adjusted_amount: 4200000000,
    ...
    #[test]
    fn test_0_ika_amount_conversion_for_eth_event() {
        let e = EthBridgeEvent::EthIkaBridgeEvents(EthIkaBridgeEvents::TokensDepositedFilter(
            TokensDepositedFilter {
                source_chain_id: BridgeChainId::EthSepolia as u8,
                nonce: 0,
                destination_chain_id: BridgeChainId::IkaTestnet as u8,
                token_id: 2,
                ika_adjusted_amount: 1,
                sender_address: EthAddress::random(),
                recipient_address: ethers::types::Bytes::from(
                    IkaAddress::random_for_testing_only().to_vec(),
                ),
            },
        ));
        assert!(e
