...
//! A mock implementation of Ika JSON-RPC client.
...
use ika_json_rpc_types::{EventFilter, EventPage, IkaEvent};
...
pub struct IkaMockClient {
    ...
    events_by_tx_digest:
        Arc<Mutex<HashMap<TransactionDigest, Result<Vec<IkaEvent>, ika_sdk::error::Error>>>>,
    transaction_responses:
        Arc<Mutex<HashMap<TransactionDigest, BridgeResult<IkaTransactionBlockResponse>>>>,
    wildcard_transaction_response: Arc<Mutex<Option<BridgeResult<IkaTransactionBlockResponse>>>>,
...
impl IkaMockClient {
    ...
    pub fn add_events_by_tx_digest(&self, tx_digest: TransactionDigest, events: Vec<IkaEvent>) {
    ...
    pub fn add_events_by_tx_digest_error(&self, tx_digest: TransactionDigest) {
        self.events_by_tx_digest.lock().unwrap().insert(
            tx_digest,
            Err(ika_sdk::error::Error::DataError("".to_string())),
        );
    }
    ...
    pub fn add_transaction_response(
        ...
        response: BridgeResult<IkaTransactionBlockResponse>,
    ) {
...
#[async_trait]
impl IkaClientInner for IkaMockClient {
    ...
    async fn get_events_by_tx_digest(
            ...
            .unwrap_or_else(|| panic!("No preset events found for tx_digest: {:?}", tx_digest))
        {
            ...
            // ika_sdk::error::Error is not Clone
            Err(_) => Err(ika_sdk::error::Error::DataError("".to_string())),
        }
    }
