...
use crate::ika_bridge_watchdog::eth_bridge_status::EthBridgeStatus;
use crate::ika_bridge_watchdog::eth_vault_balance::{EthereumVaultBalance, VaultAsset};
use crate::ika_bridge_watchdog::metrics::WatchdogMetrics;
use crate::ika_bridge_watchdog::ika_bridge_status::IkaBridgeStatus;
use crate::ika_bridge_watchdog::total_supplies::TotalSupplies;
use crate::ika_bridge_watchdog::{BridgeWatchDog, Observable};
...
pub async fn run_bridge_node(
    ...
    let ika_chain_identifier = server_config
        .ika_client
        .get_chain_identifier()
        .await
        .map_err(|e| anyhow::anyhow!("Failed to get ika chain identifier: {:?}", e))?;
    ...
    let ika_client = server_config.ika_client.clone();
    ...
    if let Some(client_config) = client_config {
        let committee_keys_to_names =
            Arc::new(get_validator_names_by_pub_keys(&committee, &ika_system).await);
    ...
    let committee_name_mapping = get_committee_voting_power_by_name(&committee, &ika_system).await;
...
async fn start_watchdog(
    ...
    let ika_bridge_status = IkaBridgeStatus::new(
        ika_client.clone(),
        watchdog_metrics.ika_bridge_paused.clone(),
    );
    ...
    if let Some(watchdog_config) = watchdog_config {
        if !watchdog_config.total_supplies.is_empty() {
            let total_supplies = TotalSupplies::new(
                Arc::new(ika_client.ika_client().clone()),
...
async fn start_client_components(
    ...
    let ika_modules_to_watch = get_ika_modules_to_watch(
        &store,
        client_config.ika_bridge_module_last_processed_event_id_override,
    );
    ...
    let ika_client = client_config.ika_client.clone();
    ...
    let (task_handles, ika_events_rx) = IkaSyncer::new(
        client_config.ika_client,
        ika_modules_to_watch,
        metrics.clone(),
    )
    .run(Duration::from_secs(2))
    .await
    .expect("Failed to start ika syncer");
    all_handles.extend(task_handles);
    ...
    let is_bridge_paused = ika_client.is_bridge_paused().await.unwrap();
    ...
    let ika_token_type_tags = Arc::new(ArcSwap::from(Arc::new(ika_token_type_tags)));
    let bridge_action_executor = BridgeActionExecutor::new(
        ika_client.clone(),
        bridge_auth_agg.clone(),
        store.clone(),
        client_config.key,
        client_config.ika_address,
...
fn get_ika_modules_to_watch(
    store: &std::sync::Arc<BridgeOrchestratorTables>,
    ika_bridge_module_last_processed_event_id_override: Option<EventID>,
) -> HashMap<Identifier, Option<EventID>> {
    let ika_bridge_modules = vec![
        BRIDGE_MODULE_NAME.to_owned(),
        BRIDGE_COMMITTEE_MODULE_NAME.to_owned(),
        BRIDGE_TREASURY_MODULE_NAME.to_owned(),
        BRIDGE_LIMITER_MODULE_NAME.to_owned(),
    ];
    if let Some(cursor) = ika_bridge_module_last_processed_event_id_override {
        info!("Overriding cursor for ika bridge modules to {:?}", cursor);
    ...
    let ika_bridge_module_stored_cursor = store
        .get_ika_event_cursors(&ika_bridge_modules)
        .expect("Failed to get eth ika event cursors from storage");
    let mut ika_modules_to_watch = HashMap::new();
    for (module_identifier, cursor) in ika_bridge_modules
        .iter()
        .zip(ika_bridge_module_stored_cursor)
    {
        if cursor.is_none() {
            info!(
                "No cursor found for ika bridge module {} in storage or config override, query start from the beginning.",
                module_identifier
            );
        }
        ika_modules_to_watch.insert(module_identifier.clone(), cursor);
    }
    ika_modules_to_watch
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_ika_modules_to_watch() {
        telemetry_subscribers::init_for_testing();
        let temp_dir = tempfile::tempdir().unwrap();
        ...
        let ika_modules_to_watch = get_ika_modules_to_watch(&store, None);
        assert_eq!(
            ika_modules_to_watch,
            vec![
        ...
        let ika_modules_to_watch = get_ika_modules_to_watch(&store, Some(override_cursor));
        assert_eq!(
            ika_modules_to_watch,
            vec![
        ...
        store
            .update_ika_event_cursor(bridge_module.clone(), stored_cursor)
            .unwrap();
        let ika_modules_to_watch = get_ika_modules_to_watch(&store, None);
        assert_eq!(
            ika_modules_to_watch,
        ...
        store
            .update_ika_event_cursor(committee_module.clone(), stored_cursor)
            .unwrap();
        let ika_modules_to_watch = get_ika_modules_to_watch(&store, Some(override_cursor));
        assert_eq!(
            ika_modules_to_watch,
    ...
    async fn test_starting_bridge_node() {
        ...
        let config = BridgeNodeConfig {
            ...
            ika: IkaConfig {
                ika_rpc_url: bridge_test_cluster.ika_rpc_url(),
            ...
            eth: EthConfig {
                ...
                eth_bridge_proxy_address: bridge_test_cluster.ika_bridge_address(),
    ...
    async fn test_starting_bridge_node_with_client() {
        ...
        let client_ika_address = IkaAddress::from(kp.public());
        let sender_address = bridge_test_cluster.ika_user_address();
        // send some gas to this address
        bridge_test_cluster
            .test_cluster
            .inner
            .transfer_ika_must_exceed(sender_address, client_ika_address, 1000000000)
            .await;
        ...
        let config = BridgeNodeConfig {
            ...
            ika: IkaConfig {
                ...
                bridge_client_key_path: None,
                bridge_client_gas_object: None,
                ika_bridge_module_last_processed_event_id_override: Some(EventID {
                    tx_digest: TransactionDigest::random(),
                    event_seq: 0,
                }),
            },
