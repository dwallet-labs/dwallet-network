...
use ika_types::bridge::{
    BRIDGE_CREATE_ADD_TOKEN_ON_IKA_MESSAGE_FUNCTION_NAME,
...
use ika_types::{
    base_types::{ObjectRef, IkaAddress},
    programmable_transaction_builder::ProgrammableTransactionBuilder,
    transaction::{ObjectArg, TransactionData},
    TypeTag,
};
use ika_types::{Identifier, BRIDGE_PACKAGE_ID};
...
pub fn build_ika_transaction(
    client_address: IkaAddress,
    ...
    // TODO: Check chain id?
    match action.data() {
        BridgeAction::EthToIkaBridgeAction(_) => build_token_bridge_approve_transaction(
            client_address,
            gas_object_ref,
            action,
            true,
        ...
        BridgeAction::IkaToEthBridgeAction(_) => build_token_bridge_approve_transaction(
            client_address,
            gas_object_ref,
            action,
            false,
        ...
        BridgeAction::EvmContractUpgradeAction(_) => {
            // It does not need a Ika tranaction to execute EVM contract upgrade
            unreachable!()
        }
        BridgeAction::AddTokensOnIkaAction(_) => build_add_tokens_on_ika_transaction(
            client_address,
            gas_object_ref,
            action,
            bridge_object_arg,
        ...
        BridgeAction::AddTokensOnEvmAction(_) => {
            // It does not need a Ika tranaction to add tokens on EVM
            unreachable!()
        }
    }
}
...
fn build_token_bridge_approve_transaction(
    ...
    let (source_chain, seq_num, sender, target_chain, target, token_type, amount) =
        match bridge_action {
            BridgeAction::IkaToEthBridgeAction(a) => {
                let bridge_event = a.ika_bridge_event;
                (
                    bridge_event.ika_chain_id,
                    bridge_event.nonce,
                    bridge_event.ika_address.to_vec(),
                    bridge_event.eth_chain_id,
                    bridge_event.eth_address.to_fixed_bytes().to_vec(),
                    bridge_event.token_id,
                    bridge_event.amount_ika_adjusted,
                )
            }
            BridgeAction::EthToIkaBridgeAction(a) => {
                ...
                (
                    ...
                    bridge_event.ika_address.to_vec(),
                    bridge_event.token_id,
                    bridge_event.ika_adjusted_amount,
                )
            }
            _ => unreachable!(),
        };
    ...
    builder.programmable_move_call(
        BRIDGE_PACKAGE_ID,
        ika_types::bridge::BRIDGE_MODULE_NAME.to_owned(),
...
pub fn build_add_tokens_on_ika_transaction(
    client_address: IkaAddress,
    gas_object_ref: &ObjectRef,
    action: VerifiedCertifiedBridgeAction,
    bridge_object_arg: ObjectArg,
    ...
    let (source_chain, seq_num, native, token_ids, token_type_names, token_prices) =
        match bridge_action {
            BridgeAction::AddTokensOnIkaAction(a) => (
                a.chain_id,
                a.nonce,
                a.native,
                a.token_ids,
    ...
    let message_arg = builder.programmable_move_call(
        ...
        BRIDGE_CREATE_ADD_TOKEN_ON_IKA_MESSAGE_FUNCTION_NAME.into(),
        vec![],
        vec![
            source_chain,
            seq_num,
...
pub fn build_committee_register_transaction(
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    let system_state = builder.obj(ObjectArg::IKA_SYSTEM_MUT).unwrap();
...
mod tests {
    ...
    use std::sync::Arc;
    use ika_types::bridge::{BridgeChainId, TOKEN_ID_BTC, TOKEN_ID_USDC};
    use ika_types::crypto::get_key_pair;
    use ika_types::crypto::ToFromBytes;

    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_ika_transaction_for_token_transfer() {
        ...
        let ika_client = IkaClient::new(&test_cluster.inner.fullnode_handle.rpc_url, metrics)
            .await
            .unwrap();
        let bridge_authority_keys = test_cluster.authority_keys_clone();

        // Note: We don't call `ika_client.get_bridge_committee` here because it will err if the committee
        ...
        // 1. Test Eth -> Ika Transfer approval
        ...
        // 2. Test Ika -> Eth Transfer approval
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_ika_transaction_for_emergency_op() {
        telemetry_subscribers::init_for_testing();
        let num_valdiator = 2;
        let mut bridge_keys = vec![];
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let summary = ika_client.get_bridge_summary().await.unwrap();
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_ika_transaction_for_committee_blocklist() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let committee = ika_client.get_bridge_summary().await.unwrap().committee;
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_ika_transaction_for_limit_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        .await;
        let new_transfer_limit = ika_client
            .get_bridge_summary()
            .await
            .unwrap()
            .limiter
        ...
        for limit in new_transfer_limit {
            if limit.0 == BridgeChainId::EthCustom && limit.1 == BridgeChainId::IkaCustom {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_ika_transaction_for_price_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let notional_values = ika_client.get_notional_values().await.unwrap();
        ...
        .await;
        let new_notional_values = ika_client.get_notional_values().await.unwrap();
