...
use crate::{
    ...
    ika_client::{IkaClient, IkaClientInner},
    ika_transaction_builder::build_ika_transaction,
...
pub struct BridgeActionExecutor<C> {
    ...
    ika_token_type_tags: Arc<ArcSwap<HashMap<u8, TypeTag>>>,
    ...
    C: IkaClientInner + 'static,
{
    ...
    fn run_inner(
        ...
        let client_clone = self.ika_client.clone();
        let mut tasks = vec![];
    ...
    async fn should_proceed_signing(ika_client: &Arc<IkaClient<C>>) -> bool {
        let Ok(Ok(is_paused)) =
            retry_with_max_elapsed_time!(ika_client.is_bridge_paused(), Duration::from_secs(600))
        else {
    ...
    async fn handle_signing_task(
        ...
        let should_proceed = Self::should_proceed_signing(ika_client).await;
        if !should_proceed {
        ...
        let ika_client_clone = ika_client.clone();
    ...
    async fn handle_already_processed_token_transfer_action_maybe(
        ...
        match status {
            ...
            // Although theoretically a legit IkaToEthBridgeAction should not have
            // status `NotFound`
    ...
    async fn request_signatures(
        ...
        match &action {
            BridgeAction::IkaToEthBridgeAction(_) | BridgeAction::EthToIkaBridgeAction(_) => (),
    ...
    // Before calling this function, `key` and `ika_address` need to be
    // verified to match.
    ...
    async fn handle_execution_task(
        ...
        >,
        bridge_object_arg: &ObjectArg,
        ika_token_type_tags: &ArcSwap<HashMap<u8, TypeTag>>,
        metrics: &Arc<BridgeMetrics>,
    ) {
        metrics
        ...
        let (gas_coin, gas_object_ref) =
            Self::get_gas_data_assert_ownership(*ika_address, gas_object_id, ika_client).await;
        ...
        if Self::handle_already_processed_token_transfer_action_maybe(
            ika_client, action, store, metrics,
        )
        .await
        {
        ...
        info!("Building Ika transaction");
        let rgp = ika_client.get_reference_gas_price_until_success().await;
        let tx_data = match build_ika_transaction(
            *ika_address,
            &gas_object_ref,
            ceriticate_clone,
            *bridge_object_arg,
            ika_token_type_tags.load().as_ref(),
            rgp,
        ) {
            Ok(tx_data) => tx_data,
            Err(err) => {
                metrics.err_build_ika_transaction.inc();
                error!(
        ...
        let sig = Signature::new_secure(
            &IntentMessage::new(Intent::ika_transaction(), &tx_data),
            ika_key,
        );
        ...
        info!(?tx_digest, ?gas_object_ref, "Sending transaction to Ika");
        match ika_client
            ...
            .await
        {
            ...
            Err(err) => {
                error!(
                    ?action_key,
                    ?tx_digest,
                    "Ika transaction failed at signing: {err:?}"
                );
                metrics.err_ika_transaction_submission.inc();
                let metrics_clone = metrics.clone();
                ...
                spawn_logged_monitored_task!(async move {
                    ...
                    if attempt_times >= MAX_EXECUTION_ATTEMPTS {
                        metrics_clone
                            .err_ika_transaction_submission_too_many_failures
                            .inc();
    ...
    async fn handle_execution_effects(
        ...
        match status {
            IkaExecutionStatus::Success => {
                ...
                info!(?tx_digest, "Ika transaction executed successfully");
                store
                    .remove_pending_actions(&[action.digest()])
                    .unwrap_or_else(|e| {
                        panic!("Write to DB should not fail: {:?}", e);
            ...
            IkaExecutionStatus::Failure { error } => {
                ...
                metrics.err_ika_transaction_execution.inc();
                error!(?tx_digest, "Manual intervention is needed. Ika transaction executed and failed with error: {error:?}");
            }
        }
    }
    ...
    async fn get_gas_data_assert_ownership(
        ika_address: IkaAddress,
        gas_object_id: ObjectID,
        ika_client: &IkaClient<C>,
    ) -> (GasCoin, ObjectRef) {
        let (gas_coin, gas_obj_ref, owner) = ika_client
        ...
        assert_eq!(
            owner,
            Owner::AddressOwner(ika_address),
...
mod tests {
    ...
    use ika_json_rpc_types::IkaTransactionBlockEvents;
    use ika_json_rpc_types::{IkaEvent, IkaTransactionBlockResponse};
    use ika_types::crypto::get_key_pair;
    use ika_types::gas_coin::GasCoin;
    use ika_types::TypeTag;
    use ika_types::{base_types::random_object_ref, transaction::TransactionData};
    ...
    use crate::{
        ...
        test_utils::{
            get_test_authorities_and_run_mock_bridge_server, get_test_eth_to_ika_bridge_action,
            get_test_ika_to_eth_bridge_action, sign_action_with_key,
        },
    ...
    async fn test_onchain_execution_loop() {
        ...
        let id_token_map = (*ika_token_type_tags.load().clone()).clone();
        let tx_data = build_ika_transaction(
            ika_address,
            &gas_object_ref,
            action_certificate,
        ...
        let tx_digest = get_tx_digest(tx_data, &dummy_ika_key);

        let gas_coin = GasCoin::new_for_testing(1_000_000_000_000); // dummy gas coin
        ika_client_mock.add_gas_object_info(
            gas_coin.clone(),
            gas_object_ref,
            Owner::AddressOwner(ika_address),
        );
        ...
        let mut event = IkaEvent::random_for_testing();
    ...
    async fn test_signature_aggregation_loop() {
        ...
        let id_token_map = (*ika_token_type_tags.load().clone()).clone();
        let (action_certificate, ika_tx_digest, ika_tx_event_index) =
        ...
        loop {
            let requested_times =
                mock0.get_ika_token_events_requested(ika_tx_digest, ika_tx_event_index);
    ...
    async fn test_skip_request_signature_if_already_processed_on_chain() {
        let (
            ...
            _gas_object_ref,
            _ika_address,
            _ika_token_type_tags,
            _bridge_pause_tx,
        ) = setup().await;

        let ika_tx_digest = TransactionDigest::random();
        let ika_tx_event_index = 0;
        let action = get_test_ika_to_eth_bridge_action(
            Some(ika_tx_digest),
            Some(ika_tx_event_index),
            None,
            None,
            None,
            None,
        ...
        ika_client_mock.set_action_onchain_status(&action, BridgeActionStatus::Approved);
    ...
    async fn test_skip_tx_submission_if_already_processed_on_chain() {
        ...
        ika_client_mock.set_action_onchain_status(&action, BridgeActionStatus::Pending);
    ...
    async fn test_skip_tx_submission_if_bridge_is_paused() {
        ...
        let id_token_map: HashMap<u8, TypeTag> = (*ika_token_type_tags.load().clone()).clone();
        let (action_certificate, _, _) = get_bridge_authority_approved_action(
            vec![&mock0, &mock1, &mock2, &mock3],
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
            None,
    ...
    async fn test_action_executor_handle_new_token() {
        ...
        let mut id_token_map: HashMap<u8, TypeTag> = (*ika_token_type_tags.load().clone()).clone();
        let (action_certificate, _, _) = get_bridge_authority_approved_action(
            vec![&mock0, &mock1, &mock2, &mock3],
            vec![&secrets[0], &secrets[1], &secrets[2], &secrets[3]],
            Some(new_token_id),
            false, // we need an eth -> ika action that entails the new token type tag in transaction building
        );
        ...
        ika_token_type_tags.store(Arc::new(id_token_map));
    ...
    fn get_bridge_authority_approved_action(
        ...
        let ika_tx_digest = TransactionDigest::random();
        let ika_tx_event_index = 1;
        let action = if ika_to_eth {
            ...
            get_test_eth_to_ika_bridge_action(None, None, None, token_id)
        };

        let sigs =
            mock_bridge_authority_sigs(mocks, &action, secrets, ika_tx_digest, ika_tx_event_index);
        let certified_action = CertifiedBridgeAction::new_from_data_and_sig(
            action,
            BridgeCommitteeValiditySignInfo { signatures: sigs },
        );
    ...
    fn get_tx_digest(tx_data: TransactionData, dummy_ika_key: &IkaKeyPair) -> TransactionDigest {
        let sig = Signature::new_secure(
            &IntentMessage::new(Intent::ika_transaction(), &tx_data),
            dummy_ika_key,
        );
    ...
    fn mock_transaction_response(
        ika_client_mock: &IkaMockClient,
        tx_digest: TransactionDigest,
        status: IkaExecutionStatus,
        events: Option<Vec<IkaEvent>>,
        wildcard: bool,
    ) {
        let mut response = IkaTransactionBlockResponse::new(tx_digest);
        let effects = IkaTransactionBlockEffects::new_for_testing(tx_digest, status);
        if let Some(events) = events {
            response.events = Some(IkaTransactionBlockEvents { data: events });
        }
        response.effects = Some(effects);
        if wildcard {
            ika_client_mock.set_wildcard_transaction_response(Ok(response));
        } else {
            ika_client_mock.add_transaction_response(tx_digest, Ok(response));
        }
    }
    ...
    fn mock_transaction_error(
        ...
        if wildcard {
            ika_client_mock.set_wildcard_transaction_response(Err(error));
        } else {
            ika_client_mock.add_transaction_response(tx_digest, Err(error));
        }
    }
    ...
    async fn setup() -> (
        ...
        let (ika_address, kp): (_, fastcrypto::secp256k1::Secp256k1KeyPair) = get_key_pair();
        let ika_key = IkaKeyPair::from(kp);
        let gas_object_ref = random_object_ref();
        let temp_dir = tempfile::tempdir().unwrap();
        let store = BridgeOrchestratorTables::new(temp_dir.path());
        let ika_client_mock = IkaMockClient::default();
        let tx_subscription = ika_client_mock.subscribe_to_requested_transactions();
        let ika_client = Arc::new(IkaClient::new_for_testing(ika_client_mock.clone()));
        ...
        let dummy_ika_key = IkaKeyPair::from(dummy_kp);
        ...
        let ika_token_type_tags = ika_client.get_token_id_map().await.unwrap();
        let ika_token_type_tags = Arc::new(ArcSwap::new(Arc::new(ika_token_type_tags)));
