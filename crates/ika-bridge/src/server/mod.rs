...
use crate::{
    ...
    types::{
        AddTokensOnEvmAction, AddTokensOnIkaAction, AssetPriceUpdateAction,
...
use ika_types::{bridge::BridgeChainId, TypeTag};
...
pub const ETH_TO_IKA_TX_PATH: &str = "/sign/bridge_tx/eth/ika/:tx_hash/:event_index";
pub const IKA_TO_ETH_TX_PATH: &str = "/sign/bridge_tx/ika/eth/:tx_digest/:event_index";
...
pub const ADD_TOKENS_ON_IKA_PATH: &str =
...
pub(crate) fn make_router(
    ...
    Router::new()
        ...
        .route(ETH_TO_IKA_TX_PATH, get(handle_eth_tx_hash))
        .route(IKA_TO_ETH_TX_PATH, get(handle_ika_tx_digest))
        .route(
        ...
        .route(ADD_TOKENS_ON_IKA_PATH, get(handle_add_tokens_on_ika))
...
async fn handle_ika_tx_digest(
    ...
    let future = async {
        let sig: Json<SignedBridgeAction> = handler
            .handle_ika_tx_digest(tx_digest_base58, event_idx)
            .await?;
        Ok(sig)
    };
    with_metrics!(metrics.clone(), "handle_ika_tx_digest", future).await
}
...
async fn handle_add_tokens_on_ika(
    ...
    let future = async {
        ...
        if !chain_id.is_ika_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_ika only expects Ika chain id".to_string(),
            ));
        }
    ...
    with_metrics!(metrics.clone(), "handle_add_tokens_on_ika", future).await
}

#[instrument(level = "error", skip_all, fields(chain_id=chain_id, nonce=nonce, native=native, token_ids=token_ids, token_addresses=token_addresses, token_ika_decimals=token_ika_decimals, token_prices=token_prices))]
async fn handle_add_tokens_on_evm(
    Path((chain_id, nonce, native, token_ids, token_addresses, token_ika_decimals, token_prices)): Path<(
        u8,
        u64,
        u8,
        String,
    ...
    let future = async {
        ...
        if chain_id.is_ika_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_evm does not expect Ika chain id".to_string(),
            ));
        }
        ...
        let token_addresses = token_addresses
            ...
            .collect::<Result<Vec<_>, _>>()?;
        let token_ika_decimals = token_ika_decimals
            .split(',')
            .map(|s| {
                s.parse::<u8>().map_err(|err| {
                    BridgeError::InvalidBridgeClientRequest(format!(
                        "Invalid token ika decimals: {:?}",
                        err
                    ))
                })
            })
...
mod tests {
    ...
    #[tokio::test]
    async fn test_bridge_server_handle_add_tokens_on_ika_action_path() {
        let client = setup();
