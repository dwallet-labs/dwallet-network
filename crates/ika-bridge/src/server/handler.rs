...
pub trait BridgeRequestHandlerTrait {
    ...
    /// from Ethereum to Ika. The inputs are a transaction hash on Ethereum
    ...
    /// Handles a request to sign a BridgeAction that bridges assets
    /// from Ika to Ethereum. The inputs are a transaction digest on Ika
    /// that emitted the bridge event and the Event index in that transaction
    async fn handle_ika_tx_digest(
        &self,
...
#[async_trait::async_trait]
impl<C> ActionVerifier<(TransactionDigest, u16)> for IkaActionVerifier<C>
where
    C: IkaClientInner + Send + Sync + 'static,
{
    fn name(&self) -> &'static str {
        "IkaActionVerifier"
    }
    ...
    async fn verify(&self, key: (TransactionDigest, u16)) -> BridgeResult<BridgeAction> {
        ...
        self.ika_client
            ...
            .await
            .tap_ok(|action| info!("Ika action found: {:?}", action))
    }
}
...
pub struct BridgeRequestHandler {
    ika_signer_tx: mysten_metrics::metered_channel::Sender<(
...
impl BridgeRequestHandler {
    pub fn new<
        SC: IkaClientInner + Send + Sync + 'static,
        EP: JsonRpcClient + Send + Sync + 'static,
    >(
        signer: BridgeAuthorityKeyPair,
        ika_client: Arc<IkaClient<SC>>,
        eth_client: Arc<EthClient<EP>>,
        approved_governance_actions: Vec<BridgeAction>,
        metrics: Arc<BridgeMetrics>,
    ) -> Self {
        let (ika_signer_tx, ika_rx) = mysten_metrics::metered_channel::channel(
            1000,
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["server_ika_action_signing_queue"]),
        );
...
impl BridgeRequestHandlerTrait for BridgeRequestHandler {
    ...
    async fn handle_ika_tx_digest(
        ...
        self.ika_signer_tx
            ...
            .await
            .unwrap_or_else(|_| panic!("Server ika signing channel is closed"));
        let signed_action = rx
            .await
            .unwrap_or_else(|_| panic!("Server signing task's oneshot channel is dropped"))?;
        Ok(Json(signed_action))
...
mod tests {
    ...
    use crate::{
        ...
        events::{init_all_struct_tags, MoveTokenDepositedEvent, IkaToEthTokenBridgeV1},
        ika_mock_client::IkaMockClient,
        test_utils::{
            get_test_log_and_action, get_test_ika_to_eth_bridge_action, mock_last_finalized_block,
        },
    ...
    use ika_types::bridge::{BridgeChainId, TOKEN_ID_USDC};
    use ika_types::{base_types::IkaAddress, crypto::get_key_pair};

    #[tokio::test]
    async fn test_ika_signer_with_cache() {
        let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        let signer = Arc::new(kp);
        let ika_client_mock = IkaMockClient::default();
        let ika_verifier = IkaActionVerifier {
            ika_client: Arc::new(IkaClient::new_for_testing(ika_client_mock.clone())),
        };
        let metrics = Arc::new(BridgeMetrics::new_for_testing());
        let mut ika_signer_with_cache = SignerWithCache::new(signer.clone(), ika_verifier, metrics);

        // Test `get_cache_entry` creates a new entry if not exist
        let ika_tx_digest = TransactionDigest::random();
        let ika_event_idx = 42;
        assert!(ika_signer_with_cache
            .get_testing_only((ika_tx_digest, ika_event_idx))
            .await
            .is_none());
        let entry = ika_signer_with_cache
            .get_cache_entry((ika_tx_digest, ika_event_idx))
            .await;
        let entry_ = ika_signer_with_cache
            .get_testing_only((ika_tx_digest, ika_event_idx))
            .await;
        ...
        ika_client_mock.add_events_by_tx_digest_error(ika_tx_digest);
        ika_signer_with_cache
            .sign((ika_tx_digest, ika_event_idx))
            .await
            .unwrap_err();
        let entry_ = ika_signer_with_cache
            .get_testing_only((ika_tx_digest, ika_event_idx))
        ...
        ika_client_mock.add_events_by_tx_digest(ika_tx_digest, vec![]);
        assert!(matches!(
            ika_signer_with_cache
                .sign((ika_tx_digest, ika_event_idx))
                .await,
        ...
        // TODO: test BridgeEventInUnrecognizedIkaPackage, IkaBridgeEvent::try_from_ika_event
        // and BridgeEventNotActionable to be cached

        // Test `sign` caches Ok result
        let emitted_event_1 = MoveTokenDepositedEvent {
            seq_num: 1,
            source_chain: BridgeChainId::IkaCustom as u8,
        ...
        let ika_tx_digest = ika_event_1.id.tx_digest;

        let mut ika_event_2 = IkaEvent::random_for_testing();
        ika_event_2.type_ = IkaToEthTokenBridgeV1.get().unwrap().clone();
        ika_event_2.bcs = bcs::to_bytes(&emitted_event_1).unwrap();
        let ika_event_idx_2 = 1;
        ika_client_mock.add_events_by_tx_digest(ika_tx_digest, vec![ika_event_2.clone()]);

        ika_client_mock.add_events_by_tx_digest(
            ika_tx_digest,
            vec![ika_event_1.clone(), ika_event_2.clone()],
        );
        let signed_1 = ika_signer_with_cache
            .sign((ika_tx_digest, ika_event_idx))
            .await
            .unwrap();
        let signed_2 = ika_signer_with_cache
            .sign((ika_tx_digest, ika_event_idx_2))
            .await
            .unwrap();
    ...
    async fn test_signer_with_governace_verifier() {
        ...
        let action_2 = BridgeAction::LimitUpdateAction(LimitUpdateAction {
            chain_id: BridgeChainId::EthCustom,
            sending_chain_id: BridgeChainId::IkaCustom,
            nonce: 1,
            new_usd_limit: 10000,
        });
        ...
        let action_4 = get_test_ika_to_eth_bridge_action(None, None, None, None, None, None, None);
        assert!(matches!(
