...
use ika_types::bridge::{
    ...
    APPROVAL_THRESHOLD_ADD_TOKENS_ON_IKA, BRIDGE_COMMITTEE_MAXIMAL_VOTING_POWER,
...
use ika_types::digests::{Digest, TransactionDigest};
...
pub struct IkaToEthBridgeAction {
    // Digest of the transaction where the event was emitted
    pub ika_tx_digest: TransactionDigest,
    // The index of the event in the transaction
    pub ika_tx_event_index: u16,
    pub ika_bridge_event: EmittedIkaToEthTokenBridgeV1,
}

#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct EthToIkaBridgeAction {
    // Digest of the transaction where the event was emitted
    pub eth_tx_hash: EthTransactionHash,
    // The index of the event in the transaction
    pub eth_event_index: u16,
    pub eth_bridge_event: EthToIkaTokenBridgeV1,
}
...
pub struct LimitUpdateAction {
    ...
    // for the limit update. For example, if chain_id is EthMainnet and sending_chain_id is IkaMainnet,
    // it means we want to update the limit for the IkaMainnet to EthMainnet route.
    pub chain_id: BridgeChainId,
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct AddTokensOnIkaAction {
    pub nonce: u64,
    pub chain_id: BridgeChainId,
    pub native: bool,
...
pub enum BridgeAction {
    /// Ika to Eth bridge action
    IkaToEthBridgeAction(IkaToEthBridgeAction),
    /// Eth to ika bridge action
    EthToIkaBridgeAction(EthToIkaBridgeAction),
    ...
    AddTokensOnIkaAction(AddTokensOnIkaAction),
...
impl BridgeAction {
    ...
    pub fn chain_id(&self) -> BridgeChainId {
        match self {
            BridgeAction::IkaToEthBridgeAction(a) => a.ika_bridge_event.ika_chain_id,
            BridgeAction::EthToIkaBridgeAction(a) => a.eth_bridge_event.eth_chain_id,
            ...
            BridgeAction::AddTokensOnIkaAction(a) => a.chain_id,
    ...
    pub fn is_governace_action(&self) -> bool {
        match self.action_type() {
            ...
            BridgeActionType::AddTokensOnIka => true,
    ...
    pub fn action_type(&self) -> BridgeActionType {
        match self {
            BridgeAction::IkaToEthBridgeAction(_) => BridgeActionType::TokenTransfer,
            BridgeAction::EthToIkaBridgeAction(_) => BridgeActionType::TokenTransfer,
            ...
            BridgeAction::AddTokensOnIkaAction(_) => BridgeActionType::AddTokensOnIka,
    ...
    pub fn seq_number(&self) -> u64 {
        match self {
            BridgeAction::IkaToEthBridgeAction(a) => a.ika_bridge_event.nonce,
            BridgeAction::EthToIkaBridgeAction(a) => a.eth_bridge_event.nonce,
            ...
            BridgeAction::AddTokensOnIkaAction(a) => a.nonce,
    ...
    pub fn approval_threshold(&self) -> u64 {
        match self {
            BridgeAction::IkaToEthBridgeAction(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            BridgeAction::EthToIkaBridgeAction(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            ...
            BridgeAction::AddTokensOnIkaAction(_) => APPROVAL_THRESHOLD_ADD_TOKENS_ON_IKA,
...
pub fn is_route_valid(one: BridgeChainId, other: BridgeChainId) -> bool {
    if one.is_ika_chain() && other.is_ika_chain() {
        return false;
    }
    if !one.is_ika_chain() && !other.is_ika_chain() {
        return false;
    ...
    if one == BridgeChainId::EthMainnet {
        return other == BridgeChainId::IkaMainnet;
    }
    if one == BridgeChainId::IkaMainnet {
        return other == BridgeChainId::EthMainnet;
    }
    if other == BridgeChainId::EthMainnet {
        return one == BridgeChainId::IkaMainnet;
    }
    if other == BridgeChainId::IkaMainnet {
...
mod tests {
    ...
    use crate::test_utils::get_test_eth_to_ika_bridge_action;
    ...
    fn test_bridge_action_approval_threshold_regression_test() -> anyhow::Result<()> {
        let action = get_test_ika_to_eth_bridge_action(None, None, None, None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);

        let action = get_test_eth_to_ika_bridge_action(None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);
