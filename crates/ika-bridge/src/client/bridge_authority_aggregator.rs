...
use ika_authority_aggregation::{quorum_map_then_reduce_with_timeout_and_prefs, SigRequestPrefs};
...
impl GetSigsState {
    ...
    fn handle_verified_signed_action(
        ...
        if self.total_ok_stake >= self.validity_threshold {
            ...
            let certified_action: ika_types::message_envelope::Envelope<
                BridgeAction,
...
async fn request_sign_bridge_action_into_certification(
    ...
    let preference = match action {
        BridgeAction::IkaToEthBridgeAction(_) => Some(SigRequestPrefs {
            ordering_pref: BTreeSet::new(),
            prefetch_timeout,
        }),
        BridgeAction::EthToIkaBridgeAction(_) => None,
        _ => {
            if action.chain_id().is_ika_chain() {
                None
            } else {
...
mod tests {
    ...
    use ika_types::committee::VALIDITY_THRESHOLD;
