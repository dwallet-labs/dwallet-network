...
use crate::abi::{EthBridgeCommittee, EthBridgeEvent, EthERC20, EthIkaBridge, EthIkaBridgeEvents};
...
use crate::ika_transaction_builder::build_committee_register_transaction;
...
use crate::types::{BridgeAction, BridgeActionStatus, IkaToEthBridgeAction};
...
use ika_json_rpc_types::IkaTransactionBlockResponse;
...
use ika_types::base_types::{ObjectID, ObjectRef};
use ika_types::bridge::get_bridge_obj_initial_shared_version;
use ika_types::bridge::BridgeChainId;
use ika_types::bridge::BridgeSummary;
use ika_types::bridge::BridgeTrait;
use ika_types::bridge::{get_bridge, BRIDGE_MODULE_NAME};
use ika_types::bridge::{TOKEN_ID_BTC, TOKEN_ID_ETH, TOKEN_ID_USDC, TOKEN_ID_USDT};
use ika_types::committee::TOTAL_VOTING_POWER;
...
use ika_types::transaction::{ObjectArg, Transaction, TransactionData};
use ika_types::{BRIDGE_PACKAGE_ID, IKA_BRIDGE_OBJECT_ID};
use tokio::join;
...
use crate::config::{BridgeNodeConfig, EthConfig, IkaConfig};
use crate::node::run_bridge_node;
use crate::ika_client::IkaBridgeClient;
use crate::BRIDGE_ENABLE_PROTOCOL_VERSION;
use anyhow::anyhow;
use ethers::prelude::*;
use move_core_types::ident_str;
use std::process::Child;
use ika_config::local_ip_utils::get_available_port;
...
const IKA_BRIDGE_NAME: &str = "IkaBridge";
...
impl BridgeTestClusterBuilder {
    pub fn new() -> Self {
        BridgeTestClusterBuilder {
            ...
            ika_chain_id: BridgeChainId::IkaCustom,
        }
    }
    ...
    pub fn with_ika_chain_id(mut self, chain_id: BridgeChainId) -> Self {
        self.ika_chain_id = chain_id;
        self
    }
    ...
    pub async fn build(self) -> BridgeTestCluster {
        ...
        let bridge_client =
            IkaBridgeClient::new(&test_cluster.inner.fullnode_handle.rpc_url, metrics)
                .await
                .unwrap();
        info!(
            "Bridge committee: {:?}",
        ...
        BridgeTestCluster {
            ...
            ika_chain_id: self.ika_chain_id,
...
impl BridgeTestCluster {
    ...
    pub fn bridge_client(&self) -> &IkaBridgeClient {
        &self.bridge_client
    }

    pub fn ika_client(&self) -> &IkaClient {
        &self.test_cluster.inner.fullnode_handle.ika_client
    }

    pub fn ika_user_address(&self) -> IkaAddress {
        self.test_cluster.inner.get_address_0()
    }

    pub fn ika_chain_id(&self) -> BridgeChainId {
        self.ika_chain_id
    }
    ...
    pub fn ika_bridge_address(&self) -> String {
        self.eth_environment.contracts().ika_bridge_addrress_hex()
    }
    ...
    pub fn ika_rpc_url(&self) -> String {
    ...
    pub async fn new_bridge_transactions(
        &mut self,
        assert_success: bool,
    ) -> Vec<IkaTransactionBlockResponse> {
        let resps = self
            .ika_client()
            .read_api()
...
impl DeployedSolContracts {
    ...
    pub fn ika_bridge_addrress_hex(&self) -> String {
        Self::eth_adress_to_hex(self.ika_bridge)
    }
}
...
pub(crate) async fn deploy_sol_contract(
    ...
    let deploy_config = SolDeployConfig {
        ...
        ika_decimals: vec![],     // this is set up in the deploy script
    ...
    let contracts = DeployedSolContracts {
        ika_bridge: deployed_contracts.remove(IKA_BRIDGE_NAME).unwrap(),
...
impl EthBridgeEnvironment {
    ...
    pub async fn get_supported_token(&self, token_id: u8) -> (EthAddress, u8, u64) {
        ...
        let token_ika_decimal = config.token_ika_decimal_of(token_id).call().await.unwrap();
        let token_price = config.token_price_of(token_id).call().await.unwrap();
        (token_address, token_ika_decimal, token_price)
    }
}
...
pub(crate) async fn start_bridge_cluster(
        ...
        .enumerate()
    {
        ...
        let config = BridgeNodeConfig {
            ...
            ika: IkaConfig {
                ika_rpc_url: test_cluster.inner.fullnode_handle.rpc_url.clone(),
                ika_bridge_chain_id: BridgeChainId::IkaCustom as u8,
...
pub async fn get_signatures(
    ika_bridge_client: &IkaBridgeClient,
    nonce: u64,
    ika_chain_id: u8,
) -> Vec<Bytes> {
    let sigs = ika_bridge_client
        .get_token_transfer_action_onchain_signatures_until_success(ika_chain_id, nonce)
        .await
        .unwrap();
...
impl TestClusterWrapperBuilder {
    ...
    pub async fn build(self) -> TestClusterWrapper {
        ...
        let gas_objects_for_authority_keys = self
            ...
            .iter()
            .map(|k| {
                let address = IkaAddress::from(k.public());
        ...
        for (node, kp) in reordered_nodes.zip(self.bridge_authority_keys.iter()) {
            let validator_address = node.config().ika_address();
        ...
        if self.deploy_tokens {
            ...
            let action = publish_and_register_coins_return_add_coins_on_ika_action(
                test_cluster.wallet(),
                bridge_arg,
                vec![
        ...
        async fn await_committee_register_tasks(
            ...
            tasks: Vec<
                impl Future<Output = Result<IkaTransactionBlockResponse, ika_sdk::error::Error>>,
            >,
        ) {
            ...
            for response in responses {
                if response.unwrap().effects.unwrap().status() != &IkaExecutionStatus::Success {
                    has_failure = true;
                }
            }
            if has_failure {
...
pub async fn initiate_bridge_eth_to_ika(
    ...
    let ika_chain_id = bridge_test_cluster.ika_chain_id();
    let eth_chain_id = bridge_test_cluster.eth_chain_id();
    let token_id = TOKEN_ID_ETH;

    let ika_amount = (U256::from(amount) * U256::exp10(8)).as_u64(); // DP for Ether on Ika
    ...
    let eth_bridge_event = tx_receipt
        ...
        .unwrap();
    let EthBridgeEvent::EthIkaBridgeEvents(EthIkaBridgeEvents::TokensDepositedFilter(
        eth_bridge_event,
    )) = eth_bridge_event
    else {
        unreachable!();
    ...
    assert_eq!(eth_bridge_event.destination_chain_id, ika_chain_id as u8);
    assert_eq!(eth_bridge_event.token_id, token_id);
    assert_eq!(eth_bridge_event.ika_adjusted_amount, ika_amount);
    assert_eq!(eth_bridge_event.sender_address, eth_address);
    assert_eq!(eth_bridge_event.recipient_address, ika_address.to_vec());
    info!(
    ...
    .await
    .tap_ok(|_| {
        info!("Eth to Ika bridge transfer claimed");
    })
}

pub async fn initiate_bridge_ika_to_eth(
    bridge_test_cluster: &BridgeTestCluster,
    eth_address: EthAddress,
    token: ObjectRef,
    nonce: u64,
    ika_amount: u64,
) -> Result<IkaToEthBridgeAction, anyhow::Error> {
    let bridge_object_arg = bridge_test_cluster
        .bridge_client()
        .get_mutable_bridge_object_arg_must_succeed()
        .await;
    let ika_client = bridge_test_cluster.ika_client();
    ...
    let resp = match deposit_eth_to_ika_package(
        ika_client,
        ika_address,
    ...
    .await
    {
        Ok(resp) => {
            if !resp.status_ok().unwrap() {
                return Err(anyhow!("Ika TX error"));
            } else {
                resp
            }
        }
    ...
    let ika_events = resp.events.unwrap().data;
    let bridge_event = ika_events
        .iter()
        .filter_map(|e| {
            let ika_bridge_event = IkaBridgeEvent::try_from_ika_event(e).unwrap()?;
            ika_bridge_event.try_into_bridge_action(e.id.tx_digest, e.id.event_seq as u16)
        })
        .find_map(|e| {
            if let BridgeAction::IkaToEthBridgeAction(a) = e {
                Some(a)
            } else {
                None
            }
    ...
    assert_eq!(bridge_event.ika_bridge_event.nonce, nonce);
    assert_eq!(
        bridge_event.ika_bridge_event.ika_chain_id,
        bridge_test_cluster.ika_chain_id()
    );
    assert_eq!(
        bridge_event.ika_bridge_event.eth_chain_id,
        bridge_test_cluster.eth_chain_id()
    );
    assert_eq!(bridge_event.ika_bridge_event.ika_address, ika_address);
    assert_eq!(bridge_event.ika_bridge_event.eth_address, eth_address);
    assert_eq!(bridge_event.ika_bridge_event.token_id, TOKEN_ID_ETH);
    assert_eq!(
        bridge_event.ika_bridge_event.amount_ika_adjusted,
        ika_amount
    );
    ...
    .await
    .unwrap();
    info!("Ika to Eth bridge transfer approved.");
...
async fn deposit_eth_to_ika_package(
    ...
    ika_token_type_tags: &HashMap<u8, TypeTag>,
) -> Result<IkaTransactionBlockResponse, anyhow::Error> {
    ...
    builder.programmable_move_call(
        ...
        vec![ika_token_type_tags.get(&TOKEN_ID_ETH).unwrap().clone()],
...
pub async fn initiate_bridge_erc20_to_ika(
    bridge_test_cluster: &BridgeTestCluster,
    amount_u64: u64,
    ...
    let allowance_call = contract.approve(bridge_test_cluster.contracts().ika_bridge, amount);
    ...
    let ika_recipient_address = bridge_test_cluster.ika_user_address();
    let ika_chain_id = bridge_test_cluster.ika_chain_id();
    let eth_chain_id = bridge_test_cluster.eth_chain_id();
    ...
    let deposit_call = contract.bridge_erc20(
        token_id,
        amount,
        ika_recipient_address.to_vec().into(),
        ika_chain_id as u8,
    );
    ...
    .tap_ok(|_| {
        info!(
            nonce,
            token_id, amount_u64, "Eth to Ika bridge transfer claimed"
        );
    })
}
...
pub(crate) async fn deposit_native_eth_to_sol_contract(
    ...
    ika_recipient_address: IkaAddress,
    ika_chain_id: BridgeChainId,
    amount: u64,
) -> ContractCall<EthSigner, ()> {
    let contract = EthIkaBridge::new(contract_address, signer.clone().into());
    let ika_recipient_address = ika_recipient_address.to_vec().into();
    let amount = U256::from(amount) * U256::exp10(18); // 1 ETH
    contract
        .bridge_eth(ika_recipient_address, ika_chain_id as u8)
        .value(amount)
}
