...
use ika_json_rpc_types::{
    ObjectChange, IkaTransactionBlockResponse, IkaTransactionBlockResponseOptions,
};
...
pub fn owner_to_owner_info(owner: &Owner) -> (OwnerType, Option<IkaAddress>) {
    match owner {
...
impl IndexedObject {
    pub fn random() -> Self {
        let mut rng = rand::thread_rng();
        let random_address = IkaAddress::random_for_testing_only();
        IndexedObject {
...
pub struct IndexedTransaction {
    ...
    pub balance_change: Vec<ika_json_rpc_types::BalanceChange>,
    pub events: Vec<ika_types::event::Event>,
...
pub struct TxIndex {
    ...
    pub payers: Vec<IkaAddress>,
    pub sender: IkaAddress,
    pub recipients: Vec<IkaAddress>,
...
// IkaTransactionBlockResponseWithOptions is only used on the reading path
pub struct IkaTransactionBlockResponseWithOptions {
    pub response: IkaTransactionBlockResponse,
    pub options: IkaTransactionBlockResponseOptions,
}

impl From<IkaTransactionBlockResponseWithOptions> for IkaTransactionBlockResponse {
    fn from(value: IkaTransactionBlockResponseWithOptions) -> Self {
        let IkaTransactionBlockResponseWithOptions { response, options } = value;
