...
use clap::Parser;
use ika_mvr_indexer::backfill::backfill_runner::BackfillRunner;
use ika_mvr_indexer::benchmark::run_indexer_benchmark;
use ika_mvr_indexer::config::{Command, UploadOptions};
use ika_mvr_indexer::database::ConnectionPool;
use ika_mvr_indexer::db::setup_postgres::clear_database;
use ika_mvr_indexer::db::{
    check_db_migration_consistency, check_prunable_tables_valid, reset_database, run_migrations,
};
use ika_mvr_indexer::indexer::Indexer;
use ika_mvr_indexer::metrics::{
    spawn_connection_pool_metric_collector, start_prometheus_server, IndexerMetrics,
};
use ika_mvr_indexer::restorer::formal_snapshot::IndexerFormalSnapshotRestorer;
use ika_mvr_indexer::store::PgIndexerStore;
use tokio_util::sync::CancellationToken;
use tracing::warn;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let opts = ika_mvr_indexer::config::IndexerConfig::parse();

    // NOTE: this is to print out tracing like info, warn & error.
    let _guard = telemetry_subscribers::TelemetryConfig::new()
        .with_env()
        .init();
    warn!("WARNING: Ika indexer is still experimental and we expect occasional breaking changes that require backfills.");
