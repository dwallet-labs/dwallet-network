...
use ika_json_rpc_api::{ReadApiServer, QUERY_MAX_RESULT_LIMIT};
use ika_json_rpc_types::{
    Checkpoint, CheckpointId, CheckpointPage, ProtocolConfigResponse, IkaEvent,
    IkaGetPastObjectRequest, IkaObjectDataOptions, IkaObjectResponse, IkaPastObjectResponse,
...
use ika_types::base_types::{ObjectID, SequenceNumber};
use ika_types::digests::{ChainIdentifier, TransactionDigest};
...
impl ReadApiServer for ReadApi {
    ...
    async fn multi_get_objects(
        ...
        if object_ids.len() > *QUERY_MAX_RESULT_LIMIT {
            return Err(
                IkaRpcInputError::SizeLimitExceeded(QUERY_MAX_RESULT_LIMIT.to_string()).into(),
            );
        }
    ...
    async fn try_get_past_object(
        ...
        _options: Option<IkaObjectDataOptions>,
    ...
    async fn try_multi_get_past_objects(
        &self,
        _past_objects: Vec<IkaGetPastObjectRequest>,
        _options: Option<IkaObjectDataOptions>,
    ...
    async fn get_checkpoints(
        ...
        let limit = ika_json_rpc_api::validate_limit(
            limit,
            ika_json_rpc_api::QUERY_MAX_RESULT_LIMIT_CHECKPOINTS,
        )
...
impl IkaRpcModule for ReadApi {
    ...
    fn rpc_doc_module() -> Module {
        ika_json_rpc_api::ReadApiOpenRpc::module_doc()
    }
}
...
async fn object_read_to_object_response(
    ...
    match object_read {
        ...
        ObjectRead::Deleted((object_id, version, digest)) => Ok(IkaObjectResponse::new_with_error(
