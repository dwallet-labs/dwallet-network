...
use ika_json_rpc::name_service::{Domain, NameRecord, NameServiceConfig, NameServiceError};
use ika_json_rpc::IkaRpcModule;
use ika_json_rpc_api::{cap_page_limit, IndexerApiServer};
use ika_json_rpc_types::{
    DynamicFieldPage, EventFilter, EventPage, ObjectsPage, Page, IkaObjectResponse,
...
use ika_types::dynamic_field::{DynamicFieldName, Field};
...
impl IndexerApi {
    ...
    async fn get_owned_objects_internal(
        ...
        let IkaObjectResponseQuery { filter, options } = query.unwrap_or_default();
                    ...
                    ObjectRead::Exists(object_ref, o, layout) => {
                        if options.show_display {
                            match inner_clone.get_display_fields(&o, &layout).await {
                                Ok(rendered_fields) => Ok(IkaObjectResponse::new_with_data(
                                    (object_ref, o, layout, options, Some(rendered_fields))
                                        .try_into()?,
                                )),
                                Err(e) => Ok(IkaObjectResponse::new(
...
impl IndexerApiServer for IndexerApi {
    ...
    async fn get_dynamic_field_object(
        ...
        let id = ika_types::dynamic_field::derive_dynamic_field_id(
            parent_object_id,
            &name.type_,
            &name_bcs_value,
        )
        ...
        let options = ika_json_rpc_types::IkaObjectDataOptions::full_content();
        match self.inner.get_object_read(id).await? {
            ika_types::object::ObjectRead::NotExists(_)
            | ika_types::object::ObjectRead::Deleted(_) => {}
            ika_types::object::ObjectRead::Exists(object_ref, o, layout) => {
                return Ok(IkaObjectResponse::new_with_data(
                    (object_ref, o, layout, options, None).try_into()?,
                ));
            }
        }
        ...
        let dynamic_object_field_struct =
            ika_types::dynamic_field::DynamicFieldInfo::dynamic_object_field_wrapper(name.type_);
        let dynamic_object_field_type = TypeTag::Struct(Box::new(dynamic_object_field_struct));
        let dynamic_object_field_id = ika_types::dynamic_field::derive_dynamic_field_id(
            parent_object_id,
        ...
        Ok(IkaObjectResponse::new_with_error(
            ika_types::error::IkaObjectResponseError::DynamicFieldNotFound { parent_object_id },
        ))
    }
    ...
    async fn resolve_name_service_address(&self, name: String) -> RpcResult<Option<IkaAddress>> {
        ...
        let domains: Vec<_> = self
            ...
            .await?
            .into_iter()
            .map(|o| ika_types::object::Object::try_from(o).ok())
            .collect();
    ...
    async fn resolve_name_service_names(
        ...
        let domain = field_reverse_record_object
            .to_rust::<Field<IkaAddress, Domain>>()
            .ok_or_else(|| {
...
impl IkaRpcModule for IndexerApi {
    fn rpc(self) -> RpcModule<Self> {
        self.into_rpc()
    }

    fn rpc_doc_module() -> Module {
        ika_json_rpc_api::IndexerApiOpenRpc::module_doc()
    }
}
