...
use ika_json_rpc_types::DisplayFieldsResponse;
use ika_json_rpc_types::{Balance, Coin as IkaCoin, IkaCoinMetadata, IkaMoveValue};
use ika_json_rpc_types::{
    CheckpointId, EpochInfo, EventFilter, IkaEvent, IkaObjectDataFilter,
    IkaTransactionBlockResponse, TransactionFilter,
};
use ika_package_resolver::Package;
use ika_package_resolver::PackageStore;
use ika_package_resolver::{PackageStoreWithLruCache, Resolver};
use ika_types::effects::TransactionEvents;
use ika_types::{balance::Supply, coin::TreasuryCap, dynamic_field::DynamicFieldName};
use ika_types::{
    base_types::{ObjectID, IkaAddress, VersionNumber},
    committee::EpochId,
    digests::TransactionDigest,
    dynamic_field::DynamicFieldInfo,
    object::{Object, ObjectRead},
    ika_system_state::{ika_system_state_summary::IkaSystemStateSummary, IkaSystemStateTrait},
};
use ika_types::{coin::CoinMetadata, event::EventID};
...
use crate::{
    ...
    models::{
        ...
        transactions::{tx_events_to_ika_tx_events, StoredTransaction},
...
impl IndexerReader {
    ...
    pub async fn get_latest_ika_system_state(&self) -> Result<IkaSystemStateSummary, IndexerError> {
        ...
        let system_state = tokio::task::spawn_blocking(move || {
            ika_types::ika_system_state::get_ika_system_state(&object_store)
        })
        .await
        .unwrap()?
    ...
    pub async fn get_validator_from_table(
        &self,
        table_id: ObjectID,
        pool_id: ika_types::id::ID,
    ) -> Result<
        ika_types::ika_system_state::ika_system_state_summary::IkaValidatorSummary,
        IndexerError,
    > {
        let object_store = ConnectionAsObjectStore::from_pool(&self.pool)
            .await
        ...
        let validator = tokio::task::spawn_blocking(move || {
            ika_types::ika_system_state::get_validator_from_table(&object_store, table_id, &pool_id)
        })
        .await
        .unwrap()?;
        Ok(validator)
    ...
    /// `get_latest_ika_system_state` for current epoch instead.
    pub async fn get_epoch_ika_system_state(
        &self,
        epoch: Option<EpochId>,
    ) -> Result<IkaSystemStateSummary, IndexerError> {
        let stored_epoch = self.get_epoch_info_from_db(epoch).await?;
        let stored_epoch = match stored_epoch {
            Some(stored_epoch) => stored_epoch,
    ...
    pub async fn get_checkpoint(
        &self,
        checkpoint_id: CheckpointId,
    ) -> Result<Option<ika_json_rpc_types::Checkpoint>, IndexerError> {
        let stored_checkpoint = match self.get_checkpoint_from_db(checkpoint_id).await? {
            Some(stored_checkpoint) => stored_checkpoint,
            None => return Ok(None),
        };

        let checkpoint = ika_json_rpc_types::Checkpoint::try_from(stored_checkpoint)?;
        Ok(Some(checkpoint))
    }

    pub async fn get_latest_checkpoint(
        &self,
    ) -> Result<ika_json_rpc_types::Checkpoint, IndexerError> {
        let stored_checkpoint = self.get_latest_checkpoint_from_db().await?;

        ika_json_rpc_types::Checkpoint::try_from(stored_checkpoint)
    }
    ...
    pub async fn get_checkpoints(
        ...
        limit: usize,
        descending_order: bool,
    ) -> Result<Vec<ika_json_rpc_types::Checkpoint>, IndexerError> {
        self.get_checkpoints_from_db(cursor, limit, descending_order)
            .await?
            .into_iter()
            .map(ika_json_rpc_types::Checkpoint::try_from)
            .collect()
    }
    ...
    async fn stored_transaction_to_transaction_block(
        ...
        options: ika_json_rpc_types::IkaTransactionBlockResponseOptions,
    ) -> IndexerResult<Vec<IkaTransactionBlockResponse>> {
        ...
        for stored_tx in stored_txes {
            ...
            tx_block_responses_futures.push(tokio::task::spawn(
                stored_tx
                    .try_into_ika_transaction_block_response(options_clone, package_resolver_clone),
            ));
        }
    ...
    pub async fn get_owned_objects(
        ...
        if let Some(filter) = filter {
            match filter {
                IkaObjectDataFilter::StructType(struct_tag) => {
                ...
                IkaObjectDataFilter::MatchAny(filters) => {
                    let mut condition = "(".to_string();
                    for (i, filter) in filters.iter().enumerate() {
                        if let IkaObjectDataFilter::StructType(struct_tag) = filter {
                            let object_type =
                ...
                IkaObjectDataFilter::MatchNone(filters) => {
    ...
    async fn multi_get_transaction_block_response_in_blocking_task_impl(
        ...
        digests: &[TransactionDigest],
        options: ika_json_rpc_types::IkaTransactionBlockResponseOptions,
    ) -> Result<Vec<ika_json_rpc_types::IkaTransactionBlockResponse>, IndexerError> {
    ...
    pub async fn get_transaction_events(
    ...
    ) -> Result<Vec<ika_json_rpc_types::IkaEvent>, IndexerError> {
        use diesel_async::RunQueryDsl;
        ...
        let ika_tx_events = tx_events_to_ika_tx_events(
            tx_events,
        ...
        .await?;
        Ok(ika_tx_events.map_or(vec![], |ste| ste.data))
    }
    ...
    async fn query_events_by_tx_digest(
        ...
        let mut ika_event_futures = vec![];
        for stored_event in stored_events {
            ika_event_futures.push(tokio::task::spawn(
                stored_event.try_into_ika_event(self.package_resolver.clone()),
            ));
        }

        let ika_events = futures::future::join_all(ika_event_futures)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| error!("Failed to join ika event futures: {}", e))?
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| error!("Failed to collect ika event futures: {}", e))?;
        Ok(ika_events)
    }
    ...
    pub async fn bcs_name_from_dynamic_field_name(
        ...
        let ika_json_value = ika_json::IkaJsonValue::new(name.value.clone())?;
        let name_bcs_value = ika_json_value.to_bcs_bytes(&move_type_layout)?;
        Ok(name_bcs_value)
    }
    ...
    ) -> Result<Option<ika_types::display::DisplayVersionUpdatedEvent>, IndexerError> {
        use diesel_async::RunQueryDsl;
    ...
    pub async fn get_owned_coins(
        ...
        cursor: ObjectID,
        limit: usize,
    ) -> Result<Vec<IkaCoin>, IndexerError> {
        use diesel_async::RunQueryDsl;
    ...
    pub(crate) async fn get_display_fields(
        &self,
        original_object: &ika_types::object::Object,
        ...
        let (object_type, layout) = if let Some((object_type, layout)) =
            ika_json_rpc::read_api::get_object_type_and_struct(original_object, original_layout)
        ...
        if let Some(display_object) = self.get_display_object_by_type(&object_type).await? {
            return ika_json_rpc::read_api::get_rendered_fields(display_object.fields, &layout)
                .map_err(|e| IndexerError::GenericError(e.to_string()));
        }
...
impl ika_types::storage::ObjectStore for ConnectionAsObjectStore {
    fn get_object(
        &self,
        object_id: &ObjectID,
    ) -> Result<Option<ika_types::object::Object>, ika_types::storage::error::Error> {
