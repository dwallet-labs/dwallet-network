...
use ika_data_ingestion_core::Worker;
use ika_rest_api::{CheckpointData, CheckpointTransaction};
use ika_synthetic_ingestion::IndexerProgress;
use ika_types::dynamic_field::DynamicFieldType;
use ika_types::effects::{ObjectChange, TransactionEffectsAPI};
use ika_types::event::SystemEpochInfoEvent;
use ika_types::messages_checkpoint::{
    CertifiedCheckpointSummary, CheckpointContents, CheckpointSequenceNumber,
};
use ika_types::object::Object;
use ika_types::object::Owner;
use ika_types::ika_system_state::{get_ika_system_state, IkaSystemStateTrait};
...
impl CheckpointHandler {
    ...
    async fn index_epoch(
        ...
        if *checkpoint_summary.sequence_number() == 0 {
            ...
            let system_state_summary =
                get_ika_system_state(&checkpoint_object_store)?.into_ika_system_state_summary();
        ...
        if epoch_event_opt.is_none() {
            ...
            assert!(
                ...
                "Ika is not in safe mode but no SystemEpochInfoEvent found at end of epoch {}",
                checkpoint_summary.epoch
            );
        }
    ...
    async fn index_checkpoint(
        ...
        let (checkpoint, db_transactions, db_events, db_tx_indices, db_event_indices, db_displays) = {
            ...
            (
                IndexedCheckpoint::from_ika_checkpoint(
                    checkpoint_summary,
                    checkpoint_contents,
        ...
        checkpoint_data
            ...
            .flat_map(|data| {
                ...
                data.transactions
                    ...
                    .flat_map(|tx| &tx.output_objects)
                    .filter_map(|o| {
                        if let ika_types::object::Data::Package(p) = &o.data {
                            Some(IndexedPackage {
                                package_id: o.id(),
