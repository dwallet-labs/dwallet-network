...
use std::sync::Arc;
use ika_json_rpc_api::ReadApiServer;
use ika_mvr_indexer::apis::read_api::ReadApi;
use ika_mvr_indexer::indexer_reader::IndexerReader;
use ika_mvr_indexer::test_utils::{set_up, wait_for_checkpoint};
...
async fn test_checkpoint_apis() -> RpcResult<()> {
    ...
    // Test get_checkpoint
    let checkpoint_id = ika_json_rpc_types::CheckpointId::SequenceNumber(1);
