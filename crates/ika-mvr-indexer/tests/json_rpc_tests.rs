...
use ika_json_rpc_api::{CoinReadApiClient, IndexerApiClient, ReadApiClient};
use ika_json_rpc_types::{
    CoinPage, EventFilter, IkaObjectDataOptions, IkaObjectResponse, IkaObjectResponseQuery,
};
use ika_swarm_config::genesis_config::DEFAULT_GAS_AMOUNT;
use ika_test_transaction_builder::publish_package;
use ika_types::{event::EventID, transaction::CallArg};
...
async fn test_get_owned_objects() -> Result<(), anyhow::Error> {
    ...
    let data_option = IkaObjectDataOptions::new().with_owner();
...
async fn test_get_coins() -> Result<(), anyhow::Error> {
    ...
    // We should get all the 5 coins for IKA with the right balance.
    let result: CoinPage = http_client
