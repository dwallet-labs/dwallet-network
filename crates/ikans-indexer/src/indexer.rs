...
use ika_json_rpc::name_service::{Domain, NameRecord, SubDomainRegistration};
use ika_types::{
...
pub struct IkansIndexer {
    registry_table_id: IkaAddress,
    subdomain_wrapper_type: StructTag,
    name_record_type: StructTag,
}

impl std::default::Default for IkansIndexer {
    fn default() -> Self {
        Self::new(
...
impl IkansIndexer {
    ...
    pub fn new(registry_address: String, wrapper_type: String, record_type: String) -> Self {
        let registry_table_id = IkaAddress::from_str(&registry_address).unwrap();
    ...
    pub fn process_checkpoint(&self, data: &CheckpointData) -> (Vec<VerifiedDomain>, Vec<String>) {
        let mut checkpoint = IkansIndexerCheckpoint::new(data.checkpoint_summary.sequence_number);
...
pub struct IkansIndexerCheckpoint {
...
impl IkansIndexerCheckpoint {
    ...
    pub fn parse_record_changes(&mut self, config: &IkansIndexer, objects: &[Object]) {
...
/// Allows us to format a IkaNS specific query for updating the DB entries
