...
use ikans_indexer::{
    get_connection_pool,
    indexer::{format_update_field_query, format_update_subdomain_wrapper_query, IkansIndexer},
    models::VerifiedDomain,
    schema::domains,
    PgConnectionPool,
};

struct IkansIndexerWorker {
    pg_pool: PgConnectionPool,
    indexer: IkansIndexer,
}

impl IkansIndexerWorker {
...
#[async_trait]
impl Worker for IkansIndexerWorker {
    type Result = ();
...
async fn main() -> Result<()> {
    ...
    let indexer_setup =
        ...
        {
            IkansIndexer::new(registry_id, subdomain_wrapper_type, name_record_type)
        } else {
            IkansIndexer::default()
        };
    ...
    let worker_pool = WorkerPool::new(
        ...
        "ikans_indexing".to_string(), /* task name used as a key in the progress store */
        100,                          /* concurrency */
    );
