...
use ika_genesis_builder::validator_info::GenesisValidatorMetadata;
use ika_move_build::{BuildConfig, CompiledPackage};
use ika_sdk::rpc_types::{
    get_new_package_obj_from_response, IkaObjectDataOptions, IkaTransactionBlockEffectsAPI,
    IkaTransactionBlockResponse,
};
use ika_sdk::wallet_context::WalletContext;
use ika_types::base_types::{ObjectID, ObjectRef, SequenceNumber, IkaAddress};
use ika_types::crypto::{get_key_pair, AccountKeyPair, Signature, Signer};
use ika_types::digests::TransactionDigest;
use ika_types::multisig::{BitmapUnit, MultiSig, MultiSigPublicKey};
...
use ika_types::{TypeTag, IKA_SYSTEM_PACKAGE_ID};
...
impl TestTransactionBuilder {
    pub fn new(sender: IkaAddress, gas_object: ObjectRef, gas_price: u64) -> Self {
        Self {
    ...
    pub fn call_nft_create(self, package_id: ObjectID) -> Self {
        self.move_call(
            ...
            vec![
                ...
                CallArg::Pure(
                    bcs::to_bytes("https://ika.io/_nuxt/img/ika-logo.8d3c44e.svg").unwrap(),
                ),
            ],
        )
    }
    ...
    pub fn call_staking(self, stake_coin: ObjectRef, validator: IkaAddress) -> Self {
        self.move_call(
            IKA_SYSTEM_PACKAGE_ID,
            IKA_SYSTEM_MODULE_NAME.as_str(),
            "request_add_stake",
            vec![
    ...
    pub fn transfer(mut self, object: ObjectRef, recipient: IkaAddress) -> Self {
        self.test_data = TestTransactionData::Transfer(TransferData { object, recipient });
        self
    }

    pub fn transfer_ika(mut self, amount: Option<u64>, recipient: IkaAddress) -> Self {
        self.test_data = TestTransactionData::TransferIka(TransferIkaData { amount, recipient });
        self
    }
    ...
    pub fn build(self) -> TransactionData {
        match self.test_data {
            ...
            TestTransactionData::TransferIka(data) => TransactionData::new_transfer_ika(
                data.recipient,
                self.sender,
                data.amount,
                self.gas_object,
    ...
    pub fn build_and_sign_multisig(
        ...
        let data = self.build();
        let intent_msg = IntentMessage::new(Intent::ika_transaction(), data.clone());
    ...
    pub fn build_and_sign_multisig_legacy(
        ...
        let data = self.build();
        let intent = Intent::ika_transaction();
...
pub async fn make_transfer_ika_transaction(
    context: &WalletContext,
    recipient: Option<IkaAddress>,
    amount: Option<u64>,
) -> Transaction {
    ...
    context.sign_transaction(
        &TestTransactionBuilder::new(sender, gas_object, gas_price)
            .transfer_ika(amount, recipient.unwrap_or(sender))
            .build(),
    )
}
...
pub async fn create_nft(
    context: &WalletContext,
    package_id: ObjectID,
) -> (IkaAddress, ObjectID, TransactionDigest) {
    let (sender, gas_object) = context.get_one_gas_object().await.unwrap().unwrap();
    let rgp = context.get_reference_gas_price().await.unwrap();
