...
use pera_graphql_rpc::test_infra::cluster::{serve_executor, SnapshotLagConfig};
use pera_json_rpc_api::QUERY_MAX_RESULT_LIMIT;
use pera_json_rpc_types::{DevInspectResults, PeraExecutionStatus, PeraTransactionBlockEffectsAPI};
use pera_protocol_config::{Chain, ProtocolConfig};
use pera_storage::{
    key_value_store::TransactionKeyValueStore, key_value_store_metrics::KeyValueStoreMetrics,
};
use pera_swarm_config::genesis_config::AccountConfig;
use pera_types::base_types::{SequenceNumber, VersionNumber};
use pera_types::committee::EpochId;
use pera_types::crypto::{get_authority_key_pair, RandomnessRound};
use pera_types::digests::{ConsensusCommitDigest, TransactionDigest, TransactionEventsDigest};
...
use pera_types::transaction::Command;
use pera_types::transaction::ProgrammableTransaction;
use pera_types::PERA_SYSTEM_ADDRESS;
use pera_types::{
    base_types::{ObjectID, ObjectRef, PeraAddress, PERA_ADDRESS_LENGTH},
    crypto::{get_key_pair_from_rng, AccountKeyPair},
    event::Event,
    object::{self, Object},
    transaction::{Transaction, TransactionData, TransactionDataAPI, VerifiedTransaction},
    MOVE_STDLIB_ADDRESS, PERA_CLOCK_OBJECT_ID, PERA_FRAMEWORK_ADDRESS, PERA_SYSTEM_STATE_OBJECT_ID,
};
use pera_types::{execution_status::ExecutionStatus, transaction::TransactionKind};
use pera_types::{gas::GasCostSummary, object::GAS_VALUE_FOR_TESTING};
use pera_types::{
    move_package::MovePackage,
    transaction::{Argument, CallArg},
};
use pera_types::{
    programmable_transaction_builder::ProgrammableTransactionBuilder, PERA_FRAMEWORK_PACKAGE_ID,
};
use pera_types::{utils::to_sender_signed_transaction, PERA_SYSTEM_PACKAGE_ID};
...
#[async_trait]
impl<'a> MoveTestAdapter<'a> for PeraTestAdapter {
    type ExtraPublishArgs = PeraPublishArgs;
    type ExtraRunArgs = PeraRunArgs;
    type ExtraInitArgs = PeraInitArgs;
    type ExtraValueArgs = PeraExtraValueArgs;
    type Subcommand = PeraSubcommand<Self::ExtraValueArgs, Self::ExtraRunArgs>;
    ...
    fn render_command_input(
        ...
        match &task.command {
            TaskCommand::Subcommand(PeraSubcommand::ProgrammableTransaction(..)) => {
    ...
    async fn init(
        ...
        assert!(
            ...
            "Must populate 'pre_compiled_deps' with Pera framework"
        );
    ...
    async fn publish_modules(
        ...
        // we are assuming that all packages depend on Move Stdlib and Pera Framework, so these
        // don't have to be provided explicitly as parameters
        dependencies.extend([MOVE_STDLIB_PACKAGE_ID, PERA_FRAMEWORK_PACKAGE_ID]);
        let data = |sender, gas| {
    ...
    async fn call_function(
        ...
        self.next_task();
        let PeraRunArgs { summarize, .. } = extra;
    ...
    async fn handle_subcommand(
        ...
        match command {
            PeraSubcommand::ForceObjectSnapshotCatchup(ForceObjectSnapshotCatchup {
                start_cp,
                end_cp,
            }) => {
            ...
            PeraSubcommand::RunGraphql(RunGraphqlCommand {
                show_usage,
                show_headers,
            ...
            PeraSubcommand::CreateCheckpoint(CreateCheckpointCommand { count }) => {
            ...
            PeraSubcommand::AdvanceEpoch(AdvanceEpochCommand {
                count,
                create_random_state,
            }) => {
                for _ in 0..count.unwrap_or(1) {
            ...
            PeraSubcommand::AdvanceClock(AdvanceClockCommand { duration_ns }) => {
                self.executor
                    .advance_clock(Duration::from_nanos(duration_ns))
                    .await?;
                Ok(None)
            }
            PeraSubcommand::SetRandomState(SetRandomStateCommand {
                randomness_round,
                random_bytes,
            ...
            PeraSubcommand::ViewObject(ViewObjectCommand { id: fake_id }) => {
            ...
            PeraSubcommand::TransferObject(TransferObjectCommand {
                id: fake_id,
                recipient,
                sender,
                gas_budget,
                ...
                let mut builder = ProgrammableTransactionBuilder::new();
                let obj_arg = PeraValue::Object(fake_id, None).into_argument(&mut builder, self)?;
                ...
                let transaction = self.sign_txn(sender, |sender, gas| {
                    ...
                    builder.command(pera_types::transaction::Command::TransferObjects(
                        vec![obj_arg],
                        rec_arg,
                    ));
                    let pt = builder.finish();
            ...
            PeraSubcommand::ConsensusCommitPrologue(ConsensusCommitPrologueCommand {
                timestamp_ms,
            }) => {
            ...
            PeraSubcommand::ProgrammableTransaction(ProgrammableTransactionCommand {
                sender,
                gas_budget,
                gas_price,
                dev_inspect,
            ...
            PeraSubcommand::UpgradePackage(UpgradePackageCommand {
                package,
                upgrade_capability,
                dependencies,
                sender,
            ...
            PeraSubcommand::StagePackage(StagePackageCommand {
                syntax,
                dependencies,
            }) => {
                let syntax = syntax.unwrap_or_else(|| self.default_syntax());
            ...
            PeraSubcommand::SetAddress(SetAddressCommand { address, input }) => {
                ...
                let (value, package) = match input {
                    ...
                    PeraValue::MoveValue(v) => {
                        let bytes = v.simple_serialize().unwrap();
                        let value: AccountAddress = bcs::from_bytes(&bytes)?;
                        (value, None)
                    }
                    PeraValue::Digest(_) => bail!("digest is not supported as an input"),
                    PeraValue::ObjVec(_) => bail!("obj vec is not supported as an input"),
                    PeraValue::Receiving(_, _) => bail!("receiving is not supported as an input"),
                    PeraValue::ImmShared(_, _) => {
...
impl<'a> PeraTestAdapter {
    ...
    async fn upgrade_package(
        ...
        // Argument::Input(0)
        PeraValue::Object(upgrade_capability, None).into_argument(&mut builder, self)?;
    ...
    async fn dev_inspect(
        ...
        match effects.status() {
            ...
            PeraExecutionStatus::Failure { error } => Err(anyhow::anyhow!(self.stabilize_str(
    ...
    fn stabilize_str(&self, input: impl AsRef<str>) -> String {
        fn candidate_is_hex(s: &str) -> bool {
            const HEX_STR_LENGTH: usize = PERA_ADDRESS_LENGTH * 2;
            let n = s.len();
...
impl<'a> GetModule for &'a PeraTestAdapter {
...
static NAMED_ADDRESSES: Lazy<BTreeMap<String, NumericalAddress>> = Lazy::new(|| {
    ...
    // TODO fix Pera framework constants
    map.insert(
        "pera".to_string(),
        NumericalAddress::new(
            PERA_FRAMEWORK_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
        "pera_system".to_string(),
        NumericalAddress::new(
            PERA_SYSTEM_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
...
pub static PRE_COMPILED: Lazy<FullyCompiledProgram> = Lazy::new(|| {
    ...
    let pera_files: &Path = Path::new(DEFAULT_FRAMEWORK_PATH);
    let pera_system_sources = {
        let mut buf = pera_files.to_path_buf();
        buf.extend(["packages", "pera-system", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let pera_sources = {
        let mut buf = pera_files.to_path_buf();
        buf.extend(["packages", "pera-framework", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let pera_deps = {
        let mut buf = pera_files.to_path_buf();
    ...
    let fully_compiled_res = move_compiler::construct_pre_compiled_lib(
        vec![PackagePaths {
            name: Some(("pera-framework".into(), config)),
            paths: vec![
                pera_system_sources,
                pera_sources,
                pera_deps,
    ...
    match fully_compiled_res {
        Err((files, diags)) => {
            eprintln!("!!!Pera framework failed to compile!!!");
...
async fn init_val_fullnode_executor(
    ...
    // For each named Pera account without an address value, create an account with an address
    // and a gas object
...
async fn init_sim_executor(
    ...
    // For each named Pera account without an address value, create a key pair
    for n in account_names {
    ...
    if custom_validator_account {
        ...
        let (a, b): (PeraAddress, Ed25519KeyPair) = get_key_pair_from_rng(&mut rng);
    ...
    let db_name = format!("pera_graphql_test_{}", hash);
...
impl ObjectStore for PeraTestAdapter {
    fn get_object(
        &self,
        object_id: &ObjectID,
...
impl ReadStore for PeraTestAdapter {
    ...
    fn get_committee(
        &self,
        epoch: pera_types::committee::EpochId,
    ...
    fn get_checkpoint_by_digest(
        &self,
        digest: &pera_types::messages_checkpoint::CheckpointDigest,
    ...
    ) -> pera_types::storage::error::Result<Option<CheckpointContents>> {
