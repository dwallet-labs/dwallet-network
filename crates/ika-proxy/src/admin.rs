...
use crate::peers::{AllowedPeer, IkaNodeProvider};
use crate::var;
use anyhow::Error;
use anyhow::Result;
...
use ika_tls::IKA_VALIDATOR_SERVER_NAME;
use ika_tls::{
...
pub fn app(
    ...
    allower: Option<IkaNodeProvider>,
) -> Router {
...
pub fn create_server_cert_default_allow(
    hostname: String,
) -> Result<ServerConfig, ika_tls::rustls::Error> {
    let CertKeyPair(server_certificate, _) = generate_self_cert(hostname);

    ClientCertVerifier::new(AllowAll, IKA_VALIDATOR_SERVER_NAME.to_string()).rustls_server_config(
        vec![server_certificate.rustls_certificate()],
        server_certificate.rustls_private_key(),
    )
}

/// Verify clients against ika blockchain, clients that are not found in ika_getValidators
/// will be rejected
pub fn create_server_cert_enforce_peer(
    dynamic_peers: DynamicPeerValidationConfig,
    static_peers: Option<StaticPeerValidationConfig>,
) -> Result<(ServerConfig, Option<IkaNodeProvider>), ika_tls::rustls::Error> {
    let (Some(certificate_path), Some(private_key_path)) =
        (dynamic_peers.certificate_file, dynamic_peers.private_key)
    else {
        return Err(ika_tls::rustls::Error::General(
            "missing certs to initialize server".into(),
        ));
    };
    let static_peers = load_static_peers(static_peers).map_err(|e| {
        ika_tls::rustls::Error::General(format!("unable to load static pub keys: {}", e))
    })?;
    let allower = IkaNodeProvider::new(dynamic_peers.url, dynamic_peers.interval, static_peers);
    allower.poll_peer_list();
    let c = ClientCertVerifier::new(allower.clone(), IKA_VALIDATOR_SERVER_NAME.to_string())
