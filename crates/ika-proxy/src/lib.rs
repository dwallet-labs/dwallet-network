...
mod tests {
    ...
    use crate::{admin::CertKeyPair, config::RemoteWriteConfig, peers::IkaNodeProvider};
    ...
    use ika_tls::{ClientCertVerifier, TlsAcceptor};
    ...
    /// ika-node client that posts data to the proxy using the protobuf format.  The server processes this
    ...
    async fn test_axum_acceptor() {
        ...
        let CertKeyPair(client_priv_cert, client_pub_key) = admin::generate_self_cert("ika".into());
        ...
        let mut allower = IkaNodeProvider::new("".into(), Duration::from_secs(30), vec![]);
        ...
        allower.get_ika_mut().write().unwrap().insert(
