...
/// IkaNodeProvider queries the ika blockchain and keeps a record of known validators based on the response from
/// ika_getValidators.  The node name, public key and other info is extracted from the chain and stored in this
...
#[derive(Debug, Clone)]
pub struct IkaNodeProvider {
    ika_nodes: AllowedPeers,
...
impl IkaNodeProvider {
    pub fn new(
        ...
        let ika_nodes = Arc::new(RwLock::new(HashMap::new()));
    ...
    pub fn get(&self, key: &Ed25519PublicKey) -> Option<AllowedPeer> {
        ...
        // check ika validators
        if let Some(v) = self.ika_nodes.read().unwrap().get(key) {
            return Some(AllowedPeer {
                name: v.name.to_owned(),
                public_key: v.public_key.to_owned(),
            });
    ...
    /// Get a mutable reference to the allowed ika validator map
    pub fn get_ika_mut(&mut self) -> &mut AllowedPeers {
        &mut self.ika_nodes
    }

    /// get_validators will retrieve known validators
    async fn get_validators(url: String) -> Result<IkaSystemStateSummary> {
        let rpc_method = "ikax_getLatestIkaSystemState";
        let observe = || {
    ...
    async fn get_bridge_validators(url: String) -> Result<BridgeSummary> {
        let rpc_method = "ikax_getLatestBridge";
    ...
    async fn update_ika_validator_set(&self) {
        match Self::get_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => {
                let validators = extract(summary);
                let mut allow = self.ika_nodes.write().unwrap();
                allow.clear();
                allow.extend(validators);
                info!(
                    "{} ika validators managed to make it on the allow list",
                    allow.len()
                );
            }
            Err(error) => {
    ...
    async fn update_bridge_validator_set(&self, metrics_keys: MetricsPubKeys) {
        let ika_system = match Self::get_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => summary,
            Err(error) => {
                JSON_RPC_STATE
                    .with_label_values(&["update_bridge_peer_count", "failed"])
                    .inc();
                error!("unable to get ika system state: {error}");
                return;
            }
        };
        match Self::get_bridge_validators(self.rpc_url.to_owned()).await {
            Ok(summary) => {
                let names = ika_system
                    .active_validators
                    .into_iter()
                    .map(|v| (v.ika_address, v.name))
                    .collect();
            ...
            Err(error) => {
                JSON_RPC_STATE
                    .with_label_values(&["update_bridge_peer_count", "failed"])
                    .inc();
                error!("unable to refresh ika bridge peer list: {error}");
            }
        };
    }
    ...
    pub fn poll_peer_list(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                cloned_self.update_ika_validator_set().await;
                cloned_self
...
/// extract will get the network pubkey bytes from a IkaValidatorSummary type.  This type comes from a
...
fn extract(
    ...
    summary.active_validators.into_iter().filter_map(|vm| {
        match Ed25519PublicKey::from_bytes(&vm.network_pubkey_bytes) {
            Ok(public_key) => {
                debug!(
                    "adding public key {:?} for ika validator {:?}",
                    public_key, vm.name
                );
                Some((
                    public_key.clone(),
            ...
            Err(error) => {
                error!(
                    "unable to decode public key for name: {:?} ika_address: {:?} error: {error}",
                    vm.name, vm.ika_address
                );
...
async fn extract_bridge(
    ...
    names: Arc<BTreeMap<IkaAddress, String>>,
    ...
    let results: Vec<_> = stream::iter(committee_members)
        .filter_map(|(_, cm)| {
            ...
            async move {
                ...
                let bridge_name = names.get(&cm.ika_address).cloned().unwrap_or_else(|| {
                    warn!(
                        address =% cm.ika_address,
                        "Bridge node not found in ika committee, using base URL as the name",
                    );
...
mod tests {
    ...
    use serde::Serialize;
    use ika_types::base_types::IkaAddress;
    use ika_types::bridge::{BridgeCommitteeSummary, BridgeSummary, MoveTypeCommitteeMember};
    use ika_types::ika_system_state::ika_system_state_summary::{
        IkaSystemStateSummary, IkaValidatorSummary,
    };

    /// creates a test that binds our proxy use case to the structure in ika_getLatestIkaSystemState
    ...
    #[test]
    fn depend_on_ika_ika_system_state_summary() {
        let CertKeyPair(_, client_pub_key) = generate_self_cert("ika".into());
        // all fields here just satisfy the field types, with exception to active_validators, we use
        // some of those.
        let depends_on = IkaSystemStateSummary {
            active_validators: vec![IkaValidatorSummary {
        ...
        let r = serde_json::to_string(&ResponseBody { result: depends_on })
            .expect("expected to serialize ResponseBody{IkaSystemStateSummary}");

        let deserialized = serde_json::from_str::<ResponseBody>(&r)
            .expect("expected to deserialize ResponseBody{IkaSystemStateSummary}");
