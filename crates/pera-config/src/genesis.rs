...
use pera_types::deny_list_v1::{get_coin_deny_list, PerTypeDenyList};
...
impl Genesis {
    ...
    pub fn validator_set_for_tooling(&self) -> Vec<PeraValidatorGenesis> {
        self.pera_system_object()
    ...
    pub fn committee_with_network(&self) -> CommitteeWithNetworkMetadata {
        self.pera_system_object().get_current_epoch_committee()
    }

    pub fn reference_gas_price(&self) -> u64 {
        self.pera_system_object().reference_gas_price()
    }

    // TODO: No need to return PeraResult. Also consider return &.
    pub fn committee(&self) -> PeraResult<Committee> {
        Ok(self.committee_with_network().committee().clone())
    }

    pub fn pera_system_wrapper_object(&self) -> PeraSystemStateWrapper {
        get_pera_system_state_wrapper(&self.objects())
            .expect("Pera System State Wrapper object must always exist")
    }

    pub fn pera_system_object(&self) -> PeraSystemState {
        get_pera_system_state(&self.objects()).expect("Pera System State object must always exist")
    }
    ...
    pub fn clock(&self) -> Clock {
        let clock = self
            .objects()
            .iter()
            .find(|o| o.id() == pera_types::PERA_CLOCK_OBJECT_ID)
...
impl UnsignedGenesis {
    ...
    pub fn has_randomness_state_object(&self) -> bool {
        self.objects()
            .get_object(&PERA_RANDOMNESS_STATE_OBJECT_ID)
...
impl GenesisCeremonyParameters {
    ...
    fn default_initial_stake_subsidy_distribution_amount() -> u64 {
        // 1M Pera
        1_000_000 * pera_types::gas_coin::NPERA_PER_PERA
    }
    ...
    pub fn to_genesis_chain_parameters(&self) -> GenesisChainParameters {
        GenesisChainParameters {
            ...
            max_validator_count: pera_types::governance::MAX_VALIDATOR_COUNT,
            min_validator_joining_stake: pera_types::governance::MIN_VALIDATOR_JOINING_STAKE_NPERA,
            validator_low_stake_threshold:
                pera_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_NPERA,
            validator_very_low_stake_threshold:
                pera_types::governance::VALIDATOR_VERY_LOW_STAKE_THRESHOLD_NPERA,
            validator_low_stake_grace_period:
                pera_types::governance::VALIDATOR_LOW_STAKE_GRACE_PERIOD,
        }
    }
}
...
pub struct TokenDistributionSchedule {
    pub stake_subsidy_fund_npera: u64,
    pub allocations: Vec<TokenAllocation>,
}

impl TokenDistributionSchedule {
    pub fn validate(&self) {
        let mut total_npera = self.stake_subsidy_fund_npera;

        for allocation in &self.allocations {
            total_npera += allocation.amount_npera;
        }

        if total_npera != TOTAL_SUPPLY_NPERA {
            panic!("TokenDistributionSchedule adds up to {total_npera} and not expected {TOTAL_SUPPLY_NPERA}");
        }
    }

    pub fn check_all_stake_operations_are_for_valid_validators<
        I: IntoIterator<Item = PeraAddress>,
    >(
        &self,
        validators: I,
    ) {
        ...
        let mut validators: HashMap<PeraAddress, u64> =
        ...
        for allocation in &self.allocations {
            if let Some(staked_with_validator) = &allocation.staked_with_validator {
                *validators
                    ...
                    allocation.amount_npera;
            }
        }

        // Check that all validators have sufficient stake allocated to ensure they meet the
        // minimum stake threshold
        let minimum_required_stake = pera_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_NPERA;
    ...
    pub fn new_for_validators_with_default_allocation<I: IntoIterator<Item = PeraAddress>>(
        validators: I,
    ) -> Self {
        let mut supply = TOTAL_SUPPLY_NPERA;
        let default_allocation = pera_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_NPERA;
        ...
        let allocations = validators
            ...
            .map(|a| {
                ...
                TokenAllocation {
                    recipient_address: a,
                    amount_npera: default_allocation,
        ...
        let schedule = Self {
            stake_subsidy_fund_npera: supply,
            allocations,
        };
    ...
    ///
    /// All entries in a token distribution schedule must add up to 10B Pera.
    pub fn from_csv<R: std::io::Read>(reader: R) -> Result<Self> {
        ...
        assert_eq!(
            TOTAL_SUPPLY_NPERA,
            allocations.iter().map(|a| a.amount_npera).sum::<u64>(),
            "Token Distribution Schedule must add up to 10B Pera",
        );
        ...
        let schedule = Self {
            stake_subsidy_fund_npera: stake_subsidy_fund_allocation.amount_npera,
            allocations,
        };
    ...
    pub fn to_csv<W: std::io::Write>(&self, writer: W) -> Result<()> {
        ...
        writer.serialize(TokenAllocation {
            recipient_address: PeraAddress::default(),
            amount_npera: self.stake_subsidy_fund_npera,
...
pub struct TokenAllocation {
    pub recipient_address: PeraAddress,
    pub amount_npera: u64,

    /// Indicates if this allocation should be staked at genesis and with which validator
    pub staked_with_validator: Option<PeraAddress>,
}
...
impl TokenDistributionScheduleBuilder {
    ...
    pub fn new() -> Self {
        Self {
            pool: TOTAL_SUPPLY_NPERA,
            allocations: vec![],
        }
    }

    pub fn default_allocation_for_validators<I: IntoIterator<Item = PeraAddress>>(
        &mut self,
        validators: I,
    ) {
        let default_allocation = pera_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_NPERA;
    ...
    pub fn add_allocation(&mut self, allocation: TokenAllocation) {
        self.pool = self.pool.checked_sub(allocation.amount_npera).unwrap();
        self.allocations.push(allocation);
    }

    pub fn build(&self) -> TokenDistributionSchedule {
        let schedule = TokenDistributionSchedule {
            stake_subsidy_fund_npera: self.pool,
