...
module dwallet_system::dwallet_system {
    ...
    fun load_inner_maybe_upgrade(self: &mut DWalletSystemState): &mut DWalletSystemStateInner {
        let version = self.version;
        // TODO: This is where we check the version and perform upgrade if necessary.

        let inner: &mut DWalletSystemStateInner = dynamic_field::borrow_mut(&mut self.id, version);
