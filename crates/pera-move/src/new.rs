...
const PERA_PKG_NAME: &str = "Pera";

// Use testnet by default. Probably want to add options to make this configurable later
const PERA_PKG_PATH: &str = "{ git = \"https://github.com/MystenLabs/sui.git\", subdir = \"crates/pera-framework/packages/pera-framework\", rev = \"framework/testnet\" }";

#[derive(Parser)]
#[group(id = "pera-move-new")]
pub struct New {
    #[clap(flatten)]
    pub new: new::New,
}
...
impl New {
    pub fn execute(self, path: Option<&Path>) -> anyhow::Result<()> {
        ...
        self.new
            .execute(path, [(PERA_PKG_NAME, PERA_PKG_PATH)], [(name, "0x0")], "")?;
