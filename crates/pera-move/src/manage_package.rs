...
const NO_LOCK_FILE: &str = "Expected a `Move.lock` file to exist in the package path, \
                            but none found. Consider running `pera move build` to \
...
/// Move.lock).
#[derive(Parser)]
#[group(id = "pera-move-manage-package")]
pub struct ManagePackage {
    #[clap(long)]
    /// The environment to associate this package information with (consider using `pera client active-env`).
