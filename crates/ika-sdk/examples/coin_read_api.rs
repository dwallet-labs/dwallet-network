...
// and add 1 IKA to the active address.
// By default, the example will use the Ika testnet network (fullnode.testnet.ika.io:443).
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // use `None` for the default `Coin<IKA>` which is represented as
    // "0x2::ika::IKA"
    let coin_type = Some("0x2::ika::IKA".to_string());
    let coins = ika
        .coin_read_api()
    ...
    let coins_stream = ika.coin_read_api().get_coins_stream(active_address, None);
    ...
    // Select coins based on the provided coin type (IKA in this example). Use `None` for the default Ika coin
    let select_coins = ika
        .coin_read_api()
    ...
    // or if None is passed, it will use Coin<IKA> as the coin type
    let balance = ika
        .coin_read_api()
    ...
    let total_balance = ika.coin_read_api().get_all_balances(active_address).await?;
    println!(" *** Balance + Total Balance *** ");
    println!("Balance: {:?}", balance);
    println!("Total Balance: {:?}", total_balance);
    println!(" *** Balance + Total Balance ***\n ");

    // Return the coin metadata for the Coin<IKA>
