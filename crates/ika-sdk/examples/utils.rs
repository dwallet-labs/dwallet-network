...
use ika_config::{
    ika_config_dir, Config, PersistedConfig, IKA_CLIENT_CONFIG, IKA_KEYSTORE_FILENAME,
};
use ika_json_rpc_types::{Coin, IkaObjectDataOptions};
...
use ika_sdk::{rpc_types::IkaTransactionBlockResponseOptions, IkaClient, IkaClientBuilder};
...
// const IKA_FAUCET: &str = "https://faucet.devnet.ika.io/gas"; // devnet faucet

pub const IKA_FAUCET: &str = "https://faucet.testnet.ika.io/v1/gas"; // testnet faucet

// const IKA_FAUCET: &str = "http://127.0.0.1:9123/gas";

/// Return a ika client to interact with the APIs,
...
pub async fn setup_for_write() -> Result<(IkaClient, IkaAddress, IkaAddress), anyhow::Error> {
    let (client, active_address) = setup_for_read().await?;
    // make sure we have some IKA (5_000_000 NIKA) on this address
...
/// Return a ika client to interact with the APIs and an active address from the local wallet.
///
/// This function sets up a wallet in case there is no wallet locally,
/// and ensures that the active address of the wallet has IKA on it.
/// If there is no IKA owned by the active address, then it will request
/// IKA from the faucet.
pub async fn setup_for_read() -> Result<(IkaClient, IkaAddress), anyhow::Error> {
    let client = IkaClientBuilder::default().build_testnet().await?;
    println!("Ika testnet version is: {}", client.api_version());
...
pub async fn request_tokens_from_faucet(
    ...
    loop {
        let resp = client
            .get("https://faucet.testnet.ika.io/v1/status")
            .header("Content-Type", "application/json")
            .json(&json_body)
            .send()
            .await?;
    ...
    loop {
        let owner = ika_client
            .read_api()
            .get_object_with_options(
                ObjectID::from_str(&coin_id)?,
                IkaObjectDataOptions::new().with_owner(),
            )
            .await?;
...
/// Return the coin owned by the address that has at least 5_000_000 NIKA, otherwise returns None
pub async fn fetch_coin(
    ika: &IkaClient,
    sender: &IkaAddress,
) -> Result<Option<Coin>, anyhow::Error> {
    let coin_type = "0x2::ika::IKA".to_string();
    let coins_stream = ika
        .coin_read_api()
...
pub async fn split_coin_digest(
    ...
    let coin = match fetch_coin(ika, sender).await? {
        None => {
            request_tokens_from_faucet(*sender, ika).await?;
            fetch_coin(ika, sender)
                .await?
                .expect("Supposed to get a coin with IKA, but didn't. Aborting")
        }
        Some(c) => c,
    };
    ...
    // first, we want to split the coin, and we specify how much IKA (in NIKA) we want
    // for the new coin
...
pub fn retrieve_wallet() -> Result<WalletContext, anyhow::Error> {
    let wallet_conf = ika_config_dir()?.join(IKA_CLIENT_CONFIG);
    let keystore_path = ika_config_dir()?.join(IKA_KEYSTORE_FILENAME);

    // check if a wallet exists and if not, create a wallet and a ika client config
    ...
    if !wallet_conf.exists() {
        let keystore = FileBasedKeystore::new(&keystore_path)?;
        let mut client_config = IkaClientConfig::new(keystore.into());

        client_config.add_env(IkaEnv::testnet());
        client_config.add_env(IkaEnv::devnet());
        client_config.add_env(IkaEnv::localnet());
    ...
    let mut client_config: IkaClientConfig = PersistedConfig::read(&wallet_conf)?;
...
async fn just_for_clippy() -> Result<(), anyhow::Error> {
    let (ika, sender, _recipient) = setup_for_write().await?;
    let _digest = split_coin_digest(&ika, &sender).await?;
    Ok(())
}
