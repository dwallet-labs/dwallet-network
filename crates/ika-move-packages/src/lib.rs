...
impl IkaMovePackage {
    ...
    pub fn modules_with_deps(&self, ika_dependencies_map: HashMap<String, ObjectID>) -> anyhow::Result<Vec<CompiledModule>> {
        ...
        let mut modules: Vec<_> = self.bytes
            .iter()
        ...
        for module in modules.iter_mut() {
            ...
            for module_handle in module.module_handles.iter_mut() {
                ...
                for (n, id) in ika_dependencies.iter() {
                    if name.as_str() == n && !address_identifiers_map.contains_key(n) {
                        address_identifiers_map.insert(n.clone(), cloned_module.address_identifiers.len() as u16);
            ...
            for module_handle in module.module_handles.iter_mut() {
                ...
                for (n, id) in ika_dependencies.iter() {
                    if name.as_str() == n {
                        module_handle.address = AddressIdentifierIndex(*address_identifiers_map.get(n).unwrap());
                    }
                }
            }
        }
    ...
    pub fn bytes_with_deps(&self, ika_dependencies_map: HashMap<String, ObjectID>) -> anyhow::Result<Vec<Vec<u8>>> {
    ...
    pub fn full_deps(&self, ika_dependencies_map: HashMap<String, ObjectID>) -> anyhow::Result<Vec<ObjectID>> {
        let mut ika_dependencies = Vec::new();
