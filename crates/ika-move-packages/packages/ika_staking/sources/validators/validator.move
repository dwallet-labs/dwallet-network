...
public(package) fun new<Witness: drop>(
  current_epoch: u64,
  name: String,
  ...
  let validator_cap = validator_cap::new_validator_cap_internal(validator_id, ctx);
  let validator_operation_cap = validator_cap::new_validator_operation_cap_internal(
    validator_id,
    ctx,
  );
  let validator_commission_cap = validator_cap::new_validator_commission_cap_internal(
    validator_id,
    ctx,
  );
...
public(package) fun activate<Witness: drop>(
  validator: &mut Validator,
  validator_cap: &ValidatorCap<Witness>,
  current_epoch: u64,
...
public(package) fun set_withdrawing<Witness: drop>(
  validator: &mut Validator,
  validator_cap: &ValidatorCap<Witness>,
  current_epoch: u64,
) {
...
public(package) fun withdraw_stake(
  ...
  let rewards_amount = if (!in_current_committee && !in_next_committee && staked_ika.is_staked()) {
...
public(package) fun set_name<Witness: drop>(
  self: &mut Validator,
  name: String,
  cap: &ValidatorOperationCap<Witness>,
) {
...
public(package) fun rotate_operation_cap<Witness: drop>(
  ...
  let operation_cap = validator_cap::new_validator_operation_cap_internal(validator_id, ctx);
...
public(package) fun rotate_commission_cap<Witness: drop>(
  ...
  let validator_id = cap.validator_id();
  self.verify_validator_cap(cap);
  let commission_cap = validator_cap::new_validator_commission_cap_internal(validator_id, ctx);
