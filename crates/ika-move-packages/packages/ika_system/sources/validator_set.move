...
use ika_system::staking_pool::{
    ...
    pool_id,
    FungibleStakedIka,
    fungible_staked_ika_pool_id
};
use ika_system::validator::{Validator, staking_pool_id, sui_address};
use ika_system::validator_cap::{Self, UnverifiedValidatorOperationCap, ValidatorOperationCap};
use ika_system::validator_wrapper::{Self, ValidatorWrapper};
use ika_system::voting_power;
...
use sui::bls12381::{Self, bls12381_min_sig_verify};
use sui::group_ops;
...
#[error]
const ETotalValidatorOrder: vector<u8> = b"Signers isn't monotonically increasing according to `validator_index`.";
#[error]
...
public(package) fun launch_ika(self: &mut ValidatorSet) {
    self.process_pending_validators(0);
...
/// Called by `ika_system` to remove a validator candidate, and move them to `inactive_validators`.
...
public(package) fun request_add_stake(
    ...
    validator.request_add_stake(epoch, stake, ctx.sender(), ctx)
}
...
public(package) fun request_withdraw_stake(
    ...
    let staking_pool_id = pool_id(&staked_ika);
    let validator = if (self.staking_pool_mappings.contains(staking_pool_id)) {
        // This is an active validator.
        let validator_address = self.staking_pool_mappings[pool_id(&staked_ika)];
    ...
    validator.request_withdraw_stake(epoch, staked_ika, ctx)
}
...
public(package) fun convert_to_fungible_staked_ika(
    ...
    validator.convert_to_fungible_staked_ika(staked_ika, ctx)
}
...
public(package) fun redeem_fungible_staked_ika(
    ...
    let staking_pool_id = fungible_staked_ika_pool_id(&fungible_staked_ika);
    ...
    validator.redeem_fungible_staked_ika(fungible_staked_ika, ctx)
}
...
public(package) fun advance_epoch(
    ...
    // computation_reward: &mut Balance<IKA>,
    // storage_fund_reward: &mut Balance<IKA>,
    ...
    adjust_stake_and_computation_price(&mut self.active_validators);

    process_pending_stakes_and_withdraws(&mut self.active_validators, new_epoch);
    ...
    process_pending_removals(self, new_epoch, validator_report_records, ctx);
...
/// Derive the computation price per unit size based on the gas price quote submitted by each validator.
/// The returned gas price should be greater than or equal to 2/3 of the validators submitted
/// gas price, weighted by stake.
public fun derive_computation_price_per_unit_size(self: &ValidatorSet): u64 {
    ...
    while (i < num_validators) {
        let v = &vs[i];
        entries.push_back(
            pq::new_entry(v.computation_price(), v.voting_power()),
        );
        i = i + 1;
    };
...
public fun get_active_validator_ref(self: &ValidatorSet, validator_address: address): &Validator {
    let mut validator_index_opt = find_validator(&self.active_validators, validator_address);
    assert!(validator_index_opt.is_some(), ENotAValidator);
    let validator_index = validator_index_opt.extract();
...
public fun get_pending_validator_ref(self: &ValidatorSet, validator_address: address): &Validator {
    let mut validator_index_opt = find_validator_from_table_vec(
        &self.pending_active_validators,
        validator_address,
    );
...
fun process_pending_stakes_and_withdraws(validators: &mut vector<Validator>, new_epoch: u64) {
    let length = validators.length();
    let mut i = 0;
    while (i < length) {
...
fun adjust_stake_and_computation_price(validators: &mut vector<Validator>) {
    let length = validators.length();
    let mut i = 0;
    while (i < length) {
        let validator = &mut validators[i];
...
fun compute_unadjusted_reward_distribution(
    ...
    while (i < length) {
        ...
        reward_amounts.push_back(reward_amount as u64);
        i = i + 1;
    };
    reward_amounts
}
...
fun emit_validator_epoch_events(
    ...
    while (i < num_validators) {
        ...
        event::emit(ValidatorEpochInfoEventV1 {
            epoch: new_epoch,
            validator_address,
            reference_gas_survey_quote: v.computation_price(),
...
/// list of indexes
/// into the `validators` vector of the committee. If there is a certificate, the
/// function
...
public(package) fun verify_certificate(
    ...
    // Use the signers flags to construct the key and the voting powers.

    // Lower bound for the next `validator_index` to ensure they are monotonically
    // increasing
    let mut min_next_validator_index = 0;
    let mut aggregate_key = bls12381::g2_identity();
    let mut aggregate_voting_power = 0;

    signers.do_ref!(|validator_index| {
        let validator_index = *validator_index as u64;
        assert!(validator_index >= min_next_validator_index, ETotalValidatorOrder);
        min_next_validator_index = validator_index + 1;

        // Bounds check happens here
        let validator = &self.active_validators[validator_index];
        let key = validator.protocol_pubkey();
        let voting_power = validator.voting_power();

        aggregate_key = bls12381::g2_add(&aggregate_key, key);
        aggregate_voting_power = aggregate_voting_power + voting_power;
    });
...
/// Returns true if the voting power is more than the aggregate voting power of quorum validators of a committee.
public(package) fun verify_quorum(aggregate_voting_power: u64): bool {
    aggregate_voting_power >= voting_power::quorum_threshold()
}