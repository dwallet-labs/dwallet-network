...
/// 1. Define a new `IkaSystemStateInner`type (e.g. `IkaSystemStateInnerV1`).
/// 2. Define a data migration function that migrates the old `IkaSystemStateInner` to the new one (i.e. IkaSystemStateInnerV1).
/// 3. Replace all uses of `IkaSystemStateInner` with `IkaSystemStateInnerV1` in both ika_system.move and ika_system_state_inner.move,
...
public entry fun request_add_validator(wrapper: &mut IkaSystemState, ctx: &mut TxContext) {
    let self = load_system_state_mut(wrapper);
...
/// of the validator.
public entry fun request_remove_validator(wrapper: &mut IkaSystemState, ctx: &mut TxContext) {
    let self = load_system_state_mut(wrapper);
...
public entry fun rotate_operation_cap(self: &mut IkaSystemState, ctx: &mut TxContext) {
...
public entry fun update_validator_next_epoch_protocol_pubkey(
    ...
    let self = load_system_state_mut(self);
    self.update_validator_next_epoch_protocol_pubkey(protocol_pubkey, proof_of_possession_bytes, ctx)
}
...
public entry fun update_candidate_validator_protocol_pubkey(
    ...
    let self = load_system_state_mut(self);
    self.update_candidate_validator_protocol_pubkey(protocol_pubkey, proof_of_possession_bytes, ctx)
}
...
public fun process_checkpoint_message(
    wrapper: &mut IkaSystemState,
    signature: vector<u8>,
    signers: vector<u16>,
    message: vector<u8>,
    ...
    let self = load_system_state_mut(wrapper);
    self.process_checkpoint_message(signature, signers, message, ctx);
}

fun load_system_state(self: &mut IkaSystemState): &IkaSystemStateInnerV1 {
    load_inner_maybe_upgrade(self)
}

fun load_system_state_mut(self: &mut IkaSystemState): &mut IkaSystemStateInnerV1 {
    load_inner_maybe_upgrade(self)
}

fun load_inner_maybe_upgrade(self: &mut IkaSystemState): &mut IkaSystemStateInnerV1 {
    let inner: &mut IkaSystemStateInnerV1 = dynamic_field::borrow_mut(
        &mut self.id,
        self.version,
    );
...
#[test_only]
public fun get_stake_subsidy_stake_subsidy_distribution_counter(wrapper: &mut IkaSystemState): u64 {
    let self = load_system_state(wrapper);
    self.get_stake_subsidy_stake_subsidy_distribution_counter()
}

#[test_only]
public fun set_stake_subsidy_stake_subsidy_distribution_counter(wrapper: &mut IkaSystemState, counter: u64) {
    let self = load_system_state_mut(wrapper);
    self.set_stake_subsidy_stake_subsidy_distribution_counter(counter)
}

#[test_only]
public fun inner_mut_for_testing(wrapper: &mut IkaSystemState): &mut IkaSystemStateInnerV1 {
