...
fun emit_event_with_current_epoch<E: copy + drop + store>(session_id: &UID, current_epoch: u64) {
    let mut dwallet_event: DWalletEvent<_> = *(dynamic_field::borrow<DWalletSessionEventKey, DWalletEvent<E>>(
        session_id, DWalletSessionEventKey {}));
    dwallet_event.epoch = current_epoch;
...
fun re_emit_missed_events(
    ...
    while (self.first_session_sequence_number < self.next_session_sequence_number) {
        ...
        if (session.session_type == DWalletSessionType::DwalletNetworkDecryptionKeyDkg) {
            emit_event_with_current_epoch<DWalletNetworkDKGDecryptionKeyRequestEvent>(&session.id, self.current_epoch);
        } else if (session.session_type == DWalletSessionType::DWalletDKGFirstRound) {
            emit_event_with_current_epoch<DWalletDKGFirstRoundRequestEvent>(&session.id, self.current_epoch);
        } else if (session.session_type == DWalletSessionType::DWalletDKGSecondRound) {
            emit_event_with_current_epoch<DWalletDKGSecondRoundRequestEvent>(&session.id, self.current_epoch);
        } else if (session.session_type == DWalletSessionType::ECDSAPresign) {
            emit_event_with_current_epoch<ECDSAPresignRequestEvent>(&session.id, self.current_epoch);
        } else if (session.session_type == DWalletSessionType::ReEncryptUserShare) {
            emit_event_with_current_epoch<EncryptedShareVerificationRequestEvent>(&session.id, self.current_epoch);
        } else if (session.session_type == DWalletSessionType::ECDSASign) {
            emit_event_with_current_epoch<ECDSASignRequestEvent>(&session.id, self.current_epoch);
        } else if (session.session_type == DWalletSessionType::ECDSAFutureSign) {
            emit_event_with_current_epoch<ECDSAFutureSignRequestEvent>(&session.id, self.current_epoch);
