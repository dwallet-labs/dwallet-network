...
use ika_system::validator_cap::{Self, ValidatorOperationCap};
...
use sui::{bls12381::{G2, g2_from_bytes, bls12381_min_sig_verify}, group_ops::Element};
use sui::bcs;
...
const PROOF_OF_POSSESSION_INTENT_SCOPE: u8 = 0;

const BLS_KEY_LEN: u64 = 96;
...
public(package) fun new_metadata(
    ...
    let protocol_pubkey = g2_from_bytes(&protocol_pubkey_bytes);
...
public(package) fun new(
    ...
    assert!(computation_price < MAX_VALIDATOR_GAS_PRICE, EGasPriceHigherThanThreshold);
...
public(package) fun adjust_stake_and_computation_price(self: &mut Validator) {
...
public(package) fun request_add_stake(
    ...
    let staked_ika = self.staking_pool.request_add_stake(stake, stake_epoch, ctx);
...
public(package) fun convert_to_fungible_staked_ika(
    ...
    let fungible_staked_ika = self.staking_pool.convert_to_fungible_staked_ika(staked_ika, ctx);
...
public(package) fun redeem_fungible_staked_ika(
    ...
    let ika = self.staking_pool.redeem_fungible_staked_ika(fungible_staked_ika, ctx);
...
public(package) fun deposit_stake_rewards(self: &mut Validator, reward: Balance<IKA>) {
    self.next_epoch_stake = self.next_epoch_stake + reward.value();
    self.staking_pool.deposit_rewards(reward);
}

/// Process pending stakes and withdraws, called at the end of the epoch.
public(package) fun process_pending_stakes_and_withdraws(self: &mut Validator, new_epoch: u64) {
...
public fun protocol_pubkey(self: &Validator): &Element<G2> {
    &self.metadata.protocol_pubkey
}

public fun proof_of_possession_bytes(self: &Validator): &vector<u8> {
...
public fun next_epoch_proof_of_possession_bytes(self: &Validator): &Option<vector<u8>> {
...
public fun next_epoch_computation_price(self: &Validator): u64 {
...
public fun computation_price(self: &Validator): u64 {
...
public(package) fun effectuate_staged_metadata(self: &mut Validator) {
    ...
    if (next_epoch_protocol_pubkey_bytes(self).is_some()) {
        ...
        self.metadata.protocol_pubkey = g2_from_bytes(&self.metadata.protocol_pubkey_bytes);
...
public(package) fun verify_proof_of_possession(
    ...
    let intent_scope = PROOF_OF_POSSESSION_INTENT_SCOPE;
    ...
    message.append(sui::address::to_bytes(sui_address));
