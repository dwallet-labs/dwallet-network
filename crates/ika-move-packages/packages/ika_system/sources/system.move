...
/// The witness for the ValidatorSet and ValidatorCap changes.
/// Can be used multiple times if there is anything wrong with the ValidatorSet or ValidatorCaps.
public struct ValidatorWitness has drop {}
...
public(package) fun create(
  ...
  validators: ValidatorSet<ValidatorWitness>,
...
public fun request_add_validator_candidate(
  ...
  ValidatorCap<ValidatorWitness>,
  ValidatorOperationCap<ValidatorWitness>,
  ValidatorCommissionCap<ValidatorWitness>,
) {
  self
    .inner_mut()
...
public fun request_remove_validator_candidate(
  self: &mut System,
  cap: &ValidatorCap<ValidatorWitness>,
) {
...
public fun request_add_validator(self: &mut System, cap: &ValidatorCap<ValidatorWitness>) {
...
/// of the validator.
public fun request_remove_validator(self: &mut System, cap: &ValidatorCap<ValidatorWitness>) {
...
public fun set_next_commission(
  ...
  cap: &ValidatorOperationCap<ValidatorWitness>,
) {
...
public fun collect_commission(
  self: &mut System,
  cap: &ValidatorCommissionCap<ValidatorWitness>,
  amount: Option<u64>,
  ctx: &mut TxContext,
): Coin<IKA> {
...
public fun verify_validator_cap(
  self: &System,
  cap: &ValidatorCap<ValidatorWitness>,
): VerifiedValidatorCap<ValidatorWitness> {
  self.inner().verify_validator_cap(cap, create_validator_witness())
}

public fun verify_operation_cap(
  self: &System,
  cap: &ValidatorOperationCap<ValidatorWitness>,
): VerifiedValidatorOperationCap<ValidatorWitness> {
  self.inner().verify_operation_cap(cap, create_validator_witness())
}

public fun verify_commission_cap(
  self: &System,
  cap: &ValidatorCommissionCap<ValidatorWitness>,
): VerifiedValidatorCommissionCap<ValidatorWitness> {
  self.inner().verify_commission_cap(cap, create_validator_witness())
}
...
public fun migrate<Witness: drop>(self: &mut System) {
  assert!(self.version < VERSION, EInvalidMigration);

  // Move the old system state inner to the new version.
  let system_inner: SystemInner<Witness> = dynamic_field::remove(&mut self.id, self.version);
...
public(package) fun create_validator_witness(): ValidatorWitness {
  ValidatorWitness {}
}
...
fun inner_mut(self: &mut System): &mut SystemInner<ValidatorWitness> {
...
fun inner(self: &System): &SystemInner<ValidatorWitness> {
  assert!(self.version == VERSION, EWrongInnerVersion);
...
public fun validator_set(self: &mut System): &ValidatorSet<ValidatorWitness> {
...
#[test_only]
public fun inner_mut_for_testing(self: &mut System): &mut SystemInner<ValidatorWitness> {
  self.inner_mut()
}
