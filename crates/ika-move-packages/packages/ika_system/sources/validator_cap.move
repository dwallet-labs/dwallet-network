...
/// (such as computation price per unit size setting or tallying rule reporting) from fund/staking, it
...
public(package) fun new_unverified_validator_operation_cap_and_transfer(
    ...
    // This function needs to be called only by the validator itself.address
    assert!(ctx.sender() == validator_address, 0);
