    ...
    let principal_withdraw_amount =
        (
            ...
            * (fungible_staked_ika_data_principal_amount as u128)
            / (fungible_staked_ika_data_total_supply as u128),
        ) as u64;
...
public(package) fun convert_to_fungible_staked_ika(
    ...
    assert!(pool_id == object::id(pool), EWrongPool);
    assert!(tx_context::epoch(ctx) >= stake_activation_epoch, ECannotMintFungibleStakedIkaYet);
    ...
    if (!bag::contains(&pool.extra_fields, FungibleStakedIkaDataKey {})) {
        ...
        fungible_staked_ika_data.total_supply =
            fungible_staked_ika_data.total_supply + pool_token_amount;
...
public fun fungible_staked_ika_pool_id(fungible_staked_ika: &FungibleStakedIka): ID {
    fungible_staked_ika.pool_id
}
...
#[test_only]
public(package) fun fungible_staked_ika_data_total_supply(
    fungible_staked_ika_data: &FungibleStakedIkaData,
): u64 {
    fungible_staked_ika_data.total_supply
}
...
#[test_only]
public(package) fun fungible_staked_ika_data_principal_value(
    fungible_staked_ika_data: &FungibleStakedIkaData,
): u64 {
