...
use pera_move_build::CompiledPackage;
use pera_sdk::apis::ReadApi;
use pera_sdk::error::Error as SdkError;
use pera_sdk::rpc_types::{PeraObjectDataOptions, PeraRawData, PeraRawMovePackage};
...
impl<'a> BytecodeSourceVerifier<'a> {
    ...
    async fn pkg_for_address(&self, addr: AccountAddress) -> Result<PeraRawMovePackage, Error> {
        // Move packages are specified with an AccountAddress, but are
        // fetched from a pera network via pera_getObject, which takes an object ID
        let obj_id = ObjectID::from(addr);

        // fetch the Pera object at the address specified for the package in the local resolution table
        ...
        let obj_read = self
            .rpc_client
            .get_object_with_options(obj_id, PeraObjectDataOptions::new().with_bcs())
            .await
            .map_err(Error::DependencyObjectReadFailure)?;

        let obj = obj_read
            .into_object()
            .map_err(Error::PeraObjectRefFailure)?
            .bcs
            .ok_or_else(|| {
        ...
        match obj {
            PeraRawData::Package(pkg) => Ok(pkg),
            PeraRawData::MoveObject(move_obj) => {
