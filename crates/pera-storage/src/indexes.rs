...
//! IndexStore supports creation of various ancillary indexes of state in PeraDataStore.
...
type OwnerIndexKey = (PeraAddress, ObjectID);
type CoinIndexKey = (PeraAddress, String, ObjectID);
...
pub struct IndexStoreCaches {
    per_coin_type_balance: ShardedLruCache<(PeraAddress, TypeTag), PeraResult<TotalBalance>>,
    all_balances: ShardedLruCache<PeraAddress, PeraResult<Arc<HashMap<TypeTag, TotalBalance>>>>,
    locks: MutexTable<PeraAddress>,
}

#[derive(Default)]
pub struct IndexStoreCacheUpdates {
    _locks: Vec<OwnedMutexGuard<()>>,
    per_coin_type_balance_changes: Vec<((PeraAddress, TypeTag), PeraResult<TotalBalance>)>,
    all_balance_changes: Vec<(PeraAddress, PeraResult<Arc<AllBalance>>)>,
}

#[derive(DBMapUtils)]
pub struct IndexStoreTables {
    /// Index from pera address to transactions initiated by that address.
    #[default_options_override_fn = "transactions_from_addr_table_default_config"]
    transactions_from_addr: DBMap<(PeraAddress, TxSequenceNumber), TransactionDigest>,

    /// Index from pera address to transactions that were sent to that address.
    #[default_options_override_fn = "transactions_to_addr_table_default_config"]
    transactions_to_addr: DBMap<(PeraAddress, TxSequenceNumber), TransactionDigest>,
    ...
    /// composite key of the PeraAddress of their owner and the object ID of the object.
    ...
    #[default_options_override_fn = "index_table_default_config"]
    event_by_sender: DBMap<(PeraAddress, EventId), EventIndex>,
    #[default_options_override_fn = "index_table_default_config"]
...
impl IndexStore {
    ...
    pub async fn index_coin(
    ...
    ) -> PeraResult<IndexStoreCacheUpdates> {
        ...
        let mut addresses: HashSet<PeraAddress> = HashSet::new();
        addresses.extend(
        ...
        let mut balance_changes: HashMap<PeraAddress, HashMap<TypeTag, TotalBalance>> =
            HashMap::new();
        // Index coin info
            ...
            .flat_map(|(address, balance_map)| {
                balance_map.iter().map(|(type_tag, balance)| {
                    (
                        ...
                        Ok::<TotalBalance, PeraError>(*balance),
                    )
                })
            })
            .collect();
        let all_balance_changes: Vec<_> = balance_changes
            ...
            .map(|(address, balance_map)| {
                (
                    address,
                    Ok::<Arc<HashMap<TypeTag, TotalBalance>>, PeraError>(Arc::new(balance_map)),
                )
            })
            .collect();
    ...
    pub fn get_transactions(
        ...
        let cursor = if let Some(cursor) = cursor {
            Some(
                self.get_transaction_seq(&cursor)?
                    .ok_or(PeraError::TransactionNotFound { digest: cursor })?,
            )
        } else {
            None
        };
        match filter {
            ...
            Some(_) => Err(PeraError::UserInputError {
    ...
    pub fn get_transaction_seq(
        &self,
        digest: &TransactionDigest,
    ) -> PeraResult<Option<TxSequenceNumber>> {
    ...
    pub fn all_events(
        ...
        limit: usize,
        descending: bool,
    ) -> PeraResult<Vec<(TransactionEventsDigest, TransactionDigest, usize, u64)>> {
        Ok(if descending {
            self.tables
                .event_order
                .unbounded_iter()
    ...
    pub fn events_by_transaction(
        ...
        let seq = self
            ...
            .ok_or(PeraError::TransactionNotFound { digest: *digest })?;
        Ok(if descending {
            self.tables
                .event_order
                .unbounded_iter()
    ...
    pub fn insert_genesis_objects(&self, object_index_changes: ObjectIndexChanges) -> PeraResult {
    ...
    pub fn checkpoint_db(&self, path: &Path) -> PeraResult {
    ...
    pub async fn get_balance(
        ...
        if force_disable_cache {
            ...
            .unwrap()
            .map_err(|e| {
                PeraError::ExecutionError(format!("Failed to read balance frm DB: {:?}", e))
            });
        }
    ...
    pub async fn get_all_balance(
        &self,
        owner: PeraAddress,
    ) -> PeraResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        let force_disable_cache = read_size_from_env(ENV_VAR_DISABLE_INDEX_CACHE).unwrap_or(0) > 0;
        let metrics_cloned = self.metrics.clone();
        let coin_index_cloned = self.tables.coin_index.clone();
        if force_disable_cache {
    ...
    /// Read balance for a `PeraAddress` and `CoinType` from the backend database
    ...
    /// Read all balances for a `PeraAddress` from the backend database
    ...
    async fn invalidate_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = (PeraAddress, TypeTag)>,
    ) -> PeraResult {
        self.caches
            .per_coin_type_balance
    ...
    async fn update_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = ((PeraAddress, TypeTag), PeraResult<TotalBalance>)>,
    ) -> PeraResult {
        self.caches
            .per_coin_type_balance
    ...
    fn merge_balance(
        old_balance: &PeraResult<TotalBalance>,
        balance_delta: &PeraResult<TotalBalance>,
    ) -> PeraResult<TotalBalance> {
    ...
    async fn update_all_balance_cache(
        &self,
        keys: impl IntoIterator<Item = (PeraAddress, PeraResult<Arc<HashMap<TypeTag, TotalBalance>>>)>,
    ) -> PeraResult {
        self.caches
            .all_balances
    ...
    fn merge_all_balance(
        old_balance: &PeraResult<Arc<HashMap<TypeTag, TotalBalance>>>,
        balance_delta: &PeraResult<Arc<HashMap<TypeTag, TotalBalance>>>,
    ) -> PeraResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        if let Ok(old_balance) = old_balance {
            if let Ok(balance_delta) = balance_delta {
...
mod tests {
    ...
    async fn test_index_cache() -> anyhow::Result<()> {
        ...
        let address: PeraAddress = AccountAddress::random().into();
