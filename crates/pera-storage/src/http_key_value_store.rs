...
trait IntoPeraResult<T> {
    fn into_pera_result(self) -> PeraResult<T>;
}

impl<T, E> IntoPeraResult<T> for Result<T, E>
where
    E: std::error::Error,
{
    fn into_pera_result(self) -> PeraResult<T> {
        self.map_err(|e| PeraError::Storage(e.to_string()))
    }
}
...
fn key_to_path_elements(key: &Key) -> PeraResult<(String, &'static str)> {
    match key {
...
impl HttpKVStore {
    ...
    pub fn new(base_url: &str) -> PeraResult<Self> {
        ...
        let base_url = Url::parse(&base_url).into_pera_result()?;

        Ok(Self { base_url, client })
    }

    fn get_url(&self, key: &Key) -> PeraResult<Url> {
        let (digest, item_type) = key_to_path_elements(key)?;
        let joined = self
            .base_url
            .join(&format!("{}/{}", digest, item_type))
            .into_pera_result()?;
        Url::from_str(joined.as_str()).into_pera_result()
    }

    async fn multi_fetch(&self, uris: Vec<Key>) -> Vec<PeraResult<Option<Bytes>>> {
    ...
    async fn fetch(&self, key: Key) -> PeraResult<Option<Bytes>> {
        ...
        if resp.status().is_success() {
            resp.bytes().await.map(Some).into_pera_result()
        } else {
            Ok(None)
        }
    }
...
fn map_fetch<'a, K>(fetch: (&'a PeraResult<Option<Bytes>>, &'a K)) -> Option<(&'a Bytes, &'a K)>
where
    K: std::fmt::Debug,
{
    let (fetch, key) = fetch;
