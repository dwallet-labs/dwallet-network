...
impl TransactionKeyValueStore {
    ...
    ) -> PeraResult<Vec<Option<CertifiedCheckpointSummary>>> {
    ...
    pub async fn multi_get_checkpoints_contents(
        &self,
        keys: &[CheckpointSequenceNumber],
    ) -> PeraResult<Vec<Option<CheckpointContents>>> {
        self.multi_get_checkpoints(&[], keys, &[], &[])
            .await
    ...
    ) -> PeraResult<Vec<Option<Transaction>>> {
        self.multi_get(keys, &[], &[])
            .await
            .map(|(txns, _, _)| txns)
    }
    ...
    pub async fn get_tx(&self, digest: TransactionDigest) -> PeraResult<Transaction> {
        self.multi_get_tx(&[digest])
            .await?
            .into_iter()
            .next()
            .flatten()
            .ok_or(PeraError::TransactionNotFound { digest })
    }
    ...
    ) -> PeraResult<CertifiedCheckpointSummary> {
