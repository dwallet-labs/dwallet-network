...
// Version 3: gas model v2, including all pera conservation fixes. Fix for loaded child object
...
//            `max_meter_ticks_per_module` limits each from 6_000_000 to 16_000_000. pera-system
//            framework changes.
...
//             Changes to pera-system package to add PTB-friendly unstake function, and minor cleanup.
...
//             Add `pera::table_vec::drop` to the framework via a system package upgrade.
...
// Version 19: Changes to pera-system package to enable liquid staking.
...
// Version 25: Add pera::table_vec::swap and pera::table_vec::swap_remove to system packages.
// Version 26: New gas model version.
//             Add support for receiving objects off of other objects in devnet only.
// Version 28: Add pera::zklogin::verify_zklogin_id and related functions to pera framework.
//             Enable transaction effects v2 in devnet.
// Version 29: Add verify_legacy_zklogin_address flag to pera framework, this add ability to verify
...
//             Add support for getting object ID referenced by receiving object in pera framework.
//             Create new execution layer version, and preserve previous behavior in v1.
//             Update semantics of `pera::transfer::receive` and add `pera::transfer::public_receive`.
// Version 32: Add delete functions for VerifiedID and VerifiedIssuer.
//             Add pera::token module to pera framework.
...
// Version 42: Migrate pera framework and related code to Move 2024
...
//             Modified pera-system package to enable withdrawal of stake before it becomes active.
//             Enable soft bundle in devnet and testnet.
//             Core macro visibility in pera core framework.
...
pub struct ProtocolConfig {
    ...
    /// Max size of objects a transaction can write to disk after completion. Enforce by the Pera adapter.
    ...
    /// Max size of objects a system transaction can write to disk after completion. Enforce by the Pera adapter.
    ...
    /// Maximum size of the `contents` part of an object, in bytes. Enforced by the Pera adapter when effects are produced.
    ...
    /// Maximum size of a Move package object, in bytes. Enforced by the Pera adapter at the end of a publish transaction.
    ...
    /// Maximum gas budget in NPERA that a transaction can use.
    max_tx_gas: Option<u64>,

    /// Maximum amount of the proposed gas price in NPERA (defined in the transaction).
    ...
    // === Execution gas costs ====
    /// Base cost for any Pera transaction
    ...
    /// Per-byte cost of storing an object in the Pera global object store. Some of this cost may be refundable if the object is later freed
    obj_data_cost_refundable: Option<u64>,

    // Per-byte cost of storing an object in the Pera transaction log (e.g., in CertifiedTransactionEffects)
    ...
    /// Unit gas price, NPera per internal gas unit.
        ...
        for cur in 2..=version.0 {
            match cur {
                ...
                3 => {
                    ...
                    // max gas budget is in NPERA and an absolute value 50PERA
                    cfg.max_tx_gas = Some(50_000_000_000);
                    // min gas budget is in NPERA and an absolute value 2000NPERA or 0.000002PERA
...
macro_rules! check_limit_by_meter {
    ($is_metered:expr, $x:expr, $metered_limit:expr, $unmetered_hard_limit:expr, $metric:expr) => {{
        ...
        use pera_protocol_config::check_limit_in_range;
