...
mod checked {
    ...
    use pera_types::metrics::BytecodeVerifierMetrics;
    ...
    ) -> PeraResult<(PeraGasStatus, CheckedInputObjects)> {
    ...
    pub fn check_dev_inspect_input(
    ...
    ) -> PeraResult<CheckedInputObjects> {
        ...
        let mut used_objects: HashSet<PeraAddress> = HashSet::new();
    ...
    fn check_gas(
        ...
        if tx_kind.is_system_tx() {
            Ok(PeraGasStatus::new_unmetered())
        } else {
            let gas_status =
                PeraGasStatus::new(gas_budget, gas_price, reference_gas_price, protocol_config)?;
    ...
    pub fn check_non_system_packages_to_be_published(
        ...
        let mut verifier = pera_execution::verifier(protocol_config, for_signing, metrics);
