...
use pera_types::coin::{TreasuryCap, COIN_MODULE_NAME};
...
async fn test_get_package_with_display_should_not_fail() -> Result<(), anyhow::Error> {
    ...
    let response = http_client
        .get_object(
            ObjectID::from(PERA_FRAMEWORK_ADDRESS),
            Some(PeraObjectDataOptions::new().with_display()),
        )
        .await;
    assert!(response.is_ok());
    let response: PeraObjectResponse = response?;
    assert!(response
        .into_object()
        .unwrap()
        .display
...
async fn test_public_transfer_object() -> Result<(), anyhow::Error> {
    ...
    let tx_response: PeraTransactionBlockResponse = http_client
...
async fn test_publish() -> Result<(), anyhow::Error> {
    ...
    let tx_response = http_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(PeraTransactionBlockResponseOptions::new().with_effects()),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;
    matches!(tx_response, PeraTransactionBlockResponse {effects, ..} if effects.as_ref().unwrap().created().len() == 6);
    Ok(())
}
...
async fn test_move_call() -> Result<(), anyhow::Error> {
    ...
    // now do the call
    let package_id = ObjectID::new(PERA_FRAMEWORK_ADDRESS.into_bytes());
    ...
    let tx_response = http_client
        ...
        .await?;
    matches!(tx_response, PeraTransactionBlockResponse {effects, ..} if effects.as_ref().unwrap().created().len() == 1);
    Ok(())
}
...
async fn test_get_object_info() -> Result<(), anyhow::Error> {
    ...
    for obj in objects {
        ...
        let result = http_client
            .get_object(
                oref.object_id,
                Some(PeraObjectDataOptions::new().with_owner()),
            )
            .await?;
        assert!(
            matches!(result, PeraObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.unwrap().get_owner_address()? == address)
        );
    }
    Ok(())
}
...
async fn test_get_object_data_with_content() -> Result<(), anyhow::Error> {
    ...
    let objects = http_client
        .get_owned_objects(
            address,
            Some(PeraObjectResponseQuery::new_with_options(
                PeraObjectDataOptions::new().with_content().with_owner(),
            )),
            None,
            None,
        )
    ...
    for obj in objects {
        ...
        let result = http_client
            .get_object(
                oref.object_id,
                Some(PeraObjectDataOptions::new().with_content().with_owner()),
            )
            .await?;
        assert!(
            matches!(result, PeraObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.unwrap().get_owner_address()? == address)
...
async fn test_get_coins() -> Result<(), anyhow::Error> {
    ...
    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::pera::TestCoin".into()), None, None)
        .await?;
    assert_eq!(0, result.data.len());

    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::pera::PERA".into()), None, None)
        .await?;
    assert_eq!(5, result.data.len());
    assert!(!result.has_next_page);
    ...
    // Test paging
    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::pera::PERA".into()), None, Some(3))
        .await?;
...
async fn test_get_balance() -> Result<(), anyhow::Error> {
    ...
    assert_eq!("0x2::pera::PERA", result.coin_type);
    assert_eq!(
...
async fn test_get_metadata() -> Result<(), anyhow::Error> {
    ...
    let result: PeraCoinMetadata = http_client
...
async fn test_get_total_supply() -> Result<(), anyhow::Error> {
    ...
    let treasury_cap = object_changes
        ...
        .find_map(|e| {
            ...
            {
                if &TreasuryCap::type_(parse_pera_struct_tag(&coin_name).unwrap()) == object_type {
                    Some(object_id)
                } else {
                    None
                }
    ...
    let PeraTransactionBlockResponse { effects, .. } = tx_response;

    assert_eq!(PeraExecutionStatus::Success, *effects.unwrap().status());
...
async fn test_staking() -> Result<(), anyhow::Error> {
    ...
    // Check StakedPera object before test
    let staked_pera: Vec<DelegatedStake> = http_client.get_stakes(address).await?;
    assert!(staked_pera.is_empty());
    ...
    http_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(PeraTransactionBlockResponseOptions::new()),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;
    ...
    let staked_pera: Vec<DelegatedStake> = http_client.get_stakes(address).await?;
    assert_eq!(1, staked_pera.len());
    assert_eq!(1000000000, staked_pera[0].stakes[0].principal);
    assert!(matches!(
        staked_pera[0].stakes[0].status,
        StakeStatus::Pending
    ));
    let staked_pera_copy = http_client
        .get_stakes_by_ids(vec![staked_pera[0].stakes[0].staked_pera_id])
        .await?;
    assert_eq!(
        staked_pera[0].stakes[0].staked_pera_id,
        staked_pera_copy[0].stakes[0].staked_pera_id
    );
    Ok(())
}
...
async fn test_unstaking() -> Result<(), anyhow::Error> {
    ...
    let staked_pera_copy = http_client
        .get_stakes_by_ids(vec![
            staked_pera[0].stakes[0].staked_pera_id,
            staked_pera[0].stakes[1].staked_pera_id,
            staked_pera[0].stakes[2].staked_pera_id,
        ])
        .await?;

    assert!(matches!(
        &staked_pera_copy[0].stakes[0].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    assert!(matches!(
        &staked_pera_copy[0].stakes[1].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    assert!(matches!(
        &staked_pera_copy[0].stakes[2].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    ...
    let transaction_bytes: TransactionBlockBytes = http_client
        .request_withdraw_stake(
            address,
            staked_pera_copy[0].stakes[2].staked_pera_id,
            None,
            1_000_000.into(),
        )
        .await?;
