...
use pera_json_rpc_types::{
    PeraObjectDataOptions, PeraObjectResponseQuery, PeraTransactionBlockResponse,
    PeraTransactionBlockResponseOptions, TransactionBlockBytes,
};
...
use pera_json_rpc_api::{IndexerApiClient, TransactionBuilderClient, WriteApiClient};
...
async fn test_get_transaction_block() -> Result<(), anyhow::Error> {
    ...
    let mut tx_responses: Vec<PeraTransactionBlockResponse> = Vec::new();
    ...
    // let batch_responses: Vec<PeraTransactionBlockResponse> = http_client
    //     .multi_get_transaction_blocks(tx, Some(PeraTransactionBlockResponseOptions::new()))
    //     .await?;
    ...
    //     assert!(tx_responses
    //         .iter()
    //         .any(|resp| matches!(resp, PeraTransactionBlockResponse {digest, ..} if *digest == r.digest)))
    // }

    // // test get_transaction
    // for tx_digest in tx {
    //     let response: PeraTransactionBlockResponse = http_client
    //         .get_transaction_block(
    //             tx_digest,
    //             Some(PeraTransactionBlockResponseOptions::new().with_raw_input()),
    //         )
    //         .await?;
    //     assert!(tx_responses.iter().any(
    //         |resp| matches!(resp, PeraTransactionBlockResponse {digest, ..} if *digest == response.digest)
    //     ));
...
async fn test_get_fullnode_transaction() -> Result<(), anyhow::Error> {
    ...
    let query = PeraTransactionBlockResponseQuery {
        options: Some(PeraTransactionBlockResponseOptions {
            show_input: true,
            show_effects: true,
            show_events: true,
            ..Default::default()
    ...
    let first_page = client
        ...
        .query_transaction_blocks(
            PeraTransactionBlockResponseQuery::default(),
            None,
            Some(5),
            false,
        )
    ...
    let address_txs_asc = client
        .read_api()
        .query_transaction_blocks(
            PeraTransactionBlockResponseQuery::new_with_filter(TransactionFilter::FromAddress(
...
async fn test_query_transaction_blocks() -> Result<(), anyhow::Error> {
    ...
    let pera_type_args = type_args![GAS::type_tag()]?;
    let type_args = pera_type_args
        .into_iter()
        .map(|ty| ty.try_into())
        .collect::<Result<Vec<_>, _>>()?;

    let pera_call_args_1 = call_args!(coin.data.clone().unwrap().object_id, 10)?;
    ...
    let pera_call_args_2 = call_args!(coin_2.data.clone().unwrap().object_id, 10)?;
    ...
    let move_call_query = PeraTransactionBlockResponseQuery::new_with_filter(filter);
    let tx = client
        .read_api()
        .query_transaction_blocks(move_call_query, None, Some(20), true)
        .await
        .unwrap();
    // verify that only 1 tx is returned and no PeraRpcInputError::ContainsDuplicates error
