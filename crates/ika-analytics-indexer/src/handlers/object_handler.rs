...
use ika_types::SYSTEM_PACKAGE_ADDRESSES;
use tokio::sync::Mutex;

use ika_json_rpc_types::IkaMoveStruct;
...
impl ObjectHandler {
    ...
    async fn process_object(
        ...
        let (struct_tag, ika_move_struct) = if let Some(move_struct) = move_struct {
            match move_struct.into() {
                IkaMoveStruct::WithTypes { type_, fields } => {
                    (Some(type_), Some(IkaMoveStruct::WithFields(fields)))
                }
        ...
        let entry = ObjectEntry {
            ...
            object_json: ika_move_struct.map(|x| x.to_json_value().to_string()),
        };
        state.objects.push(entry);
        Ok(())
    }
