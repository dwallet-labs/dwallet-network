...
module test::m1 {
    ...
    public entry fun add_child(parent: &mut Object, ctx: &mut TxContext) {
        ...
        ika::dynamic_object_field::add(&mut parent.id, 0, child);
    }

    public fun transfer_child(parent: &mut Object, ctx: &TxContext) {
        let child: Child = ika::dynamic_object_field::remove(&mut parent.id, 0);
        ika::dynamic_object_field::add(&mut parent.id, 1, child);
        let child: Child = ika::dynamic_object_field::remove(&mut parent.id, 1);
    ...
    public fun delete_child(parent: &mut Object) {
        let child: Child = ika::dynamic_object_field::remove(&mut parent.id, 0);
        ika::dynamic_object_field::add(&mut parent.id, 1, child);
        let Child { id } = ika::dynamic_object_field::remove(&mut parent.id, 1);
        object::delete(id);
    }
    ...
    public fun wrap_child(parent: &mut Object) {
        let child: Child = ika::dynamic_object_field::remove(&mut parent.id, 0);
        ika::dynamic_object_field::add(&mut parent.id, 1, child);
        let child = ika::dynamic_object_field::remove(&mut parent.id, 1);
