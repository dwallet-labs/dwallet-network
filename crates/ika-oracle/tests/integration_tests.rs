...
use ika_json_rpc_types::{ObjectChange, IkaExecutionStatus};
use ika_keys::keystore::{AccountKeystore, FileBasedKeystore, Keystore};
use ika_move_build::BuildConfig;
use ika_sdk::rpc_types::IkaTransactionBlockResponseOptions;
use ika_sdk::types::base_types::{ObjectID, IkaAddress};
use ika_sdk::types::programmable_transaction_builder::ProgrammableTransactionBuilder;
use ika_sdk::types::quorum_driver_types::ExecuteTransactionRequestType;
use ika_sdk::types::transaction::{CallArg, ObjectArg, Transaction, TransactionData};
use ika_sdk::types::Identifier;
use ika_sdk::{IkaClient, IkaClientBuilder};
use ika_types::base_types::{ObjectRef, SequenceNumber};
use ika_types::{parse_ika_type_tag, TypeTag};

// Integration tests for IKA Oracle, these test can be run manually on local or remote testnet.
#[ignore]
#[tokio::test]
...
async fn test_publish_complex_value() {
    ...
    for i in 1..200 {
        ...
        builder.programmable_move_call(
            ...
            module.clone(),
            submit_data.clone(),
            vec![parse_ika_type_tag(&format!("{package}::decimal_value::DecimalValue")).unwrap()],
...
async fn test_consume_oracle_data() {
    ...
    for _ in 0..3 {
        ...
        let ticker = builder
            .input(CallArg::Pure(bcs::to_bytes("IKAUSD".as_bytes()).unwrap()))
            .unwrap();
        let identifier = builder
            .input(CallArg::Pure(
                bcs::to_bytes("identifier".as_bytes()).unwrap(),
    ...
    builder.programmable_move_call(
        ...
        trusted_fx,
        vec![],
        vec![oracles[0], oracles[1], oracles[2], nika_amount],
    );
...
async fn get_gas(client: &IkaClient, sender: IkaAddress) -> (ObjectRef, u64) {
    let gas = client
        .coin_read_api()
...
async fn init_test_client() -> (IkaClient, Keystore, IkaAddress) {
    let client = IkaClientBuilder::default()
        .build("https://rpc.devnet.ika.io:443")
        .await
        .unwrap();
    ...
    let keystore = Keystore::File(
        FileBasedKeystore::new(
            &dirs::home_dir()
                .unwrap()
                .join(".ika/ika_config/ika.keystore"),
        )
        .unwrap(),
    );
    let sender: IkaAddress = keystore.addresses()[0];
    let gas = client
        .coin_read_api()
        .get_coins(sender, None, None, Some(1))
        .await
