...
use ika_json_rpc_types::{
    IkaObjectDataOptions, IkaTransactionBlockEffects, IkaTransactionBlockEffectsAPI,
    IkaTransactionBlockResponseOptions,
};
use ika_sdk::apis::ReadApi;
use ika_sdk::rpc_types::IkaObjectResponse;
...
use ika_types::quorum_driver_types::NON_RECOVERABLE_ERROR_MSG;
use ika_types::transaction::{Argument, Transaction};
use ika_types::transaction::{Command, ObjectArg};
...
use ika_types::base_types::{random_object_ref, ObjectID, ObjectRef};
...
impl DataProviderRunner {
    pub async fn new(
        ...
        client: Arc<IkaClient>,
        signer_address: IkaAddress,
        metrics: Arc<OracleMetrics>,
    ) -> Self {
...
async fn get_gas_obj_ref(
    read_api: &ReadApi,
    gas_obj_id: ObjectID,
    owner_address: IkaAddress,
) -> ObjectRef {
    loop {
        match read_api
            .get_object_with_options(gas_obj_id, IkaObjectDataOptions::default().with_owner())
            .await
            .map(|resp| resp.data)
        {
            Ok(Some(gas_obj)) => {
...
impl OnChainDataUploader {
    ...
    async fn upload(
    ...
    ) -> anyhow::Result<IkaTransactionBlockEffects> {
        ...
        for data_point in &data_points {
            ...
            builder.command(Command::move_call(
                ...
                vec![
                    parse_ika_type_tag(&format!("{package_id}::decimal_value::DecimalValue"))
                        .unwrap(),
                ],
                arguments,
            ));
    ...
    async fn execute(&mut self, tx: Transaction) -> anyhow::Result<IkaTransactionBlockResponse> {
        ...
        loop {
            match self
                ...
                Err(ika_sdk::error::Error::RpcError(err)) => {
                    // jsonrpsee translate every IkaError into jsonrpsee::core::Error, so we need to further distinguish 
        ...
        loop {
            ...
            for (feed_name, object_id) in &self.read_configs {
                match self
                    .client
                    .read_api()
                    .get_object_with_options(*object_id, IkaObjectDataOptions::default())
                    .await
                {
                    Ok(IkaObjectResponse {
                        data: Some(_data), ..
                    }) => {
