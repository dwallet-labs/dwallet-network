...
use pera_storage::indexes::TotalBalance;
use tap::TapFallible;
use tracing::{debug, info, instrument};

use mysten_metrics::spawn_monitored_task;
use pera_core::authority::AuthorityState;
use pera_json_rpc_api::{cap_page_limit, CoinReadApiOpenRpc, CoinReadApiServer, JsonRpcMetrics};
use pera_json_rpc_types::Balance;
use pera_json_rpc_types::{CoinPage, PeraCoinMetadata};
...
use pera_types::coin::{CoinMetadata, TreasuryCap};
use pera_types::effects::TransactionEffectsAPI;
use pera_types::gas_coin::{GAS, TOTAL_SUPPLY_NPERA};
...
pub fn parse_to_struct_tag(coin_type: &str) -> Result<StructTag, PeraRpcInputError> {
    parse_pera_struct_tag(coin_type)
        .map_err(|e| PeraRpcInputError::CannotParsePeraStructTag(format!("{e}")))
}

pub fn parse_to_type_tag(coin_type: Option<String>) -> Result<TypeTag, PeraRpcInputError> {
...
impl PeraRpcModule for CoinReadApi {
...
impl CoinReadApiServer for CoinReadApi {
                    ...
                    match obj {
                        Some(obj) => {
                            ...
                            if coin_type.is_none() {
                                Err(PeraRpcInputError::GenericInvalid(
                        ...
                        None => Err(PeraRpcInputError::GenericInvalid(
    ...
    #[instrument(skip(self))]
    async fn get_all_balances(&self, owner: PeraAddress) -> RpcResult<Vec<Balance>> {
        with_tracing!(async move {
    ...
    #[instrument(skip(self))]
    async fn get_coin_metadata(&self, coin_type: String) -> RpcResult<Option<PeraCoinMetadata>> {
        with_tracing!(async move {
...
async fn find_package_object_id(
    ...
    spawn_monitored_task!(async move {
        ...
        Err(PeraRpcInputError::GenericNotFound(format!(
...
mod tests {
    ...
    use pera_types::{parse_pera_struct_tag, TypeTag};
    ...
    mock! {
        ...
        impl TransactionKeyValueStoreTrait for KeyValueStore {
            ...
            ) -> PeraResult<KVStoreTransactionData>;
            ...
            ) -> PeraResult<KVStoreCheckpointData>;
            ...
            async fn get_object(&self, object_id: ObjectID, version: SequenceNumber) -> PeraResult<Option<Object>>;
    ...
    fn get_test_coin_type_tag(coin_type: String) -> TypeTag {
        TypeTag::Struct(Box::new(parse_pera_struct_tag(&coin_type).unwrap()))
    }
    ...
    fn get_test_treasury_cap_peripherals(
        ...
        let input_coin_struct = parse_pera_struct_tag(&coin_name).expect("should not fail");
    ...
    mod get_coins_tests {
        ...
        async fn test_coin_no_cursor() {
            ...
            let coin_type_tag =
                TypeTag::Struct(Box::new(parse_pera_struct_tag(&coin.coin_type).unwrap()));
            let mut mock_state = MockStateRead::new();
            mock_state
                .expect_get_owned_coins()
                .with(
        ...
        async fn test_unrecognized_token() {
            ...
            expected.assert_eq(&error_object.code().to_string());
            let expected =
                expect!["Invalid struct type: 0x2::pera:🤵. Got error: unrecognized token: :🤵"];
            expected.assert_eq(error_object.message());
        }
            ...
            mock_state
                ...
                .returning(move |_, _, _, _| {
                    Err(StateReadError::Client(
                        PeraError::IndexStoreNotAvailable.into(),
                    ))
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
        ...
        async fn test_get_coins_iterator_typed_store_error() {
            ...
            mock_state
                .expect_get_owned_coins()
                .returning(move |_, _, _, _| {
                    Err(PeraError::Storage("mock rocksdb error".to_string()).into())
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_coins(owner, Some(coin_type.to_string()), None, None)
    ...
    mod get_all_coins_tests {
        use pera_types::object::{MoveObject, Owner};
    ...
    mod get_balance_tests {
        ...
        async fn test_get_balance_execution_error() {
            ...
            let mut mock_state = MockStateRead::new();
            mock_state.expect_get_balance().returning(move |_, _| {
                Err(PeraError::ExecutionError("mock db error".to_string()).into())
            });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_balance(owner, Some(coin_type.to_string()))
    ...
    mod get_coin_metadata_tests {
        ...
        async fn test_valid_coin_metadata_object() {
            ...
            let metadata = PeraCoinMetadata::try_from(coin_metadata_object.clone()).unwrap();
        ...
        #[tokio::test]
        async fn test_find_package_object_not_pera_coin_metadata() {
            let package_id = get_test_package_id();
            let coin_name = get_test_coin_type(package_id);
            let input_coin_struct = parse_pera_struct_tag(&coin_name).expect("should not fail");
            let coin_metadata_struct = CoinMetadata::type_(input_coin_struct.clone());
