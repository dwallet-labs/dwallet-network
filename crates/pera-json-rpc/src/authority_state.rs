...
use pera_core::subscription_handler::SubscriptionHandler;
use pera_json_rpc_types::{
    Coin as PeraCoin, DevInspectResults, DryRunTransactionBlockResponse, EventFilter, PeraEvent,
    PeraObjectDataFilter, TransactionFilter,
};
...
use pera_types::digests::{ChainIdentifier, TransactionDigest, TransactionEventsDigest};
...
pub trait StateRead: Send + Sync {
    ...
    // governance_api
    async fn get_staked_pera(&self, owner: PeraAddress) -> StateReadResult<Vec<StakedPera>>;
    fn get_system_state(&self) -> StateReadResult<PeraSystemState>;
...
impl StateRead for AuthorityState {
    ...
    async fn get_staked_pera(&self, owner: PeraAddress) -> StateReadResult<Vec<StakedPera>> {
        Ok(self
            .get_move_objects(owner, MoveObjectType::staked_pera())
            .await?)
    }
    fn get_system_state(&self) -> StateReadResult<PeraSystemState> {
        Ok(self
    ...
    fn get_owned_coins(
        ...
        Ok(self
            ...
            .map(|(coin_type, coin_object_id, coin)| PeraCoin {
                coin_type,
                coin_object_id,
    ...
    async fn get_balance(
        ...
        Ok(self
            .indexes
            .as_ref()
            .ok_or(PeraError::IndexStoreNotAvailable)?
...
pub enum StateReadError {
    // pera_json_rpc::Error will do the final conversion to generic error message
    #[error(transparent)]
...
impl From<PeraError> for StateReadError {
    fn from(e: PeraError) -> Self {
        match e {
            PeraError::IndexStoreNotAvailable
            | PeraError::TransactionNotFound { .. }
            | PeraError::UnsupportedFeatureError { .. }
            | PeraError::UserInputError { .. }
            | PeraError::WrongMessageVersion { .. } => StateReadError::Client(e.into()),
