...
use pera_json_rpc_types::{
    DynamicFieldPage, EventFilter, EventPage, ObjectsPage, Page, PeraObjectDataOptions,
    PeraObjectResponse, PeraObjectResponseQuery, PeraTransactionBlockResponse,
    PeraTransactionBlockResponseQuery, TransactionBlocksPage, TransactionFilter,
};
...
impl<R: ReadApiServer> IndexerApi<R> {
    ...
    fn extract_values_from_dynamic_field_name(
    ...
    ) -> Result<(TypeTag, Vec<u8>), PeraRpcInputError> {
        ...
        let pera_json_value = PeraJsonValue::new(value)?;
        let name_bcs_value = pera_json_value.to_bcs_bytes(&layout)?;
...
impl<R: ReadApiServer> IndexerApiServer for IndexerApi<R> {
    ...
    async fn get_owned_objects(
        ...
        with_tracing!(async move {
            let limit =
                validate_limit(limit, *QUERY_MAX_RESULT_LIMIT).map_err(PeraRpcInputError::from)?;
            ...
            let data = match options.is_not_in_object_info() {
                ...
                false => objects
                    .into_iter()
                    .map(|o_info| PeraObjectResponse::try_from((o_info, options.clone())))
                    .collect::<Result<Vec<PeraObjectResponse>, _>>()?,
            };
    ...
    async fn query_transaction_blocks(
        ...
        with_tracing!(async move {
            ...
            let data: Vec<PeraTransactionBlockResponse> = if opts.only_digest() {
                digests
                    .into_iter()
    ...
    async fn get_dynamic_field_object(
        ...
        with_tracing!(async move {
            ...
            if let Some(id) = id {
                self.read_api
                    .get_object(id, Some(PeraObjectDataOptions::full_content()))
                    .await
                    .map_err(Error::from)
            } else {
                Ok(PeraObjectResponse::new_with_error(
                    PeraObjectResponseError::DynamicFieldNotFound { parent_object_id },
                ))
            }
        })
    }
...
impl<R: ReadApiServer> PeraRpcModule for IndexerApi<R> {
