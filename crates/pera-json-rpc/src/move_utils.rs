...
use pera_json_rpc_types::{
    MoveFunctionArgType, ObjectValueKind, PeraMoveNormalizedFunction, PeraMoveNormalizedModule,
...
impl MoveUtilsInternalTrait for MoveUtilsInternal {
    ...
    async fn get_move_module(
        ...
        Ok(match normalized.get(&module_name) {
            ...
            None => Err(PeraRpcInputError::GenericNotFound(format!(
        ...
        match object_read {
            ObjectRead::Exists(_obj_ref, object, _layout) => {
                match object.into_inner().data {
                    ...
                    _ => Err(PeraRpcInputError::GenericInvalid(format!(
                        "Object is not a package with ID {}",
                        package
                    )))?,
                }
            }
            _ => Err(PeraRpcInputError::GenericNotFound(format!(
...
impl PeraRpcModule for MoveUtils {
    ...
    async fn get_normalized_move_struct(
        ...
        with_tracing!(async move {
            ...
            let identifier = Identifier::new(struct_name.as_str())
                .map_err(|e| PeraRpcInputError::GenericInvalid(format!("{e}")))?;
...
mod tests {
    ...
    mod get_normalized_move_module_tests {
        ...
        async fn test_success_response() {
            ...
            let expected_module: PeraMoveNormalizedModule = normalized_module.clone().into();
        ...
        async fn test_no_module_found() {
            ...
            let expected_error =
                Error::PeraRpcInputError(PeraRpcInputError::GenericNotFound(error_string.clone()));
            mock_internal
                .expect_get_move_module()
