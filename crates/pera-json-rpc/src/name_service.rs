...
const DEFAULT_TLD: &str = "pera";
const ACCEPTED_SEPARATORS: [char; 2] = ['.', '*'];
const PERA_NEW_FORMAT_SEPARATOR: char = '@';

/// Two different view options for a domain.
/// `At` -> `test@example` | `Dot` -> `test.example.pera`
...
pub struct Registry {
    ...
    reverse_registry: Table<PeraAddress, Domain>,
}
...
impl Domain {
    pub fn type_(package_address: PeraAddress) -> StructTag {
        StructTag {
    ...
    /// E.g. `test.example.pera` -> `example.pera`
    ///
    ...
    pub fn format(&self, format: DomainFormat) -> String {
        ...
        format!("{}{}{}", labels.join(sep), PERA_NEW_FORMAT_SEPARATOR, sld)
    }
}
...
pub struct NameServiceConfig {
    pub package_address: PeraAddress,
...
impl NameServiceConfig {
    ...
    pub fn record_field_id(&self, domain: &Domain) -> ObjectID {
        ...
        pera_types::dynamic_field::derive_dynamic_field_id(
            self.registry_id,
...
impl Default for NameServiceConfig {
    fn default() -> Self {
        let package_address = PeraAddress::from_str(NAME_SERVICE_DEFAULT_PACKAGE_ADDRESS).unwrap();
...
/// E.g.  `example.pera` -> `.` | example*pera -> `@` | `example*pera` -> `*`
...
/// Converts @label ending to label{separator}pera ending.
///
/// E.g. `@example` -> `example.pera` | `test@example` -> `test.example.pera`
fn convert_from_new_format(s: &str, separator: &char) -> Result<String, NameServiceError> {
    let mut splits = s.split(PERA_NEW_FORMAT_SEPARATOR);
...
pub struct NameRecord {
    ...
    pub target_address: Option<PeraAddress>,
...
/// A PeransRegistration object to manage an SLD
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct PeransRegistration {
    pub id: UID,
    pub domain: Domain,
