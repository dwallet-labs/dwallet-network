...
pub enum Error {
    ...
    #[error(transparent)]
    PeraObjectResponseError(#[from] PeraObjectResponseError),

    #[error(transparent)]
    PeraRpcInputError(#[from] PeraRpcInputError),
...
impl From<PeraError> for Error {
    fn from(e: PeraError) -> Self {
        match e {
            PeraError::UserInputError { error } => Self::UserInputError(error),
            PeraError::PeraObjectResponseError { error } => Self::PeraObjectResponseError(error),
            PeraError::UnsupportedFeatureError { error } => Self::UnsupportedFeature(error),
            PeraError::IndexStoreNotAvailable => Self::UnsupportedFeature(
                "Required indexes are not available on this node".to_string(),
            ),
            other => Self::PeraError(other),
        }
    }
}
...
impl From<Error> for RpcError {
    ...
    fn from(e: Error) -> RpcError {
        match e {
            ...
            Error::PeraObjectResponseError(err) => match err {
                PeraObjectResponseError::NotExists { .. }
                | PeraObjectResponseError::DynamicFieldNotFound { .. }
                | PeraObjectResponseError::Deleted { .. }
                | PeraObjectResponseError::DisplayError { .. } => {
            ...
            Error::PeraRpcInputError(err) => RpcError::Call(CallError::InvalidParams(err.into())),
            Error::PeraError(pera_error) => match pera_error {
                PeraError::TransactionNotFound { .. }
                | PeraError::TransactionsNotFound { .. }
                | PeraError::TransactionEventsNotFound { .. } => {
                    RpcError::Call(CallError::InvalidParams(pera_error.into()))
                }
                _ => RpcError::Call(CallError::Failed(pera_error.into())),
            },
                ...
                match err {
                    QuorumDriverError::InvalidUserSignature(err) => {
                        let inner_error_str = match err {
                            // TODO(wlmyng): update PeraError display trait to render UserInputError with display
                            PeraError::UserInputError { error } => error.to_string(),
                            _ => err.to_string(),
                        };
                        ...
                        let new_errors: Vec<String> = errors
                            ...
                            .filter_map(|(err, _, _)| {
                                match &err {
                                    ...
                                    PeraError::UserInputError { error } => Some(error.to_string()),
                                    _ => {
...
pub enum PeraRpcInputError {
    ...
    #[error("{0}")]
    CannotParsePeraStructTag(String),
...
impl From<PeraRpcInputError> for RpcError {
    fn from(e: PeraRpcInputError) -> Self {
...
mod tests {
    ...
    use pera_types::crypto::AuthorityPublicKey;
    ...
    mod match_quorum_driver_error_tests {
        ...
        fn test_invalid_user_signature() {
            let quorum_driver_error =
                QuorumDriverError::InvalidUserSignature(PeraError::InvalidSignature {
        ...
        fn test_objects_double_used() {
            use pera_types::crypto::VerifyingKey;
        ...
        fn test_non_recoverable_transaction_error_with_transient_errors() {
            let quorum_driver_error = QuorumDriverError::NonRecoverableTransactionError {
                errors: vec![
                    ...
                    (
                        PeraError::RpcError("Hello".to_string(), "Testing".to_string()),
                        0,
                        vec![],
                    ),
                ],
        ...
        fn test_quorum_driver_internal_error() {
            let quorum_driver_error =
                QuorumDriverError::QuorumDriverInternalError(PeraError::UnexpectedMessage);
        ...
        fn test_system_overload() {
            let quorum_driver_error = QuorumDriverError::SystemOverload {
                overloaded_stake: 10,
                errors: vec![(PeraError::UnexpectedMessage, 0, vec![])],
            };
