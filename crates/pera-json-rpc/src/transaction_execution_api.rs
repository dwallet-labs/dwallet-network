...
use crate::error::{Error, PeraRpcInputError};
use crate::{
...
use pera_json_rpc_api::{JsonRpcMetrics, WriteApiOpenRpc, WriteApiServer};
use pera_json_rpc_types::{
    DevInspectArgs, DevInspectResults, DryRunTransactionBlockResponse, PeraTransactionBlock,
...
impl TransactionExecutionApi {
    ...
    > {
        ...
        let transaction = if opts.show_input {
            ...
            Some(PeraTransactionBlock::try_from(
                txn.data().clone(),
    ...
    ) -> Result<PeraTransactionBlockResponse, Error> {
        let request_type =
    ...
    async fn handle_post_orchestration(
        ...
        transaction: Option<PeraTransactionBlock>,
        ...
        let events = if opts.show_events {
            ...
            Some(PeraTransactionBlockEvents::try_from(
    ...
    pub fn prepare_dry_run_transaction_block(
        &self,
        tx_bytes: Base64,
    ) -> Result<(TransactionData, TransactionDigest, Vec<InputObjectKind>), PeraRpcInputError> {
        let tx_data: TransactionData = self.convert_bytes(tx_bytes)?;
