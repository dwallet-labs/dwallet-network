...
use pera_types::display::DisplayVersionUpdatedEvent;
use pera_types::effects::{TransactionEffects, TransactionEffectsAPI, TransactionEvents};
use pera_types::error::{PeraError, PeraObjectResponseError};
...
struct IntermediateTransactionResponse {
    ...
    events: Option<PeraTransactionBlockEvents>,
...
impl ReadApi {
    ...
    async fn multi_get_transaction_blocks_internal(
        ...
        opts: Option<PeraTransactionBlockResponseOptions>,
    ) -> Result<Vec<PeraTransactionBlockResponse>, Error> {
        trace!("start");
        ...
        if temp_response.len() < num_digests {
            Err(PeraRpcInputError::ContainsDuplicates)?
        }
            ...
            for (_, cache_entry) in temp_response.iter_mut() {
                ...
                if event_digest.is_some() {
                    ...
                    let events= event_digest_to_events
                        ...
                        .map(|events| to_pera_transaction_events(self, cache_entry.digest, events));
                    match events {
                    ...
                    cache_entry.events = Some(PeraTransactionBlockEvents::default());
                }
            }
        }
...
impl ReadApiServer for ReadApi {
    ...
    async fn multi_get_objects(
        ...
        with_tracing!(async move {
            if object_ids.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                let objects_result: Result<Vec<PeraObjectResponse>, String> = results
                    .into_iter()
        ...
        with_tracing!(async move {
            ...
            match past_read {
                PastObjectRead::ObjectNotExists(id) => {
                    Ok(PeraPastObjectResponse::ObjectNotExists(id))
                }
                PastObjectRead::VersionFound(object_ref, o, layout) => {
                    ...
                    Ok(PeraPastObjectResponse::VersionFound(
                        (object_ref, o, layout, options, display_fields).try_into()?,
                    ))
                }
                PastObjectRead::ObjectDeleted(oref) => {
                    Ok(PeraPastObjectResponse::ObjectDeleted(oref.into()))
                }
                PastObjectRead::VersionNotFound(id, seq_num) => {
                    Ok(PeraPastObjectResponse::VersionNotFound(id, seq_num))
                }
                PastObjectRead::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                } => Ok(PeraPastObjectResponse::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                }),
    ...
    async fn try_get_object_before_version(
        ...
        self.try_get_past_object(
            object_id,
            version,
            Some(PeraObjectDataOptions::bcs_lossless()),
        )
        .await
    }
            ...
            if past_objects.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                if !errors.is_empty() {
                    let error_string = errors
                        ...
                        .collect::<Vec<String>>()
                        .join("; ");
                    Err(anyhow!("{error_string}").into()) // Collects errors not related to PeraPastObjectResponse variants
                } else {
                    Ok(success)
                }
            } else {
            ...
            if opts.show_events && temp_response.effects.is_some() {
                ...
                if let Some(event_digest) = temp_response.effects.as_ref().unwrap().events_digest()
                {
                    ...
                    let events = spawn_monitored_task!(async move {
                        ...
                        .await
                        .map_err(Error::from)??;
                    match to_pera_transaction_events(self, digest, events) {
                    ...
                    // events field will be Some if and only if `show_events` is true and
                    // there is no error in converting fetching events
                    temp_response.events = Some(PeraTransactionBlockEvents::default());
                }
            }
...
impl PeraRpcModule for ReadApi {
...
fn to_pera_transaction_events(
    fullnode_api: &ReadApi,
    tx_digest: TransactionDigest,
    events: TransactionEvents,
) -> Result<PeraTransactionBlockEvents, Error> {
    ...
    Ok(PeraTransactionBlockEvents::try_from(
        events,
        tx_digest,
        None,
...
pub enum ObjectDisplayError {
    ...
    #[error(transparent)]
    Deserialization(#[from] PeraError),
...
pub fn get_rendered_fields(
    ...
    let pera_move_value: PeraMoveValue = MoveValue::Struct(move_struct.clone()).into();
    if let PeraMoveValue::Struct(move_struct) = pera_move_value {
        let fields =
            fields
                .contents
                .iter()
...
fn parse_template(template: &str, move_struct: &PeraMoveStruct) -> Result<String, Error> {
...
fn get_value_from_move_struct(
    ...
    let mut current_value = &PeraMoveValue::Struct(move_struct.clone());
    // iterate over the parts and try to access the corresponding field
    for part in parts {
        match current_value {
            PeraMoveValue::Struct(move_struct) => {
                if let PeraMoveStruct::WithTypes { type_: _, fields }
                | PeraMoveStruct::WithFields(fields) = move_struct
                {
    ...
    match current_value {
        PeraMoveValue::Option(move_option) => match move_option.as_ref() {
            Some(move_value) => Ok(move_value.to_string()),
            None => Ok("".to_string()),
        },
        PeraMoveValue::Vector(_) => Err(anyhow!(
...
fn convert_to_response(
    ...
    opts: &PeraTransactionBlockResponseOptions,
    module_cache: &impl GetModule,
) -> RpcInterimResult<PeraTransactionBlockResponse> {
    let mut response = PeraTransactionBlockResponse::new(cache.digest);
    ...
    if opts.show_input && cache.transaction.is_some() {
        let tx_block =
            PeraTransactionBlock::try_from(cache.transaction.unwrap().into_data(), module_cache)?;
