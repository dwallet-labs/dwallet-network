...
use ika_keys::keypair_file::{read_authority_keypair_from_file, read_keypair_from_file};
...
use ika_types::crypto::{get_key_pair_from_rng, AccountKeyPair, AuthorityKeyPair};
...
/// Default gas price of 100 NIka
pub const DEFAULT_VALIDATOR_GAS_PRICE: u64 = ika_types::transaction::DEFAULT_VALIDATOR_GAS_PRICE;
...
pub struct NodeConfig {
    ...
    pub rest: Option<ika_rest_api::Config>,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name_service_package_address: Option<IkaAddress>,
...
fn default_base_url() -> String {
    "https://transactions.ika.io/".to_string()
}
...
impl NodeConfig {
    ...
    pub fn ika_address(&self) -> IkaAddress {
...
pub struct ExpensiveSafetyCheckConfig {
    ...
    /// rebate of all live objects, and that the total IKA in the network remains
    /// the same.
    #[serde(default)]
    enable_epoch_ika_conservation_check: bool,

    /// If enabled, we will check that the total IKA in all input objects of a tx
    /// (both the Move part and the storage rebate) matches the total IKA in all
    /// output objects of the tx + gas fees
    #[serde(default)]
    enable_deep_per_tx_ika_conservation_check: bool,

    /// Disable epoch IKA conservation check even when we are running in debug mode.
    #[serde(default)]
    force_disable_epoch_ika_conservation_check: bool,
...
impl ExpensiveSafetyCheckConfig {
    pub fn new_enable_all() -> Self {
        Self {
            enable_epoch_ika_conservation_check: true,
            enable_deep_per_tx_ika_conservation_check: true,
            force_disable_epoch_ika_conservation_check: false,
    ...
    pub fn new_disable_all() -> Self {
        Self {
            enable_epoch_ika_conservation_check: false,
            enable_deep_per_tx_ika_conservation_check: false,
            force_disable_epoch_ika_conservation_check: true,
    ...
    pub fn force_disable_epoch_ika_conservation_check(&mut self) {
        self.force_disable_epoch_ika_conservation_check = true;
    }

    pub fn enable_epoch_ika_conservation_check(&self) -> bool {
        (self.enable_epoch_ika_conservation_check || cfg!(debug_assertions))
            && !self.force_disable_epoch_ika_conservation_check
    }
    ...
    pub fn enable_deep_per_tx_ika_conservation_check(&self) -> bool {
        self.enable_deep_per_tx_ika_conservation_check || cfg!(debug_assertions)
    }
...
/// Wrapper struct for IkaKeyPair that can be deserialized from a file path. Used by network, worker, and account keypair.
...
pub struct KeyPairWithPath {
    ...
    #[serde(skip)]
    keypair: OnceCell<Arc<IkaKeyPair>>,
}
...
impl KeyPairWithPath {
    pub fn new(kp: IkaKeyPair) -> Self {
        let cell: OnceCell<Arc<IkaKeyPair>> = OnceCell::new();
    ...
    pub fn keypair(&self) -> &IkaKeyPair {
        self.keypair
...
mod tests {
    ...
    use ika_keys::keypair_file::{write_authority_keypair_to_file, write_keypair_to_file};
    use ika_types::crypto::{get_key_pair_from_rng, AuthorityKeyPair, NetworkKeyPair, IkaKeyPair};
    ...
    fn load_key_pairs_to_node_config() {
        ...
        write_keypair_to_file(
            &IkaKeyPair::Ed25519(worker_key_pair.copy()),
            PathBuf::from("worker.key"),
        )
        .unwrap();
        write_keypair_to_file(
            &IkaKeyPair::Ed25519(network_key_pair.copy()),
