...
use ika_types::authenticator_state::{get_authenticator_state, AuthenticatorStateInner};
use ika_types::base_types::{ObjectID, IkaAddress};
use ika_types::clock::Clock;
use ika_types::committee::CommitteeWithNetworkMetadata;
use ika_types::crypto::DefaultHash;
use ika_types::deny_list_v1::{get_coin_deny_list, PerTypeDenyList};
...
use ika_types::ika_system_state::{
    get_ika_system_state, get_ika_system_state_wrapper, IkaSystemState, IkaSystemStateTrait,
    IkaSystemStateWrapper, IkaValidatorGenesis,
};
...
use ika_types::{IKA_BRIDGE_OBJECT_ID, IKA_RANDOMNESS_STATE_OBJECT_ID};
use tracing::trace;
...
impl Genesis {
    ...
    pub fn validator_set_for_tooling(&self) -> Vec<IkaValidatorGenesis> {
        self.ika_system_object()
    ...
    pub fn committee_with_network(&self) -> CommitteeWithNetworkMetadata {
        self.ika_system_object().get_current_epoch_committee()
    }

    pub fn reference_gas_price(&self) -> u64 {
        self.ika_system_object().reference_gas_price()
    }

    // TODO: No need to return IkaResult. Also consider return &.
    pub fn committee(&self) -> IkaResult<Committee> {
        Ok(self.committee_with_network().committee().clone())
    }

    pub fn ika_system_wrapper_object(&self) -> IkaSystemStateWrapper {
        get_ika_system_state_wrapper(&self.objects())
            .expect("Ika System State Wrapper object must always exist")
    }

    pub fn ika_system_object(&self) -> IkaSystemState {
        get_ika_system_state(&self.objects()).expect("Ika System State object must always exist")
    }
    ...
    pub fn clock(&self) -> Clock {
        let clock = self
            .objects()
            .iter()
            .find(|o| o.id() == ika_types::IKA_CLOCK_OBJECT_ID)
...
impl UnsignedGenesis {
    ...
    pub fn has_randomness_state_object(&self) -> bool {
        self.objects()
            .get_object(&IKA_RANDOMNESS_STATE_OBJECT_ID)
    ...
    pub fn has_bridge_object(&self) -> bool {
        self.objects()
            .get_object(&IKA_BRIDGE_OBJECT_ID)
            .expect("read from genesis cannot fail")
            .is_some()
    }
...
impl GenesisCeremonyParameters {
    ...
    fn default_initial_stake_subsidy_distribution_amount() -> u64 {
        // 1M Ika
        1_000_000 * ika_types::gas_coin::NIKA_PER_IKA
    }
    ...
    pub fn to_genesis_chain_parameters(&self) -> GenesisChainParameters {
        GenesisChainParameters {
            ...
            max_validator_count: ika_types::governance::MAX_VALIDATOR_COUNT,
            min_validator_joining_stake: ika_types::governance::MIN_VALIDATOR_JOINING_STAKE_NIKA,
...
pub struct TokenDistributionSchedule {
    pub stake_subsidy_fund_nika: u64,
    pub allocations: Vec<TokenAllocation>,
}

impl TokenDistributionSchedule {
    pub fn validate(&self) {
        let mut total_nika = self.stake_subsidy_fund_nika;

        for allocation in &self.allocations {
            total_nika += allocation.amount_nika;
        }

        if total_nika != TOTAL_SUPPLY_NIKA {
            panic!("TokenDistributionSchedule adds up to {total_nika} and not expected {TOTAL_SUPPLY_NIKA}");
        }
    }

    pub fn check_all_stake_operations_are_for_valid_validators<
        I: IntoIterator<Item = IkaAddress>,
    >(
        &self,
        validators: I,
    ) {
        ...
        let mut validators: HashMap<IkaAddress, u64> =
        ...
        for allocation in &self.allocations {
            if let Some(staked_with_validator) = &allocation.staked_with_validator {
                *validators
                    ...
                    allocation.amount_nika;
            }
        }

        // Check that all validators have sufficient stake allocated to ensure they meet the
        // minimum stake threshold
        let minimum_required_stake = ika_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_NIKA;
    ...
    pub fn new_for_validators_with_default_allocation<I: IntoIterator<Item = IkaAddress>>(
        validators: I,
    ) -> Self {
        let mut supply = TOTAL_SUPPLY_NIKA;
        let default_allocation = ika_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_NIKA;
        ...
        let allocations = validators
            ...
            .map(|a| {
                ...
                TokenAllocation {
                    recipient_address: a,
                    amount_nika: default_allocation,
        ...
        let schedule = Self {
            stake_subsidy_fund_nika: supply,
            allocations,
        };
    ...
    ///
    /// All entries in a token distribution schedule must add up to 10B Ika.
    pub fn from_csv<R: std::io::Read>(reader: R) -> Result<Self> {
        ...
        assert_eq!(
            TOTAL_SUPPLY_NIKA,
            allocations.iter().map(|a| a.amount_nika).sum::<u64>(),
            "Token Distribution Schedule must add up to 10B Ika",
        );
        ...
        let schedule = Self {
            stake_subsidy_fund_nika: stake_subsidy_fund_allocation.amount_nika,
            allocations,
        };
    ...
    pub fn to_csv<W: std::io::Write>(&self, writer: W) -> Result<()> {
        ...
        writer.serialize(TokenAllocation {
            recipient_address: IkaAddress::default(),
            amount_nika: self.stake_subsidy_fund_nika,
...
pub struct TokenAllocation {
    pub recipient_address: IkaAddress,
    pub amount_nika: u64,

    /// Indicates if this allocation should be staked at genesis and with which validator
    pub staked_with_validator: Option<IkaAddress>,
}
...
impl TokenDistributionScheduleBuilder {
    ...
    pub fn new() -> Self {
        Self {
            pool: TOTAL_SUPPLY_NIKA,
            allocations: vec![],
        }
    }

    pub fn default_allocation_for_validators<I: IntoIterator<Item = IkaAddress>>(
        &mut self,
        validators: I,
    ) {
        let default_allocation = ika_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_NIKA;
    ...
    pub fn add_allocation(&mut self, allocation: TokenAllocation) {
        self.pool = self.pool.checked_sub(allocation.amount_nika).unwrap();
        self.allocations.push(allocation);
    }

    pub fn build(&self) -> TokenDistributionSchedule {
        let schedule = TokenDistributionSchedule {
            stake_subsidy_fund_nika: self.pool,
