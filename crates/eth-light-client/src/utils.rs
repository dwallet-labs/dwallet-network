...
mod tests {
    ...
    #[test]
    fn compute_fork_data_root_valid() {
        let current_version: Vector<u8, 4> = Vector::<u8, 4>::try_from(vec![1, 2, 3, 4]).expect("4 byte vector");
        let genesis_validator_root = Bytes32::default();

        let mut fork_data = ForkData {
            current_version: current_version.clone(),
            genesis_validator_root: genesis_validator_root.clone(),
        };

        let expected = fork_data.hash_tree_root();

        assert_eq!(compute_fork_data_root(current_version,genesis_validator_root).unwrap(), expected.unwrap() );
    }
}