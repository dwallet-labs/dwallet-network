...
use pera_sdk2::types::CheckpointSequenceNumber;
use pera_sdk2::types::EpochId;
use pera_sdk2::types::Object;
use pera_sdk2::types::ObjectId;
use pera_sdk2::types::SignedCheckpointSummary;
use pera_sdk2::types::SignedTransaction;
use pera_sdk2::types::StructTag;
...
use crate::system::X_PERA_MAX_SUPPORTED_PROTOCOL_VERSION;
use crate::system::X_PERA_MIN_SUPPORTED_PROTOCOL_VERSION;
...
use crate::types::X_PERA_CHAIN;
use crate::types::X_PERA_CHAIN_ID;
use crate::types::X_PERA_CHECKPOINT_HEIGHT;
use crate::types::X_PERA_CURSOR;
use crate::types::X_PERA_EPOCH;
use crate::types::X_PERA_LOWEST_AVAILABLE_CHECKPOINT;
use crate::types::X_PERA_LOWEST_AVAILABLE_CHECKPOINT_OBJECTS;
use crate::types::X_PERA_TIMESTAMP_MS;
...
impl ResponseParts {
    fn from_reqwest_response(response: &reqwest::Response) -> Self {
        ...
        let checkpoint_height = headers
            .get(X_PERA_CHECKPOINT_HEIGHT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let timestamp_ms = headers
            .get(X_PERA_TIMESTAMP_MS)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let lowest_available_checkpoint = headers
            .get(X_PERA_LOWEST_AVAILABLE_CHECKPOINT)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let lowest_available_checkpoint_objects = headers
            .get(X_PERA_LOWEST_AVAILABLE_CHECKPOINT_OBJECTS)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let cursor = headers
            .get(X_PERA_CURSOR)
        ...
        let min_supported_protocol_version = headers
            .get(X_PERA_MIN_SUPPORTED_PROTOCOL_VERSION)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
        let max_supported_protocol_version = headers
            .get(X_PERA_MAX_SUPPORTED_PROTOCOL_VERSION)
            .and_then(|h| h.to_str().ok())
            .and_then(|s| s.parse().ok());
