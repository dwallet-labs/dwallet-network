...
async fn get_coin_info(
    ...
    let pera_types::storage::CoinInfo {
    ...
    let metadata = if let Some(coin_metadata_object_id) = coin_metadata_object_id {
        state
            ...
            .map(pera_types::coin::CoinMetadata::try_from)
            .transpose()
            .map_err(|_| {
                RestError::new(
    ...
    let treasury = if let Some(treasury_object_id) = treasury_object_id {
        state
            ...
            .map(pera_types::coin::TreasuryCap::try_from)
            .transpose()
            .map_err(|_| {
                RestError::new(
                    axum::http::StatusCode::INTERNAL_SERVER_ERROR,
    ...
    } else if pera_types::gas_coin::GAS::is_gas(&core_coin_type) {
        Some(CoinTreasury::PERA)
    } else {
        None
    };
...
impl From<pera_types::coin::CoinMetadata> for CoinMetadata {
    fn from(value: pera_types::coin::CoinMetadata) -> Self {
        Self {
...
impl CoinTreasury {
    const PERA: Self = Self {
        id: None,
        total_supply: pera_types::gas_coin::TOTAL_SUPPLY_NPERA,
    };
}
