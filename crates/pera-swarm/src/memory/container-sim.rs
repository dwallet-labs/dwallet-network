...
pub(crate) struct Container {
    ...
    node_watch: watch::Receiver<Weak<PeraNode>>,
}
...
impl Drop for Container {
    fn drop(&mut self) {
        if let Some(handle) = self.handle.take() {
            ...
            pera_simulator::runtime::Handle::try_current().map(|h| h.delete_node(handle.node_id));
        }
    }
}
...
impl Container {
    ...
    pub async fn spawn(config: NodeConfig, _runtime: RuntimeType) -> Self {
        ...
        let handle = pera_simulator::runtime::Handle::current();
        ...
        let node = builder
            ...
            .init(move || {
                ...
                async move {
                    ...
                    let server = PeraNode::start(config, registry_service, None)
                        .await
                        .unwrap();
    ...
    /// Get a PeraNodeHandle to the node owned by the container.
    pub fn get_node_handle(&self) -> Option<PeraNodeHandle> {
        Some(PeraNodeHandle::new(self.node_watch.borrow().upgrade()?))
    }
