...
use pera_types::{
    ...
    BRIDGE_ADDRESS, DEEPBOOK_ADDRESS, MOVE_STDLIB_ADDRESS, PERA_FRAMEWORK_ADDRESS,
    PERA_SYSTEM_ADDRESS,
};
...
/// Wrapper around the core Move `CompiledPackage` with some Pera-specific traits and info
#[derive(Debug, Clone)]
...
/// Wrapper around the core Move `BuildConfig` with some Pera-specific info
#[derive(Clone)]
...
impl BuildConfig {
    ...
    /// If we are building the Pera framework, we skip the check that the addresses should be 0
    pub fn build(self, path: &Path) -> PeraResult<CompiledPackage> {
    ...
    pub fn resolution_graph(
    ...
    ) -> PeraResult<ResolvedGraph> {
        if let Some(err_msg) = set_pera_flavor(&mut self.config) {
            return Err(PeraError::ModuleBuildFailure { error: err_msg });
        }
        ...
        .map_err(|err| PeraError::ModuleBuildFailure {
            error: format!("{:?}", err),
        })
    }
}
...
pub fn decorate_warnings(warning_diags: Diagnostics, files: Option<&MappedFiles>) {
    ...
    if any_linter_warnings {
        eprintln!("Please report feedback on the linter warnings at https://forums.pera.io\n");
    }
...
/// Sets build config's default flavor to `Flavor::Pera`. Returns error message if the flavor was
/// previously set to something else than `Flavor::Pera`.
pub fn set_pera_flavor(build_config: &mut MoveBuildConfig) -> Option<String> {
    use move_compiler::editions::Flavor;

    let flavor = build_config.default_flavor.get_or_insert(Flavor::Pera);
    if flavor != &Flavor::Pera {
        return Some(format!(
...
pub fn build_from_resolution_graph(
    ...
    let (package, fn_info) = match result {
        Err(error) => {
            return Err(PeraError::ModuleBuildFailure {
                error: format!("{:?}", error),
            })
        }
    ...
    if run_bytecode_verifier {
        ...
        for m in compiled_modules.iter_modules() {
            move_bytecode_verifier::verify_module_unmetered(m).map_err(|err| {
                PeraError::ModuleVerificationFailure {
                    error: err.to_string(),
                }
            })?;
            pera_bytecode_verifier::pera_verify_module_unmetered(m, &fn_info, &verifier_config)?;
        }
...
impl CompiledPackage {
    ...
    /// Note: these are not topologically sorted by dependency--use `get_dependency_sorted_modules` to produce a list of modules peratable
    ...
    pub fn get_dependency_sorted_modules(
        ...
        if with_unpublished_deps {
            ...
            // sufficient as we may have modules with the same names in user code and in Pera
    ...
    /// Get bytecode modules from the Pera System that are used by this package
    pub fn get_pera_system_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == PERA_SYSTEM_ADDRESS)
    }

    /// Get bytecode modules from the Pera Framework that are used by this package
    pub fn get_pera_framework_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == PERA_FRAMEWORK_ADDRESS)
    }
    ...
    pub fn verify_unpublished_dependencies(
        ...
        Err(PeraError::ModulePublishFailure {
...
impl Default for BuildConfig {
    fn default() -> Self {
        let config = MoveBuildConfig {
            default_flavor: Some(move_compiler::editions::Flavor::Pera),
...
pub struct PeraPackageHooks;

impl PackageHooks for PeraPackageHooks {
...
pub fn check_unpublished_dependencies(unpublished: &BTreeSet<Symbol>) -> Result<(), PeraError> {
...
pub fn check_invalid_dependencies(invalid: &BTreeMap<Symbol, String>) -> Result<(), PeraError> {
    if invalid.is_empty() {
        return Ok(());
    }
