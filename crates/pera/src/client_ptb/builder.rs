...
impl<'a> PTBBuilder<'a> {
    ...
    fn declare_possible_address_binding(&mut self, ident: String, possible_addr: &Spanned<PTBArg>) {
        match possible_addr.value {
            ...
            // If we encounter a dotted string e.g., "foo.0" or "pera.io" or something like that
    ...
    /// Fetch the `PeraObjectData` for an object ID -- this is used for object resolution.
    async fn get_object(&self, object_id: ObjectID, obj_loc: Span) -> PTBResult<PeraObjectData> {
        let res = self
            .reader
            .get_object_with_options(
                object_id,
                PeraObjectDataOptions::new().with_type().with_owner(),
            )
            .await
                ...
                let upgrade_ticket =
                    self.ptb
                        .command(Tx::Command::MoveCall(Box::new(Tx::ProgrammableMoveCall {
                            package: PERA_FRAMEWORK_PACKAGE_ID,
