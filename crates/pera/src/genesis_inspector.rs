...
use pera_types::{
    base_types::ObjectID,
    coin::CoinMetadata,
    gas_coin::{GasCoin, NPERA_PER_PERA, TOTAL_SUPPLY_NPERA},
    governance::StakedPera,
...
const STR_PERA: &str = "Pera";
const STR_STAKED_PERA: &str = "StakedPera";
const STR_PACKAGE: &str = "Package";
const STR_COIN_METADATA: &str = "CoinMetadata";
const STR_OTHER: &str = "Other";
const STR_PERA_DISTRIBUTION: &str = "Pera Distribution";
...
pub(crate) fn examine_genesis_checkpoint(genesis: UnsignedGenesis) {
    ...
    // Prepare Pera distribution info
    let mut pera_distribution = BTreeMap::new();
    let entry = pera_distribution
        .entry("Pera System".to_string())
    ...
    entry.insert(
        ...
        (STR_PERA, system_object.stake_subsidy.balance.value()),
    );

    // Prepare Object Info
    let mut owner_map = BTreeMap::new();
    let mut package_map = BTreeMap::new();
    let mut pera_map = BTreeMap::new();
    let mut staked_pera_map = BTreeMap::new();
    ...
    for object in genesis.objects() {
        ...
        match &object.data {
            pera_types::object::Data::Move(move_object) => {
                if let Ok(gas) = GasCoin::try_from(object) {
                    let entry = pera_distribution
                        .entry(object.owner.to_string())
                        .or_default();
                    entry.insert(object_id_str.clone(), (STR_PERA, gas.value()));
                    pera_map.insert(object.id(), gas);
                } else if let Ok(coin_metadata) = CoinMetadata::try_from(object) {
                    coin_metadata_map.insert(object.id(), coin_metadata);
                } else if let Ok(staked_pera) = StakedPera::try_from(object) {
                    let entry = pera_distribution
                        .entry(object.owner.to_string())
                        .or_default();
                    entry.insert(object_id_str, (STR_STAKED_PERA, staked_pera.principal()));
                    // Assert pool id is associated with a knonw validator.
                    let validator = validator_pool_id_map.get(&staked_pera.pool_id()).unwrap();
                    assert_eq!(validator.staking_pool.id, staked_pera.pool_id());

                    staked_pera_map.insert(object.id(), staked_pera);
                } else {
                    other_object_map.insert(object.id(), move_object);
                }
            }
            pera_types::object::Data::Package(p) => {
    ...
    loop {
        ...
        .prompt();
        match ans {
            Ok(name) if name == STR_PERA_DISTRIBUTION => {
                examine_total_supply(&pera_distribution, true)
            }
...
fn examine_validators(
    ...
    validator_map: &BTreeMap<&str, &PeraValidatorGenesis>,
) {
    loop {
...
fn examine_object(
    ...
    validator_pool_id_map: &BTreeMap<ObjectID, &PeraValidatorGenesis>,
    package_map: &BTreeMap<ObjectID, &MovePackage>,
    pera_map: &BTreeMap<ObjectID, GasCoin>,
    staked_pera_map: &BTreeMap<ObjectID, StakedPera>,
    ...
    loop {
        ...
        match ans {
            Ok(name) if name == STR_EXIT => break,
            Ok(name) if name == STR_PERA => {
                for gas_coin in pera_map.values() {
                    display_pera(gas_coin, owner_map);
                }
                print_divider("Pera");
            }
            Ok(name) if name == STR_STAKED_PERA => {
                for staked_pera_coin in staked_pera_map.values() {
                    display_staked_pera(staked_pera_coin, validator_pool_id_map, owner_map);
                }
                print_divider(STR_STAKED_PERA);
            }
...
fn examine_total_supply(
    pera_distribution: &BTreeMap<String, BTreeMap<String, (&str, u64)>>,
    print: bool,
) {
    let mut total_pera = 0;
    let mut total_staked_pera = 0;
    for (owner, coins) in pera_distribution {
        let mut amount_sum = 0;
        for (owner, value) in coins.values() {
            amount_sum += value;
            if *owner == STR_STAKED_PERA {
                total_staked_pera += value;
            }
        }
        total_pera += amount_sum;
        if print {
            println!("Owner {:?}", owner);
            println!(
                "Total Amount of Pera/StakedPera Owned: {amount_sum} NPERA or {} PERA:",
                amount_sum / NPERA_PER_PERA
            );
            println!("{:#?}\n", coins);
        }
    }
    assert_eq!(total_pera, TOTAL_SUPPLY_NPERA);
    // Always print this.
    println!(
        "Total Supply of Pera: {total_pera} NPERA or {} PERA",
        total_pera / NPERA_PER_PERA
    );
    println!(
        "Total Amount of StakedPera: {total_staked_pera} NPERA or {} PERA\n",
        total_staked_pera / NPERA_PER_PERA
    );
    if print {
        print_divider("Pera Distribution");
    }
}

fn display_validator(validator: &PeraValidatorGenesis) {
    ...
    println!(
        "Staking Pool Pera Balance: {:?}",
        validator.staking_pool.pera_balance
    );
    println!(
        "Rewards Pool: {}",
    ...
    println!(
        "Pending Total Pera Withdraw: {}",
        validator.staking_pool.pending_total_pera_withdraw
    );
    println!(
...
fn display_pera(gas_coin: &GasCoin, owner_map: &BTreeMap<ObjectID, Owner>) {
    println!("ID: {}", gas_coin.id());
    println!("Balance: {}", gas_coin.value());
    println!("Owner: {}\n", owner_map.get(gas_coin.id()).unwrap());
}

fn display_staked_pera(
    ...
    println!("Owner: {}\n", owner_map.get(&staked_pera.id()).unwrap());
}
