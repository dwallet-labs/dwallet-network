...
pub enum KeyToolCommand {
    ...
    /// encoded 33 byte flag || private key starting with "peraprivkey").
    ...
    /// Add a new key to Pera CLI Keystore using either the input mnemonic phrase or a Bech32 encoded 33-byte
    /// `flag || privkey` starting with "peraprivkey", the key scheme flag {ed25519 | secp256k1 | secp256r1}
    ...
    /// Output the private key of the given key identity in Pera CLI Keystore as Bech32
    /// encoded string starting with `peraprivkey`.
    Export {
        #[clap(long)]
        key_identity: KeyIdentity,
    },
    /// List all keys by its Pera address, Base64 encoded public key, key scheme name in
    /// pera.keystore.
    List {
        /// Sort by alias
        #[clap(long, short = 's')]
    ...
    /// result can be used as signature field for `pera client execute-signed-tx`. The sum
    ...
    /// Create signature using the private key for the given address (or its alias) in pera keystore.
    ...
    ZkLoginEnterToken {
        ...
        kp_bigint: String,
        #[clap(long)]
        ephemeral_key_identifier: PeraAddress,
        #[clap(long, default_value = "devnet")]
        network: String,
        #[clap(long, default_value = "false")]
        test_multisig: bool,
    ...
    /// Example request: pera keytool zk-login-sig-verify --sig $SERIALIZED_ZKLOGIN_SIG --bytes $BYTES --intent-scope 0 --network devnet --curr-epoch 10
    ZkLoginSigVerify {
    ...
    /// e.g. pera keytool zk-login-insecure-sign-personal-message --data "hello" --max-epoch 5
...
pub struct ConvertOutput {
    bech32_with_flag: String, // latest Pera Keystore and Pera Wallet import/export format
    base64_with_flag: String, // Pera Keystore storage format
    hex_without_flag: String, // Legacy Pera Wallet format
    scheme: String,
}
...
pub struct SignData {
    ...
    // serialized Pera signature to be send for executing the transaction.
    pera_signature: String,
}
    ...
    pub async fn execute(self, keystore: &mut Keystore) -> Result<CommandOutput, anyhow::Error> {
        let cmd_result = Ok(match self {
            ...
            KeyToolCommand::DecodeMultiSig {
                ...
                let address = PeraAddress::from(multisig.get_pk());
            ...
            KeyToolCommand::Generate {
                ...
                SignatureScheme::BLS12381 => {
                    let (pera_address, kp) = get_authority_key_pair();
                    let file_name = format!("bls-{pera_address}.key");
            ...
            KeyToolCommand::Import {
                ...
                if Hex::decode(&input_string).is_ok() {
                    return Err(anyhow!(
                        "Pera Keystore and Pera Wallet no longer support importing 
                    private key as Hex, if you are sure your private key is encoded in Hex, use 
                    `pera keytool convert $HEX` to convert first then import the Bech32 encoded 
                    private key starting with `peraprivkey`."
                    ));
                }

                match PeraKeyPair::decode(&input_string) {
                    Ok(skp) => {
                    ...
                    Err(_) => {
                        ...
                        let pera_address = keystore.import_from_mnemonic(
                            &input_string,
                            key_scheme,
                            derivation_path,
                            alias,
                        )?;
                        let skp = keystore.get_key(&pera_address)?;
                        let key = Key::from(skp);
                        CommandOutput::Import(key)
                    }
                }
            ...
            KeyToolCommand::List { sort_by_alias } => {
                let mut keys = keystore
                    ...
                    .map(|pk| {
                        ...
                        key.alias = keystore.get_alias_by_address(&key.pera_address).ok();
                        key
                    })
                    .collect::<Vec<Key>>();
                if sort_by_alias {
                ...
                let output = match read_keypair_from_file(&file) {
                    Ok(keypair) => {
                        ...
                        let network_worker_keypair = match &keypair {
                            PeraKeyPair::Ed25519(kp) => kp.encode_base64(),
                            PeraKeyPair::Secp256k1(kp) => kp.encode_base64(),
                            PeraKeyPair::Secp256r1(kp) => kp.encode_base64(),
                        };
                        KeypairData {
            ...
            KeyToolCommand::MultiSigAddress {
                ...
                let address: PeraAddress = (&multisig_pk).into();
                ...
                for (pk, w) in pks.into_iter().zip(weights.into_iter()) {
                    output.multisig.push(MultiSigOutput {
                        address: Into::<PeraAddress>::into(&pk),
                        public_base64_key: pk.encode_base64(),
                        weight: w,
                    });
                }
                    ...
                    Err(_) => match read_authority_keypair_from_file(&file) {
                        Ok(keypair) => {
                            ...
                            CommandOutput::Show(Key {
                                ...
                                pera_address: (keypair.public()).into(),
                                public_base64_key,
                                key_scheme: SignatureScheme::BLS12381.to_string(),
                                flag: SignatureScheme::BLS12381.flag(),
                                peer_id: None,
            ...
            KeyToolCommand::Sign {
                ...
                let intent = intent.unwrap_or_else(Intent::pera_transaction);
                ...
                CommandOutput::Sign(SignData {
                    ...
                    pera_signature: pera_signature.encode_base64(),
                })
            }
            ...
            KeyToolCommand::SignKMS {
                ...
                let pk_owner = PublicKey::decode_base64(&base64pk)
                    .map_err(|e| anyhow!("Invalid base64 key: {:?}", e))?;
                let address_owner = PeraAddress::from(&pk_owner);
            ...
            KeyToolCommand::Unpack { keypair } => {
                let keypair = PeraKeyPair::decode_base64(&keypair)
                    .map_err(|_| anyhow!("Invalid Base64 encode keypair"))?;

                let key = Key::from(&keypair);
                let path_str = format!("{}.key", key.pera_address).to_lowercase();
            ...
            KeyToolCommand::ZkLoginSignAndExecuteTx {
                ...
                let skp = if fixed {
                    ...
                    PeraKeyPair::Ed25519(Ed25519KeyPair::generate(&mut rand::thread_rng()))
                };
                println!("Ephemeral keypair: {:?}", skp.encode());
                let pk = skp.public();
                let ephemeral_key_identifier: PeraAddress = (&skp.public()).into();
                ...
                println!("Finish login and paste the entire URL here (e.g. https://pera.io/#id_token=...):");
...
impl From<&PeraKeyPair> for Key {
    fn from(skp: &PeraKeyPair) -> Self {
        Key::from(skp.public())
    }
}
...
impl From<PublicKey> for Key {
    fn from(pk: PublicKey) -> Self {
        Key {
            ...
            pera_address: PeraAddress::from(&pk),
            public_base64_key: pk.encode_base64(),
...
fn convert_private_key_to_bech32(value: String) -> Result<ConvertOutput, anyhow::Error> {
    let skp = match PeraKeyPair::decode(&value) {
        Ok(s) => s,
        Err(_) => match Hex::decode(&value) {
            Ok(decoded) => {
                ...
                PeraKeyPair::Ed25519(Ed25519KeyPair::from_bytes(&decoded)?)
            }
            Err(_) => match PeraKeyPair::decode_base64(&value) {
                Ok(skp) => skp,
                Err(_) => match Ed25519KeyPair::decode_base64(&value) {
                    Ok(kp) => PeraKeyPair::Ed25519(kp),
