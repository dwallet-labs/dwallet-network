...
use crate::client_commands::PeraClientCommands;
use crate::console::start_console;
use crate::fire_drill::{run_fire_drill, FireDrill};
use crate::genesis_ceremony::{run, Ceremony};
use crate::keytool::KeyToolCommand;
use crate::validator_commands::PeraValidatorCommand;
...
use pera_config::node::Genesis;
use pera_config::p2p::SeedPeer;
use pera_config::{
    genesis_blob_exists, pera_config_dir, Config, PersistedConfig, FULL_NODE_DB_PATH,
    PERA_CLIENT_CONFIG, PERA_FULLNODE_CONFIG, PERA_NETWORK_CONFIG,
};
use pera_config::{
    PERA_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME, PERA_GENESIS_FILENAME, PERA_KEYSTORE_FILENAME,
};
use pera_faucet::{create_wallet_context, start_faucet, AppState, FaucetConfig, SimpleFaucet};
#[cfg(feature = "indexer")]
use pera_graphql_rpc::{
    config::ConnectionConfig, test_infra::cluster::start_graphql_server_with_fn_rpc,
};
#[cfg(feature = "indexer")]
use pera_indexer::test_utils::{start_test_indexer, ReaderWriterConfig};
use pera_keys::keypair_file::read_key;
use pera_keys::keystore::{AccountKeystore, FileBasedKeystore, Keystore};
use pera_move::{self, execute_move_command};
use pera_move_build::PeraPackageHooks;
use pera_sdk::pera_client_config::{PeraClientConfig, PeraEnv};
use pera_sdk::wallet_context::WalletContext;
use pera_swarm::memory::Swarm;
use pera_swarm_config::genesis_config::{GenesisConfig, DEFAULT_NUMBER_OF_AUTHORITIES};
...
const DEFAULT_FAUCET_NUM_COINS: usize = 5; // 5 coins per request was the default in pera-test-validator
const DEFAULT_FAUCET_NPERA_AMOUNT: u64 = 200_000_000_000; // 200 PERA
...
pub struct IndexerFeatureArgs {
    ...
    /// DB name for the Indexer Postgres DB. Default DB name is pera_indexer.
    #[clap(long, default_value = "pera_indexer")]
    pg_db_name: String,
...
impl IndexerFeatureArgs {
    pub fn for_testing() -> Self {
        Self {
            ...
            pg_db_name: "pera_indexer".to_string(),
...
pub enum PeraCommand {
    ...
    ///
    /// By default, pera start will start a local network from the genesis blob that exists in
    /// the Pera config default dir or in the config_dir that was passed. If the default directory
    ...
    Start {
        ...
        /// pera genesis -f --with-faucet generates a genesis config that can be used to start this
        /// proces. Use with caution as the `-f` flag will overwrite the existing config directory.
        /// We can use any config dir that is generated by the `pera genesis`.
        ...
        /// run this command.
        ///
        /// To run with persisted state, do not pass this flag and use the `pera genesis` command
    ...
    /// Bootstrap and initialize a new pera network
    #[clap(name = "genesis")]
    Genesis {
        ...
        #[clap(
            ...
            num_args(1..),
            value_delimiter = ',',
            help = "A list of ip addresses to generate a genesis peratable for benchmarks"
        )]
        benchmark_ips: Option<Vec<String>>,
        #[clap(
            long,
            help = "Creates an extra faucet configuration for pera persisted runs."
        )]
        with_faucet: bool,
    },
    ...
    /// Start Pera interactive console.
    #[clap(name = "console")]
    Console {
    ...
    /// Client for interacting with the Pera network.
    #[clap(name = "client")]
    Client {
        /// Sets the file storing the state of our user accounts (an empty one will be created if missing)
        #[clap(long = "client.config")]
    ...
    Validator {
        ...
        config: Option<PathBuf>,
        #[clap(subcommand)]
        cmd: Option<PeraValidatorCommand>,
        /// Return command outputs in json format.
        #[clap(long, global = true)]
        json: bool,
        #[clap(short = 'y', long = "yes")]
    ...
    /// Invoke Pera's move-analyzer via CLI
    ...
    pub async fn execute(self) -> Result<(), anyhow::Error> {
        ...
        match self {
            PeraCommand::Network {
                ...
                let config_path = config.unwrap_or(pera_config_dir()?.join(PERA_NETWORK_CONFIG));
                let config: NetworkConfig = PersistedConfig::read(&config_path).map_err(|err| {
                    err.context(format!(
                        "Cannot open Pera network config file at {:?}",
                        config_path
                    ))
                })?;
            ...
            PeraCommand::GenesisCeremony(cmd) => run(cmd),
            PeraCommand::KeyTool {
                keystore_path,
                json,
                cmd,
                ...
                let keystore_path =
                    keystore_path.unwrap_or(pera_config_dir()?.join(PERA_KEYSTORE_FILENAME));
                let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path)?);
                cmd.execute(&mut keystore).await?.print(!json);
                Ok(())
            }
            PeraCommand::Console { config } => {
                let config = config.unwrap_or(pera_config_dir()?.join(PERA_CLIENT_CONFIG));
            ...
            PeraCommand::Client {
                ...
                let config_path = config.unwrap_or(pera_config_dir()?.join(PERA_CLIENT_CONFIG));
                ...
                if let Some(cmd) = cmd {
                    ...
                    // Print help
                    let mut app: Command = PeraCommand::command();
                    app.build();
                    app.find_subcommand_mut("client").unwrap().print_help()?;
                }
                Ok(())
            }
            PeraCommand::Validator {
                config,
                cmd,
                json,
                accept_defaults,
            ...
            PeraCommand::Move {
                ...
                match &mut cmd {
                    pera_move::Command::Build(build) if build.dump_bytecode_as_base64 => {
                        // `pera move build` does not ordinarily require a network connection.
                        ...
                        let config =
                            client_config.unwrap_or(pera_config_dir()?.join(PERA_CLIENT_CONFIG));
                        prompt_if_no_config(&config, false).await?;
                        let context = WalletContext::new(&config, None, None)?;
            ...
            PeraCommand::BridgeInitialize {
                network_config,
                client_config,
                bridge_committee_config_path,
            } => {
                // Load the config of the Pera authority.
                let network_config_path = network_config
                    .clone()
                    .unwrap_or(pera_config_dir()?.join(PERA_NETWORK_CONFIG));
                ...
                let pera_bridge_client = PeraBridgeClient::new(rpc_url).await?;
                ...
                {
                    let account_kp = node_config.account_key_pair.keypair();
                    let pera_address = PeraAddress::from(&account_kp.public());
            ...
            PeraCommand::FireDrill { fire_drill } => run_fire_drill(fire_drill).await,
            PeraCommand::Analyzer => {
                analyzer::run();
                Ok(())
            }
        }
...
async fn start(
    ...
    if epoch_duration_ms.is_some() && genesis_blob_exists(config.clone()) && !force_regenesis {
        bail!(
            ...
            there is no genesis configuration in the default Pera configuration folder or the given \
    ...
    if force_regenesis {
        ...
        let network_config_path = if let Some(ref config) = config {
            ...
            config
                .clone()
                .unwrap_or(pera_config_dir()?)
                .join(PERA_NETWORK_CONFIG)
        };
    ...
    let mut fullnode_url = pera_config::node::default_json_rpc_address();
    ...
    if let Some(input) = with_faucet {
        ...
        let config = FaucetConfig {
            ...
            amount: DEFAULT_FAUCET_NPERA_AMOUNT,
            ..Default::default()
        };
        ...
        if force_regenesis {
            ...
            let keystore_path = config_dir.join(PERA_KEYSTORE_FILENAME);
            let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
            let address: PeraAddress = kp.public().into();
            keystore.add_key(None, PeraKeyPair::Ed25519(kp)).unwrap();
            PeraClientConfig {
                keystore,
                envs: vec![PeraEnv {
            ...
            .persisted(config_dir.join(PERA_CLIENT_CONFIG).as_path())
            .save()
            .unwrap();
        }
    ...
    loop {
        ...
        for (node_index, node) in swarm.validator_nodes().enumerate() {
            if loop_index == 3 && node_index == 3 {
                ...
                node.stop();
            } else if loop_index == 13 && node_index == 3 {
...
async fn genesis(
    ...
    let pera_config_dir = &match working_dir {
        // if a directory is specified, it must exist (it
        // will not be created)
        Some(v) => v,
        // create default Pera config dir if not specified
        // on the command line and if it does not exist
        // yet
        None => {
            let config_path = pera_config_dir()?;
            fs::create_dir_all(&config_path)?;
            config_path
        }
    };

    // if Pera config dir is not empty then either clean it
    // up (if --force/-f option was specified or report an
    // error
    let dir = pera_config_dir.read_dir().map_err(|err| {
        anyhow!(err).context(format!("Cannot open Pera config dir {:?}", pera_config_dir))
    })?;
    let files = dir.collect::<Result<Vec<_>, _>>()?;

    let client_path = pera_config_dir.join(PERA_CLIENT_CONFIG);
    let keystore_path = pera_config_dir.join(PERA_KEYSTORE_FILENAME);
    ...
    if write_config.is_none() && !files.is_empty() {
        if force {
            ...
            let is_compatible = FileBasedKeystore::new(&keystore_path).is_ok()
                && PersistedConfig::<PeraClientConfig>::read(&client_path).is_ok();
            ...
            bail!("Cannot run genesis with non-empty Pera config directory {}, please use the --force/-f option to remove the existing configuration", pera_config_dir.to_str().unwrap());
        }
    }

    let network_path = pera_config_dir.join(PERA_NETWORK_CONFIG);
    let genesis_path = pera_config_dir.join(PERA_GENESIS_FILENAME);
    ...
    let mut genesis_conf = match from_config {
        ...
        None => {
            if let Some(ips) = benchmark_ips {
                ...
                let path = pera_config_dir.join(PERA_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME);
    ...
    let builder = ConfigBuilder::new(pera_config_dir);
    ...
    for validator in &mut network_config.validator_configs {
        validator.genesis = pera_config::node::Genesis::new_from_file(&genesis_path);
    }
    ...
    let fullnode_config = FullnodeConfigBuilder::new()
        ...
        .with_rpc_addr(pera_config::node::default_json_rpc_address())
        .build(&mut OsRng, &network_config);

    fullnode_config.save(pera_config_dir.join(PERA_FULLNODE_CONFIG))?;
    let mut ssfn_nodes = vec![];
    if let Some(ssfn_info) = ssfn_info {
        for (i, ssfn) in ssfn_info.into_iter().enumerate() {
            let path =
                pera_config_dir.join(pera_config::ssfn_config_file(ssfn.p2p_address.clone(), i));
            ...
            let ssfn_config = FullnodeConfigBuilder::new()
                ...
                .with_db_path(PathBuf::from("/opt/pera/db/authorities_db/full_node_db"))
                ...
                .with_genesis(Genesis::new_from_file("/opt/pera/config/genesis.blob"))
                .build(&mut OsRng, &network_config);
        ...
        for (i, mut validator) in network_config
            ...
            .enumerate()
        {
            let path = pera_config_dir.join(pera_config::validator_config_file(
    ...
    let mut client_config = if client_path.exists() {
        ...
        PeraClientConfig::new(keystore.into())
    };
    ...
    client_config.add_env(PeraEnv::devnet());
...
async fn prompt_if_no_config(
    ...
    if !wallet_conf_path.exists() {
        let env = match std::env::var_os("PERA_CONFIG_WITH_RPC_URL") {
            Some(v) => Some(PeraEnv {
            ...
            None => {
                if accept_defaults {
                    ...
                    print!(
                        "Config file [{:?}] doesn't exist, do you want to connect to a Pera Full node server [y/N]?",
                        wallet_conf_path
                    );
                }
                if accept_defaults
                    || matches!(read_line(), Ok(line) if line.trim().to_lowercase() == "y")
                {
                    let url = if accept_defaults {
                        ...
                        print!(
                            "Pera Full node server URL (Defaults to Pera Testnet if not specified) : "
                        );
                        read_line()?
                    };
