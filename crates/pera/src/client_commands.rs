...
use pera_json_rpc_types::{
    ...
    PeraExecutionStatus, PeraObjectData, PeraObjectDataOptions, PeraObjectResponse,
    PeraObjectResponseQuery, PeraParsedData, PeraProtocolConfigValue, PeraRawData,
    PeraTransactionBlockEffects, PeraTransactionBlockEffectsAPI, PeraTransactionBlockResponse,
...
use pera_package_management::{LockCommand, PublishedAtError};
use pera_replay::ReplayToolCommand;
use pera_sdk::{
    apis::ReadApi,
    pera_client_config::{PeraClientConfig, PeraEnv},
    wallet_context::WalletContext,
    PeraClient, PERA_COIN_TYPE, PERA_DEVNET_URL, PERA_LOCAL_NETWORK_URL, PERA_LOCAL_NETWORK_URL_0,
    PERA_TESTNET_URL,
};
use pera_types::{
...
pub enum PeraClientCommands {
    ...
    Balance {
        ...
        /// Show balance for the specified coin (e.g., 0x2::pera::PERA).
    ...
    Call {
        ...
        #[clap(
            long,
            value_parser = parse_pera_type_tag,
            num_args(1..),
        )]
    ...
    /// List all Pera environments
    Envs,

    /// Execute a Signed Transaction. This is useful when the user prefers to sign elsewhere and use this command to execute.
    ExecuteSignedTx {
        /// BCS serialized transaction data bytes without its type tag, as base64 encoded string. This is the output of pera client command using --serialize-unsigned-transaction.
        #[clap(long)]
        tx_bytes: String,
    ...
    ExecuteCombinedSignedTx {
        /// BCS serialized sender signed data, as base64 encoded string. This is the output of pera client command using --serialize-signed-transaction.
        #[clap(long)]
    ...
    Objects {
        ...
        /// objects owned by `pera client active-address`.
        #[clap(name = "owner_address")]
        address: Option<KeyIdentity>,
    },
    ...
    /// Pay all residual PERA coins to the recipient with input coins, after deducting the gas cost.
    ...
    /// Pay PERA coins to recipients following following specified amounts, with input coins.
    /// Length of recipients must be the same as that of amounts.
    /// The input coins also include the coin for gas payment, so no extra gas coin is required.
    PayPera {
        /// The input coins to be used for pay recipients, including the gas coin.
...
#[derive(Args, Debug)]
pub struct Opts {
    /// An optional gas budget for this transaction (in NPERA). If gas budget is not provided, the
    ...
    /// be used to execute transaction with `pera client execute-signed-tx --tx-bytes <TX_BYTES>`.
    ...
    /// `pera client execute-combined-signed-tx --signed-tx-bytes <SIGNED_TX_BYTES>`.
    #[arg(long, required = false)]
...
impl PeraClientCommands {
    pub async fn execute(
        self,
        context: &mut WalletContext,
    ) -> Result<PeraClientCommandResult, anyhow::Error> {
        let ret = match self {
            PeraClientCommands::ProfileTransaction {
                tx_digest,
                profile_output,
            } => {
                ...
                let _command_result =
                    pera_replay::execute_replay_command(Some(rpc), false, false, None, None, cmd)
                        .await?;
                // this will be displayed via trace info, so no output is needed here
                PeraClientCommandResult::NoOutput
            }
            PeraClientCommands::ReplayTransaction {
                tx_digest,
                gas_info: _,
                ptb_info: _,
                executor_version,
            ...
            PeraClientCommands::ReplayBatch {
                path,
                terminate_early,
            } => {
            ...
            PeraClientCommands::ReplayCheckpoints {
                start,
                end,
                terminate_early,
            } => {
            ...
            PeraClientCommands::Addresses { sort_by_alias } => {
                let active_address = context.active_address()?;
                let mut addresses: Vec<(String, PeraAddress)> = context
                    .config
                    .keystore
                ...
                PeraClientCommandResult::Addresses(output)
            }
            PeraClientCommands::Balance {
                address,
                coin_type,
                with_coins,
            } => {
                ...
                let pera_type_tag = canonicalize_type(PERA_COIN_TYPE)?;

                // show PERA first
                let ordered_coins_pera_first = coins_by_type
                    .remove(&pera_type_tag)
                    .into_iter()
                    .chain(coins_by_type.into_values())
                    .collect();

                PeraClientCommandResult::Balance(ordered_coins_pera_first, with_coins)
            }

            PeraClientCommands::DynamicFieldQuery { id, cursor, limit } => {
                let client = context.get_client().await?;
                let df_read = client
                    .read_api()
                    .get_dynamic_fields(id, cursor, Some(limit))
                    .await?;
                PeraClientCommandResult::DynamicFieldQuery(df_read)
            }

            PeraClientCommands::Upgrade {
                package_path,
                upgrade_capability,
                build_config,
                ...
                let package_path =
                    package_path
                        .canonicalize()
                        .map_err(|e| PeraError::ModulePublishFailure {
                ...
                let previous_id = if let Some(ref chain_id) = chain_id {
                    pera_package_management::set_package_id(
                        &package_path,
                ...
                if let (Some(chain_id), Some(previous_id)) = (chain_id, previous_id) {
                    let _ = pera_package_management::set_package_id(
                        &package_path,
                        build_config.install_dir.clone(),
                        &chain_id,
                        previous_id,
                ...
                if let PeraClientCommandResult::TransactionBlock(ref response) = result {
                    if let Err(e) = pera_package_management::update_lock_file(
                        context,
                        LockCommand::Upgrade,
            ...
            PeraClientCommands::Publish {
                ...
                if build_config.test_mode {
                    return Err(PeraError::ModulePublishFailure {
                        error:
                            ...
                            compiling the package normally with `pera move build`."
                                .to_string(),
                    }
                    .into());
                }
            ...
            PeraClientCommands::VerifyBytecodeMeter {
                protocol_version,
                module_paths,
                package_path,
                build_config,
                ...
                let mut verifier = pera_execution::verifier(
                    &protocol_config,
                    for_signing,
                ...
                PeraClientCommandResult::VerifyBytecodeMeter {
                    success: !exceeded,
            ...
            PeraClientCommands::Object { id, bcs } => {
                // Fetch the object ref
                let client = context.get_client().await?;
                if !bcs {
                    let object_read = client
                        .read_api()
                        .get_object_with_options(id, PeraObjectDataOptions::full_content())
                        .await?;
                    PeraClientCommandResult::Object(object_read)
                } else {
                    let raw_object_read = client
                        .read_api()
                        .get_object_with_options(id, PeraObjectDataOptions::bcs_lossless())
                        .await?;
                    PeraClientCommandResult::RawObject(raw_object_read)
                }
            }

            PeraClientCommands::TransactionBlock { digest } => {
                let client = context.get_client().await?;
                let tx_read = client
                    .read_api()
                    ...
                    .await?;
                PeraClientCommandResult::TransactionBlock(tx_read)
            }

            PeraClientCommands::Call {
                package,
                module,
                function,
                type_args,
            ...
            PeraClientCommands::TransferPera {
                to,
                pera_coin_object_id: object_id,
                amount,
                opts,
            } => {
                let signer = context.get_object_owner(&object_id).await?;
                ...
                let tx_kind = client
                    .transaction_builder()
                    .transfer_pera_tx_kind(to, amount);
                dry_run_or_execute_or_serialize(
                    signer,
                    tx_kind,
                    context,
            ...
            PeraClientCommands::Pay {
                ...
                let recipients = recipients
                    ...
                    .collect::<Result<Vec<PeraAddress>, anyhow::Error>>()
                    .map_err(|e| anyhow!("{e}"))?;
                ...
                if let Some(gas) = opts.gas {
                    if input_coins.contains(&gas) {
                        bail!("Gas coin is in input coins of Pay transaction, use PayPera transaction instead!");
                    }
                }
            ...
            PeraClientCommands::PayPera {
                ...
                opts,
                    ...
                    !input_coins.is_empty(),
                    "PayPera transaction requires a non-empty list of input coins"
                );
                ensure!(
                    !recipients.is_empty(),
                    "PayPera transaction requires a non-empty list of recipient addresses"
                );
                ensure!(
                    recipients.len() == amounts.len(),
                    format!(
                ...
                let tx_kind = client
                    .transaction_builder()
                    .pay_pera_tx_kind(recipients, amounts)?;
            ...
            PeraClientCommands::PayAllPera {
                ...
                opts,
                    ...
                    !input_coins.is_empty(),
                    "PayAllPera transaction requires a non-empty list of input coins"
                );
                let recipient = get_identity_address(Some(recipient), context)?;
                let signer = context.get_object_owner(&input_coins[0]).await?;
                let client = context.get_client().await?;
                let tx_kind = client.transaction_builder().pay_all_pera_tx_kind(recipient);
                dry_run_or_execute_or_serialize(
                    signer,
                    tx_kind,
                    context,
            ...
            PeraClientCommands::Objects { address } => {
                let address = get_identity_address(address, context)?;
                let client = context.get_client().await?;
                ...
                loop {
                    let response = client
                        ...
                        .get_owned_objects(
                            address,
                            Some(PeraObjectResponseQuery::new_with_options(
                                PeraObjectDataOptions::full_content(),
                            )),
                            cursor,
                            None,
                        )
                ...
                PeraClientCommandResult::Objects(objects)
            }

            PeraClientCommands::NewAddress {
                key_scheme,
                alias,
                derivation_path,
                word_length,
                ...
                PeraClientCommandResult::NewAddress(NewAddressOutput {
                    alias,
                    address,
                    key_scheme: scheme,
            ...
            PeraClientCommands::Gas { address } => {
                let address = get_identity_address(address, context)?;
                let coins = context
                    .gas_objects(address)
                    .await?
                    ...
                    .collect();
                PeraClientCommandResult::Gas(coins)
            }
            PeraClientCommands::Faucet { address, url } => {
                let address = get_identity_address(address, context)?;
                let url = if let Some(url) = url {
                    ...
                    if let Ok(env) = active_env {
                        let network = match env.rpc.as_str() {
                            PERA_DEVNET_URL => "https://faucet.devnet.pera.io/v1/gas",
                            PERA_TESTNET_URL => "https://faucet.testnet.pera.io/v1/gas",
                            PERA_LOCAL_NETWORK_URL | PERA_LOCAL_NETWORK_URL_0 => "http://127.0.0.1:9123/gas",
            ...
            PeraClientCommands::ChainIdentifier => {
                let ci = context
                    .get_client()
                    .await?
                    .read_api()
                    ...
                    .await?;
                PeraClientCommandResult::ChainIdentifier(ci)
            }
            PeraClientCommands::SplitCoin {
                coin_id,
                amounts,
                count,
                opts,
            ...
            PeraClientCommands::Switch { address, env } => {
                let mut addr = None;
                ...
                context.config.save()?;
                PeraClientCommandResult::Switch(SwitchResponse { address: addr, env })
            }
            PeraClientCommands::ActiveAddress => {
                PeraClientCommandResult::ActiveAddress(context.active_address().ok())
            }

            PeraClientCommands::ExecuteSignedTx {
                tx_bytes,
                signatures,
            } => {
                let data = bcs::from_bytes(
                    ...
                    .to_vec()
                    .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse tx bytes, check if it matches the output of pera client commands with --serialize-unsigned-transaction"))?;
                ...
                PeraClientCommandResult::TransactionBlock(response)
            }
            PeraClientCommands::ExecuteCombinedSignedTx { signed_tx_bytes } => {
                let data: SenderSignedData = bcs::from_bytes(
                    &Base64::try_from(signed_tx_bytes)
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                        .to_vec()
                        .map_err(|_| anyhow!("Invalid Base64 encoding"))?
                ).map_err(|_| anyhow!("Failed to parse SenderSignedData bytes, check if it matches the output of pera client commands with --serialize-signed-transaction"))?;
                let transaction = Envelope::<SenderSignedData, EmptySignInfo>::new(data);
                let response = context.execute_transaction_may_fail(transaction).await?;
                PeraClientCommandResult::TransactionBlock(response)
            }
            PeraClientCommands::NewEnv {
                alias,
                rpc,
                ws,
                basic_auth,
                ...
                context.config.save()?;
                PeraClientCommandResult::NewEnv(env)
            }
            PeraClientCommands::ActiveEnv => {
                PeraClientCommandResult::ActiveEnv(context.config.active_env.clone())
            }
            PeraClientCommands::Envs => PeraClientCommandResult::Envs(
                context.config.envs.clone(),
                context.config.active_env.clone(),
            ),
            PeraClientCommands::VerifySource {
                package_path,
                build_config,
                verify_deps,
                skip_source,
                ...
                PeraClientCommandResult::VerifySource
            }
            PeraClientCommands::PTB(ptb) => {
    ...
    pub fn switch_env(config: &mut PeraClientConfig, env: &str) -> Result<(), anyhow::Error> {
        let env = Some(env.into());
        ensure!(config.get_env(&env).is_some(), "Environment config not found for [{env:?}], add new environment config using the `pera client new-env` command.");
...
pub(crate) async fn upgrade_package(
        ...
        PublishedAtError::Conflict {
            ...
            anyhow!(
                "Conflicting published package address: `Move.toml` contains published-at address \
                 ...
                 - update the `Move.lock` address using the `pera manage-package` command to be the same as the `Move.toml`; OR
                 - check that your `pera active-env` {env_alias} corresponds to the chain on which the package is published (i.e., devnet, testnet, mainnet); OR
    ...
    let resp = read_api
        .get_object_with_options(
            upgrade_capability,
            PeraObjectDataOptions::default().with_bcs().with_owner(),
        )
        .await?;
...
> {
    ...
    if let Some(Some(PeraProtocolConfigValue::U32(min_version))) = protocol_config
        .attributes
    ...
    // Check that the package's Move version is compatible with the chain's
    if let Some(Some(PeraProtocolConfigValue::U32(max_version))) =
    ...
    compiled_package
        ...
        .map_err(|e| PeraError::ModuleBuildFailure {
...
impl Display for PeraClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let mut writer = String::new();
        match self {
            PeraClientCommandResult::Addresses(addresses) => {
            ...
            PeraClientCommandResult::Balance(coins, with_coins) => {
                if coins.is_empty() {
            ...
            PeraClientCommandResult::DynamicFieldQuery(df_refs) => {
            ...
            PeraClientCommandResult::Gas(gas_coins) => {
                ...
                for coin in &gas_coins {
                    builder.push_record(vec![
                        ...
                        coin.npera_balance.to_string(),
                        coin.pera_balance.to_string(),
                    ]);
                }
                let mut table = builder.build();
            ...
            PeraClientCommandResult::NewAddress(new_address) => {
                let mut builder = TableBuilder::default();
            ...
            PeraClientCommandResult::Object(object_read) => match object_read.object() {
                Ok(obj) => {
            ...
            PeraClientCommandResult::Objects(object_refs) => {
            ...
            PeraClientCommandResult::RawObject(raw_object_read) => {
                let raw_object = match raw_object_read.object() {
                    Ok(v) => match &v.bcs {
                        Some(PeraRawData::MoveObject(o)) => {
                            format!("{:?}\nNumber of bytes: {}", o.bcs_bytes, o.bcs_bytes.len())
                        }
                        Some(PeraRawData::Package(p)) => {
            ...
            PeraClientCommandResult::SerializedUnsignedTransaction(tx_data) => {
                writeln!(
                    writer,
                    "{}",
            ...
            PeraClientCommandResult::SerializedSignedTransaction(sender_signed_tx) => {
                writeln!(
                    writer,
                    "{}",
            ...
            PeraClientCommandResult::SyncClientState => {
                writeln!(writer, "Client state sync complete.")?;
            }
            PeraClientCommandResult::ChainIdentifier(ci) => {
                writeln!(writer, "{}", ci)?;
            }
            PeraClientCommandResult::Switch(response) => {
                write!(writer, "{}", response)?;
            }
            PeraClientCommandResult::ActiveAddress(response) => {
                match response {
                    Some(r) => write!(writer, "{}", r)?,
                    None => write!(writer, "None")?,
                };
            }
            PeraClientCommandResult::ActiveEnv(env) => {
                write!(writer, "{}", env.as_deref().unwrap_or("None"))?;
            }
            PeraClientCommandResult::NewEnv(env) => {
                writeln!(writer, "Added new Pera env [{}] to config.", env.alias)?;
            }
            PeraClientCommandResult::Envs(envs, active) => {
                let mut builder = TableBuilder::default();
            ...
            PeraClientCommandResult::NoOutput => {}
            PeraClientCommandResult::DryRun(response) => {
...
impl Debug for PeraClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
...
impl PeraClientCommandResult {
    pub fn objects_response(&self) -> Option<Vec<PeraObjectResponse>> {
        use PeraClientCommandResult::*;
        match self {
    ...
    pub fn tx_block_response(&self) -> Option<&PeraTransactionBlockResponse> {
        use PeraClientCommandResult::*;
        match self {
    ...
    pub async fn prerender_clever_errors(mut self, read_api: &ReadApi) -> Self {
        match &mut self {
            PeraClientCommandResult::DryRun(DryRunTransactionBlockResponse { effects, .. })
            | PeraClientCommandResult::TransactionBlock(PeraTransactionBlockResponse {
            ...
            PeraClientCommandResult::ActiveAddress(_)
            | PeraClientCommandResult::ActiveEnv(_)
            | PeraClientCommandResult::Addresses(_)
            | PeraClientCommandResult::Balance(_, _)
            | PeraClientCommandResult::ChainIdentifier(_)
            | PeraClientCommandResult::DynamicFieldQuery(_)
            | PeraClientCommandResult::Envs(_, _)
            | PeraClientCommandResult::Gas(_)
            | PeraClientCommandResult::NewAddress(_)
            | PeraClientCommandResult::NewEnv(_)
            | PeraClientCommandResult::NoOutput
            | PeraClientCommandResult::Object(_)
            | PeraClientCommandResult::Objects(_)
            | PeraClientCommandResult::RawObject(_)
            | PeraClientCommandResult::SerializedSignedTransaction(_)
            | PeraClientCommandResult::SerializedUnsignedTransaction(_)
            | PeraClientCommandResult::Switch(_)
            | PeraClientCommandResult::SyncClientState
            | PeraClientCommandResult::VerifyBytecodeMeter { .. }
            | PeraClientCommandResult::VerifySource => (),
        }
...
pub struct AddressesOutput {
    pub active_address: PeraAddress,
    pub addresses: Vec<(String, PeraAddress)>,
}
...
impl From<&PeraObjectData> for ObjectOutput {
    fn from(obj: &PeraObjectData) -> Self {
...
pub struct GasCoinOutput {
    ...
    pub npera_balance: u64,
    pub pera_balance: String,
}
...
impl From<&GasCoin> for GasCoinOutput {
    fn from(gas_coin: &GasCoin) -> Self {
        Self {
            ...
            npera_balance: gas_coin.value(),
            pera_balance: format_balance(gas_coin.value() as u128, 9, 2, None),
        }
    }
}
...
impl ObjectsOutput {
    fn from(obj: PeraObjectResponse) -> Result<Self, anyhow::Error> {
        let obj = obj.into_object()?;
        // this replicates the object type display as in the pera explorer
        let object_type = match obj.type_ {
            Some(pera_types::base_types::ObjectType::Struct(x)) => {
            ...
            Some(pera_types::base_types::ObjectType::Package) => "Package".to_string(),
            None => "unknown".to_string(),
        };
        Ok(Self {
            object_id: obj.object_id,
    ...
    fn from_vec(objs: Vec<PeraObjectResponse>) -> Result<Vec<Self>, anyhow::Error> {
        objs.into_iter()
...
#[derive(Serialize)]
#[serde(untagged)]
pub enum PeraClientCommandResult {
    ActiveAddress(Option<PeraAddress>),
    ActiveEnv(Option<String>),
    Addresses(AddressesOutput),
    Balance(Vec<(Option<PeraCoinMetadata>, Vec<Coin>)>, bool),
    ChainIdentifier(String),
    DynamicFieldQuery(DynamicFieldPage),
    DryRun(DryRunTransactionBlockResponse),
    Envs(Vec<PeraEnv>, Option<String>),
    Gas(Vec<GasCoin>),
    NewAddress(NewAddressOutput),
    NewEnv(PeraEnv),
    NoOutput,
    Object(PeraObjectResponse),
    Objects(Vec<PeraObjectResponse>),
    RawObject(PeraObjectResponse),
    SerializedSignedTransaction(SenderSignedData),
    SerializedUnsignedTransaction(TransactionData),
    Switch(SwitchResponse),
    SyncClientState,
    TransactionBlock(PeraTransactionBlockResponse),
    VerifyBytecodeMeter {
        success: bool,
    ...
    match resp.status() {
        StatusCode::ACCEPTED | StatusCode::CREATED => {
            ...
            if let Some(err) = faucet_resp.error {
                ...
                println!("Request successful. It can take up to 1 minute to get the coin. Run pera client gas to check your gas coins.");
            }
        }
...
fn pretty_print_balance(
    coins_by_type: &Vec<(Option<PeraCoinMetadata>, Vec<Coin>)>,
...
pub async fn execute_dry_run(
    ...
    let resp = PeraClientCommandResult::DryRun(response)
...
pub async fn estimate_gas_budget(
    ...
    let Ok(PeraClientCommandResult::DryRun(dry_run)) =
...
pub async fn max_gas_budget(client: &PeraClient) -> Result<u64, anyhow::Error> {
    let cfg = client.read_api().get_protocol_config(None).await?;
    Ok(match cfg.attributes.get("max_tx_gas") {
        Some(Some(pera_json_rpc_types::PeraProtocolConfigValue::U64(y))) => *y,
        _ => bail!(
...
pub(crate) async fn dry_run_or_execute_or_serialize(
    ...
    if serialize_unsigned_transaction {
        Ok(PeraClientCommandResult::SerializedUnsignedTransaction(
            tx_data,
        ))
    } else {
        ...
        if serialize_signed_transaction {
            Ok(PeraClientCommandResult::SerializedSignedTransaction(
                sender_signed_data,
            ))
        } else {
            ...
            let effects = response.effects.as_ref().ok_or_else(|| {
                anyhow!("Effects from PeraTransactionBlockResult should not be empty")
            })?;
            if let PeraExecutionStatus::Failure { error } = effects.status() {
                return Err(anyhow!(
                    "Error executing transaction '{}': {error}",
                    response.digest
                ));
            }
            Ok(PeraClientCommandResult::TransactionBlock(response))
        }
    }
}

pub(crate) async fn prerender_clever_errors(
    effects: &mut PeraTransactionBlockEffects,
    read_api: &ReadApi,
) {
    let PeraTransactionBlockEffects::V1(effects) = effects;
    if let PeraExecutionStatus::Failure { error } = &mut effects.status {
