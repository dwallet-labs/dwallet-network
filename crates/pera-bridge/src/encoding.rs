...
use crate::types::AddTokensOnPeraAction;
...
use crate::types::EthToPeraBridgeAction;
...
use pera_types::base_types::PERA_ADDRESS_LENGTH;
...
pub const ADD_TOKENS_ON_PERA_MESSAGE_VERSION: u8 = 1;
pub const ADD_TOKENS_ON_EVM_MESSAGE_VERSION: u8 = 1;

pub const BRIDGE_MESSAGE_PREFIX: &[u8] = b"PERA_BRIDGE_MESSAGE";
...
impl BridgeMessageEncoding for PeraToEthBridgeAction {
    fn as_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::new();
        let e = &self.pera_bridge_event;
        // Add message type
        ...
        // Add source chain id
        bytes.push(e.pera_chain_id as u8);
    ...
    fn as_payload_bytes(&self) -> Vec<u8> {
        ...
        bytes.push(PERA_ADDRESS_LENGTH as u8);
        // Add source address
        bytes.extend_from_slice(&e.pera_address.to_vec());
        // Add dest chain id
        ...
        // Add token amount
        bytes.extend_from_slice(&e.amount_pera_adjusted.to_be_bytes());

        bytes
    }
}

impl BridgeMessageEncoding for EthToPeraBridgeAction {
    fn as_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::new();
    ...
    fn as_payload_bytes(&self) -> Vec<u8> {
        ...
        // Add token amount
        bytes.extend_from_slice(&e.pera_adjusted_amount.to_be_bytes());
...
impl BridgeMessageEncoding for AddTokensOnPeraAction {
    fn as_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::new();
        // Add message type
        bytes.push(BridgeActionType::AddTokensOnPera as u8);
        // Add message version
        bytes.push(ADD_TOKENS_ON_PERA_MESSAGE_VERSION);
        // Add nonce
        bytes.extend_from_slice(&self.nonce.to_be_bytes());
        // Add chain id
        bytes.push(self.chain_id as u8);
...
impl BridgeMessageEncoding for AddTokensOnEvmAction {
    ...
    fn as_payload_bytes(&self) -> Vec<u8> {
        ...
        // Add token pera decimals
        // Unwrap: bcs serialization should not fail
        bytes.push(u8::try_from(self.token_pera_decimals.len()).unwrap());
        for token_pera_decimal in &self.token_pera_decimals {
            bytes.push(*token_pera_decimal);
        }
...
mod tests {
    ...
    use crate::events::EmittedPeraToEthTokenBridgeV1;
    ...
    fn test_bridge_message_encoding() -> anyhow::Result<()> {
        ...
        let pera_chain_id = BridgeChainId::PeraTestnet;
        let pera_tx_event_index = 1u16;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let pera_address = PeraAddress::random_for_testing_only();
        let eth_address = EthAddress::random();
        let token_id = TOKEN_ID_USDC;
        let amount_pera_adjusted = 1_000_000;

        let pera_bridge_event = EmittedPeraToEthTokenBridgeV1 {
            nonce,
            pera_chain_id,
            eth_chain_id,
            pera_address,
            eth_address,
            token_id,
            amount_pera_adjusted,
        };

        let encoded_bytes = BridgeAction::PeraToEthBridgeAction(PeraToEthBridgeAction {
            pera_tx_digest,
            pera_tx_event_index,
            pera_bridge_event,
        })
        ...
        let source_chain_id_bytes = vec![pera_chain_id as u8]; // len: 1

        let pera_address_length_bytes = vec![PERA_ADDRESS_LENGTH as u8]; // len: 1
        let pera_address_bytes = pera_address.to_vec(); // len: 32
        ...
        let token_amount_bytes = amount_pera_adjusted.to_be_bytes().to_vec(); // len: 8
        ...
        combined_bytes.extend_from_slice(&pera_address_length_bytes);
        combined_bytes.extend_from_slice(&pera_address_bytes);
    ...
    #[test]
    fn test_bridge_message_encoding_regression_emitted_pera_to_eth_token_bridge_v1(
    ) -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        ...
        let pera_chain_id = BridgeChainId::PeraTestnet;
        let eth_chain_id = BridgeChainId::EthSepolia;
        let pera_address = PeraAddress::from_str(
        ...
        let token_id = TOKEN_ID_USDC;
        let amount_pera_adjusted = 12345;
    ...
    #[test]
    fn test_bridge_message_encoding_regression_eth_to_pera_token_bridge_v1() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);
        ...
        let eth_address =
            EthAddress::from_str("0x00000000000000000000000000000000000000c8").unwrap();
        let token_id = TOKEN_ID_USDC;
        let pera_adjusted_amount = 12345;

        let eth_bridge_event = EthToPeraTokenBridgeV1 {
            nonce,
            pera_chain_id,
            eth_chain_id,
            pera_address,
            eth_address,
            token_id,
            pera_adjusted_amount,
        };
        let encoded_bytes = BridgeAction::EthToPeraBridgeAction(EthToPeraBridgeAction {
            eth_tx_hash,
            eth_event_index,
            eth_bridge_event,
        })
    ...
    #[test]
    fn test_bridge_message_encoding_regression_add_coins_on_pera() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();

        let action = BridgeAction::AddTokensOnPeraAction(AddTokensOnPeraAction {
            nonce: 0,
            chain_id: BridgeChainId::PeraCustom,
            native: false,
