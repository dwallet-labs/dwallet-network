...
use crate::{
    ...
    types::{
        AddTokensOnEvmAction, AddTokensOnPeraAction, AssetPriceUpdateAction,
...
pub const ETH_TO_PERA_TX_PATH: &str = "/sign/bridge_tx/eth/pera/:tx_hash/:event_index";
pub const PERA_TO_ETH_TX_PATH: &str = "/sign/bridge_tx/pera/eth/:tx_digest/:event_index";
...
pub const ADD_TOKENS_ON_PERA_PATH: &str =
...
pub(crate) fn make_router(
    ...
    Router::new()
        ...
        .route(ETH_TO_PERA_TX_PATH, get(handle_eth_tx_hash))
        .route(PERA_TO_ETH_TX_PATH, get(handle_pera_tx_digest))
        .route(
        ...
        .route(ADD_TOKENS_ON_PERA_PATH, get(handle_add_tokens_on_pera))
...
async fn handle_pera_tx_digest(
    ...
    let future = async {
        let sig: Json<SignedBridgeAction> = handler
            .handle_pera_tx_digest(tx_digest_base58, event_idx)
            .await?;
        Ok(sig)
    };
    with_metrics!(metrics.clone(), "handle_pera_tx_digest", future).await
}
...
async fn handle_add_tokens_on_pera(
    ...
    let future = async {
        ...
        if !chain_id.is_pera_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_pera only expects Pera chain id".to_string(),
            ));
        }
    ...
    with_metrics!(metrics.clone(), "handle_add_tokens_on_pera", future).await
}

#[instrument(level = "error", skip_all, fields(chain_id=chain_id, nonce=nonce, native=native, token_ids=token_ids, token_addresses=token_addresses, token_pera_decimals=token_pera_decimals, token_prices=token_prices))]
async fn handle_add_tokens_on_evm(
    Path((chain_id, nonce, native, token_ids, token_addresses, token_pera_decimals, token_prices)): Path<(
        u8,
        u64,
        u8,
        String,
    ...
    let future = async {
        ...
        if chain_id.is_pera_chain() {
            return Err(BridgeError::InvalidBridgeClientRequest(
                "handle_add_tokens_on_evm does not expect Pera chain id".to_string(),
            ));
        }
        ...
        let token_addresses = token_addresses
            ...
            .collect::<Result<Vec<_>, _>>()?;
        let token_pera_decimals = token_pera_decimals
            .split(',')
            .map(|s| {
                s.parse::<u8>().map_err(|err| {
                    BridgeError::InvalidBridgeClientRequest(format!(
                        "Invalid token pera decimals: {:?}",
                        err
                    ))
                })
            })
...
mod tests {
    ...
    #[tokio::test]
    async fn test_bridge_server_handle_add_tokens_on_pera_action_path() {
        let client = setup();
