...
pub trait BridgeRequestHandlerTrait {
    ...
    /// from Ethereum to Pera. The inputs are a transaction hash on Ethereum
    ...
    /// Handles a request to sign a BridgeAction that bridges assets
    /// from Pera to Ethereum. The inputs are a transaction digest on Pera
    /// that emitted the bridge event and the Event index in that transaction
    async fn handle_pera_tx_digest(
        &self,
...
#[async_trait::async_trait]
impl<C> ActionVerifier<(TransactionDigest, u16)> for PeraActionVerifier<C>
where
    C: PeraClientInner + Send + Sync + 'static,
{
    fn name(&self) -> &'static str {
        "PeraActionVerifier"
    }
    ...
    async fn verify(&self, key: (TransactionDigest, u16)) -> BridgeResult<BridgeAction> {
        ...
        self.pera_client
            ...
            .await
            .tap_ok(|action| info!("Pera action found: {:?}", action))
    }
}
...
pub struct BridgeRequestHandler {
    pera_signer_tx: mysten_metrics::metered_channel::Sender<(
...
impl BridgeRequestHandler {
    pub fn new<
        SC: PeraClientInner + Send + Sync + 'static,
        EP: JsonRpcClient + Send + Sync + 'static,
    >(
        signer: BridgeAuthorityKeyPair,
        pera_client: Arc<PeraClient<SC>>,
        eth_client: Arc<EthClient<EP>>,
        approved_governance_actions: Vec<BridgeAction>,
        metrics: Arc<BridgeMetrics>,
    ) -> Self {
        let (pera_signer_tx, pera_rx) = mysten_metrics::metered_channel::channel(
            1000,
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["server_pera_action_signing_queue"]),
        );
        ...
        SignerWithCache::new(
            signer.clone(),
            PeraActionVerifier { pera_client },
            metrics.clone(),
        )
        .spawn(pera_rx);
        SignerWithCache::new(
...
impl BridgeRequestHandlerTrait for BridgeRequestHandler {
    ...
    async fn handle_pera_tx_digest(
        ...
        self.pera_signer_tx
            ...
            .await
            .unwrap_or_else(|_| panic!("Server pera signing channel is closed"));
        let signed_action = rx
            .await
            .unwrap_or_else(|_| panic!("Server signing task's oneshot channel is dropped"))?;
        Ok(Json(signed_action))
...
mod tests {
    ...
    use crate::{
        ...
        events::{init_all_struct_tags, MoveTokenDepositedEvent, PeraToEthTokenBridgeV1},
        pera_mock_client::PeraMockClient,
        test_utils::{
            get_test_log_and_action, get_test_pera_to_eth_bridge_action, mock_last_finalized_block,
        },
    ...
    use pera_types::bridge::{BridgeChainId, TOKEN_ID_USDC};
    use pera_types::{base_types::PeraAddress, crypto::get_key_pair};

    #[tokio::test]
    async fn test_pera_signer_with_cache() {
        let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        let signer = Arc::new(kp);
        let pera_client_mock = PeraMockClient::default();
        let pera_verifier = PeraActionVerifier {
        ...
        let pera_event_idx = 42;
        assert!(pera_signer_with_cache
            .get_testing_only((pera_tx_digest, pera_event_idx))
            .await
            .is_none());
        let entry = pera_signer_with_cache
            .get_cache_entry((pera_tx_digest, pera_event_idx))
            .await;
        let entry_ = pera_signer_with_cache
            .get_testing_only((pera_tx_digest, pera_event_idx))
            .await;
        ...
        let pera_tx_digest = TransactionDigest::random();
        let pera_event_idx = 0;

        // Mock an non-cacheable error such as rpc error
        pera_client_mock.add_events_by_tx_digest_error(pera_tx_digest);
        pera_signer_with_cache
            .sign((pera_tx_digest, pera_event_idx))
            .await
            .unwrap_err();
        let entry_ = pera_signer_with_cache
            .get_testing_only((pera_tx_digest, pera_event_idx))
        ...
        pera_client_mock.add_events_by_tx_digest(pera_tx_digest, vec![]);
        assert!(matches!(
            pera_signer_with_cache
                .sign((pera_tx_digest, pera_event_idx))
                .await,
        ...
        // TODO: test BridgeEventInUnrecognizedPeraPackage, PeraBridgeEvent::try_from_pera_event
        // and BridgeEventNotActionable to be cached

        // Test `sign` caches Ok result
        let emitted_event_1 = MoveTokenDepositedEvent {
            seq_num: 1,
            source_chain: BridgeChainId::PeraCustom as u8,
        ...
        let pera_tx_digest = pera_event_1.id.tx_digest;

        let mut pera_event_2 = PeraEvent::random_for_testing();
        pera_event_2.type_ = PeraToEthTokenBridgeV1.get().unwrap().clone();
        pera_event_2.bcs = bcs::to_bytes(&emitted_event_1).unwrap();
        let pera_event_idx_2 = 1;
        pera_client_mock.add_events_by_tx_digest(pera_tx_digest, vec![pera_event_2.clone()]);

        pera_client_mock.add_events_by_tx_digest(
            pera_tx_digest,
            vec![pera_event_1.clone(), pera_event_2.clone()],
        );
        let signed_1 = pera_signer_with_cache
            .sign((pera_tx_digest, pera_event_idx))
            .await
            .unwrap();
        let signed_2 = pera_signer_with_cache
            .sign((pera_tx_digest, pera_event_idx_2))
            .await
            .unwrap();
    ...
    async fn test_signer_with_governace_verifier() {
        ...
        let action_2 = BridgeAction::LimitUpdateAction(LimitUpdateAction {
            chain_id: BridgeChainId::EthCustom,
            sending_chain_id: BridgeChainId::PeraCustom,
            nonce: 1,
            new_usd_limit: 10000,
        });
        ...
        let action_4 = get_test_pera_to_eth_bridge_action(None, None, None, None, None, None, None);
        assert!(matches!(
