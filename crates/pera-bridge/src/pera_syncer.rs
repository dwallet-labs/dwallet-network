...
//! The PeraSyncer module is responsible for synchronizing Events emitted
//! on Pera blockchain from concerned modules of bridge package 0x9.
...
const PERA_EVENTS_CHANNEL_SIZE: usize = 1000;

/// Map from contract address to their start cursor (exclusive)
pub type PeraTargetModules = HashMap<Identifier, Option<EventID>>;

pub struct PeraSyncer<C> {
...
{
    pub fn new(pera_client: Arc<PeraClient<C>>, cursors: PeraTargetModules) -> Self {
        Self {
            pera_client,
            cursors,
        }
    ...
    pub async fn run(
        ...
        let (events_tx, events_rx) = mysten_metrics::metered_channel::channel(
            PERA_EVENTS_CHANNEL_SIZE,
            &mysten_metrics::get_metrics()
                .unwrap()
                .channel_inflight
                .with_label_values(&["pera_events_queue"]),
        );
        ...
        for (module, cursor) in self.cursors {
            ...
            let pera_client_clone = self.pera_client.clone();
    ...
    async fn run_event_listening_task(
        ...
        events_sender: mysten_metrics::metered_channel::Sender<(Identifier, Vec<PeraEvent>)>,
        pera_client: Arc<PeraClient<C>>,
        query_interval: Duration,
    ) {
        tracing::info!(?module, ?cursor, "Starting pera events listening task");
        let mut interval = time::interval(query_interval);
        interval.set_missed_tick_behavior(time::MissedTickBehavior::Skip);
        loop {
            interval.tick().await;
            let Ok(Ok(events)) = retry_with_max_elapsed_time!(
                pera_client.query_events_by_module(BRIDGE_PACKAGE_ID, module.clone(), cursor),
                Duration::from_secs(120)
            ) else {
                tracing::error!("Failed to query events from pera client after retry");
                continue;
            };
            ...
            if len != 0 {
                events_sender
                    ...
                    .await
                    .expect("All Pera event channel receivers are closed");
                if let Some(next) = events.next_cursor {
                    cursor = Some(next);
                }
                tracing::info!(?module, ?cursor, "Observed {len} new Pera events");
            }
        }
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_pera_syncer_basic() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        let registry = Registry::new();
        mysten_metrics::init_metrics(&registry);

        let mock = PeraMockClient::default();
        let client = Arc::new(PeraClient::new_for_testing(mock.clone()));
        ...
        let (_handles, mut events_rx) = PeraSyncer::new(client, target_modules)
            .run(interval)
            .await
            .unwrap();
        ...
        let mut event_1: PeraEvent = PeraEvent::random_for_testing();
        let package_id = BRIDGE_PACKAGE_ID;
        event_1.type_.address = package_id.into();
        event_1.type_.module = module_foo.clone();
        let module_foo_events_1: pera_json_rpc_types::Page<PeraEvent, EventID> = EventPage {
        ...
        let mut event_2: PeraEvent = PeraEvent::random_for_testing();
    ...
    async fn assert_no_more_events(
        interval: Duration,
        events_rx: &mut mysten_metrics::metered_channel::Receiver<(Identifier, Vec<PeraEvent>)>,
    ) {
