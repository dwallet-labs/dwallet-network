...
impl BridgeClient {
    ...
    fn bridge_action_to_path(event: &BridgeAction) -> String {
        match event {
            BridgeAction::PeraToEthBridgeAction(e) => format!(
                "sign/bridge_tx/pera/eth/{}/{}",
                e.pera_tx_digest, e.pera_tx_event_index
            ),
            BridgeAction::EthToPeraBridgeAction(e) => format!(
                "sign/bridge_tx/eth/pera/{}/{}",
            ...
            BridgeAction::AddTokensOnPeraAction(a) => {
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_pera/{chain_id}/{nonce}/{native}/{token_ids}/{token_type_names}/{token_prices}"
                )
            }
            BridgeAction::AddTokensOnEvmAction(a) => {
                ...
                let token_prices = a
                    ...
                    .join(",");
                format!(
                    "sign/add_tokens_on_evm/{chain_id}/{nonce}/{native}/{token_ids}/{token_addresses}/{token_pera_decimals}/{token_prices}"
                )
            }
        }
    }
...
mod tests {
    ...
    use pera_types::bridge::{BridgeChainId, TOKEN_ID_BTC, TOKEN_ID_USDT};
    ...
    async fn test_bridge_client() {
        ...
        let action =
            get_test_pera_to_eth_bridge_action(None, Some(1), Some(1), Some(100), None, None, None);
        ...
        // Ok
        authority.base_url = "https://foo.perabridge.io".to_string();
        let committee = Arc::new(BridgeCommittee::new(vec![authority.clone()]).unwrap());
    ...
    async fn test_bridge_client_request_sign_action() {
        ...
        mock_handler.add_pera_event_response(tx_digest, event_idx, Ok(signed_event.clone()));
        ...
        let action2 = get_test_pera_to_eth_bridge_action(
            Some(tx_digest),
            Some(event_idx),
            Some(2),
            Some(200),
        ...
        mock_handler.add_pera_event_response(tx_digest, event_idx, Ok(wrong_signed_action));
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_pera_event_response(tx_digest, event_idx, Ok(signed_event));
        ...
        mock_handler.add_pera_event_response(tx_digest, event_idx, Ok(signed_event2));
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
        ...
        mock_handler.add_pera_event_response(tx_digest, event_idx, Ok(signed_event3));
        let err = client
            .request_sign_bridge_action(action.clone())
            .await
            .unwrap_err();
    ...
    fn test_bridge_action_path_regression_tests() {
        ...
        let pera_tx_event_index = 5;
        let action = BridgeAction::PeraToEthBridgeAction(crate::types::PeraToEthBridgeAction {
            pera_tx_digest,
            pera_tx_event_index,
        ...
        assert_eq!(
            ...
            format!(
                "sign/bridge_tx/pera/eth/{}/{}",
                pera_tx_digest, pera_tx_event_index
            )
        );

        let eth_tx_hash = TxHash::random();
        let eth_event_index = 6;
        let action = BridgeAction::EthToPeraBridgeAction(crate::types::EthToPeraBridgeAction {
            eth_tx_hash,
            eth_event_index,
        ...
        let action = BridgeAction::AddTokensOnPeraAction(crate::types::AddTokensOnPeraAction {
            nonce: 3,
            chain_id: BridgeChainId::PeraCustom,
            native: false,
