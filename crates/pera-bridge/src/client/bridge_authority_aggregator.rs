...
impl GetSigsState {
    ...
    fn handle_verified_signed_action(
        ...
        if self.total_ok_stake >= self.validity_threshold {
            ...
            let certified_action: pera_types::message_envelope::Envelope<
                BridgeAction,
...
async fn request_sign_bridge_action_into_certification(
    ...
    let preference = match action {
        BridgeAction::PeraToEthBridgeAction(_) => Some(BTreeSet::new()),
        BridgeAction::EthToPeraBridgeAction(_) => None,
        _ => {
            if action.chain_id().is_pera_chain() {
                None
            } else {
                Some(BTreeSet::new())
            }
...
mod tests {
    ...
    use pera_types::committee::VALIDITY_THRESHOLD;
