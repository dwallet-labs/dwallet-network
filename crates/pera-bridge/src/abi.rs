...
use crate::types::{
    ...
    BridgeActionType, EmergencyAction, EthLog, EthToPeraBridgeAction, EvmContractUpgradeAction,
    LimitUpdateAction, PeraToEthBridgeAction,
};
...
#[rustfmt::skip]
gen_eth_events!(
    EthPeraBridge, EthPeraBridgeEvents, "abi/pera_bridge.json",
...
impl EthBridgeEvent {
    pub fn try_into_bridge_action(
        ...
        Ok(match self {
            EthBridgeEvent::EthPeraBridgeEvents(event) => {
                match event {
                    EthPeraBridgeEvents::TokensDepositedFilter(event) => {
                        let bridge_event = match EthToPeraTokenBridgeV1::try_from(&event) {
                            Ok(bridge_event) => {
                                if bridge_event.pera_adjusted_amount == 0 {
                            ...
                            Err(e) => {
                                return Err(BridgeError::Generic(format!("Manual intervention is required. Failed to convert TokensDepositedFilter log to EthToPeraTokenBridgeV1. This indicates incorrect parameters or a bug in the code: {:?}. Err: {:?}", event, e)));
                            }
                        };

                        Some(BridgeAction::EthToPeraBridgeAction(EthToPeraBridgeAction {
                            eth_tx_hash,
                            eth_event_index,
                            eth_bridge_event: bridge_event,
                        }))
                    }
                    EthPeraBridgeEvents::TokensClaimedFilter(_event) => None,
                    EthPeraBridgeEvents::PausedFilter(_event) => None,
                    EthPeraBridgeEvents::UnpausedFilter(_event) => None,
                    EthPeraBridgeEvents::UpgradedFilter(_event) => None,
                    EthPeraBridgeEvents::InitializedFilter(_event) => None,
                }
            }
...
pub struct EthToPeraTokenBridgeV1 {
    pub nonce: u64,
    ...
    pub token_id: u8,
    pub pera_adjusted_amount: u64,
}

impl TryFrom<&TokensDepositedFilter> for EthToPeraTokenBridgeV1 {
...
impl From<PeraToEthBridgeAction> for eth_pera_bridge::Message {
    fn from(action: PeraToEthBridgeAction) -> Self {
        eth_pera_bridge::Message {
            message_type: BridgeActionType::TokenTransfer as u8,
            version: TOKEN_TRANSFER_MESSAGE_VERSION,
            nonce: action.pera_bridge_event.nonce,
            chain_id: action.pera_bridge_event.pera_chain_id as u8,
            payload: action.as_payload_bytes().into(),
        }
    }
}

impl From<ParsedTokenTransferMessage> for eth_pera_bridge::Message {
...
impl From<EmergencyAction> for eth_pera_bridge::Message {
...
mod tests {
    ...
    fn test_eth_message_conversion_emergency_action_regression() -> anyhow::Result<()> {
        ...
        let message: eth_pera_bridge::Message = action.into();
        assert_eq!(
            message,
            eth_pera_bridge::Message {
                message_type: BridgeActionType::EmergencyButton as u8,
    ...
    fn test_eth_message_conversion_update_limit_action_regression() -> anyhow::Result<()> {
        ...
        let action = LimitUpdateAction {
            nonce: 2,
            chain_id: BridgeChainId::EthSepolia,
            sending_chain_id: BridgeChainId::PeraTestnet,
    ...
    #[test]
    fn test_token_deposit_eth_log_to_pera_bridge_event_regression() -> anyhow::Result<()> {
        telemetry_subscribers::init_for_testing();
        ...
        assert_eq!(
            event,
            EthBridgeEvent::EthPeraBridgeEvents(EthPeraBridgeEvents::TokensDepositedFilter(
                TokensDepositedFilter {
                    source_chain_id: 12,
                    nonce: 0,
                    destination_chain_id: 2,
                    token_id: 2,
                    pera_adjusted_amount: 4200000000,
    ...
    #[test]
    fn test_0_pera_amount_conversion_for_eth_event() {
        let e = EthBridgeEvent::EthPeraBridgeEvents(EthPeraBridgeEvents::TokensDepositedFilter(
            TokensDepositedFilter {
                source_chain_id: BridgeChainId::EthSepolia as u8,
                nonce: 0,
                destination_chain_id: BridgeChainId::PeraTestnet as u8,
                token_id: 2,
                pera_adjusted_amount: 1,
                sender_address: EthAddress::random(),
                recipient_address: ethers::types::Bytes::from(
                    PeraAddress::random_for_testing_only().to_vec(),
                ),
            },
        ));
        assert!(e
