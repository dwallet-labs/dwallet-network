...
use crate::abi::{eth_pera_bridge, EthBridgeEvent, EthERC20, EthPeraBridge};
...
use crate::events::{
    PeraBridgeEvent, PeraToEthTokenBridgeV1, TokenTransferApproved, TokenTransferClaimed,
};
use crate::pera_client::PeraBridgeClient;
use crate::pera_transaction_builder::build_add_tokens_on_pera_transaction;
use crate::types::{AddTokensOnEvmAction, BridgeAction, BridgeActionStatus, PeraToEthBridgeAction};
use crate::utils::publish_and_register_coins_return_add_coins_on_pera_action;
use crate::utils::EthSigner;
use eth_pera_bridge::EthPeraBridgeEvents;
use ethers::prelude::*;
...
use pera_types::bridge::{BridgeChainId, BridgeTokenMetadata, BRIDGE_MODULE_NAME, TOKEN_ID_ETH};
...
async fn test_bridge_from_eth_to_pera_to_eth() {
    telemetry_subscribers::init_for_testing();

    let eth_chain_id = BridgeChainId::EthCustom as u8;
    let pera_chain_id = BridgeChainId::PeraCustom as u8;
    ...
    let amount = 42;
    let pera_amount = amount * 100_000_000;

    initiate_bridge_eth_to_pera(&bridge_test_cluster, amount, 0)
        .await
        .unwrap();
    ...
    let eth_coin = bridge_test_cluster
        .pera_client()
        .coin_read_api()
        .get_all_coins(pera_address, None, None)
        .await
        .unwrap()
        .data
        .iter()
        ...
        .clone();
    assert_eq!(eth_coin.balance, pera_amount);
    info!("Eth to pera bridge transfer finished");
    ...
    let events = bridge_test_cluster
        .new_bridge_events(
            HashSet::from_iter([
                PeraToEthTokenBridgeV1.get().unwrap().clone(),
                TokenTransferApproved.get().unwrap().clone(),
                TokenTransferClaimed.get().unwrap().clone(),
            ]),
            true,
    ...
    let parsed_msg = bridge_test_cluster
        .bridge_client()
        .get_parsed_token_transfer_message(pera_chain_id, nonce)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(parsed_msg.source_chain as u8, pera_chain_id);
    ...
    let call = eth_pera_bridge.transfer_bridged_tokens_with_signatures(signatures, message);
    let eth_claim_tx_receipt = send_eth_tx_and_get_tx_receipt(call).await;
    assert_eq!(eth_claim_tx_receipt.status.unwrap().as_u64(), 1);
    info!("Pera to Eth bridge transfer claimed");
...
// Test add new coins on both Pera and Eth
// Also test bridge ndoe handling `NewTokenEvent``
#[tokio::test(flavor = "multi_thread", worker_threads = 8)]
async fn test_add_new_coins_on_pera_and_eth() {
    telemetry_subscribers::init_for_testing();
    let mut bridge_test_cluster = BridgeTestClusterBuilder::new()
        .with_eth_env(true)
    ...
    // Register tokens on Pera
    let token_id = 5;
    let token_pera_decimal = 9; // this needs to match ka.move
    let token_price = 10000;
    let sender = bridge_test_cluster.pera_user_address();
    info!("Published new token");
    let pera_action = publish_and_register_coins_return_add_coins_on_pera_action(
    ...
    let eth_action = BridgeAction::AddTokensOnEvmAction(AddTokensOnEvmAction {
        ...
        token_pera_decimals: vec![token_pera_decimal],
    ...
    bridge_test_cluster.set_approved_governance_actions_for_next_start(vec![
        vec![pera_action.clone(), eth_action.clone()],
        vec![pera_action.clone()],
    ...
    let certified_pera_action = agg
        .request_committee_signatures(pera_action)
        .await
        .expect("Failed to request committee signatures for AddTokensOnPeraAction");
    ...
    assert_eq!(effects.status(), &PeraExecutionStatus::Success);
    assert!(response.events.unwrap().data.iter().any(|e| {
        let pera_bridge_event = PeraBridgeEvent::try_from_pera_event(e).unwrap().unwrap();
        match pera_bridge_event {
            PeraBridgeEvent::NewTokenEvent(e) => {
    ...
    info!("Approved new token on Pera");
...
pub(crate) async fn deposit_native_eth_to_sol_contract(
    ...
    pera_recipient_address: PeraAddress,
    pera_chain_id: BridgeChainId,
    amount: u64,
) -> ContractCall<EthSigner, ()> {
    let contract = EthPeraBridge::new(contract_address, signer.clone().into());
    let pera_recipient_address = pera_recipient_address.to_vec().into();
    let amount = U256::from(amount) * U256::exp10(18); // 1 ETH
    contract
        .bridge_eth(pera_recipient_address, pera_chain_id as u8)
        .value(amount)
}

async fn deposit_eth_to_pera_package(
    ...
    pera_token_type_tags: &HashMap<u8, TypeTag>,
) -> Result<PeraTransactionBlockResponse, anyhow::Error> {
    ...
    builder.programmable_move_call(
        ...
        vec![pera_token_type_tags.get(&TOKEN_ID_ETH).unwrap().clone()],
...
pub async fn initiate_bridge_erc20_to_pera(
    ...
    let allowance_call = contract.approve(bridge_test_cluster.contracts().pera_bridge, amount);
    ...
    let pera_recipient_address = bridge_test_cluster.pera_user_address();
    let pera_chain_id = bridge_test_cluster.pera_chain_id();
    ...
    let contract = EthPeraBridge::new(
        bridge_test_cluster.contracts().pera_bridge,
        eth_signer.clone().into(),
    );
    let deposit_call = contract.bridge_erc20(
        token_id,
        amount,
        pera_recipient_address.to_vec().into(),
        pera_chain_id as u8,
    );
    ...
    let eth_bridge_event = tx_receipt
        ...
        .unwrap();
    let EthBridgeEvent::EthPeraBridgeEvents(EthPeraBridgeEvents::TokensDepositedFilter(
        eth_bridge_event,
    )) = eth_bridge_event
    else {
        unreachable!();
    ...
    assert_eq!(eth_bridge_event.destination_chain_id, pera_chain_id as u8);
    assert_eq!(eth_bridge_event.token_id, token_id);
    assert_eq!(eth_bridge_event.pera_adjusted_amount, pera_amount);
    assert_eq!(eth_bridge_event.sender_address, eth_address);
    assert_eq!(
        eth_bridge_event.recipient_address,
        pera_recipient_address.to_vec()
    );
    info!(
    ...
    .tap_ok(|_| {
        info!(
            nonce,
            token_id, amount_u64, "Eth to Pera bridge transfer claimed"
        );
    })
}

pub async fn initiate_bridge_eth_to_pera(
    bridge_test_cluster: &BridgeTestCluster,
    amount: u64,
    nonce: u64,
) -> Result<(), anyhow::Error> {
    ...
    let pera_amount = (U256::from(amount) * U256::exp10(8)).as_u64(); // DP for Ether on Pera
    ...
    assert_eq!(eth_bridge_event.pera_adjusted_amount, pera_amount);
    assert_eq!(eth_bridge_event.sender_address, eth_address);
    assert_eq!(eth_bridge_event.recipient_address, pera_address.to_vec());
    ...
    .await
    .tap_ok(|_| {
        info!("Eth to Pera bridge transfer claimed");
    })
}

pub async fn initiate_bridge_pera_to_eth(
    bridge_test_cluster: &BridgeTestCluster,
    eth_address: EthAddress,
    token: ObjectRef,
    nonce: u64,
    pera_amount: u64,
) -> Result<PeraToEthBridgeAction, anyhow::Error> {
    let bridge_object_arg = bridge_test_cluster
        .bridge_client()
        .get_mutable_bridge_object_arg_must_succeed()
        .await;
    let pera_client = bridge_test_cluster.pera_client();
    ...
    let resp = match deposit_eth_to_pera_package(
        pera_client,
        pera_address,
        bridge_test_cluster.wallet(),
    ...
    .await
    {
        Ok(resp) => {
            if !resp.status_ok().unwrap() {
                return Err(anyhow!("Pera TX error"));
            } else {
                resp
            }
        }
    ...
    let pera_events = resp.events.unwrap().data;
    let bridge_event = pera_events
        .iter()
        .filter_map(|e| {
            let pera_bridge_event = PeraBridgeEvent::try_from_pera_event(e).unwrap()?;
            pera_bridge_event.try_into_bridge_action(e.id.tx_digest, e.id.event_seq as u16)
        })
        .find_map(|e| {
            if let BridgeAction::PeraToEthBridgeAction(a) = e {
                Some(a)
            } else {
                None
            }
    ...
    assert_eq!(bridge_event.pera_bridge_event.nonce, nonce);
    assert_eq!(
        bridge_event.pera_bridge_event.pera_chain_id,
        bridge_test_cluster.pera_chain_id()
    );
    assert_eq!(
        bridge_event.pera_bridge_event.eth_chain_id,
        bridge_test_cluster.eth_chain_id()
    );
    assert_eq!(bridge_event.pera_bridge_event.pera_address, pera_address);
    assert_eq!(bridge_event.pera_bridge_event.eth_address, eth_address);
    assert_eq!(bridge_event.pera_bridge_event.token_id, TOKEN_ID_ETH);
    assert_eq!(
        bridge_event.pera_bridge_event.amount_pera_adjusted,
        pera_amount
    );
    ...
    .await
    .unwrap();
    info!("Pera to Eth bridge transfer approved.");

    Ok(bridge_event)
}

async fn wait_for_transfer_action_status(
    pera_bridge_client: &PeraBridgeClient,
