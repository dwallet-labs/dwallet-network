...
use crate::config::{BridgeNodeConfig, EthConfig, PeraConfig};
...
const PERA_BRIDGE_NAME: &str = "PeraBridge";
...
pub struct BridgeTestCluster {
    ...
    bridge_client: PeraBridgeClient,
...
impl BridgeTestClusterBuilder {
    ...
    pub fn with_pera_chain_id(mut self, chain_id: BridgeChainId) -> Self {
        self.pera_chain_id = chain_id;
        self
    }
    ...
    pub async fn build(self) -> BridgeTestCluster {
        ...
        let bridge_client = PeraBridgeClient::new(&test_cluster.fullnode_handle.rpc_url)
            .await
            .unwrap();
        BridgeTestCluster {
            ...
            pera_chain_id: self.pera_chain_id,
...
impl BridgeTestCluster {
    ...
    pub fn bridge_client(&self) -> &PeraBridgeClient {
        &self.bridge_client
    }

    pub fn pera_client(&self) -> &PeraClient {
        &self.test_cluster.fullnode_handle.pera_client
    }

    pub fn pera_user_address(&self) -> PeraAddress {
        self.test_cluster.get_address_0()
    }

    pub fn pera_chain_id(&self) -> BridgeChainId {
        self.pera_chain_id
    }
    ...
    pub fn pera_bridge_address(&self) -> String {
        self.eth_environment.contracts().pera_bridge_addrress_hex()
    }
    ...
    pub fn pera_rpc_url(&self) -> String {
    ...
    pub async fn new_bridge_transactions(
        &mut self,
        assert_success: bool,
    ) -> Vec<PeraTransactionBlockResponse> {
        let resps = self
            .pera_client()
            .read_api()
...
impl DeployedSolContracts {
    ...
    pub fn pera_bridge_addrress_hex(&self) -> String {
        Self::eth_adress_to_hex(self.pera_bridge)
    }
}
...
pub(crate) async fn deploy_sol_contract(
    ...
    let contracts = DeployedSolContracts {
        pera_bridge: deployed_contracts.remove(PERA_BRIDGE_NAME).unwrap(),
...
impl EthBridgeEnvironment {
    ...
    pub(crate) async fn get_supported_token(&self, token_id: u8) -> (EthAddress, u8, u64) {
        ...
        let token_pera_decimal = config.token_pera_decimal_of(token_id).call().await.unwrap();
        let token_price = config.token_price_of(token_id).call().await.unwrap();
        (token_address, token_pera_decimal, token_price)
    }
}
...
pub(crate) async fn start_bridge_cluster(
        ...
        .enumerate()
    {
        ...
        let config = BridgeNodeConfig {
            ...
            pera: PeraConfig {
                pera_rpc_url: test_cluster.fullnode_handle.rpc_url.clone(),
...
pub(crate) async fn get_signatures(
    ...
    let sigs = pera_bridge_client
        .get_token_transfer_action_onchain_signatures_until_success(pera_chain_id, nonce)
        .await
        .unwrap();
