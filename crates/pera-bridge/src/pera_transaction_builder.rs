...
pub fn build_pera_transaction(
    client_address: PeraAddress,
    ...
    // TODO: Check chain id?
    match action.data() {
        BridgeAction::EthToPeraBridgeAction(_) => build_token_bridge_approve_transaction(
            client_address,
            gas_object_ref,
            action,
            true,
        ...
        BridgeAction::PeraToEthBridgeAction(_) => build_token_bridge_approve_transaction(
            client_address,
            gas_object_ref,
            action,
            false,
        ...
        BridgeAction::EvmContractUpgradeAction(_) => {
            // It does not need a Pera tranaction to execute EVM contract upgrade
            unreachable!()
        }
        BridgeAction::AddTokensOnPeraAction(_) => build_add_tokens_on_pera_transaction(
            client_address,
            gas_object_ref,
            action,
            bridge_object_arg,
        ...
        BridgeAction::AddTokensOnEvmAction(_) => {
            // It does not need a Pera tranaction to add tokens on EVM
            unreachable!()
        }
    }
}
...
fn build_token_bridge_approve_transaction(
    ...
    let (source_chain, seq_num, sender, target_chain, target, token_type, amount) =
        match bridge_action {
            BridgeAction::PeraToEthBridgeAction(a) => {
                let bridge_event = a.pera_bridge_event;
                (
                    bridge_event.pera_chain_id,
                    bridge_event.nonce,
                    bridge_event.pera_address.to_vec(),
                    bridge_event.eth_chain_id,
                    bridge_event.eth_address.to_fixed_bytes().to_vec(),
                    bridge_event.token_id,
                    bridge_event.amount_pera_adjusted,
                )
            }
            BridgeAction::EthToPeraBridgeAction(a) => {
                ...
                (
                    ...
                    bridge_event.pera_address.to_vec(),
                    bridge_event.token_id,
                    bridge_event.pera_adjusted_amount,
                )
            }
            _ => unreachable!(),
        };
    ...
    builder.programmable_move_call(
        BRIDGE_PACKAGE_ID,
        pera_types::bridge::BRIDGE_MODULE_NAME.to_owned(),
...
pub fn build_add_tokens_on_pera_transaction(
    client_address: PeraAddress,
    gas_object_ref: &ObjectRef,
    action: VerifiedCertifiedBridgeAction,
    bridge_object_arg: ObjectArg,
    ...
    let (source_chain, seq_num, native, token_ids, token_type_names, token_prices) =
        match bridge_action {
            BridgeAction::AddTokensOnPeraAction(a) => (
                a.chain_id,
                a.nonce,
                a.native,
                a.token_ids,
    ...
    let message_arg = builder.programmable_move_call(
        ...
        BRIDGE_CREATE_ADD_TOKEN_ON_PERA_MESSAGE_FUNCTION_NAME.into(),
        vec![],
        vec![
            source_chain,
            seq_num,
...
pub fn build_committee_register_transaction(
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    let system_state = builder.obj(ObjectArg::PERA_SYSTEM_MUT).unwrap();
...
mod tests {
    ...
    async fn test_build_pera_transaction_for_token_transfer() {
        ...
        // Note: We don't call `pera_client.get_bridge_committee` here because it will err if the committee
        ...
        // 2. Test Pera -> Eth Transfer approval
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_pera_transaction_for_emergency_op() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let summary = pera_client.get_bridge_summary().await.unwrap();
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_pera_transaction_for_committee_blocklist() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let committee = pera_client.get_bridge_summary().await.unwrap().committee;
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_pera_transaction_for_limit_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let transfer_limit = pera_client
            .get_bridge_summary()
            .await
            .unwrap()
            .limiter
        ...
        .await;
        let new_transfer_limit = pera_client
            .get_bridge_summary()
            .await
            .unwrap()
            .limiter
        ...
        for limit in new_transfer_limit {
            if limit.0 == BridgeChainId::EthCustom && limit.1 == BridgeChainId::PeraCustom {
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 8)]
    async fn test_build_pera_transaction_for_price_update() {
        telemetry_subscribers::init_for_testing();
        let mut bridge_keys = vec![];
        for _ in 0..=3 {
            let (_, kp): (_, BridgeAuthorityKeyPair) = get_key_pair();
        ...
        test_cluster
            .trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized()
            .await;
        let notional_values = pera_client.get_notional_values().await.unwrap();
        ...
        .await;
        let new_notional_values = pera_client.get_notional_values().await.unwrap();
