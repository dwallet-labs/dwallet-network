...
pub struct BridgeOrchestratorTables {
    ...
    pub(crate) pera_syncer_cursors: DBMap<Identifier, EventID>,
...
impl BridgeOrchestratorTables {
    ...
    pub(crate) fn update_pera_event_cursor(
        &self,
        module: Identifier,
        cursor: EventID,
    ) -> BridgeResult<()> {
        let mut batch = self.pera_syncer_cursors.batch();

        batch
            .insert_batch(&self.pera_syncer_cursors, [(module, cursor)])
            .map_err(|e| {
                BridgeError::StorageError(format!(
                    "Coudln't insert into pera_syncer_cursors: {:?}",
                    e
                ))
            })?;
        batch
...
mod tests {
    ...
    async fn test_bridge_storage_basic() {
        ...
        // update pera event cursor
        let pera_module = Identifier::from_str("test").unwrap();
        let pera_cursor = EventID {
