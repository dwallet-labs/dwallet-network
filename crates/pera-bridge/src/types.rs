...
pub struct PeraToEthBridgeAction {
    // Digest of the transaction where the event was emitted
    pub pera_tx_digest: TransactionDigest,
    // The index of the event in the transaction
    pub pera_tx_event_index: u16,
    pub pera_bridge_event: EmittedPeraToEthTokenBridgeV1,
}

#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct EthToPeraBridgeAction {
    // Digest of the transaction where the event was emitted
    pub eth_tx_hash: EthTransactionHash,
    // The index of the event in the transaction
    pub eth_event_index: u16,
    pub eth_bridge_event: EthToPeraTokenBridgeV1,
}
...
pub struct LimitUpdateAction {
    ...
    // for the limit update. For example, if chain_id is EthMainnet and sending_chain_id is PeraMainnet,
    // it means we want to update the limit for the PeraMainnet to EthMainnet route.
    pub chain_id: BridgeChainId,
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct AddTokensOnPeraAction {
    pub nonce: u64,
    pub chain_id: BridgeChainId,
    pub native: bool,
...
pub struct AddTokensOnEvmAction {
    ...
    pub token_pera_decimals: Vec<u8>,
    pub token_prices: Vec<u64>,
}
...
pub enum BridgeAction {
    /// Pera to Eth bridge action
    PeraToEthBridgeAction(PeraToEthBridgeAction),
    /// Eth to pera bridge action
    EthToPeraBridgeAction(EthToPeraBridgeAction),
    ...
    AddTokensOnPeraAction(AddTokensOnPeraAction),
...
impl BridgeAction {
    ...
    pub fn chain_id(&self) -> BridgeChainId {
        match self {
            BridgeAction::PeraToEthBridgeAction(a) => a.pera_bridge_event.pera_chain_id,
            BridgeAction::EthToPeraBridgeAction(a) => a.eth_bridge_event.eth_chain_id,
            ...
            BridgeAction::AddTokensOnPeraAction(a) => a.chain_id,
    ...
    pub fn is_governace_action(&self) -> bool {
        match self.action_type() {
            ...
            BridgeActionType::AddTokensOnPera => true,
    ...
    pub fn action_type(&self) -> BridgeActionType {
        match self {
            BridgeAction::PeraToEthBridgeAction(_) => BridgeActionType::TokenTransfer,
            BridgeAction::EthToPeraBridgeAction(_) => BridgeActionType::TokenTransfer,
            ...
            BridgeAction::AddTokensOnPeraAction(_) => BridgeActionType::AddTokensOnPera,
    ...
    pub fn seq_number(&self) -> u64 {
        match self {
            BridgeAction::PeraToEthBridgeAction(a) => a.pera_bridge_event.nonce,
            BridgeAction::EthToPeraBridgeAction(a) => a.eth_bridge_event.nonce,
            ...
            BridgeAction::AddTokensOnPeraAction(a) => a.nonce,
    ...
    pub fn approval_threshold(&self) -> u64 {
        match self {
            BridgeAction::PeraToEthBridgeAction(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            BridgeAction::EthToPeraBridgeAction(_) => APPROVAL_THRESHOLD_TOKEN_TRANSFER,
            ...
            BridgeAction::AddTokensOnPeraAction(_) => APPROVAL_THRESHOLD_ADD_TOKENS_ON_PERA,
...
pub fn is_route_valid(one: BridgeChainId, other: BridgeChainId) -> bool {
    if one.is_pera_chain() && other.is_pera_chain() {
        return false;
    }
    if !one.is_pera_chain() && !other.is_pera_chain() {
        return false;
    ...
    if one == BridgeChainId::EthMainnet {
        return other == BridgeChainId::PeraMainnet;
    }
    if one == BridgeChainId::PeraMainnet {
        return other == BridgeChainId::EthMainnet;
    }
    if other == BridgeChainId::EthMainnet {
        return one == BridgeChainId::PeraMainnet;
    }
    if other == BridgeChainId::PeraMainnet {
...
mod tests {
    ...
    use crate::test_utils::get_test_eth_to_pera_bridge_action;
    ...
    fn test_bridge_action_approval_threshold_regression_test() -> anyhow::Result<()> {
        let action = get_test_pera_to_eth_bridge_action(None, None, None, None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);

        let action = get_test_eth_to_pera_bridge_action(None, None, None, None);
        assert_eq!(action.approval_threshold(), 3334);
