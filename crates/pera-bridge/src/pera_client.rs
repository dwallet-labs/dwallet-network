...
use pera_json_rpc_types::{
    EventPage, PeraObjectDataOptions, PeraTransactionBlockResponse,
...
use pera_types::bridge::BridgeTreasurySummary;
use pera_types::bridge::MoveTypeCommitteeMember;
use pera_types::bridge::MoveTypeParsedTokenTransferMessage;
...
use pera_types::transaction::ProgrammableMoveCall;
...
pub struct PeraClient<P> {
    inner: P,
}

pub type PeraBridgeClient = PeraClient<PeraSdkClient>;

impl PeraBridgeClient {
    pub async fn new(rpc_url: &str) -> anyhow::Result<Self> {
        let inner = PeraClientBuilder::default()
            .build(rpc_url)
            .await
            .map_err(|e| {
                anyhow!("Can't establish connection with Pera Rpc {rpc_url}. Error: {e}")
            })?;
    ...
    pub fn pera_client(&self) -> &PeraSdkClient {
        &self.inner
    }
}
    ...
    P: PeraClientInner,
{
    ...
    async fn describe(&self) -> anyhow::Result<()> {
        ...
        tracing::info!(
            "PeraClient is connected to chain {chain_id}, current block number: {block_number}"
        );
        Ok(())
    }
    ...
    ) -> BridgeResult<Page<PeraEvent, EventID>> {
    ...
    /// Returns BridgeAction from a Pera Transaction with transaction hash
    ...
    pub async fn get_bridge_action_by_tx_digest_and_event_idx_maybe(
        ...
        if event.type_.address.as_ref() != BRIDGE_PACKAGE_ID.as_ref() {
            return Err(BridgeError::BridgeEventInUnrecognizedPeraPackage);
        }
        let bridge_event = PeraBridgeEvent::try_from_pera_event(event)?
            .ok_or(BridgeError::NoBridgeEventsInTxPosition)?;
    ...
    pub async fn get_token_id_map(&self) -> BridgeResult<HashMap<u8, TypeTag>> {
        self.get_bridge_summary()
            ...
            .into_iter()
            .map(|(id, name)| {
                parse_pera_type_tag(&format!("0x{name}"))
                    .map(|name| (id, name))
                    .map_err(|e| {
        ...
        for (_, member) in move_type_bridge_committee.members {
            ...
            let base_url = from_utf8(&http_rest_url).unwrap_or_else(|_e| {
                warn!(
                    ...
                    pera_address, bridge_pubkey_bytes, http_rest_url
                );
                ""
            });
    ...
    pub async fn execute_transaction_block_with_effects(
        &self,
        tx: pera_types::transaction::Transaction,
    ) -> BridgeResult<PeraTransactionBlockResponse> {
...
/// Use a trait to abstract over the PeraSDKClient and PeraMockClient for testing.
#[async_trait]
pub trait PeraClientInner: Send + Sync {
...
#[async_trait]
impl PeraClientInner for PeraSdkClient {
    ...
    async fn execute_transaction_block_with_effects(
        ...
        match self.quorum_driver_api().execute_transaction_block(
            tx,
            PeraTransactionBlockResponseOptions::new().with_effects().with_events(),
            Some(pera_types::quorum_driver_types::ExecuteTransactionRequestType::WaitForEffectsCert),
        ).await {
            Ok(response) => Ok(response),
            Err(e) => return Err(BridgeError::PeraTxFailureGeneric(e.to_string())),
        }
    }
        ...
        loop {
            match self
                ...
                .get_object_with_options(
                    gas_object_id,
                    PeraObjectDataOptions::default().with_owner().with_content(),
                )
                .await
                .map(|resp| resp.data)
            {
...
mod tests {
    ...
    use super::*;
    use crate::events::{init_all_struct_tags, PeraToEthTokenBridgeV1};
    ...
    async fn get_bridge_action_by_tx_digest_and_event_idx_maybe() {
        ...
        let sanitized_event_1 = EmittedPeraToEthTokenBridgeV1 {
            nonce: 1,
        ...
        let emitted_event_1 = MoveTokenDepositedEvent {
            ...
            source_chain: sanitized_event_1.pera_chain_id as u8,
            sender_address: sanitized_event_1.pera_address.to_vec(),
            ...
            amount_pera_adjusted: sanitized_event_1.amount_pera_adjusted,
        };

        let mut pera_event_1 = PeraEvent::random_for_testing();
        pera_event_1.type_ = PeraToEthTokenBridgeV1.get().unwrap().clone();
        pera_event_1.bcs = bcs::to_bytes(&emitted_event_1).unwrap();
        ...
        let mut pera_event_2 = PeraEvent::random_for_testing();
        pera_event_2.type_ = PeraToEthTokenBridgeV1.get().unwrap().clone();
        pera_event_2.type_.module = Identifier::from_str("unrecognized_module").unwrap();
        pera_event_2.bcs = bcs::to_bytes(&event_2).unwrap();

        // Event 3 is defined in non-bridge package
        let mut pera_event_3 = pera_event_1.clone();
        pera_event_3.type_.address = AccountAddress::random();
        ...
        let expected_action_1 = BridgeAction::PeraToEthBridgeAction(PeraToEthBridgeAction {
            pera_tx_digest: tx_digest,
            pera_tx_event_index: 0,
            pera_bridge_event: sanitized_event_1.clone(),
        });
        assert_eq!(
            pera_client
        ...
        let expected_action_2 = BridgeAction::PeraToEthBridgeAction(PeraToEthBridgeAction {
            pera_tx_digest: tx_digest,
            pera_tx_event_index: 2,
            pera_bridge_event: sanitized_event_1.clone(),
        });
        assert_eq!(
            pera_client
        ...
        assert!(matches!(
            pera_client
                ...
                .await
                .unwrap_err(),
            BridgeError::BridgeEventInUnrecognizedPeraPackage
        ),);
        assert!(matches!(
            pera_client
        ...
        pera_event_2.type_ = PeraToEthTokenBridgeV1.get().unwrap().clone();
        mock_client.add_events_by_tx_digest(tx_digest, vec![pera_event_2]);
        pera_client
            .get_bridge_action_by_tx_digest_and_event_idx_maybe(&tx_digest, 2)
            .await
            .unwrap_err();
    ...
    async fn test_get_action_onchain_status_for_pera_to_eth_transfer() {
        telemetry_subscribers::init_for_testing();
        ...
        let pera_client = PeraClient::new(&test_cluster.fullnode_handle.rpc_url)
            .await
            .unwrap();
        ...
        // 2. Create a Pera -> Eth Transfer, approve with validator secrets and assert its status to be Approved
        ...
        assert_eq!(bridge_event.pera_chain_id, BridgeChainId::PeraCustom);
        assert_eq!(bridge_event.eth_chain_id, BridgeChainId::EthCustom);
        assert_eq!(bridge_event.eth_address, eth_recv_address);
        assert_eq!(bridge_event.pera_address, sender);
        assert_eq!(bridge_event.token_id, TOKEN_ID_USDC);
        assert_eq!(bridge_event.amount_pera_adjusted, usdc_amount);

        let action = get_test_pera_to_eth_bridge_action(
            None,
            None,
            Some(bridge_event.nonce),
            Some(bridge_event.amount_pera_adjusted),
            Some(bridge_event.pera_address),
        ...
        let action =
            get_test_pera_to_eth_bridge_action(None, None, Some(100), None, None, None, None);
        let status = pera_client
            .inner
            .get_token_transfer_action_onchain_status(
                bridge_object_arg,
