...
use ika_package_management::{resolve_published_id, PublishedAtError};
...
use ika_types::{
    ...
    BRIDGE_ADDRESS, DEEPBOOK_ADDRESS, MOVE_STDLIB_ADDRESS, IKA_FRAMEWORK_ADDRESS,
    IKA_SYSTEM_ADDRESS,
};
use ika_verifier::verifier as ika_bytecode_verifier;

#[cfg(test)]
#[path = "unit_tests/build_tests.rs"]
mod build_tests;

/// Wrapper around the core Move `CompiledPackage` with some Ika-specific traits and info
#[derive(Debug, Clone)]
...
/// Wrapper around the core Move `BuildConfig` with some Ika-specific info
#[derive(Clone)]
...
impl BuildConfig {
    ...
    /// If we are building the Ika framework, we skip the check that the addresses should be 0
    pub fn build(self, path: &Path) -> IkaResult<CompiledPackage> {
    ...
    pub fn resolution_graph(
    ...
    ) -> IkaResult<ResolvedGraph> {
        if let Some(err_msg) = set_ika_flavor(&mut self.config) {
            return Err(IkaError::ModuleBuildFailure { error: err_msg });
        }
        ...
        .map_err(|err| IkaError::ModuleBuildFailure {
            error: format!("{:?}", err),
        })
    }
}
...
pub fn decorate_warnings(warning_diags: Diagnostics, files: Option<&MappedFiles>) {
    ...
    if any_linter_warnings {
        eprintln!("Please report feedback on the linter warnings at https://forums.ika.io\n");
    }
...
/// Sets build config's default flavor to `Flavor::Ika`. Returns error message if the flavor was
/// previously set to something else than `Flavor::Ika`.
pub fn set_ika_flavor(build_config: &mut MoveBuildConfig) -> Option<String> {
    use move_compiler::editions::Flavor;

    let flavor = build_config.default_flavor.get_or_insert(Flavor::Ika);
    if flavor != &Flavor::Ika {
        return Some(format!(
...
pub fn build_from_resolution_graph(
    ...
    for (name, pkg) in resolution_graph.package_table.iter() {
        ...
        let modules =
            pkg.get_bytecodes_bytes()
                .map_err(|error| IkaError::ModuleDeserializationFailure {
                    error: format!(
        ...
        for module in modules {
            let module =
                CompiledModule::deserialize_with_defaults(module.as_ref()).map_err(|error| {
                    IkaError::ModuleDeserializationFailure {
                        error: format!(
                            "Deserializing bytecode dependency for package {}: {:?}",
                            name, error
                        ),
    ...
    let (package, fn_info) = match result {
        Err(error) => {
            return Err(IkaError::ModuleBuildFailure {
                error: format!("{:?}", error),
            })
        }
    ...
    if run_bytecode_verifier {
        ...
        for m in compiled_modules.iter_modules() {
            move_bytecode_verifier::verify_module_unmetered(m).map_err(|err| {
                IkaError::ModuleVerificationFailure {
                    error: err.to_string(),
                }
            })?;
            ika_bytecode_verifier::ika_verify_module_unmetered(m, &fn_info, &verifier_config)?;
        }
...
impl CompiledPackage {
    ...
    /// Note: these are not topologically sorted by dependency--use `get_dependency_sorted_modules` to produce a list of modules ikatable
    ...
    pub fn get_dependency_sorted_modules(
        ...
        if with_unpublished_deps {
            ...
            // sufficient as we may have modules with the same names in user code and in Ika
    ...
    /// Get bytecode modules from the Ika System that are used by this package
    pub fn get_ika_system_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == IKA_SYSTEM_ADDRESS)
    }

    /// Get bytecode modules from the Ika Framework that are used by this package
    pub fn get_ika_framework_modules(&self) -> impl Iterator<Item = &CompiledModule> {
        self.get_modules_and_deps()
            .filter(|m| *m.self_id().address() == IKA_FRAMEWORK_ADDRESS)
    }
    ...
    pub fn verify_unpublished_dependencies(
        ...
        Err(IkaError::ModulePublishFailure {
...
impl Default for BuildConfig {
    fn default() -> Self {
        let config = MoveBuildConfig {
            default_flavor: Some(move_compiler::editions::Flavor::Ika),
...
pub struct IkaPackageHooks;

impl PackageHooks for IkaPackageHooks {
...
pub fn check_unpublished_dependencies(unpublished: &BTreeSet<Symbol>) -> Result<(), IkaError> {
...
pub fn check_invalid_dependencies(invalid: &BTreeMap<Symbol, String>) -> Result<(), IkaError> {
    if invalid.is_empty() {
        return Ok(());
    }
