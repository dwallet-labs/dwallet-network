...
use ika_execution::{self, Executor};
...
use ika_types::base_types::{
    ExecutionDigests, ObjectID, SequenceNumber, IkaAddress, TransactionDigest, TxContext,
};
use ika_types::bridge::{BridgeChainId, BRIDGE_CREATE_FUNCTION_NAME, BRIDGE_MODULE_NAME};
use ika_types::committee::Committee;
use ika_types::crypto::{
    AuthorityKeyPair, AuthorityPublicKeyBytes, AuthoritySignInfo, AuthoritySignInfoTrait,
    AuthoritySignature, DefaultHash, IkaAuthoritySignature,
};
use ika_types::deny_list_v1::{DENY_LIST_CREATE_FUNC, DENY_LIST_MODULE};
...
use ika_types::governance::StakedIka;
use ika_types::id::UID;
...
use ika_types::ika_system_state::{get_ika_system_state, IkaSystemState, IkaSystemStateTrait};
use ika_types::transaction::{
    CallArg, CheckedInputObjects, Command, InputObjectKind, ObjectReadResult, Transaction,
};
use ika_types::{BRIDGE_ADDRESS, IKA_BRIDGE_OBJECT_ID, IKA_FRAMEWORK_ADDRESS, IKA_SYSTEM_ADDRESS};
use tracing::trace;
...
impl Builder {
    ...
    pub fn add_validator_signature(mut self, keypair: &AuthorityKeyPair) -> Self {
        ...
        let checkpoint_signature = {
            let intent_msg = IntentMessage::new(
                Intent::ika_app(IntentScope::CheckpointSummary),
                checkpoint.clone(),
            );
        ...
        let token_distribution_schedule =
            if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
                ...
                TokenDistributionSchedule::new_for_validators_with_default_allocation(
                    validators.iter().map(|v| v.info.ika_address()),
                )
            };
    ...
    fn committee(objects: &[Object]) -> Committee {
        let ika_system_object =
            get_ika_system_state(&objects).expect("Ika System State object must always exist");
        ika_system_object
    ...
    fn validate_inputs(&self) -> anyhow::Result<(), anyhow::Error> {
        ...
        if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
            ...
            token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
                self.validators.values().map(|v| v.info.ika_address()),
            );
        }
    ...
    fn validate_output(&self) {
        ...
        let system_state = match unsigned_genesis.ika_system_object() {
            IkaSystemState::V1(inner) => inner,
            IkaSystemState::V2(_) => unreachable!(),
            #[cfg(msim)]
            _ => {
            ...
            .zip(system_state.validators.active_validators.iter())
        {
            ...
            assert!(address_to_pool_id
                .insert(metadata.ika_address, onchain_validator.staking_pool.id)
                .is_none());
            assert_eq!(validator.info.ika_address(), metadata.ika_address);
            assert_eq!(validator.info.protocol_key(), metadata.ika_pubkey_bytes());
        ...
        assert_eq!(
            ...
            token_distribution_schedule.stake_subsidy_fund_nika
        );
        ...
        let mut gas_objects: BTreeMap<ObjectID, (&Object, GasCoin)> = unsigned_genesis
            ...
            .collect();
        let mut staked_ika_objects: BTreeMap<ObjectID, (&Object, StakedIka)> = unsigned_genesis
            .objects()
            .iter()
            .filter_map(|o| StakedIka::try_from(o).ok().map(|s| (o.id(), (o, s))))
            .collect();
        ...
        for allocation in token_distribution_schedule.allocations {
            if let Some(staked_with_validator) = allocation.staked_with_validator {
                ...
                let staked_ika_object_id = staked_ika_objects
                    .iter()
                    .find(|(_k, (o, s))| {
                        ...
                        *owner == allocation.recipient_address
                            && s.principal() == allocation.amount_nika
                            && s.pool_id() == staking_pool_id
                    })
                    .map(|(k, _)| *k)
                    .expect("all allocations should be present");
                let staked_ika_object = staked_ika_objects.remove(&staked_ika_object_id).unwrap();
                assert_eq!(
                    staked_ika_object.0.owner,
                    Owner::AddressOwner(allocation.recipient_address)
                );
                assert_eq!(staked_ika_object.1.principal(), allocation.amount_nika);
                assert_eq!(staked_ika_object.1.pool_id(), staking_pool_id);
                assert_eq!(staked_ika_object.1.activation_epoch(), 0);
            } else {
                let gas_object_id = gas_objects
                    ...
                    .find(|(_k, (o, g))| {
                        if let Owner::AddressOwner(owner) = &o.owner {
                            *owner == allocation.recipient_address
                                && g.value() == allocation.amount_nika
                        } else {
                            false
                        }
                    })
                ...
                assert_eq!(gas_object.1.value(), allocation.amount_nika,);
            }
        }
        ...
        if !self.parameters.allow_insertion_of_extra_objects {
            ...
            assert!(staked_ika_objects.is_empty());
        }
...
fn get_genesis_protocol_config(version: ProtocolVersion) -> ProtocolConfig {
    ...
    // IKA_PROTOCOL_CONFIG_CHAIN_OVERRIDE if necessary
...
fn build_unsigned_genesis_data(
    ...
    token_distribution_schedule.validate();
    token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
        genesis_validators.iter().map(|v| v.ika_address),
    );
    ...
    let mut system_packages =
        ika_framework_snapshot::load_bytecode_snapshot(parameters.protocol_version.as_u64())
...
// on `ika_system` which is mocked in some tests, but would be in the loader
    ...
    let genesis_transaction = {
        let genesis_objects = objects
            ...
            .map(|mut object| {
                ...
                ika_types::transaction::GenesisObject::RawObject {
                    data: object.data,
                    owner: object.owner,
                }
            })
        ...
        ika_types::transaction::VerifiedTransaction::new_genesis_transaction(genesis_objects)
            .into_inner()
    };
...
pub fn generate_genesis_system_object(
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        // Step 1: Create the IkaSystemState UID
        let ika_system_state_uid = builder.programmable_move_call(
            IKA_FRAMEWORK_ADDRESS.into(),
            ident_str!("object").to_owned(),
            ident_str!("ika_system_state").to_owned(),
            vec![],
            vec![],
        );
        ...
        if protocol_config.enable_bridge() {
            let bridge_uid = builder
                .input(CallArg::Pure(UID::new(IKA_BRIDGE_OBJECT_ID).to_bcs_bytes()))
                .unwrap();
            // TODO(bridge): this needs to be passed in as a parameter for next testnet regenesis
            // Hardcoding chain id to IkaCustom
            let bridge_chain_id = builder.pure(BridgeChainId::IkaCustom).unwrap();
        ...
        // Step 4: Mint the supply of IKA.
        let ika_supply = builder.programmable_move_call(
            IKA_FRAMEWORK_ADDRESS.into(),
            ident_str!("ika").to_owned(),
        ...
        // Step 5: Run genesis.
        // The first argument is the system state uid we got from step 1 and the second one is the IKA supply we
        // got from step 3.
        let mut arguments = vec![ika_system_state_uid, ika_supply];
    ...
    {
        let object = written.get_mut(&ika_types::IKA_CLOCK_OBJECT_ID).unwrap();
        object
            .data
            .try_as_move_mut()
            .unwrap()
...
mod test {
    ...
    use ika_config::node::DEFAULT_COMMISSION_RATE;
    use ika_config::node::DEFAULT_VALIDATOR_GAS_PRICE;
