...
use ika_bridge::e2e_tests::test_utils::{
    initiate_bridge_eth_to_ika, BridgeTestCluster, BridgeTestClusterBuilder,
};
use ika_bridge_indexer::config::IndexerConfig;
use ika_bridge_indexer::metrics::BridgeIndexerMetrics;
use ika_bridge_indexer::models::{GovernanceAction, TokenTransfer};
use ika_bridge_indexer::postgres_manager::get_connection_pool;
use ika_bridge_indexer::storage::PgBridgePersistent;
use ika_bridge_indexer::{create_ika_indexer, schema};
use ika_data_ingestion_core::DataIngestionMetrics;
use ika_indexer::database::Connection;
use ika_indexer_builder::indexer_builder::IndexerProgressStore;
...
async fn test_indexing_transfer() {
    ...
    let indexer = create_ika_indexer(pool.clone(), metrics.clone(), ingestion_metrics, &config)
        .await
        .unwrap();
    ...
    // transfer eth to ika
    initiate_bridge_eth_to_ika(&cluster, 1000, 0).await.unwrap();
...
async fn setup_bridge_env(with_eth_env: bool) -> (IndexerConfig, BridgeTestCluster, TempDb) {
    ...
    let config = IndexerConfig {
        remote_store_url: format!("{}/rest", bridge_test_cluster.ika_rpc_url()),
        ...
        concurrency: 10,
        ika_bridge_genesis_checkpoint: 0,
        eth_bridge_genesis_block: 0,
        eth_ika_bridge_contract_address: bridge_test_cluster.ika_bridge_address(),
        metric_port: 9001,
    };
