...
use crate::models::IkaProgressStore;
use crate::schema::governance_actions;
use crate::schema::ika_progress_store::txn_digest;
use crate::schema::{ika_error_transactions, token_transfer_data};
...
const IKA_PROGRESS_STORE_DUMMY_KEY: i32 = 1;
...
pub async fn update_ika_progress_store(
    pool: &PgPool,
    tx_digest: TransactionDigest,
) -> Result<(), anyhow::Error> {
    let mut conn = pool.get().await?;
    diesel::insert_into(schema::ika_progress_store::table)
        .values(&IkaProgressStore {
            id: IKA_PROGRESS_STORE_DUMMY_KEY,
            txn_digest: tx_digest.inner().to_vec(),
        })
        .on_conflict(schema::ika_progress_store::dsl::id)
        .do_update()
...
pub async fn read_ika_progress_store(pool: &PgPool) -> anyhow::Result<Option<TransactionDigest>> {
    let mut conn = pool.get().await?;
    let val: Option<IkaProgressStore> = crate::schema::ika_progress_store::dsl::ika_progress_store
        .select(IkaProgressStore::as_select())
        .first(&mut conn)
        .await
        .optional()?;
    match val {
