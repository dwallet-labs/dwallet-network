...
use ika_bridge::types::{EthEvent, RawEthLog};
use ika_indexer_builder::indexer_builder::{DataMapper, DataSender, Datasource};
...
impl EthSubscriptionDatasource {
    pub async fn new(
        eth_ika_bridge_contract_addresses: Vec<EthAddress>,
        eth_client: Arc<EthClient<MeteredEthHttpProvier>>,
        eth_ws_url: String,
        metrics: Box<dyn IndexerMetricProvider>,
        genesis_block: u64,
        ...
        Ok(Self {
            addresses: eth_ika_bridge_contract_addresses,
            eth_client,
            eth_ws_url,
            metrics,
            genesis_block,
...
impl EthFinalizedSyncDatasource {
    pub async fn new(
        ...
        Ok(Self {
            bridge_addresses: eth_ika_bridge_contract_addresses,
            eth_http_url,
            eth_client,
            metrics,
            bridge_metrics,
...
impl DataMapper<RawEthData, ProcessedTxnData> for EthDataMapper {
    fn map(
        ...
        match bridge_event {
            EthBridgeEvent::EthIkaBridgeEvents(bridge_event) => match &bridge_event {
                EthIkaBridgeEvents::TokensDepositedFilter(bridge_event) => {
                    info!(
                    ...
                    processed_txn_data.push(ProcessedTxnData::TokenTransfer(TokenTransfer {
                        ...
                        data: Some(TokenTransferData {
                            ...
                            amount: bridge_event.ika_adjusted_amount,
                            is_finalized,
                        }),
                    }));
                }
                EthIkaBridgeEvents::TokensClaimedFilter(bridge_event) => {
                    info!(
                ...
                EthIkaBridgeEvents::EmergencyOperationFilter(f) => {
                    info!(
                ...
                EthIkaBridgeEvents::ContractUpgradedFilter(f) => {
                    info!(
                        "Observed Eth IkaBridge Upgrade at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
                ...
                | EthIkaBridgeEvents::PausedFilter(_)
                | EthIkaBridgeEvents::UnpausedFilter(_)
            ...
            EthBridgeEvent::EthBridgeConfigEvents(bridge_event) => match &bridge_event {
                ...
                EthBridgeConfigEvents::TokenAddedFilter(_) => {
                    info!(
                        "Observed Eth AddIkaTokens at block: {}, tx_hash: {}",
                        log.block_number(),
                        log.tx_hash
                    );
