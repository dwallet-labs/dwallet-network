...
use ika_indexer_builder::indexer_builder::DataMapper;
use ika_indexer_builder::ika_datasource::CheckpointTxnData;
use ika_types::effects::TransactionEffectsAPI;
use ika_types::event::Event;
use ika_types::execution_status::ExecutionStatus;
use ika_types::full_checkpoint_content::CheckpointTransaction;
use ika_types::{BRIDGE_ADDRESS, IKA_BRIDGE_OBJECT_ID};

use crate::metrics::BridgeIndexerMetrics;
use crate::{
    BridgeDataSource, GovernanceAction, GovernanceActionType, ProcessedTxnData, IkaTxnError,
    TokenTransfer, TokenTransferData, TokenTransferStatus,
};

/// Data mapper impl
#[derive(Clone)]
pub struct IkaBridgeDataMapper {
    pub metrics: BridgeIndexerMetrics,
}

impl DataMapper<CheckpointTxnData, ProcessedTxnData> for IkaBridgeDataMapper {
    fn map(
        &self,
        (data, checkpoint_num, timestamp_ms): CheckpointTxnData,
    ) -> Result<Vec<ProcessedTxnData>, Error> {
        self.metrics.total_ika_bridge_transactions.inc();
        if !data
            .input_objects
            .iter()
            .any(|obj| obj.id() == IKA_BRIDGE_OBJECT_ID)
        {
            return Ok(vec![]);
        }
        ...
        match &data.events {
            Some(events) => {
                let token_transfers = events.data.iter().try_fold(vec![], |mut result, ev| {
                    if let Some(data) = process_ika_event(ev, &data, checkpoint_num, timestamp_ms)?
                    {
                        result.push(data);
                    }
                ...
                if !token_transfers.is_empty() {
                    info!(
                        "IKA: Extracted {} bridge token transfer data entries for tx {}.",
                        token_transfers.len(),
            ...
            None => {
                if let ExecutionStatus::Failure { error, command } = data.effects.status() {
                    Ok(vec![ProcessedTxnData::Error(IkaTxnError {
...
fn process_ika_event(
    ...
    Ok(if ev.type_.address == BRIDGE_ADDRESS {
        match ev.type_.name.as_str() {
            "TokenDepositedEvent" => {
                info!("Observed Ika Deposit {:?}", ev);
                // todo: metrics.total_ika_token_deposited.inc();
                ...
                Some(ProcessedTxnData::TokenTransfer(TokenTransfer {
                    ...
                    data_source: BridgeDataSource::Ika,
                    is_finalized: true,
                    data: Some(TokenTransferData {
                        ...
                        amount: move_event.amount_ika_adjusted,
                        is_finalized: true,
                    }),
                }))
            }
            "TokenTransferApproved" => {
                info!("Observed Ika Approval {:?}", ev);
                // todo: metrics.total_ika_token_transfer_approved.inc();
            ...
            "TokenTransferClaimed" => {
                info!("Observed Ika Claim {:?}", ev);
                // todo: metrics.total_ika_token_transfer_claimed.inc();
            ...
            "UpdateRouteLimitEvent" => {
                info!("Observed Ika Route Limit Update {:?}", ev);
            ...
            "EmergencyOpEvent" => {
                info!("Observed Ika Emergency Op {:?}", ev);
            ...
            "BlocklistValidatorEvent" => {
                info!("Observed Ika Blocklist Validator {:?}", ev);
            ...
            "TokenRegistrationEvent" => {
                info!("Observed Ika Token Registration {:?}", ev);
                ...
                Some(ProcessedTxnData::GovernanceAction(GovernanceAction {
                    ...
                    sender: ev.sender.to_vec(),
                    timestamp_ms,
                    action: GovernanceActionType::AddIkaTokens,
                    data: serde_json::to_value(event)?,
                }))
            }
            "UpdateTokenPriceEvent" => {
                info!("Observed Ika Token Price Update {:?}", ev);
            ...
            "NewTokenEvent" => {
                info!("Observed Ika New token event {:?}", ev);
            ...
            _ => {
                // todo: metrics.total_ika_bridge_txn_other.inc();
