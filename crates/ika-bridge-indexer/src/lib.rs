...
use crate::models::{IkaErrorTransactions, TokenTransfer as DBTokenTransfer};
use crate::postgres_manager::PgPool;
use crate::storage::PgBridgePersistent;
use crate::ika_bridge_indexer::IkaBridgeDataMapper;
...
use ika_bridge::metered_eth_provider::MeteredEthHttpProvier;
...
use ika_indexer_builder::indexer_builder::{BackfillStrategy, Datasource, Indexer, IndexerBuilder};
use ika_indexer_builder::metrics::IndexerMetricProvider;
use ika_indexer_builder::progress::{
    OutOfOrderSaveAfterDurationPolicy, ProgressSavingPolicy, SaveAfterDurationPolicy,
};
use ika_indexer_builder::ika_datasource::IkaCheckpointDatasource;
...
pub mod schema;
pub mod storage;
pub mod ika_transaction_handler;
pub mod ika_transaction_queries;
pub mod types;

pub mod eth_bridge_indexer;
pub mod ika_bridge_indexer;
...
impl IkaTxnError {
    fn to_db(&self) -> IkaErrorTransactions {
        IkaErrorTransactions {
...
impl Display for BridgeDataSource {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let str = match self {
            ...
            BridgeDataSource::Ika => "IKA",
        };
        write!(f, "{str}")
    }
}

pub async fn create_ika_indexer(
    pool: PgPool,
    metrics: BridgeIndexerMetrics,
    ingestion_metrics: DataIngestionMetrics,
    config: &IndexerConfig,
) -> anyhow::Result<
    Indexer<PgBridgePersistent, IkaCheckpointDatasource, IkaBridgeDataMapper>,
    anyhow::Error,
> {
    ...
    let ika_client = Arc::new(
        IkaClientBuilder::default()
            .build(config.ika_rpc_url.clone())
            .await?,
    );

    let ika_checkpoint_datasource = IkaCheckpointDatasource::new(
        ...
        config.ika_bridge_genesis_checkpoint,
        ingestion_metrics,
        metrics.clone().boxed(),
    );
    ...
    Ok(IndexerBuilder::new(
        "IkaBridgeIndexer",
        ika_checkpoint_datasource,
        IkaBridgeDataMapper { metrics },
...
async fn get_eth_bridge_contract_addresses(
    ...
    let bridge_address = EthAddress::from_str(&config.eth_ika_bridge_contract_address)?;
