...
use std::str::FromStr;
use std::sync::Arc;
use ika_bridge::eth_client::EthClient;
use ika_bridge::metered_eth_provider::{new_metered_eth_provider, MeteredEthHttpProvier};
use ika_bridge::ika_bridge_watchdog::Observable;
use ika_bridge::ika_client::IkaBridgeClient;
use ika_bridge::utils::get_eth_contract_addresses;
...
use ika_bridge::ika_bridge_watchdog::{
    eth_bridge_status::EthBridgeStatus,
    eth_vault_balance::{EthereumVaultBalance, VaultAsset},
    metrics::WatchdogMetrics,
    ika_bridge_status::IkaBridgeStatus,
    BridgeWatchDog,
};
use ika_bridge_indexer::config::IndexerConfig;
use ika_bridge_indexer::metrics::BridgeIndexerMetrics;
use ika_bridge_indexer::postgres_manager::{get_connection_pool, read_ika_progress_store};
use ika_bridge_indexer::ika_transaction_handler::handle_ika_transactions_loop;
use ika_bridge_indexer::ika_transaction_queries::start_ika_tx_polling_task;
use ika_bridge_indexer::{
    create_eth_subscription_indexer, create_eth_sync_indexer, create_ika_indexer,
};
use ika_data_ingestion_core::DataIngestionMetrics;
...
async fn main() -> Result<()> {
    ...
    let eth_bridge_proxy_address = EthAddress::from_str(&config.eth_ika_bridge_contract_address)?;
    let mut tasks = vec![];
    ...
    let indexer = create_ika_indexer(pool, indexer_meterics, ingestion_metrics, &config).await?;
    tasks.push(spawn_logged_monitored_task!(indexer.start()));

    let ika_bridge_client =
        Arc::new(IkaBridgeClient::new(&config.ika_rpc_url, bridge_metrics.clone()).await?);
    start_watchdog(
        config,
...
async fn start_watchdog(
    ...
    let ika_bridge_status =
        IkaBridgeStatus::new(ika_client, watchdog_metrics.ika_bridge_paused.clone());
...
#[allow(unused)]
async fn start_processing_ika_checkpoints_by_querying_txns(
    ika_rpc_url: String,
    db_url: String,
    ...
    let (tx, rx) = channel(
        ...
        &mysten_metrics::get_metrics()
            .unwrap()
            .channel_inflight
            .with_label_values(&["ika_transaction_processing_queue"]),
    );
    let mut handles = vec![];
    let cursor = read_ika_progress_store(&pg_pool)
        .await
        .expect("Failed to read cursor from ika progress store");
    let ika_client = IkaClientBuilder::default().build(ika_rpc_url).await?;
    handles.push(spawn_logged_monitored_task!(
        start_ika_tx_polling_task(ika_client, cursor, tx),
        "start_ika_tx_polling_task"
    ));
    handles.push(spawn_logged_monitored_task!(
        handle_ika_transactions_loop(pg_pool.clone(), rx, indexer_metrics.clone()),
        "handle_ika_transcations_loop"
    ));
    Ok(handles)
}
