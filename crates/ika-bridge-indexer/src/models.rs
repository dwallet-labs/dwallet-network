...
use ika_indexer_builder::{Task, LIVE_TASK_TARGET_CHECKPOINT};

use crate::schema::{
    governance_actions, progress_store, ika_error_transactions, ika_progress_store, token_transfer,
    token_transfer_data,
};
...
#[derive(Queryable, Selectable, Insertable, Identifiable, Debug)]
#[diesel(table_name = ika_progress_store, primary_key(txn_digest))]
pub struct IkaProgressStore {
...
#[derive(Queryable, Selectable, Insertable, Identifiable, Debug)]
#[diesel(table_name = ika_error_transactions, primary_key(txn_digest))]
pub struct IkaErrorTransactions {
    pub txn_digest: Vec<u8>,
    pub sender_address: Vec<u8>,
    pub timestamp_ms: i64,
