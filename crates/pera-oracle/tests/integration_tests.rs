...
use pera_sdk::rpc_types::PeraTransactionBlockResponseOptions;
use pera_sdk::types::base_types::{ObjectID, PeraAddress};
use pera_sdk::types::programmable_transaction_builder::ProgrammableTransactionBuilder;
use pera_sdk::types::quorum_driver_types::ExecuteTransactionRequestType;
use pera_sdk::types::transaction::{CallArg, ObjectArg, Transaction, TransactionData};
use pera_sdk::types::Identifier;
...
async fn test_publish_primitive() {
    ...
    let signature = keystore
        .sign_secure(&sender, &data, Intent::pera_transaction())
        .unwrap();
...
async fn test_publish_complex_value() {
    ...
    for i in 1..200 {
        ...
        builder.programmable_move_call(
            ...
            module.clone(),
            submit_data.clone(),
            vec![parse_pera_type_tag(&format!("{package}::decimal_value::DecimalValue")).unwrap()],
...
async fn test_consume_oracle_data() {
    ...
    for _ in 0..3 {
        ...
        let ticker = builder
            .input(CallArg::Pure(bcs::to_bytes("PERAUSD".as_bytes()).unwrap()))
            .unwrap();
        let identifier = builder
            .input(CallArg::Pure(
                bcs::to_bytes("identifier".as_bytes()).unwrap(),
    ...
    builder.programmable_move_call(
        ...
        simple_fx,
        vec![],
        vec![simple_oracle, npera_amount],
    );
    ...
    builder.programmable_move_call(
        ...
        trusted_fx,
        vec![],
        vec![oracles[0], oracles[1], oracles[2], npera_amount],
    );
...
async fn get_gas(client: &PeraClient, sender: PeraAddress) -> (ObjectRef, u64) {
    let gas = client
        .coin_read_api()
...
async fn init_test_client() -> (PeraClient, Keystore, PeraAddress) {
    let client = PeraClientBuilder::default()
        .build("https://rpc.devnet.pera.io:443")
        .await
        .unwrap();
    ...
    let keystore = Keystore::File(
        FileBasedKeystore::new(
            &dirs::home_dir()
                .unwrap()
                .join(".pera/pera_config/pera.keystore"),
        )
        .unwrap(),
    );
    let sender: PeraAddress = keystore.addresses()[0];
    let gas = client
        .coin_read_api()
        .get_coins(sender, None, None, Some(1))
        .await
