...
use pera_sdk::apis::ReadApi;
use pera_sdk::rpc_types::PeraObjectResponse;
...
use pera_types::quorum_driver_types::NON_RECOVERABLE_ERROR_MSG;
use pera_types::transaction::{Argument, Transaction};
use pera_types::transaction::{Command, ObjectArg};
...
use pera_types::base_types::{random_object_ref, ObjectID, ObjectRef};
...
impl DataProviderRunner {
    pub async fn new(
        ...
        client: Arc<PeraClient>,
        signer_address: PeraAddress,
        metrics: Arc<OracleMetrics>,
    ) -> Self {
...
async fn get_gas_obj_ref(
    read_api: &ReadApi,
    gas_obj_id: ObjectID,
    owner_address: PeraAddress,
) -> ObjectRef {
    loop {
        match read_api
            .get_object_with_options(gas_obj_id, PeraObjectDataOptions::default().with_owner())
            .await
            .map(|resp| resp.data)
        {
            Ok(Some(gas_obj)) => {
...
impl OnChainDataUploader {
    ...
    async fn upload(
    ...
    ) -> anyhow::Result<PeraTransactionBlockEffects> {
        ...
        for data_point in &data_points {
            ...
            builder.command(Command::move_call(
                ...
                vec![
                    parse_pera_type_tag(&format!("{package_id}::decimal_value::DecimalValue"))
                        .unwrap(),
                ],
                arguments,
            ));
    ...
    async fn execute(&mut self, tx: Transaction) -> anyhow::Result<PeraTransactionBlockResponse> {
        ...
        loop {
            match self
                ...
                Err(pera_sdk::error::Error::RpcError(err)) => {
                    // jsonrpsee translate every PeraError into jsonrpsee::core::Error, so we need to further distinguish 
        ...
        loop {
            ...
            for (feed_name, object_id) in &self.read_configs {
                match self
                    .client
                    .read_api()
                    .get_object_with_options(*object_id, PeraObjectDataOptions::default())
                    .await
                {
                    Ok(PeraObjectResponse {
                        data: Some(_data), ..
                    }) => {
