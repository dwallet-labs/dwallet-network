...
use ika_indexer_builder::progress::ProgressSavingPolicy;
use ika_types::base_types::ObjectID;
use ika_types::transaction::{Command, TransactionDataAPI};
use tracing::info;

use ika_indexer_builder::indexer_builder::{DataMapper, IndexerProgressStore, Persistent};
use ika_indexer_builder::ika_datasource::CheckpointTxnData;
use ika_indexer_builder::{Task, Tasks, LIVE_TASK_TARGET_CHECKPOINT};
...
use crate::schema::{
    ...
    ika_error_transactions, trade_params_update, votes,
};
use crate::types::{
    Balances, Flashloan, OrderFill, OrderUpdate, OrderUpdateStatus, PoolPrice, ProcessedTxnData,
    Proposals, Rebates, Stakes, IkaTxnError, TradeParamsUpdate, Votes,
};
                    ...
                    for d in data {
                        match d {
                            ...
                            ProcessedTxnData::Error(e) => {
                                diesel::insert_into(ika_error_transactions::table)
                                    .values(&e.to_db())
                                    .on_conflict_do_nothing()
                                    .execute(conn)
                                    .await?;
...
/// Data mapper impl
#[derive(Clone)]
pub struct IkaDeepBookDataMapper {
    pub metrics: DeepBookIndexerMetrics,
    pub package_id: ObjectID,
}

impl DataMapper<CheckpointTxnData, ProcessedTxnData> for IkaDeepBookDataMapper {
    fn map(
        &self,
        ...
        match &data.events {
            Some(events) => {
                let processed_ika_events =
                    events
                        .data
                        .iter()
                        .enumerate()
                        .try_fold(vec![], |mut result, (i, ev)| {
                            if let Some(data) = process_ika_event(
                                ev,
                                i,
                                &data,
                                checkpoint_num,
                ...
                if !processed_ika_events.is_empty() {
                    info!(
                        "IKA: Extracted {} deepbook data entries for tx {}.",
                        processed_ika_events.len(),
                        data.transaction.digest()
                    );
                }
                Ok(processed_ika_events)
            }
            None => {
