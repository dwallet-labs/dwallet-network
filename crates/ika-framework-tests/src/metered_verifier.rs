...
use ika_adapter::adapter::run_metered_move_bytecode_verifier;
use ika_config::verifier_signing_config::VerifierSigningConfig;
use ika_framework::BuiltInFramework;
use ika_move_build::{CompiledPackage, IkaPackageHooks};
use ika_protocol_config::ProtocolConfig;
use ika_types::{
    error::{IkaError, IkaResult},
    metrics::BytecodeVerifierMetrics,
};
use ika_verifier::meter::IkaVerifierMeter;

fn build(path: &Path) -> IkaResult<CompiledPackage> {
    let mut config = ika_move_build::BuildConfig::new_for_testing();
...
fn test_metered_move_bytecode_verifier() {
    ...
    let path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../ika-framework/packages/ika-framework");
    ...
    let mut meter = IkaVerifierMeter::new(meter_config.clone());
    ...
    let mut meter = IkaVerifierMeter::new(meter_config);
    let timer_start = Instant::now();
    let r = run_metered_move_bytecode_verifier(
        &compiled_modules,
        &verifier_config,
