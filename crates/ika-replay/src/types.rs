...
use ika_sdk::error::Error as IkaRpcError;
use ika_types::base_types::{ObjectID, ObjectRef, SequenceNumber, IkaAddress, VersionNumber};
use ika_types::digests::{ObjectDigest, TransactionDigest};
use ika_types::error::{IkaError, IkaObjectResponseError, IkaResult, UserInputError};
use ika_types::object::Object;
use ika_types::transaction::{InputObjectKind, SenderSignedData, TransactionKind};
use thiserror::Error;
...
pub(crate) const EPOCH_CHANGE_STRUCT_TAG: &str =
    "0x3::ika_system_state_inner::SystemEpochInfoEvent";
...
pub enum ReplayEngineError {
    #[error("IkaError: {:#?}", err)]
    IkaError { err: IkaError },

    #[error("IkaRpcError: {:#?}", err)]
    IkaRpcError { err: String },

    #[error("IkaObjectResponseError: {:#?}", err)]
    IkaObjectResponseError { err: IkaObjectResponseError },
    ...
    #[error("IkaRpcRequestTimeout")]
    IkaRpcRequestTimeout,
    ...
    EffectsForked {
        ...
        diff: String,
        on_chain: Box<IkaTransactionBlockEffects>,
        local: Box<IkaTransactionBlockEffects>,
    },
    ...
    UnexpectedEventFormat { event: IkaEvent },
...
impl From<IkaObjectResponseError> for ReplayEngineError {
    fn from(err: IkaObjectResponseError) -> Self {
        match err {
            IkaObjectResponseError::NotExists { object_id } => {
            ...
            _ => ReplayEngineError::IkaObjectResponseError { err },
        }
    }
}

impl From<ReplayEngineError> for IkaError {
    fn from(err: ReplayEngineError) -> Self {
        IkaError::Unknown(format!("{:#?}", err))
    }
}

impl From<IkaError> for ReplayEngineError {
    fn from(err: IkaError) -> Self {
        ReplayEngineError::IkaError { err }
    }
}
impl From<IkaRpcError> for ReplayEngineError {
    fn from(err: IkaRpcError) -> Self {
        match err {
            IkaRpcError::RpcError(JsonRpseeError::RequestTimeout) => {
                ReplayEngineError::IkaRpcRequestTimeout
            }
            _ => ReplayEngineError::IkaRpcError {
                err: format!("{:?}", err),
            },
        }
    }
...
pub enum ExecutionStoreEvent {
    BackingPackageGetPackageObject {
        package_id: ObjectID,
        result: IkaResult<Option<Object>>,
    },
    ...
    ParentSyncStoreGetLatestParentEntryRef {
        object_id: ObjectID,
        result: IkaResult<Option<ObjectRef>>,
    },
    ResourceResolverGetResource {
        address: AccountAddress,
        typ: StructTag,
        result: IkaResult<Option<Vec<u8>>>,
    },
    ...
    GetModuleGetModuleByModuleId {
        id: ModuleId,
        result: IkaResult<Option<CompiledModule>>,
    },
    ReceiveObject {
        owner: ObjectID,
        receive: ObjectID,
