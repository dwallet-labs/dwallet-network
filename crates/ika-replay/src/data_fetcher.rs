...
use ika_json_rpc_types::IkaGetPastObjectRequest;
use ika_json_rpc_types::IkaObjectData;
use ika_json_rpc_types::IkaObjectDataOptions;
use ika_json_rpc_types::IkaObjectResponse;
use ika_json_rpc_types::IkaPastObjectResponse;
...
use ika_types::base_types::{ObjectID, SequenceNumber, VersionNumber};
...
use ika_types::transaction::{EndOfEpochTransactionKind, TransactionKind};
...
pub(crate) trait DataFetcher {
    ...
    ) -> Result<IkaTransactionBlockResponse, ReplayEngineError>;
    ...
    async fn get_epoch_change_events(
        &self,
        reverse: bool,
    ) -> Result<Vec<IkaEvent>, ReplayEngineError>;
...
impl RemoteFetcher {
    pub fn new(rpc_client: IkaClient) -> Self {
        Self {
            rpc_client,
...
impl DataFetcher for RemoteFetcher {
    ...
    async fn multi_get_versioned(
        ...
        let options = IkaObjectDataOptions::bcs_lossless();

        let objs: Vec<_> = to_fetch
            .iter()
            .map(|(object_id, version)| IkaGetPastObjectRequest {
                object_id: *object_id,
                version: *version,
            })
            .collect();
    ...
    async fn get_child_object(
        ...
        let response = self
            ...
            .await
            .map_err(|q| ReplayEngineError::IkaRpcError { err: q.to_string() })?;
    ...
    async fn get_transaction(
        ...
        let tx_fetch_opts = IkaTransactionBlockResponseOptions::full_content();
    ...
    async fn get_epoch_change_events(
        ...
        let mut epoch_change_events: Vec<IkaEvent> = vec![];
...
fn convert_past_obj_response(resp: IkaPastObjectResponse) -> Result<Object, ReplayEngineError> {
    match resp {
        IkaPastObjectResponse::VersionFound(o) => obj_from_ika_obj_data(&o),
        IkaPastObjectResponse::ObjectDeleted(r) => Err(ReplayEngineError::ObjectDeleted {
            id: r.object_id,
            version: r.version,
            digest: r.digest,
        }),
        IkaPastObjectResponse::ObjectNotExists(id) => Err(ReplayEngineError::ObjectNotExist { id }),
        IkaPastObjectResponse::VersionNotFound(id, version) => {
            Err(ReplayEngineError::ObjectVersionNotFound { id, version })
        }
        IkaPastObjectResponse::VersionTooHigh {
            object_id,
            asked_version,
            latest_version,
...
fn obj_from_ika_obj_response(o: &IkaObjectResponse) -> Result<Object, ReplayEngineError> {
    let o = o.object().map_err(ReplayEngineError::from)?.clone();
    obj_from_ika_obj_data(&o)
}

fn obj_from_ika_obj_data(o: &IkaObjectData) -> Result<Object, ReplayEngineError> {
...
pub fn extract_epoch_and_version(ev: IkaEvent) -> Result<(u64, u64), ReplayEngineError> {
