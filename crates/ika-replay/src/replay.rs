...
use ika_core::authority::NodeStateDump;
use ika_execution::Executor;
use ika_framework::BuiltInFramework;
use ika_json_rpc_types::{
    IkaExecutionStatus, IkaTransactionBlockEffects, IkaTransactionBlockEffectsAPI,
};
...
use ika_types::storage::{get_module, PackageObject};
use ika_types::transaction::TransactionKind::ProgrammableTransaction;
use ika_types::IKA_DENY_LIST_OBJECT_ID;
use ika_types::{
...
pub struct ExecutionSandboxState {
    ...
    pub local_exec_effects: IkaTransactionBlockEffects,
...
impl LocalExec {
    ...
    pub async fn multi_download(
        ...
        while num_retries_for_timeout >= 0 {
            match self.fetcher.multi_get_versioned(objs).await {
                ...
                Err(ReplayEngineError::IkaRpcRequestTimeout) => {
                    warn!(
    ...
    pub fn download_latest_object(
        ...
        match resp {
            ...
            Err(err) => Err(ReplayEngineError::IkaRpcError {
                err: err.to_string(),
            }),
        }
    }
    ...
    pub async fn get_checkpoint_txs(
        ...
        self.fetcher
            ...
            .await
            .map_err(|e| ReplayEngineError::IkaRpcError { err: e.to_string() })
    }
    ...
    pub async fn execution_engine_execute_with_tx_info_impl(
        ...
        let effects =
            IkaTransactionBlockEffects::try_from(effects).map_err(ReplayEngineError::from)?;
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `ika_core::authority::AuthorityState::try_execute_immediately`
    ...
    /// Must be called after `init_for_execution`
    /// This executes from `ika_adapter::execution_engine::execute_transaction_to_effects`
        ...
        match parse_effect_error_for_denied_coins(status) {
            Some(coin_type) => {
                ...
                if !config_id_and_version
                    .iter()
                    .any(|(id, _)| id == &IKA_DENY_LIST_OBJECT_ID)
                {
                    let deny_list_oid_version = self.download_latest_object(&IKA_DENY_LIST_OBJECT_ID)
                        .ok()
                        .flatten()
                        .expect("Unable to download the deny list object for a transaction that requires it")
                        .version();
                    config_id_and_version.push((IKA_DENY_LIST_OBJECT_ID, deny_list_oid_version));
                }
    ...
    async fn resolve_tx_components(
        ...
        let sender = match tx_info.clone().transaction.unwrap().data {
            ika_json_rpc_types::IkaTransactionBlockData::V1(tx) => tx.sender,
        };
        let IkaTransactionBlockEffects::V1(effects) = tx_info.clone().effects.unwrap();
        ...
        let gas_data = match tx_info.clone().transaction.unwrap().data {
            ika_json_rpc_types::IkaTransactionBlockData::V1(tx) => tx.gas_data,
        };
        ...
        Ok(OnChainTransactionInfo {
            ...
            effects: IkaTransactionBlockEffects::V1(effects),
            receiving_objs,
            config_objects,
    ...
    async fn resolve_tx_components_from_dump(
        ...
        let effects = IkaTransactionBlockEffects::try_from(effects).unwrap();
    ...
    fn read_child_object(
        ...
        fn inner(
            ...
            if child_object.owner != Owner::ObjectOwner(parent.into()) {
                return Err(IkaError::InvalidChildObjectAccess {
                    object: *child,
                    given_parent: parent,
...
impl ParentSync for LocalExec {
    ...
    fn get_latest_parent_entry_ref_deprecated(
        ...
        fn inner(self_: &LocalExec, object_id: ObjectID) -> IkaResult<Option<ObjectRef>> {
            if let Some(v) = self_
                .storage
                .live_objects_store
                .lock()
...
impl ModuleResolver for LocalExec {
    ...
    /// We do not download
    fn get_module(&self, module_id: &ModuleId) -> IkaResult<Option<Vec<u8>>> {
        fn inner(self_: &LocalExec, module_id: &ModuleId) -> IkaResult<Option<Vec<u8>>> {
...
impl GetModule for LocalExec {
    ...
    fn get_module_by_id(&self, id: &ModuleId) -> IkaResult<Option<Self::Item>> {
...
pub fn get_executor(
    ...
    let silent = true;
    ika_execution::executor(&protocol_config, silent, enable_profiler)
        .expect("Creating an executor should not fail here")
}

fn parse_effect_error_for_denied_coins(status: &IkaExecutionStatus) -> Option<String> {
    let IkaExecutionStatus::Failure { error } = status else {
        return None;
    };
