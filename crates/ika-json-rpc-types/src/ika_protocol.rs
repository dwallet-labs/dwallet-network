...
use ika_types::ika_serde::{AsProtocolVersion, BigInt};

#[serde_as]
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema, PartialEq)]
#[serde(rename_all = "camelCase", rename = "ProtocolConfigValue")]
pub enum IkaProtocolConfigValue {
    U16(
...
impl From<ProtocolConfigValue> for IkaProtocolConfigValue {
    fn from(value: ProtocolConfigValue) -> Self {
        match value {
            ProtocolConfigValue::u16(y) => IkaProtocolConfigValue::U16(y),
            ProtocolConfigValue::u32(y) => IkaProtocolConfigValue::U32(y),
            ProtocolConfigValue::u64(x) => IkaProtocolConfigValue::U64(x),
            ProtocolConfigValue::bool(z) => IkaProtocolConfigValue::Bool(z),
        }
    }
}
...
pub struct ProtocolConfigResponse {
    ...
    pub attributes: BTreeMap<String, Option<IkaProtocolConfigValue>>,
}
    ...
    fn from(config: ProtocolConfig) -> Self {
        ProtocolConfigResponse {
            ...
            attributes: config
                .attr_map()
                .into_iter()
                .map(|(k, v)| (k, v.map(IkaProtocolConfigValue::from)))
                .collect(),
