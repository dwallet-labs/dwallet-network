...
use ika_types::base_types::{
    ObjectDigest, ObjectID, ObjectInfo, ObjectRef, ObjectType, SequenceNumber, IkaAddress,
    TransactionDigest,
};
use ika_types::error::{
    ExecutionError, IkaError, IkaObjectResponseError, IkaResult, UserInputError, UserInputResult,
};
use ika_types::gas_coin::GasCoin;
use ika_types::messages_checkpoint::CheckpointSequenceNumber;
use ika_types::move_package::{MovePackage, TypeOrigin, UpgradeInfo};
use ika_types::object::{Data, MoveObject, Object, ObjectInner, ObjectRead, Owner};
use ika_types::ika_serde::BigInt;
use ika_types::ika_serde::SequenceNumber as AsSequenceNumber;
use ika_types::ika_serde::IkaStructTag;

use crate::{Page, IkaMoveStruct, IkaMoveValue};

#[derive(Serialize, Deserialize, Debug, JsonSchema, Clone, PartialEq, Eq)]
pub struct IkaObjectResponse {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub data: Option<IkaObjectData>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub error: Option<IkaObjectResponseError>,
}

impl IkaObjectResponse {
    pub fn new(data: Option<IkaObjectData>, error: Option<IkaObjectResponseError>) -> Self {
        Self { data, error }
    }

    pub fn new_with_data(data: IkaObjectData) -> Self {
        Self {
            data: Some(data),
            error: None,
        }
    ...
    pub fn new_with_error(error: IkaObjectResponseError) -> Self {
        Self {
            data: None,
            error: Some(error),
        }
...
impl Ord for IkaObjectResponse {
    fn cmp(&self, other: &Self) -> Ordering {
        match (&self.data, &other.data) {
            ...
            // In this ordering those with data will come before IkaObjectResponses that are errors.
            (Some(_), None) => Ordering::Less,
            (None, Some(_)) => Ordering::Greater,
            // IkaObjectResponses that are errors are just considered equal.
            _ => Ordering::Equal,
        }
    }
}

impl PartialOrd for IkaObjectResponse {
...
impl IkaObjectResponse {
    pub fn move_object_bcs(&self) -> Option<&Vec<u8>> {
        match &self.data {
            Some(IkaObjectData {
                bcs: Some(IkaRawData::MoveObject(obj)),
                ..
            }) => Some(&obj.bcs_bytes),
            _ => None,
        }
    ...
    pub fn object_id(&self) -> Result<ObjectID, anyhow::Error> {
        match (&self.data, &self.error) {
            ...
            (None, Some(IkaObjectResponseError::NotExists { object_id })) => Ok(*object_id),
            (
                None,
            ...
            _ => Err(anyhow!("Could not get object_id, something went wrong with IkaObjectResponse construction.")),
        }
    }
...
impl TryFrom<IkaObjectResponse> for ObjectInfo {
    type Error = anyhow::Error;

    fn try_from(value: IkaObjectResponse) -> Result<Self, Self::Error> {
        let IkaObjectData {
            object_id,
            version,
            digest,
...
pub struct IkaObjectData {
    ...
    /// The amount of IKA we would rebate if this object gets deleted.
    ...
    /// The Display metadata for frontend UI rendering, default to be None unless IkaObjectDataOptions.showContent is set to true
    /// This can also be None if the struct type does not have Display defined
    /// See more details in <https://forums.ika.io/t/nft-object-display-proposal/4872>
    #[serde(skip_serializing_if = "Option::is_none")]
    pub display: Option<DisplayFieldsResponse>,
    /// Move object content or package content, default to be None unless IkaObjectDataOptions.showContent is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    pub content: Option<IkaParsedData>,
    /// Move object content or package content in BCS, default to be None unless IkaObjectDataOptions.showBcs is set to true
    #[serde(skip_serializing_if = "Option::is_none")]
    pub bcs: Option<IkaRawData>,
}
...
impl Display for IkaObjectData {
...
impl TryFrom<&IkaObjectData> for GasCoin {
    type Error = anyhow::Error;
    fn try_from(object: &IkaObjectData) -> Result<Self, Self::Error> {
        match &object
            .content
            .as_ref()
            .ok_or_else(|| anyhow!("Expect object content to not be empty"))?
        {
            IkaParsedData::MoveObject(o) => {
                if GasCoin::type_() == o.type_ {
                    return GasCoin::try_from(&o.fields);
                }
            }
            IkaParsedData::Package(_) => {}
        }
...
impl TryFrom<&IkaMoveStruct> for GasCoin {
    type Error = anyhow::Error;
    fn try_from(move_struct: &IkaMoveStruct) -> Result<Self, Self::Error> {
        match move_struct {
            IkaMoveStruct::WithFields(fields) | IkaMoveStruct::WithTypes { type_: _, fields } => {
                if let Some(IkaMoveValue::String(balance)) = fields.get("balance") {
                    if let Ok(balance) = balance.parse::<u64>() {
                        if let Some(IkaMoveValue::UID { id }) = fields.get("id") {
...
pub struct IkaObjectDataOptions {
...
impl TryFrom<(ObjectRead, IkaObjectDataOptions)> for IkaObjectResponse {
    type Error = anyhow::Error;

    fn try_from(
        (object_read, options): (ObjectRead, IkaObjectDataOptions),
    ) -> Result<Self, Self::Error> {
        match object_read {
            ObjectRead::NotExists(id) => Ok(IkaObjectResponse::new_with_error(
                IkaObjectResponseError::NotExists { object_id: id },
            )),
            ObjectRead::Exists(object_ref, o, layout) => {
                let data = (object_ref, o, layout, options).try_into()?;
                Ok(IkaObjectResponse::new_with_data(data))
            }
            ObjectRead::Deleted((object_id, version, digest)) => Ok(
                IkaObjectResponse::new_with_error(IkaObjectResponseError::Deleted {
                    object_id,
                    version,
                    digest,
                }),
...
impl TryFrom<(ObjectInfo, IkaObjectDataOptions)> for IkaObjectResponse {
    type Error = anyhow::Error;

    fn try_from(
        (object_info, options): (ObjectInfo, IkaObjectDataOptions),
    ) -> Result<Self, Self::Error> {
        let IkaObjectDataOptions {
            show_type,
            show_owner,
        ...
        Ok(Self::new_with_data(IkaObjectData {
    ...
    )> for IkaObjectData
{
    ...
    fn try_from(
        ...
        let content: Option<IkaParsedData> = if show_content {
            let data = match o.data {
                Data::Move(m) => {
                    let layout = layout.ok_or_else(|| {
                        anyhow!("Layout is required to convert Move object to json")
                    })?;
                    IkaParsedData::try_from_object(m, layout)?
                }
                Data::Package(p) => IkaParsedData::try_from_package(p)?,
            };
            Some(data)
        } else {
            None
    ...
    )> for IkaObjectData
{
    ...
    fn try_from(
        ...
        let mut data: IkaObjectData = (object_ref, o, layout, options).try_into()?;
        if show_display {
...
impl IkaObjectResponse {
    ...
    pub fn object(&self) -> Result<&IkaObjectData, IkaObjectResponseError> {
        if let Some(data) = &self.data {
            Ok(data)
            ...
            Err(IkaObjectResponseError::Unknown)
        }
    }

    /// Returns the object value if there is any, otherwise an Err if
    /// the object does not exist or is deleted.
    pub fn into_object(self) -> Result<IkaObjectData, IkaObjectResponseError> {
        match self.object() {
...
impl TryInto<Object> for IkaObjectData {
    ...
    fn try_into(self) -> Result<Object, Self::Error> {
        ...
        let data = match self.bcs {
            ...
            Some(IkaRawData::Package(p)) => Data::Package(MovePackage::new(
                p.id,
                self.version,
                p.module_map,
            ...
            _ => Err(anyhow!(
                "BCS data is required to convert IkaObjectData to Object"
            ))?,
        };
        Ok(ObjectInner {
            data,
            owner: self
                .owner
                .ok_or_else(|| anyhow!("Owner is required to convert IkaObjectData to Object"))?,
            previous_transaction: self.previous_transaction.ok_or_else(|| {
                anyhow!("previous_transaction is required to convert IkaObjectData to Object")
            })?,
            storage_rebate: self.storage_rebate.ok_or_else(|| {
                anyhow!("storage_rebate is required to convert IkaObjectData to Object")
            })?,
        }
        .into())
    }
...
impl From<ObjectRef> for IkaObjectRef {
...
pub enum IkaRawData {
    ...
    MoveObject(IkaRawMoveObject),
    Package(IkaRawMovePackage),
}

impl IkaData for IkaRawData {
    type ObjectType = IkaRawMoveObject;
    type PackageType = IkaRawMovePackage;
...
pub enum IkaParsedData {
    // Manually handle generic schema generation
    MoveObject(IkaParsedMoveObject),
    Package(IkaMovePackage),
}

impl IkaData for IkaParsedData {
    type ObjectType = IkaParsedMoveObject;
    type PackageType = IkaMovePackage;

    fn try_from_object(
        object: MoveObject,
        layout: MoveStructLayout,
    ) -> Result<Self, anyhow::Error> {
        Ok(Self::MoveObject(IkaParsedMoveObject::try_from_layout(
            object, layout,
        )?))
    }

    fn try_from_package(package: MovePackage) -> Result<Self, anyhow::Error> {
        Ok(Self::Package(IkaMovePackage {
...
impl Display for IkaParsedData {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        let mut writer = String::new();
        match self {
            IkaParsedData::MoveObject(o) => {
...
pub struct IkaParsedMoveObject {
    #[serde(rename = "type")]
    #[serde_as(as = "IkaStructTag")]
    #[schemars(with = "String")]
    pub type_: StructTag,
    pub has_public_transfer: bool,
    pub fields: IkaMoveStruct,
}

impl IkaMoveObject for IkaParsedMoveObject {
    fn try_from_layout(
        object: MoveObject,
        layout: MoveStructLayout,
    ) -> Result<Self, anyhow::Error> {
        ...
        Ok(
            if let IkaMoveStruct::WithTypes { type_, fields } = move_struct {
                IkaParsedMoveObject {
                    type_,
                    has_public_transfer: object.has_public_transfer(),
                    fields: IkaMoveStruct::WithFields(fields),
                }
            } else {
                IkaParsedMoveObject {
...
impl IkaParsedMoveObject {
    pub fn try_from_object_read(object_read: ObjectRead) -> Result<Self, anyhow::Error> {
        let parsed_data = IkaParsedData::try_from_object_read(object_read)?;
        match parsed_data {
            IkaParsedData::MoveObject(o) => Ok(o),
            IkaParsedData::Package(_) => Err(anyhow::anyhow!("Object is not a Move object")),
        }
    }
}
...
) -> IkaResult<(StructTag, serde_json::Value)> {
    match event_data.into() {
        IkaMoveValue::Struct(move_struct) => match &move_struct {
            IkaMoveStruct::WithTypes { type_, .. } => {
                Ok((type_.clone(), move_struct.clone().to_json_value()))
            }
            _ => Err(IkaError::ObjectDeserializationError {
                error: "Found non-type IkaMoveStruct in MoveValue event".to_string(),
            }),
        },
        IkaMoveValue::Variant(v) => Ok((v.type_.clone(), v.clone().to_json_value())),
        IkaMoveValue::Vector(_)
        | IkaMoveValue::Number(_)
        | IkaMoveValue::Bool(_)
        | IkaMoveValue::Address(_)
        | IkaMoveValue::String(_)
        | IkaMoveValue::UID { .. }
        | IkaMoveValue::Option(_) => Err(IkaError::ObjectDeserializationError {
...
impl From<MoveObject> for IkaRawMoveObject {
...
impl IkaMoveObject for IkaRawMoveObject {
    fn try_from_layout(
        object: MoveObject,
...
impl From<MovePackage> for IkaRawMovePackage {
...
pub enum IkaPastObjectResponse {
    /// The object exists and is found with this version
    VersionFound(IkaObjectData),
    /// The object does not exist
    ObjectNotExists(ObjectID),
    /// The object is found to be deleted with this version
    ObjectDeleted(IkaObjectRef),
...
impl IkaPastObjectResponse {
    /// Returns a reference to the object if there is any, otherwise an Err
    pub fn object(&self) -> UserInputResult<&IkaObjectData> {
        match &self {
    ...
    pub fn into_object(self) -> UserInputResult<IkaObjectData> {
        match self {
            Self::ObjectDeleted(oref) => Err(UserInputError::ObjectDeleted {
                object_ref: oref.to_object_ref(),
            }),
...
pub type QueryObjectsPage = Page<IkaObjectResponse, CheckpointedObjectID>;
pub type ObjectsPage = Page<IkaObjectResponse, ObjectID>;
...
pub struct IkaGetPastObjectRequest {
...
pub enum IkaObjectDataFilter {
    MatchAll(Vec<IkaObjectDataFilter>),
    MatchAny(Vec<IkaObjectDataFilter>),
    MatchNone(Vec<IkaObjectDataFilter>),
...
impl IkaObjectDataFilter {
    ...
    pub fn matches(&self, object: &ObjectInfo) -> bool {
        match self {
            IkaObjectDataFilter::MatchAll(filters) => !filters.iter().any(|f| !f.matches(object)),
            IkaObjectDataFilter::MatchAny(filters) => filters.iter().any(|f| f.matches(object)),
            IkaObjectDataFilter::MatchNone(filters) => !filters.iter().any(|f| f.matches(object)),
            ...
            IkaObjectDataFilter::ObjectOwner(o) => {
                matches!(object.owner, Owner::ObjectOwner(addr) if addr == IkaAddress::from(*o))
            }
            IkaObjectDataFilter::ObjectId(id) => &object.object_id == id,
            IkaObjectDataFilter::ObjectIds(ids) => ids.contains(&object.object_id),
            IkaObjectDataFilter::Version(v) => object.version.value() == *v,
        }
    }
}
...
pub struct IkaObjectResponseQuery {
    /// If None, no filter will be applied
    pub filter: Option<IkaObjectDataFilter>,
    /// config which fields to include in the response, by default only digest is included
    pub options: Option<IkaObjectDataOptions>,
}

impl IkaObjectResponseQuery {
    pub fn new(filter: Option<IkaObjectDataFilter>, options: Option<IkaObjectDataOptions>) -> Self {
        Self { filter, options }
    }

    pub fn new_with_filter(filter: IkaObjectDataFilter) -> Self {
        Self {
            filter: Some(filter),
            options: None,
        }
    ...
    pub fn new_with_options(options: IkaObjectDataOptions) -> Self {
        Self {
            filter: None,
