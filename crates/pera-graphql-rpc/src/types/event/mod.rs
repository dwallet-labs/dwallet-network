...
use pera_indexer::models::{events::StoredEvent, transactions::StoredTransaction};
use pera_indexer::schema::{checkpoints, events};
...
/// A Pera node emits one of the following events:
/// Move event
/// Publish event
...
impl Event {
    ...
    async fn sender(&self) -> Result<Option<Address>> {
        if self.native.sender == NativePeraAddress::ZERO {
            return Ok(None);
        }
...
impl Event {
    ...
    fn try_from_stored_event(
        ...
        let sender = NativePeraAddress::from_bytes(sender_bytes)
