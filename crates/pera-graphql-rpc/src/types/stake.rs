...
use super::{
    big_int::BigInt, epoch::Epoch, move_object::MoveObject, object, pera_address::PeraAddress,
};
use async_graphql::connection::Connection;
use async_graphql::*;
use move_core_types::language_storage::StructTag;
use pera_json_rpc_types::{Stake as RpcStakedPera, StakeStatus as RpcStakeStatus};
use pera_types::base_types::MoveObjectType;
use pera_types::governance::StakedPera as NativeStakedPera;
...
pub(crate) enum StakedPeraDowncastError {
    NotAStakedPera,
    Bcs(bcs::Error),
}

#[derive(Clone)]
pub(crate) struct StakedPera {
    /// Representation of this StakedPera as a generic Move Object.
    pub super_: MoveObject,

    /// Deserialized representation of the Move Object's contents as a
    /// `0x3::staking_pool::StakedPera`.
    pub native: NativeStakedPera,
}
...
impl StakedPera {
    ...
    async fn pool_id(&self) -> Option<PeraAddress> {
...
impl StakedPera {
    /// Query the database for a `page` of Staked PERA. The page uses the same cursor type as is used
    ...
    ) -> Result<Connection<String, StakedPera>, Error> {
        let type_: StructTag = MoveObjectType::staked_pera().into();
        ...
        Object::paginate_subtype(db, page, filter, checkpoint_viewed_at, |object| {
            ...
            let move_object = MoveObject::try_from(&object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a StakedPera, but it's not a Move Object.",
                ))
            })?;

            StakedPera::try_from(&move_object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a StakedPera, but it is not."
                ))
            })
        })
        .await
    ...
    /// The JSON-RPC representation of a StakedPera so that we can "cheat" to implement fields that
    /// are not yet implemented directly for GraphQL.
    ///
    /// TODO: Make this obsolete
    async fn rpc_stake(&self, ctx: &Context<'_>) -> Result<RpcStakedPera, Error> {
        ctx.data_unchecked::<PgManager>()
            .fetch_rpc_staked_pera(self.native.clone())
            .await
    }
}

impl TryFrom<&MoveObject> for StakedPera {
    type Error = StakedPeraDowncastError;

    fn try_from(move_object: &MoveObject) -> Result<Self, Self::Error> {
        if !move_object.native.is_staked_pera() {
            return Err(StakedPeraDowncastError::NotAStakedPera);
        }

        Ok(Self {
            super_: move_object.clone(),
            native: bcs::from_bytes(move_object.native.contents())
                .map_err(StakedPeraDowncastError::Bcs)?,
        })
    }
}
