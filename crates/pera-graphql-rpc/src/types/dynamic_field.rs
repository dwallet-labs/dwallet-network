...
use pera_types::dynamic_field::{derive_dynamic_field_id, DynamicFieldInfo, DynamicFieldType};
...
use super::{
    base64::Base64, move_object::MoveObject, move_value::MoveValue, pera_address::PeraAddress,
};
...
impl TryFrom<MoveObject> for DynamicField {
    ...
    fn try_from(stored: MoveObject) -> Result<Self, Error> {
        ...
        let df_object_id = PeraAddress::from_bytes(df_object_id).map_err(|e| {
...
fn apply_filter(query: RawQuery, parent: PeraAddress, parent_version: Option<u64>) -> RawQuery {
    let query = filter!(
        query,
        format!(
