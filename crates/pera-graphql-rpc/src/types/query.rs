...
use super::perans_registration::NameService;
...
pub(crate) type PeraGraphQLSchema = async_graphql::Schema<Query, Mutation, EmptySubscription>;
...
impl Query {
    ...
    async fn dry_run_transaction_block(
        ...
        let pera_sdk_client: &Option<PeraClient> = ctx
            .data()
            .map_err(|_| Error::Internal("Unable to fetch Pera SDK client".to_string()))
            .extend()?;
        let pera_sdk_client = pera_sdk_client
            .as_ref()
            .ok_or_else(|| Error::Internal("Pera SDK client not initialized".to_string()))
            .extend()?;
    ...
    /// Look up an Owner by its PeraAddress.
    ///
    ...
    ///
    /// If `rootVersion` is omitted, dynamic fields will be from a consistent snapshot of the Pera
    ...
    async fn address(&self, ctx: &Context<'_>, address: PeraAddress) -> Result<Option<Address>> {
        let Watermark { checkpoint, .. } = *ctx.data()?;
    ...
    async fn resolve_perans_address(
        &self,
        ctx: &Context<'_>,
        domain: Domain,
