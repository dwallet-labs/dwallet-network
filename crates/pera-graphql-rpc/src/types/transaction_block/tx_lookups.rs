...
/// exclusive, but when isperang database queries, we treat them inclusively so that we can detect
...
fn select_tx(sender: Option<PeraAddress>, bound: TxBounds, from: &str) -> RawQuery {
...
fn select_pkg(pkg: &PeraAddress, sender: Option<PeraAddress>, bound: TxBounds) -> RawQuery {
    filter!(
...
fn select_sender(sender: &PeraAddress, bound: TxBounds) -> RawQuery {
    select_tx(Some(*sender), bound, "tx_senders")
}

fn select_recipient(recv: &PeraAddress, sender: Option<PeraAddress>, bound: TxBounds) -> RawQuery {
    filter!(
...
fn select_input(input: &PeraAddress, sender: Option<PeraAddress>, bound: TxBounds) -> RawQuery {
    filter!(
...
fn select_changed(changed: &PeraAddress, sender: Option<PeraAddress>, bound: TxBounds) -> RawQuery {
    filter!(
