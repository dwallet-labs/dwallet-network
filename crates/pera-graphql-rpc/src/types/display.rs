...
use pera_indexer::{models::display::StoredDisplay, schema::display};
...
pub(crate) fn get_value_from_move_struct(
    ...
    // todo: 10 is a carry-over from the pera-json-rpc implementation
    ...
    // TODO: move off dependency on PeraMoveValue
    let pera_move_value: PeraMoveValue = result.clone().into();

    match pera_move_value {
        PeraMoveValue::Option(move_option) => match move_option.as_ref() {
            Some(move_value) => Ok(move_value.to_string()),
            None => Ok("".to_string()),
        },
        PeraMoveValue::Vector(_) => Err(DisplayRenderError::Vector(var_name.to_string())),
        _ => Ok(pera_move_value.to_string()),
    }
}
