...
use crate::types::stake::StakedPera;
use async_graphql::connection::Connection;
use async_graphql::*;
use pera_json_rpc::name_service::NameServiceConfig;
use pera_types::object::{Data, MoveObject as NativeMoveObject};
...
#[graphql(
    ...
    field(
        ...
        desc = "Determines whether a transaction can transfer this object, using the \
                TransferObjects transaction command or `pera::transfer::public_transfer`, both of \
...
pub(crate) enum IMoveObject {
    ...
    StakedPera(StakedPera),
    PeransRegistration(PeransRegistration),
}
...
impl MoveObject {
    ...
    async fn as_staked_pera(&self) -> Result<Option<StakedPera>> {
        match StakedPera::try_from(self) {
            Ok(coin) => Ok(Some(coin)),
            Err(StakedPeraDowncastError::NotAStakedPera) => Ok(None),
            Err(StakedPeraDowncastError::Bcs(e)) => Err(Error::Internal(format!(
                "Failed to deserialize StakedPera: {e}"
            )))
            .extend(),
        }
    }
    ...
    async fn as_perans_registration(&self, ctx: &Context<'_>) -> Result<Option<PeransRegistration>> {
        ...
        match PeransRegistration::try_from(self, &tag) {
            Ok(registration) => Ok(Some(registration)),
            Err(PeransRegistrationDowncastError::NotAPeransRegistration) => Ok(None),
            Err(PeransRegistrationDowncastError::Bcs(e)) => Err(Error::Internal(format!(
                "Failed to deserialize PeransRegistration: {e}",
            )))
            .extend(),
        }
    }
