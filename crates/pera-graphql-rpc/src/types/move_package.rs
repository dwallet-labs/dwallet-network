...
use crate::types::pera_address::addr;
...
impl Loader<PackageVersionKey> for Db {
    type Value = PeraAddress;
    type Error = Error;

    async fn load(
        &self,
        keys: &[PackageVersionKey],
    ) -> Result<HashMap<PackageVersionKey, PeraAddress>, Error> {
        use packages::dsl;
...
impl Loader<LatestKey> for Db {
    ...
    async fn load(&self, keys: &[LatestKey]) -> Result<HashMap<LatestKey, PeraAddress>, Error> {
        use packages::dsl;
        let other = diesel::alias!(packages as other);
