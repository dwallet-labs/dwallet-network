...
/// Wrap PeraNS Domain type to expose as a string scalar in GraphQL.
#[derive(Debug)]
pub(crate) struct Domain(NativeDomain);

#[derive(Enum, Copy, Clone, Eq, PartialEq)]
#[graphql(remote = "pera_json_rpc::name_service::DomainFormat")]
...
pub(crate) struct NativePeransRegistration {
    pub id: UID,
...
#[derive(Clone)]
pub(crate) struct PeransRegistration {
    /// Representation of this PeransRegistration as a generic Move object.
    pub super_: MoveObject,

    /// The deserialized representation of the Move object's contents.
    pub native: NativePeransRegistration,
}
...
pub(crate) enum PeransRegistrationDowncastError {
    NotAPeransRegistration,
    Bcs(bcs::Error),
}

#[Object]
impl PeransRegistration {
    pub(crate) async fn address(&self) -> PeraAddress {
    ...
    pub(crate) async fn staked_peras(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
        after: Option<object::Cursor>,
        last: Option<u64>,
        before: Option<object::Cursor>,
    ) -> Result<Connection<String, StakedPera>> {
        OwnerImpl::from(&self.super_.super_)
            .staked_peras(ctx, first, after, last, before)
            .await
    }

    /// The domain explicitly configured as the default domain pointing to this object.
    pub(crate) async fn default_perans_name(
        &self,
        ctx: &Context<'_>,
        format: Option<DomainFormat>,
    ) -> Result<Option<String>> {
        OwnerImpl::from(&self.super_.super_)
            .default_perans_name(ctx, format)
            .await
    }

    /// The PeransRegistration NFTs owned by this object. These grant the owner the capability to
    /// manage the associated domain.
    pub(crate) async fn perans_registrations(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
        after: Option<object::Cursor>,
        last: Option<u64>,
        before: Option<object::Cursor>,
    ) -> Result<Connection<String, PeransRegistration>> {
        OwnerImpl::from(&self.super_.super_)
            .perans_registrations(ctx, first, after, last, before)
            .await
    }
...
impl NameService {
    /// Lookup the PeraNS NameRecord for the given `domain` name. `config` specifies where to find
    ...
    /// Lookup the PeraNS Domain for the given `address`. `config` specifies where to find the domain
    ...
    pub(crate) async fn reverse_resolve_to_name(
        ...
        let field: Field<NativePeraAddress, NativeDomain> = object
            .native
            .to_rust()
            .ok_or_else(|| Error::Internal("Malformed Perans Domain".to_string()))?;
...
impl PeransRegistration {
    /// Query the database for a `page` of PeraNS registrations. The page uses the same cursor type
    ...
    pub(crate) async fn paginate(
        ...
        owner: PeraAddress,
        checkpoint_viewed_at: u64,
    ) -> Result<Connection<String, PeransRegistration>, Error> {
        let type_ = PeransRegistration::type_(config.package_address.into());
        ...
        Object::paginate_subtype(db, page, filter, checkpoint_viewed_at, |object| {
            ...
            let move_object = MoveObject::try_from(&object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a PeransRegistration, but it's not a Move Object.",
                ))
            })?;

            PeransRegistration::try_from(&move_object, &type_).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a PeransRegistration, but it is not."
                ))
            })
        })
        .await
    ...
    /// Return the type representing a `PeransRegistration` on chain. This can change from chain to
    /// chain (mainnet, testnet, devnet etc).
    pub(crate) fn type_(package: PeraAddress) -> StructTag {
        StructTag {
    ...
    // Because the type of the PeransRegistration object is not constant,
    // we need to take it in as a param.
    pub(crate) fn try_from(
        move_object: &MoveObject,
        tag: &StructTag,
    ) -> Result<Self, PeransRegistrationDowncastError> {
        if !move_object.native.is_type(tag) {
            return Err(PeransRegistrationDowncastError::NotAPeransRegistration);
        }

        Ok(Self {
            super_: move_object.clone(),
            native: bcs::from_bytes(move_object.native.contents())
                .map_err(PeransRegistrationDowncastError::Bcs)?,
        })
    }
}
