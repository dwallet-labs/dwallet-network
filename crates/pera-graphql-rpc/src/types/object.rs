...
use super::{owner::Owner, pera_address::PeraAddress, transaction_block::TransactionBlock};
...
pub(crate) struct ObjectFilter {
    ...
    pub owner: Option<PeraAddress>,

    /// Filter for live objects by their IDs.
    pub object_ids: Option<Vec<PeraAddress>>,
...
impl Object {
    ...
    pub(crate) async fn default_perans_name(
        ...
        OwnerImpl::from(self).default_perans_name(ctx, format).await
    }
    ...
    pub(crate) async fn owner(&self, ctx: &Context<'_>) -> Option<ObjectOwner> {
        ...
        match native.owner {
            O::AddressOwner(address) => {
                let address = PeraAddress::from(address);
...
impl ObjectFilter {
    ...
    /// it).
    fn keys(&self) -> Option<BTreeMap<PeraAddress, Option<u64>>> {
...
mod tests {
    ...
    fn test_owner_filter_intersection() {
        let f0 = ObjectFilter {
            owner: Some(PeraAddress::from_str("0x1").unwrap()),
            ..Default::default()
        };

        let f1 = ObjectFilter {
            owner: Some(PeraAddress::from_str("0x2").unwrap()),
            ..Default::default()
        };
    ...
    fn test_key_filter_intersection() {
        let i1 = PeraAddress::from_str("0x1").unwrap();
        let i2 = PeraAddress::from_str("0x2").unwrap();
        let i3 = PeraAddress::from_str("0x3").unwrap();
        let i4 = PeraAddress::from_str("0x4").unwrap();
