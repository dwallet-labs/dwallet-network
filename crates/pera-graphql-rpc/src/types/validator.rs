...
use async_graphql::*;
use pera_indexer::apis::governance_api::exchange_rates;
use pera_types::pera_system_state::pera_system_state_summary::PeraValidatorSummary as NativePeraValidatorSummary;
#[derive(Clone, Debug)]
pub(crate) struct Validator {
    pub validator_summary: NativePeraValidatorSummary,
...
impl Loader<u64> for Db {
    type Value = (
        ...
        BTreeMap<NativePeraAddress, Vec<(EpochId, PoolTokenExchangeRate)>>,
    );
    type Error = Error;
    ...
    > {
        let latest_pera_system_state = self
            ...
            .await
            .map_err(|_| Error::Internal("Failed to fetch latest Pera system state".to_string()))?;
        let governance_api = GovernanceReadApi::new(self.inner.clone());
        let exchange_rates = exchange_rates(&governance_api, &latest_pera_system_state)
            .await
        ...
        let epoch_to_filter_out = if let Some(epoch) = keys.first() {
            if epoch == &latest_pera_system_state.epoch {
                *epoch - 1
            } else {
                *epoch
            }
        } else {
            latest_pera_system_state.epoch - 1
        };
        ...
        let requested_epoch = match keys.first() {
            Some(x) => *x,
            None => latest_pera_system_state.epoch,
        };

        let mut r = HashMap::new();
        r.insert(
            requested_epoch,
            (latest_pera_system_state.stake_subsidy_start_epoch, results),
        );
...
impl Validator {
    ...
    async fn address(&self) -> Address {
        Address {
            address: PeraAddress::from(self.validator_summary.pera_address),
    ...
    async fn staking_pool_id(&self) -> PeraAddress {
    ...
    async fn staking_pool_pera_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.staking_pool_pera_balance,
        ))
    }
    ...
    async fn pending_total_pera_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.pending_total_pera_withdraw,
        ))
    }
    ...
    async fn apy(&self, ctx: &Context<'_>) -> Result<Option<u64>, Error> {
        ...
        let rates = exchange_rates
            .get(&self.validator_summary.pera_address)
            .ok_or_else(|| {
                Error::Internal(format!(
                    "Failed to get the exchange rate for this validator address {} for requested epoch {}",
                    self.validator_summary.pera_address, self.requested_for_epoch
                ))
            })?;
...
impl Validator {
    pub fn operation_cap_id(&self) -> PeraAddress {
        PeraAddress::from_array(**self.validator_summary.operation_cap_id)
    }
}
