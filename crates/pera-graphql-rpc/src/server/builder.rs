...
impl ServerBuilder {
    ...
    pub async fn from_config(
        ...
        let pera_sdk_client = if let Some(url) = &config.tx_exec_full_node.node_rpc_url {
            Some(
                PeraClientBuilder::default()
                    ...
                    .build(url)
                    .await
                    .map_err(|e| Error::Internal(format!("Failed to create PeraClient: {}", e)))?,
            )
        } else {
...
async fn graphql_handler(
    ...
    schema: Extension<PeraGraphQLSchema>,
    ...
    pub async fn test_timeout_impl(wallet: &WalletContext) {
        ...
        async fn test_timeout(
            ...
            let schema = prep_schema(None, Some(cfg))
                .context_data(Some(pera_client.clone()))
                .extension(Timeout)
        ...
        let pera_client = wallet.get_client().await.unwrap();

        test_timeout(delay, timeout, query, &pera_client)
            .await
            .into_result()
            .expect("Should complete successfully");

        // Should timeout
        let errs: Vec<_> = test_timeout(delay, delay, query, &pera_client)
            .await
            .into_result()
            .unwrap_err()
            .into_iter()
