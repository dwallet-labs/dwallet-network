...
use pera_indexer::apis::{governance_api::exchange_rates, GovernanceReadApi};
        ...
        loop {
            tokio::select! {
                ...
                _ = self.epoch_rx.changed() => {
                    ...
                    let latest_pera_sytem_state = self.db.inner.spawn_blocking(move |this|
                        this.get_latest_pera_system_state()
                    ).await.map_err(|_| error!("Failed to fetch latest Pera system state"));

                    if let Ok(latest_pera_sytem_state) = latest_pera_sytem_state {
                        let db = self.db.clone();
                        let governance_api = GovernanceReadApi::new(db.inner) ;
                        exchange_rates(&governance_api, &latest_pera_sytem_state)
                            .await
