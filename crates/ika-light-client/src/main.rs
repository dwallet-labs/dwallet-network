...
use ika_json_rpc_types::{IkaObjectDataOptions, IkaTransactionBlockResponseOptions};

use ika_rest_api::CheckpointData;
use ika_types::{
    base_types::ObjectID,
    committee::Committee,
...
use ika_package_resolver::Result as ResolverResult;
use ika_package_resolver::{Package, PackageStore, Resolver};
...
/// A light client for the Ika blockchain
#[derive(Parser, Debug)]
...
async fn get_verified_effects_and_events(
    ...
    let ika_mainnet: ika_sdk::IkaClient = IkaClientBuilder::default()
        .build(config.full_node_url.as_str())
        .await
        .unwrap();
    let read_api = ika_mainnet.read_api();

    info!("Getting effects and events for TID: {}", tid);
    // Lookup the transaction id and get the checkpoint sequence number
    let options = IkaTransactionBlockResponseOptions::new();
    let seq = read_api
...
async fn get_verified_object(config: &Config, id: ObjectID) -> anyhow::Result<Object> {
    let ika_client: Arc<ika_sdk::IkaClient> = Arc::new(
    ...
    let read_api = ika_client.read_api();
