...
use pera_types::coin::PAY_JOIN_FUNC_NAME;
use pera_types::coin::PAY_MODULE_NAME;
use pera_types::coin::PAY_SPLIT_VEC_FUNC_NAME;
...
pub enum CommonTransactionCosts {
    ...
    SplitCoin(usize),
    TransferWholeCoin,
    TransferWholePeraCoin,
    TransferPortionPeraCoin,
    SharedCounterCreate,
...
// 1. Run `cargo insta test --review` under `./pera-cost`.
...
async fn create_txes(
    ...
    //
    // Transfer Whole Pera Coin and Transfer Portion of Pera Coin
    //
    let whole_pera_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_pera(None, PeraAddress::default())
            .build();
    let partial_pera_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_pera(Some(10), PeraAddress::default())
            .build();
    ret.insert(
        CommonTransactionCosts::TransferWholePeraCoin,
        whole_pera_coin_tx,
    );
    ret.insert(
        CommonTransactionCosts::TransferPortionPeraCoin,
        partial_pera_coin_tx,
    );
    ...
    let whole_coin_tx = TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
        .transfer(gas_objects.pop().unwrap(), PeraAddress::default())
        .build();
