...
use ika_json_rpc_api::{GovernanceReadApiOpenRpc, GovernanceReadApiServer, JsonRpcMetrics};
use ika_json_rpc_types::{DelegatedStake, Stake, StakeStatus};
use ika_json_rpc_types::{IkaCommittee, ValidatorApy, ValidatorApys};
use ika_open_rpc::Module;
use ika_types::base_types::{ObjectID, IkaAddress};
use ika_types::committee::EpochId;
use ika_types::dynamic_field::get_dynamic_field_from_store;
use ika_types::error::{IkaError, UserInputError};
use ika_types::governance::StakedIka;
use ika_types::id::ID;
use ika_types::object::ObjectRead;
use ika_types::ika_serde::BigInt;
use ika_types::ika_system_state::ika_system_state_summary::IkaSystemStateSummary;
use ika_types::ika_system_state::PoolTokenExchangeRate;
use ika_types::ika_system_state::IkaSystemStateTrait;
use ika_types::ika_system_state::{get_validator_from_table, IkaSystemState};

use crate::authority_state::StateRead;
use crate::error::{Error, RpcInterimResult, IkaRpcInputError};
use crate::{with_tracing, ObjectProvider, IkaRpcModule};
...
impl GovernanceReadApi {
    ...
    async fn get_staked_ika(&self, owner: IkaAddress) -> Result<Vec<StakedIka>, Error> {
        let state = self.state.clone();
        let result =
            spawn_monitored_task!(async move { state.get_staked_ika(owner).await }).await??;

        self.metrics
            .get_stake_ika_result_size
            .observe(result.len() as f64);
        self.metrics
            .get_stake_ika_result_size_total
    ...
    async fn get_stakes_by_ids(
        ...
        let mut stakes: Vec<(StakedIka, bool)> = vec![];
        for stake in stakes_read.into_iter() {
            match stake {
                ObjectRead::Exists(_, o, _) => stakes.push((StakedIka::try_from(&o)?, true)),
                ObjectRead::Deleted(oref) => {
                    match self
                        ...
                        .await?
                    {
                        Some(o) => stakes.push((StakedIka::try_from(&o)?, false)),
                        None => Err(IkaRpcInputError::UserInputError(
                ...
                ObjectRead::NotExists(id) => Err(IkaRpcInputError::UserInputError(
                    UserInputError::ObjectNotFound {
                        object_id: id,
                        version: None,
                    },
    ...
    async fn get_stakes(&self, owner: IkaAddress) -> Result<Vec<DelegatedStake>, Error> {
        let timer = self.metrics.get_stake_ika_latency.start_timer();
        let stakes = self.get_staked_ika(owner).await?;
        if stakes.is_empty() {
            return Ok(vec![]);
        }
        drop(timer);

        let _timer = self.metrics.get_delegated_ika_latency.start_timer();
    ...
    async fn get_delegated_stakes(
        ...
        let system_state_summary: IkaSystemStateSummary =
            system_state.clone().into_ika_system_state_summary();
    ...
    fn get_system_state(&self) -> Result<IkaSystemState, Error> {
...
impl GovernanceReadApiServer for GovernanceReadApi {
    ...
    async fn get_stakes_by_ids(
        ...
        with_tracing!(async move { self.get_stakes_by_ids(staked_ika_ids).await })
    }
    ...
    #[instrument(skip(self))]
    async fn get_committee_info(&self, epoch: Option<BigInt<u64>>) -> RpcResult<IkaCommittee> {
    ...
    async fn get_validators_apy(&self) -> RpcResult<ValidatorApys> {
        ...
        let system_state_summary: IkaSystemStateSummary =
            self.get_latest_ika_system_state().await?;
...
async fn exchange_rates(
    ...
    let system_state_summary: IkaSystemStateSummary = system_state.into_ika_system_state_summary();
    ...
    for df in state.get_dynamic_fields(
        ...
        let pool_id: ID =
            bcs::from_bytes(&df.1.bcs_name).map_err(|e| IkaError::ObjectDeserializationError {
                error: e.to_string(),
            })?;
    ...
    for (address, pool_id, exchange_rates_id, exchange_rates_size, active) in tables {
        let mut rates = state
            ...
            .map(|df| {
                ...
                Ok::<_, IkaError>((epoch, exchange_rate))
            })
            .collect::<Result<Vec<_>, _>>()?;
...
impl IkaRpcModule for GovernanceReadApi {
