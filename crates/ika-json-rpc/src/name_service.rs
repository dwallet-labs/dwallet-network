...
use ika_types::dynamic_field::Field;
use ika_types::id::{ID, UID};
use ika_types::object::{MoveObject, Object};
...
const DEFAULT_TLD: &str = "ika";
const ACCEPTED_SEPARATORS: [char; 2] = ['.', '*'];
const IKA_NEW_FORMAT_SEPARATOR: char = '@';

/// Two different view options for a domain.
/// `At` -> `test@example` | `Dot` -> `test.example.ika`
...
pub struct Registry {
    ...
    reverse_registry: Table<IkaAddress, Domain>,
}

/// Rust version of the Move ika::table::Table type.
...
impl Domain {
    pub fn type_(package_address: IkaAddress) -> StructTag {
        StructTag {
    ...
    /// E.g. `test.example.ika` -> `example.ika`
    ///
    ...
    pub fn format(&self, format: DomainFormat) -> String {
        ...
        format!("{}{}{}", labels.join(sep), IKA_NEW_FORMAT_SEPARATOR, sld)
    }
}
...
pub struct NameServiceConfig {
    pub package_address: IkaAddress,
...
impl NameServiceConfig {
    ...
    pub fn record_field_id(&self, domain: &Domain) -> ObjectID {
        ...
        ika_types::dynamic_field::derive_dynamic_field_id(
            self.registry_id,
...
impl Default for NameServiceConfig {
    fn default() -> Self {
        let package_address = IkaAddress::from_str(NAME_SERVICE_DEFAULT_PACKAGE_ADDRESS).unwrap();
...
/// E.g.  `example.ika` -> `.` | example*ika -> `@` | `example*ika` -> `*`
...
/// Converts @label ending to label{separator}ika ending.
///
/// E.g. `@example` -> `example.ika` | `test@example` -> `test.example.ika`
fn convert_from_new_format(s: &str, separator: &char) -> Result<String, NameServiceError> {
    let mut splits = s.split(IKA_NEW_FORMAT_SEPARATOR);
...
pub struct NameRecord {
    ...
    pub target_address: Option<IkaAddress>,
...
/// A IkansRegistration object to manage an SLD
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct IkansRegistration {
    pub id: UID,
    pub domain: Domain,
