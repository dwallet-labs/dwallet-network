...
use ika_json_rpc_api::{TRANSACTION_EXECUTION_CLIENT_ERROR_CODE, TRANSIENT_ERROR_CODE};
use ika_types::committee::{QUORUM_THRESHOLD, TOTAL_VOTING_POWER};
use ika_types::error::{IkaError, IkaObjectResponseError, UserInputError};
use ika_types::quorum_driver_types::QuorumDriverError;
use thiserror::Error;
...
pub enum Error {
    ...
    #[error(transparent)]
    IkaObjectResponseError(#[from] IkaObjectResponseError),

    #[error(transparent)]
    IkaRpcInputError(#[from] IkaRpcInputError),
...
impl From<IkaError> for Error {
    fn from(e: IkaError) -> Self {
        match e {
            IkaError::UserInputError { error } => Self::UserInputError(error),
            IkaError::IkaObjectResponseError { error } => Self::IkaObjectResponseError(error),
            IkaError::UnsupportedFeatureError { error } => Self::UnsupportedFeature(error),
            IkaError::IndexStoreNotAvailable => Self::UnsupportedFeature(
                "Required indexes are not available on this node".to_string(),
            ),
            other => Self::IkaError(other),
        }
    }
}
...
impl From<Error> for RpcError {
    ...
    fn from(e: Error) -> RpcError {
        match e {
            ...
            Error::IkaObjectResponseError(err) => match err {
                IkaObjectResponseError::NotExists { .. }
                | IkaObjectResponseError::DynamicFieldNotFound { .. }
                | IkaObjectResponseError::Deleted { .. }
                | IkaObjectResponseError::DisplayError { .. } => {
            ...
            Error::IkaRpcInputError(err) => RpcError::Call(CallError::InvalidParams(err.into())),
            Error::IkaError(ika_error) => match ika_error {
                IkaError::TransactionNotFound { .. }
                | IkaError::TransactionsNotFound { .. }
                | IkaError::TransactionEventsNotFound { .. } => {
                    RpcError::Call(CallError::InvalidParams(ika_error.into()))
                }
                _ => RpcError::Call(CallError::Failed(ika_error.into())),
            },
                        ...
                        let new_errors: Vec<String> = errors
                            ...
                            .filter_map(|(err, _, _)| {
                                match &err {
                                    ...
                                    IkaError::UserInputError { error } => Some(error.to_string()),
                                    _ => {
...
pub enum IkaRpcInputError {
    ...
    #[error("{0}")]
    CannotParseIkaStructTag(String),
...
impl From<IkaRpcInputError> for RpcError {
    fn from(e: IkaRpcInputError) -> Self {
...
mod tests {
    ...
    use ika_types::crypto::AuthorityPublicKey;
    ...
    mod match_quorum_driver_error_tests {
        ...
        fn test_invalid_user_signature() {
            let quorum_driver_error =
                QuorumDriverError::InvalidUserSignature(IkaError::InvalidSignature {
        ...
        fn test_objects_double_used() {
            use ika_types::crypto::VerifyingKey;
        ...
        fn test_non_recoverable_transaction_error_with_transient_errors() {
            let quorum_driver_error = QuorumDriverError::NonRecoverableTransactionError {
                errors: vec![
                    ...
                    (
                        IkaError::RpcError("Hello".to_string(), "Testing".to_string()),
                        0,
                        vec![],
                    ),
                ],
        ...
        fn test_quorum_driver_internal_error() {
            let quorum_driver_error = QuorumDriverError::QuorumDriverInternalError(
                IkaError::UnexpectedMessage("test".to_string()),
            );
        ...
        fn test_system_overload() {
            let quorum_driver_error = QuorumDriverError::SystemOverload {
                overloaded_stake: 10,
                errors: vec![(IkaError::UnexpectedMessage("test".to_string()), 0, vec![])],
            };
