...
use ika_json_rpc_api::{MoveUtilsOpenRpc, MoveUtilsServer};
use ika_json_rpc_types::{
    MoveFunctionArgType, ObjectValueKind, IkaMoveNormalizedFunction, IkaMoveNormalizedModule,
    IkaMoveNormalizedStruct,
};
use ika_open_rpc::Module;
use ika_types::base_types::ObjectID;
use ika_types::move_package::normalize_modules;
use ika_types::object::{Data, ObjectRead};
use tap::TapFallible;
...
impl MoveUtilsInternalTrait for MoveUtilsInternal {
    ...
    async fn get_move_module(
        ...
        Ok(match normalized.get(&module_name) {
            ...
            None => Err(IkaRpcInputError::GenericNotFound(format!(
        ...
        match object_read {
            ObjectRead::Exists(_obj_ref, object, _layout) => {
                match object.into_inner().data {
                    ...
                    _ => Err(IkaRpcInputError::GenericInvalid(format!(
                        "Object is not a package with ID {}",
                        package
                    )))?,
                }
            }
            _ => Err(IkaRpcInputError::GenericNotFound(format!(
...
impl IkaRpcModule for MoveUtils {
    ...
    async fn get_normalized_move_struct(
        ...
        with_tracing!(async move {
            ...
            let identifier = Identifier::new(struct_name.as_str())
                .map_err(|e| IkaRpcInputError::GenericInvalid(format!("{e}")))?;
...
mod tests {
    ...
    mod get_normalized_move_module_tests {
        ...
        async fn test_success_response() {
            ...
            let expected_module: IkaMoveNormalizedModule = normalized_module.clone().into();
        ...
        async fn test_no_module_found() {
            ...
            let expected_error =
                Error::IkaRpcInputError(IkaRpcInputError::GenericNotFound(error_string.clone()));
            mock_internal
                .expect_get_move_module()
