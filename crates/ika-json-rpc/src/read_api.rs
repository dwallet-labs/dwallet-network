...
use ika_json_rpc_types::{
    ...
    ObjectChange, ProtocolConfigResponse, IkaEvent, IkaGetPastObjectRequest, IkaMoveStruct,
    IkaMoveValue, IkaMoveVariant, IkaObjectDataOptions, IkaObjectResponse, IkaPastObjectResponse,
    IkaTransactionBlock, IkaTransactionBlockEvents, IkaTransactionBlockResponse,
...
use crate::error::{Error, RpcInterimResult, IkaRpcInputError};
use crate::{
    get_balance_changes_from_effect, get_object_changes, ObjectProviderCache, IkaRpcModule,
};
...
struct IntermediateTransactionResponse {
    ...
    events: Option<IkaTransactionBlockEvents>,
...
impl ReadApi {
    ...
    async fn multi_get_transaction_blocks_internal(
        ...
        opts: Option<IkaTransactionBlockResponseOptions>,
    ) -> Result<Vec<IkaTransactionBlockResponse>, Error> {
        trace!("start");

        let num_digests = digests.len();
        if num_digests > *QUERY_MAX_RESULT_LIMIT {
            Err(IkaRpcInputError::SizeLimitExceeded(
        ...
        if temp_response.len() < num_digests {
            Err(IkaRpcInputError::ContainsDuplicates)?
        }
            ...
            for (_, cache_entry) in temp_response.iter_mut() {
                    ...
                    cache_entry.effects.as_ref().and_then(|e| e.events_digest())
                {
                    let events = events_digest_to_events
                        ...
                        .map(|events| to_ika_transaction_events(self, cache_entry.digest, events));
                    match events {
                    ...
                    cache_entry.events = Some(IkaTransactionBlockEvents::default());
                }
            }
        }
            ...
            for resp in temp_response.values() {
                ...
                results.push(get_balance_changes_from_effect(
                    ...
                    resp.effects.as_ref().ok_or_else(|| {
                        IkaRpcInputError::GenericNotFound(
...
impl ReadApiServer for ReadApi {
    ...
    async fn get_object(
        &self,
        object_id: ObjectID,
        options: Option<IkaObjectDataOptions>,
                ...
                ObjectRead::Exists(object_ref, o, layout) => {
                    ...
                    if options.show_display {
                            ...
                            .await
                        {
                            ...
                            Err(e) => {
                                return Ok(IkaObjectResponse::new(
                                    Some((object_ref, o, layout, options, None).try_into()?),
                                    Some(IkaObjectResponseError::DisplayError {
                                        error: e.to_string(),
                                    }),
                                ));
                            }
                    ...
                    Ok(IkaObjectResponse::new_with_data(
    ...
    async fn multi_get_objects(
        ...
        options: Option<IkaObjectDataOptions>,
    ) -> RpcResult<Vec<IkaObjectResponse>> {
        with_tracing!(async move {
            if object_ids.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                let objects_result: Result<Vec<IkaObjectResponse>, String> = results
                    .into_iter()
    ...
    async fn try_get_past_object(
        ...
        options: Option<IkaObjectDataOptions>,
    ) -> RpcResult<IkaPastObjectResponse> {
        with_tracing!(async move {
            let state = self.state.clone();
            ...
            match past_read {
                PastObjectRead::ObjectNotExists(id) => {
                    Ok(IkaPastObjectResponse::ObjectNotExists(id))
                }
                PastObjectRead::VersionFound(object_ref, o, layout) => {
                    ...
                    Ok(IkaPastObjectResponse::VersionFound(
                        (object_ref, o, layout, options, display_fields).try_into()?,
                    ))
                }
                PastObjectRead::ObjectDeleted(oref) => {
                    Ok(IkaPastObjectResponse::ObjectDeleted(oref.into()))
                }
                PastObjectRead::VersionNotFound(id, seq_num) => {
                    Ok(IkaPastObjectResponse::VersionNotFound(id, seq_num))
                }
                PastObjectRead::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                } => Ok(IkaPastObjectResponse::VersionTooHigh {
                    object_id,
                    asked_version,
                    latest_version,
                }),
    ...
    async fn try_get_object_before_version(
        ...
        self.try_get_past_object(
            object_id,
            version,
            Some(IkaObjectDataOptions::bcs_lossless()),
        )
        .await
    }
    ...
    async fn try_multi_get_past_objects(
        ...
        options: Option<IkaObjectDataOptions>,
    ) -> RpcResult<Vec<IkaPastObjectResponse>> {
        with_tracing!(async move {
            if past_objects.len() <= *QUERY_MAX_RESULT_LIMIT {
                ...
                if !errors.is_empty() {
                    let error_string = errors
                        ...
                        .collect::<Vec<String>>()
                        .join("; ");
                    Err(anyhow!("{error_string}").into()) // Collects errors not related to IkaPastObjectResponse variants
                } else {
                    Ok(success)
                }
            } else {
            ...
            if opts.show_events && temp_response.effects.is_some() {
                ...
                if let Some(event_digest) = temp_response.effects.as_ref().unwrap().events_digest()
                {
                    ...
                    let events = spawn_monitored_task!(async move {
                        ...
                        .await
                        .map_err(Error::from)??;
                    match to_ika_transaction_events(self, digest, events) {
                    ...
                    // events field will be Some if and only if `show_events` is true and
                    // there is no error in converting fetching events
                    temp_response.events = Some(IkaTransactionBlockEvents::default());
                }
            }
    ...
    async fn multi_get_transaction_blocks(
        ...
        digests: Vec<TransactionDigest>,
        opts: Option<IkaTransactionBlockResponseOptions>,
    ) -> RpcResult<Vec<IkaTransactionBlockResponse>> {
        with_tracing!(async move {
    ...
    #[instrument(skip(self))]
    async fn get_events(&self, transaction_digest: TransactionDigest) -> RpcResult<Vec<IkaEvent>> {
        with_tracing!(async move {
            let state = self.state.clone();
            let transaction_kv_store = self.transaction_kv_store.clone();
            spawn_monitored_task!(async move{
        ...
        with_tracing!(async move {
            version
                .map(|v| {
                    ...
                    .ok_or(IkaRpcInputError::ProtocolVersionUnsupported(
...
#[instrument(skip_all)]
fn to_ika_transaction_events(
    fullnode_api: &ReadApi,
    tx_digest: TransactionDigest,
    events: TransactionEvents,
) -> Result<IkaTransactionBlockEvents, Error> {
    ...
    Ok(IkaTransactionBlockEvents::try_from(
        events,
        tx_digest,
        None,
...
pub fn get_rendered_fields(
    ...
    let ika_move_value: IkaMoveValue = MoveValue::Struct(move_struct.clone()).into();
    if let IkaMoveValue::Struct(move_struct) = ika_move_value {
        let fields =
            fields
                .contents
                .iter()
...
fn parse_template(template: &str, move_struct: &IkaMoveStruct) -> Result<String, Error> {
...
fn get_value_from_move_struct(
    ...
    let mut current_value = &IkaMoveValue::Struct(move_struct.clone());
    // iterate over the parts and try to access the corresponding field
    for part in parts {
        match current_value {
            IkaMoveValue::Struct(move_struct) => {
                if let IkaMoveStruct::WithTypes { type_: _, fields }
                | IkaMoveStruct::WithFields(fields) = move_struct
                {
    ...
    match current_value {
        IkaMoveValue::Option(move_option) => match move_option.as_ref() {
            Some(move_value) => Ok(move_value.to_string()),
            None => Ok("".to_string()),
        },
        IkaMoveValue::Vector(_) => Err(anyhow!(
...
fn convert_to_response(
    ...
    opts: &IkaTransactionBlockResponseOptions,
    module_cache: &impl GetModule,
) -> RpcInterimResult<IkaTransactionBlockResponse> {
    let mut response = IkaTransactionBlockResponse::new(cache.digest);
    ...
    if let Some(transaction) = cache.transaction {
        ...
        if opts.show_input {
            response.transaction = Some(IkaTransactionBlock::try_from(
