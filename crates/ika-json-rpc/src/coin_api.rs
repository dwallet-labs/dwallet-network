...
use ika_core::jsonrpc_index::TotalBalance;
use tap::TapFallible;
use tracing::{debug, info, instrument};

use mysten_metrics::spawn_monitored_task;
use ika_core::authority::AuthorityState;
use ika_json_rpc_api::{cap_page_limit, CoinReadApiOpenRpc, CoinReadApiServer, JsonRpcMetrics};
use ika_json_rpc_types::Balance;
use ika_json_rpc_types::{CoinPage, IkaCoinMetadata};
...
use ika_types::coin::{CoinMetadata, TreasuryCap};
use ika_types::effects::TransactionEffectsAPI;
use ika_types::gas_coin::{GAS, TOTAL_SUPPLY_NIKA};
...
use crate::{with_tracing, IkaRpcModule};

pub fn parse_to_struct_tag(coin_type: &str) -> Result<StructTag, IkaRpcInputError> {
    parse_ika_struct_tag(coin_type)
        .map_err(|e| IkaRpcInputError::CannotParseIkaStructTag(format!("{e}")))
}

pub fn parse_to_type_tag(coin_type: Option<String>) -> Result<TypeTag, IkaRpcInputError> {
...
impl IkaRpcModule for CoinReadApi {
...
impl CoinReadApiServer for CoinReadApi {
            ...
            let cursor = match cursor {
                Some(c) => {
                    let obj = self.internal.get_object(&c).await?.ok_or_else(|| {
                        IkaRpcInputError::GenericInvalid("cursor not found".to_string())
                    })?;
                    let coin = obj.as_coin_maybe().ok_or_else(|| {
                        IkaRpcInputError::GenericInvalid("cursor is not a coin".to_string())
                    })?;
                    ...
                    match obj {
                        Some(obj) => {
                            ...
                            if coin_type.is_none() {
                                Err(IkaRpcInputError::GenericInvalid(
                                    "cursor is not a coin".to_string(),
                                ))
                            } else {
                                let coin = obj.as_coin_maybe().ok_or_else(|| {
                                    IkaRpcInputError::GenericInvalid(
                                        "cursor is not a coin".to_string(),
                                    )
                                })?;
                                Ok((
                        ...
                        None => Err(IkaRpcInputError::GenericInvalid(
    ...
    #[instrument(skip(self))]
    async fn get_all_balances(&self, owner: IkaAddress) -> RpcResult<Vec<Balance>> {
        with_tracing!(async move {
    ...
    #[instrument(skip(self))]
    async fn get_coin_metadata(&self, coin_type: String) -> RpcResult<Option<IkaCoinMetadata>> {
        with_tracing!(async move {
...
async fn find_package_object_id(
    ...
    spawn_monitored_task!(async move {
        ...
        Err(IkaRpcInputError::GenericNotFound(format!(
...
mod tests {
    ...
    use ika_storage::key_value_store::{
    ...
    use ika_types::coin::TreasuryCap;
    use ika_types::digests::{ObjectDigest, TransactionDigest, TransactionEventsDigest};
    ...
    use ika_types::{parse_ika_struct_tag, TypeTag};
    ...
    mock! {
        ...
        impl TransactionKeyValueStoreTrait for KeyValueStore {
            ...
            ) -> IkaResult<KVStoreTransactionData>;
            ...
            ) -> IkaResult<KVStoreCheckpointData>;
            ...
            async fn get_object(&self, object_id: ObjectID, version: SequenceNumber) -> IkaResult<Option<Object>>;
    ...
    fn get_test_coin_type_tag(coin_type: String) -> TypeTag {
        TypeTag::Struct(Box::new(parse_ika_struct_tag(&coin_type).unwrap()))
    }
    ...
    fn get_test_treasury_cap_peripherals(
        ...
        let input_coin_struct = parse_ika_struct_tag(&coin_name).expect("should not fail");
    ...
    mod get_coins_tests {
        ...
        async fn test_coin_no_cursor() {
            ...
            let coin_type_tag =
                TypeTag::Struct(Box::new(parse_ika_struct_tag(&coin.coin_type).unwrap()));
            let mut mock_state = MockStateRead::new();
            mock_state
                .expect_get_owned_coins()
                .with(
        ...
        async fn test_coin_with_cursor() {
            ...
            let coin_type_tag =
                TypeTag::Struct(Box::new(parse_ika_struct_tag(&coins[0].coin_type).unwrap()));
            let mut mock_state = MockStateRead::new();
            mock_state
                .expect_get_owned_coins()
                .with(
        ...
        async fn test_unrecognized_token() {
            ...
            expected.assert_eq(&error_object.code().to_string());
            let expected =
                expect!["Invalid struct type: 0x2::ika:🤵. Got error: unrecognized token: :🤵"];
            expected.assert_eq(error_object.message());
        }
            ...
            mock_state
                ...
                .returning(move |_, _, _, _| {
                    Err(StateReadError::Client(
                        IkaError::IndexStoreNotAvailable.into(),
                    ))
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
        ...
        async fn test_get_coins_iterator_typed_store_error() {
            ...
            mock_state
                .expect_get_owned_coins()
                .returning(move |_, _, _, _| {
                    Err(IkaError::Storage("mock rocksdb error".to_string()).into())
                });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_coins(owner, Some(coin_type.to_string()), None, None)
    ...
    mod get_balance_tests {
        ...
        async fn test_get_balance_execution_error() {
            ...
            let mut mock_state = MockStateRead::new();
            mock_state.expect_get_balance().returning(move |_, _| {
                Err(IkaError::ExecutionError("mock db error".to_string()).into())
            });
            let coin_read_api = CoinReadApi::new_for_tests(Arc::new(mock_state), None);
            let response = coin_read_api
                .get_balance(owner, Some(coin_type.to_string()))
    ...
    mod get_coin_metadata_tests {
        ...
        async fn test_valid_coin_metadata_object() {
            ...
            let metadata = IkaCoinMetadata::try_from(coin_metadata_object.clone()).unwrap();
        ...
        #[tokio::test]
        async fn test_find_package_object_not_ika_coin_metadata() {
            let package_id = get_test_package_id();
            let coin_name = get_test_coin_type(package_id);
            let input_coin_struct = parse_ika_struct_tag(&coin_name).expect("should not fail");
            let coin_metadata_struct = CoinMetadata::type_(input_coin_struct.clone());
