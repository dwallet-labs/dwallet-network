...
use ika_json_rpc_api::{TransactionBuilderOpenRpc, TransactionBuilderServer};
use ika_json_rpc_types::{RPCTransactionRequestParams, IkaObjectDataFilter};
use ika_json_rpc_types::{
    IkaObjectDataOptions, IkaObjectResponse, IkaTransactionBlockBuilderMode, IkaTypeTag,
    TransactionBlockBytes,
};
use ika_open_rpc::Module;
use ika_transaction_builder::{DataReader, TransactionBuilder};
use ika_types::base_types::ObjectInfo;
use ika_types::base_types::{ObjectID, IkaAddress};
use ika_types::ika_serde::BigInt;

use crate::authority_state::StateRead;
use crate::IkaRpcModule;
    ...
    async fn get_owned_objects(
        ...
        Ok(self
            ...
            .get_owner_objects(
                address,
                None,
                Some(IkaObjectDataFilter::StructType(object_type)),
            )?)
    }
...
impl TransactionBuilderServer for TransactionBuilderApi {
    ...
    async fn pay_all_ika(
        ...
        let data = self
            .0
            .pay_all_ika(signer, input_coins, recipient, *gas_budget)
            .await?;
        Ok(TransactionBlockBytes::from_data(data)?)
    }
    ...
    async fn move_call(
        ...
        rpc_arguments: Vec<IkaJsonValue>,
        gas: Option<ObjectID>,
        gas_budget: BigInt<u64>,
        _txn_builder_mode: Option<IkaTransactionBlockBuilderMode>,
    ) -> RpcResult<TransactionBlockBytes> {
    ...
    async fn request_withdraw_stake(
        ...
        Ok(TransactionBlockBytes::from_data(
            self.0
                .request_withdraw_stake(signer, staked_ika, gas, *gas_budget)
                .await?,
        )?)
    }
}

impl IkaRpcModule for TransactionBuilderApi {
