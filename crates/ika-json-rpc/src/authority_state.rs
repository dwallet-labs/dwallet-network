...
use ika_core::execution_cache::ObjectCacheRead;
use ika_core::jsonrpc_index::TotalBalance;
use ika_core::subscription_handler::SubscriptionHandler;
use ika_json_rpc_types::{
    Coin as IkaCoin, DevInspectResults, DryRunTransactionBlockResponse, EventFilter, IkaEvent,
    IkaObjectDataFilter, TransactionFilter,
};
...
use ika_types::base_types::{
    MoveObjectType, ObjectID, ObjectInfo, ObjectRef, SequenceNumber, IkaAddress,
};
use ika_types::bridge::Bridge;
use ika_types::committee::{Committee, EpochId};
use ika_types::digests::{ChainIdentifier, TransactionDigest, TransactionEventsDigest};
...
use ika_types::storage::{BackingPackageStore, ObjectStore, WriteKind};
...
pub trait StateRead: Send + Sync {
    ...
    // governance_api
    async fn get_staked_ika(&self, owner: IkaAddress) -> StateReadResult<Vec<StakedIka>>;
    fn get_system_state(&self) -> StateReadResult<IkaSystemState>;
...
impl StateRead for AuthorityState {
    ...
    async fn get_staked_ika(&self, owner: IkaAddress) -> StateReadResult<Vec<StakedIka>> {
        Ok(self
            .get_move_objects(owner, MoveObjectType::staked_ika())
            .await?)
    }
    fn get_system_state(&self) -> StateReadResult<IkaSystemState> {
        Ok(self
    ...
    async fn get_balance(
        ...
        Ok(self
            .indexes
            .as_ref()
            .ok_or(IkaError::IndexStoreNotAvailable)?
...
pub enum StateReadError {
    // ika_json_rpc::Error will do the final conversion to generic error message
    #[error(transparent)]
...
impl From<IkaError> for StateReadError {
    fn from(e: IkaError) -> Self {
        match e {
            IkaError::IndexStoreNotAvailable
            | IkaError::TransactionNotFound { .. }
            | IkaError::UnsupportedFeatureError { .. }
            | IkaError::UserInputError { .. }
            | IkaError::WrongMessageVersion { .. } => StateReadError::Client(e.into()),
