...
/// Add the new service to the ika-services dockerfile in the ika repository
fn add_to_ika_dockerfile(path: &Path) -> Result<()> {
    let path = path.canonicalize().context("canonicalizing service path")?;
    let crates_dir = path.parent().unwrap();
    if !crates_dir.ends_with("ika/crates") {
        panic!("directory wasn't in the ika repo");
    }
    let ika_services_dockerfile_path = &crates_dir.join("../docker/ika-services/Dockerfile");
    // read the dockerfile
    let dockerfile = fs::read_to_string(ika_services_dockerfile_path)
        .context("reading ika-services dockerfile")?;
    ...
    let build_line = dockerfile
        ...
        .expect("couldn't find build line in ika-services dockerfile")
        .0;
    ...
    // write the file back
    fs::write(ika_services_dockerfile_path, final_dockerfile.join("\n"))
        .context("writing ika-services dockerfile after modifying it")?;
...
fn create_rust_service(path: &Path) -> Result<()> {
    ...
    let is_ika_service = path
        // expand relative paths and symlinks
        .canonicalize()
        .context("canonicalizing service path")?
        .to_string_lossy()
        .contains("ika/crates");
    debug!("ika service: {:?}", is_ika_service);
    let cargo_toml_path = if is_ika_service {
        "Cargo-ika.toml"
    } else {
        "Cargo-ext.toml"
    };
    ...
    if is_ika_service {
        add_member_to_workspace(path).context("adding crate to ika workspace")?;
    }
    // now that the source directory works, let's update/add a dockerfile
    if is_ika_service {
        // update ika-services dockerfile
        add_to_ika_dockerfile(path).context("adding crate to ika services dockerfile")?;
    } else {
