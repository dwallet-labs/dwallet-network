...
impl ProjectType {
    pub fn create_project(
        ...
        // make sure we're in ikaops
        let ikaops_path = ensure_in_ikaops_repo()?;
        info!("ikapop path: {}", ikaops_path);
        ...
        let project_subdir = match self {
            ...
            Self::Basic => Text::new("project subdir:")
                .with_initial_value(&format!("{}/pulumi/", ikaops_path))
...
fn ensure_in_ikaops_repo() -> Result<String> {
    let remote_stdout = run_cmd(
        vec!["git", "config", "--get", "remote.origin.url"],
        Some(CommandOptions::new(false, false)),
    )
    .context("run this command within the ika-operations repository")?
    .stdout;
    let raw_path = String::from_utf8_lossy(&remote_stdout);
    let in_ikaops = raw_path.trim().contains("ika-operations");
    if !in_ikaops {
        Err(anyhow!(
            "please run this command from within the ika-operations repository"
        ))
    } else {
...
fn get_pulumi_dir() -> Result<PathBuf> {
    let ikaops_dir_stdout = run_cmd(
        vec!["git", "rev-parse", "--show-toplevel"],
        Some(CommandOptions::new(false, false)),
    )
    .context("run this command from within the ika-operations repository")?
    .stdout;
    let ikaops_dir = PathBuf::from(String::from_utf8_lossy(&ikaops_dir_stdout).trim());
    Ok(ikaops_dir.join("pulumi"))
}
