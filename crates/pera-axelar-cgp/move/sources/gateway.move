...
///
/// - When call approvals is sent to Pera, it targets an object and not a module;
/// - To support cross-chain messaging, a Channel object has to be created;
/// - Channel can be either owned or shared but not frozen;
/// - Module developer on the Pera side will have to implement a system to support messaging;
...
module axelar::gateway {
    ...
    use axelar::utils::to_pera_signed;
    ...
    /// For when approval chainId is not PERA.
    ...
    /// Emitted when a new message is sent from the PERA network.
    ...
    entry fun process_commands(
        ...
        let mut allow_operatorship_transfer = validate_proof(validators, to_pera_signed(*&data), proof);
    ...
    fun test_execute() {
        use pera::test_scenario::{Self as ts, ctx};
