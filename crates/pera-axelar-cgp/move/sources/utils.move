...
module axelar::utils {
    ...
    /// Prefix for Pera Messages.
    const PREFIX: vector<u8> = b"\x19Pera Signed Message:\n";

    /// Normalize last byte of the signature. Have it 1 or 0.
    /// See https://tech.mystenlabs.com/cryptography-in-pera-cross-chain-signature-verification/
    ...
    public fun to_pera_signed(bytes: vector<u8>): vector<u8> {
        let mut res = vector[];
