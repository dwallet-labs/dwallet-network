...
use pera_types::base_types::{
    is_primitive_type_tag, ObjectID, PeraAddress, TxContext, TxContextKind, RESOLVED_ASCII_STR,
...
use pera_types::id::{ID, RESOLVED_PERA_ID};
use pera_types::move_package::MovePackage;
use pera_types::object::bounded_visitor::BoundedVisitor;
use pera_types::transfer::RESOLVED_RECEIVING_STRUCT;
use pera_types::MOVE_STDLIB_ADDRESS;
...
pub enum PeraJsonValueErrorKind {
...
#[derive(Debug)]
pub struct PeraJsonValueError {
    kind: PeraJsonValueErrorKind,
    val: JsonValue,
}

impl PeraJsonValueError {
    pub fn new(val: &JsonValue, kind: PeraJsonValueErrorKind) -> Self {
        Self {
            kind,
            val: val.clone(),
        }
...
impl std::error::Error for PeraJsonValueError {}

impl fmt::Display for PeraJsonValueError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err_str = match self.kind {
            PeraJsonValueErrorKind::ValueTypeNotAllowed => {
                format!("JSON value type {} not allowed.", self.val)
            }
            PeraJsonValueErrorKind::ArrayNotHomogeneous => {
...
pub struct PeraJsonValue(JsonValue);
impl PeraJsonValue {
    pub fn new(json_value: JsonValue) -> Result<PeraJsonValue, anyhow::Error> {
    ...
    pub fn from_object_id(id: ObjectID) -> PeraJsonValue {
    ...
    pub fn to_pera_address(&self) -> anyhow::Result<PeraAddress> {
        json_value_to_pera_address(&self.0)
    }
    ...
    pub fn to_move_value(
        ...
        Ok(match (val, ty) {
                ...
                if type_ == &ID::type_() =>
            {
                ...
                let addr = PeraAddress::from_str(s)?;
            ...
            (v, MoveTypeLayout::Address) => {
                let addr = json_value_to_pera_address(v)?;
                R::MoveValue::Address(addr.into())
            }
...
impl Debug for PeraJsonValue {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", self.0)
    }
}

fn json_value_to_pera_address(value: &JsonValue) -> anyhow::Result<PeraAddress> {
    match value {
        JsonValue::String(s) => {
        ...
        JsonValue::Array(bytes) => {
            ...
            let bytes = value_to_byte_array(bytes)
                .ok_or_else(|| anyhow!("Invalid input: Cannot parse input into PeraAddress."))?;
            Ok(PeraAddress::try_from(bytes)?)
        }
...
fn move_value_to_json(move_value: &MoveValue) -> Option<JsonValue> {
    Some(match move_value {
        ...
        MoveValue::Signer(v) | MoveValue::Address(v) => json!(PeraAddress::from(*v).to_string()),
...
impl FromStr for PeraJsonValue {
    ...
    fn from_str(s: &str) -> Result<Self, anyhow::Error> {
        ...
        PeraJsonValue::new(serde_json::from_str(s).unwrap_or_else(|_| try_escape_array(s)))
    }
}
...
pub fn check_valid_homogeneous(val: &JsonValue) -> Result<(), PeraJsonValueError> {
...
fn check_valid_homogeneous_rec(
    ...
    while let Some(v) = curr_q.pop_front() {
        let curr = match v {
            ...
            // Not valid
            _ => {
                return Err(PeraJsonValueError::new(
                    v,
                    PeraJsonValueErrorKind::ValueTypeNotAllowed,
                ))
            }
...
pub fn primitive_type(
    ...
    match param {
        ...
        SignatureToken::Datatype(struct_handle_idx) => {
            ...
            } else if resolved_struct == RESOLVED_PERA_ID {
                (
                    true,
                    Some(MoveTypeLayout::Struct(MoveStructLayout {
                        type_: resolved_to_struct(RESOLVED_PERA_ID),
                        fields: vec![MoveFieldLayout::new(
                            ident_str!("bytes").into(),
...
fn resolve_object_vec_arg(idx: usize, arg: &PeraJsonValue) -> Result<Vec<ObjectID>, anyhow::Error> {
    // Every elem has to be a string convertible to a ObjectID
...
pub fn resolve_move_function_args(
    ...
    function: Identifier,
    type_args: &[TypeTag],
    combined_args_json: Vec<PeraJsonValue>,
...
macro_rules! call_args {
        ($($value:expr),*) => {
        Ok::<_, anyhow::Error>(vec![$(pera_json::call_arg!($value)?,)*])
    };
    }
...
macro_rules! call_arg {
    ($value:expr) => {{
        ...
        trait PeraJsonArg {
            fn to_pera_json(&self) -> anyhow::Result<PeraJsonValue>;
        }
        // TODO: anyway to condense this?
        impl PeraJsonArg for &str {
            fn to_pera_json(&self) -> anyhow::Result<PeraJsonValue> {
                PeraJsonValue::from_str(self)
            }
        }
        impl PeraJsonArg for String {
            fn to_pera_json(&self) -> anyhow::Result<PeraJsonValue> {
                PeraJsonValue::from_str(&self)
            }
        }
        impl PeraJsonArg for pera_types::base_types::ObjectID {
            fn to_pera_json(&self) -> anyhow::Result<PeraJsonValue> {
                PeraJsonValue::from_str(&self.to_string())
            }
        }
        impl PeraJsonArg for pera_types::base_types::PeraAddress {
            fn to_pera_json(&self) -> anyhow::Result<PeraJsonValue> {
                PeraJsonValue::from_str(&self.to_string())
            }
        }
        impl PeraJsonArg for u64 {
            fn to_pera_json(&self) -> anyhow::Result<PeraJsonValue> {
                PeraJsonValue::from_bcs_bytes(
                    Some(&pera_json::MoveTypeLayout::U64),
                    &bcs::to_bytes(self)?,
                )
            }
        }
        impl PeraJsonArg for Vec<u8> {
            fn to_pera_json(&self) -> anyhow::Result<PeraJsonValue> {
                PeraJsonValue::from_bcs_bytes(None, &self)
            }
        }
        impl PeraJsonArg for &[u8] {
            fn to_pera_json(&self) -> anyhow::Result<PeraJsonValue> {
                PeraJsonValue::from_bcs_bytes(None, self)
            }
        }
...
macro_rules! type_args {
    ($($value:expr), *) => {{
        use pera_json_rpc_types::PeraTypeTag;
        use pera_types::TypeTag;
        trait PeraJsonTypeArg {
            fn to_pera_json(&self) -> anyhow::Result<PeraTypeTag>;
        }
        impl <T: core::fmt::Display> PeraJsonTypeArg for T {
            fn to_pera_json(&self) -> anyhow::Result<PeraTypeTag> {
                Ok(pera_types::parse_pera_type_tag(&self.to_string())?.into())
            }
        }
        Ok::<_, anyhow::Error>(vec![$($value.to_pera_json()?,)*])
    }};
    }
