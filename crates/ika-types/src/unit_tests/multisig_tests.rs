...
use crate::{
    ...
    crypto::{
        get_key_pair, get_key_pair_from_rng, Ed25519IkaSignature, PublicKey, Signature, IkaKeyPair,
        IkaSignatureInner, ZkLoginPublicIdentifier,
    },
...
fn test_combine_sigs() {
    let kp1: IkaKeyPair = IkaKeyPair::Ed25519(get_key_pair().1);
    let kp2: IkaKeyPair = IkaKeyPair::Secp256k1(get_key_pair().1);
...
fn test_serde_roundtrip() {
    ...
    let sig = Ed25519IkaSignature::default();
    ...
    assert_eq!(single_sig_bytes.len(), Ed25519IkaSignature::LENGTH);
    assert_eq!(
...
fn test_multisig_address() {
    ...
    assert_eq!(
        IkaAddress::from_str("0xe35c69eb504de34afdbd9f307fb3ca152646c92d549fea00065d26fc422109ea")
            .unwrap(),
        address
    );
}
...
fn test_max_sig() {
    ...
    for _ in 0..11 {
        let k = IkaKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1);
        pks.push(k.public());
        keys.push(k);
    }
...
fn multisig_zklogin_scenarios() {
    // consistency test with ika/sdk/typescript/test/unit/cryptography/multisig.test.ts
    let mut seed = StdRng::from_seed([0; 32]);
    let kp: Ed25519KeyPair = get_key_pair_from_rng(&mut seed).1;
    let skp: IkaKeyPair = IkaKeyPair::Ed25519(kp);
    let pk1 = skp.public();
    ...
    assert_eq!(
        multisig_addr,
        IkaAddress::from_str("0xb9c0780a3943cde13a2409bf1a6f06ae60b0dff2b2f373260cf627aa4f43a588")
            .unwrap()
    );
    ...
    let intent_msg = &IntentMessage::new(Intent::ika_transaction(), tx.clone());
...
fn zklogin_in_multisig_works_with_both_addresses() {
    ...
    let multisig_address = IkaAddress::from(&multisig_pk);
    ...
    assert!(
        matches!(res, Err(crate::error::IkaError::InvalidSignature { error }) if error.contains("General cryptographic error: Groth16 proof verify failed"))
    );
    ...
    let multisig_address_padded = IkaAddress::from(&multisig_pk_padded);
...
fn test_derive_multisig_address() {
    ...
    assert_eq!(
        multisig_addr,
        IkaAddress::from_str("0x77a9fbf3c695d78dd83449a81a9e70aa79a77dbfd6fb72037bf09201c12052cd")
            .unwrap()
    );
}
