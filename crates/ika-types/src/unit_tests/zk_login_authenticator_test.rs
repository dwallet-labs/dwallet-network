...
use crate::{
    base_types::IkaAddress, signature::GenericSignature, zk_login_util::DEFAULT_JWK_BYTES,
};
...
fn test_serde_zk_login_signature() {
    ...
    let addr: IkaAddress = (&authenticator).try_into().unwrap();
...
fn test_serde_zk_public_identifier() {
    ...
    assert_eq!(
        IkaAddress::try_from_unpadded(&modified_inputs).unwrap(),
        IkaAddress::from(&pk1)
    );
    ...
    assert_eq!(
        IkaAddress::try_from_padded(&modified_inputs).unwrap(),
        IkaAddress::from(&pk2)
    );
