...
use crate::{
    parse_ika_struct_tag, parse_ika_type_tag, DEEPBOOK_ADDRESS, IKA_CLOCK_ADDRESS,
    IKA_FRAMEWORK_ADDRESS, IKA_SYSTEM_ADDRESS, IKA_SYSTEM_STATE_ADDRESS,
};
...
pub struct IkaStructTag;

impl SerializeAs<StructTag> for IkaStructTag {
    fn serialize_as<S>(value: &StructTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let f = to_ika_struct_tag_string(value).map_err(S::Error::custom)?;
        f.serialize(serializer)
    }
}

const IKA_ADDRESSES: [AccountAddress; 7] = [
    AccountAddress::ZERO,
    AccountAddress::ONE,
    IKA_FRAMEWORK_ADDRESS,
    IKA_SYSTEM_ADDRESS,
    DEEPBOOK_ADDRESS,
    IKA_SYSTEM_STATE_ADDRESS,
    IKA_CLOCK_ADDRESS,
];
/// Serialize StructTag as a string, retaining the leading zeros in the address.
pub fn to_ika_struct_tag_string(value: &StructTag) -> Result<String, fmt::Error> {
    let mut f = String::new();
    // trim leading zeros if address is in IKA_ADDRESSES
    let address = if IKA_ADDRESSES.contains(&value.address) {
    ...
    if let Some(first_ty) = value.type_params.first() {
        write!(f, "<")?;
        write!(f, "{}", to_ika_type_tag_string(first_ty)?)?;
        for ty in value.type_params.iter().skip(1) {
            write!(f, ", {}", to_ika_type_tag_string(ty)?)?;
        }
        write!(f, ">")?;
    }
    Ok(f)
...
fn to_ika_type_tag_string(value: &TypeTag) -> Result<String, fmt::Error> {
    match value {
        TypeTag::Vector(t) => Ok(format!("vector<{}>", to_ika_type_tag_string(t)?)),
        TypeTag::Struct(s) => to_ika_struct_tag_string(s),
        _ => Ok(value.to_string()),
    }
}

impl<'de> DeserializeAs<'de, StructTag> for IkaStructTag {
    fn deserialize_as<D>(deserializer: D) -> Result<StructTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_ika_struct_tag(&s).map_err(D::Error::custom)
    }
}

pub struct IkaTypeTag;

impl SerializeAs<TypeTag> for IkaTypeTag {
    fn serialize_as<S>(value: &TypeTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s = to_ika_type_tag_string(value).map_err(S::Error::custom)?;
        s.serialize(serializer)
    }
}

impl<'de> DeserializeAs<'de, TypeTag> for IkaTypeTag {
    fn deserialize_as<D>(deserializer: D) -> Result<TypeTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_ika_type_tag(&s).map_err(D::Error::custom)
    }
}
