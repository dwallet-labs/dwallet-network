...
use crate::ika_serde::BigInt;
use crate::ika_serde::Readable;
use crate::versioned::Versioned;
use crate::IKA_BRIDGE_OBJECT_ID;
use crate::{
...
pub const BRIDGE_CREATE_ADD_TOKEN_ON_IKA_MESSAGE_FUNCTION_NAME: &IdentStr =
    ident_str!("create_add_tokens_on_ika_message");
...
impl BridgeChainId {
    pub fn is_ika_chain(&self) -> bool {
        matches!(
            self,
            BridgeChainId::IkaMainnet | BridgeChainId::IkaTestnet | BridgeChainId::IkaCustom
        )
    }
}
...
) -> IkaResult<Option<SequenceNumber>> {
    Ok(object_store
...
/// Rust version of the Move ika::bridge::Bridge type
...
pub trait BridgeTrait {
    ...
    fn frozen(&self) -> bool;
    fn try_into_bridge_summary(self) -> IkaResult<BridgeSummary>;
}
...
pub fn get_bridge_wrapper(object_store: &dyn ObjectStore) -> Result<BridgeWrapper, IkaError> {
    let wrapper = object_store
        .get_object(&IKA_BRIDGE_OBJECT_ID)?
        // Don't panic here on None because object_store is a generic store.
        .ok_or_else(|| IkaError::IkaBridgeReadError("BridgeWrapper object not found".to_owned()))?;
    let move_object = wrapper.data.try_as_move().ok_or_else(|| {
        IkaError::IkaBridgeReadError("BridgeWrapper object must be a Move object".to_owned())
    })?;
    let result = bcs::from_bytes::<BridgeWrapper>(move_object.contents())
        .map_err(|err| IkaError::IkaBridgeReadError(err.to_string()))?;
    Ok(result)
}

pub fn get_bridge(object_store: &dyn ObjectStore) -> Result<Bridge, IkaError> {
    ...
    match version {
        1 => {
            let result: BridgeInnerV1 = get_dynamic_field_from_store(object_store, id, &version)
                .map_err(|err| {
                    IkaError::IkaBridgeReadError(format!(
        ...
        _ => Err(IkaError::IkaBridgeReadError(format!(
            "Unsupported IkaBridge version: {}",
            version
        ))),
    }
}
...
pub struct MoveTypeBridgeCommittee {
    ...
    pub member_registrations: VecMap<IkaAddress, MoveTypeCommitteeMemberRegistration>,
...
pub struct BridgeCommitteeSummary {
    ...
    pub member_registration: Vec<(IkaAddress, MoveTypeCommitteeMemberRegistration)>,
    pub last_committee_update_epoch: u64,
}
...
pub fn is_bridge_committee_initiated(object_store: &dyn ObjectStore) -> IkaResult<bool> {
    match get_bridge(object_store) {
        Ok(bridge) => Ok(!bridge.committee().members.contents.is_empty()),
        Err(IkaError::IkaBridgeReadError(..)) => Ok(false),
        Err(other) => Err(other),
    }
}
