...
use crate::base_types::{AuthorityName, EpochId, IkaAddress};
...
pub trait EpochStartSystemStateTrait {
    ...
    fn get_validator_addresses(&self) -> Vec<IkaAddress>;
...
/// This type captures the minimum amount of information from IkaSystemState needed by a validator
/// to run the protocol. This allows us to decouple from the actual IkaSystemState type, and hence
/// do not need to evolve it when we upgrade the IkaSystemState type.
...
impl EpochStartSystemStateTrait for EpochStartSystemStateV1 {
    ...
    fn get_validator_addresses(&self) -> Vec<IkaAddress> {
        self.active_validators
            .iter()
            .map(|validator| validator.ika_address)
            .collect()
    }
            ...
            .map(|validator| {
                (
                    ...
                    (
                        ...
                        NetworkMetadata {
                            network_address: validator.ika_net_address.clone(),
    ...
    fn get_consensus_committee(&self) -> ConsensusCommittee {
        ...
        // in the Ika committee returned from get_ika_committee().
        authorities.sort_by(|a1, a2| a1.authority_key.cmp(&a2.authority_key));

        for ((i, mysticeti_authority), ika_authority_name) in authorities
            .iter()
            .enumerate()
            .zip(self.get_ika_committee().names())
        {
            if ika_authority_name.0 != mysticeti_authority.authority_key.to_bytes() {
                error!(
                    "Mismatched authority order between Ika and Mysticeti! Index {}, Mysticeti authority {:?}\nIka authority name {}",
                    i, mysticeti_authority, ika_authority_name
                );
            }
        }
...
mod test {
    ...
    fn test_ika_and_mysticeti_committee_are_same() {
        ...
        for i in 0..10 {
            let (ika_address, protocol_key): (IkaAddress, AuthorityKeyPair) = get_key_pair();
            ...
            active_validators.push(EpochStartValidatorInfoV1 {
                ...
                ika_net_address: Multiaddr::empty(),
