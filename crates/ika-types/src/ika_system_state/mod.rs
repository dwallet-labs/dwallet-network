...
use crate::ika_system_state::epoch_start_ika_system_state::EpochStartSystemState;
use crate::ika_system_state::ika_system_state_inner_v2::IkaSystemStateInnerV2;
use crate::versioned::Versioned;
use crate::{id::UID, MoveTypeTagTrait, IKA_SYSTEM_ADDRESS, IKA_SYSTEM_STATE_OBJECT_ID};
use anyhow::Result;
...
use self::ika_system_state_inner_v1::{IkaSystemStateInnerV1, ValidatorV1};
use self::ika_system_state_summary::{IkaSystemStateSummary, IkaValidatorSummary};

pub mod epoch_start_ika_system_state;
pub mod ika_system_state_inner_v1;
pub mod ika_system_state_inner_v2;
pub mod ika_system_state_summary;

#[cfg(msim)]
mod simtest_ika_system_state_inner;
#[cfg(msim)]
...
const IKA_SYSTEM_STATE_WRAPPER_STRUCT_NAME: &IdentStr = ident_str!("IkaSystemState");

pub const IKA_SYSTEM_MODULE_NAME: &IdentStr = ident_str!("ika_system");
...
pub struct IkaSystemStateWrapper {
    pub id: UID,
    pub version: u64,
}

impl IkaSystemStateWrapper {
    pub fn type_() -> StructTag {
        StructTag {
            address: IKA_SYSTEM_ADDRESS,
            name: IKA_SYSTEM_STATE_WRAPPER_STRUCT_NAME.to_owned(),
            module: IKA_SYSTEM_MODULE_NAME.to_owned(),
            type_params: vec![],
        }
    }
    ...
    pub fn advance_epoch_safe_mode(
        ...
        match self.version {
            1 => {
                Self::advance_epoch_safe_mode_impl::<IkaSystemStateInnerV1>(
                    move_object,
                    params,
                    protocol_config,
                );
            ...
            2 => {
                Self::advance_epoch_safe_mode_impl::<IkaSystemStateInnerV2>(
                    move_object,
                    params,
                    protocol_config,
                );
            ...
            IKA_SYSTEM_STATE_SIM_TEST_V1 => {
                Self::advance_epoch_safe_mode_impl::<SimTestIkaSystemStateInnerV1>(
                    move_object,
                    params,
                    protocol_config,
                );
            ...
            IKA_SYSTEM_STATE_SIM_TEST_SHALLOW_V2 => {
                Self::advance_epoch_safe_mode_impl::<SimTestIkaSystemStateInnerShallowV2>(
                    move_object,
                    params,
                    protocol_config,
                );
            ...
            IKA_SYSTEM_STATE_SIM_TEST_DEEP_V2 => {
                Self::advance_epoch_safe_mode_impl::<SimTestIkaSystemStateInnerDeepV2>(
                    move_object,
                    params,
                    protocol_config,
                );
    ...
    fn advance_epoch_safe_mode_impl<T>(
        ...
        T: Serialize + DeserializeOwned + IkaSystemStateTrait,
    {
        ...
        move_object
            ...
            .expect("Update ika system object content cannot fail since it should be small");
    }
}

/// This is the standard API that all inner system state object type should implement.
#[enum_dispatch]
pub trait IkaSystemStateTrait {
    fn epoch(&self) -> u64;
    ...
    fn get_pending_active_validators<S: ObjectStore + ?Sized>(
        &self,
        object_store: &S,
    ) -> Result<Vec<IkaValidatorSummary>, IkaError>;
    fn into_epoch_start_state(self) -> EpochStartSystemState;
    fn into_ika_system_state_summary(self) -> IkaSystemStateSummary;
}

/// IkaSystemState provides an abstraction over multiple versions of the inner IkaSystemStateInner object.
...
#[enum_dispatch(IkaSystemStateTrait)]
pub enum IkaSystemState {
    V1(IkaSystemStateInnerV1),
    V2(IkaSystemStateInnerV2),
    #[cfg(msim)]
    SimTestV1(SimTestIkaSystemStateInnerV1),
    #[cfg(msim)]
    SimTestShallowV2(SimTestIkaSystemStateInnerShallowV2),
    #[cfg(msim)]
    SimTestDeepV2(SimTestIkaSystemStateInnerDeepV2),
}

/// This is the fixed type used by genesis.
pub type IkaSystemStateInnerGenesis = IkaSystemStateInnerV1;
pub type IkaValidatorGenesis = ValidatorV1;
...
impl IkaSystemState {
    ...
    pub fn into_genesis_version_for_tooling(self) -> IkaSystemStateInnerGenesis {
        match self {
            IkaSystemState::V1(inner) => inner,
            _ => unreachable!(),
        }
    }
...
pub fn get_ika_system_state_wrapper(
    object_store: &dyn ObjectStore,
) -> Result<IkaSystemStateWrapper, IkaError> {
    let wrapper = object_store
        .get_object(&IKA_SYSTEM_STATE_OBJECT_ID)?
        // Don't panic here on None because object_store is a generic store.
        .ok_or_else(|| {
            IkaError::IkaSystemStateReadError("IkaSystemStateWrapper object not found".to_owned())
        })?;
    let move_object = wrapper.data.try_as_move().ok_or_else(|| {
        IkaError::IkaSystemStateReadError(
            "IkaSystemStateWrapper object must be a Move object".to_owned(),
        )
    })?;
    let result = bcs::from_bytes::<IkaSystemStateWrapper>(move_object.contents())
        .map_err(|err| IkaError::IkaSystemStateReadError(err.to_string()))?;
    Ok(result)
}

pub fn get_ika_system_state(object_store: &dyn ObjectStore) -> Result<IkaSystemState, IkaError> {
    let wrapper = get_ika_system_state_wrapper(object_store)?;
    let id = wrapper.id.id.bytes;
    match wrapper.version {
        1 => {
            let result: IkaSystemStateInnerV1 =
                get_dynamic_field_from_store(object_store, id, &wrapper.version).map_err(
                    |err| {
                        IkaError::DynamicFieldReadError(format!(
                            "Failed to load ika system state inner object with ID {:?} and version {:?}: {:?}",
                            id, wrapper.version, err
                        ))
                    },
                )?;
            Ok(IkaSystemState::V1(result))
        }
        2 => {
            let result: IkaSystemStateInnerV2 =
                get_dynamic_field_from_store(object_store, id, &wrapper.version).map_err(
                    |err| {
                        IkaError::DynamicFieldReadError(format!(
                            "Failed to load ika system state inner object with ID {:?} and version {:?}: {:?}",
            ...
            Ok(IkaSystemState::V2(result))
        }
        #[cfg(msim)]
        IKA_SYSTEM_STATE_SIM_TEST_V1 => {
            let result: SimTestIkaSystemStateInnerV1 =
                get_dynamic_field_from_store(object_store, id, &wrapper.version).map_err(
                    |err| {
                        IkaError::DynamicFieldReadError(format!(
                            "Failed to load ika system state inner object with ID {:?} and version {:?}: {:?}",
            ...
            Ok(IkaSystemState::SimTestV1(result))
        }
        #[cfg(msim)]
        IKA_SYSTEM_STATE_SIM_TEST_SHALLOW_V2 => {
            let result: SimTestIkaSystemStateInnerShallowV2 =
                get_dynamic_field_from_store(object_store, id, &wrapper.version).map_err(
                    |err| {
                        IkaError::DynamicFieldReadError(format!(
                            "Failed to load ika system state inner object with ID {:?} and version {:?}: {:?}",
            ...
            Ok(IkaSystemState::SimTestShallowV2(result))
        }
        #[cfg(msim)]
        IKA_SYSTEM_STATE_SIM_TEST_DEEP_V2 => {
            let result: SimTestIkaSystemStateInnerDeepV2 =
                get_dynamic_field_from_store(object_store, id, &wrapper.version).map_err(
                    |err| {
                        IkaError::DynamicFieldReadError(format!(
                            "Failed to load ika system state inner object with ID {:?} and version {:?}: {:?}",
            ...
            Ok(IkaSystemState::SimTestDeepV2(result))
        }
        _ => Err(IkaError::IkaSystemStateReadError(format!(
            "Unsupported IkaSystemState version: {}",
            wrapper.version
        ))),
    }
}
...
pub fn get_validator_from_table<K>(
    ...
    table_id: ObjectID,
    key: &K,
) -> Result<IkaValidatorSummary, IkaError>
where
    K: MoveTypeTagTrait + Serialize + DeserializeOwned + fmt::Debug,
{
    ...
    match version {
        1 => {
            ...
            Ok(validator.into_ika_validator_summary())
        }
        #[cfg(msim)]
        IKA_SYSTEM_STATE_SIM_TEST_V1 => {
...
pub fn get_validators_from_table_vec<S, ValidatorType>(
    ...
    table_id: ObjectID,
    table_size: u64,
) -> Result<Vec<ValidatorType>, IkaError>
where
    S: ObjectStore + ?Sized,
