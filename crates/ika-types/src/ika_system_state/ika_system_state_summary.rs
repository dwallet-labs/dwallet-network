...
use super::{IkaSystemState, IkaSystemStateTrait};
use crate::base_types::{AuthorityName, ObjectID, IkaAddress};
...
use crate::ika_system_state::get_validator_from_table;
...
/// This is the JSON-RPC type for the IKA system state object.
/// It flattens all fields to make them top-level fields such that it as minimum
/// dependencies to the internal data structures of the IKA system state type.

#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct IkaSystemStateSummary {
    ...
    pub active_validators: Vec<IkaValidatorSummary>,
    ...
    #[schemars(with = "Vec<(IkaAddress, BigInt<u64>)>")]
    #[serde_as(as = "Vec<(_, Readable<BigInt<u64>, _>)>")]
    pub at_risk_validators: Vec<(IkaAddress, u64)>,
    /// A map storing the records of validator reporting each other.
    pub validator_report_records: Vec<(IkaAddress, Vec<IkaAddress>)>,
}

impl IkaSystemStateSummary {
    pub fn get_ika_committee_for_benchmarking(&self) -> CommitteeWithNetworkMetadata {
        let validators = self
            .active_validators
            .iter()
            .map(|validator| {
...
/// This is the JSON-RPC type for the IKA validator. It flattens all inner structures
/// to top-level fields so that they are decoupled from the internal definitions.
#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct IkaValidatorSummary {
    // Metadata
...
impl Default for IkaSystemStateSummary {
    fn default() -> Self {
        Self {
            epoch: 0,
            protocol_version: 1,
...
impl Default for IkaValidatorSummary {
    fn default() -> Self {
        Self {
            ika_address: IkaAddress::default(),
    ...
    S: ObjectStore + ?Sized,
{
    ...
    let candidate_address: IkaAddress = get_dynamic_field_from_store(
        &object_store,
