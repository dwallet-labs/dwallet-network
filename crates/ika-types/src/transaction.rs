...
use super::{base_types::*, error::*, IKA_BRIDGE_OBJECT_ID};
...
use crate::crypto::{
    ...
    AuthorityStrongQuorumSignInfo, DefaultHash, Ed25519IkaSignature, EmptySignInfo,
    RandomnessRound, Signature, Signer, IkaSignatureInner, ToFromBytes,
};
...
use crate::{
    IKA_AUTHENTICATOR_STATE_OBJECT_ID, IKA_AUTHENTICATOR_STATE_OBJECT_SHARED_VERSION,
    IKA_CLOCK_OBJECT_ID, IKA_CLOCK_OBJECT_SHARED_VERSION, IKA_FRAMEWORK_PACKAGE_ID,
    IKA_RANDOMNESS_STATE_OBJECT_ID, IKA_SYSTEM_STATE_OBJECT_ID,
    IKA_SYSTEM_STATE_OBJECT_SHARED_VERSION,
};
...
impl CallArg {
    pub const IKA_SYSTEM_MUT: Self = Self::Object(ObjectArg::IKA_SYSTEM_MUT);
    pub const CLOCK_IMM: Self = Self::Object(ObjectArg::SharedObject {
        id: IKA_CLOCK_OBJECT_ID,
        initial_shared_version: IKA_CLOCK_OBJECT_SHARED_VERSION,
        mutable: false,
    });
    ...
    pub const AUTHENTICATOR_MUT: Self = Self::Object(ObjectArg::SharedObject {
        id: IKA_AUTHENTICATOR_STATE_OBJECT_ID,
        initial_shared_version: IKA_AUTHENTICATOR_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    });
}
...
impl EndOfEpochTransactionKind {
        ...
        match self {
            Self::ChangeEpoch(_) => {
                vec![InputObjectKind::SharedMoveObject {
                    id: IKA_SYSTEM_STATE_OBJECT_ID,
                    initial_shared_version: IKA_SYSTEM_STATE_OBJECT_SHARED_VERSION,
                    mutable: true,
                }]
            }
    ...
    fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match self {
            Self::ChangeEpoch(_) => {
                Either::Left(vec![SharedInputObject::IKA_SYSTEM_OBJ].into_iter())
            }
            ...
            Self::BridgeCommitteeInit(bridge_version) => Either::Left(
                vec![
                    ...
                    SharedInputObject::IKA_SYSTEM_OBJ,
                ]
                .into_iter(),
            ),
        }
...
impl ObjectArg {
    pub const IKA_SYSTEM_MUT: Self = Self::SharedObject {
        id: IKA_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: IKA_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    };
...
impl ProgrammableTransaction {
    ...
    fn validity_check(&self, config: &ProtocolConfig) -> UserInputResult {
        ...
        if let Some(random_index) = inputs.iter().position(|obj| {
            matches!(obj, CallArg::Object(ObjectArg::SharedObject { id, .. }) if *id == IKA_RANDOMNESS_STATE_OBJECT_ID)
        }) {
            fp_ensure!(
...
impl SharedInputObject {
    pub const IKA_SYSTEM_OBJ: Self = Self {
        id: IKA_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: IKA_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    };
...
impl TransactionKind {
    ...
    pub fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match &self {
            Self::ChangeEpoch(_) => {
                Either::Left(Either::Left(iter::once(SharedInputObject::IKA_SYSTEM_OBJ)))
            }
            ...
            Self::RandomnessStateUpdate(update) => {
                Either::Left(Either::Left(iter::once(SharedInputObject {
                    id: IKA_RANDOMNESS_STATE_OBJECT_ID,
...
impl TransactionData {
    ...
    pub fn new_with_gas_data(kind: TransactionKind, sender: IkaAddress, gas_data: GasData) -> Self {
        TransactionData::V1(TransactionDataV1 {
            kind,
            sender,
            gas_data,
    ...
    pub fn new_transfer_ika(
        ...
        Self::new_transfer_ika_allow_sponsor(
            recipient,
            sender,
            amount,
            gas_payment,
    ...
    pub fn new_transfer_ika_allow_sponsor(
        recipient: IkaAddress,
        sender: IkaAddress,
        amount: Option<u64>,
        gas_payment: ObjectRef,
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.transfer_ika(recipient, amount);
            builder.finish()
        };
    ...
    pub fn new_pay_ika(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.pay_ika(recipients, amounts)?;
            builder.finish()
        };
        Ok(Self::new_programmable(
    ...
    pub fn execution_parts(&self) -> (TransactionKind, IkaAddress, Vec<ObjectRef>) {
        (
            self.kind().clone(),
            self.sender(),
    ...
    pub fn uses_randomness(&self) -> bool {
        self.shared_input_objects()
            .iter()
            .any(|obj| obj.id() == IKA_RANDOMNESS_STATE_OBJECT_ID)
    }
...
pub trait TransactionDataAPI {
    ...
    fn signers(&self) -> NonEmpty<IkaAddress>;

    fn gas_data(&self) -> &GasData;

    fn gas_owner(&self) -> IkaAddress;
    ...
    fn sender_mut_for_testing(&mut self) -> &mut IkaAddress;
...
impl Serialize for SenderSignedTransaction {
        ...
        S: serde::Serializer,
    {
        ...
        if self.intent_message().intent != Intent::ika_transaction() {
...
impl<'de> Deserialize<'de> for SenderSignedTransaction {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if intent_message.intent != Intent::ika_transaction() {
...
impl SenderSignedTransaction {
    ...
    ) -> IkaResult<BTreeMap<IkaAddress, &GenericSignature>> {
        ...
        for sig in &self.tx_signatures {
            if verify_legacy_zklogin_address {
                ...
                if let GenericSignature::ZkLoginAuthenticator(z) = sig {
                    mapping.insert(IkaAddress::try_from_padded(&z.inputs)?, sig);
                };
            }
...
impl SenderSignedData {
    pub fn new(tx_data: TransactionData, tx_signatures: Vec<GenericSignature>) -> Self {
        Self(SizeOneVec::new(SenderSignedTransaction {
            intent_message: IntentMessage::new(Intent::ika_transaction(), tx_data),
            tx_signatures,
        }))
    }
    ...
    pub fn serialized_size(&self) -> IkaResult<usize> {
        bcs::serialized_size(self).map_err(|e| IkaError::TransactionSerializationError {
            error: e.to_string(),
        })
    }

    fn check_user_signature_protocol_compatibility(&self, config: &ProtocolConfig) -> IkaResult {
    ...
    pub fn validity_check(&self, config: &ProtocolConfig, epoch: EpochId) -> IkaResult {
        ...
        if match &tx_data.expiration() {
            ...
            return Err(IkaError::TransactionExpired);
        }
...
impl<S> Envelope<SenderSignedData, S> {
    pub fn sender_address(&self) -> IkaAddress {
        self.data().intent_message().value.sender()
    }

    pub fn gas_owner(&self) -> IkaAddress {
...
impl Transaction {
    pub fn from_data_and_signer(
        ...
        let signatures = {
            let intent_msg = IntentMessage::new(Intent::ika_transaction(), &data);
            signers
                .into_iter()
        ...
        system_transaction
            ...
            .pipe(|data| {
                SenderSignedData::new_from_sender_signature(
                    data,
                    Ed25519IkaSignature::from_bytes(&[0; Ed25519IkaSignature::LENGTH])
                        .unwrap()
                        .into(),
                )
            })
...
impl SignedTransaction {
    ...
    pub fn try_into_verified_for_testing(
        ...
        committee: &Committee,
        verify_params: &VerifyParams,
    ) -> IkaResult<VerifiedSignedTransaction> {
...
// An InputObjects new-type that has been verified by ika-transaction-checks, and can be
// safely passed to execution.
pub struct CheckedInputObjects(InputObjects);

// DO NOT CALL outside of ika-transaction-checks, genesis, or replay.
//
// CheckedInputObjects should really be defined in ika-transaction-checks so that we can
// make public construction impossible. But we can't do that because it would result in circular
// dependencies.
impl CheckedInputObjects {
    // Only called by ika-transaction-checks.
