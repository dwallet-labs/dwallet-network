...
impl ProgrammableTransactionBuilder {
    ...
    pub fn transfer_arg(&mut self, recipient: IkaAddress, arg: Argument) {
        self.transfer_args(recipient, vec![arg])
    }

    pub fn transfer_args(&mut self, recipient: IkaAddress, args: Vec<Argument>) {
    ...
    pub fn transfer_ika(&mut self, recipient: IkaAddress, amount: Option<u64>) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    pub fn pay_all_ika(&mut self, recipient: IkaAddress) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    fn pay_impl(
        ...
        let mut recipient_map: IndexMap<IkaAddress, Vec<usize>> = IndexMap::new();
