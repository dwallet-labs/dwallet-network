...
use crate::{id::UID, IKA_AUTHENTICATOR_STATE_OBJECT_ID, IKA_FRAMEWORK_ADDRESS};
...
pub const RESOLVED_IKA_AUTHENTICATOR_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
) -> IkaResult<Option<AuthenticatorStateInner>> {
    let outer = object_store.get_object(&IKA_AUTHENTICATOR_STATE_OBJECT_ID)?;
...
pub fn get_authenticator_state_obj_initial_shared_version(
    ...
    Ok(object_store
        .get_object(&IKA_AUTHENTICATOR_STATE_OBJECT_ID)?
