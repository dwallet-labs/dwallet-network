...
use crate::{
    ...
    crypto::DefaultHash,
    error::{ExecutionError, ExecutionErrorKind, IkaError, IkaResult},
    id::{ID, UID},
...
/// Rust representation of upgrade policy constants in `ika::package`.
#[repr(u8)]
...
/// Rust representation of `ika::package::UpgradeCap`.
...
/// Rust representation of `ika::package::UpgradeTicket`.
#[derive(Debug, Serialize, Deserialize)]
...
/// Rust representation of `ika::package::UpgradeReceipt`.
#[derive(Debug, Serialize, Deserialize)]
...
impl MovePackage {
    ...
    ) -> IkaResult<CompiledModule> {
        // TODO use the session's cache
        let bytes = self
            .serialized_module_map()
            .get(module.as_str())
            .ok_or_else(|| IkaError::ModuleNotFound {
    ...
    pub fn disassemble(&self) -> IkaResult<BTreeMap<String, Value>> {
    ...
    pub fn normalize(
        &self,
        binary_config: &BinaryConfig,
    ) -> IkaResult<BTreeMap<String, normalized::Module>> {
...
pub fn disassemble_modules<'a, I>(modules: I) -> IkaResult<BTreeMap<String, Value>>
where
