...
impl<D: Hash + Eq + Copy> VerifiedDigestCache<D> {
    ...
    pub fn is_verified<F, G>(&self, digest: D, verify_callback: F, uncached_checks: G) -> IkaResult
    where
        F: FnOnce() -> IkaResult,
        G: FnOnce() -> IkaResult,
    {
...
pub fn verify_sender_signed_data_message_signatures(
    ...
    assert_eq!(intent_message.intent, Intent::ika_transaction());
    ...
    for s in signers {
        if !present_sigs.contains_key(&s) {
            return Err(IkaError::SignerSignatureAbsent {
