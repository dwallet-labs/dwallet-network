...
use crate::base_types::{ObjectDigest, IkaAddress};
...
use crate::ika_serde::IkaTypeTag;
use crate::{MoveTypeTagTrait, ObjectID, SequenceNumber, IKA_FRAMEWORK_ADDRESS};
...
/// Rust version of the Move ika::dynamic_field::Field type
...
/// Rust version of the Move ika::dynamic_object_field::Wrapper type
...
pub struct DynamicFieldName {
    ...
    #[serde_as(as = "Readable<IkaTypeTag, _>")]
    pub type_: TypeTag,
...
impl DynamicFieldInfo {
    pub fn is_dynamic_field(tag: &StructTag) -> bool {
        tag.address == IKA_FRAMEWORK_ADDRESS
    ...
    pub fn try_extract_field_name(tag: &StructTag, type_: &DynamicFieldType) -> IkaResult<TypeTag> {
        match (type_, tag.type_params.first()) {
            ...
            (DynamicFieldType::DynamicObject, Some(TypeTag::Struct(s))) => Ok(s
                .type_params
                .first()
                .ok_or_else(|| IkaError::ObjectDeserializationError {
    ...
    pub fn try_extract_field_value(tag: &StructTag) -> IkaResult<TypeTag> {
        match tag.type_params.last() {
            Some(value_type) => Ok(value_type.clone()),
            None => Err(IkaError::ObjectDeserializationError {
    ...
    pub fn parse_move_object(
    ...
    ) -> IkaResult<(MoveValue, DynamicFieldType, ObjectID)> {
        let name = extract_field_from_move_struct(move_struct, "name").ok_or_else(|| {
            IkaError::ObjectDeserializationError {
                error: "Cannot extract [name] field from ika::dynamic_field::Field".to_string(),
            }
        })?;

        let value = extract_field_from_move_struct(move_struct, "value").ok_or_else(|| {
            IkaError::ObjectDeserializationError {
                error: "Cannot extract [value] field from ika::dynamic_field::Field".to_string(),
            }
        })?;
        ...
        Ok(if is_dynamic_object(move_struct) {
            ...
            .ok_or_else(|| IkaError::ObjectDeserializationError {
                error: "Cannot extract [name] field from ika::dynamic_object_field::Wrapper."
                    .to_string(),
            })?;
            // ID extracted from the wrapper object
            let object_id =
                extract_id_value(value).ok_or_else(|| IkaError::ObjectDeserializationError {
                    error: format!(
                        "Cannot extract dynamic object's object id from \
                        ika::dynamic_field::Field, {value:?}"
                    ),
                })?;
            ...
            let object_id = extract_object_id(move_struct).ok_or_else(|| {
                IkaError::ObjectDeserializationError {
                    error: format!(
                        "Cannot extract dynamic object's object id from \
                        ika::dynamic_field::Field, {move_struct:?}",
                    ),
                }
            })?;
...
where
    T: Into<IkaAddress>,
{
    let parent: IkaAddress = parent.into();
    ...
    K: MoveTypeTagTrait + Serialize + DeserializeOwned + fmt::Debug,
{
    let id = derive_dynamic_field_id(parent_id, &K::get_type_tag(), &bcs::to_bytes(key).unwrap())
        .map_err(|err| IkaError::DynamicFieldReadError(err.to_string()))?;
