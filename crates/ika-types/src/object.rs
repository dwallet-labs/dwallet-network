...
impl MoveObject {
    ...
    pub fn get_layout(&self, resolver: &impl GetModule) -> Result<MoveStructLayout, IkaError> {
    ...
    pub fn to_move_struct(&self, layout: &MoveStructLayout) -> Result<MoveStruct, IkaError> {
    ...
    /// Get the total amount of IKA embedded in `self`. Intended for testing purposes
    pub fn get_total_ika(&self, layout_resolver: &mut dyn LayoutResolver) -> Result<u64, IkaError> {
...
impl MoveObject {
    ...
    pub fn get_coin_balances(
    ...
    ) -> Result<BTreeMap<TypeTag, u64>, IkaError> {
        ...
        if let Some(type_tag) = self.type_.coin_type_maybe() {
            ...
            MoveValue::visit_deserialize(&self.contents, &layout.into_layout(), &mut traversal)
                .map_err(|e| IkaError::ObjectSerializationError {
                    error: e.to_string(),
                })?;
...
pub enum Owner {
    ...
    /// The object ID is converted to IkaAddress as IkaAddress is universal.
    ObjectOwner(IkaAddress),
...
impl Owner {
    ...
    pub fn get_address_owner_address(&self) -> IkaResult<IkaAddress> {
        match self {
            Self::AddressOwner(address) => Ok(*address),
            Self::Shared { .. } | Self::Immutable | Self::ObjectOwner(_) => {
                Err(IkaError::UnexpectedOwnerType)
            }
        }
    }

    // NOTE: this function will return address of both AddressOwner and ObjectOwner,
    // address of ObjectOwner is converted from object id, even though the type is IkaAddress.
    pub fn get_owner_address(&self) -> IkaResult<IkaAddress> {
        match self {
            Self::AddressOwner(address) | Self::ObjectOwner(address) => Ok(*address),
            Self::Shared { .. } | Self::Immutable => Err(IkaError::UnexpectedOwnerType),
        }
    }
...
impl PartialEq<IkaAddress> for Owner {
    fn eq(&self, other: &IkaAddress) -> bool {
        match self {
...
impl PartialEq<ObjectID> for Owner {
    fn eq(&self, other: &ObjectID) -> bool {
        let other_id: IkaAddress = (*other).into();
        match self {
...
impl ObjectInner {
    ...
    pub fn get_single_owner(&self) -> Option<IkaAddress> {
    ...
    pub fn transfer(&mut self, new_owner: IkaAddress) {
    ...
    pub fn get_layout(
        &self,
        resolver: &impl GetModule,
    ) -> Result<Option<MoveStructLayout>, IkaError> {
        match &self.data {
    ...
    pub fn get_move_template_type(&self) -> IkaResult<TypeTag> {
        let move_struct = self.data.struct_tag().ok_or_else(|| IkaError::TypeError {
...
impl Object {
    /// Get the total amount of IKA embedded in `self`, including both Move objects and the storage rebate
    pub fn get_total_ika(&self, layout_resolver: &mut dyn LayoutResolver) -> Result<u64, IkaError> {
        Ok(self.storage_rebate
            + match &self.data {
                Data::Move(m) => m.get_total_ika(layout_resolver)?,
                Data::Package(_) => 0,
            })
    }
    ...
    pub fn with_id_owner_gas_for_testing(id: ObjectID, owner: IkaAddress, gas: u64) -> Self {
        let data = Data::Move(MoveObject {
            type_: GasCoin::type_().into(),
            has_public_transfer: true,
            version: OBJECT_START_VERSION,
    ...
    pub fn with_id_owner_for_testing(id: ObjectID, owner: IkaAddress) -> Self {
    ...
    pub fn with_owner_for_testing(owner: IkaAddress) -> Self {
    ...
    pub fn new_gas_with_balance_and_owner_for_testing(value: u64, owner: IkaAddress) -> Self {
...
mod tests {
    ...
    fn test_object_digest_and_serialized_format() {
        ...
        let o = Object::new_move(
            g,
            Owner::AddressOwner(IkaAddress::ZERO),
