...
use crate::crypto::{
    AuthorityPublicKeyBytes, DefaultHash, PublicKey, SignatureScheme, IkaPublicKey, IkaSignature,
};
...
use crate::error::{ExecutionError, IkaResult};
use crate::gas_coin::GasCoin;
use crate::gas_coin::GAS;
use crate::governance::StakedIka;
use crate::governance::STAKED_IKA_STRUCT_NAME;
use crate::governance::STAKING_POOL_MODULE_NAME;
use crate::id::RESOLVED_IKA_ID;
use crate::messages_checkpoint::CheckpointTimestamp;
use crate::multisig::MultiSigPublicKey;
use crate::object::{Object, Owner};
use crate::parse_ika_struct_tag;
use crate::signature::GenericSignature;
use crate::ika_serde::Readable;
use crate::ika_serde::{to_ika_struct_tag_string, HexAccountAddress};
use crate::transaction::Transaction;
use crate::transaction::VerifiedTransaction;
use crate::zk_login_authenticator::ZkLoginAuthenticator;
use crate::MOVE_STDLIB_ADDRESS;
use crate::IKA_CLOCK_OBJECT_ID;
use crate::IKA_FRAMEWORK_ADDRESS;
...
pub enum MoveObjectType_ {
    ...
    Other(StructTag),
    /// A IKA coin (i.e., `0x2::coin::Coin<0x2::ika::IKA>`)
    GasCoin,
    /// A record of a staked IKA coin (i.e., `0x3::staking_pool::StakedIka`)
    StakedIka,
    /// A non-IKA coin type (i.e., `0x2::coin::Coin<T> where T != 0x2::ika::IKA`)
    Coin(TypeTag),
...
impl MoveObjectType {
    ...
    pub fn staked_ika() -> Self {
        Self(MoveObjectType_::StakedIka)
    }

    pub fn address(&self) -> AccountAddress {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => IKA_FRAMEWORK_ADDRESS,
            MoveObjectType_::StakedIka => IKA_SYSTEM_ADDRESS,
    ...
    pub fn module(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedIka => STAKING_POOL_MODULE_NAME,
    ...
    pub fn name(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedIka => STAKED_IKA_STRUCT_NAME,
    ...
    pub fn coin_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            ...
            MoveObjectType_::StakedIka => None,
    ...
    pub fn size_for_gas_metering(&self) -> usize {
        ...
        match &self.0 {
            ...
            MoveObjectType_::StakedIka => 1,
            MoveObjectType_::Coin(inner) => bcs::serialized_size(inner).unwrap() + 1,
            MoveObjectType_::Other(s) => bcs::serialized_size(s).unwrap() + 1,
        }
    }

    /// Return true if `self` is `0x2::coin::Coin<T>` for some T (note: T can be IKA)
    pub fn is_coin(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => true,
            MoveObjectType_::StakedIka | MoveObjectType_::Other(_) => false,
        }
    }

    /// Return true if `self` is 0x2::coin::Coin<0x2::ika::IKA>
    pub fn is_gas_coin(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin => true,
            MoveObjectType_::StakedIka | MoveObjectType_::Coin(_) | MoveObjectType_::Other(_) => {
                false
            }
        }
    }
    ...
    pub fn is_staked_ika(&self) -> bool {
        match &self.0 {
            MoveObjectType_::StakedIka => true,
    ...
    pub fn is_coin_metadata(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedIka | MoveObjectType_::Coin(_) => {
                false
            }
    ...
    pub fn is_upgrade_cap(&self) -> bool {
        self.address() == IKA_FRAMEWORK_ADDRESS
    ...
    pub fn try_extract_field_name(&self, type_: &DynamicFieldType) -> IkaResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedIka | MoveObjectType_::Coin(_) => {
    ...
    pub fn try_extract_field_value(&self) -> IkaResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedIka | MoveObjectType_::Coin(_) => {
                Err(IkaError::ObjectDeserializationError {
    ...
    pub fn is(&self, s: &StructTag) -> bool {
        match &self.0 {
            ...
            MoveObjectType_::StakedIka => StakedIka::is_staked_ika(s),
...
impl From<StructTag> for MoveObjectType {
    fn from(mut s: StructTag) -> Self {
        ...
        } else if StakedIka::is_staked_ika(&s) {
            MoveObjectType_::StakedIka
        } else {
...
impl From<MoveObjectType> for StructTag {
    fn from(t: MoveObjectType) -> Self {
        match t.0 {
            ...
            MoveObjectType_::StakedIka => StakedIka::type_(),
...
pub fn is_primitive_type_tag(t: &TypeTag) -> bool {
    ...
    match t {
        ...
        T::Struct(st) => {
            ...
            // is id or..
            if resolved_struct == RESOLVED_IKA_ID {
                return true;
            }
            // is utf8 string
...
impl FromStr for ObjectType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.to_lowercase() == PACKAGE {
            ...
            let tag = parse_ika_struct_tag(s)?;
...
pub const IKA_ADDRESS_LENGTH: usize = ObjectID::LENGTH;
...
#[cfg_attr(feature = "fuzzing", derive(proptest_derive::Arbitrary))]
pub struct IkaAddress(
    #[schemars(with = "Hex")]
    #[serde_as(as = "Readable<Hex, _>")]
    [u8; IKA_ADDRESS_LENGTH],
);

impl IkaAddress {
    pub const ZERO: Self = Self([0u8; IKA_ADDRESS_LENGTH]);
    ...
    /// Deserialize into an `Option<IkaAddress>`.
    pub fn optional_address_from_hex<'de, D>(
        deserializer: D,
    ) -> Result<Option<IkaAddress>, D::Error>
    where
    ...
    /// Return the underlying byte array of a IkaAddress.
    pub fn to_inner(self) -> [u8; IKA_ADDRESS_LENGTH] {
        self.0
    }

    /// Parse a IkaAddress from a byte array or buffer.
    pub fn from_bytes<T: AsRef<[u8]>>(bytes: T) -> Result<Self, IkaError> {
        <[u8; IKA_ADDRESS_LENGTH]>::try_from(bytes.as_ref())
            .map_err(|_| IkaError::InvalidAddress)
            .map(IkaAddress)
    }
    ...
    pub fn try_from_padded(inputs: &ZkLoginInputs) -> IkaResult<Self> {
        Ok((&PublicKey::from_zklogin_inputs(inputs)?).into())
    }

    /// Define as iss_bytes_len || iss_bytes || unpadded_32_byte_address_seed.
    pub fn try_from_unpadded(inputs: &ZkLoginInputs) -> IkaResult<Self> {
        ...
        Ok(IkaAddress(hasher.finalize().digest))
    }
}

impl From<ObjectID> for IkaAddress {
    fn from(object_id: ObjectID) -> IkaAddress {
        Self(object_id.into_bytes())
    }
}

impl From<AccountAddress> for IkaAddress {
    fn from(address: AccountAddress) -> IkaAddress {
        Self(address.into_bytes())
    }
}

impl TryFrom<&[u8]> for IkaAddress {
    type Error = IkaError;

    /// Tries to convert the provided byte array into a IkaAddress.
...
impl TryFrom<Vec<u8>> for IkaAddress {
    ...
    /// Tries to convert the provided byte buffer into a IkaAddress.
    fn try_from(bytes: Vec<u8>) -> Result<Self, IkaError> {
        Self::from_bytes(bytes)
    }
}

impl AsRef<[u8]> for IkaAddress {
    fn as_ref(&self) -> &[u8] {
        &self.0[..]
    }
}
...
impl<T: IkaPublicKey> From<&T> for IkaAddress {
    fn from(pk: &T) -> Self {
        let mut hasher = DefaultHash::default();
        hasher.update([T::SIGNATURE_SCHEME.flag()]);
        hasher.update(pk);
        let g_arr = hasher.finalize();
        IkaAddress(g_arr.digest)
    }
}

impl From<&PublicKey> for IkaAddress {
...
impl From<&MultiSigPublicKey> for IkaAddress {
    /// Derive a IkaAddress from [struct MultiSigPublicKey]. A MultiSig address
    ...
    fn from(multisig_pk: &MultiSigPublicKey) -> Self {
        ...
        IkaAddress(hasher.finalize().digest)
    }
}

/// Ika address for [struct ZkLoginAuthenticator] is defined as the black2b hash of
/// [zklogin_flag || iss_bytes_length || iss_bytes || unpadded_address_seed_in_bytes].
impl TryFrom<&ZkLoginAuthenticator> for IkaAddress {
    type Error = IkaError;
    fn try_from(authenticator: &ZkLoginAuthenticator) -> IkaResult<Self> {
        IkaAddress::try_from_unpadded(&authenticator.inputs)
    }
}

impl TryFrom<&GenericSignature> for IkaAddress {
    type Error = IkaError;
    /// Derive a IkaAddress from a serialized signature in Ika [GenericSignature].
    fn try_from(sig: &GenericSignature) -> IkaResult<Self> {
        match sig {
            ...
            GenericSignature::ZkLoginAuthenticator(zklogin) => {
                IkaAddress::try_from_unpadded(&zklogin.inputs)
            }
            GenericSignature::PasskeyAuthenticator(s) => Ok(IkaAddress::from(&s.get_pk()?)),
        }
    }
}

impl fmt::Display for IkaAddress {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "0x{}", Hex::encode(self.0))
    }
}

impl fmt::Debug for IkaAddress {
...
/// Generate a fake IkaAddress with repeated one byte.
pub fn dbg_addr(name: u8) -> IkaAddress {
    let addr = [name; IKA_ADDRESS_LENGTH];
    IkaAddress(addr)
}
...
impl TxContext {
    pub fn new(sender: &IkaAddress, digest: &TransactionDigest, epoch_data: &EpochData) -> Self {
    ...
    pub fn kind(view: &CompiledModule, s: &SignatureToken) -> TxContextKind {
        ...
        let is_tx_context_type = module_name == TX_CONTEXT_MODULE_NAME
            && module_addr == &IKA_FRAMEWORK_ADDRESS
    ...
    pub fn sender(&self) -> IkaAddress {
        IkaAddress::from(ObjectID(self.sender))
    }
    ...
    pub fn random_for_testing_only() -> Self {
        Self::new(
            &IkaAddress::random_for_testing_only(),
    ...
    pub fn with_sender_for_testing_only(sender: &IkaAddress) -> Self {
...
impl ObjectID {
    ...
    pub fn is_clock(&self) -> bool {
        *self == IKA_CLOCK_OBJECT_ID
    }
}

impl From<IkaAddress> for ObjectID {
    fn from(address: IkaAddress) -> ObjectID {
...
impl From<IkaAddress> for AccountAddress {
    fn from(address: IkaAddress) -> Self {
        Self::new(address.0)
    }
}
...
impl fmt::Display for MoveObjectType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}",
            to_ika_struct_tag_string(&s).map_err(fmt::Error::custom)?
        )
    }
}
