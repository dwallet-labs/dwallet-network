...
use pera_bridge::error::BridgeError;
use pera_bridge::eth_client::EthClient;
use pera_bridge::metered_eth_provider::MeteredEthHttpProvier;
use pera_bridge::retry_with_max_elapsed_time;
use tokio::task::JoinHandle;
use tracing::info;

use mysten_metrics::spawn_monitored_task;
use pera_bridge::abi::{EthBridgeEvent, EthPeraBridgeEvents};

use crate::metrics::BridgeIndexerMetrics;
use pera_bridge::metrics::BridgeMetrics;
use pera_bridge::types::{EthEvent, RawEthLog};
use pera_indexer_builder::indexer_builder::{DataMapper, DataSender, Datasource};
...
impl EthSubscriptionDatasource {
    pub fn new(
        eth_pera_bridge_contract_address: String,
        eth_ws_url: String,
        indexer_metrics: BridgeIndexerMetrics,
    ) -> Result<Self, anyhow::Error> {
        let bridge_address = EthAddress::from_str(&eth_pera_bridge_contract_address)?;
        Ok(Self {
            bridge_address,
            eth_ws_url,
            indexer_metrics,
...
impl<E: EthEvent> DataMapper<(E, Block<H256>, Transaction), ProcessedTxnData> for EthDataMapper {
    fn map(
        ...
        let transfer = match bridge_event {
            EthBridgeEvent::EthPeraBridgeEvents(bridge_event) => match bridge_event {
                EthPeraBridgeEvents::TokensDepositedFilter(bridge_event) => {
                    ...
                    ProcessedTxnData::TokenTransfer(TokenTransfer {
                        ...
                        data: Some(TokenTransferData {
                            ...
                            amount: bridge_event.pera_adjusted_amount,
                        }),
                    })
                }
                EthPeraBridgeEvents::TokensClaimedFilter(bridge_event) => {
                ...
                EthPeraBridgeEvents::PausedFilter(_)
                | EthPeraBridgeEvents::UnpausedFilter(_)
                | EthPeraBridgeEvents::UpgradedFilter(_)
                | EthPeraBridgeEvents::InitializedFilter(_) => {
