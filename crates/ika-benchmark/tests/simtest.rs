...
mod test {
    ...
    use ika_benchmark::bank::BenchmarkBank;
    use ika_benchmark::system_state_observer::SystemStateObserver;
    use ika_benchmark::workloads::adversarial::AdversarialPayloadCfg;
    use ika_benchmark::workloads::expected_failure::ExpectedFailurePayloadCfg;
    use ika_benchmark::workloads::workload::ExpectedFailureType;
    use ika_benchmark::workloads::workload_configuration::{
    ...
    use ika_config::{AUTHORITIES_DB_NAME, IKA_KEYSTORE_FILENAME};
    ...
    use ika_core::checkpoints::{CheckpointStore, CheckpointWatermark};
    ...
    use ika_protocol_config::{PerObjectCongestionControlMode, ProtocolConfig, ProtocolVersion};
    use ika_simulator::tempfile::TempDir;
    use ika_simulator::{configs::*, SimConfig};
    use ika_storage::blob::Blob;
    use ika_surfer::surf_strategy::SurfStrategy;
    use ika_swarm_config::network_config_builder::ConfigBuilder;
    use ika_types::base_types::{ConciseableName, ObjectID, SequenceNumber};
    ...
    fn get_keep_alive_nodes(cluster: &TestCluster) -> HashSet<ika_simulator::task::NodeId> {
        ...
        keep_alive_nodes.insert(ika_simulator::current_simnode_id());
        keep_alive_nodes
    }

    fn handle_failpoint(
        dead_validator: Arc<Mutex<Option<DeadValidator>>>,
        keep_alive_nodes: HashSet<ika_simulator::task::NodeId>,
    ...
    async fn test_simulated_load_reconfig_with_prune_and_compact() {
        ...
        let node_state = test_cluster.fullnode_handle.ika_node.clone().state();
    ...
    async fn test_protocol_upgrade_compatibility_impl() {
        ...
        let manifest = ika_framework_snapshot::load_bytecode_snapshot_manifest();
        ...
        let init_framework =
            ika_framework_snapshot::load_bytecode_snapshot(starting_version).unwrap();
        let test_cluster = Arc::new(
        ...
        let _handle = tokio::task::spawn(async move {
            ...
            for version in starting_version..=max_ver {
                ...
                let new_framework = ika_framework_snapshot::load_bytecode_snapshot(next_version);
    ...
    fn handle_bool_failpoint(
        eligible_nodes: &HashSet<ika_simulator::task::NodeId>, // only given eligible nodes may fail
        probability: f64,
    ) -> bool {
        if !eligible_nodes.contains(&ika_simulator::current_simnode_id()) {
    ...
    async fn test_simulated_load_with_test_config(
        ...
        let keystore_path = test_cluster.swarm.dir().join(IKA_KEYSTORE_FILENAME);
        ...
        let surfer_task = tokio::spawn(async move {
            // now do a ika-surfer test
            let mut test_packages_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
            test_packages_dir.extend(["..", "..", "crates", "ika-surfer", "tests"]);
            let test_package_paths: Vec<PathBuf> = std::fs::read_dir(test_packages_dir)
                ...
                .collect();
            info!("using ika_surfer test packages: {test_package_paths:?}");

            let surf_strategy = SurfStrategy::new(Duration::from_millis(400));
            let results = ika_surfer::run_with_test_cluster_and_strategy(
                surf_strategy,
                test_duration,
                test_package_paths,
                test_cluster,
            ...
            .await;
            info!("ika_surfer test complete with results: {results:?}");
