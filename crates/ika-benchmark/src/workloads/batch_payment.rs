...
use std::sync::Arc;
use ika_core::test_utils::make_pay_ika_transaction;
...
/// Value of each address's "primary coin" in nika. The first transaction gives
/// each address a coin worth PRIMARY_COIN_VALUE, and all subsequent transfers
/// send TRANSFER_AMOUNT coins each time
const PRIMARY_COIN_VALUE: u64 = 100 * NIKA_PER_IKA;

/// Number of nika sent to each address on each batch transfer
...
impl Payload for BatchPaymentTestPayload {
    ...
    fn make_transaction(&mut self) -> Transaction {
        let addrs = self.state.addresses().cloned().collect::<Vec<IkaAddress>>();
...
impl Workload<dyn Payload> for BatchPaymentWorkload {
    ...
    async fn make_test_payloads(
        ...
        let mut gas_by_address: HashMap<IkaAddress, Vec<Gas>> = HashMap::new();
        debug!(
