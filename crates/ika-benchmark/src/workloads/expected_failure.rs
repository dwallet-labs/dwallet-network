...
use std::fmt;
use std::sync::Arc;
use ika_core::test_utils::make_transfer_object_transaction;
use ika_types::base_types::IkaAddress;
use ika_types::crypto::{AccountKeyPair, Ed25519IkaSignature};
use ika_types::signature::GenericSignature;
use ika_types::{base_types::ObjectRef, crypto::get_key_pair, transaction::Transaction};
use tracing::debug;
...
pub struct ExpectedFailurePayload {
    ...
    transfer_from: IkaAddress,
    transfer_to: IkaAddress,
    gas: Vec<Gas>,
        ...
        match self.failure_type {
            ExpectedFailureType::InvalidSignature => {
                ...
                signatures.push(GenericSignature::Signature(
                    ika_types::crypto::Signature::Ed25519IkaSignature(
...
impl Workload<dyn Payload> for ExpectedFailureWorkload {
    ...
    async fn make_test_payloads(
        ...
        let addresses: Vec<IkaAddress> = gas_by_address.keys().cloned().collect();
