...
trait IntoIkaResult<T> {
    fn into_ika_result(self) -> IkaResult<T>;
}

impl<T, E> IntoIkaResult<T> for Result<T, E>
where
    E: std::error::Error,
{
    fn into_ika_result(self) -> IkaResult<T> {
        self.map_err(|e| IkaError::Storage(e.to_string()))
    }
}
...
impl HttpKVStore {
    pub fn new_kv(
        ...
        cache_size: u64,
        metrics: Arc<KeyValueStoreMetrics>,
    ) -> IkaResult<TransactionKeyValueStore> {
    ...
    pub fn new(
        ...
        let base_url = Url::parse(&base_url).into_ika_result()?;
    ...
    fn get_url(&self, key: &Key) -> IkaResult<Url> {
        let (digest, item_type) = key.to_path_elements();
        let joined = self
            .base_url
            .join(&format!("{}/{}", digest, item_type))
            .into_ika_result()?;
        Url::from_str(joined.as_str()).into_ika_result()
    }

    async fn multi_fetch(&self, uris: Vec<Key>) -> Vec<IkaResult<Option<Bytes>>> {
    ...
    async fn fetch(&self, key: Key) -> IkaResult<Option<Bytes>> {
        ...
        if resp.status().is_success() {
            let bytes = resp.bytes().await.into_ika_result()?;
...
fn map_fetch<'a, K>(fetch: (&'a IkaResult<Option<Bytes>>, &'a K)) -> Option<(&'a Bytes, &'a K)>
where
    K: std::fmt::Debug,
{
    let (fetch, key) = fetch;
