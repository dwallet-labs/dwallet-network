The `ika-metric-checker` crate is used for querying prometheus metrics and validating the results. It will primarily be used to check for performance regressions in nightly deployments. Requires `api_key`, `api_user` & prometheus `url` which can be found in `ika-ops` repo or by asking the PE team.
...
```
RUST_LOG=debug cargo run --package ika-metric-checker --bin ika-metric-checker  -- --api-key xxxxxxxx --api-user xxxx_metrics --config checks.yaml --url https://xxxx.ika.io/prometheus
```
