...
use ika_graphql_rpc::client::simple_client::GraphqlQueryVariable;
use ika_graphql_rpc::client::ClientError;
use ika_graphql_rpc::config::Limits;
use ika_graphql_rpc::config::ServiceConfig;
use ika_graphql_rpc::test_infra::cluster::prep_executor_cluster;
use ika_graphql_rpc::test_infra::cluster::start_cluster;
...
use ika_types::DEEPBOOK_ADDRESS;
use ika_types::IKA_FRAMEWORK_ADDRESS;
use ika_types::IKA_FRAMEWORK_PACKAGE_ID;
...
async fn test_simple_client_simulator_cluster() {
    ...
    let cluster = ika_graphql_rpc::test_infra::cluster::serve_executor(
        Arc::new(sim),
        None,
        None,
...
async fn test_graphql_client_variables() {
    ...
    assert_eq!(
        data.get("obj1")
            ...
            .as_str()
            .unwrap(),
        IKA_FRAMEWORK_ADDRESS.to_canonical_string(true)
    );
    assert_eq!(
        data.get("obj2")
            .unwrap()
...
async fn test_transaction_execution() {
    ...
    let tx = cluster
        ...
        .await
        .transfer_ika(Some(1_000), recipient)
        .build();
...
async fn test_zklogin_sig_verify() {
    ...
    use ika_types::utils::load_test_vectors;
    ...
    let tx_data = TestTransactionBuilder::new(zklogin_addr, gas, rgp)
        ...
        .build();
    let msg = IntentMessage::new(Intent::ika_transaction(), tx_data.clone());
...
async fn test_payload_using_vars_mutation_passes() {
    telemetry_subscribers::init_for_testing();
    let cluster = ika_graphql_rpc::test_infra::cluster::start_cluster(ServiceConfig {
        limits: Limits {
