...
use ika_graphql_rpc_client::simple_client::SimpleClient;
pub use ika_indexer::config::RetentionConfig;
pub use ika_indexer::config::SnapshotLagConfig;
use ika_indexer::errors::IndexerError;
use ika_indexer::store::PgIndexerStore;
use ika_indexer::test_utils::start_indexer_writer_for_testing;
use ika_pg_temp_db::{get_available_port, TempDb};
...
/// Takes in a simulated instantiation of a Ika blockchain and builds a cluster around it. This
...
pub async fn serve_executor(
    ...
    let executor_server_handle = tokio::spawn(async move {
        ika_rest_api::RestService::new_without_version(executor)
