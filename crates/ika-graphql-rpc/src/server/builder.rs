...
use crate::{
    ...
    types::query::{Query, IkaGraphQLSchema},
};
...
use ika_graphql_rpc_headers::LIMITS_HEADER;
use ika_indexer::db::check_db_migration_consistency;
...
impl ServerBuilder {
    ...
    pub async fn from_config(
        ...
        let ika_sdk_client = if let Some(url) = &config.tx_exec_full_node.node_rpc_url {
            Some(
                IkaClientBuilder::default()
                    ...
                    .build(url)
                    .await
                    .map_err(|e| Error::Internal(format!("Failed to create IkaClient: {}", e)))?,
            )
        } else {
...
async fn graphql_handler(
    ...
    schema: Extension<IkaGraphQLSchema>,
...
pub mod tests {
    ...
    use std::sync::Arc;
    use std::time::Duration;
    use ika_pg_temp_db::get_available_port;
    use ika_sdk::IkaClient;
    use ika_types::digests::get_mainnet_chain_identifier;
    ...
    async fn test_timeout() {
        ...
        cluster
            ...
            .await;
        // timeout test includes mutation timeout, which requires a [IkaClient] to be able to run
        ...
        async fn test_timeout(
            ...
            let schema = prep_schema(db_url, Some(cfg))
                .await
                .context_data(Some(ika_client.clone()))
                .extension(Timeout)
        ...
        let ika_client = wallet.get_client().await.unwrap();

        test_timeout(delay, timeout, query, &ika_client, db_url.clone())
            .await
            .into_result()
            .expect("Should complete successfully");

        // Should timeout
        let errs: Vec<_> = test_timeout(delay, delay, query, &ika_client, db_url.clone())
            .await
            .into_result()
            .unwrap_err()
            .into_iter()
