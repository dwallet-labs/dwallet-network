...
use ika_indexer::apis::{governance_api::exchange_rates, GovernanceReadApi};
        ...
        loop {
            tokio::select! {
                ...
                _ = self.epoch_rx.changed() => {
                    ...
                    let latest_ika_sytem_state = self.db.inner
                        .get_latest_ika_system_state()
                        .await.map_err(|_| error!("Failed to fetch latest Ika system state"));

                    if let Ok(latest_ika_sytem_state) = latest_ika_sytem_state {
                        let db = self.db.clone();
                        let governance_api = GovernanceReadApi::new(db.inner) ;
                        exchange_rates(&governance_api, &latest_ika_sytem_state)
                            .await
