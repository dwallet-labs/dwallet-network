...
use super::stake::StakedIkaDowncastError;
use super::ika_address::IkaAddress;
use super::ikans_registration::{DomainFormat, IkansRegistration, IkansRegistrationDowncastError};
...
use crate::types::stake::StakedIka;
use async_graphql::connection::Connection;
use async_graphql::*;
use ika_json_rpc::name_service::NameServiceConfig;
use ika_types::object::{Data, MoveObject as NativeMoveObject};
...
#[graphql(
    ...
    field(
        ...
        desc = "Determines whether a transaction can transfer this object, using the \
                TransferObjects transaction command or `ika::transfer::public_transfer`, both of \
...
pub(crate) enum IMoveObject {
    ...
    StakedIka(StakedIka),
    IkansRegistration(IkansRegistration),
}
...
impl MoveObject {
    ...
    async fn as_staked_ika(&self) -> Result<Option<StakedIka>> {
        match StakedIka::try_from(self) {
            Ok(coin) => Ok(Some(coin)),
            Err(StakedIkaDowncastError::NotAStakedIka) => Ok(None),
            Err(StakedIkaDowncastError::Bcs(e)) => Err(Error::Internal(format!(
                "Failed to deserialize StakedIka: {e}"
            )))
            .extend(),
        }
    }
    ...
    async fn as_ikans_registration(&self, ctx: &Context<'_>) -> Result<Option<IkansRegistration>> {
        let cfg: &NameServiceConfig = ctx.data_unchecked();
        let tag = IkansRegistration::type_(cfg.package_address.into());

        match IkansRegistration::try_from(self, &tag) {
            Ok(registration) => Ok(Some(registration)),
            Err(IkansRegistrationDowncastError::NotAIkansRegistration) => Ok(None),
            Err(IkansRegistrationDowncastError::Bcs(e)) => Err(Error::Internal(format!(
                "Failed to deserialize IkansRegistration: {e}",
            )))
            .extend(),
        }
    }
