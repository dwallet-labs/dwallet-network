...
use ika_indexer::models::objects::StoredHistoryObject;
use ika_json_rpc::name_service::{
    Domain as NativeDomain, NameRecord, NameServiceConfig, NameServiceError,
};
use ika_types::{base_types::IkaAddress as NativeIkaAddress, dynamic_field::Field, id::UID};

const MOD_REGISTRATION: &IdentStr = ident_str!("ikans_registration");
const TYP_REGISTRATION: &IdentStr = ident_str!("IkansRegistration");
...
/// Wrap IkaNS Domain type to expose as a string scalar in GraphQL.
#[derive(Debug)]
pub(crate) struct Domain(NativeDomain);

#[derive(Enum, Copy, Clone, Eq, PartialEq)]
#[graphql(remote = "ika_json_rpc::name_service::DomainFormat")]
...
pub(crate) struct NativeIkansRegistration {
    pub id: UID,
...
#[derive(Clone)]
pub(crate) struct IkansRegistration {
    /// Representation of this IkansRegistration as a generic Move object.
    pub super_: MoveObject,

    /// The deserialized representation of the Move object's contents.
    pub native: NativeIkansRegistration,
}
...
pub(crate) enum IkansRegistrationDowncastError {
    NotAIkansRegistration,
    Bcs(bcs::Error),
}

#[Object]
impl IkansRegistration {
    pub(crate) async fn address(&self) -> IkaAddress {
    ...
    pub(crate) async fn staked_ikas(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
        after: Option<object::Cursor>,
        last: Option<u64>,
        before: Option<object::Cursor>,
    ) -> Result<Connection<String, StakedIka>> {
        OwnerImpl::from(&self.super_.super_)
            .staked_ikas(ctx, first, after, last, before)
            .await
    }

    /// The domain explicitly configured as the default domain pointing to this object.
    pub(crate) async fn default_ikans_name(
        &self,
        ctx: &Context<'_>,
        format: Option<DomainFormat>,
    ) -> Result<Option<String>> {
        OwnerImpl::from(&self.super_.super_)
            .default_ikans_name(ctx, format)
            .await
    }

    /// The IkansRegistration NFTs owned by this object. These grant the owner the capability to
    /// manage the associated domain.
    pub(crate) async fn ikans_registrations(
        &self,
        ctx: &Context<'_>,
        first: Option<u64>,
        after: Option<object::Cursor>,
        last: Option<u64>,
        before: Option<object::Cursor>,
    ) -> Result<Connection<String, IkansRegistration>> {
        OwnerImpl::from(&self.super_.super_)
            .ikans_registrations(ctx, first, after, last, before)
            .await
    }
...
impl NameService {
    /// Lookup the IkaNS NameRecord for the given `domain` name. `config` specifies where to find
    ...
    /// Lookup the IkaNS Domain for the given `address`. `config` specifies where to find the domain
    ...
    pub(crate) async fn reverse_resolve_to_name(
        ...
        let field: Field<NativeIkaAddress, NativeDomain> = object
            .native
            .to_rust()
            .ok_or_else(|| Error::Internal("Malformed Ikans Domain".to_string()))?;
    ...
    async fn query_domain_expiration(
        ...
        // parent's `NameRecord`.
        let mut object_ids = vec![IkaAddress::from(config.record_field_id(&domain.0))];
        if domain.0.is_subdomain() {
            object_ids.push(IkaAddress::from(config.record_field_id(&domain.0.parent())));
        }
...
impl IkansRegistration {
    /// Query the database for a `page` of IkaNS registrations. The page uses the same cursor type
    ...
    pub(crate) async fn paginate(
        ...
        owner: IkaAddress,
        checkpoint_viewed_at: u64,
    ) -> Result<Connection<String, IkansRegistration>, Error> {
        let type_ = IkansRegistration::type_(config.package_address.into());
        ...
        Object::paginate_subtype(db, page, filter, checkpoint_viewed_at, |object| {
            ...
            let move_object = MoveObject::try_from(&object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a IkansRegistration, but it's not a Move Object.",
                ))
            })?;

            IkansRegistration::try_from(&move_object, &type_).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a IkansRegistration, but it is not."
                ))
            })
        })
        .await
    ...
    /// Return the type representing a `IkansRegistration` on chain. This can change from chain to
    /// chain (mainnet, testnet, devnet etc).
    pub(crate) fn type_(package: IkaAddress) -> StructTag {
        StructTag {
    ...
    // Because the type of the IkansRegistration object is not constant,
    // we need to take it in as a param.
    pub(crate) fn try_from(
        move_object: &MoveObject,
        tag: &StructTag,
    ) -> Result<Self, IkansRegistrationDowncastError> {
        if !move_object.native.is_type(tag) {
            return Err(IkansRegistrationDowncastError::NotAIkansRegistration);
        }

        Ok(Self {
            super_: move_object.clone(),
            native: bcs::from_bytes(move_object.native.contents())
                .map_err(IkansRegistrationDowncastError::Bcs)?,
        })
    }
}
