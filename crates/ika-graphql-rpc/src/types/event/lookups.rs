...
fn select_ev(sender: Option<IkaAddress>, from: &str) -> RawQuery {
...
pub(crate) fn select_sender(sender: IkaAddress) -> RawQuery {
    select_ev(Some(sender), "event_senders")
}

pub(crate) fn select_event_type(event_type: &TypeFilter, sender: Option<IkaAddress>) -> RawQuery {
    match event_type {
