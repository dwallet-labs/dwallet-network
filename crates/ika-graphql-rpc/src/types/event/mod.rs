...
use ika_indexer::models::{events::StoredEvent, transactions::StoredTransaction};
use ika_indexer::schema::{checkpoints, events};
use ika_types::base_types::ObjectID;
use ika_types::Identifier;
use ika_types::{
    base_types::IkaAddress as NativeIkaAddress, event::Event as NativeEvent, parse_ika_struct_tag,
};
...
/// A Ika node emits one of the following events:
/// Move event
/// Publish event
...
impl Event {
    ...
    async fn sender(&self) -> Result<Option<Address>> {
        if self.native.sender == NativeIkaAddress::ZERO {
            return Ok(None);
        }
...
impl Event {
    ...
    fn try_from_stored_event(
        ...
        let sender = NativeIkaAddress::from_bytes(sender_bytes)
            .map_err(|e| Error::Internal(e.to_string()))?;
        let package_id =
            ObjectID::from_bytes(&stored.package).map_err(|e| Error::Internal(e.to_string()))?;
        let type_ =
            parse_ika_struct_tag(&stored.event_type).map_err(|e| Error::Internal(e.to_string()))?;
