...
use ika_indexer::{models::display::StoredDisplay, schema::display};
...
pub(crate) fn get_value_from_move_struct(
    ...
    // todo: 10 is a carry-over from the ika-json-rpc implementation
    ...
    // TODO: move off dependency on IkaMoveValue
    let ika_move_value: IkaMoveValue = result.clone().into();

    match ika_move_value {
        IkaMoveValue::Option(move_option) => match move_option.as_ref() {
            Some(move_value) => Ok(move_value.to_string()),
            None => Ok("".to_string()),
        },
        IkaMoveValue::Vector(_) => Err(DisplayRenderError::Vector(var_name.to_string())),
        _ => Ok(ika_move_value.to_string()),
    }
}
