...
use ika_types::dynamic_field::visitor::{Field, FieldVisitor};
use ika_types::dynamic_field::{derive_dynamic_field_id, DynamicFieldInfo, DynamicFieldType};
...
use super::{
    base64::Base64, move_object::MoveObject, move_value::MoveValue, ika_address::IkaAddress,
};
...
impl DynamicField {
    ...
    async fn value(&self, ctx: &Context<'_>) -> Result<Option<DynamicFieldValue>> {
        ...
        if kind == DynamicFieldType::DynamicObject {
            let df_object_id: IkaAddress = bcs::from_bytes(value_bytes)
...
fn apply_filter(query: RawQuery, parent: IkaAddress, parent_version: Option<u64>) -> RawQuery {
    let query = filter!(
        query,
        format!(
