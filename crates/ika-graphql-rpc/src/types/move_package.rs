...
use crate::types::ika_address::addr;
...
use ika_indexer::models::objects::StoredFullHistoryObject;
use ika_indexer::schema::packages;
use ika_package_resolver::{error::Error as PackageCacheError, Package as ParsedMovePackage};
use ika_types::is_system_package;
use ika_types::{move_package::MovePackage as NativeMovePackage, object::Data};
...
impl MovePackage {
    ...
    pub(crate) fn try_from_serialized(
        ...
        let object = Object::new_serialized(
            IkaAddress::from_bytes(&history_object.object_id)
...
impl Loader<PackageVersionKey> for Db {
    type Value = IkaAddress;
    type Error = Error;

    async fn load(
        &self,
        keys: &[PackageVersionKey],
    ) -> Result<HashMap<PackageVersionKey, IkaAddress>, Error> {
        use packages::dsl;
...
impl Loader<LatestKey> for Db {
    ...
    async fn load(&self, keys: &[LatestKey]) -> Result<HashMap<LatestKey, IkaAddress>, Error> {
        use packages::dsl;
        let other = diesel::alias!(packages as other);
