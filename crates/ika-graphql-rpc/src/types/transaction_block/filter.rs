...
use crate::types::{digest::Digest, ika_address::IkaAddress, type_filter::FqNameFilter};
...
pub(crate) struct TransactionBlockFilter {
    ...
    pub affected_address: Option<IkaAddress>,
    ...
    #[cfg(feature = "staging")]
    pub affected_object: Option<IkaAddress>,

    /// Limit to transactions that were sent by the given address.
    pub sent_address: Option<IkaAddress>,
    ...
    pub input_object: Option<IkaAddress>,
    ...
    /// This filter will be removed with 1.36.0 (2024-10-14), or at least one release after
    /// `affectedObject` is introduced, whichever is later.
    pub changed_object: Option<IkaAddress>,
...
impl TransactionBlockFilter {
    ...
    pub(crate) fn explicit_sender(&self) -> Option<IkaAddress> {
            ...
            || matches!(
                ...
                (Some(kind), Some(signer))
                    if (kind == TransactionBlockKindInput::SystemTx)
                        != (signer == IkaAddress::from(NativeIkaAddress::ZERO))
            )
    }
}
