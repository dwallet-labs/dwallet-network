...
use ika_indexer::models::epoch::QueryableEpochInfo;
use ika_indexer::schema::epochs;
use ika_types::messages_checkpoint::CheckpointCommitment as EpochCommitment;
...
impl Epoch {
    ...
    async fn validator_set(&self, ctx: &Context<'_>) -> Result<Option<ValidatorSet>> {
        let system_state = ctx
            ...
            .fetch_ika_system_state(Some(self.stored.epoch as u64))
            .await?;
