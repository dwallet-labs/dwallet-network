...
use ika_indexer::apis::GovernanceReadApi;
use ika_types::committee::EpochId;
use ika_types::ika_system_state::PoolTokenExchangeRate;

use ika_types::base_types::IkaAddress as NativeIkaAddress;
...
use async_graphql::*;
use ika_indexer::apis::governance_api::exchange_rates;
use ika_types::ika_system_state::ika_system_state_summary::IkaValidatorSummary as NativeIkaValidatorSummary;
#[derive(Clone, Debug)]
pub(crate) struct Validator {
    pub validator_summary: NativeIkaValidatorSummary,
...
impl Loader<u64> for Db {
    type Value = (
        ...
        BTreeMap<NativeIkaAddress, Vec<(EpochId, PoolTokenExchangeRate)>>,
    );
    type Error = Error;
    ...
    async fn load(
        ...
        Error,
    > {
        let latest_ika_system_state = self
            .inner
            .get_latest_ika_system_state()
            .await
            .map_err(|_| Error::Internal("Failed to fetch latest Ika system state".to_string()))?;
        let governance_api = GovernanceReadApi::new(self.inner.clone());
        let exchange_rates = exchange_rates(&governance_api, &latest_ika_system_state)
            .await
        ...
        let epoch_to_filter_out = if let Some(epoch) = keys.first() {
            if epoch == &latest_ika_system_state.epoch {
                *epoch - 1
            } else {
                *epoch
            }
        } else {
            latest_ika_system_state.epoch - 1
        };
        ...
        let requested_epoch = match keys.first() {
            Some(x) => *x,
            None => latest_ika_system_state.epoch,
        };

        let mut r = HashMap::new();
        r.insert(
            requested_epoch,
            (latest_ika_system_state.stake_subsidy_start_epoch, results),
        );
...
impl Validator {
    ...
    async fn address(&self) -> Address {
        Address {
            address: IkaAddress::from(self.validator_summary.ika_address),
    ...
    async fn staking_pool_id(&self) -> IkaAddress {
    ...
    async fn staking_pool_ika_balance(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.staking_pool_ika_balance,
        ))
    }
    ...
    async fn pending_total_ika_withdraw(&self) -> Option<BigInt> {
        Some(BigInt::from(
            self.validator_summary.pending_total_ika_withdraw,
        ))
    }
    ...
    async fn apy(&self, ctx: &Context<'_>) -> Result<Option<u64>, Error> {
        ...
        let rates = exchange_rates
            .get(&self.validator_summary.ika_address)
            .ok_or_else(|| {
                Error::Internal(format!(
                    "Failed to get the exchange rate for this validator address {} for requested epoch {}",
                    self.validator_summary.ika_address, self.requested_for_epoch
                ))
            })?;
...
impl Validator {
    pub fn operation_cap_id(&self) -> IkaAddress {
        IkaAddress::from_array(**self.validator_summary.operation_cap_id)
    }
}
