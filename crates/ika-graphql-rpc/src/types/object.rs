...
use super::stake::StakedIka;
use super::ika_address::addr;
...
use super::{owner::Owner, ika_address::IkaAddress, transaction_block::TransactionBlock};
...
use ika_indexer::models::obj_indices::StoredObjectVersion;
use ika_indexer::models::objects::{StoredFullHistoryObject, StoredHistoryObject};
use ika_indexer::schema::{full_objects_history, objects_version};
use ika_indexer::types::ObjectStatus as NativeObjectStatus;
use ika_indexer::types::OwnerType;
...
pub(crate) struct ObjectFilter {
    ...
    pub owner: Option<IkaAddress>,

    /// Filter for live objects by their IDs.
    pub object_ids: Option<Vec<IkaAddress>>,
...
impl Object {
    ...
    pub(crate) async fn default_ikans_name(
        ...
        OwnerImpl::from(self).default_ikans_name(ctx, format).await
    }
    ...
    pub(crate) async fn owner(&self) -> Option<ObjectOwner> {
        ...
        match native.owner {
            O::AddressOwner(address) => {
                let address = IkaAddress::from(address);
...
impl ObjectFilter {
    ...
    /// it).
    fn keys(&self) -> Option<BTreeMap<IkaAddress, Option<u64>>> {
...
mod tests {
    ...
    fn test_owner_filter_intersection() {
        let f0 = ObjectFilter {
            owner: Some(IkaAddress::from_str("0x1").unwrap()),
            ..Default::default()
        };

        let f1 = ObjectFilter {
            owner: Some(IkaAddress::from_str("0x2").unwrap()),
            ..Default::default()
        };
    ...
    fn test_key_filter_intersection() {
        let i1 = IkaAddress::from_str("0x1").unwrap();
        let i2 = IkaAddress::from_str("0x2").unwrap();
        let i3 = IkaAddress::from_str("0x3").unwrap();
        let i4 = IkaAddress::from_str("0x4").unwrap();
