...
use ika_json_rpc_types::IkaArgument;
...
impl MoveCallTransaction {
    ...
    async fn package(&self) -> IkaAddress {
    ...
    fn from(pt: NativeProgrammableTransaction, checkpoint_viewed_at: u64) -> Self {
        ...
        match pt {
            ...
            N::Publish(modules, dependencies) => P::Publish(PublishTransaction {
                ...
                dependencies: dependencies.into_iter().map(IkaAddress::from).collect(),
            }),
...
impl From<IkaArgument> for TransactionArgument {
    fn from(argument: IkaArgument) -> Self {
        use IkaArgument as S;
        use TransactionArgument as A;
        match argument {
