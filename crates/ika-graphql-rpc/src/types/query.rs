...
use ika_types::transaction::{TransactionData, TransactionKind};
use ika_types::{gas_coin::GAS, transaction::TransactionDataAPI, TypeTag};
...
use super::ikans_registration::NameService;
...
pub(crate) type IkaGraphQLSchema = async_graphql::Schema<Query, Mutation, EmptySubscription>;
...
impl Query {
    ...
    async fn dry_run_transaction_block(
        ...
        let ika_sdk_client: &Option<IkaClient> = ctx
            .data()
            .map_err(|_| Error::Internal("Unable to fetch Ika SDK client".to_string()))
            .extend()?;
        let ika_sdk_client = ika_sdk_client
            .as_ref()
            .ok_or_else(|| Error::Internal("Ika SDK client not initialized".to_string()))
            .extend()?;
    ...
    async fn address(&self, ctx: &Context<'_>, address: IkaAddress) -> Result<Option<Address>> {
        let Watermark { checkpoint, .. } = *ctx.data()?;
    ...
    async fn resolve_ikans_address(
        &self,
        ctx: &Context<'_>,
        domain: Domain,
