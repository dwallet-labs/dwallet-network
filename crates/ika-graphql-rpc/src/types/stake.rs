...
use super::ikans_registration::{DomainFormat, IkansRegistration};
use super::transaction_block::{self, TransactionBlock, TransactionBlockFilter};
use super::type_filter::ExactTypeFilter;
use super::uint53::UInt53;
use super::{
    big_int::BigInt, epoch::Epoch, move_object::MoveObject, object, ika_address::IkaAddress,
};
use async_graphql::connection::Connection;
use async_graphql::*;
use move_core_types::language_storage::StructTag;
use ika_json_rpc_types::{Stake as RpcStakedIka, StakeStatus as RpcStakeStatus};
use ika_types::base_types::MoveObjectType;
use ika_types::governance::StakedIka as NativeStakedIka;
...
pub(crate) enum StakedIkaDowncastError {
    NotAStakedIka,
    Bcs(bcs::Error),
}

#[derive(Clone)]
pub(crate) struct StakedIka {
    /// Representation of this StakedIka as a generic Move Object.
    pub super_: MoveObject,

    /// Deserialized representation of the Move Object's contents as a
    /// `0x3::staking_pool::StakedIka`.
    pub native: NativeStakedIka,
}
...
impl StakedIka {
    ...
    async fn pool_id(&self) -> Option<IkaAddress> {
...
impl StakedIka {
    /// Query the database for a `page` of Staked IKA. The page uses the same cursor type as is used
    ...
    ) -> Result<Connection<String, StakedIka>, Error> {
        let type_: StructTag = MoveObjectType::staked_ika().into();
        ...
        Object::paginate_subtype(db, page, filter, checkpoint_viewed_at, |object| {
            ...
            let move_object = MoveObject::try_from(&object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a StakedIka, but it's not a Move Object.",
                ))
            })?;

            StakedIka::try_from(&move_object).map_err(|_| {
                Error::Internal(format!(
                    "Expected {address} to be a StakedIka, but it is not."
                ))
            })
        })
        .await
    ...
    /// The JSON-RPC representation of a StakedIka so that we can "cheat" to implement fields that
    /// are not yet implemented directly for GraphQL.
    ///
    /// TODO: Make this obsolete
    async fn rpc_stake(&self, ctx: &Context<'_>) -> Result<RpcStakedIka, Error> {
        ctx.data_unchecked::<PgManager>()
            .fetch_rpc_staked_ika(self.native.clone())
            .await
    }
}

impl TryFrom<&MoveObject> for StakedIka {
    type Error = StakedIkaDowncastError;

    fn try_from(move_object: &MoveObject) -> Result<Self, Self::Error> {
        if !move_object.native.is_staked_ika() {
            return Err(StakedIkaDowncastError::NotAStakedIka);
        }

        Ok(Self {
            super_: move_object.clone(),
            native: bcs::from_bytes(move_object.native.contents())
                .map_err(StakedIkaDowncastError::Bcs)?,
        })
    }
}
