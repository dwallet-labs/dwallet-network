...
use ika_json_rpc::name_service::{validate_label, Domain};
use ika_types::{
...
pub(crate) struct AppInfo {
    ...
    pub(crate) package_address: Option<IkaAddress>,
...
impl Name {
    ...
    pub(crate) fn type_(package_address: IkaAddress) -> StructTag {
        StructTag {
    ...
    pub(crate) fn to_dynamic_field_id(
        ...
        ika_types::dynamic_field::derive_dynamic_field_id(
            config.registry_id,
            &ika_types::TypeTag::Struct(Box::new(domain_type_tag)),
            &self.to_bytes(),
        )
    }
}
