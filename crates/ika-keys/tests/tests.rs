...
use ika_types::crypto::{DefaultHash, SignatureScheme, IkaSignatureInner};
use ika_types::{
    base_types::{IkaAddress, IKA_ADDRESS_LENGTH},
    crypto::Ed25519IkaSignature,
};
...
fn create_alias_keystore_file_test() {
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("ika.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    keystore
...
fn check_reading_aliases_file_correctly() {
    ...
    let temp_dir = TempDir::new().unwrap();
    let mut keystore_path = temp_dir.path().join("ika.keystore");
    let keystore_path_keep = temp_dir.path().join("ika.keystore");
    let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
    let kp = keystore
        .generate_and_add_new_key(SignatureScheme::ED25519, None, None, None)
        .unwrap();
...
fn keystore_no_aliases() {
    ...
    // ika.aliases file with the existing address in the ika.keystore,
...
/// This test confirms rust's implementation of mnemonic is the same with the Ika Wallet
#[test]
fn ika_wallet_address_mnemonic_test() -> Result<(), anyhow::Error> {
    let phrase = "result crisp session latin must fruit genuine question prevent start coconut brave speak student dismiss";
    let expected_address =
        IkaAddress::from_str("0x936accb491f0facaac668baaedcf4d0cfc6da1120b66f77fa6a43af718669973")?;
    ...
    assert_eq!(pubkey.flag(), Ed25519IkaSignature::SCHEME.flag());

    let mut hasher = DefaultHash::default();
    hasher.update([pubkey.flag()]);
    hasher.update(pubkey);
    let g_arr = hasher.finalize();
    let mut res = [0u8; IKA_ADDRESS_LENGTH];
    res.copy_from_slice(&AsRef::<[u8]>::as_ref(&g_arr)[..IKA_ADDRESS_LENGTH]);
    let address = IkaAddress::try_from(res.as_slice())?;
