...
use ika_types::crypto::{
    enum_dispatch, EncodeDecodeBase64, PublicKey, Signature, SignatureScheme, IkaKeyPair,
};
...
pub trait AccountKeystore: Send + Sync {
    fn add_key(&mut self, alias: Option<String>, keypair: IkaKeyPair) -> Result<(), anyhow::Error>;
    fn keys(&self) -> Vec<PublicKey>;
    fn get_key(&self, address: &IkaAddress) -> Result<&IkaKeyPair, anyhow::Error>;

    fn sign_hashed(&self, address: &IkaAddress, msg: &[u8]) -> Result<Signature, signature::Error>;
    ...
    where
        T: Serialize;
    fn addresses(&self) -> Vec<IkaAddress> {
        self.keys().iter().map(|k| k.into()).collect()
    }
    fn addresses_with_alias(&self) -> Vec<(&IkaAddress, &Alias)>;
    ...
    fn get_alias_by_address(&self, address: &IkaAddress) -> Result<String, anyhow::Error>;
    fn get_address_by_alias(&self, alias: String) -> Result<&IkaAddress, anyhow::Error>;
    ...
    ) -> Result<(IkaAddress, String, SignatureScheme), anyhow::Error> {
...
pub struct FileBasedKeystore {
    keys: BTreeMap<IkaAddress, IkaKeyPair>,
    aliases: BTreeMap<IkaAddress, Alias>,
    path: Option<PathBuf>,
}
...
impl AccountKeystore for FileBasedKeystore {
    ...
    fn add_key(&mut self, alias: Option<String>, keypair: IkaKeyPair) -> Result<(), anyhow::Error> {
        let address: IkaAddress = (&keypair.public()).into();
...
impl FileBasedKeystore {
        ...
        let aliases = if aliases_path.exists() {
            ...
            aliases
                ...
                .map(|alias| {
                    ...
                    key.map(|k| (Into::<IkaAddress>::into(&k), alias))
                })
                .collect::<Result<BTreeMap<_, _>, _>>()
                .map_err(|e| {
                    anyhow!(
    ...
    /// To see Bech32 format encoding, use `ika keytool export $IKA_ADDRESS` where
    /// $IKA_ADDRESS can be found with `ika keytool list`. Or use `ika keytool convert $BASE64_STR`
    ...
    pub fn key_pairs(&self) -> Vec<&IkaKeyPair> {
...
impl InMemKeystore {
    pub fn new_insecure_for_tests(initial_key_number: usize) -> Self {
        ...
        let keys = (0..initial_key_number)
            ...
            .map(|(ad, k)| (ad, IkaKeyPair::Ed25519(k)))
            .collect::<BTreeMap<IkaAddress, IkaKeyPair>>();
...
mod tests {
    ...
    fn validate_alias_test() {
        ...
        assert!(validate_alias("abc_123.ika").is_ok());
        // Not allowed
