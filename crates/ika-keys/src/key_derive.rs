...
use ika_types::{
    ...
    crypto::{SignatureScheme, IkaKeyPair},
    error::IkaError,
};
...
) -> Result<(IkaAddress, IkaKeyPair), IkaError> {
    ...
    match key_scheme {
        SignatureScheme::ED25519 => {
            ...
            let sk = Ed25519PrivateKey::from_bytes(&derived)
                .map_err(|e| IkaError::SignatureKeyGenError(e.to_string()))?;
            let kp: Ed25519KeyPair = sk.into();
            Ok((kp.public().into(), IkaKeyPair::Ed25519(kp)))
        }
        SignatureScheme::Secp256k1 => {
            ...
            Ok((kp.public().into(), IkaKeyPair::Secp256k1(kp)))
        }
        SignatureScheme::Secp256r1 => {
            ...
            Ok((kp.public().into(), IkaKeyPair::Secp256r1(kp)))
        }
        SignatureScheme::BLS12381
        | SignatureScheme::MultiSig
        | SignatureScheme::ZkLoginAuthenticator
        | SignatureScheme::PasskeyAuthenticator => Err(IkaError::UnsupportedFeatureError {
...
) -> Result<DerivationPath, IkaError> {
    match key_scheme {
        SignatureScheme::ED25519 => {
            match path {
                Some(p) => {
                    ...
                    if let &[purpose, coin_type, account, change, address] = p.as_ref() {
                            ...
                            && address.is_hardened()
                        {
                            ...
                            Err(IkaError::SignatureKeyGenError("Invalid path".to_string()))
                        }
                    } else {
                        Err(IkaError::SignatureKeyGenError("Invalid path".to_string()))
                    }
                ...
                .parse()
                .map_err(|_| IkaError::SignatureKeyGenError("Cannot parse path".to_string()))?),
            }
        }
        SignatureScheme::Secp256k1 => {
            match path {
...
) -> Result<(IkaAddress, IkaKeyPair, SignatureScheme, String), anyhow::Error> {
