...
use ika_types::storage::{get_module, load_package_object_from_object_store, PackageObject};
use ika_types::{
    base_types::{AuthorityName, ObjectID, SequenceNumber, IkaAddress},
...
impl InMemoryStore {
    ...
    pub fn get_system_state(&self) -> ika_types::ika_system_state::IkaSystemState {
        ika_types::ika_system_state::get_ika_system_state(self).expect("system state must exist")
    }

    pub fn get_clock(&self) -> ika_types::clock::Clock {
        self.get_object(&ika_types::IKA_CLOCK_OBJECT_ID)
            .expect("clock should exist")
            .to_rust()
            .expect("clock object should deserialize")
    }

    pub fn owned_objects(&self, owner: IkaAddress) -> impl Iterator<Item = &Object> {
        self.live_objects
            .iter()
...
pub struct KeyStore {
    ...
    #[allow(unused)]
    account_keys: BTreeMap<IkaAddress, AccountKeyPair>,
}

impl KeyStore {
    pub fn from_network_config(
        network_config: &ika_swarm_config::network_config::NetworkConfig,
    ) -> Self {
    ...
    pub fn accounts(&self) -> impl Iterator<Item = (&IkaAddress, &AccountKeyPair)> {
