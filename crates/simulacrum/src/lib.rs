...
//! provides the [`Simulacrum`] type which is a implementation or instantiation of a ika
...
use ika_config::{genesis, transaction_deny_config::TransactionDenyConfig};
...
use ika_types::base_types::{AuthorityName, ObjectID, VersionNumber};
use ika_types::crypto::AuthoritySignature;
use ika_types::digests::ConsensusCommitDigest;
use ika_types::object::Object;
use ika_types::storage::{ObjectStore, ReadStore, RestStateReader};
use ika_types::ika_system_state::epoch_start_ika_system_state::EpochStartSystemState;
use ika_types::transaction::EndOfEpochTransactionKind;
use ika_types::{
    base_types::IkaAddress,
    committee::Committee,
    effects::TransactionEffects,
    error::ExecutionError,
    gas_coin::NIKA_PER_IKA,
...
use ika_types::messages_checkpoint::{CheckpointContents, CheckpointSequenceNumber};
use ika_types::mock_checkpoint_builder::{MockCheckpointBuilder, ValidatorKeypairProvider};
use ika_types::{
    gas_coin::GasCoin,
...
/// A `Simulacrum` of Ika.
///
/// This type represents a simulated instantiation of a Ika blockchain that needs to be driven
...
impl<R, S: store::SimulatorStore> Simulacrum<R, S> {
    pub fn new_with_network_config_store(config: &NetworkConfig, rng: R, store: S) -> Self {
        ...
        let epoch_state = EpochState::new(genesis.ika_system_object());
    ...
    /// Request that `amount` NIka be sent to `address` from a faucet account.
    ///
    /// ```
    /// use simulacrum::Simulacrum;
    /// use ika_types::base_types::IkaAddress;
    /// use ika_types::gas_coin::NIKA_PER_IKA;
    ///
    /// # fn main() {
    /// let mut simulacrum = Simulacrum::new();
    /// let address = IkaAddress::generate(simulacrum.rng());
    /// simulacrum.request_gas(address, NIKA_PER_IKA).unwrap();
    ///
    /// // `account` now has a Coin<IKA> object with single IKA in it.
    /// // ...
    /// # }
    /// ```
    pub fn request_gas(&mut self, address: IkaAddress, amount: u64) -> Result<TransactionEffects> {
        ...
        let object = self
            ...
            .find(|object| {
                object.is_gas_coin() && object.get_coin_value_unsafe() > amount + NIKA_PER_IKA
            })
            .ok_or_else(|| {
                anyhow!("unable to find a coin with enough to satisfy request for {amount} NIka")
            })?;

        let gas_data = ika_types::transaction::GasData {
        ...
        let pt = {
            let mut builder =
                ika_types::programmable_transaction_builder::ProgrammableTransactionBuilder::new();
            builder.transfer_ika(address, Some(amount));
            builder.finish()
        };

        let kind = ika_types::transaction::TransactionKind::ProgrammableTransaction(pt);
        let tx_data =
            ika_types::transaction::TransactionData::new_with_gas_data(kind, *sender, gas_data);
...
impl<T, V: store::SimulatorStore> ReadStore for Simulacrum<T, V> {
    ...
    ) -> ika_types::storage::error::Result<Option<std::sync::Arc<Committee>>> {
        todo!()
    }
    ...
    fn get_checkpoint_contents_by_digest(
        &self,
        digest: &ika_types::messages_checkpoint::CheckpointContentsDigest,
    ...
    fn get_transaction(
        &self,
        tx_digest: &ika_types::digests::TransactionDigest,
    ...
    fn get_events(
        &self,
        event_digest: &ika_types::digests::TransactionEventsDigest,
    ) -> ika_types::storage::error::Result<Option<ika_types::effects::TransactionEvents>> {
    ...
    fn get_full_checkpoint_contents(
        &self,
        _digest: &ika_types::messages_checkpoint::CheckpointContentsDigest,
    ) -> ika_types::storage::error::Result<
        Option<ika_types::messages_checkpoint::FullCheckpointContents>,
    > {
        todo!()
...
impl<T: Send + Sync, V: store::SimulatorStore + Send + Sync> RestStateReader for Simulacrum<T, V> {
    fn get_transaction_checkpoint(
        &self,
        _digest: &ika_types::digests::TransactionDigest,
    ) -> ika_types::storage::error::Result<
        Option<ika_types::messages_checkpoint::CheckpointSequenceNumber>,
    > {
        todo!()
    }
    ...
    fn account_owned_objects_info_iter(
        ...
        Box<dyn Iterator<Item = ika_types::storage::AccountOwnedObjectInfo> + '_>,
    > {
        todo!()
    }
    ...
    fn dynamic_field_iter(
        ...
        Box<
            dyn Iterator<
                    Item = (
                        ika_types::storage::DynamicFieldKey,
                        ika_types::storage::DynamicFieldIndexInfo,
                    ),
                > + '_,
        >,
    > {
    ...
    ) -> ika_types::storage::error::Result<Option<ika_types::storage::CoinInfo>> {
        todo!()
    }
}
...
impl Simulacrum {
    ...
    /// existing code for generating transactions in ika-test-transaction-builder by defining a trait
    /// that both WalletContext and Simulacrum implement. Then we can remove this function.
    pub fn transfer_txn(&mut self, recipient: IkaAddress) -> (Transaction, u64) {
        let (sender, key) = self.keystore().accounts().next().unwrap();
        let sender = *sender;
        ...
        let pt = {
            ...
            builder.transfer_ika(recipient, Some(transfer_amount));
            builder.finish()
        };
...
mod tests {
    ...
    use ika_types::{
        base_types::IkaAddress, effects::TransactionEffectsAPI, gas_coin::GasCoin,
