...
use ika_types::base_types::{ObjectID, ObjectRef, SequenceNumber, VersionNumber};
use ika_types::committee::EpochId;
use ika_types::digests::{ObjectDigest, TransactionDigest};
use ika_types::in_memory_storage::InMemoryStorage;
use ika_types::storage::{ObjectKey, ObjectStore};
use tracing::debug;
...
use ika_types::messages_checkpoint::{CheckpointSequenceNumber, ECMHLiveObjectSetDigest};
...
impl StateAccumulator {
    ...
    pub async fn digest_epoch(
        ...
        epoch_store: Arc<AuthorityPerEpochStore>,
        last_checkpoint_of_epoch: CheckpointSequenceNumber,
    ) -> IkaResult<ECMHLiveObjectSetDigest> {
        Ok(self
            .accumulate_epoch(epoch_store, last_checkpoint_of_epoch)
            .await?
            .digest()
