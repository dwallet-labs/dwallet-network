...
use ika_types::base_types::{random_object_ref, ObjectID};
...
use ika_types::crypto::{AuthorityKeyPair, Signer};
use ika_types::effects::{SignedTransactionEffects, TestEffectsBuilder};
...
use ika_types::{
    base_types::{AuthorityName, ExecutionDigests, ObjectRef, IkaAddress, TransactionDigest},
    committee::Committee,
...
) -> Result<(CertifiedTransaction, SignedTransactionEffects), IkaError> {
...
pub fn compile_managed_coin_package() -> CompiledPackage {
    compile_example_package("../../crates/ika-core/src/unit_tests/data/managed_coin")
}
...
async fn init_genesis(
    ...
    let mut builder = ika_genesis_builder::Builder::new().add_objects(genesis_objects);
    ...
    for i in 0..committee_size {
        ...
        let account_key_pair: IkaKeyPair = get_key_pair::<AccountKeyPair>().1.into();
        let network_key_pair: NetworkKeyPair = get_key_pair().1;
        let validator_info = ValidatorInfo {
            name: format!("validator-{i}"),
            protocol_key: authority_name,
            worker_key: worker_name,
            account_address: IkaAddress::from(&account_key_pair.public()),
...
pub fn make_transfer_ika_transaction(
...
pub fn make_pay_ika_transaction(
    gas_object: ObjectRef,
    coins: Vec<ObjectRef>,
    ...
    let data = TransactionData::new_pay_ika(
