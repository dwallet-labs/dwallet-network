...
use std::path::Path;
use std::sync::Arc;
use ika_config::ExecutionCacheConfig;
...
pub fn choose_execution_cache(config: &ExecutionCacheConfig) -> ExecutionCacheConfigType {
    #[cfg(msim)]
    {
        ...
        ika_macros::fail_point_if!("select-random-cache", || {
...
/// Should only be used for ika-tool or tests. Nodes must use build_execution_cache which
...
pub trait ObjectCacheRead: Send + Sync {
    fn get_package_object(&self, id: &ObjectID) -> IkaResult<Option<PackageObject>>;
    fn force_reload_system_packages(&self, system_package_ids: &[ObjectID]);

    fn get_object(&self, id: &ObjectID) -> IkaResult<Option<Object>>;

    fn get_objects(&self, objects: &[ObjectID]) -> IkaResult<Vec<Option<Object>>> {
    ...
    fn get_latest_object_or_tombstone(
        &self,
        object_id: ObjectID,
    ) -> IkaResult<Option<(ObjectKey, ObjectOrTombstone)>>;
        ...
        for (object_opt, object_ref) in objects.into_iter().zip(object_refs) {
            match object_opt {
                None => {
                    ...
                    return Err(IkaError::UserInputError { error });
                }
                Some(object) => {
                    result.push(object);
                }
    ...
    fn get_last_shared_object_deletion_info(
        ...
        object_id: &ObjectID,
        epoch_id: EpochId,
    ) -> IkaResult<Option<(SequenceNumber, TransactionDigest)>> {
    ...
    fn get_deleted_shared_object_previous_tx_digest(
        ...
        version: SequenceNumber,
        epoch_id: EpochId,
    ) -> IkaResult<Option<TransactionDigest>> {
...
pub trait TransactionCacheRead: Send + Sync {
    ...
    ) -> IkaResult<Option<Arc<VerifiedTransaction>>> {
    ...
    fn get_transactions_and_serialized_sizes(
        &self,
        digests: &[TransactionDigest],
    ) -> IkaResult<Vec<Option<(VerifiedTransaction, usize)>>> {
    ...
    fn is_tx_already_executed(&self, digest: &TransactionDigest) -> IkaResult<bool> {
    ...
    fn get_events(&self, digest: &TransactionEventsDigest) -> IkaResult<Option<TransactionEvents>> {
    ...
    fn notify_read_executed_effects<'a>(
        &'a self,
        digests: &'a [TransactionDigest],
    ) -> BoxFuture<'a, IkaResult<Vec<TransactionEffects>>> {
        async move {
...
pub trait ExecutionCacheReconfigAPI: Send + Sync {
    fn insert_genesis_object(&self, object: Object) -> IkaResult;
    fn bulk_insert_genesis_objects(&self, objects: &[Object]) -> IkaResult;

    fn revert_state_update(&self, digest: &TransactionDigest) -> IkaResult;
    ...
    fn expensive_check_ika_conservation(
        &self,
        old_epoch_store: &AuthorityPerEpochStore,
    ) -> IkaResult;

    fn checkpoint_db(&self, path: &Path) -> IkaResult;
    ...
    ($implementor: ident) => {
        ...
        impl BackingPackageStore for $implementor {
            fn get_package_object(
                &self,
                package_id: &ObjectID,
            ) -> IkaResult<Option<PackageObject>> {
    ...
    ($implementor: ident) => {
        ...
        impl ExecutionCacheReconfigAPI for $implementor {
            ...
            fn expensive_check_ika_conservation(
                ...
                self.store
                    .expensive_check_ika_conservation(self, old_epoch_store)
            }

            fn checkpoint_db(&self, path: &std::path::Path) -> IkaResult {
