...
use ika_types::{
    base_types::{random_object_ref, IkaAddress},
...
impl Scenario {
    ...
    fn new_outputs() -> TransactionOutputs {
        ...
        let (sender, keypair): (IkaAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
        let (receiver, _): (IkaAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
    ...
    pub async fn commit(&mut self, tx: TransactionDigest) -> IkaResult {
...
async fn latest_object_cache_race_test() {
    ...
    let owner = IkaAddress::random_for_testing_only();
