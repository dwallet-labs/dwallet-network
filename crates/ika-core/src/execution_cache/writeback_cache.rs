...
use crate::authority::authority_store::{
    ExecutionLockWriteGuard, LockDetailsDeprecated, ObjectLockStatus, IkaLockResult,
};
...
use ika_types::base_types::{EpochId, ObjectID, ObjectRef, SequenceNumber, VerifiedExecutionData};
...
impl WritebackCache {
    ...
    async fn persist_transactions(&self, digests: &[TransactionDigest]) -> IkaResult {
    ...
    fn revert_state_update_impl(&self, tx: &TransactionDigest) -> IkaResult {
...
impl ObjectCacheRead for WritebackCache {
    ...
    fn get_lock(&self, obj_ref: ObjectRef, epoch_store: &AuthorityPerEpochStore) -> IkaLockResult {
        ...
        match self.get_object_by_id_cache_only("lock", &obj_ref.0) {
            ...
            CacheResult::NegativeHit => {
                Err(IkaError::from(UserInputError::ObjectNotFound {
                    object_id: obj_ref.0,
...
fn do_fallback_lookup<K: Copy, V: Default + Clone>(
    keys: &[K],
    get_cached_key: impl Fn(&K) -> IkaResult<CacheResult<V>>,
    multiget_fallback: impl Fn(&[K]) -> IkaResult<Vec<V>>,
) -> IkaResult<Vec<V>> {
