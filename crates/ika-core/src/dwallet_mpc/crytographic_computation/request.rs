...
impl Request {
    ...
    pub(crate) fn compute(
        ...
        // TODO
        let current_consensus_round: u64 = 0;
        let mpc_round_to_consensus_rounds_delay: HashMap<u64, u64> = HashMap::new();
        match &self.request_input {
            MPCRequestInput::DWalletImportedKeyVerificationRequest(event_data) => {
                ...
                let result = guaranteed_output_delivery::Party::<
                    DWalletImportedKeyVerificationParty,
                >::advance_with_guaranteed_output(
                    session_id,
                    self.party_id,
                ...
                match result {
                    GuaranteedOutputDeliveryRoundResult::Advance { wrapped_message } => {
                        Ok(GuaranteedOutputDeliveryRoundResult::Advance { wrapped_message })
                    }
            ...
            MPCRequestInput::DKGFirst(..) => {
                ...
                let result = guaranteed_output_delivery::Party::<DWalletDKGFirstParty>::advance_with_guaranteed_output(
                    session_id,
                    self.party_id,
                    &self.access_structure,
                    current_consensus_round,
            ...
            MPCRequestInput::DKGSecond(event_data) => {
                ...
                let result = guaranteed_output_delivery::Party::<DWalletDKGSecondParty>::advance_with_guaranteed_output(
                    session_id,
                    self.party_id,
                    &self.access_structure,
                    current_consensus_round,
            ...
            MPCRequestInput::Presign(..) => {
                ...
                let result = guaranteed_output_delivery::Party::<PresignParty>::advance_with_guaranteed_output(
                    session_id,
                    self.party_id,
                    &self.access_structure,
                    current_consensus_round,
            ...
            MPCRequestInput::Sign(..) => {
                if let Some(decryption_key_shares) = self.decryption_key_shares.clone() {
                    ...
                    let result = guaranteed_output_delivery::Party::<SignFirstParty>::advance_with_guaranteed_output(
                        session_id,
                        self.party_id,
                        &self.access_structure,
                        current_consensus_round,
            ...
            MPCRequestInput::PartialSignatureVerification(event_data) => {
                let hashed_message = bcs::to_bytes(&message_digest(
            ...
            MPCRequestInput::NetworkEncryptionKeyReconfiguration(_) => {
                ...
                if let Some(decryption_key_shares) = self.decryption_key_shares.clone() {
                    ...
                    let result = guaranteed_output_delivery::Party::<ReconfigurationSecp256k1Party>::advance_with_guaranteed_output(
                        session_id,
                        self.party_id,
                        &self.access_structure,
                        current_consensus_round,
