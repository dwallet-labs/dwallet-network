...
use ika_storage::write_path_pending_tx_log::WritePathPendingTransactionLog;
...
use ika_types::transaction::{TransactionData, VerifiedTransaction};
...
impl TransactiondOrchestrator<NetworkAuthorityClient> {
    pub fn new_with_auth_aggregator(
        ...
        validator_state: Arc<AuthorityState>,
        reconfig_channel: Receiver<IkaSystemState>,
        parent_path: &Path,
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    async fn submit(
        ...
        request: ExecuteTransactionRequestV3,
        client_addr: Option<SocketAddr>,
    ) -> IkaResult<impl Future<Output = IkaResult<QuorumDriverResult>> + '_> {
        let tx_digest = *transaction.digest();
    ...
    async fn wait_for_finalized_tx_executed_locally_with_timeout(
        ...
        .await
        {
            ...
            Ok(Err(err)) => {
                ...
                Err(IkaError::TransactionOrchestratorLocalExecutionError {
                    error: err.to_string(),
                })
            }
            Ok(Ok(_)) => {
...
impl<A> ika_types::transaction_executor::TransactionExecutor for TransactiondOrchestrator<A>
where
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    ) -> Result<SimulateTransactionResult, IkaError> {
