...
use ika_macros::fail_point_if;
use ika_network::randomness;
use ika_types::base_types::AuthorityName;
use ika_types::committee::{Committee, EpochId, StakeUnit};
use ika_types::crypto::{AuthorityKeyPair, RandomnessRound};
use ika_types::error::{IkaError, IkaResult};
use ika_types::messages_consensus::VersionedDkgMessage;
use ika_types::messages_consensus::{ConsensusTransaction, VersionedDkgConfirmation};
...
impl RandomnessManager {
    ...
    pub async fn start_dkg(&mut self) -> IkaResult {
    ...
    pub(crate) fn reserve_next_randomness(
        ...
        output: &mut ConsensusCommitOutput,
    ) -> IkaResult<Option<RandomnessRound>> {
        let epoch_store = self.epoch_store()?;
    ...
    fn epoch_store(&self) -> IkaResult<Arc<AuthorityPerEpochStore>> {
        self.epoch_store
            .upgrade()
            .ok_or(IkaError::EpochEnded(self.epoch))
    }
...
impl RandomnessReporter {
    ...
    /// the round.
    pub fn notify_randomness_in_checkpoint(&self, round: RandomnessRound) -> IkaResult {
    ...
    async fn test_dkg(version: u64) {
        ...
        for validator in network_config.validator_configs.iter() {
            ...
            let randomness_manager = RandomnessManager::try_new(
                ...
                ika_network::randomness::Handle::new_stub(),
