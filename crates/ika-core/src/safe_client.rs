...
use ika_types::{base_types::*, committee::*, fp_ensure};
use ika_types::{
...
impl<C: Clone> SafeClient<C> {
    ...
    fn get_committee(&self, epoch_id: &EpochId) -> IkaResult<Arc<Committee>> {
        self.committee_store
            .get_committee(epoch_id)?
            .ok_or(IkaError::MissingCommitteeAtEpoch(*epoch_id))
    }
    ...
    ) -> IkaResult<SignedTransactionEffects> {
                ...
                match cert_opt {
                    Some(cert) => {
                        ...
                        ct.verify_committee_sigs_only(&committee).map_err(|e| {
                            IkaError::FailedToVerifyTxCertWithExecutedEffects {
    ...
    ) -> IkaResult<VerifiedObjectInfoResponse> {
...
{
    /// Initiate a new transfer to a Ika or Primary account.
    pub async fn handle_transaction(
        &self,
        transaction: Transaction,
        client_addr: Option<SocketAddr>,
    ) -> Result<PlainTransactionInfoResponse, IkaError> {
    ...
    ) -> IkaResult<HandleCertificateResponseV2> {
        let signed_effects =
    ...
    ) -> IkaResult<HandleCertificateResponseV3> {
        ...
        match (&events, effects.events_digest()) {
            ...
            (Some(events), None) => {
                if !events.data.is_empty() {
                    return Err(IkaError::ByzantineAuthoritySuspicion {
                        authority: self.address,
    ...
    pub async fn handle_object_info_request(
        &self,
        request: ObjectInfoRequest,
    ) -> Result<VerifiedObjectInfoResponse, IkaError> {
    ...
    fn verify_checkpoint_sequence(
        ...
        if let (Some(e), Some(o)) = (expected_seq, observed_seq) {
            fp_ensure!(
                e == o,
                IkaError::from("Expected checkpoint number doesn't match with returned")
            );
        }
        Ok(())
    }
    ...
    fn verify_contents_exist<T, O>(
        ...
        match (request_content, checkpoint, contents) {
            ...
            (true, Some(_), None) | (false, _, Some(_)) | (_, None, Some(_)) => Err(
                IkaError::from("Checkpoint contents inconsistent with request"),
            ),
            _ => Ok(()),
        }
    }
    ...
    #[instrument(level = "trace", skip_all, fields(authority = ?self.address.concise()))]
    pub async fn handle_system_state_object(&self) -> Result<IkaSystemState, IkaError> {
        self.authority_client
