...
use ika_network::tonic::metadata::KeyAndValueRef;
use ika_network::tonic::transport::Channel;
...
pub struct NetworkAuthorityClient {
    client: IkaResult<ValidatorClient<Channel>>,
}
...
impl NetworkAuthorityClient {
    pub async fn connect(
        ...
        let tls_config = tls_target.map(|tls_target| {
            ika_tls::create_rustls_client_config(
                tls_target,
    ...
    pub fn connect_lazy(address: &Multiaddr, tls_target: Option<NetworkPublicKey>) -> Self {
        ...
        let client: IkaResult<_> = mysten_network::client::connect_lazy(address, tls_config)
    ...
    fn new_lazy(client: IkaResult<Channel>) -> Self {
        Self {
            client: client.map(ValidatorClient::new),
        }
    }

    fn client(&self) -> IkaResult<ValidatorClient<Channel>> {
        self.client.clone()
    }
}
...
pub fn make_network_authority_clients_with_network_config(
    ...
    for (name, (_state, network_metadata)) in committee.validators() {
        ...
        //     )
        // });
        // TODO: Change below code to generate a IkaError if no valid TLS config is available.
