...
use ika_macros::{register_fail_point, sim_test};
use ika_types::base_types::IkaAddress;
use ika_types::base_types::TransactionDigest;
use ika_types::crypto::{deterministic_random_account_key, get_key_pair, AccountKeyPair};
use ika_types::effects::TransactionEffectsAPI;
use ika_types::object::{generate_test_gas_objects, Object};
...
async fn test_quorum_driver_object_locked() -> Result<(), anyhow::Error> {
    ...
    let (sender, keypair): (IkaAddress, AccountKeyPair) = deterministic_random_account_key();
