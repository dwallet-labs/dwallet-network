...
impl SharedObjVerManager {
    pub async fn assign_versions_from_consensus(
    ...
    ) -> IkaResult<ConsensusSharedObjVerAssignment> {
        ...
        if let Some(round) = randomness_round {
            ...
            let version = shared_input_next_versions
                .get_mut(&IKA_RANDOMNESS_STATE_OBJECT_ID)
            ...
            assigned_versions.push((
                ...
                vec![(IKA_RANDOMNESS_STATE_OBJECT_ID, *version)],
            ));
            version.increment();
        }
    ...
    pub async fn assign_versions_from_effects(
        ...
        epoch_store: &AuthorityPerEpochStore,
        cache_reader: &dyn ObjectCacheRead,
    ) -> IkaResult<AssignedTxAndVersions> {
            ...
            for SharedInputObject { id, .. } in shared_input_objects.iter() {
                let assigned_version = match cancellation_info {
                    ...
                    Some(CancelConsensusCertificateReason::DkgFailed) => {
                        if id == &IKA_RANDOMNESS_STATE_OBJECT_ID {
...
mod tests {
    ...
    use ika_types::effects::TestEffectsBuilder;
    ...
    use ika_types::transaction::{ObjectArg, SenderSignedData, TransactionKey};
    ...
    async fn test_assign_versions_from_consensus_with_randomness() {
        ...
        assert_eq!(
            epoch_store
                .get_next_object_version(&IKA_RANDOMNESS_STATE_OBJECT_ID)
                .unwrap(),
            randomness_obj_version
        );
        ...
        assert_eq!(
            ...
            HashMap::from([(IKA_RANDOMNESS_STATE_OBJECT_ID, next_randomness_obj_version)])
        );
        assert_eq!(
            assigned_versions,
            vec![
                (
                    ...
                    vec![(IKA_RANDOMNESS_STATE_OBJECT_ID, randomness_obj_version),]
                ),
                (
                    certs[0].key(),
                    // It is critical that the randomness object version is updated before the assignment.
                    vec![(IKA_RANDOMNESS_STATE_OBJECT_ID, next_randomness_obj_version)]
                ),
                (
                    certs[1].key(),
                    // It is critical that the randomness object version is updated before the assignment.
    ...
    async fn test_assign_versions_from_consensus_with_cancellation() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                (IKA_RANDOMNESS_STATE_OBJECT_ID, SequenceNumber::from_u64(1)), // not mutable
            ])
        );
