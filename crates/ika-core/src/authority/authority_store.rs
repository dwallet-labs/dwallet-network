...
use ika_config::node::AuthorityStorePruningConfig;
use ika_macros::fail_point_arg;
use ika_storage::mutex_table::{MutexGuard, MutexTable, RwLockGuard, RwLockTable};
...
use ika_types::ika_system_state::get_ika_system_state;
use ika_types::{base_types::SequenceNumber, fp_bail, fp_ensure};
...
use ika_types::gas_coin::TOTAL_SUPPLY_NIKA;
use typed_store::rocks::util::is_ref_count_value;

const NUM_SHARDS: usize = 4096;

struct AuthorityStoreMetrics {
    ika_conservation_check_latency: IntGauge,
    ika_conservation_live_object_count: IntGauge,
    ika_conservation_live_object_size: IntGauge,
    ika_conservation_imbalance: IntGauge,
    ika_conservation_storage_fund: IntGauge,
    ika_conservation_storage_fund_imbalance: IntGauge,
...
impl AuthorityStoreMetrics {
    pub fn new(registry: &Registry) -> Self {
        Self {
            ika_conservation_check_latency: register_int_gauge_with_registry!(
                "ika_conservation_check_latency",
                "Number of seconds took to scan all live objects in the store for IKA conservation check",
                registry,
            ).unwrap(),
            ika_conservation_live_object_count: register_int_gauge_with_registry!(
                "ika_conservation_live_object_count",
                "Number of live objects in the store",
                registry,
            ).unwrap(),
            ika_conservation_live_object_size: register_int_gauge_with_registry!(
                "ika_conservation_live_object_size",
                "Size in bytes of live objects in the store",
                registry,
            ).unwrap(),
            ika_conservation_imbalance: register_int_gauge_with_registry!(
                "ika_conservation_imbalance",
                "Total amount of IKA in the network - 10B * 10^9. This delta shows the amount of imbalance",
                registry,
            ).unwrap(),
            ika_conservation_storage_fund: register_int_gauge_with_registry!(
                "ika_conservation_storage_fund",
                "Storage Fund pool balance (only includes the storage fund proper that represents object storage)",
                registry,
            ).unwrap(),
            ika_conservation_storage_fund_imbalance: register_int_gauge_with_registry!(
                "ika_conservation_storage_fund_imbalance",
...
/// S is a template on Authority signature state. This allows IkaDataStore to be used on either
/// authorities or non-authorities. Specifically, when storing transactions and effects,
/// S allows IkaDataStore to either store the authority signed version or unsigned version.
pub struct AuthorityStore {
    ...
    /// Whether to enable expensive IKA conservation check at epoch boundaries.
    enable_epoch_ika_conservation_check: bool,
...
impl AuthorityStore {
    ...
    pub async fn open(
        ...
        let enable_epoch_ika_conservation_check = config
            .expensive_safety_check_config
            .enable_epoch_ika_conservation_check();
        ...
        let epoch_start_configuration = if perpetual_tables.database_is_empty()? {
            ...
            let epoch_start_configuration = EpochStartConfiguration::new(
                genesis.ika_system_object().into_epoch_start_state(),
    ...
    ) -> IkaResult<Option<TransactionEffects>> {
    ...
    pub fn effects_exists(&self, effects_digest: &TransactionEffectsDigest) -> IkaResult<bool> {
    ...
    pub async fn notify_read_root_state_hash(
        &self,
        epoch: EpochId,
    ) -> IkaResult<(CheckpointSequenceNumber, Accumulator)> {
    ...
    ) -> IkaResult<Option<(EpochId, CheckpointSequenceNumber)>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .get(digest)?)
    ...
    pub fn deprecated_multi_get_transaction_checkpoint(
        &self,
        digests: &[TransactionDigest],
    ) -> IkaResult<Vec<Option<(EpochId, CheckpointSequenceNumber)>>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .multi_get(digests)?
    ...
    pub fn database_is_empty(&self) -> IkaResult<bool> {
    ...
    pub fn multi_object_exists_by_key(&self, object_keys: &[ObjectKey]) -> IkaResult<Vec<bool>> {
        Ok(self
            .perpetual_tables
            .objects
    ...
    fn get_object_ref_prior_to_key(
        ...
        object_id: &ObjectID,
        version: VersionNumber,
    ) -> Result<Option<ObjectRef>, IkaError> {
    ...
    /// Get many objects
    pub fn get_objects(&self, objects: &[ObjectID]) -> Result<Vec<Option<Object>>, IkaError> {
    ...
    pub(crate) fn insert_genesis_object(&self, object: Object) -> IkaResult {
    ...
    fn insert_object_direct(&self, object_ref: ObjectRef, object: &Object) -> IkaResult {
    ...
    pub(crate) fn bulk_insert_genesis_objects(&self, objects: &[Object]) -> IkaResult<()> {
    ...
    pub fn bulk_insert_live_objects(
        ...
        if *expected_sha3_digest != sha3_digest {
            ...
            return Err(IkaError::from("Sha does not match"));
        }
        batch.write()?;
        Ok(())
    }
    ...
    pub fn get_epoch_start_configuration(&self) -> IkaResult<Option<EpochStartConfiguration>> {
        ...
        for (live_marker, lock, obj_ref) in izip!(
            ...
            if let Some(previous_tx_digest) = &lock {
                if previous_tx_digest == &tx_digest {
                    ...
                    return Err(IkaError::ObjectLockConflict {
                        obj_ref: *obj_ref,
    ...
    pub fn check_owned_objects_are_live(&self, objects: &[ObjectRef]) -> IkaResult {
        let locks = self
            .perpetual_tables
            .live_owned_object_markers
            .multi_get(objects)?;
    ...
    /// Returns IkaError::ObjectLockAlreadyInitialized if the lock already exists and is locked to a transaction
    ...
    pub fn initialize_live_object_markers(
        ...
        if !is_force_reset {
            ...
            if !existing_live_object_markers.is_empty() {
                ...
                return Err(IkaError::ObjectLockAlreadyInitialized {
    ...
    pub fn revert_state_update(&self, tx_digest: &TransactionDigest) -> IkaResult {
    ...
    ) -> IkaResult<Vec<Option<VerifiedTransaction>>> {
        Ok(self
            .perpetual_tables
            .transactions
    ...
    /// If the intent is for testing, you can use AuthorityState:: get_ika_system_state_object_for_testing.
    pub fn get_ika_system_state_object_unsafe(&self) -> IkaResult<IkaSystemState> {
        get_ika_system_state(self.perpetual_tables.as_ref())
    }

    pub fn expensive_check_ika_conservation<T>(
        self: &Arc<Self>,
        type_layout_store: T,
    ...
    {
        if !self.enable_epoch_ika_conservation_check {
            return Ok(());
        }

        let executor = old_epoch_store.executor();
        info!("Starting IKA conservation check. This may take a while..");
        let cur_time = Instant::now();
        let mut pending_objects = vec![];
        let mut count = 0;
        let mut size = 0;
        let (mut total_ika, mut total_storage_rebate) = thread::scope(|s| {
                        ...
                        if count % 1_000_000 == 0 {
                            ...
                            pending_tasks.push(s.spawn(move || {
                                ...
                                for object in task_objects {
                                    ...
                                    // get_total_ika includes storage rebate, however all storage rebate is
                                    // also stored in the storage fund, so we need to subtract it here.
                                    total_ika +=
                                        object.get_total_ika(layout_resolver.as_mut()).unwrap()
                                            - object.storage_rebate;
                                }
                                if count % 50_000_000 == 0 {
                                    info!("Processed {} objects", count);
                                }
                                (total_ika, total_storage_rebate)
                            }));
                        }
                    }
        ...
        let system_state = self
            ...
            .expect("Reading ika system state object cannot fail")
            .into_ika_system_state_summary();
        let storage_fund_balance = system_state.storage_fund_total_object_storage_rebates;
        info!(
            "Total IKA amount in the network: {}, storage fund balance: {}, total storage rebate: {} at beginning of epoch {}",
            total_ika, storage_fund_balance, total_storage_rebate, system_state.epoch
        );
        ...
        self.metrics
            .ika_conservation_imbalance
            .set((total_ika as i128 - TOTAL_SUPPLY_NIKA as i128) as i64);
        ...
        if let Some(expected_ika) = self
            .perpetual_tables
            .expected_network_ika_amount
            .get(&())
            .expect("DB read cannot fail")
        {
            fp_ensure!(
                total_ika == expected_ika,
                IkaError::from(
                    format!(
                        "Inconsistent state detected at epoch {}: total ika: {}, expecting {}",
                        system_state.epoch, total_ika, expected_ika
                    )
                    .as_str()
                )
            );
...
pub type IkaLockResult = IkaResult<ObjectLockStatus>;
