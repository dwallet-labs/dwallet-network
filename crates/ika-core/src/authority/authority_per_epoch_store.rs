...
use std::sync::Arc;
use ika_config::node::ExpensiveSafetyCheckConfig;
use ika_macros::fail_point_arg;
use ika_types::accumulator::Accumulator;
use ika_types::authenticator_state::{get_authenticator_state, ActiveJwk};
use ika_types::base_types::{AuthorityName, EpochId, ObjectID, SequenceNumber, TransactionDigest};
use ika_types::base_types::{ConciseableName, ObjectRef};
...
use ika_types::digests::{ChainIdentifier, TransactionEffectsDigest};
use ika_types::error::{IkaError, IkaResult};
use ika_types::signature::GenericSignature;
use ika_types::storage::{BackingPackageStore, InputKey, ObjectStore};
...
use ika_storage::mutex_table::{MutexGuard, MutexTable};
use ika_types::effects::TransactionEffects;
use ika_types::executable_transaction::{
...
use ika_types::messages_consensus::VersionedDkgConfirmation;
...
use ika_types::storage::GetSharedLocks;
use ika_types::ika_system_state::epoch_start_ika_system_state::{
...
pub enum ConsensusCertificateResult {
    ...
    IkaTransaction(VerifiedExecutableTransaction),
...
impl AuthorityEpochTables {
    ...
    pub fn remove_executed_tx_subtle(&self, digest: &TransactionDigest) -> IkaResult {
    ...
    pub fn get_locked_transaction(&self, obj_ref: &ObjectRef) -> IkaResult<Option<LockDetails>> {
        Ok(self
    ...
    ) -> IkaResult<Vec<Option<LockDetails>>> {
        Ok(self
            .owned_object_locked_transactions
    ...
    ) -> IkaResult<impl IntoIterator<Item = (ObjectID, u64)>> {
...
impl AuthorityPerEpochStore {
    ...
    pub fn insert_signed_transaction(&self, transaction: VerifiedSignedTransaction) -> IkaResult {
        Ok(self
            .tables()?
            .signed_transactions
    ...
    pub fn revert_executed_transaction(&self, tx_digest: &TransactionDigest) -> IkaResult {
        let tables = self.tables()?;
    ...
    pub fn get_effects_signature(
        &self,
        tx_digest: &TransactionDigest,
    ) -> IkaResult<Option<AuthoritySignInfo>> {
        let tables = self.tables()?;
    ...
    pub fn get_signed_effects_digest(
        &self,
        tx_digest: &TransactionDigest,
    ) -> IkaResult<Option<TransactionEffectsDigest>> {
        let tables = self.tables()?;
    ...
    pub fn get_transaction_cert_sig(
        &self,
        tx_digest: &TransactionDigest,
    ) -> IkaResult<Option<AuthorityStrongQuorumSignInfo>> {
                ...
                Ok(match kind {
                    InputObjectKind::SharedMoveObject { id, .. } => {
                        let shared_locks = shared_locks
                            ...
                            .ok_or(IkaError::GenericAuthorityError {
    ...
    ) -> IkaResult<Vec<(CheckpointSequenceNumber, Accumulator)>> {
        self.tables()?
            .state_hash_by_checkpoint
    ...
    ) -> IkaResult<Vec<Accumulator>> {
        self.checkpoint_state_notify_read
            .read(&checkpoints, |checkpoints| -> IkaResult<_> {
                Ok(self
                    .tables()?
                    .state_hash_by_checkpoint
    ...
    #[instrument(level = "trace", skip_all)]
    pub fn handle_committed_transactions(&self, digests: &[TransactionDigest]) -> IkaResult<()> {
    ...
    ) -> IkaResult<HashMap<ObjectID, SequenceNumber>> {
    ...
    ) -> IkaResult<Vec<(DeferralKey, Vec<VerifiedSequencedConsensusTransaction>)>> {
    ...
    pub fn user_signatures_for_checkpoint(
        ...
        digests: &[TransactionDigest],
    ) -> IkaResult<Vec<Vec<GenericSignature>>> {
    ...
    pub fn record_capabilities(&self, capabilities: &AuthorityCapabilitiesV1) -> IkaResult {
    ...
    /// Record most recently advertised capabilities of all authorities
    pub fn record_capabilities_v2(&self, capabilities: &AuthorityCapabilitiesV2) -> IkaResult {
    ...
    pub fn get_capabilities_v2(&self) -> IkaResult<Vec<AuthorityCapabilitiesV2>> {
        assert!(self.protocol_config.authority_capabilities_v2());
    ...
    pub(crate) fn get_new_jwks(&self, round: u64) -> IkaResult<Vec<ActiveJwk>> {
    ...
    #[instrument(level = "trace", skip_all)]
    pub fn verify_transaction(&self, tx: Transaction) -> IkaResult<VerifiedTransaction> {
    ...
    fn add_consensus_commit_prologue_transaction(
    ...
    ) -> IkaResult<Option<TransactionKey>> {
        ...
        let consensus_commit_prologue_root = match self.process_consensus_system_transaction(&transaction) {
            ConsensusCertificateResult::IkaTransaction(processed_tx) => {
    ...
    async fn process_consensus_transaction<C: CheckpointServiceNotify>(
        ...
        match &transaction {
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                // Safe because signatures are verified when consensus called into IkaTxValidator::validate_batch.
    ...
    fn process_consensus_user_transaction(
        ...
        Ok(ConsensusCertificateResult::IkaTransaction(transaction))
    }
    ...
    ) -> IkaResult<Vec<(CheckpointHeight, PendingCheckpointV2)>> {
        let tables = self.tables()?;
    ...
    ) -> IkaResult<Option<PendingCheckpointV2>> {
    ...
    pub fn last_built_checkpoint_builder_summary(
        &self,
    ) -> IkaResult<Option<BuilderCheckpointSummary>> {
        Ok(self
            .tables()?
            .builder_checkpoint_summary_v2
            .unbounded_iter()
    ...
    pub fn last_built_checkpoint_summary(
        &self,
    ) -> IkaResult<Option<(CheckpointSequenceNumber, CheckpointSummary)>> {
        Ok(self
            .tables()?
            .builder_checkpoint_summary_v2
            .unbounded_iter()
    ...
    pub fn get_built_checkpoint_summary(
        &self,
        sequence: CheckpointSequenceNumber,
    ) -> IkaResult<Option<CheckpointSummary>> {
        Ok(self
            .tables()?
            .builder_checkpoint_summary_v2
            .get(&sequence)?
...
impl GetSharedLocks for AuthorityPerEpochStore {
    fn get_shared_locks(
        &self,
        key: &TransactionKey,
    ) -> IkaResult<Option<Vec<(ObjectID, SequenceNumber)>>> {
