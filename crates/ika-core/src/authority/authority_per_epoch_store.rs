...
use ika_types::crypto::{
    AuthorityPublicKeyBytes, AuthoritySignInfo, AuthorityStrongQuorumSignInfo, AuthorityName,
};
...
use ika_types::messages_checkpoint::{
    CheckpointContents, CheckpointSequenceNumber, CheckpointSignatureMessage, CheckpointMessage,
};
use ika_types::messages_consensus::{Round, TimestampMs, VersionedDkgConfirmation};
use ika_types::messages_consensus::{
    AuthorityCapabilitiesV1, ConsensusTransaction,
...
use ika_types::digests::ActionDigest;
use ika_types::action::{ActionData, ActionKind};
...
pub struct AuthorityPerEpochStore {
    ...
    executed_transactions_to_checkpoint_notify_read:
        NotifyRead<ActionDigest, CheckpointSequenceNumber>,

    executed_digests_notify_read: NotifyRead<TransactionKey, ActionDigest>,
    ...
    mutex_table: MutexTable<ActionDigest>,
...
pub struct AuthorityEpochTables {
    ...
    executed_in_epoch: DBMap<ActionDigest, ()>,
    ...
    pub(crate) pending_execution: DBMap<ActionDigest, TrustedExecutableTransaction>,
    ...
    builder_digest_to_checkpoint: DBMap<ActionDigest, CheckpointSequenceNumber>,

    /// Maps non-digest TransactionKeys to the corresponding digest after execution, for use
    /// by checkpoint builder.
    transaction_key_to_digest: DBMap<TransactionKey, ActionDigest>,
    ...
    user_signatures_for_checkpoints: DBMap<ActionDigest, Vec<GenericSignature>>,
    ...
    pub(crate) executed_transactions_to_checkpoint:
        DBMap<ActionDigest, CheckpointSequenceNumber>,
}
...
impl AuthorityEpochTables {
    ...
    pub fn remove_executed_tx_subtle(&self, digest: &ActionDigest) -> IkaResult {
...
impl AuthorityPerEpochStore {
    ...
    pub fn computation_price_per_unit_size(&self) -> u64 {
        self.epoch_start_state().computation_price_per_unit_size()
    }
    ...
    pub fn handle_committed_transactions(&self, digests: &[ActionDigest]) -> IkaResult<()> {
    ...
    >(
        ...
        // PostConsensusTxReorder::reorder, so that for transactions with the same computation price, deferred transactions
        ...
        if make_checkpoint {
            ...
            let pending_checkpoint = PendingCheckpoint::V1(PendingCheckpointV1 {
                messages: verified_transactions.clone(),
    ...
    pub(crate) async fn process_consensus_transactions<C: CheckpointServiceNotify>(
        ...
        Vec<ActionData>,    // transactions to schedule
        ...
        let mut cancelled_txns: BTreeMap<ActionDigest, CancelConsensusCertificateReason> =
            BTreeMap::new();
    ...
    async fn process_consensus_transaction<C: CheckpointServiceNotify>(
        ...
        match &transaction {
            ...
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                Ok(self.process_consensus_system_transaction(&ActionData::new(ActionKind::TestMessage(self.committee.authority_index(&authority).unwrap(), *num))))
            }
    ...
    pub fn process_pending_checkpoint(
        ...
        content_info: Vec<(CheckpointMessage, CheckpointContents)>,
    ) -> IkaResult<()> {
        let tables = self.tables()?;
        ...
        for (position_in_commit, (summary, transactions)) in content_info.into_iter().enumerate() {
            ...
            batch.insert_batch(
                ...
                transactions
                    .iter()
                    .map(|digest| (digest, sequence_number)),
            )?;

            batch.delete_batch(
                &tables.user_signatures_for_checkpoints,
                transactions.iter().map(|digest| digest),
            )?;
        }
    ...
    pub fn put_genesis_checkpoint_in_builder(
        ...
        let builder_summary = BuilderCheckpointMessage {
            checkpoint_message: summary.clone(),
    ...
    pub fn builder_included_transactions_in_checkpoint<'a>(
        &self,
        digests: impl Iterator<Item = &'a ActionDigest>,
    ) -> IkaResult<Vec<bool>> {
        Ok(self
            .tables()?
            .builder_digest_to_checkpoint
...
pub type LockDetails = ActionDigest;
