...
//! IndexStore supports creation of various ancillary indexes of state in IkaDataStore.
...
use ika_json_rpc_types::{IkaObjectDataFilter, TransactionFilter};
use ika_storage::mutex_table::MutexTable;
use ika_storage::sharded_lru::ShardedLruCache;
use ika_types::base_types::{
    ObjectDigest, ObjectID, SequenceNumber, IkaAddress, TransactionDigest, TxSequenceNumber,
};
use ika_types::base_types::{ObjectInfo, ObjectRef};
use ika_types::digests::TransactionEventsDigest;
use ika_types::dynamic_field::{self, DynamicFieldInfo};
use ika_types::effects::TransactionEvents;
use ika_types::error::{IkaError, IkaResult, UserInputError};
use ika_types::inner_temporary_store::TxCoins;
...
type OwnerIndexKey = (IkaAddress, ObjectID);
type CoinIndexKey = (IkaAddress, String, ObjectID);
...
impl CoinIndexKey2 {
    ...
    pub fn new(owner: IkaAddress, coin_type: String, balance: u64, object_id: ObjectID) -> Self {
        Self {
            owner,
            coin_type,
...
pub struct IndexStoreCaches {
    per_coin_type_balance: ShardedLruCache<(IkaAddress, TypeTag), IkaResult<TotalBalance>>,
    all_balances: ShardedLruCache<IkaAddress, IkaResult<Arc<HashMap<TypeTag, TotalBalance>>>>,
    locks: MutexTable<IkaAddress>,
}

#[derive(Default)]
pub struct IndexStoreCacheUpdates {
    _locks: Vec<OwnedMutexGuard<()>>,
    per_coin_type_balance_changes: Vec<((IkaAddress, TypeTag), IkaResult<TotalBalance>)>,
    all_balance_changes: Vec<(IkaAddress, IkaResult<Arc<AllBalance>>)>,
}
...
pub struct IndexStoreTables {
    ...
    /// Index from ika address to transactions initiated by that address.
    #[default_options_override_fn = "transactions_from_addr_table_default_config"]
    transactions_from_addr: DBMap<(IkaAddress, TxSequenceNumber), TransactionDigest>,

    /// Index from ika address to transactions that were sent to that address.
    #[default_options_override_fn = "transactions_to_addr_table_default_config"]
    transactions_to_addr: DBMap<(IkaAddress, TxSequenceNumber), TransactionDigest>,
    ...
    /// composite key of the IkaAddress of their owner and the object ID of the object.
    ...
    #[default_options_override_fn = "index_table_default_config"]
    event_by_sender: DBMap<(IkaAddress, EventId), EventIndex>,
    #[default_options_override_fn = "index_table_default_config"]
...
impl IndexStore {
    ...
    pub async fn index_coin(
    ...
    ) -> IkaResult<IndexStoreCacheUpdates> {
        ...
        let mut addresses: HashSet<IkaAddress> = HashSet::new();
        addresses.extend(
        ...
        let mut balance_changes: HashMap<IkaAddress, HashMap<TypeTag, TotalBalance>> =
            HashMap::new();
        // Index coin info
            ...
            .flat_map(|(address, balance_map)| {
                balance_map.iter().map(|(type_tag, balance)| {
                    (
                        ...
                        Ok::<TotalBalance, IkaError>(*balance),
                    )
                })
            })
            .collect();
        let all_balance_changes: Vec<_> = balance_changes
            ...
            .map(|(address, balance_map)| {
                (
                    address,
                    Ok::<Arc<HashMap<TypeTag, TotalBalance>>, IkaError>(Arc::new(balance_map)),
                )
            })
            .collect();
    ...
    pub fn get_transactions(
        ...
        let cursor = if let Some(cursor) = cursor {
            Some(
                self.get_transaction_seq(&cursor)?
                    .ok_or(IkaError::TransactionNotFound { digest: cursor })?,
            )
        } else {
            None
        };
        match filter {
            ...
            Some(_) => Err(IkaError::UserInputError {
    ...
    pub fn get_transaction_seq(
        &self,
        digest: &TransactionDigest,
    ) -> IkaResult<Option<TxSequenceNumber>> {
    ...
    pub fn all_events(
        ...
        limit: usize,
        descending: bool,
    ) -> IkaResult<Vec<(TransactionEventsDigest, TransactionDigest, usize, u64)>> {
        Ok(if descending {
            self.tables
                .event_order
                .unbounded_iter()
    ...
    pub fn events_by_transaction(
        ...
        let seq = self
            ...
            .ok_or(IkaError::TransactionNotFound { digest: *digest })?;
        Ok(if descending {
            self.tables
                .event_order
                .unbounded_iter()
    ...
    pub fn insert_genesis_objects(&self, object_index_changes: ObjectIndexChanges) -> IkaResult {
    ...
    pub async fn get_balance(
        ...
        if force_disable_cache {
            ...
            .unwrap()
            .map_err(|e| {
                IkaError::ExecutionError(format!("Failed to read balance frm DB: {:?}", e))
            });
        }
    ...
    pub async fn get_all_balance(
        &self,
        owner: IkaAddress,
    ) -> IkaResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        let force_disable_cache = read_size_from_env(ENV_VAR_DISABLE_INDEX_CACHE).unwrap_or(0) > 0;
        let metrics_cloned = self.metrics.clone();
        let coin_index_cloned = self.tables.coin_index_2.clone();
        if force_disable_cache {
            ...
            .unwrap()
            .map_err(|e| {
                IkaError::ExecutionError(format!("Failed to read all balance from DB: {:?}", e))
            });
        }
    ...
    /// Read balance for a `IkaAddress` and `CoinType` from the backend database
    #[instrument(skip_all)]
    ...
    /// Read all balances for a `IkaAddress` from the backend database
    #[instrument(skip_all)]
    pub fn get_all_balances_from_db(
        ...
        for (coin_type, coins) in &coins {
            ...
            let coin_type =
                TypeTag::Struct(Box::new(parse_ika_struct_tag(&coin_type).map_err(|e| {
                    IkaError::ExecutionError(format!(
    ...
    async fn invalidate_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = (IkaAddress, TypeTag)>,
    ) -> IkaResult {
        self.caches
            .per_coin_type_balance
    ...
    async fn update_per_coin_type_cache(
        &self,
        keys: impl IntoIterator<Item = ((IkaAddress, TypeTag), IkaResult<TotalBalance>)>,
    ) -> IkaResult {
        self.caches
            .per_coin_type_balance
    ...
    fn merge_balance(
        old_balance: &IkaResult<TotalBalance>,
        balance_delta: &IkaResult<TotalBalance>,
    ) -> IkaResult<TotalBalance> {
    ...
    async fn update_all_balance_cache(
        &self,
        keys: impl IntoIterator<Item = (IkaAddress, IkaResult<Arc<HashMap<TypeTag, TotalBalance>>>)>,
    ) -> IkaResult {
        self.caches
            .all_balances
    ...
    fn merge_all_balance(
        old_balance: &IkaResult<Arc<HashMap<TypeTag, TotalBalance>>>,
        balance_delta: &IkaResult<Arc<HashMap<TypeTag, TotalBalance>>>,
    ) -> IkaResult<Arc<HashMap<TypeTag, TotalBalance>>> {
        if let Ok(old_balance) = old_balance {
            if let Ok(balance_delta) = balance_delta {
...
mod tests {
    ...
    use ika_types::base_types::{ObjectInfo, ObjectType, IkaAddress};
    ...
    async fn test_index_cache() -> anyhow::Result<()> {
        ...
        let address: IkaAddress = AccountAddress::random().into();
