...
use ika_types::traffic_control::{ClientIdSource, PolicyConfig, RemoteFirewallConfig, Weight};
use ika_types::{effects::TransactionEffectsAPI, messages_grpc::HandleTransactionRequestV2};
use ika_types::{error::*, transaction::*};
use ika_types::{
    fp_ensure,
    messages_checkpoint::{
...
use ika_config::local_ip_utils::new_local_tcp_address_for_testing;
...
impl ValidatorService {
    ...
    // `handle_transaction_v2()` and `IkaTxValidator::vote_transaction()` as well.
    async fn handle_transaction(
        ...
        let info = state
            ...
            .await
            .tap_err(|e| {
                if let IkaError::ValidatorHaltedAtEpochEnd = e {
    ...
    async fn handle_certificates(
        ...
        fp_ensure!(
            ...
            IkaError::FullNodeCantHandleCertificate.into()
        );
    ...
    async fn handle_submit_to_consensus(
        ...
        let consensus_transactions: Vec<_> = consensus_transactions.into();
        {
            ...
            if !reconfiguration_lock.should_accept_user_certs() {
                ...
                return Err(IkaError::ValidatorHaltedAtEpochEnd.into());
            }
        ...
        let responses = futures::future::try_join_all(consensus_transactions.into_iter().map(
            |tx| async move {
                ...
                Ok::<_, IkaError>(HandleTransactionResponseV2 {
                    effects,
                    events,
                    input_objects,
                    output_objects,
...
impl ValidatorService {
    ...
    async fn soft_bundle_validity_check(
        ...
        // Enforce these checks per [SIP-19](https://github.com/ika-foundation/sips/blob/main/sips/sip-19.md):
    ...
    async fn handle_soft_bundle_certificates_v3_impl(
        ...
        let certificates = NonEmpty::from_vec(request.certificates)
            .ok_or_else(|| IkaError::NoCertificateProvidedError)?;
        for certificate in &certificates {
    ...
    ) -> WrappedServiceResponse<IkaSystemState> {
        let response = self
            .state
    ...
    async fn handle_traffic_req(&self, client: Option<IpAddr>) -> Result<(), tonic::Status> {
        if let Some(traffic_controller) = &self.traffic_controller {
            if !traffic_controller.check(&client, &None).await {
                // Entity in blocklist
                Err(tonic::Status::from_error(IkaError::TooManyRequests.into()))
            } else {
                Ok(())
            }
        } else {
    ...
    fn handle_traffic_resp<T>(
        ...
        let (error, spam_weight, unwrapped_response) = match wrapped_response {
            ...
            Err(status) => (
                Some(IkaError::from(status.clone())),
                Weight::zero(),
                Err(status.clone()),
            ),
        };
...
fn normalize(err: IkaError) -> Weight {
    match err {
        IkaError::UserInputError {
        ...
        | IkaError::IncorrectSigner { .. }
        | IkaError::UnknownSigner { .. }
        | IkaError::WrongEpoch { .. } => Weight::one(),
        _ => Weight::zero(),
    }
}
...
impl Validator for ValidatorService {
    ...
    ) -> Result<tonic::Response<IkaSystemState>, tonic::Status> {
