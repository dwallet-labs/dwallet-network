...
impl AuthorityAPI for LocalAuthorityClient {
    async fn handle_transaction(
    ...
    ) -> Result<HandleTransactionResponse, IkaError> {
        if self.fault_config.fail_before_handle_transaction {
            return Err(IkaError::from("Mock error before handle_transaction"));
        }
        ...
        if self.fault_config.fail_after_handle_transaction {
            return Err(IkaError::GenericAuthorityError {
        ...
        if let Some(duration) = self.fault_config.overload_retry_after_handle_transaction {
            return Err(IkaError::ValidatorOverloadedRetryAfter {
    ...
    async fn handle_certificate_v2(
        ...
        _client_addr: Option<SocketAddr>,
    ) -> Result<HandleCertificateResponseV2, IkaError> {
        let state = self.state.clone();
    ...
    async fn handle_certificate_v3(
        ...
        _client_addr: Option<SocketAddr>,
    ) -> Result<HandleCertificateResponseV3, IkaError> {
        let state = self.state.clone();
        let fault_config = self.fault_config;
        spawn_monitored_task!(Self::handle_certificate(state, request, fault_config))
            .await
    ...
    async fn handle_soft_bundle_certificates_v3(
        ...
        _client_addr: Option<SocketAddr>,
    ) -> Result<HandleSoftBundleCertificatesResponseV3, IkaError> {
        unimplemented!()
    }
    ...
    ) -> Result<ObjectInfoResponse, IkaError> {
        let state = self.state.clone();
    ...
    ) -> Result<TransactionInfoResponse, IkaError> {
        let state = self.state.clone();
    ...
    ) -> Result<CheckpointResponse, IkaError> {
        let state = self.state.clone();
    ...
    ) -> Result<CheckpointResponseV2, IkaError> {
        let state = self.state.clone();
    ...
    ) -> Result<IkaSystemState, IkaError> {
        self.state.get_ika_system_state_object_for_testing()
    }
}
...
pub struct MockAuthorityApi {
    delay: Duration,
    count: Arc<Mutex<u32>>,
    handle_object_info_request_result: Option<IkaResult<ObjectInfoResponse>>,
}
...
impl MockAuthorityApi {
    ...
    pub fn set_handle_object_info_request(&mut self, result: IkaResult<ObjectInfoResponse>) {
...
impl AuthorityAPI for MockAuthorityApi {
    /// Initiate a new transaction to a Ika or Primary account.
    async fn handle_transaction(
        &self,
    ...
    async fn handle_transaction_info_request(
        ...
        Err(IkaError::TransactionNotFound {
...
pub struct HandleTransactionTestAuthorityClient {
    pub tx_info_resp_to_return: IkaResult<HandleTransactionResponse>,
    pub cert_resp_to_return: IkaResult<HandleCertificateResponseV2>,
...
impl HandleTransactionTestAuthorityClient {
    pub fn new() -> Self {
        Self {
            tx_info_resp_to_return: Err(IkaError::Unknown("".to_string())),
            cert_resp_to_return: Err(IkaError::Unknown("".to_string())),
    ...
    pub fn set_tx_info_response_error(&mut self, error: IkaError) {
        self.tx_info_resp_to_return = Err(error);
    }

    pub fn reset_tx_info_response(&mut self) {
        self.tx_info_resp_to_return = Err(IkaError::Unknown("".to_string()));
    }
    ...
    pub fn set_cert_resp_to_return_error(&mut self, error: IkaError) {
        self.cert_resp_to_return = Err(error);
    }

    pub fn reset_cert_response(&mut self) {
        self.cert_resp_to_return = Err(IkaError::Unknown("".to_string()));
    }
