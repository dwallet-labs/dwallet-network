...
use std::ops::Deref;
use ika_types::crypto::{PublicKey, IkaSignature, ToFromBytes, ZkLoginPublicIdentifier};
use ika_types::messages_grpc::HandleSoftBundleCertificatesRequestV3;
use ika_types::utils::get_one_zklogin_inputs;
use ika_types::{
    authenticator_state::ActiveJwk,
    base_types::dbg_addr,
    crypto::{get_key_pair, AccountKeyPair, Signature, IkaKeyPair},
    error::{IkaError, UserInputError},
...
async fn test_handle_transfer_transaction_bad_signature() {
    do_transaction_test(
        ...
        |err| {
            assert_matches!(err, IkaError::SignerSignatureAbsent { .. });
        },
    )
    .await;
}
...
async fn test_handle_transfer_transaction_no_signature() {
    do_transaction_test(
        ...
        |err| {
            assert_matches!(
                err,
                IkaError::SignerSignatureNumberMismatch {
                    expected: 1,
                    actual: 0
                }
            );
...
async fn test_zklogin_transfer_with_large_address_seed() {
    ...
    let sender = IkaAddress::generate(StdRng::from_seed([3; 32]));
    let recipient = dbg_addr(2);
...
async fn zklogin_test_caching_scenarios() {
    ...
    let (skp, _eph_pk, zklogin) =
        &load_test_vectors("../ika-types/src/unit_tests/zklogin_test_vectors.json")[1];
    ...
    let zklogin_json_string =
        &get_one_zklogin_inputs("../ika-types/src/unit_tests/zklogin_test_vectors.json");
    let bad_zklogin_inputs = ZkLoginInputs::from_json(zklogin_json_string, "111").unwrap();
    let sender = IkaAddress::try_from_unpadded(&bad_zklogin_inputs).unwrap();
...
async fn setup_zklogin_network(
    ...
    ika_types::message_envelope::Envelope<SenderSignedData, ika_types::crypto::EmptySignInfo>,
    ...
    let sender = IkaAddress::try_from_unpadded(zklogin).unwrap();
    ...
    let sender_2 = IkaAddress::from(&multisig_pk);
...
async fn zk_multisig_test() {
    ...
    .unwrap();
    let victim_addr = IkaAddress::from(&multisig_pk);
    ...
    for (kp, _pk_zklogin, inputs) in test_vectors {
        let intent_message = IntentMessage::new(Intent::ika_transaction(), data.clone());
...
async fn test_very_large_certificate() {
    ...
    let quorum_signature = ika_types::crypto::AuthorityQuorumSignInfo {
        epoch: 0,
        signature: ika_types::crypto::AggregateAuthoritySignature::aggregate(&sigs)
    ...
    let cert = ika_types::message_envelope::Envelope::new_from_data_and_sig(
...
async fn test_handle_soft_bundle_certificates_errors() {
    ...
    // Case 0: submit an empty soft bundle.
    {
        ...
        assert_matches!(
            response.unwrap_err(),
            IkaError::NoCertificateProvidedError { .. }
        );
    }
...
fn sender_signed_data_serialized_intent() {
    ...
    assert_eq!(txn.intent_message().intent, Intent::ika_transaction());
