...
use ika_types::error::{ExecutionError, IkaError};
use ika_types::execution_status::ExecutionFailureStatus::{
    InputObjectDeleted, SharedObjectOperationNotAllowed,
};
use ika_types::transaction::{ObjectArg, VerifiedCertificate};
...
async fn test_object_lock_conflict() {
    ...
    assert!(matches!(
        mutate_cert_res.err(),
        Some(IkaError::ObjectLockConflict { .. })
    ));
}
