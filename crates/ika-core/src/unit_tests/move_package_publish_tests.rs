...
use ika_move_build::{check_unpublished_dependencies, gather_published_ids, BuildConfig};
use ika_types::{
...
async fn test_generate_lock_file() {
    ...
    let expected = expect![[r##"
        ...
        [[move.package]]
        id = "MoveStdlib"
        source = { local = "../../../../../ika-framework/packages/move-stdlib" }

        [[move.package]]
        id = "Ika"
        source = { local = "../../../../../ika-framework/packages/ika-framework" }
...
async fn test_custom_property_check_unpublished_dependencies() {
    ...
    let IkaError::ModulePublishFailure { error } = check_unpublished_dependencies(
