...
use std::sync::Arc;
use ika_config::certificate_deny_config::CertificateDenyConfigBuilder;
use ika_config::transaction_deny_config::{TransactionDenyConfig, TransactionDenyConfigBuilder};
...
use ika_types::messages_grpc::HandleTransactionResponse;
...
use ika_types::utils::get_zklogin_user_address;
use ika_types::utils::{
...
async fn setup_test(deny_config: TransactionDenyConfig) -> (NetworkConfig, Arc<AuthorityState>) {
    let network_config =
        ika_swarm_config::network_config_builder::ConfigBuilder::new_with_temp_dir()
            .with_accounts(vec![
                AccountConfig {
                    address: None,
...
type Account = (IkaAddress, Ed25519KeyPair, Vec<ObjectRef>);
...
fn get_accounts_and_coins(
    ...
    let accounts: Vec<_> = network_config
        ...
        .iter()
        .map(|account| {
            let address: IkaAddress = account.public().into();
...
async fn transfer_with_account(
    ...
    let data = TransactionData::new_transfer_ika_allow_sponsor(
        sender_account.0,
        sender_account.0,
        None,
        sponsor_account.2[0],
...
fn assert_denied<T: std::fmt::Debug>(result: &IkaResult<T>) {
    assert!(matches!(
...
async fn test_shared_object_transaction_disabled() {
    ...
    let tx = TestTransactionBuilder::new(account.0, account.2[0], gas_price)
        .call_staking(account.2[1], IkaAddress::default())
