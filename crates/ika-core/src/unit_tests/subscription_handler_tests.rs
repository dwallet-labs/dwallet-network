...
use ika_types::{MOVE_STDLIB_ADDRESS, IKA_FRAMEWORK_ADDRESS};
...
fn test_to_json_value() {
    ...
    let ika_move_struct: IkaMoveStruct =
        BoundedVisitor::deserialize_struct(&event_bytes, &TestEvent::layout())
            .unwrap()
            .into();
    let json_value = ika_move_struct.to_json_value();
    assert_eq!(
        Some(&json!("1000000")),
...
// TODO: Do we need this in the ika-types lib?
