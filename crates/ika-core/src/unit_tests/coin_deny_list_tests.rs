...
use ika_types::base_types::{dbg_addr, ObjectID, ObjectRef, IkaAddress};
use ika_types::crypto::{get_account_key_pair, AccountKeyPair};
use ika_types::deny_list_v1::{CoinDenyCap, RegulatedCoinMetadata};
use ika_types::deny_list_v2::{
...
use ika_types::transaction::{CallArg, ObjectArg, TEST_ONLY_GAS_UNIT_FOR_PUBLISH};
use ika_types::{IKA_DENY_LIST_OBJECT_ID, IKA_FRAMEWORK_PACKAGE_ID};
...
async fn test_regulated_coin_v2_types() {
    ...
    let deny_list_object_init_version = env.get_latest_object_ref(&IKA_DENY_LIST_OBJECT_ID).await.1;
