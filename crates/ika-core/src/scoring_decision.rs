...
mod tests {
    ...
    pub fn test_update_low_scoring_authorities() {
        ...
        let (ika_committee, consensus_committee) = generate_committees(8);
        ...
        assert_eq!(
            *low_scoring
                ...
                .get(ika_committee.authority_by_index(2).unwrap())
                .unwrap(),
            50
        );
        assert_eq!(
            *low_scoring
                ...
                .get(ika_committee.authority_by_index(4).unwrap())
                .unwrap(),
            0
        );
    ...
    /// Generate a pair of Ika and consensus committees for the given size.
    fn generate_committees(committee_size: usize) -> (Committee, ConsensusCommittee) {
        ...
        let public_keys = consensus_committee
            ...
            .collect::<Vec<_>>();
        let ika_authorities = public_keys
            .iter()
            .map(|key| (AuthorityPublicKeyBytes::from(*key), 1))
            .collect::<Vec<_>>();
        let ika_committee = Committee::new_for_testing_with_normalized_voting_power(
            0,
            ika_authorities.iter().cloned().collect(),
        );

        (ika_committee, consensus_committee)
    }
}
