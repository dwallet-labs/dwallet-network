            ...
            match tx {
                ...
                ConsensusTransactionKind::RandomnessDkgMessage(_, bytes) => {
                    if bytes.len() > dkg_v1::DKG_MESSAGES_MAX_SIZE {
                        ...
                        return Err(IkaError::InvalidDkgMessageSize);
                    }
                }
                ...
                ConsensusTransactionKind::UserTransaction(_tx) => {
                    if !epoch_store.protocol_config().mysticeti_fastpath() {
                        return Err(IkaError::UnexpectedMessage(
...
mod tests {
    ...
    use ika_types::{
        crypto::Ed25519IkaSignature, messages_consensus::ConsensusTransaction, object::Object,
    ...
    use crate::{
        ...
        consensus_validator::{IkaTxValidator, IkaTxValidatorMetrics},
    };
    ...
    async fn accept_valid_transaction() {
        ...
        let metrics = IkaTxValidatorMetrics::new(&Default::default());
        let validator = IkaTxValidator::new(
            state.clone(),
        ...
        let bogus_transaction_bytes: Vec<_> = certificates
            ...
            .map(|mut cert| {
                ...
                cert.tx_signatures_mut_for_testing()[0] =
                    GenericSignature::Signature(ika_types::crypto::Signature::Ed25519IkaSignature(
                        Ed25519IkaSignature::default(),
                    ));
                bcs::to_bytes(&ConsensusTransaction::new_certificate_message(&name1, cert)).unwrap()
            })
            .collect();
