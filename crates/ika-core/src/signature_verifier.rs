...
use std::sync::Arc;
use ika_types::digests::SenderSignedDataDigest;
use ika_types::digests::ZKLoginInputsDigest;
...
type Sender = oneshot::Sender<IkaResult<VerifiedCertificate>>;
...
impl SignatureVerifier {
    ...
    pub async fn verify_cert(&self, cert: CertifiedTransaction) -> IkaResult<VerifiedCertificate> {
    ...
    pub async fn multi_verify_certs(
        &self,
        certs: Vec<CertifiedTransaction>,
    ) -> Vec<IkaResult<VerifiedCertificate>> {
    ...
    pub fn verify_tx(&self, signed_tx: &SenderSignedData) -> IkaResult {
...
fn batch_verify(
    ...
    for cert in certs {
        let idx = obligation.add_message(cert.data(), cert.epoch(), Intent::ika_app(cert.scope()));
        cert.auth_sig()
            .add_to_verification_obligation(committee, &mut obligation, idx)?;
    }

    for ckpt in checkpoints {
        let idx = obligation.add_message(ckpt.data(), ckpt.epoch(), Intent::ika_app(ckpt.scope()));
        ckpt.auth_sig()
            .add_to_verification_obligation(committee, &mut obligation, idx)?;
    }
