...
use crate::{consumer::ProtobufDecoder, peers::PeraNodeProvider};
use axum::{
    async_trait,
    body::Body,
    body::Bytes,
...
/// we expect pera-node to send us an http header content-length encoding.
...
/// we expect pera-node to send us an http header content-type encoding.
...
/// we expect that calling pera-nodes are known on the blockchain and we enforce
/// their pub key tls creds here
pub async fn expect_valid_public_key(
    Extension(allower): Extension<Arc<PeraNodeProvider>>,
