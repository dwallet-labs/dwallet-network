...
mod tests {
    ...
    use crate::{admin::CertKeyPair, config::RemoteWriteConfig, peers::PeraNodeProvider};
    ...
    use pera_tls::{ClientCertVerifier, TlsAcceptor};
    ...
    /// pera-node client that posts data to the proxy using the protobuf format.  The server processes this
    ...
    async fn axum_acceptor() {
        ...
        let CertKeyPair(client_priv_cert, client_pub_key) = admin::generate_self_cert("pera".into());
        ...
        let mut allower = PeraNodeProvider::new("".into(), Duration::from_secs(30), vec![]);
        let tls_config = ClientCertVerifier::new(
            allower.clone(),
            pera_tls::PERA_VALIDATOR_SERVER_NAME.to_string(),
        )
