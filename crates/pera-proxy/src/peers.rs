...
/// PeraNods a mapping of public key to PeraPeer data
pub type PeraPeers = Arc<RwLock<HashMap<Ed25519PublicKey, PeraPeer>>>;

/// A PeraPeer is the collated pera chain data we have about validators
...
/// PeraNodeProvider queries the pera blockchain and keeps a record of known validators based on the response from
/// pera_getValidators.  The node name, public key and other info is extracted from the chain and stored in this
...
#[derive(Debug, Clone)]
pub struct PeraNodeProvider {
    nodes: PeraPeers,
    static_nodes: PeraPeers,
    rpc_url: String,
    rpc_poll_interval: Duration,
}

impl Allower for PeraNodeProvider {
...
impl PeraNodeProvider {
    pub fn new(rpc_url: String, rpc_poll_interval: Duration, static_peers: Vec<PeraPeer>) -> Self {
        // build our hashmap with the static pub keys. we only do this one time at binary startup.
        let static_nodes: HashMap<Ed25519PublicKey, PeraPeer> = static_peers
            .into_iter()
    ...
    pub fn get_ref(&self) -> &PeraPeers {
        &self.nodes
    }

    /// Get a mutable reference to the inner service
    pub fn get_mut(&mut self) -> &mut PeraPeers {
        &mut self.nodes
    }

    /// get_validators will retrieve known validators
    async fn get_validators(url: String) -> Result<PeraSystemStateSummary> {
        let rpc_method = "perax_getLatestPeraSystemState";
        let observe = || {
...
/// extract will get the network pubkey bytes from a PeraValidatorSummary type.  This type comes from a
...
fn extract(summary: PeraSystemStateSummary) -> impl Iterator<Item = (Ed25519PublicKey, PeraPeer)> {
    summary.active_validators.into_iter().filter_map(|vm| {
        match Ed25519PublicKey::from_bytes(&vm.network_pubkey_bytes) {
            ...
            Err(error) => {
                error!(
                    "unable to decode public key for name: {:?} pera_address: {:?} error: {error}",
                    vm.name, vm.pera_address
                );
...
mod tests {
    ...
    #[test]
    fn depend_on_pera_pera_system_state_summary() {
        let CertKeyPair(_, client_pub_key) = generate_self_cert("pera".into());
        ...
        // some of those.
        let depends_on = PeraSystemStateSummary {
            active_validators: vec![PeraValidatorSummary {
        ...
        let r = serde_json::to_string(&ResponseBody { result: depends_on })
            .expect("expected to serialize ResponseBody{PeraSystemStateSummary}");

        let deserialized = serde_json::from_str::<ResponseBody>(&r)
            .expect("expected to deserialize ResponseBody{PeraSystemStateSummary}");
