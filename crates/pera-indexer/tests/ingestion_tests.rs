...
mod ingestion_tests {
    ...
    use pera_indexer::models::{objects::StoredObject, transactions::StoredTransaction};
    use pera_indexer::schema::{objects, transactions};
    use pera_indexer::store::{indexer_store::IndexerStore, PgIndexerStore};
    ...
    const DEFAULT_DB_URL: &str = "postgres://postgres:postgrespw@localhost:5432/pera_indexer";
    ...
    async fn set_up(
        ...
        let server_handle = tokio::spawn(async move {
            pera_rest_api::RestService::new_without_version(sim)
    ...
    pub async fn test_transaction_table() -> Result<(), IndexerError> {
        ...
        let transfer_recipient = PeraAddress::random_for_testing_only();
    ...
    pub async fn test_object_type() -> Result<(), IndexerError> {
        ...
        assert_eq!(
            ...
            Some(PERA_FRAMEWORK_PACKAGE_ID.to_vec())
        );
