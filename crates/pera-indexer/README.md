Pera indexer is an off-fullnode service to serve data from Pera protocol, including both data directly generated from chain and derivative data.

&#9888; **Warning:** Pera indexer is still experimental and we expect occasional breaking changes that require backfills.
...
See the [docs](https://docs.pera.io/guides/developer/getting-started/local-network) for detailed information. Below is a quick start guide:

Start a local network using the `pera` binary:
```sh
cargo run --bin pera -- start --with-faucet --force-regenesis 
```
...
```sh
cargo run --bin pera --features indexer -- start --with-faucet --force-regenesis --with-indexer --pg-port 5432 --pg-db-name pera_indexer_v2
```

### Running standalone indexer
1. DB setup, under `pera/crates/pera-indexer` run:
```sh
...
```
3. Start indexer binary, under `pera/crates/pera-indexer` run:
- run indexer as a writer, which pulls data from fullnode and writes data to DB
```sh
# Change the RPC_CLIENT_URL to http://0.0.0.0:9000 to run indexer against local validator & fullnode
cargo run --bin pera-indexer -- --db-url "<DATABASE_URL>" --rpc-client-url "https://fullnode.devnet.pera.io:443" --fullnode-sync-worker --reset-db
```
- run indexer as a reader, which is a JSON RPC server with the [interface](https://docs.pera.io/pera-api-ref#perax_getallbalances)
```
cargo run --bin pera-indexer -- --db-url "<DATABASE_URL>" --rpc-client-url "https://fullnode.devnet.pera.io:443" --rpc-server-worker
```
More flags info can be found in this [file](https://github.com/MystenLabs/sui/blob/main/crates/pera-indexer/src/lib.rs#L83-L123).
### DB reset
Run this command under `pera/crates/pera-indexer`, which will wipe DB; In case of schema changes in `.sql` files, this will also update corresponding `schema.rs` file.
```sh
diesel database reset --database-url="<DATABASE_URL>"
```
...
3.DB setup, under `pera/crates/pera-indexer` run:
...
```sh
# Change the RPC_CLIENT_URL to http://0.0.0.0:9000 to run indexer against local validator & fullnode
cargo run --bin pera-indexer --features mysql-feature --no-default-features -- --db-url "<DATABASE_URL>" --rpc-client-url "https://fullnode.devnet.pera.io:443" --fullnode-sync-worker --reset-db
```
