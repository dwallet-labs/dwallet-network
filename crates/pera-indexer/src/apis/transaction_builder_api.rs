...
use pera_json_rpc::transaction_builder_api::TransactionBuilderApi as PeraTransactionBuilderApi;
use pera_json_rpc_types::{PeraObjectDataFilter, PeraObjectDataOptions, PeraObjectResponse};
use pera_transaction_builder::DataReader;
...
impl<T: R2D2Connection> TransactionBuilderApi<T> {
    ...
    pub fn new(inner: IndexerReader<T>) -> PeraTransactionBuilderApi {
        PeraTransactionBuilderApi::new_with_data_reader(std::sync::Arc::new(Self { inner }))
    }
}
