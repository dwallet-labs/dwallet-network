...
use pera_types::{
    base_types::{MoveObjectType, ObjectID, PeraAddress},
    committee::EpochId,
    governance::StakedPera,
    pera_serde::BigInt,
    pera_system_state::{pera_system_state_summary::PeraSystemStateSummary, PoolTokenExchangeRate},
};
...
impl<T: R2D2Connection + 'static> GovernanceReadApi<T> {
    ...
    async fn get_latest_pera_system_state(&self) -> Result<PeraSystemStateSummary, IndexerError> {
        self.inner
            .spawn_blocking(|this| this.get_latest_pera_system_state())
            .await
    }
    ...
    async fn get_stakes_by_ids(
        ...
        for stored_object in self.inner.multi_get_objects_in_blocking_task(ids).await? {
            let object = pera_types::object::Object::try_from(stored_object)?;
            let stake_object = StakedPera::try_from(&object)?;
    ...
    async fn get_staked_by_owner(
        ...
        for stored_object in self
            ...
            .get_owned_objects_in_blocking_task(
                owner,
                Some(PeraObjectDataFilter::StructType(
                    MoveObjectType::staked_pera().into(),
                )),
                None,
    ...
    pub async fn get_delegated_stakes(
        ...
        let system_state_summary = self.get_latest_pera_system_state().await?;
        ...
        for (pool_id, stakes) in pools {
            ...
            for stake in stakes {
                ...
                delegations.push(pera_json_rpc_types::Stake {
...
pub async fn exchange_rates(
    ...
    for df in state
        ...
        .await?
    {
        let pool_id: pera_types::id::ID = bcs::from_bytes(&df.bcs_name).map_err(|e| {
        ...
        let validator = state
            ...
            .spawn_blocking(move |this| {
                pera_types::pera_system_state::get_validator_from_table(
                    &this,
                    inactive_pools_id,
                    &pool_id,
                )
...
impl<T: R2D2Connection + 'static> GovernanceReadApiServer for GovernanceReadApi<T> {
    async fn get_stakes_by_ids(
        ...
        self.get_stakes_by_ids(staked_pera_ids)
            .await
            .map_err(Into::into)
    }
...
impl<T: R2D2Connection> PeraRpcModule for GovernanceReadApi<T> {
    fn rpc(self) -> RpcModule<Self> {
        self.into_rpc()
    }

    fn rpc_doc_module() -> Module {
        pera_json_rpc_api::GovernanceReadApiOpenRpc::module_doc()
    }
}
