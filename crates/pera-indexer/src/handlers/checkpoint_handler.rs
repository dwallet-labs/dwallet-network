...
use pera_json_rpc_types::PeraMoveValue;
use pera_package_resolver::{PackageStore, PackageStoreWithLruCache, Resolver};
use pera_rest_api::{CheckpointData, CheckpointTransaction};
...
use pera_types::pera_system_state::{get_pera_system_state, PeraSystemStateTrait};
    ...
    T: R2D2Connection + 'static,
{
    ...
    async fn index_epoch(
        ...
        if *checkpoint_summary.sequence_number() == 0 {
            ...
            let system_state: PeraSystemStateSummary =
                get_pera_system_state(&checkpoint_object_store)?.into_pera_system_state_summary();
    ...
    async fn index_checkpoint(
        ...
        let (checkpoint, db_transactions, db_events, db_tx_indices, db_event_indices, db_displays) = {
            ...
            (
                IndexedCheckpoint::from_pera_checkpoint(
                    &checkpoint_summary,
                    &checkpoint_contents,
        ...
        checkpoint_data
            ...
            .flat_map(|data| {
                ...
                data.transactions
                    ...
                    .flat_map(|tx| &tx.output_objects)
                    .filter_map(|o| {
                        if let pera_types::object::Data::Package(p) = &o.data {
                            Some(IndexedPackage {
                                package_id: o.id(),
