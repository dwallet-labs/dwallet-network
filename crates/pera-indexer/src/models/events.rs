...
use pera_json_rpc_types::{type_and_fields_from_move_event_data, PeraEvent};
...
impl StoredEvent {
    pub async fn try_into_pera_event(
        self,
        package_resolver: Arc<Resolver<impl PackageStore>>,
    ) -> Result<PeraEvent, IndexerError> {
        ...
        // Note: PeraEvent only has one sender today, so we always use the first one.
        let sender = {
            #[cfg(feature = "postgres-feature")]
            {
        ...
        let sender = match sender {
            Some(ref s) => PeraAddress::from_bytes(s).map_err(|_e| {
                IndexerError::PersistentStorageDataCorruptionError(format!(
        ...
        let type_ = parse_pera_struct_tag(&self.event_type)?;
