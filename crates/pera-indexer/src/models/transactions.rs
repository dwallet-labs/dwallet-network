...
use pera_json_rpc_types::{
    BalanceChange, ObjectChange, PeraEvent, PeraTransactionBlock, PeraTransactionBlockEffects,
...
impl StoredTransaction {
    ...
    pub async fn try_into_pera_transaction_block_response(
        self,
        options: PeraTransactionBlockResponseOptions,
        package_resolver: Arc<Resolver<impl PackageStore>>,
    ) -> IndexerResult<PeraTransactionBlockResponse> {
        ...
        let transaction = if options.show_input {
            ...
            let tx_block = PeraTransactionBlock::try_from_with_package_resolver(
                sender_signed_data,
        ...
        let effects = if options.show_effects {
            let effects = self.try_into_pera_transaction_effects()?;
            Some(effects)
        } else {
            None
        };
        ...
        let events = if options.show_events {
            ...
            tx_events_to_pera_tx_events(tx_events, package_resolver, tx_digest, timestamp).await?
        } else {
            None
        };
        ...
        Ok(PeraTransactionBlockResponse {
            digest: tx_digest,
            transaction,
            raw_transaction,
            effects,
    ...
    pub fn try_into_pera_transaction_effects(&self) -> IndexerResult<PeraTransactionBlockEffects> {
        ...
        let effects = PeraTransactionBlockEffects::try_from(effects)?;
        Ok(effects)
    }
}
...
pub async fn tx_events_to_pera_tx_events(
    tx_events: TransactionEvents,
    package_resolver: Arc<Resolver<impl PackageStore>>,
    tx_digest: TransactionDigest,
    timestamp: u64,
) -> Result<Option<PeraTransactionBlockEvents>, IndexerError> {
    ...
    for tx_event in tx_events.data.clone() {
        ...
        pera_event_futures.push(tokio::task::spawn(async move {
    ...
    let event_move_type_layouts = futures::future::join_all(pera_event_futures)
        .await
        .into_iter()
        ...
        .map_err(|e| {
            IndexerError::ResolveMoveStructError(format!(
                "Failed to convert to pera event with Error: {e}",
            ))
        })?;
    ...
    let pera_events = tx_events
        ...
        .collect::<Result<Vec<_>, _>>()?;
    let pera_tx_events = PeraTransactionBlockEvents { data: pera_events };
    Ok(Some(pera_tx_events))
}
