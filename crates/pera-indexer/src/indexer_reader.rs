...
use pera_json_rpc_types::DisplayFieldsResponse;
use pera_json_rpc_types::{
    Balance, Coin as PeraCoin, PeraCoinMetadata, PeraTransactionBlockEffects,
    PeraTransactionBlockEffectsAPI,
};
use pera_json_rpc_types::{
    CheckpointId, EpochInfo, EventFilter, PeraEvent, PeraObjectDataFilter,
    PeraTransactionBlockResponse, TransactionFilter,
};
use pera_package_resolver::Package;
use pera_package_resolver::PackageStore;
...
use pera_types::{
    ...
    pera_system_state::{pera_system_state_summary::PeraSystemStateSummary, PeraSystemStateTrait},
};
use pera_types::{coin::CoinMetadata, event::EventID};
...
use crate::{
    ...
    models::{
        ...
        transactions::{tx_events_to_pera_tx_events, StoredTransaction},
...
impl<U: R2D2Connection> IndexerReader<U> {
    ...
    pub fn get_latest_pera_system_state(&self) -> Result<PeraSystemStateSummary, IndexerError> {
        let system_state: PeraSystemStateSummary =
            pera_types::pera_system_state::get_pera_system_state(self)?
    ...
    /// `get_latest_pera_system_state` for current epoch instead.
    pub fn get_epoch_pera_system_state(
        &self,
        epoch: Option<EpochId>,
    ) -> Result<PeraSystemStateSummary, IndexerError> {
        let stored_epoch = self.get_epoch_info_from_db(epoch)?;
        let stored_epoch = match stored_epoch {
            Some(stored_epoch) => stored_epoch,
        ...
        let system_state: PeraSystemStateSummary = bcs::from_bytes(&stored_epoch.system_state)
            .map_err(|_| {
    ...
    pub fn get_checkpoint(
        &self,
        checkpoint_id: CheckpointId,
    ) -> Result<Option<pera_json_rpc_types::Checkpoint>, IndexerError> {
        let stored_checkpoint = match self.get_checkpoint_from_db(checkpoint_id)? {
            Some(stored_checkpoint) => stored_checkpoint,
            None => return Ok(None),
        };

        let checkpoint = pera_json_rpc_types::Checkpoint::try_from(stored_checkpoint)?;
        Ok(Some(checkpoint))
    }

    pub fn get_latest_checkpoint(&self) -> Result<pera_json_rpc_types::Checkpoint, IndexerError> {
        let stored_checkpoint = self.get_latest_checkpoint_from_db()?;

        pera_json_rpc_types::Checkpoint::try_from(stored_checkpoint)
    }
    ...
    pub fn get_checkpoints(
        ...
        limit: usize,
        descending_order: bool,
    ) -> Result<Vec<pera_json_rpc_types::Checkpoint>, IndexerError> {
        self.get_checkpoints_from_db(cursor, limit, descending_order)?
            .into_iter()
            .map(pera_json_rpc_types::Checkpoint::try_from)
            .collect()
    }
    ...
    fn get_transaction_effects_with_digest(
    ...
    ) -> Result<PeraTransactionBlockEffects, IndexerError> {
        ...
        stored_txn.try_into_pera_transaction_effects()
    }
    ...
    async fn stored_transaction_to_transaction_block(
        ...
        options: pera_json_rpc_types::PeraTransactionBlockResponseOptions,
    ) -> IndexerResult<Vec<PeraTransactionBlockResponse>> {
        ...
        run_query!(&self.pool, |conn| {
            ...
            if let Some(filter) = filter {
                match filter {
                    ...
                    PeraObjectDataFilter::MatchNone(filters) => {
    ...
    async fn multi_get_transaction_block_response_in_blocking_task_impl(
        ...
        digests: &[TransactionDigest],
        options: pera_json_rpc_types::PeraTransactionBlockResponseOptions,
    ) -> Result<Vec<pera_json_rpc_types::PeraTransactionBlockResponse>, IndexerError> {
    ...
    pub async fn get_transaction_events_in_blocking_task(
        &self,
        digest: TransactionDigest,
    ) -> Result<Vec<pera_json_rpc_types::PeraEvent>, IndexerError> {
        let pool = self.get_pool();
        ...
        let pera_tx_events = tx_events_to_pera_tx_events(
            tx_events,
        ...
        .await?;
        Ok(pera_tx_events.map_or(vec![], |ste| ste.data))
    }
    ...
    pub async fn query_events_in_blocking_task(
        ...
        let mut pera_event_futures = vec![];
        for stored_event in stored_events {
            pera_event_futures.push(tokio::task::spawn(
                stored_event.try_into_pera_event(self.package_resolver.clone()),
            ));
        }

        let pera_events = futures::future::join_all(pera_event_futures)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| tracing::error!("Failed to join pera event futures: {}", e))?
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
            .tap_err(|e| tracing::error!("Failed to collect pera event futures: {}", e))?;
        Ok(pera_events)
    }
    ...
    pub async fn bcs_name_from_dynamic_field_name(
        ...
        let pera_json_value = pera_json::PeraJsonValue::new(name.value.clone())?;
        let name_bcs_value = pera_json_value.to_bcs_bytes(&move_type_layout)?;
        Ok(name_bcs_value)
    }
    ...
    ) -> Result<Option<pera_types::display::DisplayVersionUpdatedEvent>, IndexerError> {
    ...
    pub async fn get_owned_coins_in_blocking_task(
        ...
        cursor: ObjectID,
        limit: usize,
    ) -> Result<Vec<PeraCoin>, IndexerError> {
    ...
    pub(crate) async fn get_display_fields(
        &self,
        original_object: &pera_types::object::Object,
        ...
        let (object_type, layout) = if let Some((object_type, layout)) =
            pera_json_rpc::read_api::get_object_type_and_struct(original_object, original_layout)
        ...
        if let Some(display_object) = self.get_display_object_by_type(&object_type).await? {
            return pera_json_rpc::read_api::get_rendered_fields(display_object.fields, &layout)
                .map_err(|e| IndexerError::GenericError(e.to_string()));
        }
    ...
    ) -> Result<Option<PeraCoinMetadata>, IndexerError> {
    ...
    fn get_coin_metadata(
        ...
        if let Some(id) = coin_metadata_obj_id {
            ...
            Ok(metadata_object.and_then(|v| PeraCoinMetadata::try_from(v).ok()))
        } else {
            Ok(None)
        }
    }
...
impl<U: R2D2Connection> pera_types::storage::ObjectStore for IndexerReader<U> {
    fn get_object(
        &self,
        object_id: &ObjectID,
    ) -> Result<Option<pera_types::object::Object>, pera_types::storage::error::Error> {
