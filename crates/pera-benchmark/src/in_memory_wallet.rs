...
/// A Pera account and all of the objects it owns
#[derive(Debug)]
pub struct PeraAccount {
...
pub struct InMemoryWallet {
    accounts: BTreeMap<PeraAddress, PeraAccount>, // TODO: track shared and immutable objects as well
}
...
impl InMemoryWallet {
    ...
    pub fn add_account(
        ...
        self.accounts.insert(addr, PeraAccount::new(key, gas, objs));
    }
    ...
    pub fn account_mut(&mut self, addr: &PeraAddress) -> Option<&mut PeraAccount> {
        self.accounts.get_mut(addr)
    }

    pub fn account(&self, addr: &PeraAddress) -> Option<&PeraAccount> {
        self.accounts.get(addr)
    }

    pub fn gas(&self, addr: &PeraAddress) -> Option<&ObjectRef> {
        self.accounts.get(addr).map(|a| &a.gas)
    }

    pub fn owned_object(&self, addr: &PeraAddress, id: &ObjectID) -> Option<&ObjectRef> {
        self.accounts.get(addr).and_then(|a| a.owned.get(id))
    }

    pub fn owned_objects(&self, addr: &PeraAddress) -> Option<impl Iterator<Item = &ObjectRef>> {
    ...
    pub fn keypair(&self, addr: &PeraAddress) -> Option<Arc<AccountKeyPair>> {
    ...
    pub fn addresses(&self) -> impl Iterator<Item = &PeraAddress> {
        self.accounts.keys()
    }
