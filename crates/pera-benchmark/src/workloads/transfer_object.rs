...
use pera_core::test_utils::make_transfer_object_transaction;
use pera_types::{
...
/// Number of npera sent to each address on each batch transfer
const _TRANSFER_AMOUNT: u64 = 1;

#[derive(Debug)]
pub struct TransferObjectTestPayload {
    transfer_object: ObjectRef,
    transfer_from: PeraAddress,
    transfer_to: PeraAddress,
    gas: Vec<Gas>,
...
impl Workload<dyn Payload> for TransferObjectWorkload {
    ...
    async fn make_test_payloads(
        ...
        let mut gas_by_address: HashMap<PeraAddress, Vec<Gas>> = HashMap::new();
        ...
        let addresses: Vec<PeraAddress> = gas_by_address.keys().cloned().collect();
