...
use pera_types::crypto::{deterministic_random_account_key, AccountKeyPair};
...
impl Env {
    ...
    async fn setup_local_env(
        ...
        let (primary_gas_owner, keypair): (PeraAddress, AccountKeyPair) =
            deterministic_random_account_key();
        let keypair = Arc::new(keypair);

        // spawn a thread to spin up pera nodes on the multi-threaded server runtime.
        // running forever
            ...
            server_runtime.block_on(async move {
                let cluster = TestClusterBuilder::new()
                    .with_accounts(vec![AccountConfig {
                        ...
                        // We can't use TOTAL_SUPPLY_NPERA because we need to account for validator stakes in genesis allocation.
                        gas_amounts: vec![TOTAL_SUPPLY_NPERA / 2],
                    }])
    ...
    async fn setup_remote_env(
        ...
        let genesis = pera_config::node::Genesis::new_from_file(genesis_blob_path);
