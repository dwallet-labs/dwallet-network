...
use pera_types::{base_types::ObjectRef, crypto::AuthorityStrongQuorumSignInfo, object::Owner};
use pera_types::{base_types::SequenceNumber, gas_coin::GasCoin};
use pera_types::{
    base_types::{AuthorityName, PeraAddress},
...
use pera_types::messages_grpc::{HandleCertificateResponseV2, TransactionStatus};
use pera_types::quorum_driver_types::{QuorumDriverError, QuorumDriverResponse};
...
pub enum ExecutionEffects {
    ...
    PeraTransactionBlockEffects(PeraTransactionBlockEffects),
}
...
impl ExecutionEffects {
    pub fn mutated(&self) -> Vec<(ObjectRef, Owner)> {
        match self {
            ...
            ExecutionEffects::PeraTransactionBlockEffects(pera_tx_effects) => pera_tx_effects
                .mutated()
                .iter()
    ...
    pub fn quorum_sig(&self) -> Option<&AuthorityStrongQuorumSignInfo> {
        match self {
            ...
            ExecutionEffects::PeraTransactionBlockEffects(_) => None,
        }
    }
    ...
    pub fn gas_object(&self) -> (ObjectRef, Owner) {
        match self {
            ...
            ExecutionEffects::PeraTransactionBlockEffects(pera_tx_effects) => {
                let refe = &pera_tx_effects.gas_object();
    ...
    pub fn gas_cost_summary(&self) -> GasCostSummary {
        match self {
            ...
            crate::ExecutionEffects::PeraTransactionBlockEffects(b) => {
...
pub trait ValidatorProxy {
    ...
    async fn get_validators(&self) -> Result<Vec<PeraAddress>, anyhow::Error>;
}
    ...
    async fn execute_transaction_block(&self, tx: Transaction) -> anyhow::Result<ExecutionEffects> {
        ...
        while retry_cnt < 3 {
            let ticket = self
                .qd
                .submit_transaction(
                    pera_types::quorum_driver_types::ExecuteTransactionRequestV3 {
...
impl FullNodeProxy {
    pub async fn from_url(http_url: &str) -> Result<Self, anyhow::Error> {
        ...
        let resp = pera_client.read_api().get_committee_info(None).await?;
...
impl ValidatorProxy for FullNodeProxy {
    async fn get_object(&self, object_id: ObjectID) -> Result<Object, anyhow::Error> {
        let response = self
            .pera_client
            .read_api()
            .get_object_with_options(object_id, PeraObjectDataOptions::bcs_lossless())
            .await?;

        if let Some(pera_object) = response.data {
            pera_object.try_into()
            ...
            let response = self
                ...
                .get_owned_objects(
                    ...
                    Some(PeraObjectResponseQuery::new_with_options(
                        PeraObjectDataOptions::bcs_lossless(),
                    )),
                    cursor,
                    None,
                )
    ...
    async fn execute_transaction_block(&self, tx: Transaction) -> anyhow::Result<ExecutionEffects> {
        ...
        while retry_cnt < 10 {
            ...
            // PeraClient times out after 60s
            match self
                .pera_client
                .quorum_driver_api()
            ...
            {
                Ok(resp) => {
                    let effects = ExecutionEffects::PeraTransactionBlockEffects(
    ...
    fn clone_new(&self) -> Box<dyn ValidatorProxy + Send + Sync> {
        Box::new(Self {
            pera_client: self.pera_client.clone(),
    ...
    async fn get_validators(&self) -> Result<Vec<PeraAddress>, anyhow::Error> {
        let validators = self
            ...
            .await?
            .active_validators;
        Ok(validators.into_iter().map(|v| v.pera_address).collect())
    }
}
