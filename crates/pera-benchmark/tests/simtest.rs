...
#[cfg(msim)]
mod test {
    use pera_benchmark::bank::BenchmarkBank;
    use pera_benchmark::system_state_observer::SystemStateObserver;
    use pera_benchmark::workloads::adversarial::AdversarialPayloadCfg;
    use pera_benchmark::workloads::workload_configuration::WorkloadConfiguration;
    use pera_benchmark::{
    ...
    use pera_config::{AUTHORITIES_DB_NAME, PERA_KEYSTORE_FILENAME};
    ...
    use pera_core::checkpoints::{CheckpointStore, CheckpointWatermark};
    ...
    use pera_protocol_config::{PerObjectCongestionControlMode, ProtocolConfig, ProtocolVersion};
    use pera_simulator::tempfile::TempDir;
    use pera_simulator::{configs::*, SimConfig};
    use pera_storage::blob::Blob;
    use pera_surfer::surf_strategy::SurfStrategy;
    use pera_types::base_types::{ConciseableName, ObjectID, SequenceNumber};
    ...
    struct DeadValidator {
        node_id: pera_simulator::task::NodeId,
    ...
    async fn test_simulated_load_with_reconfig() {
        pera_protocol_config::ProtocolConfig::poison_get_for_min_version();
    ...
    fn get_keep_alive_nodes(cluster: &TestCluster) -> HashSet<pera_simulator::task::NodeId> {
        ...
        keep_alive_nodes.insert(pera_simulator::current_simnode_id());
        keep_alive_nodes
    }

    fn handle_failpoint(
        dead_validator: Arc<Mutex<Option<DeadValidator>>>,
        keep_alive_nodes: HashSet<pera_simulator::task::NodeId>,
        ...
        let cur_node = pera_simulator::current_simnode_id();
        ...
        if rng.gen_range(0.0..1.0) < probability {
            ...
            *dead_validator = Some(DeadValidator {
            ...
            pera_simulator::task::kill_current_node(Some(restart_after));
        }
    }
    ...
    async fn test_simulated_load_reconfig_with_prune_and_compact() {
        ...
        let node_state = test_cluster.fullnode_handle.pera_node.clone().state();
    ...
    async fn test_protocol_upgrade_compatibility_impl() {
        ...
        let manifest = pera_framework_snapshot::load_bytecode_snapshot_manifest();
        ...
        let init_framework =
            pera_framework_snapshot::load_bytecode_snapshot(starting_version).unwrap();
        let test_cluster = Arc::new(
        ...
        let _handle = tokio::task::spawn(async move {
            ...
            for version in starting_version..=max_ver {
                ...
                let new_framework = pera_framework_snapshot::load_bytecode_snapshot(next_version);
    ...
    fn handle_bool_failpoint(
        eligible_nodes: &HashSet<pera_simulator::task::NodeId>, // only given eligible nodes may fail
        probability: f64,
    ) -> bool {
        if !eligible_nodes.contains(&pera_simulator::current_simnode_id()) {
    ...
    async fn test_simulated_load_with_test_config(
        ...
        let keystore_path = test_cluster.swarm.dir().join(PERA_KEYSTORE_FILENAME);
        ...
        let surfer_task = tokio::spawn(async move {
            // now do a pera-surfer test
            let mut test_packages_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
            test_packages_dir.extend(["..", "..", "crates", "pera-surfer", "tests"]);
            let test_package_paths: Vec<PathBuf> = std::fs::read_dir(test_packages_dir)
                ...
                .collect();
            info!("using pera_surfer test packages: {test_package_paths:?}");

            let surf_strategy = SurfStrategy::new(Duration::from_millis(400));
            let results = pera_surfer::run_with_test_cluster_and_strategy(
                surf_strategy,
                test_duration,
                test_package_paths,
                test_cluster,
            ...
            .await;
            info!("pera_surfer test complete with results: {results:?}");
