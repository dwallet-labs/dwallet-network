...
use pera_types::gas_coin::TOTAL_SUPPLY_NPERA;
use typed_store::rocks::util::is_ref_count_value;

const NUM_SHARDS: usize = 4096;

struct AuthorityStoreMetrics {
    pera_conservation_check_latency: IntGauge,
    pera_conservation_live_object_count: IntGauge,
    pera_conservation_live_object_size: IntGauge,
    pera_conservation_imbalance: IntGauge,
    pera_conservation_storage_fund: IntGauge,
    pera_conservation_storage_fund_imbalance: IntGauge,
...
impl AuthorityStoreMetrics {
    pub fn new(registry: &Registry) -> Self {
        Self {
            pera_conservation_check_latency: register_int_gauge_with_registry!(
                "pera_conservation_check_latency",
                "Number of seconds took to scan all live objects in the store for PERA conservation check",
                registry,
            ).unwrap(),
            pera_conservation_live_object_count: register_int_gauge_with_registry!(
                "pera_conservation_live_object_count",
                "Number of live objects in the store",
                registry,
            ).unwrap(),
            pera_conservation_live_object_size: register_int_gauge_with_registry!(
                "pera_conservation_live_object_size",
                "Size in bytes of live objects in the store",
                registry,
            ).unwrap(),
            pera_conservation_imbalance: register_int_gauge_with_registry!(
                "pera_conservation_imbalance",
                "Total amount of PERA in the network - 10B * 10^9. This delta shows the amount of imbalance",
                registry,
            ).unwrap(),
            pera_conservation_storage_fund: register_int_gauge_with_registry!(
                "pera_conservation_storage_fund",
                "Storage Fund pool balance (only includes the storage fund proper that represents object storage)",
                registry,
            ).unwrap(),
            pera_conservation_storage_fund_imbalance: register_int_gauge_with_registry!(
                "pera_conservation_storage_fund_imbalance",
...
/// S is a template on Authority signature state. This allows PeraDataStore to be used on either
/// authorities or non-authorities. Specifically, when storing transactions and effects,
/// S allows PeraDataStore to either store the authority signed version or unsigned version.
pub struct AuthorityStore {
    ...
    /// Whether to enable expensive PERA conservation check at epoch boundaries.
...
impl AuthorityStore {
    ...
    pub async fn open(
        ...
        let enable_epoch_pera_conservation_check = config
            .expensive_safety_check_config
            .enable_epoch_pera_conservation_check();
        ...
        let epoch_start_configuration = if perpetual_tables.database_is_empty()? {
            ...
            let epoch_start_configuration = EpochStartConfiguration::new(
                genesis.pera_system_object().into_epoch_start_state(),
    ...
    pub fn get_recovery_epoch_at_restart(&self) -> PeraResult<EpochId> {
    ...
    pub fn effects_exists(&self, effects_digest: &TransactionEffectsDigest) -> PeraResult<bool> {
    ...
    pub async fn notify_read_root_state_hash(
        &self,
        epoch: EpochId,
    ) -> PeraResult<(CheckpointSequenceNumber, Accumulator)> {
    ...
    ) -> PeraResult<Option<(EpochId, CheckpointSequenceNumber)>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .get(digest)?)
    ...
    pub fn deprecated_multi_get_transaction_checkpoint(
        &self,
        digests: &[TransactionDigest],
    ) -> PeraResult<Vec<Option<(EpochId, CheckpointSequenceNumber)>>> {
        Ok(self
            .perpetual_tables
            .executed_transactions_to_checkpoint
            .multi_get(digests)?
    ...
    pub fn database_is_empty(&self) -> PeraResult<bool> {
    ...
    pub fn multi_object_exists_by_key(&self, object_keys: &[ObjectKey]) -> PeraResult<Vec<bool>> {
        Ok(self
            .perpetual_tables
            .objects
    ...
    fn get_object_ref_prior_to_key(
        ...
        object_id: &ObjectID,
        version: VersionNumber,
    ) -> Result<Option<ObjectRef>, PeraError> {
    ...
    /// Get many objects
    pub fn get_objects(&self, objects: &[ObjectID]) -> Result<Vec<Option<Object>>, PeraError> {
    ...
    pub(crate) fn insert_genesis_object(&self, object: Object) -> PeraResult {
    ...
    fn insert_object_direct(&self, object_ref: ObjectRef, object: &Object) -> PeraResult {
    ...
    pub(crate) fn bulk_insert_genesis_objects(&self, objects: &[Object]) -> PeraResult<()> {
    ...
    pub fn bulk_insert_live_objects(
        ...
        if *expected_sha3_digest != sha3_digest {
            ...
            return Err(PeraError::from("Sha does not match"));
        }
        batch.write()?;
        Ok(())
    }
    ...
    pub fn get_epoch_start_configuration(&self) -> PeraResult<Option<EpochStartConfiguration>> {
        ...
        for (live_marker, lock, obj_ref) in izip!(
            ...
            if let Some(previous_tx_digest) = &lock {
                if previous_tx_digest == &tx_digest {
                    ...
                    return Err(PeraError::ObjectLockConflict {
                        obj_ref: *obj_ref,
    ...
    pub fn check_owned_objects_are_live(&self, objects: &[ObjectRef]) -> PeraResult {
        let locks = self
            .perpetual_tables
            .live_owned_object_markers
            .multi_get(objects)?;
    ...
    /// Returns PeraError::ObjectLockAlreadyInitialized if the lock already exists and is locked to a transaction
    ...
    pub fn initialize_live_object_markers(
        ...
        if !is_force_reset {
            ...
            if !existing_live_object_markers.is_empty() {
                ...
                return Err(PeraError::ObjectLockAlreadyInitialized {
    ...
    pub fn revert_state_update(&self, tx_digest: &TransactionDigest) -> PeraResult {
    ...
    ) -> PeraResult<Vec<Option<VerifiedTransaction>>> {
        Ok(self
            .perpetual_tables
            .transactions
    ...
    /// If the intent is for testing, you can use AuthorityState:: get_pera_system_state_object_for_testing.
    pub fn get_pera_system_state_object_unsafe(&self) -> PeraResult<PeraSystemState> {
        get_pera_system_state(self.perpetual_tables.as_ref())
    }

    pub fn expensive_check_pera_conservation<T>(
        self: &Arc<Self>,
        type_layout_store: T,
    ...
    {
        if !self.enable_epoch_pera_conservation_check {
            return Ok(());
        }

        let executor = old_epoch_store.executor();
        info!("Starting PERA conservation check. This may take a while..");
        let cur_time = Instant::now();
        let mut pending_objects = vec![];
        let mut count = 0;
        let mut size = 0;
        let (mut total_pera, mut total_storage_rebate) = thread::scope(|s| {
        ...
        for object in pending_objects {
            total_storage_rebate += object.storage_rebate;
            total_pera +=
                object.get_total_pera(layout_resolver.as_mut()).unwrap() - object.storage_rebate;
        }
        info!(
        ...
        let system_state = self
            ...
            .expect("Reading pera system state object cannot fail")
            .into_pera_system_state_summary();
        let storage_fund_balance = system_state.storage_fund_total_object_storage_rebates;
        info!(
            "Total PERA amount in the network: {}, storage fund balance: {}, total storage rebate: {} at beginning of epoch {}",
            total_pera, storage_fund_balance, total_storage_rebate, system_state.epoch
        );
        ...
        self.metrics
            .pera_conservation_imbalance
            .set((total_pera as i128 - TOTAL_SUPPLY_NPERA as i128) as i64);
        ...
        if let Some(expected_pera) = self
            .perpetual_tables
            .expected_network_pera_amount
            .get(&())
            .expect("DB read cannot fail")
        {
            fp_ensure!(
                total_pera == expected_pera,
                PeraError::from(
                    format!(
                        "Inconsistent state detected at epoch {}: total pera: {}, expecting {}",
                        system_state.epoch, total_pera, expected_pera
                    )
                    .as_str()
                )
            );
...
pub type PeraLockResult = PeraResult<ObjectLockStatus>;
