...
pub struct AuthorityEpochTables {
    ...
    /// Holds all the DWallet MPC related messages that have been received since the beinning of the epoch
    pub(crate) dwallet_mpc_messages: DBMap<u64, Vec<DWalletMPCChannelMessage>>,
}
...
impl AuthorityPerEpochStore {
    ...
    async fn get_dwallet_mpc_sender(&self) -> DWalletMPCSender {
        loop {
            match self.dwallet_mpc_sender.get() {
                Some(sender) => return sender.clone(),
                None => {
                    error!("failed to get DWalletMPC sender, retrying...");
                }
            }
    ...
    pub(crate) async fn save_dwallet_mpc_message(&self, message: DWalletMPCChannelMessage) {
        let mut dwallet_mpc_epoch_messages = self.dwallet_mpc_round_messages.lock().await;
        dwallet_mpc_epoch_messages.push(message.clone());
    }

    /// Sends a message to the DWalletMPCManager & save it in the dwallet_mpc_round_messages.
    pub(crate) async fn send_message_to_dwallet_mpc_manager(
        &self,
        message: DWalletMPCChannelMessage,
    ) {
        let mut dwallet_mpc_epoch_messages = self.dwallet_mpc_round_messages.lock().await;
        dwallet_mpc_epoch_messages.push(message.clone());
        let sender = self.get_dwallet_mpc_sender().await;
        loop {
            match sender.send(message.clone()) {
                Ok(_) => break,
                Err(err) => {
                    error!(
                        "failed to send message to DWalletMPC sender, retrying... {:?}",
                        err
                    );
                }
            }
    ...
    /// Sends a message to the DWalletMPCManager without saving it in the dwallet_mpc_round_messages.
    /// This is useful while performing a state sync, where we don't want to save the messages, as they already have
    /// been saved.
    pub(crate) async fn send_msg_to_dwallet_mpc_manager_without_save(
        &self,
        message: &DWalletMPCChannelMessage,
    ) {
        let sender = self.get_dwallet_mpc_sender().await;
...
pub(crate) struct ConsensusCommitOutput {
    ...
    dwallet_mpc_round_messages: Vec<DWalletMPCChannelMessage>,
}
...
impl ConsensusCommitOutput {
    ...
    pub fn set_dwallet_mpc_round_messages(&mut self, new_value: Vec<DWalletMPCChannelMessage>) {
