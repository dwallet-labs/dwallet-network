...
use pera_types::crypto::{AccountKeyPair, AuthorityKeyPair};
...
pub async fn certify_transaction(
    authority: &AuthorityState,
    transaction: Transaction,
) -> Result<VerifiedCertificate, PeraError> {
...
pub async fn init_state_validator_with_fullnode() -> (Arc<AuthorityState>, Arc<AuthorityState>) {
    use pera_types::crypto::get_authority_key_pair;
...
pub async fn init_state_with_ids<I: IntoIterator<Item = (PeraAddress, ObjectID)>>(
    objects: I,
) -> Arc<AuthorityState> {
...
pub async fn init_state_with_ids_and_versions<
    I: IntoIterator<Item = (PeraAddress, ObjectID, SequenceNumber)>,
>(
    objects: I,
) -> Arc<AuthorityState> {
    let state = TestAuthorityBuilder::new().build().await;
...
pub async fn init_state_with_objects<I: IntoIterator<Item = Object>>(
    ...
    let network_config = pera_swarm_config::network_config_builder::ConfigBuilder::new(&dir).build();
...
) -> Result<Vec<TransactionEffects>, PeraError> {
...
pub async fn execute_sequenced_certificate_to_effects(
    authority: &AuthorityState,
    certificate: VerifiedCertificate,
) -> Result<(TransactionEffects, Option<ExecutionError>), PeraError> {
    authority.enqueue_certificates_for_execution(
