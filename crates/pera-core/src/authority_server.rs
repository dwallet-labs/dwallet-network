...
use pera_types::messages_grpc::{HandleCertificateRequestV3, HandleCertificateResponseV3};
...
use pera_types::traffic_control::{ClientIdSource, PolicyConfig, RemoteFirewallConfig, Weight};
use pera_types::{error::*, transaction::*};
use pera_types::{
    fp_ensure,
    messages_checkpoint::{
...
use pera_config::local_ip_utils::new_local_tcp_address_for_testing;
...
impl ValidatorService {
    ...
    async fn handle_transaction(
        ...
        let info = state
            ...
            .await
            .tap_err(|e| {
                if let PeraError::ValidatorHaltedAtEpochEnd = e {
    ...
    async fn handle_certificates(
        ...
        fp_ensure!(
            ...
            PeraError::FullNodeCantHandleCertificate.into()
        );
...

        {
            ...
            if !reconfiguration_lock.should_accept_user_certs() {
                ...
                return Err(PeraError::ValidatorHaltedAtEpochEnd.into());
            }
        ...
        let responses = futures::future::try_join_all(verified_certificates.into_iter().map(
            |certificate| async move {
                ...
                Ok::<_, PeraError>(HandleCertificateResponseV3 {
                    effects: signed_effects.into_inner(),
                    events,
                    input_objects,
                    output_objects,
...
impl ValidatorService {
    ...
    async fn soft_bundle_validity_check(
        ...
        fp_ensure!(
            ...
            PeraError::UnsupportedFeatureError {
                error: "Soft Bundle".to_string()
            }
            .into()
        );

        // Enforce these checks per [SIP-19](https://github.com/pera-foundation/sips/blob/main/sips/sip-19.md):
    ...
    async fn handle_soft_bundle_certificates_v3_impl(
        ...
        let certificates = NonEmpty::from_vec(request.certificates)
            .ok_or_else(|| PeraError::NoCertificateProvidedError)?;
        for certificate in &certificates {
    ...
    ) -> WrappedServiceResponse<PeraSystemState> {
        let response = self
            .state
    ...
    async fn handle_traffic_req(&self, client: Option<IpAddr>) -> Result<(), tonic::Status> {
        if let Some(traffic_controller) = &self.traffic_controller {
            if !traffic_controller.check(&client, &None).await {
                // Entity in blocklist
                Err(tonic::Status::from_error(PeraError::TooManyRequests.into()))
            } else {
                Ok(())
            }
        } else {
    ...
    fn handle_traffic_resp<T>(
        ...
        let (error, spam_weight, unwrapped_response) = match wrapped_response {
            ...
            Err(status) => (
                Some(PeraError::from(status.clone())),
                Weight::zero(),
                Err(status.clone()),
            ),
        };
...
impl Validator for ValidatorService {
    ...
    ) -> Result<tonic::Response<PeraSystemState>, tonic::Status> {
