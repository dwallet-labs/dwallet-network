...
use crate::authority::authority_store::{
    ExecutionLockWriteGuard, LockDetailsDeprecated, ObjectLockStatus, PeraLockResult,
};
...
use pera_types::base_types::{EpochId, ObjectID, ObjectRef, SequenceNumber, VerifiedExecutionData};
...
impl WritebackCache {
    ...
    async fn persist_transactions(&self, digests: &[TransactionDigest]) -> PeraResult {
    ...
    fn revert_state_update_impl(&self, tx: &TransactionDigest) -> PeraResult {
...
impl ObjectCacheRead for WritebackCache {
    ...
    fn get_lock(&self, obj_ref: ObjectRef, epoch_store: &AuthorityPerEpochStore) -> PeraLockResult {
        ...
        match self.get_object_by_id_cache_only("lock", &obj_ref.0) {
            ...
            CacheResult::NegativeHit => {
                Err(PeraError::from(UserInputError::ObjectNotFound {
                    object_id: obj_ref.0,
...
fn do_fallback_lookup<K: Copy, V: Default + Clone>(
    keys: &[K],
    get_cached_key: impl Fn(&K) -> PeraResult<CacheResult<V>>,
    multiget_fallback: impl Fn(&[K]) -> PeraResult<Vec<V>>,
) -> PeraResult<Vec<V>> {
