...
use pera_types::{
    base_types::{random_object_ref, PeraAddress},
...
impl Scenario {
    ...
    fn new_outputs() -> TransactionOutputs {
        ...
        let (sender, keypair): (PeraAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
        let (receiver, _): (PeraAddress, AccountKeyPair) = get_key_pair_from_rng(&mut rng);
    ...
    pub async fn commit(&mut self, tx: TransactionDigest) -> PeraResult {
...
async fn latest_object_cache_race_test() {
    ...
    let owner = PeraAddress::random_for_testing_only();
