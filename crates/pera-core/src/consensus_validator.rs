...
impl PeraTxValidator {
    pub fn new(
        ...
        info!(
            "PeraTxValidator constructed for epoch {}",
            epoch_store.epoch()
        );
        Self {
            epoch_store,
            ...
            match tx {
                ...
                ConsensusTransactionKind::RandomnessDkgMessage(_, bytes) => {
                    if bytes.len() > dkg::DKG_MESSAGES_MAX_SIZE {
                        ...
                        return Err(PeraError::InvalidDkgMessageSize.into());
                    }
                }
                ...
                | ConsensusTransactionKind::DWalletMPCOutput(_, _, _, _, _) => {}
            }
        }
...
impl TransactionValidator for PeraTxValidator {
    type Error = eyre::Report;

    fn validate(&self, _tx: &[u8]) -> Result<(), Self::Error> {
        // We only accept transactions from local pera instance so no need to re-verify it
        Ok(())
    }
...
impl TransactionVerifier for PeraTxValidator {
    fn verify_batch(
        &self,
...
mod tests {
    ...
    use pera_types::{
        crypto::Ed25519PeraSignature, messages_consensus::ConsensusTransaction, object::Object,
    ...
    async fn accept_valid_transaction() {
        ...
        let metrics = PeraTxValidatorMetrics::new(&Default::default());
        let validator = PeraTxValidator::new(
