...
//! - **Handling Signature MPC Output**: When a validator detects a **SignatureMPCOutput** transaction,
...
impl<P: CreatableParty> SignatureMPCInstance<P> {
    ...
    fn new_signature_mpc_message(&self, message: P::Message) -> Option<ConsensusTransaction> {
        self.epoch_store().ok().map(|epoch_store| {
            ConsensusTransaction::new_signature_mpc_message(
                epoch_store.name,
            ...
            authority_name_to_party_id(epoch_store.name, &epoch_store)
                ...
                .and_then(|_| {
                    bcs::to_bytes(&output).ok().map(|output_bytes| {
                        ConsensusTransaction::new_signature_mpc_output(
                            output_bytes,
                            self.session_id.clone(),
...
impl<P: CreatableParty + Sync + Send> SignatureMPCManager<P> {
    ...
    pub async fn handle_end_of_delivery(&mut self) -> PeraResult<()> {
        ...
        let mut ready_to_advance: Vec<_> = self
            ...
            .filter_map(|(_, instance)| {
                // todo: https://github.com/dwallet-labs/dwallet-network/pull/280/files#r1807768657
                // todo: https://github.com/dwallet-labs/dwallet-network/pull/280/files#r1799325701
    ...
    pub fn handle_message(
        ...
        match self.mpc_instances.get_mut(&session_id) {
            Some(instance) => instance.handle_message(SignatureMPCMessage {
