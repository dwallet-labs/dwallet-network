...
use dwallet_classgroups_types::class_groups_key::generate_class_groups_keypair_and_proof_from_seed;
...
use pera_types::{
    base_types::{AuthorityName, ExecutionDigests, ObjectRef, PeraAddress, TransactionDigest},
    committee::Committee,
...
) -> Result<(CertifiedTransaction, SignedTransactionEffects), PeraError> {
...
pub fn compile_managed_coin_package() -> CompiledPackage {
    compile_example_package("../../crates/pera-core/src/unit_tests/data/managed_coin")
}
...
async fn init_genesis(
    ...
    let mut builder = pera_genesis_builder::Builder::new().add_objects(genesis_objects);
...
pub fn make_transfer_pera_transaction(
...
pub fn make_pay_pera_transaction(
    gas_object: ObjectRef,
    coins: Vec<ObjectRef>,
    ...
    let data = TransactionData::new_pay_pera(
