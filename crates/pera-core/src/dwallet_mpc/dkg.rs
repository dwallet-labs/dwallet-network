...
use group::PartyID;
use mpc::{Advance, AsynchronousOutput, Party, WeightedThresholdAccessStructure};
use std::collections::{HashMap, HashSet};
use serde::de::DeserializeOwned;
use twopc_mpc::dkg::Protocol;
use pera_types::error::{PeraError, PeraResult};
use crate::dwallet_mpc::bytes_party::AdvanceResult;
...
pub(super) fn advance<P: Advance>(
    ...
    Ok(match res {
        mpc::AdvanceResult::Advance((msg, party)) => {
            mpc::AdvanceResult::Advance((bcs::to_bytes(&msg)?, party))
        }
        mpc::AdvanceResult::Finalize(output) => {
            let output: P::OutputValue = output.into();
            mpc::AdvanceResult::Finalize(bcs::to_bytes(&output)?)
        }
        mpc::AdvanceResult::FinalizeAsync(output) => {
            let output_val: P::OutputValue = output.output.into();
            let output = AsynchronousOutput {
                malicious_parties: output.malicious_parties,
                output: bcs::to_bytes(&output_val)?,
            };
            mpc::AdvanceResult::FinalizeAsync(output)
        }
    })
