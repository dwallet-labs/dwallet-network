...
use pera_types::crypto::{deterministic_random_account_key, get_key_pair, AccountKeyPair};
use pera_types::effects::TransactionEffectsAPI;
use pera_types::object::{generate_test_gas_objects, Object};
...
fn make_tx(
    ...
    make_transfer_pera_transaction(
...
async fn test_quorum_driver_object_locked() -> Result<(), anyhow::Error> {
    ...
    let (sender, keypair): (PeraAddress, AccountKeyPair) = deterministic_random_account_key();
