...
impl<A: Clone> QuorumDriver<A> {
    ...
    async fn enqueue_task(&self, task: QuorumDriverTask) -> PeraResult<()> {
        self.task_sender
            .send(task.clone())
            .await
            ...
            .map_err(|e| PeraError::QuorumDriverCommunicationError {
                error: e.to_string(),
            })
    }
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    pub async fn submit_transaction(
        &self,
        request: ExecuteTransactionRequestV3,
    ) -> PeraResult<Registration<TransactionDigest, QuorumDriverResult>> {
