...
impl TransactiondOrchestrator<NetworkAuthorityClient> {
    pub fn new_with_auth_aggregator(
        ...
        validator_state: Arc<AuthorityState>,
        reconfig_channel: Receiver<PeraSystemState>,
        parent_path: &Path,
    ...
    async fn execute_finalized_tx_locally_with_timeout(
        ...
        .await
        {
            ...
            Ok(Err(err)) => {
                ...
                Err(PeraError::TransactionOrchestratorLocalExecutionError {
                    error: err.to_string(),
                })
            }
            Ok(Ok(_)) => {
...
impl<A> pera_types::transaction_executor::TransactionExecutor for TransactiondOrchestrator<A>
where
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
