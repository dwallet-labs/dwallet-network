...
use pera_types::{base_types::*, committee::*, fp_ensure};
use pera_types::{
...
impl<C: Clone> SafeClient<C> {
    ...
    fn get_committee(&self, epoch_id: &EpochId) -> PeraResult<Arc<Committee>> {
        self.committee_store
            .get_committee(epoch_id)?
            .ok_or(PeraError::MissingCommitteeAtEpoch(*epoch_id))
    }
    ...
    ) -> PeraResult<SignedTransactionEffects> {
                ...
                match cert_opt {
                    Some(cert) => {
                        ...
                        ct.verify_committee_sigs_only(&committee).map_err(|e| {
                            PeraError::FailedToVerifyTxCertWithExecutedEffects {
    ...
    ) -> PeraResult<VerifiedObjectInfoResponse> {
    ...
    C: AuthorityAPI + Send + Sync + Clone + 'static,
{
    ...
    ) -> Result<PlainTransactionInfoResponse, PeraError> {
    ...
    ) -> PeraResult<HandleCertificateResponseV2> {
        let signed_effects =
    ...
    ) -> PeraResult<HandleCertificateResponseV3> {
        ...
        match (&events, effects.events_digest()) {
            ...
            (Some(events), None) => {
                if !events.data.is_empty() {
                    return Err(PeraError::ByzantineAuthoritySuspicion {
                        authority: self.address,
    ...
    pub async fn handle_object_info_request(
        &self,
        request: ObjectInfoRequest,
    ) -> Result<VerifiedObjectInfoResponse, PeraError> {
    ...
    fn verify_checkpoint_sequence(
        ...
        if let (Some(e), Some(o)) = (expected_seq, observed_seq) {
            fp_ensure!(
                e == o,
                PeraError::from("Expected checkpoint number doesn't match with returned")
            );
        }
        Ok(())
    }
    ...
    fn verify_contents_exist<T, O>(
        ...
        match (request_content, checkpoint, contents) {
            ...
            (true, Some(_), None) | (false, _, Some(_)) | (_, None, Some(_)) => Err(
                PeraError::from("Checkpoint contents inconsistent with request"),
            ),
            _ => Ok(()),
        }
    }
    ...
    #[instrument(level = "trace", skip_all, fields(authority = ?self.address.concise()))]
    pub async fn handle_system_state_object(&self) -> Result<PeraSystemState, PeraError> {
        self.authority_client
