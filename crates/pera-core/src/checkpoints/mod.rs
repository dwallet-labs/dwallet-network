...
use pera_types::digests::{CheckpointContentsDigest, CheckpointDigest};
...
impl CheckpointStore {
    ...
    fn prune_local_summaries(&self) -> PeraResult {
...
impl CheckpointBuilder {
    ...
    ) -> PeraResult<Vec<TransactionEffects>> {
        self.metrics
    ...
    ) -> PeraResult<Option<(TransactionDigest, TransactionEffects)>> {
    ...
    async fn augment_epoch_last_checkpoint(
        ...
        // TODO: Check whether we must use anyhow::Result or can we use PeraResult.
...
impl CheckpointAggregator {
    ...
    async fn run_and_notify(&mut self) -> PeraResult {
    ...
    fn run_inner(&mut self) -> PeraResult<Vec<CertifiedCheckpointSummary>> {
...
impl CheckpointSignatureAggregator {
    ...
    pub fn try_aggregate(
        ...
        match self.signatures_by_digest.insert(their_digest, envelope) {
            ...
            InsertResult::Failed {
                error:
                    PeraError::StakeAggregatorRepeatedSigner {
...
async fn diagnose_split_brain(
    ...
    let committee = epoch_store
        .epoch_start_state()
        .get_pera_committee_with_network_metadata();
...
pub trait CheckpointServiceNotify {
    ...
    fn notify_checkpoint(&self) -> PeraResult;
}

/// This is a service used to communicate with other pieces of pera(for ex. authority)
...
mod tests {
    ...
    use pera_types::crypto::{AuthoritySignInfo, Signature};
    use pera_types::digests::TransactionEventsDigest;
    use pera_types::effects::{TransactionEffects, TransactionEvents};
    use pera_types::messages_checkpoint::SignedCheckpointSummary;
    ...
    pub async fn checkpoint_builder_test() {
        ...
        for digest in all_digests {
            let signature = Signature::Ed25519PeraSignature(Default::default()).into();
            state
    ...
    impl TransactionCacheRead for HashMap<TransactionDigest, TransactionEffects> {
        ...
        ) -> BoxFuture<'_, PeraResult<Vec<TransactionEffects>>> {
        ...
        fn notify_read_executed_effects_digests(
            &self,
            digests: &[TransactionDigest],
        ) -> BoxFuture<'_, PeraResult<Vec<TransactionEffectsDigest>>> {
            std::future::ready(Ok(digests
                .iter()
                .map(|d| {
                    self.get(d)
