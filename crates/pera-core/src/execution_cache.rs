...
pub fn choose_execution_cache(config: &ExecutionCacheConfig) -> ExecutionCacheConfigType {
    #[cfg(msim)]
    {
        ...
        pera_macros::fail_point_if!("select-random-cache", || {
...
/// Should only be used for pera-tool or tests. Nodes must use build_execution_cache which
...
pub trait ObjectCacheRead: Send + Sync {
    fn get_package_object(&self, id: &ObjectID) -> PeraResult<Option<PackageObject>>;
    fn force_reload_system_packages(&self, system_package_ids: &[ObjectID]);

    fn get_object(&self, id: &ObjectID) -> PeraResult<Option<Object>>;

    fn get_objects(&self, objects: &[ObjectID]) -> PeraResult<Vec<Option<Object>>> {
    ...
    fn get_latest_object_or_tombstone(
        &self,
        object_id: ObjectID,
    ) -> PeraResult<Option<(ObjectKey, ObjectOrTombstone)>>;
        ...
        for (object_opt, object_ref) in objects.into_iter().zip(object_refs) {
            match object_opt {
                None => {
                    ...
                    return Err(PeraError::UserInputError { error });
                }
                Some(object) => {
                    result.push(object);
                }
    ...
    fn get_last_shared_object_deletion_info(
        ...
        object_id: &ObjectID,
        epoch_id: EpochId,
    ) -> PeraResult<Option<(SequenceNumber, TransactionDigest)>> {
    ...
    fn get_deleted_shared_object_previous_tx_digest(
        ...
        version: SequenceNumber,
        epoch_id: EpochId,
    ) -> PeraResult<Option<TransactionDigest>> {
...
pub trait TransactionCacheRead: Send + Sync {
    ...
    ) -> PeraResult<Option<Arc<VerifiedTransaction>>> {
    ...
    fn get_transactions_and_serialized_sizes(
        &self,
        digests: &[TransactionDigest],
    ) -> PeraResult<Vec<Option<(VerifiedTransaction, usize)>>> {
    ...
    fn is_tx_already_executed(&self, digest: &TransactionDigest) -> PeraResult<bool> {
    ...
    fn notify_read_executed_effects<'a>(
        &'a self,
        digests: &'a [TransactionDigest],
    ) -> BoxFuture<'a, PeraResult<Vec<TransactionEffects>>> {
        async move {
...
pub trait ExecutionCacheReconfigAPI: Send + Sync {
    fn insert_genesis_object(&self, object: Object) -> PeraResult;
    fn bulk_insert_genesis_objects(&self, objects: &[Object]) -> PeraResult;

    fn revert_state_update(&self, digest: &TransactionDigest) -> PeraResult;
    ...
    fn expensive_check_pera_conservation(
        &self,
        old_epoch_store: &AuthorityPerEpochStore,
    ) -> PeraResult;

    fn checkpoint_db(&self, path: &Path) -> PeraResult;
    ...
    ($implementor: ident) => {
        ...
        impl BackingPackageStore for $implementor {
            fn get_package_object(
                &self,
                package_id: &ObjectID,
            ) -> PeraResult<Option<PackageObject>> {
    ...
    ($implementor: ident) => {
        ...
        impl ExecutionCacheReconfigAPI for $implementor {
            ...
            fn expensive_check_pera_conservation(
                ...
                self.store
                    .expensive_check_pera_conservation(self, old_epoch_store)
            }

            fn checkpoint_db(&self, path: &std::path::Path) -> PeraResult {
