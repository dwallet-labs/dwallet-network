                        ...
                        &transaction.kind
                    {
                        ...
                        let output_verification_result = match dwallet_mpc_manager {
                            Some(mpc_manager) => {
                                ...
                                dwallet_mpc_manager.try_verify_output(output, &session_info).unwrap_or_else(|e| {
                                    // TODO (#311): Make validator don't mark other validators as malicious or take any active action while syncing
                                    error!("error verifying DWalletMPCOutput for the session {:?} from party {:?}: {:?}", session_info.session_id, authority_index, e);
