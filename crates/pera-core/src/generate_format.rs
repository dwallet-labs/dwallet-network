...
use pera_types::messages_grpc::ObjectInfoRequestKind;
use pera_types::move_package::TypeOrigin;
use pera_types::{
...
use pera_types::{
    ...
    crypto::{
        ...
        AuthorityPublicKeyBytes, AuthoritySignature, KeypairTraits, Signature, PeraKeyPair,
    },
...
fn get_registry() -> Result<Registry> {
    ...
    let kp1: PeraKeyPair =
        PeraKeyPair::Ed25519(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp2: PeraKeyPair =
        PeraKeyPair::Secp256k1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp3: PeraKeyPair =
        PeraKeyPair::Secp256r1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    ...
    // ObjectID and PeraAddress are the same length
    ...
    let struct_tag = StructTag::from_str("0x2::coin::Coin<0x2::pera::PERA>").unwrap();
    ...
    tracer.trace_type::<base_types::PeraAddress>(&samples)?;
...
#[derive(Debug, Parser)]
#[clap(
    name = "Pera format generator",
    about = "Trace serde (de)serialization to generate format descriptions for Pera types"
)]
struct Options {
...
const FILE_PATH: &str = "pera-core/tests/staged/pera.yaml";
