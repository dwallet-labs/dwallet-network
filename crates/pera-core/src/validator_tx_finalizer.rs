...
mod tests {
    ...
    use pera_types::base_types::{AuthorityName, ObjectID, PeraAddress, TransactionDigest};
    use pera_types::committee::{CommitteeTrait, StakeUnit};
    use pera_types::crypto::{get_account_key_pair, AccountKeyPair};
    use pera_types::effects::{TransactionEffectsAPI, TransactionEvents};
    ...
    impl AuthorityAPI for MockAuthorityClient {
        async fn handle_transaction(
            ...
            if self.inject_fault.load(Relaxed) {
                return Err(PeraError::TimeoutError);
            }
