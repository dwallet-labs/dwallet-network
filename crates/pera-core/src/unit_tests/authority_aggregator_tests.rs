...
use pera_types::crypto::{AuthoritySignature, Signer};
...
async fn test_map_reducer() {
    ...
    let res = quorum_map_then_reduce_with_timeout::<_, _, _, _, _, (), _, _, _>(
        ...
        |_name, _client| {
            Box::pin(async move {
                let res: Result<usize, PeraError> = Err(PeraError::TooManyIncorrectAuthorities {
                    errors: vec![],
                    action: "".to_string(),
                });
                res
        ...
        |mut accumulated_state, _authority_name, _authority_weight, result| {
            Box::pin(async move {
                assert!(matches!(
                    result,
                    Err(PeraError::TooManyIncorrectAuthorities { .. })
                ));
                accumulated_state += 1;
            ...
            |_name, client| {
                ...
                Box::pin(async move {
                    ...
                    match res {
                        ...
                        Err(PeraError::TransactionNotFound { .. }) => Ok(()),
                        Err(err) => Err(err),
                    }
                })
            },
...
async fn test_handle_transaction_fork() {
    ...
    let tx = make_transfer_pera_transaction(
        gas_object,
        PeraAddress::default(),
        None,
        sender,
...
async fn test_handle_certificate_response() {
    ...
    let (sender, sender_kp): (_, AccountKeyPair) = get_key_pair();
    let gas_object = random_object_ref();
    let tx = VerifiedTransaction::new_unchecked(make_transfer_pera_transaction(
        gas_object,
        PeraAddress::default(),
        None,
        sender,
    ...
    assert_resp_err(&agg, tx.clone().into(), |e| matches!(e, AggregatorProcessTransactionError::RetryableTransaction { .. }),
        |e| matches!(e, PeraError::WrongEpoch { expected_epoch, actual_epoch } if *expected_epoch == 1 && *actual_epoch == 0)
    ).await;
    ...
    assert_matches!(
        ...
        AggregatorProcessCertificateError::RetryableExecuteCertificate {
            retryable_errors, ..
        } if retryable_errors.iter().any(|(error, _, _)| matches!(error, PeraError::WrongEpoch {
...
async fn test_handle_transaction_response() {
    ...
    let tx2 = VerifiedTransaction::new_unchecked(make_transfer_pera_transaction(
        gas_object,
        PeraAddress::default(),
        Some(1),
        sender,
    ...
    let package_not_found_error = PeraError::UserInputError {
        error: UserInputError::DependentPackageNotFound {
            package_id: gas_object.0,
        },
    };
    let object_not_found_error = PeraError::UserInputError {
    ...
    assert_resp_err(
        ...
        |e| matches!(e, PeraError::MissingCommitteeAtEpoch(e) if *e == 1),
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                PeraError::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. }
                    | PeraError::RpcError(..)
            )
        },
    )
    ...
    assert_resp_err(
        ...
        |e| matches!(e, PeraError::WrongEpoch { expected_epoch, actual_epoch } if *expected_epoch == 0 && *actual_epoch == 1)
    )
    .await;
...
async fn test_handle_conflicting_transaction_response() {
    ...
    let tx1 = VerifiedTransaction::new_unchecked(make_transfer_pera_transaction(
        conflicting_object,
        PeraAddress::default(),
        Some(1),
        sender,
    ...
    let conflicting_tx2 = VerifiedTransaction::new_unchecked(make_transfer_pera_transaction(
        conflicting_object,
        PeraAddress::default(),
        Some(2),
        sender,
    ...
    let conflicting_error = PeraError::ObjectLockConflict {
        obj_ref: conflicting_object,
        pending_transaction: *conflicting_tx2.digest(),
    };
    let retryable_error = PeraError::RpcError("RPC".into(), "Error".into());
    let non_retryable_error = PeraError::ByzantineAuthoritySuspicion {
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                PeraError::ObjectLockConflict { .. } | PeraError::RpcError(..)
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| matches!(e, PeraError::ObjectLockConflict { .. }),
    )
    .await;
    ...
    let conflicting_tx3 = make_transfer_pera_transaction(
        conflicting_object,
        PeraAddress::default(),
        Some(3),
        sender,
    ...
    let conflicting_error_2 = PeraError::ObjectLockConflict {
        obj_ref: conflicting_object,
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                PeraError::ObjectLockConflict { .. } | PeraError::UserInputError { .. }
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                PeraError::MissingCommitteeAtEpoch(..) | PeraError::ObjectLockConflict { .. }
            )
        },
    )
    .await;
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                PeraError::WrongEpoch { .. } | PeraError::ObjectLockConflict { .. }
            )
        },
    )
    .await;
...
async fn test_handle_overload_response() {
    ...
    let (sender, sender_kp): (_, AccountKeyPair) = get_key_pair();
    let gas_object = random_object_ref();
    let txn = make_transfer_pera_transaction(
        gas_object,
        PeraAddress::default(),
        None,
        sender,
    ...
    let overload_error = PeraError::TooManyTransactionsPendingExecution {
        queue_len: 100,
        threshold: 100,
    };
    let rpc_error = PeraError::RpcError("RPC".into(), "Error".into());
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                PeraError::TooManyTransactionsPendingExecution { .. } | PeraError::RpcError(..)
            )
        },
    )
    .await;
...
// Tests that authority aggregator can aggregate PeraError::ValidatorOverloadedRetryAfter into
...
async fn test_handle_overload_retry_response() {
    ...
    for (index, (name, _)) in authority_keys.iter().enumerate() {
        clients.get_mut(name).unwrap().set_tx_info_response_error(
            PeraError::ValidatorOverloadedRetryAfter {
    ...
    assert_resp_err(
        ...
        |e| {
            matches!(
                e,
                PeraError::ValidatorOverloadedRetryAfter { .. } | PeraError::RpcError(..)
            )
        },
    )
    .await;
    ...
    for (name, secret) in &authority_keys {
        let auth_signature = if byzantines.contains(name) {
            ...
            AuthoritySignInfo::new(
                0,
                tx.clone().data(),
                Intent::pera_app(IntentScope::ProofOfPossession), // bad intent
                *name,
                secret,
            )
        } else {
    ...
    match agg.process_transaction(tx, Some(make_socket_addr())).await {
        Err(received_agg_err) if agg_err_checker(&received_agg_err) => match received_agg_err {
            AggregatorProcessTransactionError::RetryableConflictingTransaction {
                ...
                assert!(errors.iter().map(|e| &e.0).all(pera_err_checker));
            }
...
fn set_cert_response_with_certified_tx(
    ...
    let effects = effects_with_tx(*cert.digest());
    for (name, secret) in authority_keys {
        let resp = pera_types::messages_grpc::HandleCertificateResponseV2 {
...
fn set_retryable_tx_info_response_error(
    ...
    let error = PeraError::RpcError("RPC".into(), "Error".into());
