...
use pera_types::{MOVE_STDLIB_ADDRESS, PERA_FRAMEWORK_ADDRESS};
...
fn test_to_json_value() {
    ...
    let pera_move_struct: PeraMoveStruct =
        BoundedVisitor::deserialize_struct(&event_bytes, &TestEvent::layout())
            .unwrap()
            .into();
    let json_value = pera_move_struct.to_json_value();
    assert_eq!(
        Some(&json!("1000000")),
...
impl TestEvent {
    fn type_() -> StructTag {
        StructTag {
            ...
            module: ident_str!("PERA").to_owned(),
...
// TODO: Do we need this in the pera-types lib?
