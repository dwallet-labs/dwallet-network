...
use pera_move_build::{check_unpublished_dependencies, gather_published_ids, BuildConfig};
use pera_types::{
...
async fn test_generate_lock_file() {
    ...
    let expected = expect![[r##"
        ...
        [[move.package]]
        name = "MoveStdlib"
        source = { local = "../../../../../pera-framework/packages/move-stdlib" }

        [[move.package]]
        name = "Pera"
        source = { local = "../../../../../pera-framework/packages/pera-framework" }
...
async fn test_custom_property_check_unpublished_dependencies() {
    ...
    let PeraError::ModulePublishFailure { error } = check_unpublished_dependencies(
