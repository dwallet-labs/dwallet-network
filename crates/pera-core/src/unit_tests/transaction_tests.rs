...
async fn test_handle_transfer_transaction_bad_signature() {
    do_transaction_test(
        ...
        |err| {
            assert_matches!(err, PeraError::SignerSignatureAbsent { .. });
        },
    )
    .await;
}
...
async fn test_handle_transfer_transaction_no_signature() {
    do_transaction_test(
        ...
        |err| {
            assert_matches!(
                err,
                PeraError::SignerSignatureNumberMismatch {
                    expected: 1,
                    actual: 0
                }
            );
...
async fn test_zklogin_transfer_with_large_address_seed() {
    ...
    let sender = PeraAddress::generate(StdRng::from_seed([3; 32]));
    let recipient = dbg_addr(2);
...
async fn zklogin_test_caching_scenarios() {
    ...
    let (skp, _eph_pk, zklogin) =
        &load_test_vectors("../pera-types/src/unit_tests/zklogin_test_vectors.json")[1];
    ...
    let zklogin_json_string =
        &get_one_zklogin_inputs("../pera-types/src/unit_tests/zklogin_test_vectors.json");
    let bad_zklogin_inputs = ZkLoginInputs::from_json(zklogin_json_string, "111").unwrap();
    let sender = PeraAddress::try_from_unpadded(&bad_zklogin_inputs).unwrap();
...
async fn setup_zklogin_network(
    ...
    pera_types::message_envelope::Envelope<SenderSignedData, pera_types::crypto::EmptySignInfo>,
    ...
    let sender = PeraAddress::try_from_unpadded(zklogin).unwrap();
    ...
    let sender_2 = PeraAddress::from(&multisig_pk);
...
async fn zk_multisig_test() {
    ...
    let keys = pera_types::utils::keys();
    ...
    .unwrap();
    let victim_addr = PeraAddress::from(&multisig_pk);
    ...
    for (kp, _pk_zklogin, inputs) in test_vectors {
        let intent_message = IntentMessage::new(Intent::pera_transaction(), data.clone());
...
async fn test_very_large_certificate() {
    ...
    let quorum_signature = pera_types::crypto::AuthorityQuorumSignInfo {
        epoch: 0,
        signature: pera_types::crypto::AggregateAuthoritySignature::aggregate(&sigs)
    ...
    let cert = pera_types::message_envelope::Envelope::new_from_data_and_sig(
    ...
    assert!(
        matches!(err, PeraError::RpcError(..))
...
async fn test_handle_soft_bundle_certificates_errors() {
    ...
    // Case 0: submit an empty soft bundle.
    {
        ...
        assert_matches!(
            response.unwrap_err(),
            PeraError::NoCertificateProvidedError { .. }
        );
    }
...
fn sender_signed_data_serialized_intent() {
    ...
    assert_eq!(txn.intent_message().intent, Intent::pera_transaction());
