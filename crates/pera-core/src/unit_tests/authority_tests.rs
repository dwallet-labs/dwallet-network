...
use pera_types::execution::SharedInput;
...
use pera_types::{
    ...
    MOVE_STDLIB_PACKAGE_ID, PERA_AUTHENTICATOR_STATE_OBJECT_ID, PERA_CLOCK_OBJECT_ID,
    PERA_FRAMEWORK_PACKAGE_ID, PERA_RANDOMNESS_STATE_OBJECT_ID, PERA_SYSTEM_STATE_OBJECT_ID,
};
...
async fn test_dry_run_no_gas_big_transfer() {
    ...
    builder.transfer_pera(recipient, Some(amount));
    ...
    let (dry_run_res, _, _, _) = fullnode
        ...
        .await
        .unwrap();
    assert_eq!(*dry_run_res.effects.status(), PeraExecutionStatus::Success);
}
...
async fn test_dev_inspect_gas_coin_argument() {
    ...
    let sender = PeraAddress::random_for_testing_only();
    let recipient = PeraAddress::random_for_testing_only();
    let amount = 500;
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_pera(vec![recipient], vec![amount]).unwrap();
        builder.finish()
    };
    let kind = TransactionKind::programmable(pt);
    ...
    assert_eq!(arg, &PeraArgument::GasCoin);
    check_coin_value(
        arg_value,
        arg_type,
...
fn check_coin_value(actual_value: &[u8], actual_type: &PeraTypeTag, expected_value: u64) {
...
async fn test_transfer_package() {
    ...
    let package_object_ref = authority_state
        .get_pera_system_package_object_ref()
        .await
        .unwrap();
...
async fn test_objected_owned_gas() {
    ...
    authority_state
        ...
        .await;
    let rgp = authority_state.reference_gas_price_for_testing().unwrap();
    let data = TransactionData::new_transfer_pera(
        recipient,
        sender,
        None,
...
async fn test_conflicting_transactions() {
    ...
    for _ in 0..100 {
        ...
        assert!(matches!(err, PeraError::ObjectLockConflict { .. }));
...
#[tokio::test]
async fn test_handle_transfer_pera_with_amount_insufficient_gas() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let object_id = ObjectID::random();
    let authority_state = init_state_with_ids(vec![(sender, object_id)]).await;
...
async fn test_invalid_mutable_clock_parameter() {
    ...
    assert_eq!(
        ...
        UserInputError::ImmutableParameterExpectedError {
            object_id: PERA_CLOCK_OBJECT_ID
        }
    );
}
...
async fn test_invalid_authenticator_state_parameter() {
    ...
    assert_eq!(
        ...
        UserInputError::InaccessibleSystemObject {
            object_id: PERA_AUTHENTICATOR_STATE_OBJECT_ID
        }
    );
}
...
async fn test_invalid_randomness_parameter() {
    ...
    assert_eq!(
        UserInputError::try_from(e).unwrap(),
        UserInputError::ImmutableParameterExpectedError {
            object_id: PERA_RANDOMNESS_STATE_OBJECT_ID
        }
    );
}
...
#[tokio::test]
async fn test_genesis_pera_system_state_object() {
    // This test verifies that we can read the genesis PeraSystemState object.
    ...
    let _pera_system_state =
        bcs::from_bytes::<PeraSystemStateWrapper>(move_object.contents()).unwrap();
    assert!(move_object.type_().is(&PeraSystemStateWrapper::type_()));
    let pera_system_state = authority_state
        .get_pera_system_state_object_for_testing()
        .unwrap();
    assert_eq!(
        &pera_system_state
...
#[tokio::test]
async fn test_transfer_pera_no_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
    let init_balance = pera_types::gas::get_gas_balance(&gas_object).unwrap();
    ...
    let gas_ref = gas_object.compute_object_reference();
    let tx_data = TransactionData::new_transfer_pera(
        recipient,
        sender,
        None,
        gas_ref,
    ...
    let new_balance = pera_types::gas::get_gas_balance(
        &authority_state
            .get_object(&gas_object_id)
            .await
            .unwrap()
...
#[tokio::test]
async fn test_transfer_pera_with_amount() {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let recipient = dbg_addr(2);
    let gas_object_id = ObjectID::random();
    let gas_object = Object::with_id_owner_for_testing(gas_object_id, sender);
    ...
    let new_gas = authority_state
        ...
        .unwrap()
        .unwrap();
    assert_eq!(pera_types::gas::get_gas_balance(&new_gas).unwrap(), 500);
    assert!(gas_ref.1 < effects.gas_object().0 .1);
...
#[tokio::test]
async fn test_store_revert_transfer_pera() {
    // This test checks the correctness of revert_state_update in PeraDataStore.
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
...
async fn create_and_retrieve_df_info(function: &IdentStr) -> (PeraAddress, Vec<DynamicFieldInfo>) {
    let (sender, sender_key): (_, AccountKeyPair) = get_key_pair();
    let gas_object_id = ObjectID::random();
    let (authority_state, object_basics) =
        init_state_with_ids_and_object_basics(vec![(sender, gas_object_id)]).await;
...
pub async fn init_state_with_ids_and_object_basics<
    I: IntoIterator<Item = (PeraAddress, ObjectID)>,
>(
    objects: I,
) -> (Arc<AuthorityState>, ObjectRef) {
    let state = TestAuthorityBuilder::new().build().await;
...
async fn test_consensus_message_processed() {
    ...
    let network_config = pera_swarm_config::network_config_builder::ConfigBuilder::new(&dir)
...
async fn test_gas_smashing() {
    ...
    fn make_gas_coins(owner: PeraAddress, gas_amount: u64, coin_num: u64) -> Vec<Object> {
        let mut objects = vec![];
    ...
    async fn run_and_check(
        ...
        let balance = pera_types::gas::get_gas_balance(
...
async fn test_publish_missing_dependency() {
    ...
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.publish_immutable(modules, vec![PERA_FRAMEWORK_PACKAGE_ID]);
    let kind = TransactionKind::programmable(builder.finish());
...
async fn test_publish_not_a_package_dependency() {
    ...
    deps.push(PERA_SYSTEM_STATE_OBJECT_ID);
    builder.publish_immutable(modules, deps);
    let kind = TransactionKind::programmable(builder.finish());
    ...
    assert_eq!(
        PeraError::UserInputError {
            error: UserInputError::MoveObjectAsPackage {
                object_id: PERA_SYSTEM_STATE_OBJECT_ID
            }
        },
        failure,
    )
...
pub fn create_gas_objects(num: u32, owner: PeraAddress) -> Vec<Object> {
    let mut objects = vec![];
    for _ in 0..num {
        let gas_object_id = ObjectID::random();
