...
const IKA_SYSTEM_STATE_OBJECT_ID: address = @0x5;

/// The hardcoded ID for the singleton Clock Object.
const IKA_CLOCK_OBJECT_ID: address = @0x6;

/// The hardcoded ID for the singleton AuthenticatorState Object.
const IKA_AUTHENTICATOR_STATE_ID: address = @0x7;

/// The hardcoded ID for the singleton Random Object.
const IKA_RANDOM_ID: address = @0x8;

/// The hardcoded ID for the singleton DenyList.
const IKA_DENY_LIST_OBJECT_ID: address = @0x403;

/// The hardcoded ID for the Bridge Object.
const IKA_BRIDGE_ID: address = @0x9;
...
/// Create the `UID` for the singleton `IkaSystemState` object.
/// This should only be called once from `ika_system`.
fun ika_system_state(ctx: &TxContext): UID {
    assert!(ctx.sender() == @0x0, ENotSystemAddress);
    UID {
        id: ID { bytes: IKA_SYSTEM_STATE_OBJECT_ID },
    }
}
...
public(package) fun clock(): UID {
    UID {
        id: ID { bytes: IKA_CLOCK_OBJECT_ID },
    }
}
...
public(package) fun authenticator_state(): UID {
    UID {
        id: ID { bytes: IKA_AUTHENTICATOR_STATE_ID },
    }
}
...
public(package) fun randomness_state(): UID {
    UID {
        id: ID { bytes: IKA_RANDOM_ID },
    }
}

/// Create the `UID` for the singleton `DenyList` object.
/// This should only be called once from `deny_list`.
public(package) fun ika_deny_list_object_id(): UID {
    UID {
        id: ID { bytes: IKA_DENY_LIST_OBJECT_ID },
    }
}
...
/// Create a new object. Returns the `UID` that must be stored in a Ika object.
