...
use ika::event;
use ika::package::{Self, Publisher};
use ika::ika::IKA;
...
public struct TransferPolicy<phantom T> has key, store {
    id: UID,
    /// The Balance of the `TransferPolicy` which collects `IKA`.
...
/// Add some `IKA` to the balance of a `TransferPolicy`.
public fun add_to_balance<T, Rule: drop>(_: Rule, policy: &mut TransferPolicy<T>, coin: Coin<IKA>) {
