...
/// `list_with_purchase_cap` therefore performing a trade (isikang a
...
use ika::dynamic_object_field as dof;
use ika::event;
use ika::ika::IKA;
use ika::transfer_policy::{Self, TransferPolicy, TransferRequest};
...
entry fun default(ctx: &mut TxContext) {
    ...
    ika::transfer::share_object(kiosk);
}
...
public fun close_and_withdraw(self: Kiosk, cap: KioskOwnerCap, ctx: &mut TxContext): Coin<IKA> {
...
public fun profits_mut(self: &mut Kiosk, cap: &KioskOwnerCap): &mut Balance<IKA> {
    assert!(self.has_access(cap), ENotOwner);
    &mut self.profits
}
