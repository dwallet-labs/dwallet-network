...
/// Utility for mocking a multi-transaction Ika execution in a single Move procedure.
...
public fun return_to_address<T: key>(account: address, t: T) {
    ...
    ika::transfer::transfer_impl(t, account)
}
...
public fun return_immutable<T: key>(t: T) {
    ...
    ika::transfer::freeze_object_impl(t)
}
...
public fun return_shared<T: key>(t: T) {
    ...
    ika::transfer::share_object_impl(t)
}
...
public fun most_recent_receiving_ticket<T: key>(owner: &ID): ika::transfer::Receiving<T> {
...
public fun receiving_ticket_by_id<T: key>(object_id: ID): ika::transfer::Receiving<T> {
    let version = allocate_receiving_ticket_for_object<T>(object_id);
    ika::transfer::make_receiver(object_id, version)
}
...
/// test scenario.
public fun return_receiving_ticket<T: key>(ticket: ika::transfer::Receiving<T>) {
    let id = ika::transfer::receiving_id(&ticket);
