...
fun add_per_type_config(
    ...
    ika::event::emit(PerTypeConfigCreated { key, config_id });
}
...
public struct PerTypeList has key, store {
    ...
    /// For example with `ika::coin::Coin`: If addresses A and B are banned from using
...
fun create(ctx: &mut TxContext) {
    ...
    let deny_list_object = DenyList {
        id: object::ika_deny_list_object_id(),
        lists,
    };
