...
#[test_only]
module ika::kiosk_marketplace_ext {
    use ika::ika::IKA;
    use ika::coin::Coin;
    use ika::kiosk_extension as ext;
    use ika::kiosk::{Self, KioskOwnerCap, Kiosk, PurchaseCap};
    use ika::transfer_policy::{Self as policy, TransferPolicy, TransferRequest};

    /// Trying to access an owner-only action.
    const ENotOwner: u64 = 0;
    /// Trying to purchase an item with an incorrect amount of IKA.
    ...
    /// Collection bidding: the Kiosk Owner offers a bid (in IKA) for an item of type `T`.
    ///
    /// There can be only one bid per type.
    public fun bid<Market, T: key + store>(
        kiosk: &mut Kiosk, cap: &KioskOwnerCap, bid: Coin<IKA>
    ) {
    ...
    public fun accept_bid<Market, T: key + store>(
        ...
        let bid: Coin<IKA> = ext::storage_mut(Ext<Market> {}, destination).remove(Bid<T> {});
...
#[test_only]
module ika::kiosk_extensions_tests {
    use ika::kiosk_test_utils::{Self as test};
    use ika::kiosk_extension as ext;
    ...
    #[test, expected_failure(abort_code = ika::kiosk_extension::EExtensionNotAllowed)]
    ...
    #[test, expected_failure(abort_code = ika::kiosk_extension::EExtensionNotInstalled)]
