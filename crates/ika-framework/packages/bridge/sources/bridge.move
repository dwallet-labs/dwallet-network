...
module bridge::bridge {
    use ika::address;
    use ika::clock::Clock;
    use ika::coin::{Coin, TreasuryCap, CoinMetadata};
    use ika::event::emit;
    use ika::linked_table::{Self, LinkedTable};
    use ika::package::UpgradeCap;
    use ika::vec_map::{Self, VecMap};
    use ika::versioned::{Self, Versioned};
    ...
    use bridge::message::{
        ...
        UpdateBridgeLimit, AddTokenOnIka, ParsedTokenTransferMessage,
    ...
    const EInvariantIkaInitializedTokenTransferShouldNotBeClaimed: u64 = 10;
    ...
    // Record bridge message approvals in Ika, called by the bridge client
    ...
    public fun approve_token_transfer(
        ...
        // retrieve pending message if source chain is Ika, the initial message
        // must exist on chain
        if (message.source_chain() == inner.chain_id) {
            ...
            assert!(!record.claimed, EInvariantIkaInitializedTokenTransferShouldNotBeClaimed);
    ...
    public fun execute_system_message(
        ...
        } else if (message_type == message_types::add_tokens_on_ika()) {
            let payload = message.extract_add_tokens_on_ika();
            inner.execute_add_tokens_on_ika(payload);
        } else {
    ...
    fun execute_add_tokens_on_ika(inner: &mut BridgeInner, payload: AddTokenOnIka) {
    ...
    #[test_only]
    public fun test_execute_add_tokens_on_ika(bridge: &mut Bridge, payload: AddTokenOnIka) {
        let inner = load_inner_mut(bridge);
        inner.execute_add_tokens_on_ika(payload);
    }
