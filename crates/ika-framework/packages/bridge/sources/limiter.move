...
module bridge::limiter {
    ...
    // specify routes from Ethereum to Ika.
    fun initial_transfer_limits(): VecMap<BridgeRoute, u64> {
        ...
        transfer_limits.insert(
            chain_ids::get_route(chain_ids::eth_sepolia(), chain_ids::ika_custom()),
            MAX_TRANSFER_LIMIT
        );

        transfer_limits.insert(
            chain_ids::get_route(chain_ids::eth_custom(), chain_ids::ika_testnet()),
            MAX_TRANSFER_LIMIT
        );

        transfer_limits.insert(
            chain_ids::get_route(chain_ids::eth_custom(), chain_ids::ika_custom()),
            MAX_TRANSFER_LIMIT
        );
