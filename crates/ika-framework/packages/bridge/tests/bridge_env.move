...
module bridge::bridge_env {
    ...
    use bridge::message::{
        Self,
        BridgeMessage,
        create_add_tokens_on_ika_message,
    ...
    use ika::address;
    use ika::clock::Clock;
    use ika::coin::{Self, Coin, CoinMetadata, TreasuryCap};
    use ika::ecdsa_k1::{KeyPair, secp256k1_keypair_from_seed, secp256k1_sign};
    use ika::event;
    use ika::package::UpgradeCap;
    use ika::test_scenario::{Self, Scenario};
    use ika::test_utils::destroy;
    ...
    public fun create_env(chain_id: u8): BridgeEnv {
        ...
        let mut clock = ika::clock::create_for_testing(ctx);
    ...
    public fun register_committee(env: &mut BridgeEnv) {
        ...
        let mut system_state = test_scenario::take_shared<IkaSystemState>(
            scenario,
        );
    ...
    fun add_default_tokens(env: &mut BridgeEnv, sender: address) {
        ...
        let add_token_message = create_add_tokens_on_ika_message(
            env.chain_id,
            bridge.get_seq_num_for(message_types::add_tokens_on_ika()),
            false,
    ...
    const IKA_MESSAGE_PREFIX: vector<u8> = b"IKA_BRIDGE_MESSAGE";

    fun sign_message(
        env: &BridgeEnv,
        message: BridgeMessage,
    ): vector<vector<u8>> {
        let mut message_bytes = IKA_MESSAGE_PREFIX;
    ...
    public fun bridge_to_ika<Token>(
        env: &mut BridgeEnv,
        source_chain: u8,
        source_address: vector<u8>,
        target_address: address,
...
module bridge::test_token {
    ...
    use std::type_name;
    use ika::address;
    use ika::coin::{CoinMetadata, TreasuryCap, create_currency};
    use ika::hex;
    use ika::package::{UpgradeCap, test_publish};
    use ika::test_utils::create_one_time_witness;
