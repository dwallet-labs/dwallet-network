...
fun test_bridge_and_claim() {
    ...
    //
    // move from eth and transfer to ika account
    let transfer_id1 = env.bridge_to_ika<ETH>(
        source_chain,
        eth_address,
        ika_address,
    ...
    let transfer_id = env.bridge_to_ika<ETH>(
        source_chain,
        eth_address,
        ika_address,
        amount,
    );
    let token = env.claim_token<ETH>(ika_address, source_chain, transfer_id);
    env.send_token<ETH>(
        ika_address,
        source_chain,
        eth_address,
