...
module ika_system::validator_set {
    ...
    use ika::ika::IKA;
    use ika_system::validator::{Validator, staking_pool_id, ika_address};
    use ika_system::validator_cap::{Self, UnverifiedValidatorOperationCap, ValidatorOperationCap};
    use ika_system::staking_pool::{PoolTokenExchangeRate, StakedIka, pool_id, FungibleStakedIka, fungible_staked_ika_pool_id};
    use ika::priority_queue as pq;
    ...
    public(package) fun new(init_active_validators: vector<Validator>, ctx: &mut TxContext): ValidatorSet {
        ...
        while (i < num_validators) {
            ...
            staking_pool_mappings.add(staking_pool_id(validator), ika_address(validator));
            i = i + 1;
        };
    ...
    public(package) fun request_add_validator_candidate(
        ...
        let validator_address = ika_address(&validator);
        assert!(
    ...
    public(package) fun request_withdraw_stake(
        ...
        validator.request_withdraw_stake(staked_ika, ctx)
    }
    ...
    fun update_and_process_low_stake_departures(
        ...
        while (i > 0) {
            ...
            let validator_address = validator_ref.ika_address();
    ...
    /// Called by `ika_system` to derive reference gas price for the new epoch.
    ...
    public(package) fun is_active_validator_by_ika_address(
        self: &ValidatorSet,
        validator_address: address,
    ): bool {
    ...
    /// It differs from `is_active_validator_by_ika_address` in that the former checks
    ...
    fun find_validator(validators: &vector<Validator>, validator_address: address): Option<u64> {
        ...
        while (i < length) {
            let v = &validators[i];
            if (v.ika_address() == validator_address) {
    ...
    fun process_validator_departure(
        ...
        let new_epoch = ctx.epoch() + 1;
        let validator_address = validator.ika_address();
    ...
    fun process_pending_validators(
        ...
        while (!self.pending_active_validators.is_empty()) {
            ...
            event::emit(
                ValidatorJoinEvent {
                    epoch: new_epoch,
                    validator_address: validator.ika_address(),
                    staking_pool_id: staking_pool_id(&validator),
                }
            );
    ...
    fun compute_slashed_validators(
        ...
        while (!validator_report_records.is_empty()) {
            ...
            assert!(
                is_active_validator_by_ika_address(self, validator_address),
    ...
    fun emit_validator_epoch_events(
        ...
        while (i < num_validators) {
            let v = &vs[i];
            let validator_address = v.ika_address();
    ...
    public(package) fun active_validator_addresses(self: &ValidatorSet): vector<address> {
        ...
        while (i < length) {
            let validator_address = vs[i].ika_address();
