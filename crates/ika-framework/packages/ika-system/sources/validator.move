...
module ika_system::validator {
    ...
    use ika_system::staking_pool::{Self, PoolTokenExchangeRate, StakedIka, StakingPool, FungibleStakedIka};
    ...
    public(package) fun convert_to_fungible_staked_ika(
        ...
        event::emit(
            ConvertingToFungibleStakedIkaEvent {
    ...
    public(package) fun redeem_fungible_staked_ika(
        ...
        event::emit(
            RedeemingFungibleStakedIkaEvent {
                pool_id: self.staking_pool_id(),
    ...
    public(package) fun request_add_stake_at_genesis(
        ...
        let staked_ika = self.staking_pool.request_add_stake(
            stake,
            0, // epoch 0 -- genesis
            ctx
        );

        transfer::public_transfer(staked_ika, staker_address);
    ...
    public(package) fun request_withdraw_stake(
        ...
        let stake_activation_epoch = staked_ika.stake_activation_epoch();
        let withdrawn_stake = self.staking_pool.request_withdraw_stake(staked_ika, ctx);
    ...
    public(package) fun request_set_gas_price(
        ...
        assert!(validator_address == self.metadata.ika_address, EInvalidCap);
    ...
    public fun ika_address(self: &Validator): address {
    ...
    public(package) fun new_unverified_validator_operation_cap_and_transfer(self: &mut Validator, ctx: &mut TxContext) {
        ...
        assert!(address == self.metadata.ika_address, ENewCapNotCreatedByValidatorItself);
    ...
    fun new_from_metadata(
        ...
        let ika_address = metadata.ika_address;

        let staking_pool = staking_pool::new(ctx);

        let operation_cap_id = validator_cap::new_unverified_validator_operation_cap_and_transfer(ika_address, ctx);
        Validator {
            metadata,
    ...
    // Note: `proof_of_possession` MUST be a valid signature using ika_address and
    ...
    public(package) fun new_for_testing(
        ...
        if (initial_stake_option.is_some()) {
            request_add_stake_at_genesis(
                ...
                ika_address, // give the stake to the validator
                ctx
            );
        };
