...
module ika_system::storage_fund {
    ...
    public struct StorageFund has store {
        total_object_storage_rebates: Balance<IKA>,
        non_refundable_balance: Balance<IKA>,
    }

    /// Called by `ika_system` at genesis time.
    public(package) fun new(initial_fund: Balance<IKA>) : StorageFund {
        StorageFund {
    ...
    /// Called by `ika_system` at epoch change times to process the inflows and outflows of storage fund.
