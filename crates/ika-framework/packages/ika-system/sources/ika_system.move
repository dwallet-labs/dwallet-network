...
/// `IkaSystemState` is a thin wrapper around `IkaSystemStateInner` that provides a versioned interface.
/// The `IkaSystemState` object has a fixed ID 0x5, and the `IkaSystemStateInner` object is stored as a dynamic field.
/// There are a few different ways to upgrade the `IkaSystemStateInner` type:
///
/// The simplest and one that doesn't involve a real upgrade is to just add dynamic fields to the `extra_fields` field
/// of `IkaSystemStateInner` or any of its sub type. This is useful when we are in a rush, or making a small change,
/// or still experimenting a new field.
///
/// To properly upgrade the `IkaSystemStateInner` type, we need to ship a new framework that does the following:
/// 1. Define a new `IkaSystemStateInner`type (e.g. `IkaSystemStateInnerV2`).
/// 2. Define a data migration function that migrates the old `IkaSystemStateInner` to the new one (i.e. IkaSystemStateInnerV2).
/// 3. Replace all uses of `IkaSystemStateInner` with `IkaSystemStateInnerV2` in both ika_system.move and ika_system_state_inner.move,
///    with the exception of the `ika_system_state_inner::create` function, which should always return the genesis type.
...
/// 1. Define a new `IkaSystemStateInner` struct type that matches the new Move type, and implement the IkaSystemStateTrait.
/// 2. Update the `IkaSystemState` struct to include the new version as a new enum variant.
...
module ika_system::ika_system {
    ...
    use ika::coin::Coin;
    use ika_system::staking_pool::{StakedIka, FungibleStakedIka};
    use ika::ika::IKA;
    use ika::table::Table;
    use ika_system::validator::Validator;
    use ika_system::validator_cap::UnverifiedValidatorOperationCap;
    use ika_system::ika_system_state_inner::{Self, SystemParameters, IkaSystemStateInner, IkaSystemStateInnerV2};
    use ika_system::stake_subsidy::StakeSubsidy;
    use ika_system::staking_pool::PoolTokenExchangeRate;
    use ika::dynamic_field;
    use ika::vec_map::VecMap;

    #[test_only] use ika::balance;
    #[test_only] use ika_system::validator_set::ValidatorSet;
    #[test_only] use ika::vec_set::VecSet;
    ...
    /// Note: `proof_of_possession` MUST be a valid signature using ika_address and protocol_pubkey_bytes.
    ...
    /// At the end of the epoch, the `validator` object will be returned to the ika_address
    /// of the validator.
    ...
    public entry fun request_add_stake(
        ...
        let staked_ika = request_add_stake_non_entry(wrapper, stake, validator_address, ctx);
    ...
    public entry fun request_add_stake_mul_coin(
        ...
        let self = load_system_state_mut(wrapper);
        let staked_ika = self.request_add_stake_mul_coin(stakes, stake_amount, validator_address, ctx);
        transfer::public_transfer(staked_ika, ctx.sender());
    }
    ...
    public entry fun request_withdraw_stake(
        ...
        let withdrawn_stake = request_withdraw_stake_non_entry(wrapper, staked_ika, ctx);
    ...
    public fun redeem_fungible_staked_ika(
        ...
        let self = load_system_state_mut(wrapper);
        self.redeem_fungible_staked_ika(fungible_staked_ika, ctx)
    }
    ...
    public fun request_withdraw_stake_non_entry(
        ...
        let self = load_system_state_mut(wrapper);
        self.request_withdraw_stake(staked_ika, ctx)
    }
    ...
    public fun validator_address_by_pool_id(wrapper: &mut IkaSystemState, pool_id: &ID): address {
        let self = load_system_state_mut(wrapper);
    ...
    fun load_system_state(self: &mut IkaSystemState): &IkaSystemStateInnerV2 {
    ...
    fun load_inner_maybe_upgrade(self: &mut IkaSystemState): &mut IkaSystemStateInnerV2 {
        if (self.version == 1) {
          let v1: IkaSystemStateInner = dynamic_field::remove(&mut self.id, self.version);
          let v2 = v1.v1_to_v2();
          self.version = 2;
          dynamic_field::add(&mut self.id, self.version, v2);
        };

        let inner: &mut IkaSystemStateInnerV2 = dynamic_field::borrow_mut(
            &mut self.id,
            self.version
        );
    ...
    fun validator_voting_powers(wrapper: &mut IkaSystemState): VecMap<address, u64> {
        let self = load_system_state(wrapper);
        ika_system_state_inner::active_validator_voting_powers(self)
    }

    #[test_only]
    public fun validator_voting_powers_for_testing(wrapper: &mut IkaSystemState): VecMap<address, u64> {
    ...
    public fun epoch(wrapper: &mut IkaSystemState): u64 {
        let self = load_system_state(wrapper);
        self.epoch()
    }
    ...
    public fun epoch_start_timestamp_ms(wrapper: &mut IkaSystemState): u64 {
        let self = load_system_state(wrapper);
    ...
    public fun validator_stake_amount(wrapper: &mut IkaSystemState, validator_addr: address): u64 {
        let self = load_system_state(wrapper);
    ...
    /// Aborts if `validator_addr` is not an active validator.
    public fun validator_staking_pool_id(wrapper: &mut IkaSystemState, validator_addr: address): ID {
        let self = load_system_state(wrapper);
    ...
    public fun validator_staking_pool_mappings(wrapper: &mut IkaSystemState): &Table<ID, address> {
        let self = load_system_state(wrapper);
    ...
    public fun get_reporters_of(wrapper: &mut IkaSystemState, addr: address): VecSet<address> {
        let self = load_system_state(wrapper);
    ...
    public fun validators(wrapper: &mut IkaSystemState): &ValidatorSet {
        let self = load_system_state(wrapper);
        self.validators()
    }
    ...
    public fun active_validator_by_address(self: &mut IkaSystemState, validator_address: address): &Validator {
        validators(self).get_active_validator_ref(validator_address)
    }

    #[test_only]
    /// Return the currently pending validator by address
    public fun pending_validator_by_address(self: &mut IkaSystemState, validator_address: address): &Validator {
        validators(self).get_pending_validator_ref(validator_address)
    }

    #[test_only]
    /// Return the currently candidate validator by address
    public fun candidate_validator_by_address(self: &mut IkaSystemState, validator_address: address): &Validator {
        validators(self).get_candidate_validator_ref(validator_address)
    }

    #[test_only]
    public fun set_epoch_for_testing(wrapper: &mut IkaSystemState, epoch_num: u64) {
        let self = load_system_state_mut(wrapper);
    ...
    #[test_only]
    public fun get_storage_fund_total_balance(wrapper: &mut IkaSystemState): u64 {
        let self = load_system_state(wrapper);
        self.get_storage_fund_total_balance()
    }

    #[test_only]
    public fun get_storage_fund_object_rebates(wrapper: &mut IkaSystemState): u64 {
        let self = load_system_state(wrapper);
        self.get_storage_fund_object_rebates()
    }

    #[test_only]
    public fun get_stake_subsidy_distribution_counter(wrapper: &mut IkaSystemState): u64 {
        let self = load_system_state(wrapper);
        self.get_stake_subsidy_distribution_counter()
    }

    #[test_only]
    public fun set_stake_subsidy_distribution_counter(wrapper: &mut IkaSystemState, counter: u64) {
        let self = load_system_state_mut(wrapper);
        self.set_stake_subsidy_distribution_counter(counter)
    }

    #[test_only]
    public fun inner_mut_for_testing(wrapper: &mut IkaSystemState): &mut IkaSystemStateInnerV2 {
