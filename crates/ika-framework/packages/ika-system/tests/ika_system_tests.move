...
// This file contains tests testing functionalities in `ika_system` that are not
// already tested by the other more themed tests such as `stake_tests` or
// `rewards_distribution_tests`.

#[test_only]
module ika_system::ika_system_tests {
    use ika::test_scenario::{Self, Scenario};
    use ika::ika::IKA;
    use ika::coin::Self;
    use ika_system::governance_test_utils::{add_validator_full_flow, advance_epoch, remove_validator, set_up_ika_system_state, create_ika_system_state_for_testing, stake_with, unstake};
    ...
    fun test_validator_ops_by_stakee_ok() {
        let mut scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        set_up_ika_system_state(vector[@0x1, @0x2]);
    ...
    #[test]
    #[expected_failure(abort_code = ::ika_system::validator_set::EInvalidCap)]
    ...
    #[test]
    #[expected_failure(abort_code = ika_system_state_inner::ENotValidator)]
    ...
    #[test]
    #[expected_failure(abort_code = ika_system_state_inner::ECannotReportOneself)]
    ...
    #[test]
    #[expected_failure(abort_code = ika_system_state_inner::EReportRecordNotFound)]
    ...
    fun test_validator_address_by_pool_id() {
        ...
        set_up_ika_system_state(vector[@0x1, @0x2, @0x3, @0x4]);
        scenario.next_tx(@0x1);
    ...
    fun update_candidate(
        scenario: &mut Scenario,
        system_state: &mut IkaSystemState,
        name: vector<u8>,
    ...
    // Note: `pop` MUST be a valid signature using ika_address and protocol_pubkey_bytes.
    ...
    fun test_active_validator_update_metadata() {
        ...
        // Set up IkaSystemState with an active validator
        ...
        let validator = validator::new_for_testing(
            ...
            b"/ip4/127.0.0.1/udp/80",
            b"/ip4/127.0.0.1/udp/80",
            option::some(balance::create_for_testing<IKA>(100_000_000_000)),
            1,
            0,
            true,
            ctx
    ...
    fun test_add_validator_candidate_failure_duplicate_with_active() {
        ...
        create_ika_system_state_for_testing(vector[validator], 1000, 0, ctx);
    ...
    fun test_withdraw_inactive_stake() {
...

        {
            ...
            assert!(staking_pool.ika_balance() == 100 * 1_000_000_000, 0);
    ...
    #[test]
    fun test_convert_to_fungible_staked_ika_and_redeem() {
        let mut scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        // Epoch duration is set to be 42 here.
        set_up_ika_system_state(vector[@0x1, @0x2]);
        ...
        let staked_ika = system_state.request_add_stake_non_entry(
            coin::mint_for_testing(100_000_000_000, scenario.ctx()),
            @0x1,
            scenario.ctx()
        );

        assert!(staked_ika.amount() == 100_000_000_000, 0);

        test_scenario::return_shared(system_state);
        advance_epoch(scenario);

        let mut system_state = scenario.take_shared<IkaSystemState>();
        let fungible_staked_ika = system_state.convert_to_fungible_staked_ika(
            staked_ika,
            scenario.ctx()
        );

        assert!(fungible_staked_ika.value() == 100_000_000_000, 0);

        let ika = system_state.redeem_fungible_staked_ika(
            fungible_staked_ika,
            scenario.ctx()
        );

        assert!(ika.value() == 100_000_000_000, 0);
