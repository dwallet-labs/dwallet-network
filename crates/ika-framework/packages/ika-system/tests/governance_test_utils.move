...
#[test_only]
module ika_system::governance_test_utils {
    use ika::address;
    use ika::balance;
    use ika::ika::IKA;
    use ika::coin::{Self, Coin};
    use ika_system::staking_pool::{StakedIka, StakingPool};
    ...
    public fun create_validator_for_testing(
        addr: address, init_stake_amount_in_ika: u64, ctx: &mut TxContext
    ): Validator {
        let validator = validator::new_for_testing(
            ...
            b"/ip4/127.0.0.1/udp/80",
            b"/ip4/127.0.0.1/udp/80",
            option::some(balance::create_for_testing<IKA>(init_stake_amount_in_ika * NIKA_PER_IKA)),
            1,
            0,
            true,
            ctx
    ...
    public fun create_ika_system_state_for_testing(
        validators: vector<Validator>, ika_supply_amount: u64, storage_fund_amount: u64, ctx: &mut TxContext
    ) {
        ...
        let stake_subsidy = stake_subsidy::create(
            balance::create_for_testing<IKA>(ika_supply_amount * NIKA_PER_IKA), // ika_supply
        ...
        ika_system::create(
            object::new(ctx), // it doesn't matter what ID ika system state has in tests
            validators,
            balance::create_for_testing<IKA>(storage_fund_amount * NIKA_PER_IKA), // storage_fund
            1,   // protocol version
    ...
    public fun set_up_ika_system_state(mut addrs: vector<address>) {
    ...
    public fun advance_epoch_with_reward_amounts(
        ...
        let storage_rebate = advance_epoch_with_reward_amounts_return_rebate(storage_charge * NIKA_PER_IKA, computation_charge * NIKA_PER_IKA, 0, 0, scenario);
    ...
    public fun advance_epoch_with_reward_amounts_and_slashing_rates(
        ...
        let storage_rebate = system_state.advance_epoch_for_testing(
            new_epoch, 1, storage_charge * NIKA_PER_IKA, computation_charge * NIKA_PER_IKA, 0, 0, 0, reward_slashing_rate, 0, ctx
        );
        test_utils::destroy(storage_rebate);
        test_scenario::return_shared(system_state);
        scenario.next_epoch(@0x0);
    ...
    public fun stake_with(
        ...
        system_state.request_add_stake(coin::mint_for_testing(amount * NIKA_PER_IKA, ctx), validator, ctx);
        test_scenario::return_shared(system_state);
    }

    public fun unstake(
        staker: address, staked_ika_idx: u64, scenario: &mut Scenario
    ) {
        scenario.next_tx(staker);
        let stake_ika_ids = scenario.ids_for_sender<StakedIka>();
        let staked_ika = scenario.take_from_sender_by_id(stake_ika_ids[staked_ika_idx]);
        let mut system_state = scenario.take_shared<IkaSystemState>();

        let ctx = scenario.ctx();
        system_state.request_withdraw_stake(staked_ika, ctx);
        test_scenario::return_shared(system_state);
    }
    ...
    public fun add_validator_full_flow(validator: address, name: vector<u8>, net_addr: vector<u8>, init_stake_amount: u64, pubkey: vector<u8>, pop: vector<u8>, scenario: &mut Scenario) {
        ...
        system_state.request_add_stake(coin::mint_for_testing<IKA>(init_stake_amount * NIKA_PER_IKA, ctx), validator, ctx);
    ...
    /// Return the rewards for the validator at `addr` in terms of IKA.
    public fun stake_plus_current_rewards_for_validator(addr: address, system_state: &mut IkaSystemState, scenario: &mut Scenario): u64 {
    ...
    public fun stake_plus_current_rewards(addr: address, staking_pool: &StakingPool, scenario: &mut Scenario): u64 {
        ...
        let mut stake_ids = scenario.ids_for_sender<StakedIka>();
        let current_epoch = scenario.ctx().epoch();

        while (!stake_ids.is_empty()) {
            let staked_ika_id = stake_ids.pop_back();
            let staked_ika = scenario.take_from_sender_by_id<StakedIka>(staked_ika_id);
            sum = sum + staking_pool.calculate_rewards(&staked_ika, current_epoch);
            scenario.return_to_sender(staked_ika);
        };
        sum
    }

    public fun total_ika_balance(addr: address, scenario: &mut Scenario): u64 {
        let mut sum = 0;
        scenario.next_tx(addr);
        let coin_ids = scenario.ids_for_sender<Coin<IKA>>();
        let mut i = 0;
        while (i < coin_ids.length()) {
            let coin = scenario.take_from_sender_by_id<Coin<IKA>>(coin_ids[i]);
            sum = sum + coin.value();
