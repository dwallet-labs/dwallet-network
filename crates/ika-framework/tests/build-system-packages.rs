...
fn build_system_packages() {
    ...
    let ika_system_path = packages_path.join("ika-system");
    let ika_framework_path = packages_path.join("ika-framework");
...
fn build_packages_with_move_config(
    ...
    let ika_system = system_pkg.get_ika_system_modules();
    let ika_framework = framework_pkg.get_ika_framework_modules();
    ...
    let ika_system_members =
        serialize_modules_to_file(ika_system, &compiled_packages_dir.join(system_dir)).unwrap();
    let ika_framework_members =
        serialize_modules_to_file(ika_framework, &compiled_packages_dir.join(framework_dir))
            .unwrap();
