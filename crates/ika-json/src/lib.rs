...
use ika_types::base_types::{
    is_primitive_type_tag, move_ascii_str_layout, move_utf8_str_layout, ObjectID, IkaAddress,
...
use ika_types::id::{self, ID, RESOLVED_IKA_ID};
use ika_types::move_package::MovePackage;
use ika_types::object::bounded_visitor::BoundedVisitor;
use ika_types::transfer::RESOLVED_RECEIVING_STRUCT;
use ika_types::MOVE_STDLIB_ADDRESS;
...
pub enum IkaJsonValueErrorKind {
...
#[derive(Debug)]
pub struct IkaJsonValueError {
    kind: IkaJsonValueErrorKind,
    val: JsonValue,
}

impl IkaJsonValueError {
    pub fn new(val: &JsonValue, kind: IkaJsonValueErrorKind) -> Self {
        Self {
            kind,
            val: val.clone(),
        }
...
impl std::error::Error for IkaJsonValueError {}

impl fmt::Display for IkaJsonValueError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let err_str = match self.kind {
            IkaJsonValueErrorKind::ValueTypeNotAllowed => {
                format!("JSON value type {} not allowed.", self.val)
            }
            IkaJsonValueErrorKind::ArrayNotHomogeneous => {
...
pub struct IkaJsonValue(JsonValue);
impl IkaJsonValue {
    pub fn new(json_value: JsonValue) -> Result<IkaJsonValue, anyhow::Error> {
    ...
    pub fn from_object_id(id: ObjectID) -> IkaJsonValue {
    ...
    pub fn to_ika_address(&self) -> anyhow::Result<IkaAddress> {
        json_value_to_ika_address(&self.0)
    }
    ...
    pub fn to_move_value(
        ...
        Ok(match (val, ty) {
                ...
                if struct_layout.type_ == ID::type_() =>
            {
                ...
                let addr = IkaAddress::from_str(s)?;
            ...
            (v, MoveTypeLayout::Address) => {
                let addr = json_value_to_ika_address(v)?;
                R::MoveValue::Address(addr.into())
            }
...
impl Debug for IkaJsonValue {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", self.0)
    }
}

fn json_value_to_ika_address(value: &JsonValue) -> anyhow::Result<IkaAddress> {
    match value {
        JsonValue::String(s) => {
        ...
        JsonValue::Array(bytes) => {
            ...
            let bytes = value_to_byte_array(bytes)
                .ok_or_else(|| anyhow!("Invalid input: Cannot parse input into IkaAddress."))?;
            Ok(IkaAddress::try_from(bytes)?)
        }
...
fn move_value_to_json(move_value: &MoveValue) -> Option<JsonValue> {
    Some(match move_value {
        ...
        MoveValue::Signer(v) | MoveValue::Address(v) => json!(IkaAddress::from(*v).to_string()),
...
impl FromStr for IkaJsonValue {
    ...
    fn from_str(s: &str) -> Result<Self, anyhow::Error> {
        ...
        IkaJsonValue::new(serde_json::from_str(s).unwrap_or_else(|_| try_escape_array(s)))
    }
}
...
pub fn check_valid_homogeneous(val: &JsonValue) -> Result<(), IkaJsonValueError> {
...
fn check_valid_homogeneous_rec(curr_q: &mut VecDeque<&JsonValue>) -> Result<(), IkaJsonValueError> {
    if curr_q.is_empty() {
        // Nothing to do
        return Ok(());
    }
    ...
    while let Some(v) = curr_q.pop_front() {
        let curr = match v {
            ...
            // Not valid
            _ => {
                return Err(IkaJsonValueError::new(
                    v,
                    IkaJsonValueErrorKind::ValueTypeNotAllowed,
                ))
            }
...
pub fn primitive_type(
    ...
    Some(match param {
        ...
        SignatureToken::Datatype(struct_handle_idx) => {
            ...
            } else if resolved_struct == RESOLVED_IKA_ID {
...
fn resolve_object_vec_arg(idx: usize, arg: &IkaJsonValue) -> Result<Vec<ObjectID>, anyhow::Error> {
    // Every elem has to be a string convertible to a ObjectID
...
pub fn resolve_move_function_args(
    ...
    function: Identifier,
    type_args: &[TypeTag],
    combined_args_json: Vec<IkaJsonValue>,
...
macro_rules! call_args {
        ($($value:expr),*) => {
        Ok::<_, anyhow::Error>(vec![$(ika_json::call_arg!($value)?,)*])
    };
    }
...
macro_rules! call_arg {
    ($value:expr) => {{
        ...
        trait IkaJsonArg {
            fn to_ika_json(&self) -> anyhow::Result<IkaJsonValue>;
        }
        // TODO: anyway to condense this?
        impl IkaJsonArg for &str {
            fn to_ika_json(&self) -> anyhow::Result<IkaJsonValue> {
                IkaJsonValue::from_str(self)
            }
        }
        impl IkaJsonArg for String {
            fn to_ika_json(&self) -> anyhow::Result<IkaJsonValue> {
                IkaJsonValue::from_str(&self)
            }
        }
        impl IkaJsonArg for ika_types::base_types::ObjectID {
            fn to_ika_json(&self) -> anyhow::Result<IkaJsonValue> {
                IkaJsonValue::from_str(&self.to_string())
            }
        }
        impl IkaJsonArg for ika_types::base_types::IkaAddress {
            fn to_ika_json(&self) -> anyhow::Result<IkaJsonValue> {
                IkaJsonValue::from_str(&self.to_string())
            }
        }
        impl IkaJsonArg for u64 {
            fn to_ika_json(&self) -> anyhow::Result<IkaJsonValue> {
                IkaJsonValue::from_bcs_bytes(
                    Some(&ika_json::MoveTypeLayout::U64),
                    &bcs::to_bytes(self)?,
                )
            }
        }
        impl IkaJsonArg for Vec<u8> {
            fn to_ika_json(&self) -> anyhow::Result<IkaJsonValue> {
                IkaJsonValue::from_bcs_bytes(None, &self)
            }
        }
        impl IkaJsonArg for &[u8] {
            fn to_ika_json(&self) -> anyhow::Result<IkaJsonValue> {
                IkaJsonValue::from_bcs_bytes(None, self)
            }
        }
...
macro_rules! type_args {
    ($($value:expr), *) => {{
        use ika_json_rpc_types::IkaTypeTag;
        use ika_types::TypeTag;
        trait IkaJsonTypeArg {
            fn to_ika_json(&self) -> anyhow::Result<IkaTypeTag>;
        }
        impl <T: core::fmt::Display> IkaJsonTypeArg for T {
            fn to_ika_json(&self) -> anyhow::Result<IkaTypeTag> {
                Ok(ika_types::parse_ika_type_tag(&self.to_string())?.into())
            }
        }
        Ok::<_, anyhow::Error>(vec![$($value.to_ika_json()?,)*])
    }};
    }
