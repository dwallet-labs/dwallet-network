...
use ika_types::base_types::{
    ObjectID, IkaAddress, TransactionDigest, STD_ASCII_MODULE_NAME, STD_ASCII_STRUCT_NAME,
    STD_OPTION_MODULE_NAME, STD_OPTION_STRUCT_NAME,
};
use ika_types::dynamic_field::derive_dynamic_field_id;
use ika_types::gas_coin::GasCoin;
use ika_types::object::Object;
use ika_types::{parse_ika_type_tag, MOVE_STDLIB_ADDRESS};

use crate::ResolvedCallArg;

use super::{check_valid_homogeneous, HEX_PREFIX};
use super::{resolve_move_function_args, IkaJsonValue};
...
fn test_json_struct_homogeneous() {
    ...
    assert!(IkaJsonValue::new(positive).is_ok());

    let negative = json!({"inner_vec":[1, 2, 3, true, 5, 6, 7]});
    assert!(IkaJsonValue::new(negative).is_err());
}

#[test]
fn test_json_is_not_valid_ika_json() {
    let checks = vec![
        // Not homogeneous
        (json!([1, 2, 3, true, 5, 6, 7])),
        // Not homogeneous
    ...
    // Driver
    for arg in checks {
        assert!(IkaJsonValue::new(arg).is_err());
    }
}

#[test]
fn test_json_is_valid_ika_json() {
    let checks = vec![
        // Homogeneous
        (json!([1, 2, 3, 4, 5, 6, 7])),
        // String allowed
    ...
    // Driver
    for arg in checks {
        assert!(IkaJsonValue::new(arg).is_ok());
    }
}
...
fn test_basic_args_linter_top_level() {
    ...
    let recipient_addr = IkaAddress::random_for_testing_only();
    ...
    .into_iter()
    .map(|q| IkaJsonValue::new(q.clone()).unwrap())
    .collect();
...
fn test_convert_address_from_bcs() {
    ...
    let value = IkaJsonValue::from_bcs_bytes(Some(&MoveTypeLayout::Signer), &bcs_bytes).unwrap();
...
fn test_convert_number_from_bcs() {
    ...
    let value = IkaJsonValue::from_bcs_bytes(Some(&MoveTypeLayout::U32), &bcs_bytes).unwrap();
...
fn test_no_address_zero_trimming() {
    ...
    .unwrap();
    let value = IkaJsonValue::from_bcs_bytes(Some(&MoveTypeLayout::Address), &bcs_bytes).unwrap();
    assert_eq!(
        "0x0000000000000000000000000000011111111111111111111111111111111111",
        value.0.as_str().unwrap()
    );
...
fn test_convert_number_array_from_bcs() {
    ...
    let value = IkaJsonValue::from_bcs_bytes(
...
fn test_from_str() {
    // test number
    let test = IkaJsonValue::from_str("10000").unwrap();
    assert!(test.0.is_number());
    // Test array
    let test = IkaJsonValue::from_str("[10,10,10,10]").unwrap();
    ...
    // test bool
    let test = IkaJsonValue::from_str("true").unwrap();
    assert!(test.0.is_boolean());

    // test id without quotes
    let object_id = ObjectID::random().to_hex_uncompressed();
    let test = IkaJsonValue::from_str(&object_id).unwrap();
    assert!(test.0.is_string());
    assert_eq!(object_id, test.0.as_str().unwrap());

    // test id with quotes
    let test = IkaJsonValue::from_str(&format!("\"{}\"", &object_id)).unwrap();
    assert!(test.0.is_string());
    assert_eq!(object_id, test.0.as_str().unwrap());

    // test string without quotes
    let test = IkaJsonValue::from_str("Some string").unwrap();
    assert!(test.0.is_string());
    ...
    let test = IkaJsonValue::from_object_id(
...
#[test]
fn test_ika_call_arg_string_type() {
    ...
    let v = IkaJsonValue::from_bcs_bytes(string_layout.as_ref(), &arg1).unwrap();

    assert_eq!(json! {"Some String"}, v.to_json_value());
}

#[test]
fn test_ika_call_arg_option_type() {
    ...
    let v = IkaJsonValue::from_bcs_bytes(Some(option_layout).as_ref(), &arg1).unwrap();
    ...
    let s = IkaJsonValue::from_str("[test, test2]").unwrap();
    println!("{s:?}");
}
...
fn test_convert_struct() {
    ...
    let ika_json = IkaJsonValue::new(value).unwrap();

    println!("JS: {:#?}", ika_json);

    let bcs = ika_json.to_bcs_bytes(&layout).unwrap();
...
fn test_convert_string_vec() {
    ...
    let bcs2 = ika_json.to_bcs_bytes(&layout).unwrap();
...
fn test_string_vec_df_name_child_id_eq() {
    ...
    let ika_json = IkaJsonValue::new(name).unwrap();
    let bcs2 = ika_json.to_bcs_bytes(&layout).unwrap();
