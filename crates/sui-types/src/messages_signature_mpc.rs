...
pub fn decrypt_signature_decentralized_party_sign(
    ...
    let signatures_s = decryption_shares.into_iter().map(|(partial_signature_decryption_shares, masked_nonce_decryption_shares)| {
            SignSignatureMPCDecentralizedParty::decrypt_signature(
        ...
        .collect::<TwopcMPCResult<Vec<secp256k1::Scalar>>>()?;

    public_nonce_encrypted_partial_signature_and_proofs.into_iter().zip(signatures_s.into_iter()).map(|(public_nonce_encrypted_partial_signature_and_proof, signature_s)| {
