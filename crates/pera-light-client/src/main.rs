...
use pera_rest_api::{CheckpointData, Client};
use pera_types::{
    base_types::ObjectID,
    committee::Committee,
...
use pera_package_resolver::Result as ResolverResult;
use pera_package_resolver::{Package, PackageStore, Resolver};
...
/// A light client for the Pera blockchain
#[derive(Parser, Debug)]
...
async fn get_verified_effects_and_events(
    ...
    let pera_mainnet: Arc<pera_sdk::PeraClient> = Arc::new(
        PeraClientBuilder::default()
            .build(config.full_node_url.as_str())
            .await
            .unwrap(),
    );
    let read_api = pera_mainnet.read_api();
    ...
    match args.command {
        Some(SCommands::Transaction { tid }) => {
            ...
            for event in events.as_ref().unwrap().data.iter() {
                ...
                let json_val =
                    PeraJsonValue::from_bcs_bytes(Some(&type_layout), &event.contents).unwrap();
        ...
        Some(SCommands::Object { oid }) => {
            ...
            if let Data::Move(move_object) = &object.data {
                ...
                let json_val =
                    PeraJsonValue::from_bcs_bytes(Some(&type_layout), move_object.contents())
                        .unwrap();
