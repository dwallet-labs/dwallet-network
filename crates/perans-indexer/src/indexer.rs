...
use pera_json_rpc::name_service::{Domain, NameRecord, SubDomainRegistration};
use pera_types::{
...
pub struct PeransIndexer {
    registry_table_id: PeraAddress,
    subdomain_wrapper_type: StructTag,
    name_record_type: StructTag,
}

impl std::default::Default for PeransIndexer {
    fn default() -> Self {
        Self::new(
...
impl PeransIndexer {
    ...
    pub fn new(registry_address: String, wrapper_type: String, record_type: String) -> Self {
        let registry_table_id = PeraAddress::from_str(&registry_address).unwrap();
    ...
    pub fn process_checkpoint(&self, data: &CheckpointData) -> (Vec<VerifiedDomain>, Vec<String>) {
        let mut checkpoint = PeransIndexerCheckpoint::new(data.checkpoint_summary.sequence_number);
...
pub struct PeransIndexerCheckpoint {
...
impl PeransIndexerCheckpoint {
    ...
    pub fn parse_record_changes(&mut self, config: &PeransIndexer, objects: &[Object]) {
...
/// Allows us to format a PeraNS specific query for updating the DB entries
