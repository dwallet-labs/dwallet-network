...
use perans_indexer::{
    get_connection_pool,
    indexer::{format_update_field_query, format_update_subdomain_wrapper_query, PeransIndexer},
...
struct PeransIndexerWorker {
    pg_pool: PgConnectionPool,
    indexer: PeransIndexer,
}

impl PeransIndexerWorker {
...
#[async_trait]
impl Worker for PeransIndexerWorker {
...
async fn main() -> Result<()> {
    ...
    let indexer_setup =
        ...
        {
            PeransIndexer::new(registry_id, subdomain_wrapper_type, name_record_type)
        } else {
            PeransIndexer::default()
        };
