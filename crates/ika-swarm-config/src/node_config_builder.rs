...
use crate::validator_initialization_config::{ValidatorInitializationConfig, ValidatorInitializationConfigBuilder};
use fastcrypto::encoding::{Encoding, Hex};
use fastcrypto::traits::KeyPair;
use std::path::PathBuf;
use sui_types::base_types::ObjectID;
use ika_config::node::{default_end_of_epoch_broadcast_channel_capacity, AuthorityKeyPairWithPath, AuthorityOverloadConfig, KeyPairWithPath, SuiChainIdentifier, SuiConnectorConfig, StateArchiveConfig, RunWithRange};

use ika_config::p2p::{P2pConfig, SeedPeer, StateSyncConfig};

use ika_config::{local_ip_utils, ConsensusConfig, NodeConfig, AUTHORITIES_DB_NAME, CONSENSUS_DB_NAME, FULL_NODE_DB_PATH};
...
impl ValidatorConfigBuilder {
    ...
    pub fn build(
        ...
        NodeConfig {
            ...
            network_key_pair: KeyPairWithPath::new(SuiKeyPair::Ed25519(validator.network_key_pair.copy())),
            account_key_pair: KeyPairWithPath::new(validator.account_key_pair.copy()),
            worker_key_pair: KeyPairWithPath::new(SuiKeyPair::Ed25519(validator.worker_key_pair.copy())),
    ...
    pub fn build_new_validator<R: rand::RngCore + rand::CryptoRng>(
        ...
        let validator_initialization_config = ValidatorInitializationConfigBuilder::new().build(rng);
        self.build(&validator_initialization_config, sui_rpc_url, ika_package_id, ika_system_package_id, ika_system_state_id)
    }
}
            ...
            let seed_peers = validators
                ...
                .map(|v| SeedPeer {
                    peer_id: Some(anemo::PeerId(
                        v.network_key_pair.public().0.to_bytes(),
                    )),
