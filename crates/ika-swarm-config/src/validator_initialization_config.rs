...
use ika_types::crypto::{generate_proof_of_possession, get_key_pair_from_rng, AccountKeyPair, AuthorityKeyPair, AuthorityPublicKeyBytes, AuthoritySignature, NetworkKeyPair, NetworkPublicKey};
...
impl ValidatorInitializationConfig {
    pub fn to_validator_initialization_metadata(&self) -> ValidatorInitializationMetadata {
        ...
        ValidatorInitializationMetadata {
            ...
            proof_of_possession: generate_proof_of_possession(&self.key_pair, (&self.account_key_pair.public()).into())
        }
    }
}
...
pub struct ValidatorInitializationMetadata {
    ...
    pub worker_public_key: NetworkPublicKey,
...
impl ValidatorInitializationConfigBuilder {
    ...
    pub fn build<R: rand::RngCore + rand::CryptoRng>(self, rng: &mut R) -> ValidatorInitializationConfig {
        ...
        let account_key_pair = self
            .account_key_pair
            .unwrap_or_else(|| get_key_pair_from_rng(rng).1);
        let computation_price = self.computation_price.unwrap_or(DEFAULT_VALIDATOR_GAS_PRICE);
