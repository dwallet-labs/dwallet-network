...
use ika_config::genesis::{GenesisCeremonyParameters, TokenAllocation};
use ika_config::node::{DEFAULT_COMMISSION_RATE, DEFAULT_VALIDATOR_GAS_PRICE};
use ika_config::{local_ip_utils, Config};
use ika_genesis_builder::validator_info::{GenesisValidatorInfo, ValidatorInfo};
use ika_types::base_types::IkaAddress;
use ika_types::crypto::{
    generate_proof_of_possession, get_key_pair_from_rng, AccountKeyPair, AuthorityKeyPair,
    AuthorityPublicKeyBytes, NetworkKeyPair, NetworkPublicKey, PublicKey, IkaKeyPair,
};
...
pub struct ValidatorGenesisConfig {
    ...
    pub account_key_pair: IkaKeyPair,
    #[serde(default = "default_ed25519_key_pair")]
...
impl ValidatorGenesisConfig {
    pub fn to_validator_info(&self, name: String) -> GenesisValidatorInfo {
        ...
        let info = ValidatorInfo {
            ...
            worker_key,
            network_key,
            account_address: IkaAddress::from(&account_key),
...
impl ValidatorGenesisConfigBuilder {
    ...
    pub fn build<R: rand::RngCore + rand::CryptoRng>(self, rng: &mut R) -> ValidatorGenesisConfig {
        ...
        ValidatorGenesisConfig {
            ...
            narwhal_worker_address,
            consensus_address,
            stake: ika_types::governance::VALIDATOR_LOW_STAKE_THRESHOLD_NIKA,
            name: None,
        }
    }
}
...
fn default_ika_key_pair() -> IkaKeyPair {
    IkaKeyPair::Ed25519(get_key_pair_from_rng(&mut rand::rngs::OsRng).1)
}

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct AccountConfig {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub address: Option<IkaAddress>,
...
impl GenesisConfig {
    ...
    pub fn for_local_testing_with_addresses(addresses: Vec<IkaAddress>) -> Self {
    ...
    pub fn benchmark_gas_keys(n: usize) -> Vec<IkaKeyPair> {
        let mut rng = StdRng::seed_from_u64(Self::BENCHMARKS_RNG_SEED);
        (0..n)
            .map(|_| IkaKeyPair::Ed25519(NetworkKeyPair::generate(&mut rng)))
            .collect()
    }
