...
// 1. Run `cargo insta test --review` under `./ika-config`.
...
fn populated_genesis_snapshot_matches() {
    ...
    let consensus_key: NetworkKeyPair = get_key_pair_from_rng(&mut rng).1;
    ...
    let genesis = Builder::new()
        ...
        .build();
    assert_yaml_snapshot!(genesis.ika_system_wrapper_object());
