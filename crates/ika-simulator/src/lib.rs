...
// Re-export things used by ika-macros
...
pub use ika_framework;
pub use ika_move_build;
pub use ika_types;
...
pub mod configs {
    ...
    /// Select from among a number of configs using the IKA_SIM_CONFIG env var.
    pub fn env_config(
        // Config to use when IKA_SIM_CONFIG is not set.
        default: SimConfig,
        // List of (&str, SimConfig) pairs - the SimConfig associated with the value
        // of the IKA_SIM_CONFIG var is chosen.
        ...
        if let Some(env) = std::env::var("IKA_SIM_CONFIG").ok() {
            if let Some(cfg) = env_configs.remove(env.as_str()) {
                info!("Using test config for IKA_SIM_CONFIG={}", env);
                cfg
            } else {
                panic!(
                    "No config found for IKA_SIM_CONFIG={}. Available configs are: {:?}",
                    env,
                    env_configs.keys()
                );
            }
