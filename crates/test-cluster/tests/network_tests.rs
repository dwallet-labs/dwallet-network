...
use ika_json_rpc_api::ReadApiClient;
...
use ika_types::{
    ...
    IKA_FRAMEWORK_PACKAGE_ID, IKA_SYSTEM_ADDRESS, IKA_SYSTEM_PACKAGE_ID,
};
...
async fn test_additional_objects() {
    ...
    assert!(matches!(resp, IkaObjectResponse { data: Some(_), .. }));
}
...
async fn test_package_override() {
    ...
    let framework_ref = {
        ...
        let obj = client
            .get_object(IKA_SYSTEM_PACKAGE_ID, None)
            .await
            .unwrap();
    ...
    let modified_ref = {
        let mut framework_modules = BuiltInFramework::get_package_by_id(&IKA_SYSTEM_PACKAGE_ID)
            .modules()
            .to_vec();

        // Create an empty module that is pretending to be part of the ika framework.
        ...
        test_module.address_identifiers[address_idx] = IKA_SYSTEM_ADDRESS;

        // Add the dummy module to the rest of the ika-frameworks.  We can't replace the framework
