...
use ika_types::sui::error::{BridgeError, BridgeResult};
use ika_types::sui::events::SuiBridgeEvent;
use crate::metrics::SuiClientMetrics;
use ika_types::sui::ika_system_state::{IkaSystemState, IkaSystemStateWrapper};
use ika_types::sui::ika_system_state::ika_system_state_inner_v1::{IkaSystemStateInnerV1};
use ika_types::sui::types::BridgeActionStatus;
use ika_types::sui::types::ParsedTokenTransferMessage;
use ika_types::sui::types::{BridgeAction};
...
impl SuiBridgeClient {
    pub async fn new(
        rpc_url: &str,
        bridge_metrics: Arc<SuiClientMetrics>,
        ika_package_id: ObjectID,
        ika_system_package_id: ObjectID,
        ika_system_state_id: ObjectID,
    ) -> anyhow::Result<Self> {
        ...
        let self_ = Self {
            inner,
            sui_client_metrics: bridge_metrics,
            ika_package_id,
    ...
    P: SuiClientInner,
{
    pub fn new_for_testing(inner: P) -> Self {
        Self {
            ...
            ika_system_state_id: ObjectID::random(),
        }
    }
    ...
    pub async fn get_ika_system_state(&self) -> BridgeResult<IkaSystemState> {
        let result = self.inner.get_ika_system_state_wrapper(self.ika_system_state_id).await.map_err(|e| BridgeError::InternalError(format!("Can't get IkaSystemStateWrapper: {e}")))?;
        let wrapper = bcs::from_bytes::<IkaSystemStateWrapper>(&result).map_err(|e| BridgeError::SuiHandleSerializationError(format!("Can't serialization IkaSystemStateWrapper: {e}")))?;

        match wrapper.version {
            1 => {
                let result = self.inner.get_ika_system_state(self.ika_system_state_id, wrapper.version).await.map_err(|e| BridgeError::InternalError(format!("Can't get IkaSystemState: {e}")))?;
                let dynamic_field_inner = bcs::from_bytes::<Field<u64, IkaSystemStateInnerV1>>(&result).map_err(|e| BridgeError::SuiHandleSerializationError(format!("Can't serialization IkaSystemStateInnerV1: {e}")))?;
                Ok(IkaSystemState::V1(dynamic_field_inner.value))
            }
            _ => {
                Err(BridgeError::InternalError(format!(
                    "Unsupported IkaSystemState version: {}", wrapper.version
                )))
            }
        }
    }
    ...
    pub async fn get_mutable_ika_system_state_arg_must_succeed(&self) -> ObjectArg {
        ...
        *ARG.get_or_init(|| async move {
            let Ok(Ok(bridge_object_arg)) = retry_with_max_elapsed_time!(
                self.inner.get_mutable_ika_system_state_arg(self.ika_system_state_id),
    ...
    pub async fn get_available_move_packages(
        &self,
    ) -> BridgeResult<Vec<(ObjectID, MovePackageDigest)>> {
        Ok(self.inner.get_available_move_packages(self.ika_package_id, self.ika_system_package_id).await?)
    }
    ...
    pub async fn query_events_by_module(
        ...
        assert!(events
            .data
            .iter()
            .all(|event| event.type_.address.as_ref() == self.ika_package_id.as_ref()
    ...
    pub async fn get_bridge_action_by_tx_digest_and_event_idx_maybe(
        ...
        if event.type_.address.as_ref() != self.ika_system_package_id.as_ref() {
    ...
    pub async fn get_ika_system_state_until_success(
        &self
    ) -> IkaSystemState {
        loop {
            let Ok(Ok(ika_system_state)) = retry_with_max_elapsed_time!(
                self.get_ika_system_state(),
                Duration::from_secs(30)
            ) else {
                self.sui_client_metrics
                    .sui_rpc_errors
                    .with_label_values(&["get_ika_system_state_until_success"])
                    .inc();
                error!("Failed to get ika system state until success");
                continue;
            };
    ...
    pub async fn get_token_transfer_action_onchain_signatures_until_success(
        ...
        loop {
            let bridge_object_arg = self.get_mutable_ika_system_state_arg_must_succeed().await;
            let Ok(Ok(sigs)) = retry_with_max_elapsed_time!(
                self.inner.get_token_transfer_action_onchain_signatures(
                    self.ika_system_state_id,
                    bridge_object_arg,
                    source_chain_id,
                    seq_number
                ),
    ...
    pub async fn get_parsed_token_transfer_message(
        ...
        let message = self
            .inner
            .get_parsed_token_transfer_message(self.ika_system_state_id, bridge_object_arg, source_chain_id, seq_number)
            .await?;
        Ok(match message {
...
pub trait SuiClientInner: Send + Sync {
    ...
    async fn get_ika_system_state_wrapper(&self, ika_system_state_id: ObjectID) -> Result<Vec<u8>, Self::Error>;

    async fn get_ika_system_state(&self, ika_system_state_id: ObjectID, version: u64) -> Result<Vec<u8>, Self::Error>;

    async fn get_mutable_ika_system_state_arg(&self, ika_system_state_id: ObjectID) -> Result<ObjectArg, Self::Error>;
...
impl SuiClientInner for SuiSdkClient {
    ...
    async fn get_ika_system_state_wrapper(&self, ika_system_state_id: ObjectID) -> Result<Vec<u8>, Self::Error> {
        self.read_api().get_move_object_bcs(ika_system_state_id)
            .await
    }
    ...
    async fn get_ika_system_state(&self, ika_system_state_id: ObjectID, version: u64) -> Result<Vec<u8>, Self::Error> {
        let dynamic_fields = self
            .read_api()
            .get_dynamic_fields(ika_system_state_id, None, None)
            .await?;
        let dynamic_field = dynamic_fields.data.iter().find(|df| df.name.type_ == TypeTag::U64 && df.name.value.as_str().map(|v| v == version.to_string().as_str()).unwrap_or(false));
        if let Some(dynamic_field) = dynamic_field {
            let result = self.read_api().get_dynamic_field_object(ika_system_state_id, dynamic_field.name.clone())
                .await?;

            if let Some(dynamic_field) = result.data {
                let SuiMoveValue::Struct(obj) = dynamic_field.clone().content.unwrap().try_as_move().unwrap().fields.field_value("value").unwrap() else { todo!() };
                //println!("result: {:#?}",&dynamic_field);

                // let SuiMoveValue::Struct(obj) = obj.field_value("id").unwrap() else { todo!() };
                // let SuiMoveValue::Address(object_id) = obj.field_value("id").unwrap() else { todo!() };
                let object_id = dynamic_field.object_id;
                let dynamic_field_response = self
                .read_api()
                .get_object_with_options(object_id, SuiObjectDataOptions::bcs_lossless()).await?;
                let resp = dynamic_field_response.into_object()
                    .map_err(|e| {
        ...
        Err(Self::Error::DataError(format!(
            ...
            ika_system_state_id, version
        )))

    }

    async fn get_mutable_ika_system_state_arg(&self, ika_system_state_id: ObjectID) -> Result<ObjectArg, Self::Error> {
        let response = self.read_api().get_object_with_options(ika_system_state_id, SuiObjectDataOptions::new().with_owner()).await?;
        let Some(Owner::Shared {initial_shared_version}) = response.owner() else {
    ...
    async fn get_available_move_packages(
        ...
        let ika_packages = vec![("ika".to_string(), ika_package_id), ("ika_system".to_string(), ika_system_package_id)];
        for (name, package_id) in ika_packages.clone() {
            ...
            let bytes = move_package.bytes_with_deps(ika_packages.clone().into_iter().collect()).map_err(|e: anyhow::Error| Self::Error::DataError(e.to_string()))?;
            let full_deps = move_package.full_deps(ika_packages.clone().into_iter().collect()).map_err(|e: anyhow::Error| Self::Error::DataError(e.to_string()))?;
            let digest = MovePackage::compute_digest_for_modules_and_deps(bytes.iter(), full_deps.iter(), true);
...
//             .await;
//
//         let bridge_metrics = Arc::new(SuiHandlerMetrics::new_for_testing());
//         let sui_client =
