...
use crate::payload::{PayPera, ProcessPayload, RpcCommandProcessor, SignerInfo};
...
#[async_trait]
impl<'a> ProcessPayload<'a, &'a PayPera> for RpcCommandProcessor {
    async fn process(
        &'a self,
        _op: &'a PayPera,
        ...
        let keypair =
            PeraKeyPair::decode_base64(&encoded_keypair).expect("Decoding keypair should not fail");

        debug!(
            "Transfer Pera {} time to {recipient} with {amount} NPERA with {gas_payments:?}",
            gas_payments.len()
        );
        ...
        join_all(gas_payments.iter().map(|gas| async {
            let tx = TransactionData::new_transfer_pera(
                recipient,
                sender,
                Some(amount),
