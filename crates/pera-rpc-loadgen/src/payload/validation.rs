...
pub(crate) async fn check_transactions(
    ...
    cross_validate: bool,
    verify_objects: bool,
) -> Vec<Vec<PeraTransactionBlockResponse>> {
    let transactions: Vec<Vec<PeraTransactionBlockResponse>> =
        join_all(clients.iter().map(|client| async move {
            client
                .read_api()
                .multi_get_transactions_with_options(
                    digests.to_vec(),
                    PeraTransactionBlockResponseOptions::full_content(), // todo(Will) support options for this
                )
                .await
        }))
        .await
...
pub(crate) fn get_all_object_ids(response: &PeraTransactionBlockResponse) -> Vec<ObjectID> {
...
pub(crate) async fn multi_get_object(
    clients: &[PeraClient],
    object_ids: &[ObjectID],
) -> Vec<Vec<PeraObjectResponse>> {
    let objects: Vec<Vec<PeraObjectResponse>> = join_all(clients.iter().map(|client| async move {
        ...
        client
            ...
            .multi_get_object_with_options(
                object_ids.to_vec(),
                PeraObjectDataOptions::full_content(), // todo(Will) support options for this
            )
            .await
    }))
    .await
