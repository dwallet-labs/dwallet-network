...
use pera_json_rpc_types::{PeraObjectDataOptions, PeraObjectResponse};
...
pub(crate) async fn get_object(
    client: &PeraClient,
    object_id: ObjectID,
) -> Result<PeraObjectResponse> {
    let result = client
        .read_api()
        .get_object_with_options(object_id, PeraObjectDataOptions::full_content())
        .await
        .unwrap();
    Ok(result)
}
