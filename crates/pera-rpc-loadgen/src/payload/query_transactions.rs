...
use pera_json_rpc_types::{
    Page, PeraTransactionBlockResponse, PeraTransactionBlockResponseOptions,
...
impl<'a> ProcessPayload<'a, &'a QueryTransactionBlocks> for RpcCommandProcessor {
    async fn process(
        ...
        let queries: Vec<PeraTransactionBlockResponseQuery> = filters
            .into_iter()
            .map(|filter| PeraTransactionBlockResponseQuery {
                filter,
...
) -> Result<Page<PeraTransactionBlockResponse, TransactionDigest>> {
