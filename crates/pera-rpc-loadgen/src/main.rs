...
use pera_types::crypto::{EncodeDecodeBase64, PeraKeyPair};
use tracing::info;
...
#[derive(Parser)]
#[clap(
    name = "Pera RPC Load Generator",
    version = "0.1",
    about = "A load test application for Pera RPC"
)]
struct Opts {
    ...
    #[clap(long, default_value = "~/.pera/pera_config/logs")]
    logs_directory: String,

    #[clap(long, default_value = "~/.pera/loadgen/data")]
...
fn get_keypair() -> Result<SignerInfo> {
    ...
    // Load keystore from ~/.pera/pera_config/pera.keystore
    let keystore_path = get_pera_config_directory().join("pera.keystore");
    let keystore = Keystore::from(FileBasedKeystore::new(&keystore_path)?);
    let active_address = keystore.addresses().pop().unwrap();
    let keypair: &PeraKeyPair = keystore.get_key(&active_address)?;
    println!("using address {active_address} for signing");
    Ok(SignerInfo::new(keypair.encode_base64()))
}

fn get_pera_config_directory() -> PathBuf {
    match dirs::home_dir() {
        Some(v) => v.join(".pera").join("pera_config"),
...
fn get_log_file_path(dir_path: String) -> String {
    ...
    let log_filename = format!("pera-rpc-loadgen.{}.log", timestamp);
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (command, common, need_keystore) = match opts.command {
        ...
        ClapCommand::PayPera { common } => (Command::new_pay_pera(), common, true),
