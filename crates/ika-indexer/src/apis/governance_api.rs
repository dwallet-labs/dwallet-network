...
use ika_json_rpc::{governance_api::ValidatorExchangeRates, IkaRpcModule};
use ika_json_rpc_api::GovernanceReadApiServer;
use ika_json_rpc_types::{
    DelegatedStake, EpochInfo, StakeStatus, IkaCommittee, IkaObjectDataFilter, ValidatorApys,
};
use ika_open_rpc::Module;
use ika_types::{
    base_types::{MoveObjectType, ObjectID, IkaAddress},
    committee::EpochId,
    governance::StakedIka,
    ika_serde::BigInt,
    ika_system_state::{ika_system_state_summary::IkaSystemStateSummary, PoolTokenExchangeRate},
};
...
impl GovernanceReadApi {
    ...
    async fn get_latest_ika_system_state(&self) -> Result<IkaSystemStateSummary, IndexerError> {
        self.inner.get_latest_ika_system_state().await
    }
    ...
    async fn get_stakes_by_ids(
        ...
        for stored_object in self.inner.multi_get_objects(ids).await? {
            let object = ika_types::object::Object::try_from(stored_object)?;
            let stake_object = StakedIka::try_from(&object)?;
    ...
    async fn get_staked_by_owner(
        ...
        for stored_object in self
            ...
            .get_owned_objects(
                owner,
                Some(IkaObjectDataFilter::StructType(
                    MoveObjectType::staked_ika().into(),
                )),
                None,
    ...
    pub async fn get_delegated_stakes(
        ...
        let system_state_summary = self.get_latest_ika_system_state().await?;
        ...
        for (pool_id, stakes) in pools {
            ...
            for stake in stakes {
                ...
                delegations.push(ika_json_rpc_types::Stake {
...
pub async fn exchange_rates(
    ...
    system_state_summary: &IkaSystemStateSummary,
    ...
    for df in state
        ...
        .await?
    {
        let pool_id: ika_types::id::ID = bcs::from_bytes(&df.bcs_name).map_err(|e| {
            ika_types::error::IkaError::ObjectDeserializationError {
                error: e.to_string(),
            }
        })?;
    ...
    for (address, pool_id, exchange_rates_id, exchange_rates_size, active) in tables {
            ...
            .await?
        {
            let dynamic_field = df
                ...
                .ok_or_else(|| ika_types::error::IkaError::ObjectDeserializationError {
...
impl GovernanceReadApiServer for GovernanceReadApi {
    async fn get_stakes_by_ids(
        ...
        self.get_stakes_by_ids(staked_ika_ids)
            .await
            .map_err(Into::into)
    }
    ...
    async fn get_latest_ika_system_state(&self) -> RpcResult<IkaSystemStateSummary> {
        self.get_latest_ika_system_state().await.map_err(Into::into)
    }
...
impl IkaRpcModule for GovernanceReadApi {
    ...
    fn rpc_doc_module() -> Module {
        ika_json_rpc_api::GovernanceReadApiOpenRpc::module_doc()
    }
}
