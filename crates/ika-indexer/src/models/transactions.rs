...
use ika_json_rpc_types::{
    BalanceChange, ObjectChange, IkaEvent, IkaTransactionBlock, IkaTransactionBlockEffects,
    IkaTransactionBlockEvents, IkaTransactionBlockResponse, IkaTransactionBlockResponseOptions,
};
...
impl StoredTransaction {
    ...
    pub async fn try_into_ika_transaction_block_response(
        self,
        options: IkaTransactionBlockResponseOptions,
        package_resolver: Arc<Resolver<impl PackageStore>>,
    ) -> IndexerResult<IkaTransactionBlockResponse> {
        ...
        let transaction = if options.show_input {
            ...
            let tx_block = IkaTransactionBlock::try_from_with_package_resolver(
                sender_signed_data,
        ...
        let effects = if options.show_effects {
            let effects = self.try_into_ika_transaction_effects()?;
            Some(effects)
        } else {
            None
        };
        ...
        let events = if options.show_events {
            ...
            tx_events_to_ika_tx_events(tx_events, package_resolver, tx_digest, timestamp).await?
        } else {
            None
        };
        ...
        Ok(IkaTransactionBlockResponse {
            digest: tx_digest,
            transaction,
            raw_transaction,
            effects,
    ...
    pub fn try_into_ika_transaction_effects(&self) -> IndexerResult<IkaTransactionBlockEffects> {
        ...
        let effects = IkaTransactionBlockEffects::try_from(effects)?;
        Ok(effects)
    }
}
...
pub async fn tx_events_to_ika_tx_events(
    tx_events: TransactionEvents,
    package_resolver: Arc<Resolver<impl PackageStore>>,
    tx_digest: TransactionDigest,
    timestamp: u64,
) -> Result<Option<IkaTransactionBlockEvents>, IndexerError> {
    ...
    for tx_event in tx_events.data.clone() {
        ...
        ika_event_futures.push(tokio::task::spawn(async move {
    ...
    let event_move_type_layouts = futures::future::join_all(ika_event_futures)
        .await
        .into_iter()
        ...
        .map_err(|e| {
            IndexerError::ResolveMoveStructError(format!(
                "Failed to convert to ika event with Error: {e}",
            ))
        })?;
    ...
    let ika_events = tx_events
        ...
        .collect::<Result<Vec<_>, _>>()?;
    let ika_tx_events = IkaTransactionBlockEvents { data: ika_events };
    Ok(Some(ika_tx_events))
}
