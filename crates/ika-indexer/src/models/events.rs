...
use ika_json_rpc_types::{type_and_fields_from_move_event_data, IkaEvent};
...
impl StoredEvent {
    pub async fn try_into_ika_event(
        self,
        package_resolver: Arc<Resolver<impl PackageStore>>,
    ) -> Result<IkaEvent, IndexerError> {
        ...
        // Note: IkaEvent only has one sender today, so we always use the first one.
        let sender = {
        ...
        let sender = match sender {
            Some(ref s) => IkaAddress::from_bytes(s).map_err(|_e| {
                IndexerError::PersistentStorageDataCorruptionError(format!(
        ...
        let type_ = parse_ika_struct_tag(&self.event_type)?;
