...
use clap::Parser;
use ika_indexer::backfill::backfill_runner::BackfillRunner;
use ika_indexer::benchmark::run_indexer_benchmark;
use ika_indexer::config::{Command, RetentionConfig, UploadOptions};
use ika_indexer::database::ConnectionPool;
use ika_indexer::db::setup_postgres::clear_database;
use ika_indexer::db::{
    check_db_migration_consistency, check_prunable_tables_valid, reset_database, run_migrations,
};
use ika_indexer::indexer::Indexer;
use ika_indexer::metrics::{
    spawn_connection_pool_metric_collector, start_prometheus_server, IndexerMetrics,
};
use ika_indexer::restorer::formal_snapshot::IndexerFormalSnapshotRestorer;
use ika_indexer::store::PgIndexerStore;
use tokio_util::sync::CancellationToken;
use tracing::warn;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let opts = ika_indexer::config::IndexerConfig::parse();
