...
use ika_indexer::errors::IndexerError;
use ika_indexer::handlers::TransactionObjectChangesToCommit;
use ika_indexer::models::{
    checkpoints::StoredCheckpoint, objects::StoredObject, objects::StoredObjectSnapshot,
    transactions::StoredTransaction,
};
use ika_indexer::schema::epochs;
use ika_indexer::schema::events;
use ika_indexer::schema::full_objects_history;
use ika_indexer::schema::objects_history;
use ika_indexer::schema::{checkpoints, objects, objects_snapshot, transactions};
use ika_indexer::store::indexer_store::IndexerStore;
use ika_indexer::test_utils::set_up_on_mvr_mode;
use ika_indexer::test_utils::{
    set_up, set_up_with_start_and_end_checkpoints, wait_for_checkpoint, wait_for_objects_snapshot,
};
use ika_indexer::types::EventIndex;
use ika_indexer::types::IndexedDeletedObject;
use ika_indexer::types::IndexedObject;
use ika_indexer::types::TxIndex;
...
pub async fn test_transaction_table() -> Result<(), IndexerError> {
    ...
    let transfer_recipient = IkaAddress::random_for_testing_only();
...
pub async fn test_object_type() -> Result<(), IndexerError> {
    ...
    assert_eq!(
        ...
        Some(IKA_FRAMEWORK_PACKAGE_ID.to_vec())
    );
