...
use ika_move_packages::{BuiltInIkaMovePackages, IkaMovePackage};
use ika_move_packages_snapshot::update_bytecode_snapshot_manifest;
...
fn main() {
    ...
    let mut files = vec![];
    for package in BuiltInIkaMovePackages::iter_system_packages() {
...
fn write_package_to_file(version: u64, package: &IkaMovePackage) {
