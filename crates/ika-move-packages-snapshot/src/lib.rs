...
use ika_types::{
    BRIDGE_PACKAGE_ID, DEEPBOOK_PACKAGE_ID, MOVE_STDLIB_PACKAGE_ID, IKA_FRAMEWORK_PACKAGE_ID,
...
pub fn load_bytecode_snapshot(protocol_version: u64) -> anyhow::Result<Vec<IkaMovePackage>> {
    ...
    let mut snapshots: BTreeMap<ObjectID, IkaMovePackage> = fs::read_dir(&snapshot_path)?
        .flatten()
        .map(|entry| {
            ...
            let package: IkaMovePackage = bcs::from_bytes(&buffer)?;
