...
use pera_sdk::rpc_types::{
    get_new_package_obj_from_response, PeraObjectDataOptions, PeraTransactionBlockEffectsAPI,
...
impl TestTransactionBuilder {
    pub fn new(sender: PeraAddress, gas_object: ObjectRef, gas_price: u64) -> Self {
        Self {
    ...
    pub fn call_nft_create(self, package_id: ObjectID) -> Self {
        self.move_call(
            ...
            vec![
                ...
                CallArg::Pure(
                    bcs::to_bytes("https://pera.io/_nuxt/img/pera-logo.8d3c44e.svg").unwrap(),
                ),
            ],
        )
    }
    ...
    pub fn call_staking(self, stake_coin: ObjectRef, validator: PeraAddress) -> Self {
        self.move_call(
            PERA_SYSTEM_PACKAGE_ID,
            PERA_SYSTEM_MODULE_NAME.as_str(),
            "request_add_stake",
            vec![
    ...
    pub fn call_request_add_validator_candidate(
        ...
        self.move_call(
            ...
            vec![
                ...
                CallArg::Pure(bcs::to_bytes(&validator.class_groups_public_key_and_proof).unwrap()),
    ...
    pub fn transfer(mut self, object: ObjectRef, recipient: PeraAddress) -> Self {
        self.test_data = TestTransactionData::Transfer(TransferData { object, recipient });
        self
    }

    pub fn transfer_pera(mut self, amount: Option<u64>, recipient: PeraAddress) -> Self {
        self.test_data = TestTransactionData::TransferPera(TransferPeraData { amount, recipient });
        self
    }
    ...
    pub fn build(self) -> TransactionData {
        match self.test_data {
            ...
            TestTransactionData::TransferPera(data) => TransactionData::new_transfer_pera(
                data.recipient,
                self.sender,
                data.amount,
                self.gas_object,
    ...
    pub fn build_and_sign_multisig(
        ...
        let data = self.build();
        let intent_msg = IntentMessage::new(Intent::pera_transaction(), data.clone());
...
pub async fn make_transfer_pera_transaction(
    context: &WalletContext,
    recipient: Option<PeraAddress>,
    amount: Option<u64>,
) -> Transaction {
    ...
    context.sign_transaction(
        &TestTransactionBuilder::new(sender, gas_object, gas_price)
            .transfer_pera(amount, recipient.unwrap_or(sender))
            .build(),
    )
}
...
pub async fn create_nft(
    context: &WalletContext,
    package_id: ObjectID,
) -> (PeraAddress, ObjectID, TransactionDigest) {
    let (sender, gas_object) = context.get_one_gas_object().await.unwrap().unwrap();
    let rgp = context.get_reference_gas_price().await.unwrap();
