...
mod sim_only_tests {
    ...
    use std::sync::Arc;
    use ika_core::authority::framework_injection;
    use ika_framework::BuiltInFramework;
    use ika_json_rpc_api::WriteApiClient;
    use ika_json_rpc_types::{IkaTransactionBlockEffects, IkaTransactionBlockEffectsAPI};
    use ika_macros::*;
    use ika_move_build::{BuildConfig, CompiledPackage};
    ...
    use ika_types::ika_system_state::{
        epoch_start_ika_system_state::EpochStartSystemStateTrait, get_validator_from_table,
        IkaSystemState, IkaSystemStateTrait, IKA_SYSTEM_STATE_SIM_TEST_DEEP_V2,
        IKA_SYSTEM_STATE_SIM_TEST_SHALLOW_V2, IKA_SYSTEM_STATE_SIM_TEST_V1,
    };
    use ika_types::supported_protocol_versions::SupportedProtocolVersions;
    ...
    use ika_types::{
        base_types::{SequenceNumber, IkaAddress},
        digests::TransactionDigest,
        object::Object,
        programmable_transaction_builder::ProgrammableTransactionBuilder,
        transaction::TransactionKind,
        MOVE_STDLIB_PACKAGE_ID, IKA_BRIDGE_OBJECT_ID, IKA_FRAMEWORK_PACKAGE_ID,
        IKA_SYSTEM_PACKAGE_ID,
    };
    use ika_types::{
        IKA_AUTHENTICATOR_STATE_OBJECT_ID, IKA_CLOCK_OBJECT_ID, IKA_RANDOMNESS_STATE_OBJECT_ID,
    ...
    async fn test_framework_add_struct_ability() {
        ...
        transfer_obj(&cluster, IkaAddress::ZERO, to_transfer0).await;
        wrap_obj(&cluster, to_wrap1).await;
        transfer_obj(&cluster, IkaAddress::ZERO, to_transfer1).await;
    }
    ...
    async fn test_new_framework_package() {
        ...
        let ika_extra = ObjectID::from_single_byte(0x42);
        framework_injection::set_override(ika_extra, fixture_modules("extra_package"));
        ...
        // initializer
        let effects = get_framework_upgrade_effects(&cluster, &ika_extra).await;
            ...
            .find_map(|(obj, owner)| {
                if let Owner::Shared { .. } = owner {
                    let is_framework_obj = [
                        IKA_SYSTEM_STATE_OBJECT_ID,
                        IKA_CLOCK_OBJECT_ID,
                        IKA_AUTHENTICATOR_STATE_OBJECT_ID,
                        IKA_RANDOMNESS_STATE_OBJECT_ID,
                        IKA_BRIDGE_OBJECT_ID,
                    ]
                    .contains(&obj.0);
    ...
    async fn run_framework_upgrade(from: &str, to: &str) -> TestCluster {
        ...
        override_ika_system_modules(to);
        TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([ika_system_package_object(from)])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    async fn get_framework_upgrade_versions(
        ...
        let effects = get_framework_upgrade_effects(cluster, &IKA_SYSTEM_PACKAGE_ID).await;

        let modified_at = effects
            .modified_at_versions()
            .iter()
            .find_map(|(id, v)| (id == &IKA_SYSTEM_PACKAGE_ID).then_some(*v));

        let mutated_to = effects
            .mutated()
            .iter()
            .find_map(|((id, v, _), _)| (id == &IKA_SYSTEM_PACKAGE_ID).then_some(*v));
    ...
    async fn get_framework_upgrade_effects(
        ...
        let node_handle = &cluster.fullnode_handle.ika_node;
    ...
    async fn test_framework_compatible_upgrade_no_protocol_version() {
        ...
        override_ika_system_modules("compatible");
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            .with_objects([ika_system_package_object("base")])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, START,
            ))
            .build()
    ...
    async fn test_framework_upgrade_conflicting_versions() {
        ...
        override_ika_system_modules_cb(Box::new(move |name| {
            if name == first_name {
                info!("node {:?} using compatible packages", name.concise());
                Some(ika_system_modules("base"))
            } else {
                Some(ika_system_modules("compatible"))
            }
        }));
    ...
    async fn test_safe_mode_recovery() {
        ...
        override_ika_system_modules("mock_ika_systems/base");
        let test_cluster = TestClusterBuilder::new()
            .with_epoch_duration_ms(20000)
            // Overrides with a ika system package that would abort during epoch change txn
            .with_objects([ika_system_package_object("mock_ika_systems/safe_mode")])
            .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(
                START, FINISH,
            ))
            .build()
    ...
    #[sim_test]
    async fn ika_system_mock_smoke_test() {
        let _guard = ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
            config.set_disable_bridge_for_testing();
            config
        });
        ...
        let test_cluster = TestClusterBuilder::new()
            ...
            .with_objects([ika_system_package_object("mock_ika_systems/base")])
            .build()
            .await;
    ...
    #[sim_test]
    async fn ika_system_state_shallow_upgrade_test() {
        let _guard = ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
            config.set_disable_bridge_for_testing();
            config
        });

        override_ika_system_modules("mock_ika_systems/shallow_upgrade");
        ...
        assert_eq!(
            ...
            IKA_SYSTEM_STATE_SIM_TEST_V1
        );
        assert!(matches!(system_state, IkaSystemState::SimTestV1(_)));
        ...
        assert_eq!(
            system_state.system_state_version(),
            IKA_SYSTEM_STATE_SIM_TEST_SHALLOW_V2
        );
        assert!(matches!(system_state, IkaSystemState::SimTestShallowV2(_)));
    }

    #[sim_test]
    async fn ika_system_state_deep_upgrade_test() {
        let _guard = ProtocolConfig::apply_overrides_for_testing(|_, mut config| {
            config.set_disable_bridge_for_testing();
            config
        });

        override_ika_system_modules("mock_ika_systems/deep_upgrade");
        ...
        if let IkaSystemState::SimTestV1(inner) = system_state {
        ...
        assert_eq!(
            system_state.system_state_version(),
            IKA_SYSTEM_STATE_SIM_TEST_DEEP_V2
        );
        if let IkaSystemState::SimTestDeepV2(inner) = system_state {
            // Make sure we have 1 inactive validator for latter testing.
            assert_eq!(inner.validators.inactive_validators.size, 1);
            get_validator_from_table(
                test_cluster
    ...
    #[sim_test]
    async fn ika_system_state_production_upgrade_test() {
        // Use this test to test a real ika system state upgrade. To make this test work,
        // put the new ika system in a new path and point to it in the override.
        ...
        override_ika_system_modules("../../../ika-framework/packages/ika-system");
        // Wait for the upgrade to finish. After the upgrade, the new framework will be installed,
        // but the system state object hasn't been upgraded yet.
        let system_state = test_cluster.wait_for_epoch(Some(1)).await;
        assert_eq!(system_state.protocol_version(), FINISH);
        ...
        // at epoch boundary.
        let system_state = test_cluster.wait_for_epoch(Some(2)).await;
        if let IkaSystemState::V2(inner) = system_state {
            assert_eq!(inner.parameters.min_validator_count, 4);
        } else {
            unreachable!("Unexpected ika system state version");
        }
    }
    ...
    fn override_ika_system_modules(path: &str) {
        framework_injection::set_override(IKA_SYSTEM_PACKAGE_ID, ika_system_modules(path));
    }

    fn override_ika_system_modules_cb(f: framework_injection::PackageUpgradeCallback) {
        framework_injection::set_override_cb(IKA_SYSTEM_PACKAGE_ID, f)
    }

    /// Get compiled modules for Ika System, built from fixture `fixture` in the
    /// `framework_upgrades` directory.
    fn ika_system_modules(fixture: &str) -> Vec<CompiledModule> {
        fixture_package(fixture)
            .get_ika_system_modules()
            .cloned()
            .collect()
    }

    /// Like `ika_system_modules`, but package the modules in an `Object`.
    fn ika_system_package_object(fixture: &str) -> Object {
        Object::new_package(
            &ika_system_modules(fixture),
            TransactionDigest::genesis_marker(),
            u64::MAX,
            VERSION_MAX,
            &[
                BuiltInFramework::get_package_by_id(&MOVE_STDLIB_PACKAGE_ID).genesis_move_package(),
                BuiltInFramework::get_package_by_id(&IKA_FRAMEWORK_PACKAGE_ID)
