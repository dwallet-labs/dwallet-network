...
use ika_types::base_types::{EpochId, ObjectID, ObjectRef, IkaAddress};
use ika_types::programmable_transaction_builder::ProgrammableTransactionBuilder;
use ika_types::transaction::{CallArg, ObjectArg, TransactionData};
use ika_types::{IKA_DENY_LIST_OBJECT_ID, IKA_FRAMEWORK_PACKAGE_ID};
use test_cluster::{TestCluster, TestClusterBuilder};

const DENY_ADDRESS: IkaAddress = IkaAddress::ZERO;
...
struct TestEnv {
    ...
    regulated_coin_owner: IkaAddress,
...
async fn create_test_env() -> TestEnv {
    ...
    let deny_list_object_init_version = test_cluster
        .get_object_from_fullnode_store(&IKA_DENY_LIST_OBJECT_ID)
        .await
        .unwrap()
        .version();
