...
use ika_core::authority_client::make_network_authority_clients_with_network_config;
use ika_core::authority_client::AuthorityAPI;
use ika_core::traffic_controller::{
...
async fn test_fullnode_traffic_control_dry_run() -> Result<(), anyhow::Error> {
    ...
    let response: IkaTransactionBlockResponse = jsonrpc_client
        .request("ika_executeTransactionBlock", params.clone())
        .await
        .unwrap();
    ...
    for _ in 0..txn_count {
        let response: RpcResult<IkaTransactionBlockResponse> = jsonrpc_client
...
async fn test_validator_traffic_control_error_blocked() -> Result<(), anyhow::Error> {
    ...
    signatures.push(GenericSignature::Signature(
        ika_types::crypto::Signature::Ed25519IkaSignature(Ed25519IkaSignature::default()),
    ));
