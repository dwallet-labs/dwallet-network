...
module ika_system::ika_system {
    ...
    fun load_inner_maybe_upgrade(self: &mut IkaSystemState): &mut IkaSystemStateInner {
        let version = self.version;
        // TODO: This is where we check the version and perform upgrade if necessary.

        let inner: &mut IkaSystemStateInner = dynamic_field::borrow_mut(&mut self.id, version);
