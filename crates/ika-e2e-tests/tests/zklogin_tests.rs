...
use ika_types::IKA_AUTHENTICATOR_STATE_OBJECT_ID;
use test_cluster::TestCluster;
use test_cluster::TestClusterBuilder;

async fn do_zklogin_test(address: IkaAddress, legacy: bool) -> IkaResult {
...
async fn test_zklogin_feature_legacy_address_deny() {
    ...
    let err = do_zklogin_test(get_legacy_zklogin_user_address(), true)
        .await
        .unwrap_err();
    assert!(matches!(err, IkaError::SignerSignatureAbsent { .. }));
}
...
async fn test_legacy_zklogin_address_accept() {
    ...
    assert!(matches!(err, IkaError::InvalidSignature { .. }));
}
    ...
    for h in &handles {
        h.with(|node| {
            assert!(node
                ...
                .get_latest_object_ref_or_tombstone(IKA_AUTHENTICATOR_STATE_OBJECT_ID)
                .unwrap()
                .is_none());
        });
    }
...
async fn test_conflicting_jwks() {
    ...
    use ika_types::transaction::{TransactionDataAPI, TransactionKind};
