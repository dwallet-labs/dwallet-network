...
use ika_core::authority_client::AuthorityAPI;
use ika_macros::sim_test;
use ika_protocol_config::ProtocolConfig;
use ika_test_transaction_builder::TestTransactionBuilder;
use ika_types::error::UserInputError;
use ika_types::multisig_legacy::MultiSigLegacy;
use ika_types::{
    base_types::IkaAddress,
    crypto::{
        get_key_pair, CompressedSignature, PublicKey, Signature, IkaKeyPair,
...
async fn do_upgraded_multisig_test() -> IkaResult {
...
async fn test_upgraded_multisig_feature_allow() {
    ...
    // feature gate.
    assert!(matches!(res.unwrap_err(), IkaError::UserInputError { .. }));
}
...
async fn test_multisig_e2e() {
    ...
    let multisig_addr = IkaAddress::from(&multisig_pk);
    ...
    let tx1 = TestTransactionBuilder::new(multisig_addr, gas, rgp)
        .transfer_ika(None, IkaAddress::ZERO)
    ...
    let kp3: IkaKeyPair = IkaKeyPair::Secp256r1(get_key_pair().1);
    let pk3 = kp3.public();
    ...
    .unwrap();
    let wrong_sender = IkaAddress::from(&wrong_multisig_pk);
    let gas = test_cluster
...
async fn test_multisig_with_zklogin_scenerios() {
    ...
    let (eph_kp_1, _, _) =
        &load_test_vectors("../ika-types/src/unit_tests/zklogin_test_vectors.json")[2];
    ...
    let tx_data = TestTransactionBuilder::new(multisig_addr, gas, rgp)
        .transfer_ika(None, IkaAddress::ZERO)
        .build();
    let intent_msg = IntentMessage::new(Intent::ika_transaction(), tx_data.clone());
    ...
    let wrong_multisig_addr = IkaAddress::from(
    ...
    let sender = IkaAddress::try_from(&multisig).unwrap();
    ...
    let bad_multisig_addr = IkaAddress::from(&bad_multisig_pk);
    let gas = test_cluster
    ...
    let bad_multisig_addr_2 = IkaAddress::from(&bad_multisig_pk_2);
    let gas = test_cluster
    ...
    let bad_multisig_addr_3 = IkaAddress::from(&small_multisig_pk);
    let gas = test_cluster
    ...
    let bad_multisig_addr_4 = IkaAddress::from(&multisig_pk_with_dup);
    let gas = test_cluster
    ...
    let bad_multisig_addr_11 = IkaAddress::from(&multisig_pk_11);
    let gas = test_cluster
    ...
    let bad_multisig_addr = IkaAddress::from(&multisig_pk_12);
    let gas = test_cluster
        .fund_address_and_return_gas(rgp, Some(20000000000), bad_multisig_addr)
        .await;
    let tx_data = TestTransactionBuilder::new(bad_multisig_addr, gas, rgp)
    ...
    let bad_multisig_addr = IkaAddress::from(&bad_multisig_empty_pk);
    let gas = test_cluster
        .fund_address_and_return_gas(rgp, Some(20000000000), bad_multisig_addr)
        .await;
    let tx_data = TestTransactionBuilder::new(bad_multisig_addr, gas, rgp)
...
async fn test_random_zklogin_in_multisig() {
    let test_vectors =
        &load_test_vectors("../ika-types/src/unit_tests/zklogin_test_vectors.json")[1..11];
    let test_cluster = TestClusterBuilder::new()
        .with_epoch_duration_ms(15000)
        .with_default_jwks()
        .build()
...
async fn test_multisig_legacy_works() {
    ...
    let transfer_from_multisig = TestTransactionBuilder::new(multisig_addr, gas, rgp)
        .transfer_ika(Some(1000000), IkaAddress::ZERO)
