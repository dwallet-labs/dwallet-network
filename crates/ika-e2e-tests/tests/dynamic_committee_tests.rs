...
use ika_macros::*;
use ika_swarm_config::genesis_config::{AccountConfig, DEFAULT_GAS_AMOUNT};
...
struct StressTestRunner {
    ...
    pub accounts: Vec<IkaAddress>,
    pub active_validators: BTreeSet<IkaAddress>,
    pub preactive_validators: BTreeMap<IkaAddress, u64>,
    pub removed_validators: BTreeSet<IkaAddress>,
    pub delegation_requests_this_epoch: BTreeMap<ObjectID, IkaAddress>,
    pub delegation_withdraws_this_epoch: u64,
    pub delegations: BTreeMap<ObjectID, IkaAddress>,
    pub reports: BTreeMap<IkaAddress, BTreeSet<IkaAddress>>,
    pub rng: StdRng,
}
...
impl StressTestRunner {
    ...
    pub fn pick_random_sender(&mut self) -> IkaAddress {
        self.accounts[self.rng.gen_range(0..self.accounts.len())]
    }

    pub fn system_state(&self) -> IkaSystemStateSummary {
        self.state()
    ...
    pub fn pick_random_active_validator(&mut self) -> IkaValidatorSummary {
    ...
    pub async fn run(&self, sender: IkaAddress, pt: ProgrammableTransaction) -> TransactionEffects {
    ...
    pub fn display_effects(&self, effects: &TransactionEffects) {
        ...
        for (obj_ref, _) in effects.created() {
            ...
            println!(">> {struct_tag} TOTAL_IKA: {total_ika}");
        }
    ...
    pub fn state(&self) -> Arc<AuthorityState> {
        self.test_cluster.fullnode_handle.ika_node.state()
    }
...
mod add_stake {
    ...
    impl GenStateChange for RequestAddStakeGen {
        ...
        fn create(&self, runner: &mut StressTestRunner) -> Self::StateChange {
            ...
            let staked_with = runner.pick_random_active_validator().ika_address;
    ...
    impl StatePredicate for RequestAddStake {
        async fn run(&mut self, runner: &mut StressTestRunner) -> Result<TransactionEffects> {
            let pt = {
                ...
                builder.obj(ObjectArg::IKA_SYSTEM_MUT).unwrap();
                builder.pure(self.staked_with).unwrap();
                let coin = StressTestRunner::split_off(&mut builder, self.stake_amount);
                move_call! {
                    builder,
                    (IKA_SYSTEM_PACKAGE_ID)::ika_system::request_add_stake(Argument::Input(0), coin, Argument::Input(1))
                };
                builder.finish()
            };
        ...
        async fn pre_epoch_post_condition(
            ...
            // Assert that a `StakedIka` object matching the amount delegated is created.
            // Assert that this staked ika
            let object = runner
                .get_created_object_of_type_name(effects, "StakedIka")
                .await
                .unwrap();
