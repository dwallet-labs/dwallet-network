...
use ika_rest_api::client::sdk::Client;
use ika_rest_api::client::Client as CoreClient;
use ika_rest_api::{CheckpointResponse, ListCheckpointsQueryParameters};
...
async fn list_checkpoint() {
    ...
    let _checkpoints = client
        .get(&url)
        .header(reqwest::header::ACCEPT, ika_rest_api::APPLICATION_JSON)
        .send()
        .await
        .unwrap()
    ...
    let bytes = client
        .get(&url)
        .header(reqwest::header::ACCEPT, ika_rest_api::APPLICATION_PROTOBUF)
        .send()
        .await
        .unwrap()
        .bytes()
        .await
        .unwrap();
    let _checkpoints = ika_rest_api::proto::ListCheckpointResponse::decode(bytes).unwrap();
    ...
    let bytes = client
        .get(&url)
        .header(reqwest::header::ACCEPT, ika_rest_api::APPLICATION_BCS)
        .send()
        .await
        .unwrap()
        .bytes()
...
async fn get_checkpoint() {
    ...
    let bytes = client
        ...
        .await
        .unwrap();
    let _checkpoints = ika_rest_api::proto::GetCheckpointResponse::decode(bytes).unwrap();
...
async fn get_full_checkpoint() {
    ...
    let bytes = client
        ...
        .await
        .unwrap();
    let _checkpoints = ika_rest_api::proto::FullCheckpoint::decode(bytes).unwrap();
    ...
    let bytes = client
        ...
        .unwrap();
    let _checkpoints =
        bcs::from_bytes::<ika_types::full_checkpoint_content::CheckpointData>(&bytes).unwrap();
}
