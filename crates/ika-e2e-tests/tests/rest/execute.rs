...
use ika_rest_api::client::BalanceChange;
...
async fn execute_transaction_transfer() {
    ...
    let txn =
        make_transfer_ika_transaction(&test_cluster.wallet, Some(address), Some(amount)).await;
    ...
    let mut expected = vec![
        BalanceChange {
            address: sender,
            coin_type: ika_types::gas_coin::GAS::type_tag(),
