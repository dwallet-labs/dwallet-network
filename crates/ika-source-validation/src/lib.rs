...
use ika_move_build::CompiledPackage;
use ika_sdk::apis::ReadApi;
use ika_sdk::error::Error as SdkError;
use ika_sdk::rpc_types::{IkaObjectDataOptions, IkaRawData, IkaRawMovePackage};
...
impl ValidationMode {
    ...
    fn local(&self, package: &CompiledPackage) -> Result<LocalModules, Error> {
        ...
        if self.verify_deps() {
            ...
            for (package, module) in ika_package.bytecode_deps.iter() {
...
impl<'a> BytecodeSourceVerifier<'a> {
    ...
    async fn pkg_for_address(&self, addr: AccountAddress) -> Result<IkaRawMovePackage, Error> {
        // Move packages are specified with an AccountAddress, but are
        // fetched from a ika network via ika_getObject, which takes an object ID
        let obj_id = ObjectID::from(addr);

        // fetch the Ika object at the address specified for the package in the local resolution table
        ...
        let obj_read = self
            .rpc_client
            .get_object_with_options(obj_id, IkaObjectDataOptions::new().with_bcs())
            .await
            .map_err(Error::DependencyObjectReadFailure)?;

        let obj = obj_read
            .into_object()
            .map_err(Error::IkaObjectRefFailure)?
            .bcs
            .ok_or_else(|| {
        ...
        match obj {
            IkaRawData::Package(pkg) => Ok(pkg),
            IkaRawData::MoveObject(move_obj) => {
