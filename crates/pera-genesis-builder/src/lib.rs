...
use pera_types::crypto::{
    ...
    AuthoritySignature, DefaultHash, PeraAuthoritySignature,
};
...
impl Builder {
    ...
    pub fn add_validator_signature(mut self, keypair: &AuthorityKeyPair) -> Self {
        ...
        let checkpoint_signature = {
            let intent_msg = IntentMessage::new(
                Intent::pera_app(IntentScope::CheckpointSummary),
                checkpoint.clone(),
            );
        ...
        let token_distribution_schedule =
            if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
                ...
                TokenDistributionSchedule::new_for_validators_with_default_allocation(
                    validators.iter().map(|v| v.info.pera_address()),
                )
            };
    ...
    fn committee(objects: &[Object]) -> Committee {
        let pera_system_object =
            get_pera_system_state(&objects).expect("Pera System State object must always exist");
        pera_system_object
    ...
    fn validate_inputs(&self) -> anyhow::Result<(), anyhow::Error> {
        ...
        if let Some(token_distribution_schedule) = &self.token_distribution_schedule {
            ...
            token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
                self.validators.values().map(|v| v.info.pera_address()),
            );
        }
    ...
    fn validate_output(&self) {
        ...
        let system_state = match unsigned_genesis.pera_system_object() {
            PeraSystemState::V1(inner) => inner,
            PeraSystemState::V2(_) => unreachable!(),
            #[cfg(msim)]
            _ => {
            ...
            .zip(system_state.validators.active_validators.iter())
        {
            ...
            assert!(address_to_pool_id
                .insert(metadata.pera_address, onchain_validator.staking_pool.id)
                .is_none());
            assert_eq!(validator.info.pera_address(), metadata.pera_address);
            assert_eq!(validator.info.protocol_key(), metadata.pera_pubkey_bytes());
        ...
        assert_eq!(
            ...
            token_distribution_schedule.stake_subsidy_fund_npera
        );
        ...
        let mut gas_objects: BTreeMap<ObjectID, (&Object, GasCoin)> = unsigned_genesis
            ...
            .collect();
        let mut staked_pera_objects: BTreeMap<ObjectID, (&Object, StakedPera)> = unsigned_genesis
            .objects()
            .iter()
            .filter_map(|o| StakedPera::try_from(o).ok().map(|s| (o.id(), (o, s))))
            .collect();
        ...
        for allocation in token_distribution_schedule.allocations {
            if let Some(staked_with_validator) = allocation.staked_with_validator {
                ...
                let staked_pera_object_id = staked_pera_objects
                    .iter()
                    .find(|(_k, (o, s))| {
                        ...
                        *owner == allocation.recipient_address
                            && s.principal() == allocation.amount_npera
                ...
                assert_eq!(staked_pera_object.1.principal(), allocation.amount_npera);
                assert_eq!(staked_pera_object.1.pool_id(), staking_pool_id);
                assert_eq!(staked_pera_object.1.activation_epoch(), 0);
            } else {
                let gas_object_id = gas_objects
                    ...
                    .find(|(_k, (o, g))| {
                        if let Owner::AddressOwner(owner) = &o.owner {
                            *owner == allocation.recipient_address
                                && g.value() == allocation.amount_npera
                        } else {
                            false
                        }
                    })
                ...
                assert_eq!(gas_object.1.value(), allocation.amount_npera,);
            }
        }
        ...
        if !self.parameters.allow_insertion_of_extra_objects {
            ...
            assert!(staked_pera_objects.is_empty());
        }
...
fn get_genesis_protocol_config(version: ProtocolVersion) -> ProtocolConfig {
    ...
    // PERA_PROTOCOL_CONFIG_CHAIN_OVERRIDE if necessary
...
fn build_unsigned_genesis_data(
    ...
    token_distribution_schedule.validate();
    token_distribution_schedule.check_all_stake_operations_are_for_valid_validators(
        genesis_validators.iter().map(|v| v.pera_address),
    );
    ...
    let mut system_packages =
        pera_framework_snapshot::load_bytecode_snapshot(parameters.protocol_version.as_u64())
...
// on `pera_system` which is mocked in some tests, but would be in the loader
    ...
    let genesis_transaction = {
        let genesis_objects = objects
            ...
            .map(|mut object| {
                ...
                pera_types::transaction::GenesisObject::RawObject {
                    data: object.data,
                    owner: object.owner,
                }
            })
        ...
        pera_types::transaction::VerifiedTransaction::new_genesis_transaction(genesis_objects)
            .into_inner()
    };
...
pub fn generate_genesis_system_object(
    ...
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        // Step 1: Create the PeraSystemState UID
        let pera_system_state_uid = builder.programmable_move_call(
            PERA_FRAMEWORK_ADDRESS.into(),
            ident_str!("object").to_owned(),
            ident_str!("pera_system_state").to_owned(),
            vec![],
            vec![],
        );
        ...
        if protocol_config.enable_bridge() {
            ...
            let bridge_chain_id = builder.pure(BridgeChainId::PeraCustom).unwrap();
        ...
        // Step 4: Mint the supply of PERA.
        let pera_supply = builder.programmable_move_call(
            PERA_FRAMEWORK_ADDRESS.into(),
            ident_str!("pera").to_owned(),
        ...
        // Step 5: Run genesis.
        // The first argument is the system state uid we got from step 1 and the second one is the PERA supply we
        // got from step 3.
        let mut arguments = vec![pera_system_state_uid, pera_supply];
...
mod test {
    ...
    use pera_config::node::DEFAULT_COMMISSION_RATE;
    use pera_config::node::DEFAULT_VALIDATOR_GAS_PRICE;
