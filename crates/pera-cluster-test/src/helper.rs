...
use pera_json_rpc_types::{BalanceChange, PeraData, PeraObjectData, PeraObjectDataOptions};
use pera_sdk::PeraClient;
use pera_types::error::PeraObjectResponseError;
use pera_types::gas_coin::GasCoin;
use pera_types::{base_types::ObjectID, object::Owner, parse_pera_type_tag};
use tracing::{debug, trace};

/// A util struct that helps verify Pera Object.
...
/// `check_into_gas_coin` expect to get a `PeraObjectData` and `GasCoin`
/// respectfully.
#[derive(Debug)]
...
impl ObjectChecker {
    ...
    pub fn is_pera_coin(mut self, is_pera_coin: bool) -> Self {
        self.is_pera_coin = Some(is_pera_coin);
        self
    }

    pub async fn check_into_gas_coin(self, client: &PeraClient) -> GasCoin {
        if self.is_pera_coin == Some(false) {
            panic!("'check_into_gas_coin' shouldn't be called with 'is_pera_coin' set as false");
        }
        self.is_pera_coin(true)
            .check(client)
            .await
            .unwrap()
            .into_gas_coin()
    ...
    pub async fn check_into_object(self, client: &PeraClient) -> PeraObjectData {
        self.check(client).await.unwrap().into_object()
    }

    pub async fn check(self, client: &PeraClient) -> Result<CheckerResultObject, anyhow::Error> {
        debug!(?self);
        ...
        match (object_info.data, object_info.error) {
            (None, Some(PeraObjectResponseError::NotExists { object_id })) => {
                panic!(
            ...
            (
                None,
                Some(PeraObjectResponseError::DynamicFieldNotFound {
            ...
            (Some(object), _) => {
                ...
                if self.is_pera_coin == Some(true) {
                    let move_obj = object
                        .bcs
                        .as_ref()
            ...
            (None, Some(PeraObjectResponseError::DisplayError { error })) => {
                panic!("Display Error: {error:?}");
            }
            (None, None) | (None, Some(PeraObjectResponseError::Unknown)) => {
...
pub struct CheckerResultObject {
    ...
    object: Option<PeraObjectData>,
}

impl CheckerResultObject {
    pub fn new(gas_coin: Option<GasCoin>, object: Option<PeraObjectData>) -> Self {
        Self { gas_coin, object }
    }
    pub fn into_gas_coin(self) -> GasCoin {
        self.gas_coin.unwrap()
    }
    pub fn into_object(self) -> PeraObjectData {
        self.object.unwrap()
    }
}
...
impl BalanceChangeChecker {
    ...
    pub fn coin_type(mut self, coin_type: &str) -> Self {
        self.coin_type = Some(parse_pera_type_tag(coin_type).unwrap());
        self
    }
