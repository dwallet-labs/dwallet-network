...
use pera_json_rpc_types::{Balance, PeraTransactionBlockResponseOptions};
...
impl TestCaseImpl for CoinIndexTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        ctx.get_pera_from_faucet(None).await;
        ...
        assert_eq!(
            ...
            "total_balance: {}, old_total_balance: {}, pera_balance_change.amount: {}",
            total_balance,
            old_total_balance,
        ...
        let pera_type_str = "0x2::pera::PERA";
        ...
        let args = vec![
            PeraJsonValue::from_object_id(cap.0),
            PeraJsonValue::new(json!("10000"))?,
            PeraJsonValue::new(json!(account))?,
        ];
        let txn = client
            .transaction_builder()
            .move_call(
        ...
        let pera_balance_change = balance_changes
            .iter()
            .find(|b| b.coin_type.to_string().contains("PERA"))
            .unwrap();
        ...
        assert_eq!(pera_balance_change.owner, Owner::AddressOwner(account));
        ...
        assert_eq!(
            total_balance,
            (old_total_balance as i128 + pera_balance_change.amount) as u128,
            "total_balance: {}, old_total_balance: {}, pera_balance_change.amount: {}",
            total_balance,
            old_total_balance,
            pera_balance_change.amount
        );
        old_coin_object_count = coin_object_count;
        ...
        let args = vec![PeraJsonValue::from_object_id(envelope.0)];
        let txn = client
            .transaction_builder()
            .move_call(
                account,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    PeraJsonValue::from_object_id(cap.0),
                    PeraJsonValue::from_object_id(envelope.0),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    PeraJsonValue::from_object_id(cap.0),
                    PeraJsonValue::from_object_id(managed_coin_id_10k),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let pera_coins = client
            .coin_read_api()
            .get_coins(account, Some(pera_type_str.into()), None, None)
            .await
            .unwrap()
            .data;
        ...
        assert_eq!(
            // this is only PERA coins at the moment
            pera_coins,
            client
                .coin_read_api()
        ...
        let pera_balance = client
            .coin_read_api()
            .get_balance(account, None)
            .await
            .unwrap();
        assert_eq!(
            pera_balance.total_balance,
            pera_coins.iter().map(|c| c.balance as u128).sum::<u128>()
        );
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    PeraJsonValue::from_object_id(cap.0),
                    PeraJsonValue::new(json!("5"))?,  // balance = 5
                    PeraJsonValue::new(json!("40"))?, // num = 40
                    PeraJsonValue::new(json!(account))?,
                ],
                None,
                rgp * 2_000_000,
        ...
        assert_eq!(
            ...
            client
                .coin_read_api()
                .get_coins(account, None, None, Some(pera_coins.len() + 1))
                .await
                .unwrap()
                .data,
        );
        ...
        assert_eq!(pera_coins.len() + managed_coins.len(), total_coins,);

        let pera_coins_with_managed_coin_1 = client
            .coin_read_api()
            .get_all_coins(account, None, Some(pera_coins.len() + 1))
            .await
            .unwrap();
        assert_eq!(
            pera_coins_with_managed_coin_1.data.len(),
            pera_coins.len() + 1
        );
        assert_eq!(
            pera_coins_with_managed_coin_1.next_cursor,
            Some(first_managed_coin)
        );
        assert!(pera_coins_with_managed_coin_1.has_next_page);
        let cursor = pera_coins_with_managed_coin_1.next_cursor;
...
async fn add_to_envelope(
    ...
    let txn = client
        ...
        .move_call(
            ...
            vec![],
            vec![
                PeraJsonValue::from_object_id(envelope),
                PeraJsonValue::from_object_id(coin),
            ],
            None,
            rgp * 2_000_000,
            None,
