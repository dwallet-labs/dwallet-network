...
use ika_types::storage::{ReadStore, RestStateReader};
use ika_types::{
    base_types::{ObjectID, SequenceNumber, IkaAddress, VersionNumber},
...
pub struct PersistedStoreInner {
    // Checkpoint data
    checkpoints: DBMap<CheckpointSequenceNumber, ika_types::messages_checkpoint::TrustedCheckpoint>,
    ...
    // Transaction data
    transactions: DBMap<TransactionDigest, ika_types::transaction::TrustedTransaction>,
...
impl SimulatorStore for PersistedStore {
    ...
    fn get_system_state(&self) -> ika_types::ika_system_state::IkaSystemState {
        ika_types::ika_system_state::get_ika_system_state(self).expect("system state must exist")
    }

    fn get_clock(&self) -> ika_types::clock::Clock {
        SimulatorStore::get_object(self, &ika_types::IKA_CLOCK_OBJECT_ID)
    ...
    fn backing_store(&self) -> &dyn ika_types::storage::BackingStore {
        self
    }
}
...
impl BackingPackageStore for PersistedStore {
    fn get_package_object(
        &self,
        package_id: &ObjectID,
    ) -> ika_types::error::IkaResult<Option<PackageObject>> {
...
impl ChildObjectResolver for PersistedStore {
    ...
    ) -> ika_types::error::IkaResult<Option<Object>> {
...
impl ParentSync for PersistedStore {
    fn get_latest_parent_entry_ref_deprecated(
        &self,
        _object_id: ObjectID,
    ) -> ika_types::error::IkaResult<Option<ika_types::base_types::ObjectRef>> {
