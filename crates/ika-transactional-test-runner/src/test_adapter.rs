...
use ika_framework::DEFAULT_FRAMEWORK_PATH;
use ika_graphql_rpc::test_infra::cluster::ExecutorCluster;
use ika_graphql_rpc::test_infra::cluster::{serve_executor, RetentionConfig, SnapshotLagConfig};
use ika_json_rpc_api::QUERY_MAX_RESULT_LIMIT;
use ika_json_rpc_types::{DevInspectResults, IkaExecutionStatus, IkaTransactionBlockEffectsAPI};
use ika_protocol_config::{Chain, ProtocolConfig};
use ika_storage::{
    key_value_store::TransactionKeyValueStore, key_value_store_metrics::KeyValueStoreMetrics,
};
use ika_swarm_config::genesis_config::AccountConfig;
use ika_types::base_types::{SequenceNumber, VersionNumber};
use ika_types::committee::EpochId;
use ika_types::crypto::{get_authority_key_pair, RandomnessRound};
use ika_types::digests::{ConsensusCommitDigest, TransactionDigest, TransactionEventsDigest};
...
use ika_types::utils::to_sender_signed_transaction_with_multi_signers;
use ika_types::IKA_SYSTEM_ADDRESS;
use ika_types::{
    base_types::{ObjectID, ObjectRef, IkaAddress, IKA_ADDRESS_LENGTH},
    crypto::{get_key_pair_from_rng, AccountKeyPair},
    event::Event,
    object::{self, Object},
    transaction::{Transaction, TransactionData, TransactionDataAPI, VerifiedTransaction},
    MOVE_STDLIB_ADDRESS, IKA_CLOCK_OBJECT_ID, IKA_FRAMEWORK_ADDRESS, IKA_SYSTEM_STATE_OBJECT_ID,
};
use ika_types::{execution_status::ExecutionStatus, transaction::TransactionKind};
use ika_types::{gas::GasCostSummary, object::GAS_VALUE_FOR_TESTING};
use ika_types::{
    move_package::MovePackage,
    transaction::{Argument, CallArg},
};
use ika_types::{
    programmable_transaction_builder::ProgrammableTransactionBuilder, IKA_FRAMEWORK_PACKAGE_ID,
};
use ika_types::{utils::to_sender_signed_transaction, IKA_SYSTEM_PACKAGE_ID};
use ika_types::{BRIDGE_ADDRESS, MOVE_STDLIB_PACKAGE_ID};
use ika_types::{DEEPBOOK_ADDRESS, IKA_DENY_LIST_OBJECT_ID};
use ika_types::{DEEPBOOK_PACKAGE_ID, IKA_RANDOMNESS_STATE_OBJECT_ID};
...
const WELL_KNOWN_OBJECTS: &[ObjectID] = &[
    ...
    IKA_CLOCK_OBJECT_ID,
    IKA_DENY_LIST_OBJECT_ID,
    IKA_RANDOMNESS_STATE_OBJECT_ID,
];
...
#[async_trait]
impl<'a> MoveTestAdapter<'a> for IkaTestAdapter {
    type ExtraPublishArgs = IkaPublishArgs;
    type ExtraRunArgs = IkaRunArgs;
    type ExtraInitArgs = IkaInitArgs;
    type ExtraValueArgs = IkaExtraValueArgs;
    type Subcommand = IkaSubcommand<Self::ExtraValueArgs, Self::ExtraRunArgs>;
    ...
    fn render_command_input(
        ...
        match &task.command {
            TaskCommand::Subcommand(IkaSubcommand::ProgrammableTransaction(..)) => {
    ...
    async fn init(
        ...
        assert!(
            ...
            "Must populate 'pre_compiled_deps' with Ika framework"
        );
    ...
    async fn publish_modules(
        ...
        // we are assuming that all packages depend on Move Stdlib and Ika Framework, so these
        // don't have to be provided explicitly as parameters
        dependencies.extend([MOVE_STDLIB_PACKAGE_ID, IKA_FRAMEWORK_PACKAGE_ID]);
        let data = |sender, gas| {
    ...
    async fn call_function(
        ...
        self.next_task();
        let IkaRunArgs { summarize, .. } = extra;
    ...
    async fn handle_subcommand(
        ...
        match command {
            IkaSubcommand::RunGraphql(RunGraphqlCommand {
                show_usage,
                show_headers,
            ...
            IkaSubcommand::CreateCheckpoint(CreateCheckpointCommand { count }) => {
            ...
            IkaSubcommand::AdvanceEpoch(AdvanceEpochCommand {
                count,
                create_random_state,
            }) => {
                for _ in 0..count.unwrap_or(1) {
            ...
            IkaSubcommand::AdvanceClock(AdvanceClockCommand { duration_ns }) => {
                self.executor
                    .advance_clock(Duration::from_nanos(duration_ns))
                    .await?;
                Ok(None)
            }
            IkaSubcommand::SetRandomState(SetRandomStateCommand {
                randomness_round,
                random_bytes,
            ...
            IkaSubcommand::ViewObject(ViewObjectCommand { id: fake_id }) => {
            ...
            IkaSubcommand::TransferObject(TransferObjectCommand {
                id: fake_id,
                recipient,
                sender,
                gas_budget,
                ...
                let mut builder = ProgrammableTransactionBuilder::new();
                let obj_arg = IkaValue::Object(fake_id, None).into_argument(&mut builder, self)?;
                ...
                let transaction = self.sign_txn(sender, |sender, gas| {
                    ...
                    builder.command(ika_types::transaction::Command::TransferObjects(
                        vec![obj_arg],
                        rec_arg,
                    ));
                    let pt = builder.finish();
            ...
            IkaSubcommand::ConsensusCommitPrologue(ConsensusCommitPrologueCommand {
                timestamp_ms,
            }) => {
            ...
            IkaSubcommand::ProgrammableTransaction(ProgrammableTransactionCommand {
                sender,
                sponsor,
                gas_budget,
                gas_price,
            ...
            IkaSubcommand::UpgradePackage(UpgradePackageCommand {
                package,
                upgrade_capability,
                dependencies,
                sender,
            ...
            IkaSubcommand::StagePackage(StagePackageCommand {
                syntax,
                dependencies,
            }) => {
                let syntax = syntax.unwrap_or_else(|| self.default_syntax());
            ...
            IkaSubcommand::SetAddress(SetAddressCommand { address, input }) => {
                ...
                let (value, package) = match input {
                    ...
                    IkaValue::MoveValue(v) => {
                        let bytes = v.simple_serialize().unwrap();
                        let value: AccountAddress = bcs::from_bytes(&bytes)?;
                        (value, None)
                    }
                    IkaValue::Digest(_) => bail!("digest is not supported as an input"),
                    IkaValue::ObjVec(_) => bail!("obj vec is not supported as an input"),
                    IkaValue::Receiving(_, _) => bail!("receiving is not supported as an input"),
                    IkaValue::ImmShared(_, _) => {
...
impl<'a> IkaTestAdapter {
    ...
    async fn upgrade_package(
        ...
        // Argument::Input(0)
        IkaValue::Object(upgrade_capability, None).into_argument(&mut builder, self)?;
    ...
    fn sign_txn(
        ...
        txn_data: impl FnOnce(/* sender */ IkaAddress, /* gas */ ObjectRef) -> TransactionData,
    ) -> Transaction {
    ...
    async fn dev_inspect(
        ...
        match effects.status() {
            ...
            IkaExecutionStatus::Failure { error } => Err(anyhow::anyhow!(self.stabilize_str(
    ...
    fn stabilize_str(&self, input: impl AsRef<str>) -> String {
        fn candidate_is_hex(s: &str) -> bool {
            const HEX_STR_LENGTH: usize = IKA_ADDRESS_LENGTH * 2;
            let n = s.len();
...
impl<'a> GetModule for &'a IkaTestAdapter {
...
static NAMED_ADDRESSES: Lazy<BTreeMap<String, NumericalAddress>> = Lazy::new(|| {
    ...
    // TODO fix Ika framework constants
    map.insert(
        "ika".to_string(),
        NumericalAddress::new(
            IKA_FRAMEWORK_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
        "ika_system".to_string(),
        NumericalAddress::new(
            IKA_SYSTEM_ADDRESS.into_bytes(),
            move_compiler::shared::NumberFormat::Hex,
        ),
    );
    map.insert(
...
pub static PRE_COMPILED: Lazy<FullyCompiledProgram> = Lazy::new(|| {
    ...
    let ika_files: &Path = Path::new(DEFAULT_FRAMEWORK_PATH);
    let ika_system_sources = {
        let mut buf = ika_files.to_path_buf();
        buf.extend(["packages", "ika-system", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let ika_sources = {
        let mut buf = ika_files.to_path_buf();
        buf.extend(["packages", "ika-framework", "sources"]);
        buf.to_string_lossy().to_string()
    };
    let ika_deps = {
        let mut buf = ika_files.to_path_buf();
    ...
    let fully_compiled_res = move_compiler::construct_pre_compiled_lib(
        vec![PackagePaths {
            name: Some(("ika-framework".into(), config)),
            paths: vec![
                ika_system_sources,
                ika_sources,
                ika_deps,
    ...
    match fully_compiled_res {
        Err((files, diags)) => {
            eprintln!("!!!Ika framework failed to compile!!!");
...
async fn init_val_fullnode_executor(
    ...
    // For each named Ika account without an address value, create an account with an address
    // and a gas object
...
async fn init_sim_executor(
    ...
    // For each named Ika account without an address value, create a key pair
    for n in account_names {
    ...
    if custom_validator_account {
        ...
        let (a, b): (IkaAddress, Ed25519KeyPair) = get_key_pair_from_rng(&mut rng);
...
impl ObjectStore for IkaTestAdapter {
    fn get_object(
        &self,
        object_id: &ObjectID,
...
impl ReadStore for IkaTestAdapter {
    ...
    fn get_committee(
        &self,
        epoch: ika_types::committee::EpochId,
    ...
    fn get_checkpoint_by_digest(
        &self,
        digest: &ika_types::messages_checkpoint::CheckpointDigest,
    ...
    ) -> ika_types::storage::error::Result<Option<CheckpointContents>> {
