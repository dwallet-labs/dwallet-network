...
use sui_json_rpc_types::{Coin, SuiObjectDataOptions, SuiTransactionBlockResponseOptions};
...
// todo(zeev): in sync and modified to remove useless code.
fn read_checkpoint(
    config: &Config,
    seq: u64,
...
async fn update_state_proof_registry_in_dwallet_network(
    ...
    let coin_gas = max_coin_for_addr(&dwallet_client, &sender).await?;
...
async fn max_coin_for_addr(dwallet_client: &SuiClient, sender: &SuiAddress) -> Result<Coin> {
    ...
    loop {
        let coins = dwallet_client
            ...
            .await
            .context("Failed to retrieve coins")?;
...
// todo(yuval): detect the out of gas error in `sync` and request tokens.
// todo(yuval): add retry to sync on 4xx errors.
...
async fn init_light_client(
    ...
    let coins = dwallet_client
        ...
        .await
        .context(format!("Failed to retrieve coins for address: {sender}"))?;
    let coin_gas = coins
        .data
        .iter()
