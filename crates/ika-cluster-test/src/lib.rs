...
impl TestContext {
    async fn get_ika_from_faucet(&self, minimum_coins: Option<usize>) -> Vec<GasCoin> {
        let addr = self.get_wallet_address();
        let faucet_response = self.faucet.request_ika_coins(addr).await;
        ...
        if gas_coins.len() < minimum_coins {
            panic!(
                "Expect to get at least {minimum_coins} Ika Coins for address {addr}, but only got {}",
                gas_coins.len()
            )
        }
    ...
    fn get_fullnode_client(&self) -> &IkaClient {
        self.client.get_fullnode_client()
    }

    fn clone_fullnode_client(&self) -> IkaClient {
    ...
    fn get_wallet_address(&self) -> IkaAddress {
    ...
    async fn get_tx_with_retry_times(
        ...
        match self
            ...
            .get_fullnode_client()
            .read_api()
            .get_transaction_with_options(digest, IkaTransactionBlockResponseOptions::new())
            .await
        {
