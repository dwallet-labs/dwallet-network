...
use ika_core::test_utils::compile_managed_coin_package;
use ika_json::IkaJsonValue;
use ika_json_rpc_types::ObjectChange;
use ika_json_rpc_types::IkaTransactionBlockResponse;
use ika_json_rpc_types::{Balance, IkaTransactionBlockResponseOptions};
...
impl TestCaseImpl for CoinIndexTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        assert_eq!(
            ...
            "total_balance: {}, old_total_balance: {}, ika_balance_change.amount: {}",
            total_balance,
            old_total_balance,
        ...
        let args = vec![
            IkaJsonValue::from_object_id(cap.0),
            IkaJsonValue::new(json!("10000"))?,
            IkaJsonValue::new(json!(account))?,
        ];
        let txn = client
            .transaction_builder()
            .move_call(
        ...
        let ika_balance_change = balance_changes
            .iter()
            .find(|b| b.coin_type.to_string().contains("IKA"))
            .unwrap();
        ...
        assert_eq!(ika_balance_change.owner, Owner::AddressOwner(account));
        ...
        assert_eq!(
            total_balance,
            (old_total_balance as i128 + ika_balance_change.amount) as u128,
            "total_balance: {}, old_total_balance: {}, ika_balance_change.amount: {}",
            total_balance,
            old_total_balance,
            ika_balance_change.amount
        );
        old_coin_object_count = coin_object_count;
        ...
        let args = vec![IkaJsonValue::from_object_id(envelope.0)];
        let txn = client
            .transaction_builder()
            .move_call(
                account,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    IkaJsonValue::from_object_id(cap.0),
                    IkaJsonValue::from_object_id(envelope.0),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    IkaJsonValue::from_object_id(cap.0),
                    IkaJsonValue::from_object_id(managed_coin_id_10k),
                ],
                None,
                rgp * 2_000_000,
                None,
        ...
        let ika_coins = client
            .coin_read_api()
            .get_coins(account, Some(ika_type_str.into()), None, None)
            .await
            .unwrap()
            .data;
        ...
        assert_eq!(
            // this is only IKA coins at the moment
            ika_coins,
            client
                .coin_read_api()
        ...
        let ika_balance = client
            .coin_read_api()
            .get_balance(account, None)
            .await
            .unwrap();
        assert_eq!(
            ika_balance.total_balance,
            ika_coins.iter().map(|c| c.balance as u128).sum::<u128>()
        );
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![
                    IkaJsonValue::from_object_id(cap.0),
                    IkaJsonValue::new(json!("5"))?,  // balance = 5
                    IkaJsonValue::new(json!("40"))?, // num = 40
                    IkaJsonValue::new(json!(account))?,
                ],
                None,
                rgp * 2_000_000,
        ...
        assert_eq!(
            ...
            client
                .coin_read_api()
                .get_coins(account, None, None, Some(ika_coins.len() + 1))
                .await
                .unwrap()
                .data,
        );
        ...
        assert_eq!(ika_coins.len() + managed_coins.len(), total_coins,);

        let ika_coins_with_managed_coin_1 = client
            .coin_read_api()
            .get_all_coins(account, None, Some(ika_coins.len() + 1))
            .await
            .unwrap();
        assert_eq!(
            ika_coins_with_managed_coin_1.data.len(),
            ika_coins.len() + 1
        );
        assert_eq!(
            ika_coins_with_managed_coin_1.next_cursor,
            Some(first_managed_coin)
        );
        assert!(ika_coins_with_managed_coin_1.has_next_page);
        let cursor = ika_coins_with_managed_coin_1.next_cursor;
...
async fn add_to_envelope(
    ...
    let txn = client
        ...
        .move_call(
            ...
            vec![],
            vec![
                IkaJsonValue::from_object_id(envelope),
                IkaJsonValue::from_object_id(coin),
            ],
            None,
            rgp * 2_000_000,
            None,
