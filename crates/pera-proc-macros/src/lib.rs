    ...
    input.block = syn::parse2(quote! {
        {
            ...
            std::thread::spawn(|| {
                ...
                ::pera_simulator::telemetry_subscribers::init_for_testing();
                ::pera_simulator::pera_types::execution::get_denied_certificates();
                ::pera_simulator::pera_framework::BuiltInFramework::all_package_ids();
                ::pera_simulator::pera_types::gas::PeraGasStatus::new_unmetered();
                ...
                let mut cache = ::pera_simulator::lru::LruCache::new(1.try_into().unwrap());
                cache.put(1, 1);

                {
                    ...
                    use pera_simulator::pera_move_build::{BuildConfig, PeraPackageHooks};
                    use pera_simulator::tempfile::TempDir;
                    use pera_simulator::move_package::package_hooks::register_package_hooks;

                    register_package_hooks(Box::new(PeraPackageHooks {}));
                ...
                use ::pera_simulator::anemo_tower::callback::CallbackLayer;
                use ::pera_simulator::anemo_tower::trace::DefaultMakeSpan;
                use ::pera_simulator::anemo_tower::trace::DefaultOnFailure;
                use ::pera_simulator::anemo_tower::trace::TraceLayer;
                use ::pera_simulator::narwhal_network::metrics::MetricsMakeCallbackHandler;
                use ::pera_simulator::narwhal_network::metrics::NetworkMetrics;

                use std::sync::Arc;
                use ::pera_simulator::fastcrypto::traits::KeyPair;
                use ::pera_simulator::rand_crate::rngs::{StdRng, OsRng};
                use ::pera_simulator::rand::SeedableRng;
                use ::pera_simulator::tower::ServiceBuilder;
                ...
                let rt = ::pera_simulator::runtime::Runtime::new();
                rt.block_on(async move {
                    use ::pera_simulator::anemo::{Network, Request};

                    let make_network = |port: u16| {
                        let registry = prometheus::Registry::new();
                        let inbound_network_metrics =
                            NetworkMetrics::new("pera", "inbound", &registry);
                        let outbound_network_metrics =
                            NetworkMetrics::new("pera", "outbound", &registry);
                        ...
                        let service = ServiceBuilder::new()
                            ...
                            .service(::pera_simulator::anemo::Router::new());
                        ...
                        Network::bind(format!("127.0.0.1:{}", port))
                            ...
                            .private_key(
                                ::pera_simulator::fastcrypto::ed25519::Ed25519KeyPair::generate(&mut StdRng::from_rng(OsRng).unwrap())
                                    .private()
                                    .0
                                    .to_bytes(),
                            )
...
/// The pera_test macro will invoke either `#[msim::test]` or `#[tokio::test]`,
...
#[proc_macro_attribute]
pub fn pera_test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let header = if cfg!(msim) {
        quote! {
            #[::pera_simulator::sim_test(crate = "pera_simulator", #(#args)* )]
        }
    } else {
        quote! {
            #[::tokio::test(#(#args)*)]
    ...
    let result = quote! {
        #header
        #[::pera_macros::init_static_initializers]
        #input
    };
    ...
    let result = if cfg!(msim) {
        ...
        quote! {
            ...
            #sig {
                ...
                ::pera_simulator::task::shutdown_all_nodes();
                ...
                assert_eq!(
                    pera_simulator::NodeLeakDetector::get_current_node_count(),
                    0,
                    "PeraNode leak detected"
                );
        ...
        quote! {
            ...
            #ignore
            #sig {
                if std::env::var("PERA_SKIP_SIMTESTS").is_ok() {
                    println!("not running test {} in `cargo test`: PERA_SKIP_SIMTESTS is set", stringify!(#fn_name));
...
pub fn enum_variant_order_derive(input: TokenStream) -> TokenStream {
    ...
    if let Data::Enum(DataEnum { variants, .. }) = ast.data {
        ...
        let deriv = quote! {
            impl pera_enum_compat_util::EnumOrderMap for #name {
