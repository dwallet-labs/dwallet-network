...
#[derive(Parser)]
#[clap(
    name = "Ika RPC Load Generator",
    version = "0.1",
    about = "A load test application for Ika RPC"
)]
struct Opts {
    ...
    #[clap(long, default_value = "~/.ika/ika_config/logs")]
    logs_directory: String,

    #[clap(long, default_value = "~/.ika/loadgen/data")]
...
fn get_keypair() -> Result<SignerInfo> {
    ...
    // Load keystore from ~/.ika/ika_config/ika.keystore
    let keystore_path = get_ika_config_directory().join("ika.keystore");
    let keystore = Keystore::from(FileBasedKeystore::new(&keystore_path)?);
    let active_address = keystore.addresses().pop().unwrap();
    let keypair: &IkaKeyPair = keystore.get_key(&active_address)?;
    println!("using address {active_address} for signing");
    Ok(SignerInfo::new(keypair.encode_base64()))
}

fn get_ika_config_directory() -> PathBuf {
    match dirs::home_dir() {
        Some(v) => v.join(".ika").join("ika_config"),
...
fn get_log_file_path(dir_path: String) -> String {
    ...
    let log_filename = format!("ika-rpc-loadgen.{}.log", timestamp);
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (command, common, need_keystore) = match opts.command {
        ...
        ClapCommand::PayIka { common } => (Command::new_pay_ika(), common, true),
