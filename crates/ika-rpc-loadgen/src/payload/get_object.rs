...
use ika_json_rpc_types::{IkaObjectDataOptions, IkaObjectResponse};
...
pub(crate) async fn get_object(
    client: &IkaClient,
    object_id: ObjectID,
) -> Result<IkaObjectResponse> {
    let result = client
        .read_api()
        .get_object_with_options(object_id, IkaObjectDataOptions::full_content())
        .await
        .unwrap();
    Ok(result)
}
