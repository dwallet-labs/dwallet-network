...
use ika_json_rpc_types::{
    Page, IkaTransactionBlockResponse, IkaTransactionBlockResponseOptions,
...
impl<'a> ProcessPayload<'a, &'a QueryTransactionBlocks> for RpcCommandProcessor {
    async fn process(
        ...
        let queries: Vec<IkaTransactionBlockResponseQuery> = filters
            .into_iter()
            .map(|filter| IkaTransactionBlockResponseQuery {
                filter,
...
) -> Result<Page<IkaTransactionBlockResponse, TransactionDigest>> {
