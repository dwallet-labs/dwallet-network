...
use ika_json_rpc_types::{
    IkaExecutionStatus, IkaObjectDataOptions, IkaTransactionBlockDataAPI,
    IkaTransactionBlockEffectsAPI, IkaTransactionBlockResponse, IkaTransactionBlockResponseOptions,
};
...
use ika_types::crypto::{get_key_pair, AccountKeyPair, EncodeDecodeBase64, Signature, IkaKeyPair};
...
pub struct RpcCommandProcessor {
    clients: Arc<RwLock<Vec<IkaClient>>>,
    ...
    addresses: Arc<DashSet<IkaAddress>>,
    data_dir: String,
}
...
impl RpcCommandProcessor {
    ...
    async fn process_command_data(
        ...
        match command {
            ...
            CommandData::PayIka(ref v) => self.process(v, signer_info).await,
    ...
    pub(crate) async fn get_clients(&self) -> Result<Vec<IkaClient>> {
    ...
    pub(crate) fn add_addresses_from_response(&self, responses: &[IkaTransactionBlockResponse]) {
    ...
    pub(crate) fn add_object_ids_from_response(&self, responses: &[IkaTransactionBlockResponse]) {
        for response in responses {
    ...
    pub(crate) fn dump_cache_to_file(&self) {
        ...
        let addresses: Vec<IkaAddress> = self.addresses.iter().map(|x| *x).collect();
        if !addresses.is_empty() {
            debug!("dumping addresses to file {:?}", addresses.len());
            write_data_to_file(
                &addresses,
                &format!("{}/{}", &self.data_dir, CacheType::IkaAddress),
            )
            .unwrap();
        }
...
impl fmt::Display for CacheType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CacheType::IkaAddress => write!(f, "IkaAddress"),
...
pub fn load_addresses_from_file(filepath: String) -> Vec<IkaAddress> {
    let path = format!("{}/{}", filepath, CacheType::IkaAddress);
    let addresses: Vec<IkaAddress> = read_data_from_file(&path).expect("Failed to read addresses");
    addresses
}
...
async fn prepare_new_signer_and_coins(
    ...
    let (gas_fee_for_split, gas_fee_for_pay_ika) = (
        DEFAULT_LARGE_GAS_BUDGET * num_split_txns as u64,
        DEFAULT_GAS_BUDGET,
    );

    let primary_keypair = IkaKeyPair::decode_base64(&signer_info.encoded_keypair)
        .expect("Decoding keypair should not fail");
    let sender = IkaAddress::from(&primary_keypair.public());
    let (coin, balance) = get_coin_with_max_balance(client, sender).await;
    // The balance needs to cover `pay_amount` plus
    // 1. gas fee for pay_ika from the primary address to the burner address
    // 2. gas fee for splitting the primary coin into `num_coins`
    let required_balance = pay_amount + gas_fee_for_split + gas_fee_for_pay_ika;
    if required_balance > balance {
        panic!("Current balance {balance} is smaller than require amount of NIKA to fund the operation {required_balance}");
    }
    ...
    let burner_keypair = IkaKeyPair::Ed25519(burner_keypair);
    ...
    let coins = get_ika_coin_ids(client, burner_address).await;
...
async fn get_coin_with_max_balance(client: &IkaClient, address: IkaAddress) -> (ObjectID, u64) {
    let coins = get_ika_coin_ids(client, address).await;
    assert!(!coins.is_empty());
...
async fn get_ika_coin_ids(client: &IkaClient, address: IkaAddress) -> Vec<(ObjectID, u64)> {
    match client
        .coin_read_api()
        ...
        .await
    {
        Ok(page) => page
            ...
            .collect::<Vec<_>>(),
        Err(e) => {
            panic!("get_ika_coin_ids error for address {address} {e}")
        }
    }
...
async fn pay_ika(
    ...
    let tx = client
        ...
        .await
        .expect("Failed to construct pay ika transaction");
    sign_and_execute(
        client,
        keypair,
        tx,
...
pub(crate) async fn sign_and_execute(
    ...
    let signature = Signature::new_secure(
        &IntentMessage::new(Intent::ika_transaction(), &txn_data),
        keypair,
    );
