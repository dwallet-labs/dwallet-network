...
use ika_json_rpc_types::{
    IkaObjectDataOptions, IkaObjectResponse, IkaTransactionBlockEffectsAPI,
    IkaTransactionBlockResponse, IkaTransactionBlockResponseOptions,
};
use ika_sdk::IkaClient;
use ika_types::base_types::{ObjectID, TransactionDigest};
use tracing::error;
...
pub(crate) async fn check_transactions(
    ...
    cross_validate: bool,
    verify_objects: bool,
) -> Vec<Vec<IkaTransactionBlockResponse>> {
    let transactions: Vec<Vec<IkaTransactionBlockResponse>> =
        join_all(clients.iter().map(|client| async move {
            client
                .read_api()
                .multi_get_transactions_with_options(
                    digests.to_vec(),
                    IkaTransactionBlockResponseOptions::full_content(), // todo(Will) support options for this
                )
                .await
        }))
        .await
...
pub(crate) fn get_all_object_ids(response: &IkaTransactionBlockResponse) -> Vec<ObjectID> {
...
pub(crate) async fn multi_get_object(
    clients: &[IkaClient],
    object_ids: &[ObjectID],
) -> Vec<Vec<IkaObjectResponse>> {
    let objects: Vec<Vec<IkaObjectResponse>> = join_all(clients.iter().map(|client| async move {
        ...
        client
            ...
            .multi_get_object_with_options(
                object_ids.to_vec(),
                IkaObjectDataOptions::full_content(), // todo(Will) support options for this
            )
            .await
    }))
    .await
