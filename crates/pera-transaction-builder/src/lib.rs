...
use pera_types::{coin, fp_ensure, PERA_FRAMEWORK_PACKAGE_ID, PERA_SYSTEM_PACKAGE_ID};
...
impl TransactionBuilder {
    ...
    async fn select_gas(
        ...
        if let Some(gas) = input_gas {
            ...
            for obj in gas_objs {
                let response = self
                    .0
                    .get_object_with_options(obj.object_id, PeraObjectDataOptions::new().with_bcs())
                    .await?;
            ...
            Err(anyhow!("Cannot find gas coin for signer address {signer} with amount sufficient for the required gas budget {gas_budget}. If you are using the pay or transfer commands, you can use pay-pera or transfer-pera commands instead, which will use the only object as gas payment."))
        }
    }
    ...
    pub async fn transfer_pera(
        ...
        let object = self.get_object_ref(pera_object_id).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_transfer_pera(
    ...
    pub async fn pay(
        ...
        if let Some(gas) = gas {
            if input_coins.contains(&gas) {
                return Err(anyhow!("Gas coin is in input coins of Pay transaction, use PayPera transaction instead!"));
            }
        }
    ...
    /// Construct a transaction kind for the PayPera transaction type
    ///
    /// Use this function together with tx_data_for_dry_run or tx_data
    /// for maximum reusability
    pub fn pay_pera_tx_kind(
        &self,
        recipients: Vec<PeraAddress>,
        amounts: Vec<u64>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_pera(recipients.clone(), amounts.clone())?;
        let pt = builder.finish();
        let tx_kind = TransactionKind::programmable(pt);
        Ok(tx_kind)
    }

    pub async fn pay_pera(
        &self,
        signer: PeraAddress,
        input_coins: Vec<ObjectID>,
        recipients: Vec<PeraAddress>,
    ...
    pub fn pay_all_pera_tx_kind(&self, recipient: PeraAddress) -> TransactionKind {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn pay_all_pera(
        ...
        let gas_object_ref = coin_refs.remove(0);
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_pay_all_pera(
            signer,
            coin_refs,
            recipient,
            gas_object_ref,
    ...
    pub async fn move_call_tx_kind(
        ...
        module: &str,
        function: &str,
        type_args: Vec<PeraTypeTag>,
        call_args: Vec<PeraJsonValue>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn resolve_and_checks_json_args(
        ...
        function: &Identifier,
        type_args: &[TypeTag],
        json_args: Vec<PeraJsonValue>,
    ) -> Result<Vec<Argument>, anyhow::Error> {
        let object = self
            .0
            .get_object_with_options(package_id, PeraObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
        let Some(PeraRawData::Package(package)) = object.bcs else {
            bail!(
    ...
    pub async fn split_coin_tx_kind(
        ...
        let coin = self
            .0
            .get_object_with_options(coin_object_id, PeraObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
        let coin_object_ref = coin.object_ref();
        let coin: Object = coin.try_into()?;
        let type_args = vec![coin.get_move_template_type()?];
        let package = PERA_FRAMEWORK_PACKAGE_ID;
    ...
    pub async fn merge_coins_tx_kind(
        ...
        let coin = self
            .0
            .get_object_with_options(primary_coin, PeraObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
    ...
    pub async fn request_add_stake(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            let arguments = vec![
                builder.input(CallArg::PERA_SYSTEM_MUT).unwrap(),
    ...
    pub async fn request_withdraw_stake(
        ...
        let staked_pera = self.get_object_ref(staked_pera).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        let gas = self
        ...
        TransactionData::new_move_call(
            ...
            vec![
                ...
                CallArg::Object(ObjectArg::ImmOrOwnedObject(staked_pera)),
            ],
            gas_budget,
            gas_price,
        )
    ...
    async fn get_object_ref_and_type(
        ...
        let object = self
            .0
            .get_object_with_options(object_id, PeraObjectDataOptions::new().with_type())
            .await?
            .into_object()?;
