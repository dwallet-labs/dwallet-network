...
module pera_system::pera_system {
    ...
    fun load_inner_maybe_upgrade(self: &mut PeraSystemState): &mut PeraSystemStateInner {
        let version = self.version;
        // TODO: This is where we check the version and perform upgrade if necessary.

        let inner: &mut PeraSystemStateInner = dynamic_field::borrow_mut(&mut self.id, version);
