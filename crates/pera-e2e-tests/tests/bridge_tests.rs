...
use pera_bridge::crypto::BridgeAuthorityKeyPair;
use pera_bridge::BRIDGE_ENABLE_PROTOCOL_VERSION;
use pera_json_rpc_api::BridgeReadApiClient;
use pera_macros::sim_test;
use pera_types::bridge::get_bridge;
use pera_types::bridge::BridgeTrait;
    ...
    for h in &handles {
        h.with(|node| {
            assert!(node
                ...
                .get_latest_object_ref_or_tombstone(PERA_BRIDGE_OBJECT_ID)
                .unwrap()
                .is_none());
        });
    }
