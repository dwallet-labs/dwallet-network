...
struct StressTestRunner {
    ...
    pub accounts: Vec<PeraAddress>,
    pub active_validators: BTreeSet<PeraAddress>,
    pub preactive_validators: BTreeMap<PeraAddress, u64>,
    pub removed_validators: BTreeSet<PeraAddress>,
    pub delegation_requests_this_epoch: BTreeMap<ObjectID, PeraAddress>,
    pub delegation_withdraws_this_epoch: u64,
    pub delegations: BTreeMap<ObjectID, PeraAddress>,
    pub reports: BTreeMap<PeraAddress, BTreeSet<PeraAddress>>,
    pub rng: StdRng,
}
...
impl StressTestRunner {
    ...
    pub fn pick_random_sender(&mut self) -> PeraAddress {
        self.accounts[self.rng.gen_range(0..self.accounts.len())]
    }

    pub fn system_state(&self) -> PeraSystemStateSummary {
        self.state()
    ...
    pub fn pick_random_active_validator(&mut self) -> PeraValidatorSummary {
    ...
    pub fn display_effects(&self, effects: &TransactionEffects) {
        ...
        for (obj_ref, _) in effects.created() {
            ...
            println!(">> {struct_tag} TOTAL_PERA: {total_pera}");
        }
    ...
    pub fn state(&self) -> Arc<AuthorityState> {
        self.test_cluster.fullnode_handle.pera_node.state()
    }
...
mod add_stake {
    ...
    impl GenStateChange for RequestAddStakeGen {
        ...
        fn create(&self, runner: &mut StressTestRunner) -> Self::StateChange {
            ...
            let staked_with = runner.pick_random_active_validator().pera_address;
    ...
    impl StatePredicate for RequestAddStake {
        async fn run(&mut self, runner: &mut StressTestRunner) -> Result<TransactionEffects> {
            let pt = {
                ...
                builder.obj(ObjectArg::PERA_SYSTEM_MUT).unwrap();
                builder.pure(self.staked_with).unwrap();
                let coin = StressTestRunner::split_off(&mut builder, self.stake_amount);
                move_call! {
                    builder,
                    (PERA_SYSTEM_PACKAGE_ID)::pera_system::request_add_stake(Argument::Input(0), coin, Argument::Input(1))
                };
                builder.finish()
            };
        ...
        async fn pre_epoch_post_condition(
            ...
            // Assert that a `StakedPera` object matching the amount delegated is created.
            // Assert that this staked pera
            let object = runner
                .get_created_object_of_type_name(effects, "StakedPera")
                .await
                .unwrap();
