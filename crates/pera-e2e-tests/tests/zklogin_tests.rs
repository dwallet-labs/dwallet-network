...
async fn do_zklogin_test(address: PeraAddress, legacy: bool) -> PeraResult {
...
async fn test_zklogin_feature_legacy_address_deny() {
    ...
    let err = do_zklogin_test(get_legacy_zklogin_user_address(), true)
        .await
        .unwrap_err();
    assert!(matches!(err, PeraError::SignerSignatureAbsent { .. }));
}
...
async fn test_legacy_zklogin_address_accept() {
    ...
    assert!(matches!(err, PeraError::InvalidSignature { .. }));
}
    ...
    for h in &handles {
        h.with(|node| {
            assert!(node
                ...
                .get_latest_object_ref_or_tombstone(PERA_AUTHENTICATOR_STATE_OBJECT_ID)
                .unwrap()
                .is_none());
        });
    }
