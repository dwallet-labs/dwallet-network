...
use pera_types::base_types::{EpochId, ObjectID, ObjectRef, PeraAddress};
...
struct TestEnv {
    ...
    regulated_coin_owner: PeraAddress,
...
async fn create_test_env() -> TestEnv {
    ...
    let deny_list_object_init_version = test_cluster
        .get_object_from_fullnode_store(&PERA_DENY_LIST_OBJECT_ID)
        .await
        .unwrap()
        .version();
