...
use pera_bridge::abi::EthBridgeCommittee;
use pera_bridge::abi::{eth_pera_bridge, EthPeraBridge};
use pera_bridge::crypto::BridgeAuthorityPublicKeyBytes;
use pera_bridge::error::BridgeResult;
use pera_bridge::pera_client::PeraBridgeClient;
use pera_bridge::types::BridgeAction;
use pera_bridge::types::{
    AddTokensOnEvmAction, AddTokensOnPeraAction, AssetPriceUpdateAction, BlocklistCommitteeAction,
    BlocklistType, EmergencyAction, EmergencyActionType, EvmContractUpgradeAction,
    LimitUpdateAction,
};
use pera_bridge::utils::{get_eth_signer_client, EthSigner};
...
use pera_types::bridge::{BridgeChainId, BRIDGE_MODULE_NAME};
use pera_types::crypto::{PeraKeyPair, Signature};
use pera_types::programmable_transaction_builder::ProgrammableTransactionBuilder;
use pera_types::transaction::{ObjectArg, Transaction, TransactionData};
...
pub enum BridgeCommand {
    ...
    ViewBridgeRegistration {
        #[clap(long = "pera-rpc-url")]
        pera_rpc_url: String,
    },
    /// View current status of Pera bridge
    #[clap(name = "view-pera-bridge")]
    ViewPeraBridge {
        #[clap(long = "pera-rpc-url")]
        pera_rpc_url: String,
...
pub enum GovernanceClientCommands {
    ...
    AddTokensOnEvm {
        ...
        #[clap(name = "token-prices", use_value_delimiter = true, long)]
        token_prices: Vec<u64>,
        #[clap(name = "token-pera-decimals", use_value_delimiter = true, long)]
        token_pera_decimals: Vec<u8>,
    },
...
pub fn make_action(chain_id: BridgeChainId, cmd: &GovernanceClientCommands) -> BridgeAction {
    match cmd {
        ...
        GovernanceClientCommands::AddTokensOnPera {
            nonce,
            token_ids,
            token_type_names,
            token_prices,
        ...
        GovernanceClientCommands::AddTokensOnEvm {
            ...
            assert_eq!(token_ids.len(), token_prices.len());
            assert_eq!(token_ids.len(), token_pera_decimals.len());
            BridgeAction::AddTokensOnEvmAction(AddTokensOnEvmAction {
                ...
                token_prices: token_prices.clone(),
                token_pera_decimals: token_pera_decimals.clone(),
            })
        }
...
pub fn select_contract_address(
    ...
    match cmd {
        ...
        GovernanceClientCommands::AddTokensOnPera { .. } => unreachable!(),
...
pub struct BridgeCliConfig {
    ...
    /// Proxy address for PeraBridge deployed on Eth
    ...
    /// At leaset one of `pera_key_path` or `eth_key_path` must be provided.
    /// If only one is provided, it will be used for both Pera and Eth.
    pub pera_key_path: Option<PathBuf>,
    /// See `pera_key_path`. Must be Secp256k1 key.
...
impl LoadedBridgeCliConfig {
    pub async fn load(cli_config: BridgeCliConfig) -> anyhow::Result<Self> {
        if cli_config.eth_key_path.is_none() && cli_config.pera_key_path.is_none() {
            return Err(anyhow!(
                "At least one of `pera_key_path` or `eth_key_path` must be provided"
            ));
        }
        let pera_key = if let Some(pera_key_path) = &cli_config.pera_key_path {
            Some(read_key(pera_key_path, false)?)
        } else {
            None
        };
        ...
        let (eth_key, pera_key) = {
            if eth_key.is_none() {
                let pera_key = pera_key.unwrap();
                if !matches!(pera_key, PeraKeyPair::Secp256k1(_)) {
                    return Err(anyhow!("Eth key must be an ECDSA key"));
                }
                (pera_key.copy(), pera_key)
            } else if pera_key.is_none() {
                let eth_key = eth_key.unwrap();
                (eth_key.copy(), eth_key)
            } else {
                (eth_key.unwrap(), pera_key.unwrap())
            }
        };
        ...
        let pera_bridge = EthPeraBridge::new(cli_config.eth_bridge_proxy_address, provider.clone());
        let eth_bridge_committee_proxy_address: EthAddress = pera_bridge.committee().call().await?;
        let eth_bridge_limiter_proxy_address: EthAddress = pera_bridge.limiter().call().await?;
        let eth_committee =
        ...
        let pera_address = PeraAddress::from(&pera_key.public());
        println!("Using Pera address: {:?}", pera_address);
        println!("Using Eth address: {:?}", eth_address);
        println!("Using Eth chain: {:?}", eth_chain_id);

        Ok(Self {
            pera_rpc_url: cli_config.pera_rpc_url,
...
impl LoadedBridgeCliConfig {
    ...
    pub async fn get_pera_account_info(
        self: &LoadedBridgeCliConfig,
    ) -> anyhow::Result<(PeraKeyPair, PeraAddress, ObjectRef)> {
        let pubkey = self.pera_key.public();
        let pera_client_address = PeraAddress::from(&pubkey);
        let pera_sdk_client = PeraClientBuilder::default()
            .build(self.pera_rpc_url.clone())
            .await?;
        let gases = pera_sdk_client
            .coin_read_api()
            .get_coins(pera_client_address, None, None, None)
            .await?
            .data;
        // TODO: is 5 Pera a good number?
        let gas = gases
            .into_iter()
        ...
        Ok((self.pera_key.copy(), pera_client_address, gas.object_ref()))
    }
}
#[derive(Parser)]
#[clap(rename_all = "kebab-case")]
    ...
    pub async fn handle(
        ...
        match self {
            BridgeClientCommands::DepositNativeEtherOnEth {
                ...
                let eth_tx = eth_pera_bridge
                    .bridge_eth(pera_recipient_address.to_vec().into(), target_chain)
                    .value(amount);
                let pending_tx = eth_tx.send().await.unwrap();
                let tx_receipt = pending_tx.await.unwrap().unwrap();
                info!(
                    "Deposited {ether_amount} Ethers to {:?} (target chain {target_chain}). Receipt: {:?}", pera_recipient_address, tx_receipt,
                );
                Ok(())
            }
            BridgeClientCommands::ClaimOnEth { seq_num } => {
                claim_on_eth(seq_num, config, pera_bridge_client)
                    .await
                    .map_err(|e| anyhow!("{:?}", e))
            }
            BridgeClientCommands::DepositOnPera {
                coin_object_id,
                coin_type,
                target_chain,
                recipient_address,
...
async fn deposit_on_pera(
    ...
    let pera_client = pera_bridge_client.pera_client();
    let bridge_object_arg = pera_bridge_client
    ...
    let rgp = pera_client
        ...
        .await
        .unwrap();
    let sender = PeraAddress::from(&config.pera_key.public());
    let gas_obj_ref = pera_client
        .coin_read_api()
        ...
        .object_ref();
    let coin_obj_ref = pera_client
        .read_api()
        .get_object_with_options(coin_object_id, PeraObjectDataOptions::default())
        .await?
        .data
        .unwrap()
        .object_ref();
    ...
    info!(?tx_digest, "Sending deposit transction to Pera.");
...
async fn claim_on_eth(
    ...
    let pera_chain_id = pera_bridge_client.get_bridge_summary().await?.chain_id;
    let parsed_message = pera_bridge_client
        .get_parsed_token_transfer_message(pera_chain_id, seq_num)
        .await?;
    if parsed_message.is_none() {
        println!("No record found for seq_num: {seq_num}, chain id: {pera_chain_id}");
        return Ok(());
    }
    let parsed_message = parsed_message.unwrap();
    let sigs = pera_bridge_client
        .get_token_transfer_action_onchain_signatures_until_success(pera_chain_id, seq_num)
        .await;
    if sigs.is_none() {
        println!("No signatures found for seq_num: {seq_num}, chain id: {pera_chain_id}");
        return Ok(());
    }
    let signatures = sigs
        .unwrap()
    ...
    let message = eth_pera_bridge::Message::from(parsed_message);
    let tx = eth_pera_bridge.transfer_bridged_tokens_with_signatures(signatures, message);
...
mod tests {
    ...
    async fn test_encode_call_data() {
        let abi_json =
            std::fs::read_to_string("../pera-bridge/abi/tests/mock_pera_bridge_v2.json").unwrap();
