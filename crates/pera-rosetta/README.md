...
Pera-Rosetta is an implementation of the Rosetta API for the Pera network, the Pera-Rosetta server uses the Pera fullnode to
read and write transactions to the Pera network.

## Local network quick start

### Build from source
#### 0. Checkout and build Pera
Checkout the [Pera source code](https://github.com/MystenLabs/sui) and compile using `cargo build --release`, the binaries will be located in `target/release` directory.

#### 1. Genesis

`./pera genesis -f`  
The Pera genesis process will create configs and coins for testing, the config files are stored in `~/.pera/pera_config` by
default.
...
`./pera-rosetta start-online-server`

#### 4. Start Rosetta Offline server

`./pera-rosetta start-offline-server`

#### 5. Generate configuration with prefunded accounts for rosetta-cli

`./pera-rosetta generate-rosetta-cli-config`
This will generate the `rosetta-cli.json` and `pera.ros` file to be used by the [Rosetta-CLI](https://github.com/coinbase/rosetta-cli)
...
```shell
cd <pera project directory>/crate/pera-rosetta/docker/pera-rosetta-local
```   
#### 2. Build the image
```shell
./build.sh
...
```shell
docker run pera-io/pera-rosetta-devnet pera-rosetta start-online-server
```
Alternatively, to start the rosetta-offline server, run
```shell
docker run pera-io/pera-rosetta-devnet pera-rosetta start-offline-server
```
...
| Method | Endpoint         | Description                    | Pera Supported? |  Server Type  |
|--------|------------------|--------------------------------|:--------------:|:-------------:|
...
| Method | Endpoint           | Description             |                                      Pera Supported?                                       |  Server Type  |
|--------|--------------------|-------------------------|:-----------------------------------------------------------------------------------------:|:-------------:|
| POST   | /block             | Get a Block             | Yes (One transaction per block in phase 1, will be replaced by Pera checkpoint in phase 2) |    Online     |
...
## Pera transaction <> Rosetta Operation conversion explained
There are 2 places we convert Pera's transaction to Rosetta's operations, 
...
the `/construction/parse` endpoint is used for checking transaction correctness during transaction construction, in our case we only support `TransferPera` for now, the operations created looks like this (negative amount indicate sender):
```json
{
...
Since the intent operations of TransferPera contains all the balance change information(amount field) already, 
