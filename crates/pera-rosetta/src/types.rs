...
impl PeraEnv {
    pub fn check_network_identifier(
        ...
        if &network_identifier.blockchain != "pera" {
...
impl From<PeraAddress> for AccountIdentifier {
...
impl From<pera_sdk::rpc_types::Coin> for Coin {
    fn from(coin: pera_sdk::rpc_types::Coin) -> Self {
        Self {
...
impl From<PeraPublicKey> for PublicKey {
    fn from(pk: PeraPublicKey) -> Self {
        match pk {
            PeraPublicKey::Ed25519(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Edwards25519,
            },
            PeraPublicKey::Secp256k1(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256k1,
            },
            PeraPublicKey::Secp256r1(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256r1,
            },
            PeraPublicKey::ZkLogin(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::ZkLogin, // inaccurate but added for completeness.
            },
            PeraPublicKey::Passkey(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256r1,
            },
        }
...
impl TryInto<PeraAddress> for PublicKey {
    type Error = Error;

    fn try_into(self) -> Result<PeraAddress, Self::Error> {
        let key_bytes = self.hex_bytes.to_vec()?;
        let pub_key = PeraPublicKey::try_from_bytes(self.curve_type.into(), &key_bytes)?;
        Ok((&pub_key).into())
    }
}
...
pub enum OperationType {
    ...
    StakeReward,
    StakePrinciple,
    // pera-rosetta supported operation type
    PayPera,
    Stake,
    WithdrawStake,
    // All other Pera transaction types, readonly
    EpochChange,
    Genesis,
...
impl From<&PeraTransactionBlockKind> for OperationType {
    fn from(tx: &PeraTransactionBlockKind) -> Self {
        match tx {
            PeraTransactionBlockKind::ChangeEpoch(_) => OperationType::EpochChange,
            PeraTransactionBlockKind::Genesis(_) => OperationType::Genesis,
            PeraTransactionBlockKind::ConsensusCommitPrologue(_)
            | PeraTransactionBlockKind::ConsensusCommitPrologueV2(_)
            | PeraTransactionBlockKind::ConsensusCommitPrologueV3(_) => {
                OperationType::ConsensusCommitPrologue
            }
            PeraTransactionBlockKind::ProgrammableTransaction(_) => {
                OperationType::ProgrammableTransaction
            }
            PeraTransactionBlockKind::AuthenticatorStateUpdate(_) => {
                OperationType::AuthenticatorStateUpdate
            }
            PeraTransactionBlockKind::RandomnessStateUpdate(_) => {
            ...
            PeraTransactionBlockKind::LockNextCommittee => OperationType::LockNextCommittee,
        }
    }
}
...
impl From<PeraExecutionStatus> for OperationStatus {
    fn from(es: PeraExecutionStatus) -> Self {
        match es {
            PeraExecutionStatus::Success => OperationStatus::Success,
            PeraExecutionStatus::Failure { .. } => OperationStatus::Failure,
        }
    }
}
...
impl InternalOperation {
    pub fn sender(&self) -> PeraAddress {
        match self {
            InternalOperation::PayPera { sender, .. }
        ...
        let pt = match self {
            ...
            InternalOperation::Stake {
                ...
                let (validator, system_state, amount) = if let Some(amount) = amount {
                    ...
                    let state = builder.input(CallArg::PERA_SYSTEM_MUT)?;
            ...
            InternalOperation::WithdrawStake { stake_ids, .. } => {
                ...
                for stake_id in metadata.objects {
                    ...
                    let (system_state, id) = if !stake_ids.is_empty() {
                        let system_state = builder.input(CallArg::PERA_SYSTEM_MUT)?;
