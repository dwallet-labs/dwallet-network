...
use pera_types::crypto::{DefaultHash, SignatureScheme, ToFromBytes};
...
use crate::{OnlineServerContext, PeraEnv};
...
pub async fn derive(
    Extension(env): Extension<PeraEnv>,
    ...
    let address: PeraAddress = request.public_key.try_into()?;
...
pub async fn submit(
    ...
    let dry_run = context
        ...
        .await?;
    if let PeraExecutionStatus::Failure { error } = dry_run.effects.status() {
    ...
    if let PeraExecutionStatus::Failure { error } = response
        .effects
...
/// For Pera, we are returning the latest object refs for all the input objects,
...
pub async fn metadata(
    ...
    let (total_required_amount, objects) = match &option.internal_operation {
        InternalOperation::PayPera { amounts, .. } => {
        ...
        InternalOperation::WithdrawStake { sender, stake_ids } => {
            ...
            let responses = context
                .client
                .read_api()
                .multi_get_object_with_options(stake_ids, PeraObjectDataOptions::default())
                .await?;
    ...
    // If required amount is None (all PERA) or failed to select coin (might not have enough PERA), select all coins.
