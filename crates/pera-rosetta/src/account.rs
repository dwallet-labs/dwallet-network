...
use pera_sdk::rpc_types::StakeStatus;
use pera_sdk::{PeraClient, PERA_COIN_TYPE};
    ...
    if let Some(SubAccount { account_type }) = request.account_identifier.sub_account {
        ...
        while retry_attempts > 0 {
            let balances_first = ctx
                .client
                .coin_read_api()
                .get_balance(address, Some(PERA_COIN_TYPE.to_string()))
                .await?
