...
use pera_json_rpc_types::PeraProgrammableMoveCall;
use pera_json_rpc_types::PeraProgrammableTransactionBlock;
use pera_json_rpc_types::{BalanceChange, PeraArgument};
use pera_json_rpc_types::{PeraCallArg, PeraCommand};
...
use pera_types::{PERA_SYSTEM_ADDRESS, PERA_SYSTEM_PACKAGE_ID};
...
impl Operations {
    ...
    pub fn into_internal(self) -> Result<InternalOperation, Error> {
        ...
        match type_ {
            OperationType::PayPera => self.pay_pera_ops_to_internal(),
    ...
    fn pay_pera_ops_to_internal(self) -> Result<InternalOperation, Error> {
    ...
    fn from_transaction(
        tx: PeraTransactionBlockKind,
        sender: PeraAddress,
        status: Option<OperationStatus>,
    ) -> Result<Vec<Operation>, Error> {
        Ok(match tx {
            PeraTransactionBlockKind::ProgrammableTransaction(pt) => {
    ...
    fn parse_programmable_transaction(
        sender: PeraAddress,
        status: Option<OperationStatus>,
        pt: PeraProgrammableTransactionBlock,
    ) -> Result<Vec<Operation>, Error> {
        #[derive(Debug)]
        enum KnownValue {
            GasCoin(u64),
        ...
        fn transfer_object(
            aggregated_recipients: &mut HashMap<PeraAddress, u64>,
            inputs: &[PeraCallArg],
            known_results: &[Vec<KnownValue>],
            objs: &[PeraArgument],
            recipient: PeraArgument,
        ) -> Option<Vec<KnownValue>> {
            ...
            for obj in objs {
                let value = match *obj {
                    ...
                    PeraArgument::GasCoin | PeraArgument::Input(_) => return None,
                };
        ...
        fn stake_call(
            inputs: &[PeraCallArg],
            known_results: &[Vec<KnownValue>],
            call: &PeraProgrammableMoveCall,
        ) -> Result<Option<(Option<u64>, PeraAddress)>, Error> {
            let PeraProgrammableMoveCall { arguments, .. } = call;
            let (amount, validator) = match &arguments[..] {
                [_, coin, validator] => {
                    ...
                    let (some_amount, validator) = match validator {
                        ...
                        PeraArgument::Input(i) => (*i==1, inputs[*i as usize].pure().map(|v|v.to_pera_address()).transpose()),
                        _=> return Ok(None),
                    };
        ...
        let PeraProgrammableTransactionBlock { inputs, commands } = &pt;
        let mut known_results: Vec<Vec<KnownValue>> = vec![];
        let mut aggregated_recipients: HashMap<PeraAddress, u64> = HashMap::new();
        ...
        for command in commands {
            let result = match command {
                ...
                PeraCommand::TransferObjects(objs, addr) => transfer_object(
                ...
                PeraCommand::MoveCall(m) if Self::is_stake_call(m) => {
                ...
                PeraCommand::MoveCall(m) if Self::is_unstake_call(m) => {
            ...
            operations.extend(
                aggregated_recipients
                    ...
                    .map(|(recipient, amount)| {
                        Operation::pay_pera(status, recipient, amount.into())
                    }),
            );
            operations.push(Operation::pay_pera(status, sender, -(total_paid as i128)));
    ...
    fn is_stake_call(tx: &PeraProgrammableMoveCall) -> bool {
        tx.package == PERA_SYSTEM_PACKAGE_ID
            && tx.module == PERA_SYSTEM_MODULE_NAME.as_str()
            && tx.function == ADD_STAKE_FUN_NAME.as_str()
    }

    fn is_unstake_call(tx: &PeraProgrammableMoveCall) -> bool {
        tx.package == PERA_SYSTEM_PACKAGE_ID
            && tx.module == PERA_SYSTEM_MODULE_NAME.as_str()
    ...
    fn process_balance_change(
        gas_owner: PeraAddress,
        gas_used: i128,
        balance_changes: &[BalanceChange],
        status: Option<OperationStatus>,
        balances: HashMap<PeraAddress, i128>,
...
impl TryFrom<PeraTransactionBlockData> for Operations {
    type Error = Error;
    fn try_from(data: PeraTransactionBlockData) -> Result<Self, Self::Error> {
...
impl TryFrom<PeraTransactionBlockResponse> for Operations {
    type Error = Error;
...
fn is_unstake_event(tag: &StructTag) -> bool {
    tag.address == PERA_SYSTEM_ADDRESS
...
impl TryFrom<TransactionData> for Operations {
    ...
    fn try_from(data: TransactionData) -> Result<Self, Self::Error> {
        ...
        PeraTransactionBlockData::try_from(data, &&mut NoOpsModuleResolver)?.try_into()
    }
}
...
pub enum OperationMetadata {
    GenericTransaction(PeraTransactionBlockKind),
    Stake { validator: PeraAddress },
...
impl Operation {
    ...
    pub fn genesis(index: u64, sender: PeraAddress, coin: GasCoin) -> Self {
        Operation {
    ...
    fn pay_pera(status: Option<OperationStatus>, address: PeraAddress, amount: i128) -> Self {
        Operation {
            operation_identifier: Default::default(),
            type_: OperationType::PayPera,
            status,
    ...
    fn balance_change(status: Option<OperationStatus>, addr: PeraAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
            type_: OperationType::PeraBalanceChange,
            status,
    ...
    fn gas(addr: PeraAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_reward(status: Option<OperationStatus>, addr: PeraAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
    ...
    fn stake_principle(status: Option<OperationStatus>, addr: PeraAddress, amount: i128) -> Self {
        Self {
            operation_identifier: Default::default(),
