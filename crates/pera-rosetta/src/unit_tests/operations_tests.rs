...
async fn test_operation_data_parsing() -> Result<(), anyhow::Error> {
    ...
    let pt = {
        ...
        builder
            .pay_pera(vec![PeraAddress::random_for_testing_only()], vec![10000])
            .unwrap();
        builder.finish()
    };
    let gas_price = 10;
...
#[tokio::test]
async fn test_pera_json() {
    let arg1 = CallArg::Pure(bcs::to_bytes(&1000000u64).unwrap());
    let arg2 = CallArg::Pure(bcs::to_bytes(&30215u64).unwrap());
    let json1 = PeraCallArg::try_from(arg1, Some(&MoveTypeLayout::U64)).unwrap();
    let json2 = PeraCallArg::try_from(arg2, Some(&MoveTypeLayout::U64)).unwrap();
    println!("{:?}, {:?}", json1, json2);
}
