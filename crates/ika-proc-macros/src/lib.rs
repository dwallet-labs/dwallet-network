    ...
    input.block = syn::parse2(quote! {
        {
            ...
            std::thread::spawn(|| {
                ...
                ::ika_simulator::telemetry_subscribers::init_for_testing();
                ::ika_simulator::ika_types::execution::get_denied_certificates();
                ::ika_simulator::ika_framework::BuiltInFramework::all_package_ids();
                ::ika_simulator::ika_types::gas::IkaGasStatus::new_unmetered();
                ...
                let mut cache = ::ika_simulator::lru::LruCache::new(1.try_into().unwrap());
                cache.put(1, 1);

                {
                    ...
                    use ika_simulator::ika_move_build::{BuildConfig, IkaPackageHooks};
                    use ika_simulator::tempfile::TempDir;
                    use ika_simulator::move_package::package_hooks::register_package_hooks;

                    register_package_hooks(Box::new(IkaPackageHooks {}));
                ...
                use ::ika_simulator::anemo_tower::callback::CallbackLayer;
                use ::ika_simulator::anemo_tower::trace::DefaultMakeSpan;
                use ::ika_simulator::anemo_tower::trace::DefaultOnFailure;
                use ::ika_simulator::anemo_tower::trace::TraceLayer;
                use ::ika_simulator::narwhal_network::metrics::MetricsMakeCallbackHandler;
                use ::ika_simulator::narwhal_network::metrics::NetworkMetrics;

                use std::sync::Arc;
                use ::ika_simulator::fastcrypto::traits::KeyPair;
                use ::ika_simulator::rand_crate::rngs::{StdRng, OsRng};
                use ::ika_simulator::rand::SeedableRng;
                use ::ika_simulator::tower::ServiceBuilder;
                ...
                let rt = ::ika_simulator::runtime::Runtime::new();
                rt.block_on(async move {
                    use ::ika_simulator::anemo::{Network, Request};

                    let make_network = |port: u16| {
                        let registry = prometheus::Registry::new();
                        let inbound_network_metrics =
                            NetworkMetrics::new("ika", "inbound", &registry);
                        let outbound_network_metrics =
                            NetworkMetrics::new("ika", "outbound", &registry);
                        ...
                        let service = ServiceBuilder::new()
                            ...
                            .service(::ika_simulator::anemo::Router::new());
                        ...
                        Network::bind(format!("127.0.0.1:{}", port))
                            ...
                            .private_key(
                                ::ika_simulator::fastcrypto::ed25519::Ed25519KeyPair::generate(&mut StdRng::from_rng(OsRng).unwrap())
                                    .private()
                                    .0
                                    .to_bytes(),
                            )
...
/// The ika_test macro will invoke either `#[msim::test]` or `#[tokio::test]`,
...
#[proc_macro_attribute]
pub fn ika_test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let header = if cfg!(msim) {
        quote! {
            #[::ika_simulator::sim_test(crate = "ika_simulator", #(#args)* )]
        }
    } else {
        quote! {
            #[::tokio::test(#(#args)*)]
    ...
    let result = quote! {
        #header
        #[::ika_macros::init_static_initializers]
        #input
    };
    ...
    let result = if cfg!(msim) {
        ...
        quote! {
            ...
            #sig {
                ...
                ::ika_simulator::task::shutdown_all_nodes();
                ...
                assert_eq!(
                    ika_simulator::NodeLeakDetector::get_current_node_count(),
                    0,
                    "IkaNode leak detected"
                );
        ...
        quote! {
            ...
            #ignore
            #sig {
                if std::env::var("IKA_SKIP_SIMTESTS").is_ok() {
                    println!("not running test {} in `cargo test`: IKA_SKIP_SIMTESTS is set", stringify!(#fn_name));
...
pub fn enum_variant_order_derive(input: TokenStream) -> TokenStream {
    ...
    if let Data::Enum(DataEnum { variants, .. }) = ast.data {
        ...
        let deriv = quote! {
            impl ika_enum_compat_util::EnumOrderMap for #name {
