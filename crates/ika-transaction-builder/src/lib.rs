...
use ika_json::{is_receiving_argument, resolve_move_function_args, ResolvedCallArg, IkaJsonValue};
use ika_json_rpc_types::{
    RPCTransactionRequestParams, IkaData, IkaObjectDataOptions, IkaObjectResponse, IkaRawData,
    IkaTypeTag,
};
use ika_protocol_config::ProtocolConfig;
use ika_types::base_types::{ObjectID, ObjectInfo, ObjectRef, ObjectType, IkaAddress};
use ika_types::error::UserInputError;
use ika_types::gas_coin::GasCoin;
use ika_types::governance::{ADD_STAKE_MUL_COIN_FUN_NAME, WITHDRAW_STAKE_FUN_NAME};
...
use ika_types::{coin, fp_ensure, IKA_FRAMEWORK_PACKAGE_ID, IKA_SYSTEM_PACKAGE_ID};
...
impl TransactionBuilder {
    ...
    async fn select_gas(
        ...
        if let Some(gas) = input_gas {
            ...
            for obj in gas_objs {
                let response = self
                    .0
                    .get_object_with_options(obj.object_id, IkaObjectDataOptions::new().with_bcs())
                    .await?;
            ...
            Err(anyhow!("Cannot find gas coin for signer address {signer} with amount sufficient for the required gas budget {gas_budget}. If you are using the pay or transfer commands, you can use pay-ika or transfer-ika commands instead, which will use the only object as gas payment."))
        }
    }
    ...
    pub async fn transfer_ika(
        ...
        let object = self.get_object_ref(ika_object_id).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_transfer_ika(
    ...
    pub async fn pay(
        ...
        if let Some(gas) = gas {
            if input_coins.contains(&gas) {
                return Err(anyhow!("Gas coin is in input coins of Pay transaction, use PayIka transaction instead!"));
            }
        }
    ...
    /// Construct a transaction kind for the PayIka transaction type
    ///
    /// Use this function together with tx_data_for_dry_run or tx_data
    /// for maximum reusability
    pub fn pay_ika_tx_kind(
        &self,
        recipients: Vec<IkaAddress>,
        amounts: Vec<u64>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.pay_ika(recipients.clone(), amounts.clone())?;
        let pt = builder.finish();
        let tx_kind = TransactionKind::programmable(pt);
        Ok(tx_kind)
    }

    pub async fn pay_ika(
        &self,
        signer: IkaAddress,
        input_coins: Vec<ObjectID>,
        recipients: Vec<IkaAddress>,
    ...
    pub fn pay_all_ika_tx_kind(&self, recipient: IkaAddress) -> TransactionKind {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn pay_all_ika(
        ...
        let gas_object_ref = coin_refs.remove(0);
        let gas_price = self.0.get_reference_gas_price().await?;
        Ok(TransactionData::new_pay_all_ika(
            signer,
            coin_refs,
            recipient,
            gas_object_ref,
    ...
    pub async fn move_call_tx_kind(
        ...
        module: &str,
        function: &str,
        type_args: Vec<IkaTypeTag>,
        call_args: Vec<IkaJsonValue>,
    ) -> Result<TransactionKind, anyhow::Error> {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    pub async fn resolve_and_checks_json_args(
        ...
        function: &Identifier,
        type_args: &[TypeTag],
        json_args: Vec<IkaJsonValue>,
    ) -> Result<Vec<Argument>, anyhow::Error> {
        let object = self
            .0
            .get_object_with_options(package_id, IkaObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
        let Some(IkaRawData::Package(package)) = object.bcs else {
            bail!(
    ...
    pub async fn upgrade_tx_kind(
        ...
        let upgrade_capability = self
            .0
            .get_object_with_options(upgrade_capability, IkaObjectDataOptions::new().with_owner())
            .await?
            .into_object()?;
    ...
    pub async fn merge_coins_tx_kind(
        ...
        let coin = self
            .0
            .get_object_with_options(primary_coin, IkaObjectDataOptions::bcs_lossless())
            .await?
            .into_object()?;
    ...
    pub async fn request_add_stake(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            let arguments = vec![
                builder.input(CallArg::IKA_SYSTEM_MUT).unwrap(),
    ...
    pub async fn request_withdraw_stake(
        ...
        let staked_ika = self.get_object_ref(staked_ika).await?;
        let gas_price = self.0.get_reference_gas_price().await?;
        let gas = self
        ...
        TransactionData::new_move_call(
            ...
            vec![
                ...
                CallArg::Object(ObjectArg::ImmOrOwnedObject(staked_ika)),
            ],
            gas_budget,
            gas_price,
        )
    ...
    async fn get_object_ref_and_type(
        ...
        let object = self
            .0
            .get_object_with_options(object_id, IkaObjectDataOptions::new().with_type())
            .await?
            .into_object()?;
