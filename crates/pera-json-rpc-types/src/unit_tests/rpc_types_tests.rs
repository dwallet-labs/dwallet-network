...
use pera_types::base_types::{ObjectDigest, SequenceNumber};
use pera_types::base_types::{ObjectID, PeraAddress};
use pera_types::gas_coin::GasCoin;
use pera_types::object::{MoveObject, Owner};
use pera_types::{parse_pera_struct_tag, MOVE_STDLIB_ADDRESS, PERA_FRAMEWORK_ADDRESS};

use crate::{ObjectChange, PeraMoveStruct, PeraMoveValue};

#[test]
fn test_move_value_to_pera_coin() {
    ...
    let pera_struct = PeraMoveStruct::from(move_struct);
    let gas_coin = GasCoin::try_from(&pera_struct).unwrap();
...
fn test_move_value_to_string() {
    ...
    let pera_value = PeraMoveValue::from(move_value);

    assert!(matches!(pera_value, PeraMoveValue::String(s) if s == test_string));
}
...
fn test_option() {
    ...
    let pera_value = PeraMoveValue::from(option);
    assert!(matches!(
        pera_value,
        PeraMoveValue::Option(value) if *value == Some(PeraMoveValue::Number(5))
    ));
}
...
fn test_move_value_to_url() {
    ...
    let pera_value = PeraMoveValue::from(url_move_value);

    assert!(matches!(pera_value, PeraMoveValue::String(s) if s == test_url));
}

#[test]
fn test_serde() {
    let test_values = [
        PeraMoveValue::Number(u32::MAX),
        PeraMoveValue::UID {
            id: ObjectID::random(),
        },
        PeraMoveValue::String("some test string".to_string()),
        PeraMoveValue::Address(PeraAddress::random_for_testing_only()),
        PeraMoveValue::Bool(true),
        PeraMoveValue::Option(Box::new(None)),
        PeraMoveValue::Vector(vec![
            PeraMoveValue::Number(1000000),
            PeraMoveValue::Number(2000000),
            PeraMoveValue::Number(3000000),
        ]),
    ];

    for value in test_values {
        let json = serde_json::to_string(&value).unwrap();
        let serde_value: PeraMoveValue = serde_json::from_str(&json)
...
fn test_serde_bytearray() {
    ...
    let pera_move_value = PeraMoveValue::from(test_values);
    let json = serde_json::to_value(&pera_move_value).unwrap();
...
fn test_type_tag_struct_tag_devnet_inc_222() {
    ...
    for tag in offending_tags {
        let oc = ObjectChange::Created {
            ...
            object_type: parse_pera_struct_tag(tag).unwrap(),
