...
/// is configured.
pub fn start_metrics_push_task(config: &ika_config::NodeConfig, registry: RegistryService) {
    use fastcrypto::traits::KeyPair;
    use ika_config::node::MetricsConfig;
        ...
        loop {
            ...
            if let Err(error) = push_metrics(&client, &url, &registry).await {
                ...
                if errors >= 10 {
                    ...
                    tracing::error!(
                        ?error,
                        "unable to push metrics; a new client will be created"
                    );
                } else {
                    tracing::warn!(
                        ?error,
