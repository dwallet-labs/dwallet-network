...
use ika_types::sui::ika_system_state::IkaSystemState;
...
use ika_types::sui::ika_system_state::epoch_start_ika_system_state::EpochStartSystemState;
    ...
    pub async fn start_async(
        ...
        let sui_client = Arc::new(
            SuiClient::new(
                ...
                config.sui_connector_config.ika_system_state_id,
            ).await?,
        );

        let latest_system_state = sui_client.get_ika_system_state_until_success().await;

        let previous_epoch_last_checkpoint_sequence_number = latest_system_state.previous_epoch_last_checkpoint_sequence_number();

        let epoch_start_state = latest_system_state.into_epoch_start_state();
        
        let committee = Arc::new(epoch_start_state.get_ika_committee());
        ...
        let chain_identifier = ChainIdentifier::from(config.sui_connector_config.ika_system_state_id);
        ...
        let sui_connector_service = Arc::new(SuiConnectorService::new(checkpoint_store.clone(), sui_client, config.sui_connector_config.clone(), sui_connector_metrics).await?);
            ...
            if let Some(components) = &*self.validator_components.lock().await {
                ...
                let transaction = ConsensusTransaction::new_capability_notification_v1(
                    AuthorityCapabilitiesV1::new(
                        ...
                        self.sui_connector_service.get_available_move_packages().await?,
                        // self.state
                ...
                let consensus_adapter = components
                    .consensus_adapter.clone();
                
                let versions = self.config
                    .supported_protocol_versions
                    .expect("Supported versions should be populated")
                    // no need to send digests of versions less than the current version
                    .truncate_below(config.version);
                ...
                spawn_monitored_task!(async move {
                    ...
                    loop {
                        ...
                        consensus_adapter
                        .submit_to_consensus(&[transaction], &cur_epoch_store).await.expect("ConsensusTransaction::new_test_message");
                        num += 1;
                    }
                });
            }

            let stop_condition = self.sui_connector_service.run_epoch(cur_epoch_store.epoch(), run_with_range).await;

            let latest_system_state = match stop_condition {
                StopReason::EpochComplete(latest_system_state) => latest_system_state,
