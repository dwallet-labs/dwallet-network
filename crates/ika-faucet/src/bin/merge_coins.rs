...
use ika_config::{ika_config_dir, IKA_CLIENT_CONFIG};
use ika_faucet::FaucetError;
...
use ika_types::{base_types::ObjectID, gas_coin::GasCoin, transaction::Transaction};
use tracing::info;
...
async fn _split_coins_equally(
    ...
    let signature = wallet
        .config
        .keystore
        .sign_secure(&active_address, &tx_data, Intent::ika_transaction())
        .unwrap();
...
async fn _merge_coins(gas_coin: &str, mut wallet: WalletContext) -> Result<(), anyhow::Error> {
    ...
    for chunk in small_coins.chunks(254) {
        ...
        let tx_data = client
            .transaction_builder()
            .pay_ika(active_address, coin_vector, target, target_amount, 1000000)
            .await?;
        let signature = wallet
            .config
            .keystore
