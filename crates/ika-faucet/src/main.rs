...
use std::env;
use std::sync::Arc;
use ika_config::ika_config_dir;
use ika_faucet::{create_wallet_context, start_faucet, AppState};
use ika_faucet::{FaucetConfig, SimpleFaucet};
use tracing::info;
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let context = create_wallet_context(wallet_client_timeout_secs, ika_config_dir()?)?;
