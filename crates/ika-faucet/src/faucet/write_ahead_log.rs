...
use ika_types::{base_types::ObjectID, transaction::TransactionData};
...
/// Persistent log of transactions paying out ika from the faucet, keyed by the coin serving the
...
mod tests {
    ...
    fn random_request(coin: ObjectRef) -> (IkaAddress, TransactionData) {
        let gas_price = 1;
        let send = IkaAddress::random_for_testing_only();
        let recv = IkaAddress::random_for_testing_only();
        (
            recv,
