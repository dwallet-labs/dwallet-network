...
#[async_trait]
pub trait Faucet {
    /// Send `Coin<IKA>` of the specified amount to the recipient
    async fn send(
        &self,
        id: Uuid,
    ...
    /// Send `Coin<IKA>` of the specified amount to the recipient in a batch request
    async fn batch_send(
        &self,
        id: Uuid,
        recipient: IkaAddress,
...
#[clap(
    name = "Ika Faucet",
    about = "Faucet for requesting test tokens on Ika",
