...
use crate::client_commands::IkaClientCommands;
...
use std::sync::Arc;
use std::{fs, io};
use ika_bridge::config::BridgeCommitteeConfig;
use ika_bridge::metrics::BridgeMetrics;
use ika_bridge::ika_client::IkaBridgeClient;
use ika_bridge::ika_transaction_builder::build_committee_register_transaction;
use ika_config::node::Genesis;
use ika_config::p2p::SeedPeer;
use ika_config::{
    genesis_blob_exists, ika_config_dir, Config, PersistedConfig, FULL_NODE_DB_PATH,
...
use ika_faucet::{create_wallet_context, start_faucet, AppState, FaucetConfig, SimpleFaucet};
use ika_indexer::test_utils::{
...
use ika_move::{self, execute_move_command};
use ika_move_build::IkaPackageHooks;
use ika_sdk::ika_client_config::{IkaClientConfig, IkaEnv};
use ika_sdk::wallet_context::WalletContext;
use ika_swarm::memory::Swarm;
use ika_swarm_config::genesis_config::{GenesisConfig, DEFAULT_NUMBER_OF_AUTHORITIES};
...
use ika_types::crypto::{SignatureScheme, IkaKeyPair, ToFromBytes};
...
const DEFAULT_FAUCET_NUM_COINS: usize = 5; // 5 coins per request was the default in ika-test-validator
const DEFAULT_FAUCET_NIKA_AMOUNT: u64 = 200_000_000_000; // 200 IKA
...
pub struct IndexerArgs {
    ...
    /// DB name for the Indexer Postgres DB. Default DB name is ika_indexer.
    #[clap(long, default_value = "ika_indexer")]
    pg_db_name: String,
...
impl IndexerArgs {
    pub fn for_testing() -> Self {
        Self {
            ...
            pg_db_name: "ika_indexer".to_string(),
...
pub enum IkaCommand {
    ...
    ///
    /// By default, ika start will start a local network from the genesis blob that exists in
    ...
    Start {
        ...
        /// ika genesis -f --with-faucet generates a genesis config that can be used to start this
        /// proces. Use with caution as the `-f` flag will overwrite the existing config directory.
        /// We can use any config dir that is generated by the `ika genesis`.
        ...
        /// run this command.
        ///
        /// To run with persisted state, do not pass this flag and use the `ika genesis` command
    ...
    /// Bootstrap and initialize a new ika network
    #[clap(name = "genesis")]
    Genesis {
        ...
        #[clap(
            long,
            help = "Creates an extra faucet configuration for ika persisted runs."
        )]
        with_faucet: bool,
    },
    ...
    /// Client for interacting with the Ika network.
    #[clap(name = "client")]
    Client {
        /// Sets the file storing the state of our user accounts (an empty one will be created if missing)
        #[clap(long = "client.config")]
    ...
    /// Invoke Ika's move-analyzer via CLI
...
impl IkaCommand {
    pub async fn execute(self) -> Result<(), anyhow::Error> {
        ...
        match self {
            ...
            IkaCommand::GenesisCeremony(cmd) => run(cmd),
            IkaCommand::KeyTool {
                keystore_path,
                json,
                cmd,
                ...
                let keystore_path =
                    keystore_path.unwrap_or(ika_config_dir()?.join(IKA_KEYSTORE_FILENAME));
                let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path)?);
                cmd.execute(&mut keystore).await?.print(!json);
                Ok(())
            }
            IkaCommand::Console { config } => {
                let config = config.unwrap_or(ika_config_dir()?.join(IKA_CLIENT_CONFIG));
            ...
            IkaCommand::Client {
                ...
                let config_path = config.unwrap_or(ika_config_dir()?.join(IKA_CLIENT_CONFIG));
            ...
            IkaCommand::Move {
                ...
                match &mut cmd {
                    ika_move::Command::Build(build) if build.dump_bytecode_as_base64 => {
                        // `ika move build` does not ordinarily require a network connection.
                        ...
                        let config =
                            client_config.unwrap_or(ika_config_dir()?.join(IKA_CLIENT_CONFIG));
                        prompt_if_no_config(&config, false).await?;
                        let context = WalletContext::new(&config, None, None)?;
                        if let Err(e) = context.get_client().await?.check_api_version() {
                            eprintln!("{}", format!("[warning] {e}").yellow().bold());
            ...
            IkaCommand::BridgeInitialize {
                ...
                let network_config_path = network_config
                    .clone()
                    .unwrap_or(ika_config_dir()?.join(IKA_NETWORK_CONFIG));
                ...
                let ika_bridge_client = IkaBridgeClient::new(rpc_url, bridge_metrics).await?;
                let bridge_arg = ika_bridge_client
                ...
                {
                    let account_kp = node_config.account_key_pair.keypair();
                    let ika_address = IkaAddress::from(&account_kp.public());
            ...
            IkaCommand::FireDrill { fire_drill } => run_fire_drill(fire_drill).await,
            IkaCommand::Analyzer => {
                analyzer::run();
                Ok(())
            }
...
async fn start(
    ...
    if epoch_duration_ms.is_some() && genesis_blob_exists(config.clone()) && !force_regenesis {
        bail!(
            ...
            there is no genesis configuration in the default Ika configuration folder or the given \
    ...
    if force_regenesis {
        ...
        if config.is_none() && !ika_config_dir()?.join(IKA_NETWORK_CONFIG).exists() {
            genesis(None, None, None, false, epoch_duration_ms, None, false).await.map_err(|_| anyhow!("Cannot run genesis with non-empty Ika config directory: {}.\n\nIf you are trying to run a local network without persisting the data (so a new genesis that is randomly generated and will not be saved once the network is shut down), use --force-regenesis flag.\nIf you are trying to persist the network data and start from a new genesis, use ika genesis --help to see how to generate a new genesis.", ika_config_dir().unwrap().display()))?;
        }
    ...
    let mut fullnode_url = ika_config::node::default_json_rpc_address();
    ...
    if let Some(input) = with_faucet {
        ...
        let config = FaucetConfig {
            ...
            amount: DEFAULT_FAUCET_NIKA_AMOUNT,
            ..Default::default()
        };
        ...
        if force_regenesis {
            ...
            let keystore_path = config_dir.join(IKA_KEYSTORE_FILENAME);
            let mut keystore = Keystore::from(FileBasedKeystore::new(&keystore_path).unwrap());
            let address: IkaAddress = kp.public().into();
            keystore.add_key(None, IkaKeyPair::Ed25519(kp)).unwrap();
            IkaClientConfig {
                keystore,
                envs: vec![IkaEnv {
            ...
            .persisted(config_dir.join(IKA_CLIENT_CONFIG).as_path())
            .save()
            .unwrap();
        }
...
async fn genesis(
    ...
    let ika_config_dir = &match working_dir {
        // if a directory is specified, it must exist (it
        // will not be created)
        Some(v) => v,
        // create default Ika config dir if not specified
    ...
    // error
    let dir = ika_config_dir.read_dir().map_err(|err| {
        anyhow!(err).context(format!("Cannot open Ika config dir {:?}", ika_config_dir))
    })?;
    let files = dir.collect::<Result<Vec<_>, _>>()?;

    let client_path = ika_config_dir.join(IKA_CLIENT_CONFIG);
    let keystore_path = ika_config_dir.join(IKA_KEYSTORE_FILENAME);
    ...
    if write_config.is_none() && !files.is_empty() {
        if force {
            ...
            let is_compatible = FileBasedKeystore::new(&keystore_path).is_ok()
                && PersistedConfig::<IkaClientConfig>::read(&client_path).is_ok();
            ...
            if is_compatible {
                ...
                fs::remove_dir_all(ika_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot remove Ika config dir {:?}", ika_config_dir))
                })?;
                fs::create_dir(ika_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot create Ika config dir {:?}", ika_config_dir))
                })?;
            }
        } else if files.len() != 2 || !client_path.exists() || !keystore_path.exists() {
            bail!("Cannot run genesis with non-empty Ika config directory {}, please use the --force/-f option to remove the existing configuration", ika_config_dir.to_str().unwrap());
        }
    }

    let network_path = ika_config_dir.join(IKA_NETWORK_CONFIG);
    let genesis_path = ika_config_dir.join(IKA_GENESIS_FILENAME);
    ...
    let mut genesis_conf = match from_config {
        ...
        None => {
            if let Some(ips) = benchmark_ips {
                ...
                let path = ika_config_dir.join(IKA_BENCHMARK_GENESIS_GAS_KEYSTORE_FILENAME);
    ...
    let builder = ConfigBuilder::new(ika_config_dir);
    ...
    for validator in &mut network_config.validator_configs {
        validator.genesis = ika_config::node::Genesis::new_from_file(&genesis_path);
    }
    ...
    let fullnode_config = FullnodeConfigBuilder::new()
        ...
        .with_rpc_addr(ika_config::node::default_json_rpc_address())
        .build(&mut OsRng, &network_config);

    fullnode_config.save(ika_config_dir.join(IKA_FULLNODE_CONFIG))?;
    let mut ssfn_nodes = vec![];
    if let Some(ssfn_info) = ssfn_info {
        for (i, ssfn) in ssfn_info.into_iter().enumerate() {
            let path =
                ika_config_dir.join(ika_config::ssfn_config_file(ssfn.p2p_address.clone(), i));
            ...
            let ssfn_config = FullnodeConfigBuilder::new()
                ...
                .with_db_path(PathBuf::from("/opt/ika/db/authorities_db/full_node_db"))
                ...
                .with_genesis(Genesis::new_from_file("/opt/ika/config/genesis.blob"))
                .build(&mut OsRng, &network_config);
    ...
    let mut client_config = if client_path.exists() {
        ...
        IkaClientConfig::new(keystore.into())
    };
    ...
    client_config.add_env(IkaEnv::devnet());
