...
use crate::client_commands::{IkaClientCommandResult, IkaClientCommands};
use crate::shell::{
    install_shell_plugins, AsyncHandler, CacheKey, CommandStructure, CompletionCache, Shell,
};

const IKA: &str = "   _____       _    ______                       __
...
pub async fn start_console(
    ...
    let app: Command = IkaClientCommands::command();
    writeln!(out, "{}", IKA.cyan().bold())?;
    let mut version = env!("CARGO_PKG_VERSION").to_owned();
    if let Some(git_rev) = std::option_env!("GIT_REVISION") {
        version.push('-');
        version.push_str(git_rev);
    }
    writeln!(out, "--- Ika Console {version} ---")?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "Connecting to Ika full node. API version {}",
        client.api_version()
    )?;
    ...
    writeln!(out)?;
    writeln!(out, "Welcome to the Ika interactive console.")?;
    writeln!(out)?;
...
async fn handle_command(
    ...
    if let Ok(mut cache) = completion_cache.write() {
        match result {
            IkaClientCommandResult::Addresses(ref addresses) => {
            ...
            IkaClientCommandResult::Objects(ref objects) => {
                let objects = objects
                    .iter()
    ...
    if matches!(
        result,
        IkaClientCommandResult::Switch(SwitchResponse { env: Some(_), .. })
    ) {
        println!("Ika environment switch completed, please restart Ika console.");
        return Ok(true);
    }
    Ok(false)
}
