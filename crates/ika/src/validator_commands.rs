...
use ika_genesis_builder::validator_info::GenesisValidatorInfo;
...
use ika_types::{
    ...
    object::Owner,
    ika_system_state::{
        ika_system_state_inner_v1::{UnverifiedValidatorOperationCapV1, ValidatorV1},
        ika_system_state_summary::{IkaSystemStateSummary, IkaValidatorSummary},
    },
...
use ika_bridge::metrics::BridgeMetrics;
use ika_bridge::ika_client::IkaClient as IkaBridgeClient;
use ika_bridge::ika_transaction_builder::{
    build_committee_register_transaction, build_committee_update_url_transaction,
};
use ika_json_rpc_types::{
    IkaObjectDataOptions, IkaTransactionBlockResponse, IkaTransactionBlockResponseOptions,
};
use ika_keys::{
...
use ika_keys::{keypair_file::read_key, keystore::AccountKeystore};
...
use ika_types::crypto::{AuthorityKeyPair, NetworkKeyPair, SignatureScheme, IkaKeyPair};
use ika_types::transaction::{CallArg, ObjectArg, Transaction, TransactionData};

#[path = "unit_tests/validator_tests.rs"]
#[cfg(test)]
mod validator_tests;

const DEFAULT_GAS_BUDGET: u64 = 200_000_000; // 0.2 IKA
...
pub enum IkaValidatorCommand {
    ...
    DisplayMetadata {
        ...
        validator_address: Option<IkaAddress>,
        #[clap(name = "json", long)]
        json: Option<bool>,
    },
    ...
    ReportValidator {
        ...
        #[clap(name = "operation-cap-id", long)]
        operation_cap_id: Option<ObjectID>,
        /// The Ika Address of the validator is being reported or un-reported
        #[clap(name = "reportee-address")]
        reportee_address: IkaAddress,
    ...
    /// Ika native bridge committee member registration
    ...
    /// Update ika native bridge committee node url
...
pub enum IkaValidatorCommandResponse {
    MakeValidatorInfo,
    DisplayMetadata,
    BecomeCandidate(IkaTransactionBlockResponse),
    JoinCommittee(IkaTransactionBlockResponse),
    LeaveCommittee(IkaTransactionBlockResponse),
    UpdateMetadata(IkaTransactionBlockResponse),
    UpdateGasPrice(IkaTransactionBlockResponse),
    ReportValidator(IkaTransactionBlockResponse),
    ...
    RegisterBridgeCommittee {
        execution_response: Option<IkaTransactionBlockResponse>,
...
fn make_key_files(
    ...
    if file_name.exists() {
        ...
        let kp = match key {
            Some(key) => {
                println!(
                    "Generated new key file {:?} based on ika.keystore file.",
                    file_name
                );
                key
            }
...
impl IkaValidatorCommand {
    pub async fn execute(
        ...
        let ika_address = context.active_address()?;
        ...
        let ret = Ok(match self {
            IkaValidatorCommand::MakeValidatorInfo {
                ...
                let account_key = match context.config.keystore.get_key(&ika_address)? {
                    IkaKeyPair::Ed25519(account_key) => IkaKeyPair::Ed25519(account_key.copy()),
                    _ => panic!(
                ...
                let account_keypair: IkaKeyPair = read_keypair_from_file(account_key_file_name)?;
                ...
                let validator_info = GenesisValidatorInfo {
                    info: ika_genesis_builder::validator_info::ValidatorInfo {
                        name,
                        protocol_key: keypair.public().into(),
                        worker_key: worker_keypair.public().clone(),
                        account_address: IkaAddress::from(&account_keypair.public()),
                        network_key: network_keypair.public().clone(),
                        gas_price,
                        commission_rate: ika_config::node::DEFAULT_COMMISSION_RATE,
            ...
            IkaValidatorCommand::BecomeCandidate { file, gas_budget } => {
                ...
                IkaValidatorCommandResponse::BecomeCandidate(response)
            }

            IkaValidatorCommand::JoinCommittee { gas_budget } => {
                let gas_budget = gas_budget.unwrap_or(DEFAULT_GAS_BUDGET);
                let response =
            ...
            IkaValidatorCommand::LeaveCommittee { gas_budget } => {
            ...
            IkaValidatorCommand::DisplayMetadata {
                validator_address,
                json,
            } => {
                ...
                let ika_client = context.get_client().await?;
                display_metadata(&ika_client, validator_address, json.unwrap_or(true)).await?;
                IkaValidatorCommandResponse::DisplayMetadata
            }

            IkaValidatorCommand::UpdateMetadata {
                metadata,
                gas_budget,
            } => {
                let gas_budget = gas_budget.unwrap_or(DEFAULT_GAS_BUDGET);
                let resp = update_metadata(context, metadata, gas_budget).await?;
                IkaValidatorCommandResponse::UpdateMetadata(resp)
            }

            IkaValidatorCommand::UpdateGasPrice {
                operation_cap_id,
                gas_price,
                gas_budget,
            } => {
                ...
                IkaValidatorCommandResponse::UpdateGasPrice(resp)
            }
            ...
            IkaValidatorCommand::ReportValidator {
                ...
                .await?;
                IkaValidatorCommandResponse::ReportValidator(resp)
            }

            IkaValidatorCommand::SerializePayloadForPoP {
                account_address,
                protocol_public_key,
            } => {
                ...
                let mut intent_msg_bytes = bcs::to_bytes(&IntentMessage::new(
                    Intent::ika_app(IntentScope::ProofOfPossession),
                    msg,
                ))
                .expect("Message serialization should not fail");
                DEFAULT_EPOCH_ID.write(&mut intent_msg_bytes);
                IkaValidatorCommandResponse::SerializedPayload(Base64::encode(&intent_msg_bytes))
            }

            IkaValidatorCommand::DisplayGasPriceUpdateRawTxn {
                sender_address,
                operation_cap_id,
                new_gas_price,
                gas_budget,
                ...
                IkaValidatorCommandResponse::DisplayGasPriceUpdateRawTxn {
                    data,
                    serialized_data,
                }
            }
            IkaValidatorCommand::RegisterBridgeCommittee {
                ...
                let ika_client = context.get_client().await?;
                let active_validators = ika_client
                    .governance_api()
                ...
                if !active_validators
                    .into_iter()
                    .any(|s| s.ika_address == address)
                {
                    bail!("Address {} is not in the committee", address);
                }
                println!("Starting bridge committee registration for Ika validator: {address}, with bridge public key: {} and url: {}", ecdsa_keypair.public, bridge_authority_url);
                let ika_rpc_url = &context.config.get_active_env().unwrap().rpc;
                let bridge_metrics = Arc::new(BridgeMetrics::new_for_testing());
                let bridge_client = IkaBridgeClient::new(ika_rpc_url, bridge_metrics).await?;
                ...
                if print_unsigned_transaction_only {
                    ...
                    IkaValidatorCommandResponse::RegisterBridgeCommittee {
            ...
            IkaValidatorCommand::UpdateBridgeCommitteeNodeUrl {
                bridge_authority_url,
                print_unsigned_transaction_only,
                validator_address,
                gas_budget,
                ...
                if !committee_members
                    .into_iter()
                    .any(|(_, m)| m.ika_address == address)
                {
                    bail!("Address {} is not in the committee", address);
                }
                println!(
                    "Updating bridge committee node URL for Ika validator: {address}, url: {}",
                    bridge_authority_url
                );
                ...
                if print_unsigned_transaction_only {
                    let serialized_data = Base64::encode(bcs::to_bytes(&tx_data)?);
                    IkaValidatorCommandResponse::UpdateBridgeCommitteeURL {
                        execution_response: None,
                        serialized_unsigned_transaction: Some(serialized_data),
                    }
                } else {
...
fn check_address(
    active_address: IkaAddress,
    validator_address: Option<IkaAddress>,
    print_unsigned_transaction_only: bool,
) -> Result<IkaAddress, anyhow::Error> {
    if !print_unsigned_transaction_only {
...
async fn get_cap_object_ref(
    context: &mut WalletContext,
    operation_cap_id: Option<ObjectID>,
) -> Result<(ValidatorStatus, IkaValidatorSummary, ObjectRef)> {
    let ika_client = context.get_client().await?;
    if let Some(operation_cap_id) = operation_cap_id {
        let (status, summary) =
            get_validator_summary_from_cap_id(&ika_client, operation_cap_id).await?;
        let cap_obj_ref = ika_client
            .read_api()
            .get_object_with_options(
                summary.operation_cap_id,
                IkaObjectDataOptions::default().with_owner(),
            )
            .await?
            .object_ref_if_exists()
            .ok_or_else(|| anyhow!("OperationCap {} does not exist", operation_cap_id))?;
        Ok::<(ValidatorStatus, IkaValidatorSummary, ObjectRef), anyhow::Error>((
            status,
            summary,
            cap_obj_ref,
        ))
        ...
        let (status, summary) = get_validator_summary(&ika_client, validator_address)
            .await?
        ...
        let resp = ika_client
            .read_api()
            .get_object_with_options(cap_object_id, IkaObjectDataOptions::default().with_owner())
            .await
            .map_err(|e| anyhow!(e))?;
...
async fn update_gas_price(
    ...
    gas_price: u64,
    gas_budget: u64,
) -> Result<IkaTransactionBlockResponse> {
...
async fn report_validator(
    ...
    let validator_address = summary.ika_address;
...
async fn get_validator_summary_from_cap_id(
    client: &IkaClient,
    operation_cap_id: ObjectID,
) -> anyhow::Result<(ValidatorStatus, IkaValidatorSummary)> {
    let resp = client
        .read_api()
        .get_object_with_options(operation_cap_id, IkaObjectDataOptions::default().with_bcs())
        .await?;
...
async fn construct_unsigned_0x5_txn(
    ...
    let ika_client = context.get_client().await?;
    let mut args = vec![CallArg::IKA_SYSTEM_MUT];
    ...
    let gas_obj_ref = get_gas_obj_ref(sender, &ika_client, gas_budget).await?;
    TransactionData::new_move_call(
        sender,
        IKA_SYSTEM_PACKAGE_ID,
        ident_str!("ika_system").to_owned(),
...
async fn call_0x5(
    ...
    let signature =
        context
            .config
            .keystore
            .sign_secure(&sender, &tx_data, Intent::ika_transaction())?;
...
impl Display for IkaValidatorCommandResponse {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            ...
            IkaValidatorCommandResponse::UpdateMetadata(response) => {
                write!(writer, "{}", write_transaction_response(response)?)?;
            }
            IkaValidatorCommandResponse::UpdateGasPrice(response) => {
                write!(writer, "{}", write_transaction_response(response)?)?;
            }
            IkaValidatorCommandResponse::ReportValidator(response) => {
                write!(writer, "{}", write_transaction_response(response)?)?;
            }
            IkaValidatorCommandResponse::SerializedPayload(response) => {
...
pub async fn get_validator_summary(
    client: &IkaClient,
    validator_address: IkaAddress,
) -> anyhow::Result<Option<(ValidatorStatus, IkaValidatorSummary)>> {
    let IkaSystemStateSummary {
        active_validators,
...
async fn get_pending_candidate_summary(
    ...
    let pending_validators = ika_client
        .read_api()
        ...
        .collect::<Vec<_>>();
    let resps = ika_client
        .read_api()
        .multi_get_object_with_options(
            pending_validators,
            IkaObjectDataOptions::default().with_bcs(),
        )
        .await?;
    for resp in resps {
        ...
        if val.verified_metadata().ika_address == validator_address {
            return Ok(Some(val));
        }
    }
    Ok(None)
...
async fn update_metadata(
    ...
    match metadata {
        ...
        MetadataUpdate::ProtocolPubKey { file } => {
            ...
            let pop = generate_proof_of_possession(&protocol_key_pair, ika_address);
            let args = vec![
                CallArg::Pure(
                    bcs::to_bytes(&AuthorityPublicKeyBytes::from_bytes(
...
async fn check_status(
    ...
    let ika_client = context.get_client().await?;
    let validator_address = context.active_address()?;
    let summary = get_validator_summary(&ika_client, validator_address).await?;
    if summary.is_none() {
