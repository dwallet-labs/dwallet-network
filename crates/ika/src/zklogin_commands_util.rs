...
use ika_types::crypto::{PublicKey, IkaKeyPair};
use ika_types::multisig::{MultiSig, MultiSigPublicKey};
...
pub async fn perform_zk_login_test_tx(
    ...
    let skp1 = IkaKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([1; 32])));
    ...
    let sender = if test_multisig {
        ...
        IkaAddress::from(&multisig_pk)
    } else {
        println!("Use single zklogin address as sender");
        IkaAddress::try_from_unpadded(&zk_login_inputs)?
    };
    println!("Sender: {:?}", sender);

    // Request some coin from faucet and build a test transaction.
    let ika = IkaClientBuilder::default().build(fullnode_url).await?;
    ...
    if response.data.len() != 2 {
        panic!("Faucet did not work correctly and the provided Ika address has no coins")
    }
    ...
    let txb_res = ika
        ...
        .transfer_object(
            ...
            Some(gas_coin),
            5000000,
            IkaAddress::ZERO, // as a demo, send to a dummy address
        )
        .await?;
    println!(
...
fn get_config(network: &str) -> (&str, &str) {
    match network {
        "devnet" => (
            "https://faucet.devnet.ika.io/gas",
            "https://rpc.devnet.ika.io:443",
        ),
