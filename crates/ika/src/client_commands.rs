...
// use ika_config::verifier_signing_config::VerifierSigningConfig;
...
//     IkaCoinMetadata, IkaData, IkaExecutionStatus, IkaObjectData, IkaObjectDataOptions,
//     IkaObjectResponse, IkaObjectResponseQuery, IkaParsedData, IkaProtocolConfigValue, IkaRawData,
//     IkaTransactionBlockEffects, IkaTransactionBlockEffectsAPI, IkaTransactionBlockResponse,
...
// };
// use ika_package_management::{LockCommand, PublishedAtError};
// use ika_replay::ReplayToolCommand;
// use ika_sdk::{
//     apis::ReadApi,
//     ika_client_config::{IkaClientConfig, IkaEnv},
//     wallet_context::WalletContext,
//     IkaClient, IKA_COIN_TYPE, IKA_DEVNET_URL, IKA_LOCAL_NETWORK_URL, IKA_LOCAL_NETWORK_URL_0,
//     IKA_TESTNET_URL,
// };
// use ika_types::{
...
//         /// Show balance for the specified coin (e.g., 0x2::ika::IKA).
...
//         #[clap(
//             long,
//             value_parser = parse_ika_type_tag,
//             num_args(1..),
//         )]
...
//         args: Vec<IkaJsonValue>,
//         /// Optional computation price for this call. Currently use only for testing and not in production environments.
//         #[clap(hide = true)]
...
//     },
// 
//     /// List all Ika environments
//     Envs,
// 
//     /// Execute a Signed Transaction. This is useful when the user prefers to sign elsewhere and use this command to execute.
//     ExecuteSignedTx {
//         /// BCS serialized transaction data bytes without its type tag, as base64 encoded string. This is the output of ika client command using --serialize-unsigned-transaction.
//         #[clap(long)]
//         tx_bytes: String,
// 
...
//         /// BCS serialized sender signed data, as base64 encoded string. This is the output of ika client command using --serialize-signed-transaction.
//         #[clap(long)]
...
//         /// objects owned by `ika client active-address`.
//         #[clap(name = "owner_address")]
//         address: Option<KeyIdentity>,
//     },
...
//     },
// 
//     /// Pay all residual IKA coins to the recipient with input coins, after deducting the gas cost.
...
//     },
// 
//     /// Pay IKA coins to recipients following following specified amounts, with input coins.
//     /// Length of recipients must be the same as that of amounts.
//     /// The input coins also include the coin for gas payment, so no extra gas coin is required.
//     PayIka {
//         /// The input coins to be used for pay recipients, including the gas coin.
...
// #[derive(Args, Debug)]
// pub struct Opts {
//     /// An optional gas budget for this transaction (in NIKA). If gas budget is not provided, the
...
//     /// be used to execute transaction with `ika client execute-signed-tx --tx-bytes <TX_BYTES>`.
...
//     /// `ika client execute-combined-signed-tx --signed-tx-bytes <SIGNED_TX_BYTES>`.
//     #[arg(long, required = false)]
...
// }
// 
// impl IkaClientCommands {
//     pub async fn execute(
//         self,
//         context: &mut WalletContext,
//     ) -> Result<IkaClientCommandResult, anyhow::Error> {
//         let ret = match self {
//             IkaClientCommands::ProfileTransaction {
//                 tx_digest,
//                 profile_output,
//             } => {
...
//                 let _command_result =
//                     ika_replay::execute_replay_command(Some(rpc), false, false, None, None, cmd)
//                         .await?;
//                 // this will be displayed via trace info, so no output is needed here
//                 IkaClientCommandResult::NoOutput
//             }
//             IkaClientCommands::ReplayTransaction {
//                 tx_digest,
//                 gas_info: _,
//                 ptb_info: _,
//                 executor_version,
...
//                 IkaClientCommandResult::NoOutput
//             }
//             IkaClientCommands::ReplayCheckpoints {
//                 start,
//                 end,
//                 terminate_early,
//             } => {
...
//                 IkaClientCommandResult::NoOutput
//             }
//             IkaClientCommands::Addresses { sort_by_alias } => {
//                 let active_address = context.active_address()?;
//                 let mut addresses: Vec<(String, IkaAddress)> = context
//                     .config
//                     .keystore
...
//                     addresses,
//                 };
//                 IkaClientCommandResult::Addresses(output)
//             }
//             IkaClientCommands::Balance {
//                 address,
//                 coin_type,
//                 with_coins,
//             } => {
...
//                     coins.push(c);
//                 }
//                 let ika_type_tag = canonicalize_type(IKA_COIN_TYPE)?;
// 
//                 // show IKA first
//                 let ordered_coins_ika_first = coins_by_type
//                     .remove(&ika_type_tag)
//                     .into_iter()
//                     .chain(coins_by_type.into_values())
//                     .collect();
// 
//                 IkaClientCommandResult::Balance(ordered_coins_ika_first, with_coins)
//             }
// 
//             IkaClientCommands::DynamicFieldQuery { id, cursor, limit } => {
//                 let client = context.get_client().await?;
//                 let df_read = client
//                     .read_api()
//                     .get_dynamic_fields(id, cursor, Some(limit))
//                     .await?;
//                 IkaClientCommandResult::DynamicFieldQuery(df_read)
//             }
// 
//             IkaClientCommands::Upgrade {
//                 package_path,
//                 upgrade_capability,
//                 build_config,
...
//                     package_path
//                         .canonicalize()
//                         .map_err(|e| IkaError::ModulePublishFailure {
...
//                     ika_package_management::set_package_id(
//                         &package_path,
...
//                     let _ = ika_package_management::set_package_id(
//                         &package_path,
//                         build_config.install_dir.clone(),
//                         &chain_id,
//                         previous_id,
...
//                 .await?;
// 
//                 if let IkaClientCommandResult::TransactionBlock(ref response) = result {
//                     if let Err(e) = ika_package_management::update_lock_file(
//                         context,
//                         LockCommand::Upgrade,
...
//                     return Err(IkaError::ModulePublishFailure {
//                         error:
...
//                             compiling the package normally with `ika move build`."
//                                 .to_string(),
//                     }
//                     .into());
//                 }
...
//             }
// 
//             IkaClientCommands::VerifyBytecodeMeter {
//                 protocol_version,
//                 module_paths,
//                 package_path,
//                 build_config,
...
//                 let mut verifier = ika_execution::verifier(
//                     &protocol_config,
//                     signing_limits,
...
//                 );
// 
//                 IkaClientCommandResult::VerifyBytecodeMeter {
//                     success: !exceeded,
...
//             }
// 
//             IkaClientCommands::Object { id, bcs } => {
//                 // Fetch the object ref
//                 let client = context.get_client().await?;
//                 if !bcs {
//                     let object_read = client
//                         .read_api()
//                         .get_object_with_options(id, IkaObjectDataOptions::full_content())
//                         .await?;
//                     IkaClientCommandResult::Object(object_read)
//                 } else {
//                     let raw_object_read = client
//                         .read_api()
//                         .get_object_with_options(id, IkaObjectDataOptions::bcs_lossless())
//                         .await?;
//                     IkaClientCommandResult::RawObject(raw_object_read)
//                 }
//             }
// 
//             IkaClientCommands::TransactionBlock { digest } => {
//                 let client = context.get_client().await?;
//                 let tx_read = client
//                     .read_api()
...
//                     )
//                     .await?;
//                 IkaClientCommandResult::TransactionBlock(tx_read)
//             }
// 
//             IkaClientCommands::Call {
//                 package,
//                 module,
//                 function,
//                 type_args,
...
//                 // without failing IkaJSON's checks.
//                 let args = args
//                     .into_iter()
//                     .map(|value| IkaJsonValue::new(convert_number_to_string(value.to_json_value())))
//                     .collect::<Result<_, _>>()?;
// 
...
//                     sender, tx_kind, context, None, computation_price, opts.gas, opts.rest,
//                 )
//                 .await?
//             }
// 
//             IkaClientCommands::Transfer {
//                 to,
//                 object_id,
//                 opts,
//             } => {
...
//             }
// 
//             IkaClientCommands::TransferIka {
//                 to,
//                 ika_coin_object_id: object_id,
//                 amount,
//                 opts,
//             } => {
//                 let signer = context.get_object_owner(&object_id).await?;
...
//                 let tx_kind = client
//                     .transaction_builder()
//                     .transfer_ika_tx_kind(to, amount);
//                 dry_run_or_execute_or_serialize(
//                     signer,
//                     tx_kind,
//                     context,
...
//                     .collect::<Result<Vec<IkaAddress>, anyhow::Error>>()
//                     .map_err(|e| anyhow!("{e}"))?;
...
//                         bail!("Gas coin is in input coins of Pay transaction, use PayIka transaction instead!");
//                     }
//                 }
// 
//                 dry_run_or_execute_or_serialize(
...
//                 ensure!(
//                     !input_coins.is_empty(),
//                     "PayIka transaction requires a non-empty list of input coins"
//                 );
//                 ensure!(
//                     !recipients.is_empty(),
//                     "PayIka transaction requires a non-empty list of recipient addresses"
//                 );
//                 ensure!(
//                     recipients.len() == amounts.len(),
//                     format!(
...
//                 let tx_kind = client
//                     .transaction_builder()
//                     .pay_ika_tx_kind(recipients, amounts)?;
// 
//                 dry_run_or_execute_or_serialize(
//                     signer,
//                     tx_kind,
...
//                 ensure!(
//                     !input_coins.is_empty(),
//                     "PayAllIka transaction requires a non-empty list of input coins"
//                 );
//                 let recipient = get_identity_address(Some(recipient), context)?;
//                 let signer = context.get_object_owner(&input_coins[0]).await?;
//                 let client = context.get_client().await?;
//                 let tx_kind = client.transaction_builder().pay_all_ika_tx_kind(recipient);
//                 dry_run_or_execute_or_serialize(
//                     signer,
//                     tx_kind,
//                     context,
...
//             }
// 
//             IkaClientCommands::Objects { address } => {
//                 let address = get_identity_address(address, context)?;
//                 let client = context.get_client().await?;
...
//                                 IkaObjectDataOptions::full_content(),
//                             )),
//                             cursor,
//                             None,
//                         )
...
//                     }
//                 }
//                 IkaClientCommandResult::Objects(objects)
//             }
// 
//             IkaClientCommands::NewAddress {
//                 key_scheme,
//                 alias,
//                 derivation_path,
//                 word_length,
...
//                 };
// 
//                 IkaClientCommandResult::NewAddress(NewAddressOutput {
//                     alias,
//                     address,
//                     key_scheme: scheme,
...
//                     .map(|(_val, object)| IKACoin::try_from(object).unwrap())
//                     .collect();
//                 IkaClientCommandResult::Gas(coins)
//             }
//             IkaClientCommands::Faucet { address, url } => {
//                 let address = get_identity_address(address, context)?;
...
//                             IKA_DEVNET_URL => "https://faucet.devnet.ika.io/v1/gas",
//                             IKA_TESTNET_URL => "https://faucet.testnet.ika.io/v1/gas",
//                             IKA_LOCAL_NETWORK_URL | IKA_LOCAL_NETWORK_URL_0 => "http://127.0.0.1:9123/gas",
...
//                 IkaClientCommandResult::NoOutput
//             }
//             IkaClientCommands::ChainIdentifier => {
//                 let ci = context
//                     .get_client()
//                     .await?
//                     .read_api()
...
//                     .await?;
//                 IkaClientCommandResult::ChainIdentifier(ci)
//             }
//             IkaClientCommands::SplitCoin {
//                 coin_id,
//                 amounts,
//                 count,
//                 opts,
...
//                 .await?
//             }
//             IkaClientCommands::Switch { address, env } => {
//                 let mut addr = None;
// 
...
//                 }
//                 context.config.save()?;
//                 IkaClientCommandResult::Switch(SwitchResponse { address: addr, env })
//             }
//             IkaClientCommands::ActiveAddress => {
//                 IkaClientCommandResult::ActiveAddress(context.active_address().ok())
//             }
// 
//             IkaClientCommands::ExecuteSignedTx {
//                 tx_bytes,
//                 signatures,
//             } => {
...
//                     .to_vec()
//                     .map_err(|_| anyhow!("Invalid Base64 encoding"))?
//                 ).map_err(|_| anyhow!("Failed to parse tx bytes, check if it matches the output of ika client commands with --serialize-unsigned-transaction"))?;
// 
...
//                 IkaClientCommandResult::TransactionBlock(response)
//             }
//             IkaClientCommands::ExecuteCombinedSignedTx { signed_tx_bytes } => {
//                 let data: SenderSignedData = bcs::from_bytes(
//                     &Base64::try_from(signed_tx_bytes)
//                         .map_err(|_| anyhow!("Invalid Base64 encoding"))?
//                         .to_vec()
//                         .map_err(|_| anyhow!("Invalid Base64 encoding"))?
//                 ).map_err(|_| anyhow!("Failed to parse SenderSignedData bytes, check if it matches the output of ika client commands with --serialize-signed-transaction"))?;
//                 let transaction = Envelope::<SenderSignedData, EmptySignInfo>::new(data);
//                 let response = context.execute_transaction_may_fail(transaction).await?;
//                 IkaClientCommandResult::TransactionBlock(response)
//             }
//             IkaClientCommands::NewEnv {
//                 alias,
//                 rpc,
//                 ws,
//                 basic_auth,
...
//                 context.config.save()?;
//                 IkaClientCommandResult::NewEnv(env)
//             }
//             IkaClientCommands::ActiveEnv => {
//                 IkaClientCommandResult::ActiveEnv(context.config.active_env.clone())
//             }
//             IkaClientCommands::Envs => IkaClientCommandResult::Envs(
//                 context.config.envs.clone(),
//                 context.config.active_env.clone(),
//             ),
//             IkaClientCommands::VerifySource {
//                 package_path,
//                 build_config,
//                 verify_deps,
//                 skip_source,
...
//                     .await?;
// 
//                 IkaClientCommandResult::VerifySource
//             }
//             IkaClientCommands::PTB(ptb) => {
...
//     }
// 
//     pub fn switch_env(config: &mut IkaClientConfig, env: &str) -> Result<(), anyhow::Error> {
//         let env = Some(env.into());
//         ensure!(config.get_env(&env).is_some(), "Environment config not found for [{env:?}], add new environment config using the `ika client new-env` command.");
...
//                  - update the `Move.lock` address using the `ika manage-package` command to be the same as the `Move.toml`; OR
//                  - check that your `ika active-env` {env_alias} corresponds to the chain on which the package is published (i.e., devnet, testnet, mainnet); OR
...
//             upgrade_capability,
//             IkaObjectDataOptions::default().with_bcs().with_owner(),
//         )
//         .await?;
// 
...
//     if let Some(Some(IkaProtocolConfigValue::U32(min_version))) = protocol_config
//         .attributes
...
// 
//     // Check that the package's Move version is compatible with the chain's
//     if let Some(Some(IkaProtocolConfigValue::U32(max_version))) =
...
//         .map_err(|e| IkaError::ModuleBuildFailure {
...
// }
// 
// impl Display for IkaClientCommandResult {
//     fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
//         let mut writer = String::new();
//         match self {
//             IkaClientCommandResult::Addresses(addresses) => {
...
//                 write!(f, "{}", table)?
//             }
//             IkaClientCommandResult::Balance(coins, with_coins) => {
//                 if coins.is_empty() {
...
//                 write!(f, "{}", table)?;
//             }
//             IkaClientCommandResult::DynamicFieldQuery(df_refs) => {
...
//                 write!(f, "{}", table)?
//             }
//             IkaClientCommandResult::Gas(gas_coins) => {
...
// 
//                 let mut builder = TableBuilder::default();
//                 builder.set_header(vec!["gasCoinId", "nikaBalance (NIKA)", "ikaBalance (IKA)"]);
//                 for coin in &gas_coins {
//                     builder.push_record(vec![
//                         coin.gas_coin_id.to_string(),
//                         coin.nika_balance.to_string(),
//                         coin.ika_balance.to_string(),
//                     ]);
//                 }
//                 let mut table = builder.build();
...
//                 write!(f, "{}", table)?;
//             }
//             IkaClientCommandResult::NewAddress(new_address) => {
//                 let mut builder = TableBuilder::default();
...
//                 write!(f, "{}", table)?
//             }
//             IkaClientCommandResult::Object(object_read) => match object_read.object() {
//                 Ok(obj) => {
...
//             },
//             IkaClientCommandResult::Objects(object_refs) => {
...
//                     Ok(v) => match &v.bcs {
//                         Some(IkaRawData::MoveObject(o)) => {
//                             format!("{:?}\nNumber of bytes: {}", o.bcs_bytes, o.bcs_bytes.len())
//                         }
//                         Some(IkaRawData::Package(p)) => {
...
//             }
//             IkaClientCommandResult::SerializedUnsignedTransaction(tx_data) => {
//                 writeln!(
//                     writer,
//                     "{}",
...
//                 )?;
//             }
//             IkaClientCommandResult::SerializedSignedTransaction(sender_signed_tx) => {
//                 writeln!(
//                     writer,
//                     "{}",
...
//                 )?;
//             }
//             IkaClientCommandResult::SyncClientState => {
//                 writeln!(writer, "Client state sync complete.")?;
//             }
//             IkaClientCommandResult::ChainIdentifier(ci) => {
//                 writeln!(writer, "{}", ci)?;
//             }
//             IkaClientCommandResult::Switch(response) => {
//                 write!(writer, "{}", response)?;
//             }
//             IkaClientCommandResult::ActiveAddress(response) => {
//                 match response {
//                     Some(r) => write!(writer, "{}", r)?,
//                     None => write!(writer, "None")?,
//                 };
//             }
//             IkaClientCommandResult::ActiveEnv(env) => {
//                 write!(writer, "{}", env.as_deref().unwrap_or("None"))?;
//             }
//             IkaClientCommandResult::NewEnv(env) => {
//                 writeln!(writer, "Added new Ika env [{}] to config.", env.alias)?;
//             }
//             IkaClientCommandResult::Envs(envs, active) => {
//                 let mut builder = TableBuilder::default();
...
//             }
//             IkaClientCommandResult::NoOutput => {}
//             IkaClientCommandResult::DryRun(response) => {
//                 writeln!(f, "{}", Pretty(response))?;
//             }
//             IkaClientCommandResult::DevInspect(response) => {
//                 writeln!(f, "{}", Pretty(response))?;
//             }
//         }
...
// }
// 
// impl Debug for IkaClientCommandResult {
//     fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
...
// }
// 
// impl IkaClientCommandResult {
//     pub fn objects_response(&self) -> Option<Vec<IkaObjectResponse>> {
//         use IkaClientCommandResult::*;
//         match self {
...
//     }
// 
//     pub fn tx_block_response(&self) -> Option<&IkaTransactionBlockResponse> {
//         use IkaClientCommandResult::*;
//         match self {
...
//         match &mut self {
//             IkaClientCommandResult::DryRun(DryRunTransactionBlockResponse { effects, .. })
//             | IkaClientCommandResult::TransactionBlock(IkaTransactionBlockResponse {
...
//                 ..
//             }) => (),
//             IkaClientCommandResult::ActiveAddress(_)
//             | IkaClientCommandResult::ActiveEnv(_)
//             | IkaClientCommandResult::Addresses(_)
//             | IkaClientCommandResult::Balance(_, _)
//             | IkaClientCommandResult::ChainIdentifier(_)
//             | IkaClientCommandResult::DynamicFieldQuery(_)
//             | IkaClientCommandResult::DevInspect(_)
//             | IkaClientCommandResult::Envs(_, _)
//             | IkaClientCommandResult::Gas(_)
//             | IkaClientCommandResult::NewAddress(_)
//             | IkaClientCommandResult::NewEnv(_)
//             | IkaClientCommandResult::NoOutput
//             | IkaClientCommandResult::Object(_)
//             | IkaClientCommandResult::Objects(_)
//             | IkaClientCommandResult::RawObject(_)
//             | IkaClientCommandResult::SerializedSignedTransaction(_)
//             | IkaClientCommandResult::SerializedUnsignedTransaction(_)
//             | IkaClientCommandResult::Switch(_)
//             | IkaClientCommandResult::SyncClientState
//             | IkaClientCommandResult::VerifyBytecodeMeter { .. }
//             | IkaClientCommandResult::VerifySource => (),
//         }
...
//     pub active_address: IkaAddress,
//     pub addresses: Vec<(String, IkaAddress)>,
// }
// 
// #[derive(Serialize)]
// #[serde(rename_all = "camelCase")]
...
//     #[serde(skip_serializing_if = "Option::is_none")]
//     pub content: Option<IkaParsedData>,
// }
// 
// impl From<&IkaObjectData> for ObjectOutput {
//     fn from(obj: &IkaObjectData) -> Self {
...
//     pub nika_balance: u64,
//     pub ika_balance: String,
// }
// 
// impl From<&IKACoin> for GasCoinOutput {
//     fn from(gas_coin: &IKACoin) -> Self {
//         Self {
//             gas_coin_id: *gas_coin.id(),
//             nika_balance: gas_coin.value(),
//             ika_balance: format_balance(gas_coin.value() as u128, 9, 2, None),
//         }
//     }
// }
// 
...
// 
// impl ObjectsOutput {
//     fn from(obj: IkaObjectResponse) -> Result<Self, anyhow::Error> {
//         let obj = obj.into_object()?;
//         // this replicates the object type display as in the ika explorer
//         let object_type = match obj.type_ {
//             Some(ika_types::base_types::ObjectType::Struct(x)) => {
...
//             }
//             Some(ika_types::base_types::ObjectType::Package) => "Package".to_string(),
//             None => "unknown".to_string(),
//         };
//         Ok(Self {
//             object_id: obj.object_id,
...
//         })
//     }
//     fn from_vec(objs: Vec<IkaObjectResponse>) -> Result<Vec<Self>, anyhow::Error> {
//         objs.into_iter()
...
// #[derive(Serialize)]
// #[serde(untagged)]
// pub enum IkaClientCommandResult {
//     ActiveAddress(Option<IkaAddress>),
//     ActiveEnv(Option<String>),
//     Addresses(AddressesOutput),
//     Balance(Vec<(Option<IkaCoinMetadata>, Vec<Coin>)>, bool),
//     ChainIdentifier(String),
//     DynamicFieldQuery(DynamicFieldPage),
//     DryRun(DryRunTransactionBlockResponse),
//     DevInspect(DevInspectResults),
//     Envs(Vec<IkaEnv>, Option<String>),
//     Gas(Vec<IKACoin>),
//     NewAddress(NewAddressOutput),
//     NewEnv(IkaEnv),
//     NoOutput,
//     Object(IkaObjectResponse),
//     Objects(Vec<IkaObjectResponse>),
//     RawObject(IkaObjectResponse),
//     SerializedSignedTransaction(SenderSignedData),
//     SerializedUnsignedTransaction(TransactionData),
//     Switch(SwitchResponse),
//     SyncClientState,
//     TransactionBlock(IkaTransactionBlockResponse),
//     VerifyBytecodeMeter {
//         success: bool,
...
//             } else {
//                 println!("Request successful. It can take up to 1 minute to get the coin. Run ika client gas to check your gas coins.");
//             }
//         }
...
//     coins_by_type: &Vec<(Option<IkaCoinMetadata>, Vec<Coin>)>,
...
//         .transaction_builder()
//         .tx_data_for_dry_run(signer, kind, gas_budget, computation_price, gas_payment, sponsor)
//         .await;
...
//     let resp = IkaClientCommandResult::DryRun(response)
...
// ///
// /// A = computation cost + GAS_SAFE_OVERHEAD * computation price per unit size
// /// B = computation cost + storage cost - storage rebate + GAS_SAFE_OVERHEAD * computation price per unit size
// /// overhead
// ///
...
//     let client = context.get_client().await?;
//     let Ok(IkaClientCommandResult::DryRun(dry_run)) =
//         execute_dry_run(context, signer, kind, None, computation_price, gas_payment, sponsor).await
//     else {
//         bail!("Could not automatically determine the gas budget. Please supply one using the --gas-budget flag.")
//     };
// 
//     let rgp = client.read_api().get_computation_price_per_unit_size().await?;
// 
...
// ) -> u64 {
//     let safe_overhead = GAS_SAFE_OVERHEAD * computation_price_per_unit_size;
...
// pub async fn max_gas_budget(client: &IkaClient) -> Result<u64, anyhow::Error> {
//     let cfg = client.read_api().get_protocol_config(None).await?;
//     Ok(match cfg.attributes.get("max_tx_gas") {
//         Some(Some(ika_json_rpc_types::IkaProtocolConfigValue::U64(y))) => *y,
//         _ => bail!(
...
// // TODO (stefan): Add computation_price option for all commands and remove it from this function
...
//     );
//     let computation_price = if let Some(computation_price) = computation_price {
//         computation_price
//     } else {
//         context.get_computation_price_per_unit_size().await?
//     };
// 
//     let client = context.get_client().await?;
// 
...
//         Ok(IkaClientCommandResult::SerializedUnsignedTransaction(
//             tx_data,
//         ))
//     } else {
...
//             Ok(IkaClientCommandResult::SerializedSignedTransaction(
//                 sender_signed_data,
//             ))
//         } else {
...
//                 anyhow!("Effects from IkaTransactionBlockResult should not be empty")
//             })?;
...
//                 ));
//             }
//             Ok(IkaClientCommandResult::TransactionBlock(response))
//         }
//     }
// }
// 
...
//     computation_price: u64,
//     gas_payment: Option<Vec<ObjectID>>,
//     gas_sponsor: Option<IkaAddress>,
//     skip_checks: Option<bool>,
// ) -> Result<IkaClientCommandResult, anyhow::Error> {
//     let client = context.get_client().await?;
//     let gas_budget = gas_budget.map(ika_serde::BigInt::from);
...
//             signer,
//             tx_kind,
//             Some(ika_serde::BigInt::from(computation_price)),
//             None,
//             Some(dev_inspect_args),
//         )
//         .await?;
//     Ok(IkaClientCommandResult::DevInspect(dev_inspect_result))
// }
// 
// pub(crate) async fn prerender_clever_errors(
//     effects: &mut IkaTransactionBlockEffects,
//     read_api: &ReadApi,
// ) {
//     let IkaTransactionBlockEffects::V1(effects) = effects;
//     if let IkaExecutionStatus::Failure { error } = &mut effects.status {
