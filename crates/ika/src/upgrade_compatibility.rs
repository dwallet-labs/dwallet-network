...
use ika_json_rpc_types::{IkaObjectDataOptions, IkaRawData};
use ika_move_build::CompiledPackage;
use ika_protocol_config::ProtocolConfig;
use ika_sdk::IkaClient;
use ika_types::{base_types::ObjectID, execution_config_utils::to_binary_config};
...
pub(crate) async fn check_compatibility(
    ...
    let existing_obj_read = client
        .read_api()
        .get_object_with_options(package_id, IkaObjectDataOptions::new().with_bcs())
        .await
    ...
    let existing_package = match existing_obj {
        ...
        IkaRawData::MoveObject(_) => Err(anyhow!("Object found when package expected")),
    }?;
