...
use ika_json::{is_receiving_argument, primitive_type};
use ika_json_rpc_types::{IkaObjectData, IkaObjectDataOptions, IkaRawData};
...
use ika_types::{
    ...
    Identifier, TypeTag, IKA_FRAMEWORK_PACKAGE_ID,
};
...
impl<'a> PTBBuilder<'a> {
    ...
    fn declare_possible_address_binding(&mut self, ident: String, possible_addr: &Spanned<PTBArg>) {
        match possible_addr.value {
            ...
            // If we encounter a dotted string e.g., "foo.0" or "ika.io" or something like that
    ...
    /// Fetch the `IkaObjectData` for an object ID -- this is used for object resolution.
    async fn get_object(&self, object_id: ObjectID, obj_loc: Span) -> PTBResult<IkaObjectData> {
        let res = self
            .reader
            .get_object_with_options(
                object_id,
                IkaObjectDataOptions::new().with_type().with_owner(),
            )
            .await
