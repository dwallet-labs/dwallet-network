...
use crate::{
    client_commands::{dry_run_or_execute_or_serialize, Opts, OptsWithGas, IkaClientCommandResult},
    client_ptb::{
...
use ika_sdk::{wallet_context::WalletContext, IkaClient};
use ika_types::{
...
impl PTB {
    ...
    pub async fn execute(self, context: &mut WalletContext) -> Result<(), Error> {
        ...
        let transaction_response = match transaction_response {
            IkaClientCommandResult::DryRun(_) => {
            ...
            IkaClientCommandResult::TransactionBlock(response) => response,
...
pub fn ptb_description() -> clap::Command {
    clap::Command::new("ika client ptb")
        .about(
        ...
        .arg(arg!(
            --"gas-budget" <NIKA>
            "An optional gas budget for this PTB (in NIKA). If gas budget is not provided, the \
        ...
        .long_help(
            ...
            \n --make-move-vec <ika::coin::Coin<ika::ika::IKA>> [gas]"
        )
