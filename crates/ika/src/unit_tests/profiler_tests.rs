...
/// `ika = { path = ".", features = ["tracing"] }` to our dev-dependencies.
...
async fn test_profiler() {
    ...
    let testnet_url = "https://fullnode.testnet.ika.io:443".to_string();
    ...
    let command_result =
        ika_replay::execute_replay_command(Some(testnet_url), false, false, None, None, cmd).await;
