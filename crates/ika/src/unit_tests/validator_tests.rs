...
use crate::validator_commands::{
    get_validator_summary, IkaValidatorCommand, IkaValidatorCommandResponse,
};
use anyhow::Ok;
...
use ika_types::{base_types::IkaAddress, crypto::Signature, transaction::Transaction};
use test_cluster::TestClusterBuilder;

#[tokio::test]
async fn test_print_raw_rgp_txn() -> Result<(), anyhow::Error> {
    let test_cluster = TestClusterBuilder::new().build().await;
    let keypair: &IkaKeyPair = test_cluster
        .swarm
        .config()
        .validator_configs
        .first()
        ...
        .account_key_pair
        .keypair();
    let validator_address: IkaAddress = IkaAddress::from(&keypair.public());
    let mut context = test_cluster.wallet;
    let ika_client = context.get_client().await?;
    let (_, summary) = get_validator_summary(&ika_client, validator_address)
        .await?
        .unwrap();
    let operation_cap_id = summary.operation_cap_id;

    // Execute the command and get the serialized transaction data.
    let response = IkaValidatorCommand::DisplayGasPriceUpdateRawTxn {
    ...
    .execute(&mut context)
    .await?;
    let IkaValidatorCommandResponse::DisplayGasPriceUpdateRawTxn {
        data,
        serialized_data,
    } = response
    else {
    ...
    let signature = Signature::new_secure(
        &IntentMessage::new(Intent::ika_transaction(), deserialized_data),
        keypair,
    );
