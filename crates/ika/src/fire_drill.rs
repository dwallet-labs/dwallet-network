...
//!
//! Example usage:
//! ika fire-drill metadata-rotation \
//! --ika-node-config-path validator.yaml \
...
use ika_config::node::{AuthorityKeyPairWithPath, KeyPairWithPath};
use ika_config::{local_ip_utils, Config, NodeConfig, PersistedConfig};
use ika_json_rpc_types::{IkaExecutionStatus, IkaTransactionBlockResponseOptions};
use ika_keys::keypair_file::read_keypair_from_file;
use ika_sdk::{rpc_types::IkaTransactionBlockEffectsAPI, IkaClient, IkaClientBuilder};
use ika_types::base_types::{ObjectRef, IkaAddress};
use ika_types::crypto::{generate_proof_of_possession, get_key_pair, IkaKeyPair};
use ika_types::multiaddr::{Multiaddr, Protocol};
use ika_types::transaction::{
    CallArg, Transaction, TransactionData, TEST_ONLY_GAS_UNIT_FOR_GENERIC,
};
use ika_types::{committee::EpochId, crypto::get_authority_key_pair, IKA_SYSTEM_PACKAGE_ID};
use tracing::info;
...
pub struct MetadataRotation {
    /// Path to ika node config.
    #[clap(long = "ika-node-config-path")]
    ika_node_config_path: PathBuf,
...
async fn run_metadata_rotation(metadata_rotation: MetadataRotation) -> anyhow::Result<()> {
    let MetadataRotation {
        ika_node_config_path,
        account_key_path,
        fullnode_rpc_url,
    } = metadata_rotation;
    let account_key = read_keypair_from_file(&account_key_path)?;
    let config: NodeConfig = PersistedConfig::read(&ika_node_config_path).map_err(|err| {
        err.context(format!(
            "Cannot open Ika Node Config file at {:?}",
            ika_node_config_path
        ))
    })?;

    let ika_client = IkaClientBuilder::default().build(fullnode_rpc_url).await?;
    let ika_address = IkaAddress::from(&account_key.public());
    let starting_epoch = current_epoch(&ika_client).await?;
    info!("Running Metadata Rotation fire drill for validator address {ika_address} in epoch {starting_epoch}.");

    // Prepare new metadata for next epoch
    let new_config_path =
        update_next_epoch_metadata(&ika_node_config_path, &config, &ika_client, &account_key)
            .await?;

    let current_epoch = current_epoch(&ika_client).await?;
    if current_epoch > starting_epoch {
        bail!("Epoch already advanced to {current_epoch}");
    }
    let target_epoch = starting_epoch + 1;
    wait_for_next_epoch(&ika_client, target_epoch).await?;
    info!("Just advanced to epoch {target_epoch}");

    // Replace new config
    std::fs::rename(new_config_path, ika_node_config_path)?;
    info!("Updated Ika Node config.");
...
pub async fn get_gas_obj_ref(
    ...
    let coins = ika_client
        .coin_read_api()
        .get_coins(ika_address, Some("0x2::ika::IKA".into()), None, None)
        .await?
        .data;
    let gas_obj = coins.iter().find(|c| c.balance >= minimal_gas_balance);
    if gas_obj.is_none() {
        bail!("Validator doesn't have enough Ika coins to cover transaction fees.");
    }
    Ok(gas_obj.unwrap().object_ref())
}

async fn update_next_epoch_metadata(
    ika_node_config_path: &Path,
    config: &NodeConfig,
    ika_client: &IkaClient,
    account_key: &IkaKeyPair,
) -> anyhow::Result<PathBuf> {
    // Save backup config just in case
    let mut backup_config_path = ika_node_config_path.to_path_buf();
    ...
    let pop = generate_proof_of_possession(&new_protocol_key_pair, ika_address);
    ...
    new_config.network_key_pair = KeyPairWithPath::new(IkaKeyPair::Ed25519(new_network_key_pair));

    // worker key
    let new_worker_key_pair: Ed25519KeyPair = get_key_pair().1;
    let new_worker_key_pair_copy = new_worker_key_pair.copy();
    new_config.worker_key_pair = KeyPairWithPath::new(IkaKeyPair::Ed25519(new_worker_key_pair));

    let validators = ika_client
        .governance_api()
    ...
    let self_validator = validators
        .iter()
        .find(|v| v.ika_address == ika_address)
        .unwrap();
    ...
    // Save new config
    let mut new_config_path = ika_node_config_path.to_path_buf();
    new_config_path.pop();
    new_config_path.push(
        String::from(ika_node_config_path.file_name().unwrap().to_str().unwrap()) + ".next_epoch",
    );
...
async fn update_metadata_on_chain(
    ...
    let ika_address = IkaAddress::from(&account_key.public());
    let gas_obj_ref = get_gas_obj_ref(ika_address, ika_client, 10000 * 100).await?;
    let rgp = ika_client
        .governance_api()
    ...
    .unwrap();
    execute_tx(account_key, ika_client, tx_data, function).await?;
...
async fn execute_tx(
    ...
    let resp = ika_client
        ...
        .await
        .unwrap();
    if *resp.effects.unwrap().status() != IkaExecutionStatus::Success {
...
async fn wait_for_next_epoch(ika_client: &IkaClient, target_epoch: EpochId) -> anyhow::Result<()> {
    loop {
        let epoch_id = current_epoch(ika_client).await?;
...
async fn current_epoch(ika_client: &IkaClient) -> anyhow::Result<EpochId> {
    Ok(ika_client.read_api().get_committee_info(None).await?.epoch)
}
