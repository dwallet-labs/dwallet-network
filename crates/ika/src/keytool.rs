...
use std::sync::Arc;
use ika_keys::key_derive::generate_new_key;
...
use ika_types::crypto::{
    get_authority_key_pair, EncodeDecodeBase64, Signature, SignatureScheme, IkaKeyPair,
    ZkLoginPublicIdentifier,
};
use ika_types::crypto::{DefaultHash, PublicKey};
use ika_types::error::IkaResult;
use ika_types::multisig::{MultiSig, MultiSigPublicKey, ThresholdUnit, WeightUnit};
use ika_types::multisig_legacy::{MultiSigLegacy, MultiSigPublicKeyLegacy};
use ika_types::signature::{GenericSignature, VerifyParams};
use ika_types::signature_verification::VerifiedDigestCache;
use ika_types::transaction::{TransactionData, TransactionDataAPI};
use ika_types::zk_login_authenticator::ZkLoginAuthenticator;
...
pub enum KeyToolCommand {
    ...
    /// encoded 33 byte flag || private key starting with "ikaprivkey").
    /// Hex private key format import and export are both deprecated in
    /// Ika Wallet and Ika CLI Keystore. Use `ika keytool import` if you
    /// wish to import a key to Ika Keystore.
    ...
    ///
    /// Use `ika client new-address` if you want to generate and save the key into ika.keystore.
    Generate {
    ...
    /// Add a new key to Ika CLI Keystore using either the input mnemonic phrase or a Bech32 encoded 33-byte
    /// `flag || privkey` starting with "ikaprivkey", the key scheme flag {ed25519 | secp256k1 | secp256r1}
    ...
    /// Output the private key of the given key identity in Ika CLI Keystore as Bech32
    /// encoded string starting with `ikaprivkey`.
    Export {
        #[clap(long)]
        key_identity: KeyIdentity,
    },
    /// List all keys by its Ika address, Base64 encoded public key, key scheme name in
    /// ika.keystore.
    List {
        /// Sort by alias
        #[clap(long, short = 's')]
    ...
    /// [enum IkaKeyPair] (Base64 encoded of 33-byte `flag || privkey`) or `type AuthorityKeyPair`
    ...
    /// To MultiSig Ika Address. Pass in a list of all public keys `flag || pk` in Base64.
    ...
    /// result can be used as signature field for `ika client execute-signed-tx`. The sum
    ...
    Show { file: PathBuf },
    /// Create signature using the private key for for the given address (or its alias) in ika keystore.
    ...
    /// This takes [enum IkaKeyPair] of Base64 encoded of 33-byte `flag || privkey`). It
    /// outputs the keypair into a file at the current directory where the address is the filename,
    /// and prints out its Ika address, Base64 encoded public key, the key scheme, and the key scheme flag.
    ...
    ZkLoginEnterToken {
        ...
        kp_bigint: String,
        #[clap(long)]
        ephemeral_key_identifier: IkaAddress,
        #[clap(long, default_value = "devnet")]
        network: String,
        #[clap(long, default_value = "false")]
        test_multisig: bool,
    ...
    /// Example request: ika keytool zk-login-sig-verify --sig $SERIALIZED_ZKLOGIN_SIG --bytes $BYTES --intent-scope 0 --network devnet --curr-epoch 10
    ZkLoginSigVerify {
    ...
    /// e.g. ika keytool zk-login-insecure-sign-personal-message --data "hello" --max-epoch 5
...
pub struct ConvertOutput {
    bech32_with_flag: String, // latest Ika Keystore and Ika Wallet import/export format
    base64_with_flag: String, // Ika Keystore storage format
    hex_without_flag: String, // Legacy Ika Wallet format
    scheme: String,
}
...
pub struct SignData {
    ...
    // serialized Ika signature to be send for executing the transaction.
    ika_signature: String,
}
    ...
    pub async fn execute(self, keystore: &mut Keystore) -> Result<CommandOutput, anyhow::Error> {
        let cmd_result = Ok(match self {
            ...
            KeyToolCommand::DecodeMultiSig {
                ...
                let address = IkaAddress::from(multisig.get_pk());
            ...
            KeyToolCommand::DecodeOrVerifyTx {
                ...
                match sig {
                    ...
                    Some(s) => {
                        let res = s.verify_authenticator(
                            &IntentMessage::new(Intent::ika_transaction(), tx_data.clone()),
                            tx_data.sender(),
                            cur_epoch,
                            &VerifyParams::default(),
                            Arc::new(VerifiedDigestCache::new_empty()),
            ...
            KeyToolCommand::Generate {
                ...
                SignatureScheme::BLS12381 => {
                    let (ika_address, kp) = get_authority_key_pair();
                    let file_name = format!("bls-{ika_address}.key");
                ...
                _ => {
                    let (ika_address, skp, _scheme, phrase) =
                        generate_new_key(key_scheme, derivation_path, word_length)?;
                    let file = format!("{ika_address}.key");
            ...
            KeyToolCommand::Import {
                ...
                if Hex::decode(&input_string).is_ok() {
                    return Err(anyhow!(
                        "Ika Keystore and Ika Wallet no longer support importing 
                    private key as Hex, if you are sure your private key is encoded in Hex, use 
                    `ika keytool convert $HEX` to convert first then import the Bech32 encoded 
                    private key starting with `ikaprivkey`."
                    ));
                }

                match IkaKeyPair::decode(&input_string) {
                    Ok(skp) => {
                        ...
                        let alias = match alias {
                            Some(x) => x,
                            None => keystore.get_alias_by_address(&key.ika_address)?,
                        };
                    ...
                    Err(_) => {
                        ...
                        let ika_address = keystore.import_from_mnemonic(
                            &input_string,
                            key_scheme,
                            derivation_path,
                            alias.clone(),
                        )?;
                        let skp = keystore.get_key(&ika_address)?;
                        let mut key = Key::from(skp);
            ...
            KeyToolCommand::List { sort_by_alias } => {
                let mut keys = keystore
                    ...
                    .map(|pk| {
                        ...
                        key.alias = keystore.get_alias_by_address(&key.ika_address).ok();
                        key
                    })
                    .collect::<Vec<Key>>();
                if sort_by_alias {
                ...
                let output = match read_keypair_from_file(&file) {
                    Ok(keypair) => {
                        ...
                        let network_worker_keypair = match &keypair {
                            IkaKeyPair::Ed25519(kp) => kp.encode_base64(),
                            IkaKeyPair::Secp256k1(kp) => kp.encode_base64(),
                            IkaKeyPair::Secp256r1(kp) => kp.encode_base64(),
                        };
                        KeypairData {
            ...
            KeyToolCommand::MultiSigAddress {
                ...
                let address: IkaAddress = (&multisig_pk).into();
                ...
                for (pk, w) in pks.into_iter().zip(weights.into_iter()) {
                    output.multisig.push(MultiSigOutput {
                        address: Into::<IkaAddress>::into(&pk),
                        public_base64_key: pk.encode_base64(),
                        weight: w,
                    });
                }
                    ...
                    Err(_) => match read_authority_keypair_from_file(&file) {
                        Ok(keypair) => {
                            ...
                            CommandOutput::Show(Key {
                                ...
                                ika_address: (keypair.public()).into(),
                                public_base64_key,
                                key_scheme: SignatureScheme::BLS12381.to_string(),
                                flag: SignatureScheme::BLS12381.flag(),
                                peer_id: None,
            ...
            KeyToolCommand::Sign {
                ...
                let intent = intent.unwrap_or_else(Intent::ika_transaction);
                ...
                CommandOutput::Sign(SignData {
                    ...
                    ika_signature: ika_signature.encode_base64(),
                })
            }
            ...
            KeyToolCommand::SignKMS {
                ...
                let pk_owner = PublicKey::decode_base64(&base64pk)
                    .map_err(|e| anyhow!("Invalid base64 key: {:?}", e))?;
                let address_owner = IkaAddress::from(&pk_owner);
            ...
            KeyToolCommand::Unpack { keypair } => {
                let keypair = IkaKeyPair::decode_base64(&keypair)
                    .map_err(|_| anyhow!("Invalid Base64 encode keypair"))?;

                let key = Key::from(&keypair);
                let path_str = format!("{}.key", key.ika_address).to_lowercase();
            ...
            KeyToolCommand::ZkLoginSignAndExecuteTx {
                ...
                let skp = if fixed {
                    ...
                    IkaKeyPair::Ed25519(Ed25519KeyPair::generate(&mut rand::thread_rng()))
                };
                println!("Ephemeral keypair: {:?}", skp.encode());
                let pk = skp.public();
                let ephemeral_key_identifier: IkaAddress = (&skp.public()).into();
                ...
                println!("Finish login and paste the entire URL here (e.g. https://ika.io/#id_token=...):");
...
impl From<&IkaKeyPair> for Key {
    fn from(skp: &IkaKeyPair) -> Self {
        Key::from(skp.public())
    }
}
...
impl From<PublicKey> for Key {
    fn from(pk: PublicKey) -> Self {
        Key {
            ...
            ika_address: IkaAddress::from(&pk),
            public_base64_key: pk.encode_base64(),
...
fn convert_private_key_to_bech32(value: String) -> Result<ConvertOutput, anyhow::Error> {
    let skp = match IkaKeyPair::decode(&value) {
        Ok(s) => s,
        Err(_) => match Hex::decode(&value) {
            Ok(decoded) => {
                ...
                IkaKeyPair::Ed25519(Ed25519KeyPair::from_bytes(&decoded)?)
            }
            Err(_) => match IkaKeyPair::decode_base64(&value) {
                Ok(skp) => skp,
                Err(_) => match Ed25519KeyPair::decode_base64(&value) {
                    Ok(kp) => IkaKeyPair::Ed25519(kp),
