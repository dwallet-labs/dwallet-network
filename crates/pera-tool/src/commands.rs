...
pub enum ToolCommand {
    ...
    FetchObject {
        ...
        /// prints tabular output peratable for processing with unix tools. For
        /// instance, to quickly check that all validators agree on the history of an object:
        /// ```text
        /// $ pera-tool fetch-object --id 0x260efde76ebccf57f4c5e951157f5c361cde822c \
        ///      --genesis $HOME/.pera/pera_config/genesis.blob \
    ...
    /// contains Move bytecode (peratable for passing into a disassembler).
    ...
    Replay {
        ...
        #[arg(
            ...
            If not specified we will use the default network config file at ~/.pera-replay/network-config.yaml"
        )]
...
async fn check_locked_object(
    ...
    let clients = Arc::new(make_clients(pera_client).await?);
    ...
    pub async fn execute(self, tracing_handle: TracingHandle) -> Result<(), anyhow::Error> {
        match self {
            ToolCommand::LockedObject {
                ...
                let pera_client =
                    Arc::new(PeraClientBuilder::default().build(fullnode_rpc_url).await?);
            ...
            ToolCommand::DumpPackages {
                ...
                pera_package_dump::dump(rpc_url, output_dir, before_checkpoint).await?;
            }
                ...
                if !concise {
                    ...
                    for (i, val_info) in genesis.validator_set_for_tooling().iter().enumerate() {
                        ...
                        println!(
                            ...
                            i,
                            metadata.name,
                            metadata.pera_pubkey_bytes().concise(),
            ...
            ToolCommand::RestoreFromDBCheckpoint {
                ...
                let config = pera_config::NodeConfig::load(config_path)?;
                ...
                let aws_endpoint = env::var("AWS_SNAPSHOT_ENDPOINT").ok().or_else(|| {
                    if no_sign_request {
                        if network == Chain::Mainnet {
                            Some("https://formal-snapshot.mainnet.pera.io".to_string())
                        } else if network == Chain::Testnet {
                            Some("https://formal-snapshot.testnet.pera.io".to_string())
                        } else {
                            None
                        }
                    } else {
            ...
            ToolCommand::DownloadDBSnapshot {
                ...
                let snapshot_store_config = if no_sign_request {
                    let aws_endpoint = env::var("AWS_SNAPSHOT_ENDPOINT").ok().or_else(|| {
                        if network == Chain::Mainnet {
                            Some("https://db-snapshot.mainnet.pera.io".to_string())
                        } else if network == Chain::Testnet {
                            Some("https://db-snapshot.testnet.pera.io".to_string())
                        } else {
                            None
                        }
                    });
