...
pub struct ConsensusTransaction {
    /// Encodes an u64 unique tracking id to allow us trace a message between Pera and Narwhal.
...
pub enum ConsensusTransactionKey {
    ...
    /// The message sent between MPC parties in a signature MPC session.
...
impl Debug for ConsensusTransactionKey {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ConsensusTransactionKey::DwalletMPCOutput(value, session_id, sender_address) => {
                write!(
                    f,
                    "DwalletMPCOutput({:?}, {:?}, {:?})",
...
impl AuthorityCapabilitiesV1 {
    pub fn new(
        ...
        let generation = SystemTime::now()
            ...
            .expect("Pera did not exist prior to 1970")
            .as_millis()
            .try_into()
            .expect("This build of pera is not supported in the year 500,000,000");
        Self {
            authority,
            generation,
            supported_protocol_versions,
