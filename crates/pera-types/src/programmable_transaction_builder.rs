...
impl ProgrammableTransactionBuilder {
    ...
    pub fn transfer_arg(&mut self, recipient: PeraAddress, arg: Argument) {
        self.transfer_args(recipient, vec![arg])
    }

    pub fn transfer_args(&mut self, recipient: PeraAddress, args: Vec<Argument>) {
    ...
    pub fn transfer_pera(&mut self, recipient: PeraAddress, amount: Option<u64>) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    pub fn pay_all_pera(&mut self, recipient: PeraAddress) {
        let rec_arg = self.pure(recipient).unwrap();
    ...
    fn pay_impl(
        ...
        let mut recipient_map: IndexMap<PeraAddress, Vec<usize>> = IndexMap::new();
