...
use crate::pera_serde::AsProtocolVersion;
use crate::pera_serde::BigInt;
use crate::pera_serde::Readable;
use crate::transaction::{Transaction, TransactionData};
use crate::{base_types::AuthorityName, committee::Committee, error::PeraError};
use anyhow::Result;
...
impl CheckpointSummary {
    ...
    pub fn verify_epoch(&self, epoch: EpochId) -> PeraResult {
        fp_ensure!(
            self.epoch == epoch,
...
impl CertifiedCheckpointSummary {
    pub fn verify_authority_signatures(&self, committee: &Committee) -> PeraResult {
    ...
    pub fn try_into_verified(self, committee: &Committee) -> PeraResult<VerifiedCheckpoint> {
    ...
    pub fn verify_with_contents(
        ...
        if let Some(contents) = contents {
            ...
            fp_ensure!(
                ...
                PeraError::GenericAuthorityError{error:format!("Checkpoint contents digest mismatch: summary={:?}, received content digest {:?}, received {} transactions", self.data(), content_digest, contents.size())}
            );
        }
...
impl SignedCheckpointSummary {
    ...
    pub fn try_into_verified(
        self,
        committee: &Committee,
    ) -> PeraResult<VerifiedEnvelope<CheckpointSummary, AuthoritySignInfo>> {
        self.verify_authority_signatures(committee)?;
...
impl CheckpointSignatureMessage {
    pub fn verify(&self, committee: &Committee) -> PeraResult {
