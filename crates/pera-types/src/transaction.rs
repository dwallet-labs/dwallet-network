...
use crate::messages_dwallet_mpc::SignatureMPCOutput;
...
use crate::{
    PERA_AUTHENTICATOR_STATE_OBJECT_ID, PERA_AUTHENTICATOR_STATE_OBJECT_SHARED_VERSION,
    PERA_CLOCK_OBJECT_ID, PERA_CLOCK_OBJECT_SHARED_VERSION, PERA_FRAMEWORK_PACKAGE_ID,
    PERA_RANDOMNESS_STATE_OBJECT_ID, PERA_SYSTEM_STATE_OBJECT_ID,
    PERA_SYSTEM_STATE_OBJECT_SHARED_VERSION,
};
...
impl CallArg {
    pub const PERA_SYSTEM_MUT: Self = Self::Object(ObjectArg::PERA_SYSTEM_MUT);
    pub const CLOCK_IMM: Self = Self::Object(ObjectArg::SharedObject {
        id: PERA_CLOCK_OBJECT_ID,
        initial_shared_version: PERA_CLOCK_OBJECT_SHARED_VERSION,
        mutable: false,
    });
    ...
    pub const AUTHENTICATOR_MUT: Self = Self::Object(ObjectArg::SharedObject {
        ...
        initial_shared_version: PERA_AUTHENTICATOR_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    });
}
...
impl EndOfEpochTransactionKind {
    ...
    fn shared_input_objects(&self) -> impl Iterator<Item = SharedInputObject> + '_ {
        match self {
            Self::ChangeEpoch(_) => {
                Either::Left(vec![SharedInputObject::PERA_SYSTEM_OBJ].into_iter())
            }
            ...
            Self::BridgeCommitteeInit(bridge_version) => Either::Left(
                vec![
                    ...
                    SharedInputObject::PERA_SYSTEM_OBJ,
                ]
                .into_iter(),
            ),
        }
...
impl ObjectArg {
    pub const PERA_SYSTEM_MUT: Self = Self::SharedObject {
        id: PERA_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: PERA_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    };
...
impl ProgrammableTransaction {
    ...
    fn validity_check(&self, config: &ProtocolConfig) -> UserInputResult {
        ...
        if let Some(random_index) = inputs.iter().position(|obj| {
            matches!(obj, CallArg::Object(ObjectArg::SharedObject { id, .. }) if *id == PERA_RANDOMNESS_STATE_OBJECT_ID)
        }) {
            fp_ensure!(
...
impl SharedInputObject {
    pub const PERA_SYSTEM_OBJ: Self = Self {
        id: PERA_SYSTEM_STATE_OBJECT_ID,
        initial_shared_version: PERA_SYSTEM_STATE_OBJECT_SHARED_VERSION,
        mutable: true,
    };
...
impl TransactionData {
    fn new_system_transaction(kind: TransactionKind) -> Self {
        ...
        let sender = PeraAddress::default();
    ...
    pub fn new_transfer_pera(
        ...
        Self::new_transfer_pera_allow_sponsor(
            recipient,
            sender,
            amount,
            gas_payment,
    ...
    pub fn new_transfer_pera_allow_sponsor(
        recipient: PeraAddress,
        sender: PeraAddress,
        amount: Option<u64>,
        gas_payment: ObjectRef,
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.transfer_pera(recipient, amount);
            builder.finish()
        };
    ...
    pub fn new_pay_pera(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.pay_pera(recipients, amounts)?;
            builder.finish()
        };
        Ok(Self::new_programmable(
    ...
    pub fn new_pay_all_pera(
        ...
        let pt = {
            let mut builder = ProgrammableTransactionBuilder::new();
            builder.pay_all_pera(recipient);
            builder.finish()
        };
    ...
    pub fn execution_parts(&self) -> (TransactionKind, PeraAddress, Vec<ObjectRef>) {
        (
            self.kind().clone(),
            self.sender(),
    ...
    pub fn uses_randomness(&self) -> bool {
        self.shared_input_objects()
            .iter()
            .any(|obj| obj.id() == PERA_RANDOMNESS_STATE_OBJECT_ID)
    }
...
pub trait TransactionDataAPI {
    ...
    fn signers(&self) -> NonEmpty<PeraAddress>;

    fn gas_data(&self) -> &GasData;

    fn gas_owner(&self) -> PeraAddress;
    ...
    fn sender_mut_for_testing(&mut self) -> &mut PeraAddress;
...
impl Serialize for SenderSignedTransaction {
        ...
        S: serde::Serializer,
    {
        ...
        if self.intent_message().intent != Intent::pera_transaction() {
...
impl<'de> Deserialize<'de> for SenderSignedTransaction {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if intent_message.intent != Intent::pera_transaction() {
...
impl SenderSignedTransaction {
    ...
    ) -> PeraResult<BTreeMap<PeraAddress, &GenericSignature>> {
        ...
        for sig in &self.tx_signatures {
            if verify_legacy_zklogin_address {
                ...
                if let GenericSignature::ZkLoginAuthenticator(z) = sig {
                    mapping.insert(PeraAddress::try_from_padded(&z.inputs)?, sig);
                };
            }
...
impl SenderSignedData {
    pub fn new(tx_data: TransactionData, tx_signatures: Vec<GenericSignature>) -> Self {
        Self(SizeOneVec::new(SenderSignedTransaction {
            intent_message: IntentMessage::new(Intent::pera_transaction(), tx_data),
            tx_signatures,
        }))
    }
    ...
    pub fn serialized_size(&self) -> PeraResult<usize> {
        bcs::serialized_size(self).map_err(|e| PeraError::TransactionSerializationError {
            error: e.to_string(),
        })
    }

    fn check_user_signature_protocol_compatibility(&self, config: &ProtocolConfig) -> PeraResult {
    ...
    pub fn validity_check(&self, config: &ProtocolConfig, epoch: EpochId) -> PeraResult {
        ...
        if match &tx_data.expiration() {
            ...
            return Err(PeraError::TransactionExpired);
        }
...
impl<S> Envelope<SenderSignedData, S> {
    pub fn sender_address(&self) -> PeraAddress {
...
impl Transaction {
    pub fn from_data_and_signer(
        ...
        let signatures = {
            let intent_msg = IntentMessage::new(Intent::pera_transaction(), &data);
            signers
                .into_iter()
        ...
        system_transaction
            ...
            .pipe(|data| {
                SenderSignedData::new_from_sender_signature(
                    data,
                    Ed25519PeraSignature::from_bytes(&[0; Ed25519PeraSignature::LENGTH])
                        .unwrap()
                        .into(),
                )
            })
...
impl Transaction {
    ...
    pub fn try_into_verified_for_testing(
        ...
        current_epoch: EpochId,
        verify_params: &VerifyParams,
    ) -> PeraResult<VerifiedTransaction> {
...
impl CertifiedTransaction {
    ...
    pub fn try_into_verified_for_testing(
        ...
        committee: &Committee,
        verify_params: &VerifyParams,
    ) -> PeraResult<VerifiedCertificate> {
    ...
    pub fn verify_committee_sigs_only(&self, committee: &Committee) -> PeraResult {
        self.auth_sig().verify_secure(
            self.data(),
            Intent::pera_app(IntentScope::SenderSignedTransaction),
            committee,
...
// An InputObjects new-type that has been verified by pera-transaction-checks, and can be
// safely passed to execution.
pub struct CheckedInputObjects(InputObjects);

// DO NOT CALL outside of pera-transaction-checks, genesis, or replay.
//
// CheckedInputObjects should really be defined in pera-transaction-checks so that we can
// make public construction impossible. But we can't do that because it would result in circular
// dependencies.
impl CheckedInputObjects {
    // Only called by pera-transaction-checks.
