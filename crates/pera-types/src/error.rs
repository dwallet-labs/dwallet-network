...
pub enum UserInputError {
    ...
    #[error("Gas price cannot exceed {:?} npera", max_gas_price)]
    ...
    #[error("PERA payment transactions use first input coin for gas payment, but found a different gas object")]
...
pub enum PeraObjectResponseError {
    ...
    // TODO: also integrate PeraPastObjectResponse (VersionNotFound,  VersionTooHigh)
}

/// Custom error type for Pera.
#[derive(
    Eq, PartialEq, Clone, Debug, Serialize, Deserialize, Error, Hash, AsRefStr, IntoStaticStr,
)]
pub enum PeraError {
    ...
    PeraObjectResponseError { error: PeraObjectResponseError },
    ...
    TooManyIncorrectAuthorities {
        errors: Vec<(AuthorityName, PeraError)>,
        action: String,
    },
    ...
    #[error("Failed to execute handle_consensus_transaction on Pera: {0}")]
    ...
    // todo(zeev): remove all MPC errors.
...
pub type PeraResult<T = ()> = Result<T, PeraError>;
pub type UserInputResult<T = ()> = Result<T, UserInputError>;

impl From<pera_protocol_config::Error> for PeraError {
    fn from(error: pera_protocol_config::Error) -> Self {
        PeraError::WrongMessageVersion { error: error.0 }
    }
}
...
impl From<Status> for PeraError {
    fn from(status: Status) -> Self {
        ...
        let result = bcs::from_bytes::<PeraError>(status.details());
        if let Ok(pera_error) = result {
            pera_error
        } else {
            Self::RpcError(
...
impl From<TypedStoreError> for PeraError {
    fn from(e: TypedStoreError) -> Self {
        Self::Storage(e.to_string())
    }
}

impl From<crate::storage::error::Error> for PeraError {
    fn from(e: crate::storage::error::Error) -> Self {
        Self::Storage(e.to_string())
    }
}

impl From<PeraError> for Status {
    fn from(error: PeraError) -> Self {
        let bytes = bcs::to_bytes(&error).unwrap();
        Status::with_details(tonic::Code::Internal, error.to_string(), bytes.into())
    }
}

impl From<ExecutionErrorKind> for PeraError {
    fn from(kind: ExecutionErrorKind) -> Self {
        ExecutionError::from_kind(kind).into()
    }
}

impl From<&str> for PeraError {
    fn from(error: &str) -> Self {
        PeraError::GenericAuthorityError {
            error: error.to_string(),
        }
    }
}

impl From<String> for PeraError {
    fn from(error: String) -> Self {
        PeraError::GenericAuthorityError { error }
    }
}

impl TryFrom<PeraError> for UserInputError {
    type Error = anyhow::Error;

    fn try_from(err: PeraError) -> Result<Self, Self::Error> {
        match err {
            PeraError::UserInputError { error } => Ok(error),
            other => anyhow::bail!("error {:?} is not UserInputError", other),
        }
    }
}

impl From<UserInputError> for PeraError {
    fn from(error: UserInputError) -> Self {
        PeraError::UserInputError { error }
    }
}
...
impl PeraError {
    pub fn individual_error_indicates_epoch_change(&self) -> bool {
        matches!(
            self,
            PeraError::ValidatorHaltedAtEpochEnd | PeraError::MissingCommitteeAtEpoch(_)
        )
    }
    ...
    pub fn is_retryable(&self) -> (bool, bool) {
        let retryable = match self {
            // Network error
            PeraError::RpcError { .. } => true,

            // Reconfig error
            PeraError::ValidatorHaltedAtEpochEnd => true,
            PeraError::MissingCommitteeAtEpoch(..) => true,
            PeraError::WrongEpoch { .. } => true,
            ...
            PeraError::PotentiallyTemporarilyInvalidSignature { .. } => true,

            // Overload errors
            PeraError::TooManyTransactionsPendingExecution { .. } => true,
            PeraError::TooManyTransactionsPendingOnObject { .. } => true,
            PeraError::TooOldTransactionPendingOnObject { .. } => true,
            PeraError::TooManyTransactionsPendingConsensus => true,
            PeraError::ValidatorOverloadedRetryAfter { .. } => true,

            // Non retryable error
            PeraError::ExecutionError(..) => false,
            PeraError::ByzantineAuthoritySuspicion { .. } => false,
            PeraError::QuorumFailedToGetEffectsQuorumWhenProcessingTransaction { .. } => false,
            PeraError::TxAlreadyFinalizedWithDifferentUserSigs => false,
            PeraError::FailedToVerifyTxCertWithExecutedEffects { .. } => false,
            PeraError::ObjectLockConflict { .. } => false,
            ...
            PeraError::TooManyRequests => false,
    ...
    pub fn is_overload(&self) -> bool {
        matches!(
            self,
            PeraError::TooManyTransactionsPendingExecution { .. }
                | PeraError::TooManyTransactionsPendingOnObject { .. }
                | PeraError::TooOldTransactionPendingOnObject { .. }
                | PeraError::TooManyTransactionsPendingConsensus
        )
    }

    pub fn is_retryable_overload(&self) -> bool {
        matches!(self, PeraError::ValidatorOverloadedRetryAfter { .. })
    }

    pub fn retry_after_secs(&self) -> u64 {
        match self {
            PeraError::ValidatorOverloadedRetryAfter { retry_after_secs } => *retry_after_secs,
            _ => 0,
        }
    }
}

impl Ord for PeraError {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        Ord::cmp(self.as_ref(), other.as_ref())
    }
}

impl PartialOrd for PeraError {
