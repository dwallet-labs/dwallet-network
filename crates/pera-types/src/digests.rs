...
impl TryFrom<Vec<u8>> for Digest {
    ...
    fn try_from(bytes: Vec<u8>) -> Result<Self, PeraError> {
        let bytes: [u8; 32] =
            <[u8; 32]>::try_from(&bytes[..]).map_err(|_| PeraError::InvalidDigestLength {
                expected: 32,
                actual: bytes.len(),
            })?;
...
static PERA_PROTOCOL_CONFIG_CHAIN_OVERRIDE: Lazy<Option<Chain>> = Lazy::new(|| {
    if let Ok(s) = env::var(PERA_PROTOCOL_CONFIG_CHAIN_OVERRIDE_ENV_VAR_NAME) {
        info!("PERA_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {:?}", s);
        match s.as_str() {
            "mainnet" => Some(Chain::Mainnet),
            "testnet" => Some(Chain::Testnet),
            "" => None,
            _ => panic!("unrecognized PERA_PROTOCOL_CONFIG_CHAIN_OVERRIDE: {s:?}"),
        }
    } else {
        None
    }
...
impl ChainIdentifier {
    pub fn chain(&self) -> Chain {
        ...
        if let Some(override_chain) = *PERA_PROTOCOL_CONFIG_CHAIN_OVERRIDE {
...
pub struct ProofMPCResultOnChainDigest(pub Digest);

impl ProofMPCResultOnChainDigest {
    pub const fn new(digest: [u8; 32]) -> Self {
        Self(Digest::new(digest))
    }
...
impl TryFrom<&[u8]> for TransactionDigest {
    type Error = crate::error::PeraError;

    fn try_from(bytes: &[u8]) -> Result<Self, crate::error::PeraError> {
        let arr: [u8; 32] = bytes
            .try_into()
            .map_err(|_| crate::error::PeraError::InvalidTransactionDigest)?;
        Ok(Self::new(arr))
    }
}
