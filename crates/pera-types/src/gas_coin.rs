...
/// The number of NPera per Pera token
pub const NPERA_PER_PERA: u64 = 1_000_000_000;

/// Total supply denominated in Pera
pub const TOTAL_SUPPLY_PERA: u64 = 10_000_000_000;

// Note: cannot use checked arithmetic here since `const unwrap` is still unstable.
/// Total supply denominated in NPera
pub const TOTAL_SUPPLY_NPERA: u64 = TOTAL_SUPPLY_PERA * NPERA_PER_PERA;

pub const GAS_MODULE_NAME: &IdentStr = ident_str!("pera");
pub const GAS_STRUCT_NAME: &IdentStr = ident_str!("PERA");
...
mod checked {
    ...
    /// Rust version of the Move pera::coin::Coin<Pera::pera::PERA> type
    ...
    impl GasCoin {
        ...
        /// Return `true` if `s` is the type of a gas coin (i.e., 0x2::coin::Coin<0x2::pera::PERA>)
        pub fn is_gas_coin(s: &StructTag) -> bool {
            Coin::is_coin(s) && s.type_params.len() == 1 && GAS::is_gas_type(&s.type_params[0])
        }

        /// Return `true` if `s` is the type of a gas balance (i.e., 0x2::balance::Balance<0x2::pera::PERA>)
