...
use crate::error::{ExecutionError, PeraError};
...
) -> PeraResult<PackageFetchResults<PackageObject>> {
...
pub fn get_module(
    store: impl BackingPackageStore,
    module_id: &ModuleId,
) -> Result<Option<Vec<u8>>, PeraError> {
    Ok(store
...
pub fn get_module_by_id<S: BackingPackageStore>(
    store: &S,
    id: &ModuleId,
) -> anyhow::Result<Option<CompiledModule>, PeraError> {
