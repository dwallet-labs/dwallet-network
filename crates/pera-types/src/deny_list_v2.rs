...
use crate::base_types::{EpochId, ObjectID, PeraAddress};
...
use crate::{MoveTypeTagTrait, PERA_DENY_LIST_OBJECT_ID, PERA_FRAMEWORK_PACKAGE_ID};
...
impl ConfigKey {
    pub fn type_() -> StructTag {
        StructTag {
            address: PERA_FRAMEWORK_PACKAGE_ID.into(),
...
fn check_new_regulated_coin_owners(
    new_regulated_coin_owners: BTreeMap<String, (Config, BTreeSet<PeraAddress>)>,
    cur_epoch: EpochId,
    object_store: &dyn ObjectStore,
...
pub fn get_per_type_coin_deny_list_v2(
    ...
    let config: Config =
        get_dynamic_field_from_store(object_store, PERA_DENY_LIST_OBJECT_ID, &config_key).ok()?;
    Some(config)
}
