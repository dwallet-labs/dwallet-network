...
use crate::{
    parse_pera_struct_tag, parse_pera_type_tag, DEEPBOOK_ADDRESS, PERA_CLOCK_ADDRESS,
    PERA_FRAMEWORK_ADDRESS, PERA_SYSTEM_ADDRESS, PERA_SYSTEM_STATE_ADDRESS,
};
...
pub struct PeraBitmap;

impl SerializeAs<roaring::RoaringBitmap> for PeraBitmap {
...
impl<'de> DeserializeAs<'de, roaring::RoaringBitmap> for PeraBitmap {
...
pub struct PeraStructTag;

impl SerializeAs<StructTag> for PeraStructTag {
    fn serialize_as<S>(value: &StructTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let f = to_pera_struct_tag_string(value).map_err(S::Error::custom)?;
        f.serialize(serializer)
    }
}

const PERA_ADDRESSES: [AccountAddress; 7] = [
    AccountAddress::ZERO,
    AccountAddress::ONE,
    PERA_FRAMEWORK_ADDRESS,
    PERA_SYSTEM_ADDRESS,
    DEEPBOOK_ADDRESS,
    PERA_SYSTEM_STATE_ADDRESS,
    PERA_CLOCK_ADDRESS,
];
/// Serialize StructTag as a string, retaining the leading zeros in the address.
pub fn to_pera_struct_tag_string(value: &StructTag) -> Result<String, fmt::Error> {
    let mut f = String::new();
    // trim leading zeros if address is in PERA_ADDRESSES
    let address = if PERA_ADDRESSES.contains(&value.address) {
    ...
    if let Some(first_ty) = value.type_params.first() {
        write!(f, "<")?;
        write!(f, "{}", to_pera_type_tag_string(first_ty)?)?;
        for ty in value.type_params.iter().skip(1) {
            write!(f, ", {}", to_pera_type_tag_string(ty)?)?;
        }
        write!(f, ">")?;
    }
    Ok(f)
...
fn to_pera_type_tag_string(value: &TypeTag) -> Result<String, fmt::Error> {
    match value {
        TypeTag::Vector(t) => Ok(format!("vector<{}>", to_pera_type_tag_string(t)?)),
        TypeTag::Struct(s) => to_pera_struct_tag_string(s),
        _ => Ok(value.to_string()),
    }
}

impl<'de> DeserializeAs<'de, StructTag> for PeraStructTag {
    fn deserialize_as<D>(deserializer: D) -> Result<StructTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_pera_struct_tag(&s).map_err(D::Error::custom)
    }
}

pub struct PeraTypeTag;

impl SerializeAs<TypeTag> for PeraTypeTag {
    fn serialize_as<S>(value: &TypeTag, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let s = to_pera_type_tag_string(value).map_err(S::Error::custom)?;
        s.serialize(serializer)
    }
}

impl<'de> DeserializeAs<'de, TypeTag> for PeraTypeTag {
    fn deserialize_as<D>(deserializer: D) -> Result<TypeTag, D::Error>
    where
        D: Deserializer<'de>,
    {
        let s = String::deserialize(deserializer)?;
        parse_pera_type_tag(&s).map_err(D::Error::custom)
    }
}
