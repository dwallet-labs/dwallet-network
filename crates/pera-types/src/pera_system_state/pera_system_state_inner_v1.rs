...
use super::epoch_start_pera_system_state::EpochStartValidatorInfoV1;
use super::pera_system_state_summary::{PeraSystemStateSummary, PeraValidatorSummary};
use super::{get_validators_from_table_vec, AdvanceEpochParams, PeraSystemStateTrait};
...
/// Rust version of the Move pera::pera_system::SystemParameters type
...
impl VerifiedValidatorMetadataV1 {
    pub fn pera_pubkey_bytes(&self) -> AuthorityPublicKeyBytes {
...
impl ValidatorMetadataV1 {
    ...
    pub fn verify(&self) -> Result<VerifiedValidatorMetadataV1, u64> {
        ...
        verify_proof_of_possession(&pop, &protocol_pubkey, self.pera_address)
            .map_err(|_| E_METADATA_INVALID_POP)?;
        ...
        Ok(VerifiedValidatorMetadataV1 {
            pera_address: self.pera_address,
            protocol_pubkey,
            network_pubkey,
            worker_pubkey,
...
/// Rust version of the Move pera::validator::Validator type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
...
impl ValidatorV1 {
    ...
    pub fn into_pera_validator_summary(self) -> PeraValidatorSummary {
        let Self {
            metadata:
                ValidatorMetadataV1 {
                    pera_address,
            ...
            staking_pool:
                StakingPoolV1 {
                    ...
                    pera_balance: staking_pool_pera_balance,
                    rewards_pool,
                    pool_token_balance,
                    exchange_rates:
                        Table {
...
/// Rust version of the Move pera_system::staking_pool::StakingPool type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct StakingPoolV1 {
    pub id: ObjectID,
    pub activation_epoch: Option<u64>,
    pub deactivation_epoch: Option<u64>,
    pub pera_balance: u64,
...
/// Rust version of the Move pera_system::validator_set::ValidatorSet type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
...
/// Rust version of the Move pera_system::pera_system::PeraSystemStateInner type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct PeraSystemStateInnerV1 {
    pub epoch: u64,
    ...
    pub validator_report_records: VecMap<PeraAddress, VecSet<PeraAddress>>,
...
impl PeraSystemStateTrait for PeraSystemStateInnerV1 {
    fn epoch(&self) -> u64 {
        self.epoch
    }
    ...
    fn get_current_epoch_committee(&self) -> CommitteeWithNetworkMetadata {
        let validators = self
            ...
            .map(|validator| {
                ...
                let name = verified_metadata.pera_pubkey_bytes();
                (
                    name,
                    (
            ...
            self.validators
                ...
                .map(|validator| {
                    ...
                    EpochStartValidatorInfoV1 {
                        pera_address: metadata.pera_address,
    ...
    fn into_pera_system_state_summary(self) -> PeraSystemStateSummary {
        // If you are making any changes to PeraSystemStateV1 or any of its dependent types before
        // mainnet, please also update PeraSystemStateSummary and its corresponding TS type.
...
/// Rust version of the Move pera_system::validator_cap::UnverifiedValidatorOperationCap type
#[derive(Debug, Serialize, Deserialize, Clone, Eq, PartialEq)]
pub struct UnverifiedValidatorOperationCapV1 {
    pub id: ObjectID,
    pub authorizer_validator_address: PeraAddress,
}
