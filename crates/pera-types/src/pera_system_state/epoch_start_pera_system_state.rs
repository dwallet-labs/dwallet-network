...
pub trait EpochStartSystemStateTrait {
    ...
    fn get_validator_addresses(&self) -> Vec<PeraAddress>;
    fn get_pera_committee(&self) -> Committee;
    fn get_pera_committee_with_network_metadata(&self) -> CommitteeWithNetworkMetadata;
...
/// This type captures the minimum amount of information from PeraSystemState needed by a validator
/// to run the protocol. This allows us to decouple from the actual PeraSystemState type, and hence
/// do not need to evolve it when we upgrade the PeraSystemState type.
...
impl EpochStartSystemState {
    pub fn new_v1(
        ...
        decryption_key_shares: Option<VecMap<u8, Vec<DwalletMPCNetworkKey>>>,
    ) -> Self {
...
impl EpochStartSystemStateTrait for EpochStartSystemStateV1 {
    ...
    fn get_validator_addresses(&self) -> Vec<PeraAddress> {
        self.active_validators
            .iter()
            .map(|validator| validator.pera_address)
            .collect()
    }
            ...
            .map(|validator| {
                (
                    ...
                    (
                        ...
                        NetworkMetadata {
                            network_address: validator.pera_net_address.clone(),
    ...
    fn get_mysticeti_committee(&self) -> ConsensusCommittee {
        ...
        // in the Pera committee returned from get_pera_committee().
        authorities.sort_by(|a1, a2| a1.authority_key.cmp(&a2.authority_key));

        for ((i, mysticeti_authority), pera_authority_name) in authorities
            .iter()
            .enumerate()
            .zip(self.get_pera_committee().names())
        {
            if pera_authority_name.0 != mysticeti_authority.authority_key.to_bytes() {
                error!(
                    "Mismatched authority order between Pera and Mysticeti! Index {}, Mysticeti authority {:?}\nPera authority name {}",
                    i, mysticeti_authority, pera_authority_name
                );
            }
        }
...
mod test {
    ...
    fn test_pera_and_mysticeti_committee_are_same() {
        ...
        for i in 0..10 {
            let (pera_address, protocol_key): (PeraAddress, AuthorityKeyPair) = get_key_pair();
        ...
        // WHEN
        let pera_committee = state.get_pera_committee();
        let mysticeti_committee = state.get_mysticeti_committee();

        // THEN
        // assert the validators details
        assert_eq!(pera_committee.num_members(), 10);
        assert_eq!(pera_committee.num_members(), mysticeti_committee.size());
        assert_eq!(
            pera_committee.validity_threshold(),
            mysticeti_committee.validity_threshold()
        );
        assert_eq!(
            pera_committee.quorum_threshold(),
        ...
        for (authority_index, mysticeti_authority) in mysticeti_committee.authorities() {
            let pera_authority_name = pera_committee
                .authority_by_index(authority_index.value() as u32)
                .unwrap();

            assert_eq!(
                mysticeti_authority.authority_key.to_bytes(),
                pera_authority_name.0,
                "Mysten & PERA committee member of same index correspond to different public key"
            );
            assert_eq!(
                mysticeti_authority.stake,
                pera_committee.weight(pera_authority_name),
                "Mysten & PERA committee member stake differs"
            );
        }
    }
}
