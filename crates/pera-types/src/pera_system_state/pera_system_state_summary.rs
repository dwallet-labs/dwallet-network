...
use super::{PeraSystemState, PeraSystemStateTrait};
...
/// dependencies to the internal data structures of the PERA system state type.

#[serde_as]
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct PeraSystemStateSummary {
    ...
    pub active_validators: Vec<PeraValidatorSummary>,
    ...
    #[schemars(with = "Vec<(PeraAddress, BigInt<u64>)>")]
    ...
    pub decryption_key_shares: Vec<(DWalletMPCNetworkKeyScheme, NetworkDecryptionKeyShares)>,
}

impl PeraSystemStateSummary {
    pub fn get_pera_committee_for_benchmarking(&self) -> CommitteeWithNetworkMetadata {
        let validators = self
            .active_validators
            .iter()
            .map(|validator| {
...
impl Default for PeraValidatorSummary {
    fn default() -> Self {
        Self {
            pera_address: PeraAddress::default(),
...
/// Given the staking pool id of a validator, return the validator's `PeraValidatorSummary`,
/// works for validator candidates, active validators, as well as inactive validators.
pub fn get_validator_by_pool_id<S>(
    object_store: &S,
    system_state: &PeraSystemState,
    ...
    S: ObjectStore + ?Sized,
{
    ...
    let candidate_address: PeraAddress = get_dynamic_field_from_store(
        &object_store,
