...
impl<D: Hash + Eq + Copy> VerifiedDigestCache<D> {
    ...
    pub fn is_verified<F, G>(&self, digest: D, verify_callback: F, uncached_checks: G) -> PeraResult
    where
        F: FnOnce() -> PeraResult,
        G: FnOnce() -> PeraResult,
    {
...
pub fn verify_sender_signed_data_message_signatures(
    ...
    assert_eq!(intent_message.intent, Intent::pera_transaction());
    ...
    for s in signers {
        if !present_sigs.contains_key(&s) {
            return Err(PeraError::SignerSignatureAbsent {
