...
use crate::{id::UID, PERA_FRAMEWORK_ADDRESS};
...
impl Clock {
    ...
    /// Detects a `&mut pera::clock::Clock` or `pera::clock::Clock` in the signature.
    pub fn is_mutable(view: &CompiledModule, s: &SignatureToken) -> bool {
        use SignatureToken as S;
        match s {
            S::MutableReference(inner) => Self::is_mutable(view, inner),
            S::Datatype(idx) => resolve_struct(view, *idx) == RESOLVED_PERA_CLOCK,
            _ => false,
        }
    }
}
