...
use crate::{
    ...
    crypto::DefaultHash,
    error::{ExecutionError, ExecutionErrorKind, PeraError, PeraResult},
    id::{ID, UID},
...
/// Rust representation of upgrade policy constants in `pera::package`.
#[repr(u8)]
...
/// Rust representation of `pera::package::UpgradeCap`.
...
/// Rust representation of `pera::package::UpgradeTicket`.
#[derive(Debug, Serialize, Deserialize)]
...
/// Rust representation of `pera::package::UpgradeReceipt`.
#[derive(Debug, Serialize, Deserialize)]
...
impl MovePackage {
    ...
    ) -> PeraResult<CompiledModule> {
        // TODO use the session's cache
        let bytes = self
            .serialized_module_map()
            .get(module.as_str())
            .ok_or_else(|| PeraError::ModuleNotFound {
                module_name: module.to_string(),
            })?;
        CompiledModule::deserialize_with_config(bytes, binary_config).map_err(|error| {
            PeraError::ModuleDeserializationFailure {
                error: error.to_string(),
            }
        })
    }

    pub fn disassemble(&self) -> PeraResult<BTreeMap<String, Value>> {
    ...
    pub fn normalize(
        &self,
        binary_config: &BinaryConfig,
    ) -> PeraResult<BTreeMap<String, normalized::Module>> {
...
pub fn disassemble_modules<'a, I>(modules: I) -> PeraResult<BTreeMap<String, Value>>
where
