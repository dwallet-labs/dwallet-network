...
pub const BRIDGE_CREATE_ADD_TOKEN_ON_PERA_MESSAGE_FUNCTION_NAME: &IdentStr =
    ident_str!("create_add_tokens_on_pera_message");
...
impl BridgeChainId {
    pub fn is_pera_chain(&self) -> bool {
        matches!(
            self,
            BridgeChainId::PeraMainnet | BridgeChainId::PeraTestnet | BridgeChainId::PeraCustom
        )
    }
}
...
) -> PeraResult<Option<SequenceNumber>> {
    Ok(object_store
...
/// Rust version of the Move pera::bridge::Bridge type
...
pub trait BridgeTrait {
    ...
    fn frozen(&self) -> bool;
    fn try_into_bridge_summary(self) -> PeraResult<BridgeSummary>;
}
...
pub fn get_bridge_wrapper(object_store: &dyn ObjectStore) -> Result<BridgeWrapper, PeraError> {
    ...
    let result = bcs::from_bytes::<BridgeWrapper>(move_object.contents())
        .map_err(|err| PeraError::PeraBridgeReadError(err.to_string()))?;
    Ok(result)
}

pub fn get_bridge(object_store: &dyn ObjectStore) -> Result<Bridge, PeraError> {
    ...
    match version {
        1 => {
            let result: BridgeInnerV1 = get_dynamic_field_from_store(object_store, id, &version)
                .map_err(|err| {
                    PeraError::PeraBridgeReadError(format!(
        ...
        _ => Err(PeraError::PeraBridgeReadError(format!(
            "Unsupported PeraBridge version: {}",
            version
        ))),
    }
}
...
pub struct MoveTypeBridgeCommittee {
    ...
    pub member_registrations: VecMap<PeraAddress, MoveTypeCommitteeMemberRegistration>,
...
pub struct BridgeCommitteeSummary {
    ...
    pub member_registration: Vec<(PeraAddress, MoveTypeCommitteeMemberRegistration)>,
    pub last_committee_update_epoch: u64,
}
...
pub fn is_bridge_committee_initiated(object_store: &dyn ObjectStore) -> PeraResult<bool> {
    match get_bridge(object_store) {
        Ok(bridge) => Ok(!bridge.committee().members.contents.is_empty()),
        Err(PeraError::PeraBridgeReadError(..)) => Ok(false),
        Err(other) => Err(other),
    }
}
