...
pub const PERA_PRIV_KEY_PREFIX: &str = "peraprivkey";
...
pub fn generate_proof_of_possession(
    ...
    AuthoritySignature::new_secure(
        &IntentMessage::new(Intent::pera_app(IntentScope::ProofOfPossession), msg),
        &DEFAULT_EPOCH_ID,
        keypair,
    )
}
...
/// * accounts to interact with Pera.
/// * Currently we support eddsa and ecdsa on Pera.
///

#[allow(clippy::large_enum_variant)]
#[derive(Debug, From, PartialEq, Eq)]
pub enum PeraKeyPair {
...
impl PeraKeyPair {
    pub fn public(&self) -> PublicKey {
        match self {
            PeraKeyPair::Ed25519(kp) => PublicKey::Ed25519(kp.public().into()),
            PeraKeyPair::Secp256k1(kp) => PublicKey::Secp256k1(kp.public().into()),
            PeraKeyPair::Secp256r1(kp) => PublicKey::Secp256r1(kp.public().into()),
        }
    }

    pub fn copy(&self) -> Self {
        match self {
            PeraKeyPair::Ed25519(kp) => kp.copy().into(),
            PeraKeyPair::Secp256k1(kp) => kp.copy().into(),
            PeraKeyPair::Secp256r1(kp) => kp.copy().into(),
        }
    }
}

impl Signer<Signature> for PeraKeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        match self {
            PeraKeyPair::Ed25519(kp) => kp.sign(msg),
            PeraKeyPair::Secp256k1(kp) => kp.sign(msg),
            PeraKeyPair::Secp256r1(kp) => kp.sign(msg),
        }
    }
}

impl EncodeDecodeBase64 for PeraKeyPair {
...
impl PeraKeyPair {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, eyre::Report> {
        ...
        {
            Ok(x) => match x {
                SignatureScheme::ED25519 => Ok(PeraKeyPair::Ed25519(Ed25519KeyPair::from_bytes(
                    bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                )?)),
                SignatureScheme::Secp256k1 => {
                    Ok(PeraKeyPair::Secp256k1(Secp256k1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
                SignatureScheme::Secp256r1 => {
                    Ok(PeraKeyPair::Secp256r1(Secp256r1KeyPair::from_bytes(
                        bytes.get(1..).ok_or_else(|| eyre!("Invalid length"))?,
                    )?))
                }
    ...
    pub fn to_bytes_no_flag(&self) -> Vec<u8> {
        match self {
            PeraKeyPair::Ed25519(kp) => kp.as_bytes().to_vec(),
            PeraKeyPair::Secp256k1(kp) => kp.as_bytes().to_vec(),
            PeraKeyPair::Secp256r1(kp) => kp.as_bytes().to_vec(),
        }
    }

    /// Encode a PeraKeyPair as `flag || privkey` in Bech32 starting with "peraprivkey" to a string. Note that the pubkey is not encoded.
    pub fn encode(&self) -> Result<String, eyre::Report> {
        Bech32::encode(self.to_bytes(), PERA_PRIV_KEY_PREFIX).map_err(|e| eyre!(e))
    }

    /// Decode a PeraKeyPair from `flag || privkey` in Bech32 starting with "peraprivkey" to PeraKeyPair. The public key is computed directly from the private key bytes.
    pub fn decode(value: &str) -> Result<Self, eyre::Report> {
        let bytes = Bech32::decode(value, PERA_PRIV_KEY_PREFIX)?;
        Self::from_bytes(&bytes)
    }
}

impl Serialize for PeraKeyPair {
...
impl<'de> Deserialize<'de> for PeraKeyPair {
        ...
        D: Deserializer<'de>,
    {
        ...
        PeraKeyPair::decode_base64(&s).map_err(|e| Error::custom(e.to_string()))
    }
}
...
impl ZkLoginPublicIdentifier {
    ...
    pub fn new(iss: &str, address_seed: &Bn254FrElement) -> PeraResult<Self> {
...
impl PublicKey {
    ...
    pub fn scheme(&self) -> SignatureScheme {
        match self {
            PublicKey::Ed25519(_) => Ed25519PeraSignature::SCHEME,
            PublicKey::Secp256k1(_) => Secp256k1PeraSignature::SCHEME,
            PublicKey::Secp256r1(_) => Secp256r1PeraSignature::SCHEME,
            PublicKey::ZkLogin(_) => SignatureScheme::ZkLoginAuthenticator,
            PublicKey::Passkey(_) => SignatureScheme::PasskeyAuthenticator,
        }
    }

    pub fn from_zklogin_inputs(inputs: &ZkLoginInputs) -> PeraResult<Self> {
...
pub trait PeraAuthoritySignature {
    fn verify_secure<T>(
        &self,
...
impl PeraAuthoritySignature for AuthoritySignature {
...
pub fn get_key_pair<KP: KeypairTraits>() -> (PeraAddress, KP)
where
    <KP as KeypairTraits>::PubKey: PeraPublicKey,
{
...
pub fn random_committee_key_pairs_of_size(size: usize) -> Vec<AuthorityKeyPair> {
    ...
    (0..size)
        .map(|_| {
            ...
            // keys in ConfigBuilder::build (pera-config/src/network_config_builder). This is because
...
pub fn deterministic_random_account_key() -> (PeraAddress, AccountKeyPair) {
    let mut rng = StdRng::from_seed([0; 32]);
    get_key_pair_from_rng(&mut rng)
}

pub fn get_account_key_pair() -> (PeraAddress, AccountKeyPair) {
    get_key_pair()
}

pub fn get_authority_key_pair() -> (PeraAddress, AuthorityKeyPair) {
    get_key_pair()
}

/// Generate a keypair from the specified RNG (useful for testing with seedable rngs).
pub fn get_key_pair_from_rng<KP: KeypairTraits, R>(csprng: &mut R) -> (PeraAddress, KP)
where
...
// TODO: C-GETTER
pub fn get_key_pair_from_bytes<KP: KeypairTraits>(bytes: &[u8]) -> PeraResult<(PeraAddress, KP)>
where
    <KP as KeypairTraits>::PubKey: PeraPublicKey,
{
    ...
    if bytes.len() != priv_length + pub_key_length {
        return Err(PeraError::KeyConversionError(format!(
    ...
    let sk = <KP as KeypairTraits>::PrivKey::from_bytes(
        bytes
            .get(..priv_length)
            .ok_or(PeraError::InvalidPrivateKey)?,
    )
    .map_err(|_| PeraError::InvalidPrivateKey)?;
    let kp: KP = sk.into();
...
pub enum Signature {
    Ed25519PeraSignature,
    Secp256k1PeraSignature,
    Secp256r1PeraSignature,
}
...
impl AsRef<[u8]> for Signature {
    fn as_ref(&self) -> &[u8] {
        match self {
            Signature::Ed25519PeraSignature(sig) => sig.as_ref(),
            Signature::Secp256k1PeraSignature(sig) => sig.as_ref(),
            Signature::Secp256r1PeraSignature(sig) => sig.as_ref(),
        }
    }
}
impl AsMut<[u8]> for Signature {
    fn as_mut(&mut self) -> &mut [u8] {
        match self {
            Signature::Ed25519PeraSignature(sig) => sig.as_mut(),
            Signature::Secp256k1PeraSignature(sig) => sig.as_mut(),
            Signature::Secp256r1PeraSignature(sig) => sig.as_mut(),
        }
    }
}
...
impl ToFromBytes for Signature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        match bytes.first() {
            Some(x) => {
                if x == &Ed25519PeraSignature::SCHEME.flag() {
                    Ok(<Ed25519PeraSignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else if x == &Secp256k1PeraSignature::SCHEME.flag() {
                    Ok(<Secp256k1PeraSignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else if x == &Secp256r1PeraSignature::SCHEME.flag() {
                    Ok(<Secp256r1PeraSignature as ToFromBytes>::from_bytes(bytes)?.into())
                } else {
                    Err(FastCryptoError::InvalidInput)
                }
            }
...
impl PeraPublicKey for BLS12381PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::BLS12381;
}

//
// Ed25519 Pera Signature port
//
...
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Ed25519PeraSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
...
impl Default for Ed25519PeraSignature {
    fn default() -> Self {
        Self([0; Ed25519PublicKey::LENGTH + Ed25519Signature::LENGTH + 1])
    }
}

impl PeraSignatureInner for Ed25519PeraSignature {
...
impl PeraPublicKey for Ed25519PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::ED25519;
}

impl ToFromBytes for Ed25519PeraSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
...
impl Signer<Signature> for Ed25519KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Ed25519PeraSignature::new(self, msg).into()
    }
}

//
// Secp256k1 Pera Signature port
//
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash, AsRef, AsMut)]
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Secp256k1PeraSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
    [u8; Secp256k1PublicKey::LENGTH + Secp256k1Signature::LENGTH + 1],
);

impl PeraSignatureInner for Secp256k1PeraSignature {
...
impl PeraPublicKey for Secp256k1PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::Secp256k1;
}

impl ToFromBytes for Secp256k1PeraSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        if bytes.len() != Self::LENGTH {
            return Err(FastCryptoError::InputLengthWrong(Self::LENGTH));
        }
...
impl Signer<Signature> for Secp256k1KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Secp256k1PeraSignature::new(self, msg).into()
    }
}

//
// Secp256r1 Pera Signature port
//
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash, AsRef, AsMut)]
#[as_ref(forward)]
#[as_mut(forward)]
pub struct Secp256r1PeraSignature(
    #[schemars(with = "Base64")]
    #[serde_as(as = "Readable<Base64, Bytes>")]
    [u8; Secp256r1PublicKey::LENGTH + Secp256r1Signature::LENGTH + 1],
);

impl PeraSignatureInner for Secp256r1PeraSignature {
...
impl PeraPublicKey for Secp256r1PublicKey {
    const SIGNATURE_SCHEME: SignatureScheme = SignatureScheme::Secp256r1;
}

impl ToFromBytes for Secp256r1PeraSignature {
    fn from_bytes(bytes: &[u8]) -> Result<Self, FastCryptoError> {
        if bytes.len() != Self::LENGTH {
            return Err(FastCryptoError::InputLengthWrong(Self::LENGTH));
        }
...
impl Signer<Signature> for Secp256r1KeyPair {
    fn sign(&self, msg: &[u8]) -> Signature {
        Secp256r1PeraSignature::new(self, msg).into()
    }
}
...
//
pub trait PeraSignatureInner: Sized + ToFromBytes + PartialEq + Eq + Hash {
    type Sig: Authenticator<PubKey = Self::PubKey>;
    type PubKey: VerifyingKey<Sig = Self::Sig> + PeraPublicKey;
    ...
    fn get_verification_inputs(&self) -> PeraResult<(Self::Sig, Self::PubKey)> {
        let pk = Self::PubKey::from_bytes(self.public_key_bytes())
            .map_err(|_| PeraError::KeyConversionError("Invalid public key".to_string()))?;

        // deserialize the signature
        let signature = Self::Sig::from_bytes(self.signature_bytes()).map_err(|_| {
            PeraError::InvalidSignature {
    ...
    fn new(kp: &Self::KeyPair, message: &[u8]) -> Self {
        ...
        signature_bytes
            .extend_from_slice(&[<Self::PubKey as PeraPublicKey>::SIGNATURE_SCHEME.flag()]);
...
pub trait PeraPublicKey: VerifyingKey {
    const SIGNATURE_SCHEME: SignatureScheme;
}

#[enum_dispatch(Signature)]
pub trait PeraSignature: Sized + ToFromBytes {
...
impl<S: PeraSignatureInner + Sized> PeraSignature for S {
    fn signature_bytes(&self) -> &[u8] {
        ...
        T: Serialize,
    {
        ...
        match scheme {
            ...
            _ => {
                let address = PeraAddress::from(pk);
                if author != address {
                    return Err(PeraError::IncorrectSigner {
                        error: format!(
...
impl AuthoritySignInfoTrait for AuthoritySignInfo {
    ...
    fn add_to_verification_obligation<'a>(
        ...
        obligation
            ...
            .ok_or(PeraError::InvalidAddress)?
...
#[serde_as]
#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema)]
pub struct PeraAuthorityStrongQuorumSignInfo {
    pub epoch: EpochId,
...
impl From<&AuthorityStrongQuorumSignInfo> for PeraAuthorityStrongQuorumSignInfo {
...
impl TryFrom<&PeraAuthorityStrongQuorumSignInfo> for AuthorityStrongQuorumSignInfo {
    type Error = FastCryptoError;

    fn try_from(info: &PeraAuthorityStrongQuorumSignInfo) -> Result<Self, Self::Error> {
        Ok(Self {
            epoch: info.epoch,
    ...
    for AuthorityQuorumSignInfo<STRONG_THRESHOLD>
{
    ...
    fn add_to_verification_obligation<'a>(
        ...
        obligation
            .signatures
            .get_mut(message_index)
            .ok_or(PeraError::InvalidAuthenticator)?
        ...
        for authority_index in self.signers_map.iter() {
            let authority = committee
                ...
                .ok_or_else(|| PeraError::UnknownSigner {
                    signer: None,
        ...
        fp_ensure!(
            ...
            PeraError::CertificateRequiresQuorum
        );
...
impl<const STRONG_THRESHOLD: bool> AuthorityQuorumSignInfo<STRONG_THRESHOLD> {
    ...
    pub fn authorities<'a>(
        &'a self,
        committee: &'a Committee,
    ) -> impl Iterator<Item = PeraResult<&AuthorityName>> {
...
/// * We use `BCS` to generate canonical bytes peratable for hashing and signing.
///
/// # Safety
...
mod bcs_signable {
    ...
    impl BcsSignable for crate::messages_signature_mpc::ProofMPCResultOnChain {}
...
impl<'a> VerificationObligation<'a> {
    ...
    pub fn verify_all(self) -> PeraResult<()> {
...
pub mod bcs_signable_test {
        ...
        T: super::bcs_signable::BcsSignable,
    {
        ...
        let idx = obligation.add_message(
            value,
            0,
            Intent::pera_app(IntentScope::SenderSignedTransaction),
        );
        (obligation, idx)
    }
}
...
pub enum SignatureScheme {
    ...
    Secp256k1,
    Secp256r1,
    BLS12381, // This is currently not supported for user Pera Address.
    MultiSig,
...
impl SignatureScheme {
    ...
    pub fn from_flag(flag: &str) -> Result<SignatureScheme, PeraError> {
        let byte_int = flag
            .parse::<u8>()
            .map_err(|_| PeraError::KeyConversionError("Invalid key scheme".to_string()))?;
        Self::from_flag_byte(&byte_int)
    }

    pub fn from_flag_byte(byte_int: &u8) -> Result<SignatureScheme, PeraError> {
        match byte_int {
