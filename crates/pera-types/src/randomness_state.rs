...
use crate::{
    error::PeraResult, object::Owner, storage::ObjectStore, PERA_FRAMEWORK_ADDRESS,
...
pub const RESOLVED_PERA_RANDOMNESS_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
pub fn is_mutable_random(view: &CompiledModule, s: &SignatureToken) -> bool {
    match s {
        ...
        SignatureToken::Datatype(idx) => {
            resolve_struct(view, *idx) == RESOLVED_PERA_RANDOMNESS_STATE
        }
        _ => false,
    }
}
