...
use crate::crypto::{Signer, PeraKeyPair};
...
pub fn make_transaction_data(sender: PeraAddress) -> TransactionData {
    let object = Object::immutable_with_id_for_testing(ObjectID::random_from_rng(
        &mut StdRng::from_seed([0; 32]),
    ));
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
        builder.transfer_pera(dbg_addr(2), None);
        builder.finish()
    };
...
pub fn make_transaction(sender: PeraAddress, kp: &PeraKeyPair) -> Transaction {
...
mod zk_login {
    ...
    pub fn load_test_vectors(path: &str) -> Vec<(PeraKeyPair, PublicKey, ZkLoginInputs)> {
        ...
        for test in test_datum {
            let kp = PeraKeyPair::decode(&test.kp).unwrap();
            let inputs =
    ...
    pub fn get_zklogin_user_address() -> PeraAddress {
        thread_local! {
            static USER_ADDRESS: PeraAddress = {
                ...
                PeraAddress::from_bytes(hasher.finalize().digest).unwrap()
            };
        }
        USER_ADDRESS.with(|a| *a)
    }

    fn get_zklogin_user_key() -> PeraKeyPair {
    ...
    pub fn get_legacy_zklogin_user_address() -> PeraAddress {
        thread_local! {
            static USER_ADDRESS: PeraAddress = {
                let inputs = get_inputs_with_bad_address_seed();
                PeraAddress::from(&PublicKey::from_zklogin_inputs(&inputs).unwrap())
            };
        }
        USER_ADDRESS.with(|a| *a)
    }

    pub fn sign_zklogin_personal_msg(data: PersonalMessage) -> (PeraAddress, GenericSignature) {
        let inputs = get_zklogin_inputs();
    ...
    pub fn sign_zklogin_tx_with_default_proof(
        data: TransactionData,
        legacy: bool,
    ) -> (PeraAddress, Transaction, GenericSignature) {
    ...
    pub fn keys() -> Vec<PeraKeyPair> {
        let mut seed = StdRng::from_seed([0; 32]);
        let kp1: PeraKeyPair = PeraKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1);
        let kp2: PeraKeyPair = PeraKeyPair::Secp256k1(get_key_pair_from_rng(&mut seed).1);
        let kp3: PeraKeyPair = PeraKeyPair::Secp256r1(get_key_pair_from_rng(&mut seed).1);
        vec![kp1, kp2, kp3]
    }
    ...
    pub fn make_upgraded_multisig_tx() -> Transaction {
        ...
        .unwrap();
        let addr = PeraAddress::from(&multisig_pk);
        let tx = make_transaction(addr, &keys[0]);

        let msg = IntentMessage::new(Intent::pera_transaction(), tx.transaction_data().clone());
