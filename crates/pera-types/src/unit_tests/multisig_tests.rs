...
use crate::{
    ...
    crypto::{
        get_key_pair, get_key_pair_from_rng, Ed25519PeraSignature, PublicKey, Signature, PeraKeyPair,
        PeraSignatureInner, ZkLoginPublicIdentifier,
    },
...
fn test_combine_sigs() {
    let kp1: PeraKeyPair = PeraKeyPair::Ed25519(get_key_pair().1);
    let kp2: PeraKeyPair = PeraKeyPair::Secp256k1(get_key_pair().1);
    let kp3: PeraKeyPair = PeraKeyPair::Secp256r1(get_key_pair().1);
...
fn test_serde_roundtrip() {
    ...
    let sig = Ed25519PeraSignature::default();
    ...
    assert_eq!(single_sig_bytes.len(), Ed25519PeraSignature::LENGTH);
    assert_eq!(
...
fn test_multisig_address() {
    ...
    assert_eq!(
        PeraAddress::from_str("0xe35c69eb504de34afdbd9f307fb3ca152646c92d549fea00065d26fc422109ea")
            .unwrap(),
        address
    );
}
...
fn test_max_sig() {
    ...
    for _ in 0..11 {
        let k = PeraKeyPair::Ed25519(get_key_pair_from_rng(&mut seed).1);
        pks.push(k.public());
        keys.push(k);
    }
...
fn multisig_zklogin_scenarios() {
    // consistency test with pera/sdk/typescript/test/unit/cryptography/multisig.test.ts
    let mut seed = StdRng::from_seed([0; 32]);
    let kp: Ed25519KeyPair = get_key_pair_from_rng(&mut seed).1;
    let skp: PeraKeyPair = PeraKeyPair::Ed25519(kp);
    let pk1 = skp.public();
    ...
    let multisig_addr = PeraAddress::from(&multisig_pk);
    assert_eq!(
        multisig_addr,
        PeraAddress::from_str("0xb9c0780a3943cde13a2409bf1a6f06ae60b0dff2b2f373260cf627aa4f43a588")
            .unwrap()
    );
    ...
    let intent_msg = &IntentMessage::new(Intent::pera_transaction(), tx.clone());
...
fn zklogin_in_multisig_works_with_both_addresses() {
    ...
    let multisig_address = PeraAddress::from(&multisig_pk);
    ...
    assert!(
        matches!(res, Err(crate::error::PeraError::InvalidSignature { error }) if error.contains("General cryptographic error: Groth16 proof verify failed"))
    );
    ...
    let multisig_address_padded = PeraAddress::from(&multisig_pk_padded);
...
fn test_derive_multisig_address() {
    ...
    assert_eq!(
        multisig_addr,
        PeraAddress::from_str("0x77a9fbf3c695d78dd83449a81a9e70aa79a77dbfd6fb72037bf09201c12052cd")
            .unwrap()
    );
}
