...
use crate::{id::UID, PERA_AUTHENTICATOR_STATE_OBJECT_ID, PERA_FRAMEWORK_ADDRESS};
...
pub const RESOLVED_PERA_AUTHENTICATOR_STATE: (&AccountAddress, &IdentStr, &IdentStr) = (
...
) -> PeraResult<Option<AuthenticatorStateInner>> {
    let outer = object_store.get_object(&PERA_AUTHENTICATOR_STATE_OBJECT_ID)?;
...
pub fn get_authenticator_state_obj_initial_shared_version(
    ...
    Ok(object_store
        .get_object(&PERA_AUTHENTICATOR_STATE_OBJECT_ID)?
