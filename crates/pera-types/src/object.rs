...
impl MoveObject {
    ...
    pub fn get_layout(&self, resolver: &impl GetModule) -> Result<MoveStructLayout, PeraError> {
    ...
    ) -> Result<MoveStructLayout, PeraError> {
    ...
    pub fn to_move_struct(&self, layout: &MoveStructLayout) -> Result<MoveStruct, PeraError> {
...
impl MoveObject {
    ...
    pub fn get_coin_balances(
    ...
    ) -> Result<BTreeMap<TypeTag, u64>, PeraError> {
        ...
        if let Some(type_tag) = self.type_.coin_type_maybe() {
            ...
            MoveValue::visit_deserialize(&self.contents, &layout.into_layout(), &mut traversal)
                .map_err(|e| PeraError::ObjectSerializationError {
                    error: e.to_string(),
                })?;
...
pub enum Owner {
    ...
    /// The object ID is converted to PeraAddress as PeraAddress is universal.
    ObjectOwner(PeraAddress),
...
impl Owner {
    ...
    pub fn get_address_owner_address(&self) -> PeraResult<PeraAddress> {
        match self {
            Self::AddressOwner(address) => Ok(*address),
            Self::Shared { .. } | Self::Immutable | Self::ObjectOwner(_) => {
                Err(PeraError::UnexpectedOwnerType)
            }
        }
    }

    // NOTE: this function will return address of both AddressOwner and ObjectOwner,
    // address of ObjectOwner is converted from object id, even though the type is PeraAddress.
    pub fn get_owner_address(&self) -> PeraResult<PeraAddress> {
        match self {
            Self::AddressOwner(address) | Self::ObjectOwner(address) => Ok(*address),
            Self::Shared { .. } | Self::Immutable => Err(PeraError::UnexpectedOwnerType),
        }
    }
...
impl PartialEq<PeraAddress> for Owner {
    fn eq(&self, other: &PeraAddress) -> bool {
        match self {
...
impl PartialEq<ObjectID> for Owner {
    fn eq(&self, other: &ObjectID) -> bool {
        let other_id: PeraAddress = (*other).into();
        match self {
...
impl ObjectInner {
    ...
    pub fn get_single_owner(&self) -> Option<PeraAddress> {
    ...
    pub fn transfer(&mut self, new_owner: PeraAddress) {
    ...
    pub fn get_layout(
        &self,
        resolver: &impl GetModule,
    ) -> Result<Option<MoveStructLayout>, PeraError> {
        match &self.data {
    ...
    pub fn get_move_template_type(&self) -> PeraResult<TypeTag> {
        let move_struct = self.data.struct_tag().ok_or_else(|| PeraError::TypeError {
...
impl Object {
    ...
    pub fn with_id_owner_gas_for_testing(id: ObjectID, owner: PeraAddress, gas: u64) -> Self {
        let data = Data::Move(MoveObject {
            type_: GasCoin::type_().into(),
            has_public_transfer: true,
            version: OBJECT_START_VERSION,
    ...
    pub fn with_id_owner_for_testing(id: ObjectID, owner: PeraAddress) -> Self {
    ...
    pub fn with_owner_for_testing(owner: PeraAddress) -> Self {
    ...
    pub fn new_gas_with_balance_and_owner_for_testing(value: u64, owner: PeraAddress) -> Self {
...
fn test_object_digest_and_serialized_format() {
    ...
    let o = Object::new_move(
        g,
        Owner::AddressOwner(PeraAddress::ZERO),
