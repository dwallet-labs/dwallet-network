...
use crate::gas_coin::NPERA_PER_PERA;
use crate::id::{ID, UID};
...
///
/// 30 million PERA
pub const MIN_VALIDATOR_JOINING_STAKE_NPERA: u64 = 30_000_000 * NPERA_PER_PERA;
...
///
/// 20 million PERA
pub const VALIDATOR_LOW_STAKE_THRESHOLD_NPERA: u64 = 20_000_000 * NPERA_PER_PERA;

/// Validators with stake below `validator_very_low_stake_threshold` will be removed
/// immediately at epoch change, no grace period.
///
/// 15 million PERA
pub const VALIDATOR_VERY_LOW_STAKE_THRESHOLD_NPERA: u64 = 15_000_000 * NPERA_PER_PERA;
...
pub const STAKED_PERA_STRUCT_NAME: &IdentStr = ident_str!("StakedPera");
...
impl StakedPera {
    pub fn type_() -> StructTag {
        StructTag {
            ...
            name: STAKED_PERA_STRUCT_NAME.to_owned(),
            type_params: vec![],
        }
    }

    pub fn is_staked_pera(s: &StructTag) -> bool {
        s.address == PERA_SYSTEM_ADDRESS
            && s.module.as_ident_str() == STAKING_POOL_MODULE_NAME
            && s.name.as_ident_str() == STAKED_PERA_STRUCT_NAME
...
impl TryFrom<&Object> for StakedPera {
    type Error = PeraError;
    fn try_from(object: &Object) -> Result<Self, Self::Error> {
        match &object.data {
            Data::Move(o) => {
                if o.type_().is_staked_pera() {
                    return bcs::from_bytes(o.contents()).map_err(|err| PeraError::TypeError {
                        error: format!("Unable to deserialize StakedPera object: {:?}", err),
                    });
                }
            }
            Data::Package(_) => {}
        ...
        Err(PeraError::TypeError {
            error: format!("Object type is not a StakedPera: {:?}", object),
        })
    }
}
