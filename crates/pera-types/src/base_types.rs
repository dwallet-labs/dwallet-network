...
use crate::error::{ExecutionError, PeraResult};
use crate::gas_coin::GasCoin;
use crate::gas_coin::GAS;
use crate::governance::StakedPera;
use crate::governance::STAKED_PERA_STRUCT_NAME;
use crate::governance::STAKING_POOL_MODULE_NAME;
use crate::id::RESOLVED_PERA_ID;
...
use crate::PERA_CLOCK_OBJECT_ID;
use crate::PERA_FRAMEWORK_ADDRESS;
...
pub enum MoveObjectType_ {
    ...
    Other(StructTag),
    /// A PERA coin (i.e., `0x2::coin::Coin<0x2::pera::PERA>`)
    GasCoin,
    /// A record of a staked PERA coin (i.e., `0x3::staking_pool::StakedPera`)
    StakedPera,
    /// A non-PERA coin type (i.e., `0x2::coin::Coin<T> where T != 0x2::pera::PERA`)
    Coin(TypeTag),
...
impl MoveObjectType {
    ...
    pub fn staked_pera() -> Self {
        Self(MoveObjectType_::StakedPera)
    }

    pub fn address(&self) -> AccountAddress {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => PERA_FRAMEWORK_ADDRESS,
            MoveObjectType_::StakedPera => PERA_SYSTEM_ADDRESS,
    ...
    pub fn module(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedPera => STAKING_POOL_MODULE_NAME,
    ...
    pub fn name(&self) -> &IdentStr {
        match &self.0 {
            ...
            MoveObjectType_::StakedPera => STAKED_PERA_STRUCT_NAME,
    ...
    pub fn type_params(&self) -> Vec<TypeTag> {
        match &self.0 {
            ...
            MoveObjectType_::StakedPera => vec![],
    ...
    pub fn coin_type_maybe(&self) -> Option<TypeTag> {
        match &self.0 {
            ...
            MoveObjectType_::StakedPera => None,
    ...
    pub fn size_for_gas_metering(&self) -> usize {
        ...
        match &self.0 {
            ...
            MoveObjectType_::StakedPera => 1,
            MoveObjectType_::Coin(inner) => bcs::serialized_size(inner).unwrap() + 1,
            MoveObjectType_::Other(s) => bcs::serialized_size(s).unwrap() + 1,
        }
    }

    /// Return true if `self` is `0x2::coin::Coin<T>` for some T (note: T can be PERA)
    pub fn is_coin(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::Coin(_) => true,
            MoveObjectType_::StakedPera | MoveObjectType_::Other(_) => false,
        }
    }

    /// Return true if `self` is 0x2::coin::Coin<0x2::pera::PERA>
    pub fn is_gas_coin(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin => true,
            MoveObjectType_::StakedPera | MoveObjectType_::Coin(_) | MoveObjectType_::Other(_) => {
                false
            }
        }
    }
    ...
    pub fn is_staked_pera(&self) -> bool {
        match &self.0 {
            MoveObjectType_::StakedPera => true,
    ...
    pub fn is_coin_metadata(&self) -> bool {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedPera | MoveObjectType_::Coin(_) => {
                false
            }
    ...
    pub fn is_upgrade_cap(&self) -> bool {
        self.address() == PERA_FRAMEWORK_ADDRESS
    ...
    pub fn try_extract_field_name(&self, type_: &DynamicFieldType) -> PeraResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedPera | MoveObjectType_::Coin(_) => {
    ...
    pub fn try_extract_field_value(&self) -> PeraResult<TypeTag> {
        match &self.0 {
            MoveObjectType_::GasCoin | MoveObjectType_::StakedPera | MoveObjectType_::Coin(_) => {
                Err(PeraError::ObjectDeserializationError {
    ...
    pub fn is(&self, s: &StructTag) -> bool {
        match &self.0 {
            ...
            MoveObjectType_::StakedPera => StakedPera::is_staked_pera(s),
...
impl From<StructTag> for MoveObjectType {
    fn from(mut s: StructTag) -> Self {
        ...
        } else if StakedPera::is_staked_pera(&s) {
            MoveObjectType_::StakedPera
        } else {
...
impl From<MoveObjectType> for StructTag {
    fn from(t: MoveObjectType) -> Self {
        match t.0 {
            ...
            MoveObjectType_::StakedPera => StakedPera::type_(),
...
pub fn is_primitive_type_tag(t: &TypeTag) -> bool {
    ...
    match t {
        ...
        T::Struct(st) => {
            ...
            // is id or..
            if resolved_struct == RESOLVED_PERA_ID {
                return true;
            }
...
impl FromStr for ObjectType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.to_lowercase() == PACKAGE {
            ...
            let tag = parse_pera_struct_tag(s)?;
...
pub const PERA_ADDRESS_LENGTH: usize = ObjectID::LENGTH;
...
#[cfg_attr(feature = "fuzzing", derive(proptest_derive::Arbitrary))]
pub struct PeraAddress(
    #[schemars(with = "Hex")]
    #[serde_as(as = "Readable<Hex, _>")]
    [u8; PERA_ADDRESS_LENGTH],
);

impl PeraAddress {
    pub const ZERO: Self = Self([0u8; PERA_ADDRESS_LENGTH]);
    ...
    #[cfg(any(feature = "test-utils", test))]
    /// Return a random PeraAddress.
    pub fn random_for_testing_only() -> Self {
        AccountAddress::random().into()
    }

    pub fn generate<R: rand::RngCore + rand::CryptoRng>(mut rng: R) -> Self {
        let buf: [u8; PERA_ADDRESS_LENGTH] = rng.gen();
        Self(buf)
    }

    /// Serialize an `Option<PeraAddress>` in Hex.
    pub fn optional_address_as_hex<S>(
        key: &Option<PeraAddress>,
        serializer: S,
    ) -> Result<S::Ok, S::Error>
    where
    ...
    /// Deserialize into an `Option<PeraAddress>`.
    pub fn optional_address_from_hex<'de, D>(
        deserializer: D,
    ) -> Result<Option<PeraAddress>, D::Error>
    where
    ...
    /// Return the underlying byte array of a PeraAddress.
    pub fn to_inner(self) -> [u8; PERA_ADDRESS_LENGTH] {
        self.0
    }

    /// Parse a PeraAddress from a byte array or buffer.
    pub fn from_bytes<T: AsRef<[u8]>>(bytes: T) -> Result<Self, PeraError> {
        <[u8; PERA_ADDRESS_LENGTH]>::try_from(bytes.as_ref())
            .map_err(|_| PeraError::InvalidAddress)
            .map(PeraAddress)
    }
    ...
    pub fn try_from_padded(inputs: &ZkLoginInputs) -> PeraResult<Self> {
        Ok((&PublicKey::from_zklogin_inputs(inputs)?).into())
    }

    /// Define as iss_bytes_len || iss_bytes || unpadded_32_byte_address_seed.
    pub fn try_from_unpadded(inputs: &ZkLoginInputs) -> PeraResult<Self> {
        ...
        Ok(PeraAddress(hasher.finalize().digest))
    }
}

impl From<ObjectID> for PeraAddress {
    fn from(object_id: ObjectID) -> PeraAddress {
        Self(object_id.into_bytes())
    }
}

impl From<AccountAddress> for PeraAddress {
    fn from(address: AccountAddress) -> PeraAddress {
        Self(address.into_bytes())
    }
}

impl TryFrom<&[u8]> for PeraAddress {
    type Error = PeraError;

    /// Tries to convert the provided byte array into a PeraAddress.
    fn try_from(bytes: &[u8]) -> Result<Self, PeraError> {
...
impl TryFrom<Vec<u8>> for PeraAddress {
    ...
    /// Tries to convert the provided byte buffer into a PeraAddress.
    fn try_from(bytes: Vec<u8>) -> Result<Self, PeraError> {
        Self::from_bytes(bytes)
    }
}

impl AsRef<[u8]> for PeraAddress {
    fn as_ref(&self) -> &[u8] {
        &self.0[..]
    }
}
...
impl<T: PeraPublicKey> From<&T> for PeraAddress {
    fn from(pk: &T) -> Self {
        let mut hasher = DefaultHash::default();
        hasher.update([T::SIGNATURE_SCHEME.flag()]);
        hasher.update(pk);
        let g_arr = hasher.finalize();
        PeraAddress(g_arr.digest)
    }
}

impl From<&PublicKey> for PeraAddress {
...
impl From<&MultiSigPublicKey> for PeraAddress {
    /// Derive a PeraAddress from [struct MultiSigPublicKey]. A MultiSig address
    ...
    fn from(multisig_pk: &MultiSigPublicKey) -> Self {
        ...
        PeraAddress(hasher.finalize().digest)
    }
}

/// Pera address for [struct ZkLoginAuthenticator] is defined as the black2b hash of
/// [zklogin_flag || iss_bytes_length || iss_bytes || unpadded_address_seed_in_bytes].
impl TryFrom<&ZkLoginAuthenticator> for PeraAddress {
    type Error = PeraError;
    fn try_from(authenticator: &ZkLoginAuthenticator) -> PeraResult<Self> {
        PeraAddress::try_from_unpadded(&authenticator.inputs)
    }
}

impl TryFrom<&GenericSignature> for PeraAddress {
    type Error = PeraError;
    /// Derive a PeraAddress from a serialized signature in Pera [GenericSignature].
    fn try_from(sig: &GenericSignature) -> PeraResult<Self> {
        match sig {
            ...
            GenericSignature::ZkLoginAuthenticator(zklogin) => {
                PeraAddress::try_from_unpadded(&zklogin.inputs)
            }
            GenericSignature::PasskeyAuthenticator(s) => Ok(PeraAddress::from(&s.get_pk()?)),
        }
    }
}

impl fmt::Display for PeraAddress {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "0x{}", Hex::encode(self.0))
    }
}

impl fmt::Debug for PeraAddress {
...
/// Generate a fake PeraAddress with repeated one byte.
pub fn dbg_addr(name: u8) -> PeraAddress {
    let addr = [name; PERA_ADDRESS_LENGTH];
    PeraAddress(addr)
}
...
impl TxContext {
    pub fn new(sender: &PeraAddress, digest: &TransactionDigest, epoch_data: &EpochData) -> Self {
    ...
    pub fn kind(view: &CompiledModule, s: &SignatureToken) -> TxContextKind {
        ...
        let is_tx_context_type = module_name == TX_CONTEXT_MODULE_NAME
            && module_addr == &PERA_FRAMEWORK_ADDRESS
    ...
    pub fn sender(&self) -> PeraAddress {
        PeraAddress::from(ObjectID(self.sender))
    }
    ...
    pub fn random_for_testing_only() -> Self {
        Self::new(
            &PeraAddress::random_for_testing_only(),
    ...
    pub fn with_sender_for_testing_only(sender: &PeraAddress) -> Self {
...
impl ObjectID {
    ...
    pub fn is_clock(&self) -> bool {
        *self == PERA_CLOCK_OBJECT_ID
    }
}

impl From<PeraAddress> for ObjectID {
    fn from(address: PeraAddress) -> ObjectID {
...
impl From<PeraAddress> for AccountAddress {
    fn from(address: PeraAddress) -> Self {
        Self::new(address.0)
    }
}
...
impl fmt::Display for MoveObjectType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}",
            to_pera_struct_tag_string(&s).map_err(fmt::Error::custom)?
        )
    }
}
