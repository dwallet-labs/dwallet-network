...
use ika_json::{call_args, type_args};
...
use ika_json_rpc_types::ObjectChange;
use ika_json_rpc_types::ObjectsPage;
use ika_json_rpc_types::{
    Balance, CoinPage, DelegatedStake, StakeStatus, IkaCoinMetadata, IkaExecutionStatus,
    IkaObjectDataOptions, IkaObjectResponse, IkaObjectResponseQuery, IkaTransactionBlockEffectsAPI,
    IkaTransactionBlockResponse, IkaTransactionBlockResponseOptions, TransactionBlockBytes,
};
use ika_macros::sim_test;
use ika_move_build::BuildConfig;
use ika_swarm_config::genesis_config::{DEFAULT_GAS_AMOUNT, DEFAULT_NUMBER_OF_OBJECT_PER_ACCOUNT};
use ika_test_transaction_builder::make_transfer_ika_transaction;
use ika_types::balance::Supply;
use ika_types::base_types::SequenceNumber;
use ika_types::base_types::{ObjectID, IkaAddress};
use ika_types::coin::{TreasuryCap, COIN_MODULE_NAME};
...
use ika_types::{parse_ika_struct_tag, IKA_FRAMEWORK_ADDRESS};
...
async fn test_get_package_with_display_should_not_fail() -> Result<(), anyhow::Error> {
    ...
    let response = http_client
        .get_object(
            ObjectID::from(IKA_FRAMEWORK_ADDRESS),
            Some(IkaObjectDataOptions::new().with_display()),
        )
        .await;
    assert!(response.is_ok());
    let response: IkaObjectResponse = response?;
    assert!(response
        .into_object()
        .unwrap()
        .display
...
async fn test_public_transfer_object() -> Result<(), anyhow::Error> {
    ...
    let tx_response: IkaTransactionBlockResponse = http_client
...
async fn test_publish() -> Result<(), anyhow::Error> {
    ...
    let tx_response = http_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(IkaTransactionBlockResponseOptions::new().with_effects()),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;
    matches!(tx_response, IkaTransactionBlockResponse {effects, ..} if effects.as_ref().unwrap().created().len() == 6);
    Ok(())
}
...
async fn test_move_call() -> Result<(), anyhow::Error> {
    ...
    // now do the call
    let package_id = ObjectID::new(IKA_FRAMEWORK_ADDRESS.into_bytes());
    ...
    let tx_response = http_client
        ...
        .await?;
    matches!(tx_response, IkaTransactionBlockResponse {effects, ..} if effects.as_ref().unwrap().created().len() == 1);
    Ok(())
}
...
async fn test_get_object_info() -> Result<(), anyhow::Error> {
    ...
    for obj in objects {
        ...
        let result = http_client
            .get_object(
                oref.object_id,
                Some(IkaObjectDataOptions::new().with_owner()),
            )
            .await?;
        assert!(
            matches!(result, IkaObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.unwrap().get_owner_address()? == address)
        );
    }
    Ok(())
}
...
async fn test_get_object_data_with_content() -> Result<(), anyhow::Error> {
    ...
    let objects = http_client
        .get_owned_objects(
            address,
            Some(IkaObjectResponseQuery::new_with_options(
                IkaObjectDataOptions::new().with_content().with_owner(),
            )),
            None,
            None,
        )
    ...
    for obj in objects {
        ...
        let result = http_client
            .get_object(
                oref.object_id,
                Some(IkaObjectDataOptions::new().with_content().with_owner()),
            )
            .await?;
        assert!(
            matches!(result, IkaObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.unwrap().get_owner_address()? == address)
...
async fn test_get_coins() -> Result<(), anyhow::Error> {
    ...
    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::ika::TestCoin".into()), None, None)
        .await?;
    assert_eq!(0, result.data.len());

    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::ika::IKA".into()), None, None)
        .await?;
    assert_eq!(5, result.data.len());
    assert!(!result.has_next_page);
    ...
    // Test paging
    let result: CoinPage = http_client
        .get_coins(address, Some("0x2::ika::IKA".into()), None, Some(3))
        .await?;
...
async fn test_sorted_get_coin_response() {
    ...
    let address = IkaAddress::random_for_testing_only();

    // send 5 coins to address `address` with different values
    let amounts = [1, 2, 3, 4, 5];
    for amount in amounts {
        let tx = make_transfer_ika_transaction(&cluster.wallet, Some(address), Some(amount)).await;
        let (tx_bytes, signatures) = tx.to_tx_bytes_and_signatures();
...
async fn test_get_balance() -> Result<(), anyhow::Error> {
    ...
    assert_eq!("0x2::ika::IKA", result.coin_type);
    assert_eq!(
...
async fn test_get_metadata() -> Result<(), anyhow::Error> {
    ...
    let result: IkaCoinMetadata = http_client
...
async fn test_get_total_supply() -> Result<(), anyhow::Error> {
    ...
    let treasury_cap = object_changes
        ...
        .find_map(|e| {
            ...
            {
                if &TreasuryCap::type_(parse_ika_struct_tag(&coin_name).unwrap()) == object_type {
                    Some(object_id)
                } else {
                    None
                }
    ...
    let IkaTransactionBlockResponse { effects, .. } = tx_response;

    assert_eq!(IkaExecutionStatus::Success, *effects.unwrap().status());
...
async fn test_staking() -> Result<(), anyhow::Error> {
    ...
    // Check StakedIka object before test
    let staked_ika: Vec<DelegatedStake> = http_client.get_stakes(address).await?;
    assert!(staked_ika.is_empty());
    ...
    http_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(IkaTransactionBlockResponseOptions::new()),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;
    ...
    let staked_ika: Vec<DelegatedStake> = http_client.get_stakes(address).await?;
    assert_eq!(1, staked_ika.len());
    assert_eq!(1000000000, staked_ika[0].stakes[0].principal);
    assert!(matches!(
        staked_ika[0].stakes[0].status,
        StakeStatus::Pending
    ));
    let staked_ika_copy = http_client
        .get_stakes_by_ids(vec![staked_ika[0].stakes[0].staked_ika_id])
        .await?;
    assert_eq!(
        staked_ika[0].stakes[0].staked_ika_id,
        staked_ika_copy[0].stakes[0].staked_ika_id
    );
    Ok(())
}
...
async fn test_unstaking() -> Result<(), anyhow::Error> {
    ...
    let staked_ika_copy = http_client
        .get_stakes_by_ids(vec![
            staked_ika[0].stakes[0].staked_ika_id,
            staked_ika[0].stakes[1].staked_ika_id,
            staked_ika[0].stakes[2].staked_ika_id,
        ])
        .await?;

    assert!(matches!(
        &staked_ika_copy[0].stakes[0].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    assert!(matches!(
        &staked_ika_copy[0].stakes[1].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    assert!(matches!(
        &staked_ika_copy[0].stakes[2].status,
        StakeStatus::Active {
            estimated_reward: _
        }
    ));
    ...
    let transaction_bytes: TransactionBlockBytes = http_client
        .request_withdraw_stake(
            address,
            staked_ika_copy[0].stakes[2].staked_ika_id,
            None,
            1_000_000.into(),
        )
        .await?;
