...
use pera_config::{
    pera_config_dir, Config, PersistedConfig, PERA_CLIENT_CONFIG, PERA_KEYSTORE_FILENAME,
};
use pera_json_rpc_types::{Coin, PeraObjectDataOptions};
...
// const PERA_FAUCET: &str = "https://faucet.devnet.pera.io/gas"; // devnet faucet

pub const PERA_FAUCET: &str = "https://faucet.testnet.pera.io/v1/gas"; // testnet faucet

// const PERA_FAUCET: &str = "http://127.0.0.1:9123/gas";

/// Return a pera client to interact with the APIs,
...
pub async fn setup_for_write() -> Result<(PeraClient, PeraAddress, PeraAddress), anyhow::Error> {
    let (client, active_address) = setup_for_read().await?;
    // make sure we have some PERA (5_000_000 NPERA) on this address
...
/// Return a pera client to interact with the APIs and an active address from the local wallet.
///
/// This function sets up a wallet in case there is no wallet locally,
/// and ensures that the active address of the wallet has PERA on it.
/// If there is no PERA owned by the active address, then it will request
/// PERA from the faucet.
pub async fn setup_for_read() -> Result<(PeraClient, PeraAddress), anyhow::Error> {
    let client = PeraClientBuilder::default().build_testnet().await?;
    println!("Pera testnet version is: {}", client.api_version());
...
pub async fn request_tokens_from_faucet(
    ...
    loop {
        let resp = client
            .get("https://faucet.testnet.pera.io/v1/status")
            .header("Content-Type", "application/json")
            .json(&json_body)
            .send()
            .await?;
...
/// Return the coin owned by the address that has at least 5_000_000 NPERA, otherwise returns None
pub async fn fetch_coin(
    pera: &PeraClient,
    sender: &PeraAddress,
) -> Result<Option<Coin>, anyhow::Error> {
    let coin_type = "0x2::pera::PERA".to_string();
    let coins_stream = pera
        .coin_read_api()
...
pub async fn split_coin_digest(
    ...
    let coin = match fetch_coin(pera, sender).await? {
        None => {
            request_tokens_from_faucet(*sender, pera).await?;
            fetch_coin(pera, sender)
                .await?
                .expect("Supposed to get a coin with PERA, but didn't. Aborting")
        }
        Some(c) => c,
    };
    ...
    // first, we want to split the coin, and we specify how much PERA (in NPERA) we want
    // for the new coin
...
pub fn retrieve_wallet() -> Result<WalletContext, anyhow::Error> {
    ...
    // check if a wallet exists and if not, create a wallet and a pera client config
    ...
    if !wallet_conf.exists() {
        let keystore = FileBasedKeystore::new(&keystore_path)?;
        let mut client_config = PeraClientConfig::new(keystore.into());

        client_config.add_env(PeraEnv::testnet());
        client_config.add_env(PeraEnv::devnet());
        client_config.add_env(PeraEnv::localnet());
    ...
    let mut client_config: PeraClientConfig = PersistedConfig::read(&wallet_conf)?;
...
async fn just_for_clippy() -> Result<(), anyhow::Error> {
    let (pera, sender, _recipient) = setup_for_write().await?;
    let _digest = split_coin_digest(&pera, &sender).await?;
    Ok(())
}
