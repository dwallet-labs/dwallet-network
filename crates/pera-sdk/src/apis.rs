...
use pera_json_rpc_types::CheckpointPage;
...
impl ReadApi {
    ...
    ///     let pera = PeraClientBuilder::default().build_localnet().await?;
    ///     let address = PeraAddress::from_str("0x0000....0000")?;
    ...
    ///         "No object data for this PeraObjectResponse {:?}",
    ///         object
    ///     ));
    ...
    pub async fn get_dynamic_fields(
        ...
        cursor: Option<ObjectID>,
        limit: Option<usize>,
    ) -> PeraRpcResult<DynamicFieldPage> {
        Ok(self
            .api
            .http
    ...
    ) -> PeraRpcResult<PeraObjectResponse> {
        Ok(self
            .api
            .http
    ...
    /// use pera_sdk::PeraClientBuilder;
    /// use pera_types::base_types::{ObjectID, PeraAddress};
    /// use pera_json_rpc_types::PeraObjectDataOptions;
    /// use std::str::FromStr;
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), anyhow::Error> {
    ...
    ) -> PeraRpcResult<PeraPastObjectResponse> {
        Ok(self
            .api
            .http
    ...
    /// Return a list of [PeraPastObjectResponse] objects, or an error upon failure.
    ///
    ...
    /// use pera_sdk::PeraClientBuilder;
    /// use pera_types::base_types::{ObjectID, PeraAddress};
    /// use pera_json_rpc_types::{PeraObjectDataOptions, PeraGetPastObjectRequest};
    /// use std::str::FromStr;
    ///
    /// #[tokio::main]
    /// async fn main() -> Result<(), anyhow::Error> {
    ...
    pub async fn try_multi_get_parsed_past_object(
        &self,
        past_objects: Vec<PeraGetPastObjectRequest>,
        options: PeraObjectDataOptions,
    ) -> PeraRpcResult<Vec<PeraPastObjectResponse>> {
        Ok(self
            .api
            .http
    ...
    /// Return a [PeraObjectResponse] based on the provided [ObjectID] and [PeraObjectDataOptions], or an error upon failure.
    ///
    /// The [PeraObjectResponse] contains two fields:
    /// 1) `data` for the object's data (see [PeraObjectData](pera_json_rpc_types::PeraObjectData)),
    /// 2) `error` for the error (if any) (see [PeraObjectResponseError](pera_types::error::PeraObjectResponseError)).
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///     ));
    ///     let object_id = object_data.object_id;
    ///     let object = pera.read_api().get_object_with_options(object_id,
    ///             PeraObjectDataOptions {
    ///                 show_type: true,
    ///                 show_owner: true,
    ///                 show_previous_transaction: true,
    ...
    /// Return a list of [PeraObjectResponse] from the given vector of [ObjectID]s and [PeraObjectDataOptions], or an error upon failure.
    ///
    ...
    ///     let object = pera.read_api().multi_get_object_with_options(object_ids,
    ///             PeraObjectDataOptions {
    ///                 show_type: true,
    ///                 show_owner: true,
    ///                 show_previous_transaction: true,
    ...
    pub async fn multi_get_object_with_options(
        ...
        options: PeraObjectDataOptions,
    ) -> PeraRpcResult<Vec<PeraObjectResponse>> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_move_object_bcs(&self, object_id: ObjectID) -> PeraRpcResult<Vec<u8>> {
        let resp = self
            .get_object_with_options(object_id, PeraObjectDataOptions::default().with_bcs())
            .await?
            .into_object()
            .map_err(|e| {
    ...
    /// async fn main() -> Result<(), anyhow::Error> {
    ///     let pera = PeraClientBuilder::default().build_localnet().await?;
    ///     let total_transaction_blocks = pera
    ///         .read_api()
    ...
    /// }
    /// ```
    pub async fn get_total_transaction_blocks(&self) -> PeraRpcResult<u64> {
        Ok(*self.api.http.get_total_transaction_blocks().await?)
    }

    /// Return a transaction and its effects in a [PeraTransactionBlockResponse] based on its
    ...
    ) -> PeraRpcResult<PeraTransactionBlockResponse> {
        Ok(self
            .api
            .http
    ...
    /// Return a list of [PeraTransactionBlockResponse] based on the given vector of [TransactionDigest], or an error upon failure.
    ///
    ...
    pub async fn multi_get_transactions_with_options(
        ...
        options: PeraTransactionBlockResponseOptions,
    ) -> PeraRpcResult<Vec<PeraTransactionBlockResponse>> {
        Ok(self
            .api
            .http
    ...
    /// Return the [PeraCommittee] information for the provided `epoch`, or an error upon failure.
    ///
    /// The [PeraCommittee] contains the validators list and their information (name and stakes).
    ///
    ...
    ) -> PeraRpcResult<TransactionBlocksPage> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_chain_identifier(&self) -> PeraRpcResult<String> {
        Ok(self.api.http.get_chain_identifier().await?)
    }

    /// Return a checkpoint, or an error upon failure.
    ///
    /// A Pera checkpoint is a sequence of transaction sets that a quorum of validators
    /// agree upon as having been executed within the Pera system.
    pub async fn get_checkpoint(&self, id: CheckpointId) -> PeraRpcResult<Checkpoint> {
    ...
    pub async fn get_latest_checkpoint_sequence_number(
        &self,
    ) -> PeraRpcResult<CheckpointSequenceNumber> {
        Ok(*self
            .api
            .http
    ...
    /// Return a stream of [PeraTransactionBlockResponse], or an error upon failure.
    pub fn get_transactions_stream(
        &self,
        query: PeraTransactionBlockResponseQuery,
        cursor: Option<TransactionDigest>,
        descending_order: bool,
    ) -> impl Stream<Item = PeraTransactionBlockResponse> + '_ {
        stream::unfold(
    ...
    ) -> PeraRpcResult<impl Stream<Item = PeraRpcResult<PeraTransactionBlockEffects>>> {
        let Some(c) = &self.api.ws else {
            return Err(Error::Subscription(
                "Subscription only supported by WebSocket client.".to_string(),
            ));
        };
        let subscription: Subscription<PeraTransactionBlockEffects> =
    ...
    pub async fn get_normalized_move_modules_by_package(
        &self,
        package: ObjectID,
    ) -> PeraRpcResult<BTreeMap<String, PeraMoveNormalizedModule>> {
        Ok(self
            .api
            .http
    ...
    pub async fn get_reference_gas_price(&self) -> PeraRpcResult<u64> {
    ...
    pub async fn dry_run_transaction_block(
        &self,
        tx: TransactionData,
    ) -> PeraRpcResult<DryRunTransactionBlockResponse> {
        Ok(self
            .api
            .http
    ...
    ) -> PeraRpcResult<DevInspectResults> {
        Ok(self
            .api
            .http
    ...
    ) -> PeraRpcResult<ProtocolConfigResponse> {
...
/// Coin Read API provides the functionality needed to get information from the Pera network regarding the coins owned by an address.
#[derive(Debug, Clone)]
...
impl CoinReadApi {
    ...
    /// or use `None` for the default `Coin<PERA>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///
    /// The coins can be filtered by `coin_type` (e.g., 0x168da5bf1f48dafc111b0a488fa454aca95e0b5e::usdc::USDC)
    /// or use `None` to use the default `Coin<PERA>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// it will use the default `Coin<PERA>`.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// }
    /// ```
    pub async fn get_all_balances(&self, owner: PeraAddress) -> PeraRpcResult<Vec<Balance>> {
    ...
    pub async fn get_coin_metadata(
        &self,
        coin_type: String,
    ) -> PeraRpcResult<Option<PeraCoinMetadata>> {
    ...
    /// }
    /// ```
    pub async fn get_total_supply(&self, coin_type: String) -> PeraRpcResult<Supply> {
        Ok(self.api.http.get_total_supply(coin_type).await?)
    }
}

/// Event API provides the functionality to fetch, query, or subscribe to events on the Pera network.
#[derive(Clone)]
pub struct EventApi {
    api: Arc<RpcClient>,
}
...
impl EventApi {
    ...
    ///         .ws_url("wss://rpc.mainnet.pera.io:443")
    ///         .build("https://fullnode.mainnet.pera.io:443")
    ///         .await?;
    ///     let mut subscribe_all = pera
    ///         .event_api()
    ...
    pub async fn subscribe_event(
        &self,
        filter: EventFilter,
    ) -> PeraRpcResult<impl Stream<Item = PeraRpcResult<PeraEvent>>> {
        match &self.api.ws {
            Some(c) => {
                let subscription: Subscription<PeraEvent> = c.subscribe_event(filter).await?;
                Ok(subscription.map(|item| Ok(item?)))
            }
    ...
    pub async fn get_events(&self, digest: TransactionDigest) -> PeraRpcResult<Vec<PeraEvent>> {
...
impl GovernanceApi {
    ...
    pub async fn get_stakes(&self, owner: PeraAddress) -> PeraRpcResult<Vec<DelegatedStake>> {
        Ok(self.api.http.get_stakes(owner).await?)
    }

    /// Return the [PeraCommittee] information for the given `epoch`, or an error upon failure.
    ///
    ...
    /// Return the latest PERA system state object on-chain, or an error upon failure.
    ///
    ...
    /// and much more. See the [PeraSystemStateSummary] for all the available fields.
    pub async fn get_latest_pera_system_state(&self) -> PeraRpcResult<PeraSystemStateSummary> {
        Ok(self.api.http.get_latest_pera_system_state().await?)
    }
