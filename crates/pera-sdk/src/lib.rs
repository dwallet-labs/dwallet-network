...
//! Pera Rust SDK builds on top of the [JSON RPC API](https://docs.pera.io/pera-jsonrpc)
//! and therefore many of the return types are the ones specified in [pera_types].
//!
...
//! * <a href="../pera_transaction_builder/struct.TransactionBuilder.html" title="struct pera_transaction_builder::TransactionBuilder">TransactionBuilder</a> - provides functions for building transactions
//!
//! # Usage
//! The main way to interact with the API is through the [PeraClientBuilder],
//! which returns a [PeraClient] object from which the user can access the
//! various APIs.
//!
//! ## Getting Started
//! Add the Rust SDK to the project by running `cargo add pera-sdk` in the root
//! folder of your Rust project.
//!
//! The main building block for the Pera Rust SDK is the [PeraClientBuilder],
//! which provides a simple and straightforward way of connecting to a Pera
//! network and having access to the different available APIs.
//!
//! A simple example that connects to a running Pera local network,
//! the Pera devnet, and the Pera testnet is shown below.
...
//! (see [here](https://github.com/stefan-pera-io/pera/tree/rust_sdk_api_examples/crates/pera-sdk/examples#preqrequisites) for more information).
//!
//! ```rust,no_run
...
//! in the [main repository](https://github.com/MystenLabs/sui/tree/main/crates/pera-sdk/examples).
...
pub use pera_json as json;
use pera_json_rpc_api::{
    CLIENT_SDK_TYPE_HEADER, CLIENT_SDK_VERSION_HEADER, CLIENT_TARGET_API_VERSION_HEADER,
};
pub use pera_json_rpc_types as rpc_types;
use pera_json_rpc_types::{
    ObjectsPage, PeraObjectDataFilter, PeraObjectDataOptions, PeraObjectResponse,
    PeraObjectResponseQuery,
};
use pera_transaction_builder::{DataReader, TransactionBuilder};
pub use pera_types as types;
use pera_types::base_types::{ObjectID, ObjectInfo, PeraAddress};
...
pub mod pera_client_config;
pub mod wallet_context;

pub const PERA_COIN_TYPE: &str = "0x2::pera::PERA";
pub const PERA_LOCAL_NETWORK_URL: &str = "http://127.0.0.1:9000";
pub const PERA_LOCAL_NETWORK_URL_0: &str = "http://0.0.0.0:9000";
pub const PERA_LOCAL_NETWORK_GAS_URL: &str = "http://127.0.0.1:5003/gas";
pub const PERA_DEVNET_URL: &str = "https://fullnode.devnet.pera.io:443";
pub const PERA_TESTNET_URL: &str = "https://fullnode.testnet.pera.io:443";

/// A Pera client builder for connecting to the Pera network
///
...
impl Default for PeraClientBuilder {
    fn default() -> Self {
        Self {
...
impl PeraClientBuilder {
    ...
    /// Set the WebSocket URL for the Pera network
    ...
    /// Returns a [PeraClient] object connected to the Pera network running at the URI provided.
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///         .await?;
    ///
    ///     println!("Pera local version: {:?}", pera.api_version());
    ///     Ok(())
    /// }
    /// ```
    pub async fn build(self, http: impl AsRef<str>) -> PeraRpcResult<PeraClient> {
    ...
    /// Returns a [PeraClient] object that is ready to interact with the local
    /// development network (by default it expects the Pera network to be
    ...
    /// }
    /// ```
    pub async fn build_localnet(self) -> PeraRpcResult<PeraClient> {
        self.build(PERA_LOCAL_NETWORK_URL).await
    }

    /// Returns a [PeraClient] object that is ready to interact with the Pera devnet.
    ///
    /// For connecting to a custom URI, use the `build` function instead..
    ///
    /// # Examples
    ...
    ///         .await?;
    ///
    ///     println!("{:?}", pera.api_version());
    ///     Ok(())
    /// }
    /// ```
    pub async fn build_devnet(self) -> PeraRpcResult<PeraClient> {
        self.build(PERA_DEVNET_URL).await
    }

    /// Returns a [PeraClient] object that is ready to interact with the Pera testnet.
    ///
    /// For connecting to a custom URI, use the `build` function instead.
    ///
    /// # Examples
    ...
    /// }
    /// ```
    pub async fn build_testnet(self) -> PeraRpcResult<PeraClient> {
        self.build(PERA_TESTNET_URL).await
    }
...
/// PeraClient is the basic type that provides all the necessary abstractions for interacting with the Pera network.
///
/// # Usage
///
/// Use [PeraClientBuilder] to build a [PeraClient].
///
/// # Examples
///
/// ```rust,no_run
...
///      .await?;
///
///     println!("{:?}", pera.available_rpc_methods());
///     println!("{:?}", pera.available_subscriptions());
///     println!("{:?}", pera.api_version());
///
...
impl PeraClient {
    ...
    pub fn check_api_version(&self) -> PeraRpcResult<()> {
...
impl DataReader for ReadApi {
    async fn get_owned_objects(
        ...
        let mut result = vec![];
        let query = Some(PeraObjectResponseQuery {
            filter: Some(PeraObjectDataFilter::StructType(object_type)),
            options: Some(
