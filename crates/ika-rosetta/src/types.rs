...
use ika_sdk::rpc_types::{IkaExecutionStatus, IkaTransactionBlockKind};
use ika_types::base_types::{ObjectID, ObjectRef, SequenceNumber, IkaAddress, TransactionDigest};
use ika_types::crypto::PublicKey as IkaPublicKey;
...
use ika_types::transaction::{Argument, CallArg, Command, ObjectArg, TransactionData};
use ika_types::IKA_SYSTEM_PACKAGE_ID;
...
impl IkaEnv {
    pub fn check_network_identifier(
        ...
        if &network_identifier.blockchain != "ika" {
...
impl From<IkaAddress> for AccountIdentifier {
...
impl From<ika_sdk::rpc_types::Coin> for Coin {
    fn from(coin: ika_sdk::rpc_types::Coin) -> Self {
        Self {
...
impl From<IkaPublicKey> for PublicKey {
    fn from(pk: IkaPublicKey) -> Self {
        match pk {
            IkaPublicKey::Ed25519(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Edwards25519,
            },
            IkaPublicKey::Secp256k1(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256k1,
            },
            IkaPublicKey::Secp256r1(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256r1,
            },
            IkaPublicKey::ZkLogin(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::ZkLogin, // inaccurate but added for completeness.
            },
            IkaPublicKey::Passkey(k) => PublicKey {
                hex_bytes: Hex::from_bytes(&k.0),
                curve_type: CurveType::Secp256r1,
            },
        }
...
impl TryInto<IkaAddress> for PublicKey {
    type Error = Error;

    fn try_into(self) -> Result<IkaAddress, Self::Error> {
        let key_bytes = self.hex_bytes.to_vec()?;
        let pub_key = IkaPublicKey::try_from_bytes(self.curve_type.into(), &key_bytes)?;
        Ok((&pub_key).into())
    }
}
...
pub enum OperationType {
    ...
    StakeReward,
    StakePrinciple,
    // ika-rosetta supported operation type
    PayIka,
    PayCoin,
    Stake,
    WithdrawStake,
    // All other Ika transaction types, readonly
    EpochChange,
    Genesis,
...
impl From<&IkaTransactionBlockKind> for OperationType {
    fn from(tx: &IkaTransactionBlockKind) -> Self {
        match tx {
            IkaTransactionBlockKind::ChangeEpoch(_) => OperationType::EpochChange,
            IkaTransactionBlockKind::Genesis(_) => OperationType::Genesis,
            IkaTransactionBlockKind::ConsensusCommitPrologue(_)
            | IkaTransactionBlockKind::ConsensusCommitPrologueV2(_)
            | IkaTransactionBlockKind::ConsensusCommitPrologueV3(_) => {
                OperationType::ConsensusCommitPrologue
            }
            IkaTransactionBlockKind::ProgrammableTransaction(_) => {
                OperationType::ProgrammableTransaction
            }
            IkaTransactionBlockKind::AuthenticatorStateUpdate(_) => {
                OperationType::AuthenticatorStateUpdate
            }
            IkaTransactionBlockKind::RandomnessStateUpdate(_) => {
                OperationType::RandomnessStateUpdate
            }
            IkaTransactionBlockKind::EndOfEpochTransaction(_) => {
...
impl From<IkaExecutionStatus> for OperationStatus {
    fn from(es: IkaExecutionStatus) -> Self {
        match es {
            IkaExecutionStatus::Success => OperationStatus::Success,
            IkaExecutionStatus::Failure { .. } => OperationStatus::Failure,
        }
    }
}
...
impl InternalOperation {
    pub fn sender(&self) -> IkaAddress {
        match self {
            InternalOperation::PayIka { sender, .. }
        ...
        let pt = match self {
            ...
            InternalOperation::Stake {
                ...
                let (validator, system_state, amount) = if let Some(amount) = amount {
                    ...
                    let state = builder.input(CallArg::IKA_SYSTEM_MUT)?;
            ...
            InternalOperation::WithdrawStake { stake_ids, .. } => {
                ...
                for stake_id in metadata.objects {
                    ...
                    let (system_state, id) = if !stake_ids.is_empty() {
                        let system_state = builder.input(CallArg::IKA_SYSTEM_MUT)?;
