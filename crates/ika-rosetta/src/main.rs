...
use ika_config::{ika_config_dir, Config, NodeConfig, IKA_FULLNODE_CONFIG, IKA_KEYSTORE_FILENAME};
use ika_node::IkaNode;
use ika_rosetta::types::{CurveType, PrefundedAccount, IkaEnv};
use ika_rosetta::{RosettaOfflineServer, RosettaOnlineServer, IKA};
use ika_sdk::{IkaClient, IkaClientBuilder};
use ika_types::base_types::IkaAddress;
use ika_types::crypto::{KeypairTraits, IkaKeyPair, ToFromBytes};
use tracing::info;
use tracing::log::warn;

#[derive(Parser)]
#[clap(name = "ika-rosetta", rename_all = "kebab-case", author, version)]
        ...
        match self {
            RosettaServerCommand::GenerateRosettaCLIConfig {
                ...
                let path = keystore_path
                    .unwrap_or_else(|| ika_config_dir().unwrap().join(IKA_KEYSTORE_FILENAME));

                let prefunded_accounts = read_prefunded_account(&path)?;

                info!(
                    "Retrieved {} Ika address from keystore file {:?}",
                ...
                let dsl_path = PathBuf::from(".").join("ika.ros");
                let dsl = include_str!("../resources/ika.ros");
                fs::write(
                    &dsl_path,
                    dsl.replace("{{ika.env}}", json!(env).as_str().unwrap()),
                )?;
            ...
            RosettaServerCommand::StartOnlineRemoteServer {
                ...
                info!(
                    "Starting Rosetta Online Server with remove Ika full node [{full_node_url}]."
                );
                let ika_client = wait_for_ika_client(full_node_url).await;
                let rosetta_path = data_path.join("rosetta_db");
                info!("Rosetta db path : {rosetta_path:?}");
                let rosetta = RosettaOnlineServer::new(env, ika_client);
            ...
            RosettaServerCommand::StartOnlineServer {
                ...
                info!("Starting Rosetta Online Server with embedded Ika full node.");
                info!("Data directory path: {data_path:?}");

                let node_config = node_config.unwrap_or_else(|| {
                    let path = ika_config_dir().unwrap().join(IKA_FULLNODE_CONFIG);
                    info!("Using default node config from {path:?}");
                    path
                });

                let mut config = NodeConfig::load(&node_config)?;
                config.db_path = data_path.join("ika_db");
                info!("Overriding Ika db path to : {:?}", config.db_path);
                ...
                let _node = IkaNode::start(config, registry_service, None).await?;

                let ika_client = wait_for_ika_client(rpc_address).await;
...
async fn wait_for_ika_client(rpc_address: String) -> IkaClient {
    loop {
        match IkaClientBuilder::default()
            ...
            .await
        {
            ...
            Err(e) => {
                warn!("Error connecting to Ika RPC server [{rpc_address}]: {e}, retrying in 5 seconds.");
...
/// This method reads the keypairs from the Ika keystore to create the PrefundedAccount objects,
...
fn read_prefunded_account(path: &Path) -> Result<Vec<PrefundedAccount>, anyhow::Error> {
    ...
    let keys = kp_strings
        .iter()
        .map(|kpstr| {
            let key = IkaKeyPair::decode_base64(kpstr);
            key.map(|k| (IkaAddress::from(&k.public()), k))
        })
