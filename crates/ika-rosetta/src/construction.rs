...
use ika_json_rpc_types::{
    StakeStatus, IkaObjectDataOptions, IkaTransactionBlockEffectsAPI,
    IkaTransactionBlockResponseOptions,
};
use ika_sdk::rpc_types::IkaExecutionStatus;
use ika_types::base_types::{ObjectRef, IkaAddress};
use ika_types::crypto::{DefaultHash, SignatureScheme, ToFromBytes};
use ika_types::error::IkaError;
use ika_types::signature::{GenericSignature, VerifyParams};
use ika_types::signature_verification::{
...
use crate::{OnlineServerContext, IkaEnv};
...
pub async fn derive(
    Extension(env): Extension<IkaEnv>,
    ...
    let address: IkaAddress = request.public_key.try_into()?;
...
pub async fn submit(
    ...
    let dry_run = context
        ...
        .await?;
    if let IkaExecutionStatus::Failure { error } = dry_run.effects.status() {
    ...
    if let IkaExecutionStatus::Failure { error } = response
        .effects
...
/// For Ika, we are returning the latest object refs for all the input objects,
...
pub async fn metadata(
    ...
    let (total_required_amount, objects) = match &option.internal_operation {
        InternalOperation::PayIka { amounts, .. } => {
        ...
        InternalOperation::WithdrawStake { sender, stake_ids } => {
            ...
            let responses = context
                .client
                .read_api()
                .multi_get_object_with_options(stake_ids, IkaObjectDataOptions::default())
                .await?;
    ...
    // If required amount is None (all IKA) or failed to select coin (might not have enough IKA), select all coins.
