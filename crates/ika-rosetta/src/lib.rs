...
pub static IKA: Lazy<Currency> = Lazy::new(|| Currency {
    symbol: "IKA".to_string(),
    decimals: 9,
    metadata: CurrencyMetadata {
        coin_type: IKA_COIN_TYPE.to_string(),
    },
});
...
impl RosettaOnlineServer {
    pub fn new(env: IkaEnv, client: IkaClient) -> Self {
    ...
    pub async fn serve(self, addr: SocketAddr) {
        ...
        info!(
            "Ika Rosetta online server listening on {}",
...
impl RosettaOfflineServer {
    ...
    pub async fn serve(self, addr: SocketAddr) {
        ...
        info!(
            "Ika Rosetta offline server listening on {}",
            listener.local_addr().unwrap()
        );
        axum::serve(listener, app).await.unwrap();
    }
...
impl CoinMetadataCache {
    pub fn new(client: IkaClient, size: NonZeroUsize) -> Self {
        Self {
            client,
