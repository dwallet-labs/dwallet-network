...
use ika_types::{Identifier, TypeTag, IKA_FRAMEWORK_PACKAGE_ID};
...
pub async fn select_gas(
    ...
    if let Some(gas) = input_gas {
        ...
        let object = read_api
            .get_object_with_options(gas, IkaObjectDataOptions::new())
            .await?
    ...
    let gas_objs = read_api
        .get_owned_objects(
            signer_addr,
            Some(IkaObjectResponseQuery {
                filter: Some(IkaObjectDataFilter::StructType(GasCoin::type_())),
                options: Some(IkaObjectDataOptions::new().with_bcs()),
            }),
            None,
            None,
        )
    ...
    for obj in gas_objs {
        let IkaRawData::MoveObject(raw_obj) = &obj
            .data
            .as_ref()
...
pub async fn init_package(
    ...
    let sig = keystore.sign_secure(&tx_data.sender(), &tx_data, Intent::ika_transaction())?;
...
pub async fn mint(
    ...
    keystore: &Keystore,
    init_ret: InitRet,
    balances_to: Vec<(u64, IkaAddress)>,
