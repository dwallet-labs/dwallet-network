...
use ika_json_rpc_types::{
    IkaExecutionStatus, IkaTransactionBlockEffectsAPI, IkaTransactionBlockResponseOptions,
};
...
use ika_rosetta::types::{
    ...
    NetworkIdentifier, IkaEnv,
};
use ika_rosetta::types::{Currencies, OperationType};
use ika_rosetta::CoinMetadataCache;
use ika_rosetta::IKA;
...
async fn test_custom_coin_balance() {
    ...
    const IKA_BALANCE: u64 = 150_000_000_000_000_000;
    ...
    let request = AccountBalanceRequest {
        ...
        currencies: Currencies(vec![ika_currency, test_coin_currency]),
    };
    ...
    assert_eq!(response.balances[0].value, IKA_BALANCE as i128);
    assert_eq!(
...
async fn test_custom_coin_without_symbol() {
    ...
    for op in ops {
        if op.type_ == OperationType::IkaBalanceChange {
