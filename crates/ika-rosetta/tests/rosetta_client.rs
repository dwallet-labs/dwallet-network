...
use ika_rosetta::operations::Operations;
use ika_rosetta::types::{
    ...
    Signature, SignatureType, SubAccount, SubAccountType, IkaEnv, TransactionIdentifierResponse,
};
use ika_rosetta::{RosettaOfflineServer, RosettaOnlineServer};
use ika_sdk::IkaClient;
use ika_types::base_types::IkaAddress;
use ika_types::crypto::IkaSignature;

pub async fn start_rosetta_test_server(client: IkaClient) -> (RosettaClient, Vec<JoinHandle<()>>) {
    let online_server = RosettaOnlineServer::new(IkaEnv::LocalNet, client);
    let offline_server = RosettaOfflineServer::new(IkaEnv::LocalNet);
...
impl RosettaClient {
    ...
    pub async fn rosetta_flow(
        ...
        let network_identifier = NetworkIdentifier {
            blockchain: "ika".to_string(),
            network: IkaEnv::LocalNet,
        };
        // Preprocess
            ...
            .call(
                ...
                &ConstructionCombineRequest {
                    ...
                    signatures: vec![Signature {
                        ...
                        hex_bytes: Hex::from_bytes(IkaSignature::signature_bytes(&signature)),
                    }],
                },
            )
            .await;
