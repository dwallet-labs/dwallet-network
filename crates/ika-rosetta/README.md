...
Ika-Rosetta is an implementation of the Rosetta API for the Ika network, the Ika-Rosetta server uses the Ika fullnode to
read and write transactions to the Ika network.

## Local network quick start

### Build from source
#### 0. Checkout and build Ika
Checkout the [Ika source code](https://github.com/MystenLabs/sui) and compile using `cargo build --release`, the binaries will be located in `target/release` directory.

#### 1. Genesis

`./ika genesis -f`  
The Ika genesis process will create configs and coins for testing, the config files are stored in `~/.ika/ika_config` by
default.
...
`./ika-rosetta start-online-server`
...
`./ika-rosetta generate-rosetta-cli-config`
This will generate the `rosetta-cli.json` and `ika.ros` file to be used by the [Rosetta-CLI](https://github.com/coinbase/rosetta-cli)
...
```shell
cd <ika project directory>/crate/ika-rosetta/docker/ika-rosetta-local
```   
#### 2. Build the image
```shell
./build.sh
...
```shell
docker run ika-io/ika-rosetta-devnet ika-rosetta start-online-server
```
Alternatively, to start the rosetta-offline server, run
```shell
docker run ika-io/ika-rosetta-devnet ika-rosetta start-offline-server
```
...
| Method | Endpoint         | Description                    | Ika Supported? |  Server Type  |
|--------|------------------|--------------------------------|:--------------:|:-------------:|
...
| Method | Endpoint           | Description             |                                      Ika Supported?                                       |  Server Type  |
|--------|--------------------|-------------------------|:-----------------------------------------------------------------------------------------:|:-------------:|
| POST   | /block             | Get a Block             | Yes (One transaction per block in phase 1, will be replaced by Ika checkpoint in phase 2) |    Online     |
...
## Ika transaction <> Rosetta Operation conversion explained
There are 2 places we convert Ika's transaction to Rosetta's operations, 
...
the `/construction/parse` endpoint is used for checking transaction correctness during transaction construction, in our case we only support `TransferIka` for now, the operations created looks like this (negative amount indicate sender):
```json
{
...
Since the intent operations of TransferIka contains all the balance change information(amount field) already, 
