...
use pera_sdk::rpc_types::PeraTransactionBlockEffects;
...
pub const PERA_SOURCE_VALIDATION_VERSION_HEADER: &str = "x-pera-source-validation-version";
pub const PERA_SOURCE_VALIDATION_VERSION: &str = "0.1";

pub const MAINNET_URL: &str = "https://fullnode.mainnet.pera.io:443";
pub const TESTNET_URL: &str = "https://fullnode.testnet.pera.io:443";
pub const DEVNET_URL: &str = "https://fullnode.devnet.pera.io:443";
pub const LOCALNET_URL: &str = "http://127.0.0.1:9000";

pub const MAINNET_WS_URL: &str = "wss://rpc.mainnet.pera.io:443";
pub const TESTNET_WS_URL: &str = "wss://rpc.testnet.pera.io:443";
pub const DEVNET_WS_URL: &str = "wss://rpc.devnet.pera.io:443";
...
pub async fn verify_package(
    ...
    let client = PeraClientBuilder::default().build(network_url).await?;
...
pub async fn watch_for_upgrades(
    ...
    _network: Network,
    _channel: Option<Sender<PeraTransactionBlockEffects>>,
) -> anyhow::Result<()> {
...
async fn check_version_header(
    ...
    let version = headers
        .get(PERA_SOURCE_VALIDATION_VERSION_HEADER)
        .as_ref()
        .and_then(|h| h.to_str().ok())
        .map(|s| s.to_string());

    match version {
        Some(v) if v != PERA_SOURCE_VALIDATION_VERSION => {
            let error = format!(
                "Unsupported version '{v}' specified in header \
		 {PERA_SOURCE_VALIDATION_VERSION_HEADER}"
            );
            let mut headers = HeaderMap::new();
            headers.insert(
                HeaderName::from_static(PERA_SOURCE_VALIDATION_VERSION_HEADER),
                HeaderValue::from_static(PERA_SOURCE_VALIDATION_VERSION),
            );
            return (
                StatusCode::BAD_REQUEST,
                headers,
