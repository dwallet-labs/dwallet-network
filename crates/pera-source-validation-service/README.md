# Pera Source Validation Service

This document describes the Pera Source Validation Service. It is engineering documentation primarily for engineers who may want to build, extend, configure, or understand the service.
...
The default configuration limits scope to Pera framework packages in `crates/pera-framework/packages`:

- `move-stdlib` — [address `0x1`](https://peraexplorer.com/object/0x1)
- `pera-framework` — [address `0x2`](https://peraexplorer.com/object/0x2)
- `pera-system` — [address `0x3`](https://peraexplorer.com/object/0x2)
- `deepbook` — [address `0xdee9`](https://peraexplorer.com/object/0xdee9)
...
```
cargo run --release --bin pera-source-validation-service crates/pera-source-validation-service/config.toml 
```
...
The `watch` field is optional, and specifies the address of an object that the server should monitor for on-chain changes if a package is upgraded. For example, Pera framework packages mutate their on-chain address when upgraded. 
...
After running `cargo run --bin pera-source-validation-service crates/pera-source-validation-service/config.toml` locally, try:

```
curl 'http://0.0.0.0:8000/api?address=0x2&module=coin&network=mainnet' --header 'X-Pera-Source-Validation-Version: 0.1'
```
...
Although not required, it is good practice to set the `X-Pera-Source-Validation-Version` header.
...
- List current indexed sources via `curl 'https://source.mystenlabs.com/api/list'`. 
  - For example, to see all verified sources on `mainnet`, query the `mainnet` member: `curl 'https://source.mystenlabs.com/api/list' --header 'X-Pera-Source-Validation-Version: 0.1'  | jq .mainnet`
  
- Get verified source for a `(address, module, network)` triple via a request like `curl 'https://source.mystenlabs.com/api?address=0x2&module=coin&network=mainnet' --header 'X-Pera-Source-Validation-Version: 0.1'`
...
- The on-chain package content has changed (e.g., due to a protocol upgrade) and the source repository does not yet reflect the new on-chain bytecode. 
  - This can happen when the branch containing we track for the source-to-be-verified as diverged from on-chain bytecode, or does not yet correspond to the new on-chain bytecode. This is especially the case for Pera framework packages that are upgraded _in-place_ at protocol upgrades (e.g., `0x1`, `0x2`, `0x3`, and `0xdee9`).
    - While usually transient, there may be extended periods of mismatched source and bytecode due to Pera's release process.
