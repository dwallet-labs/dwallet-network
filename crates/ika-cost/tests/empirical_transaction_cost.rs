...
use ika_test_transaction_builder::publish_basics_package_and_make_counter;
use ika_test_transaction_builder::TestTransactionBuilder;
use ika_types::base_types::{ObjectRef, IkaAddress};
use ika_types::coin::PAY_JOIN_FUNC_NAME;
use ika_types::coin::PAY_MODULE_NAME;
use ika_types::coin::PAY_SPLIT_VEC_FUNC_NAME;
...
pub enum CommonTransactionCosts {
    ...
    SplitCoin(usize),
    TransferWholeCoin,
    TransferWholeIkaCoin,
    TransferPortionIkaCoin,
    SharedCounterCreate,
...
// 1. Run `cargo insta test --review` under `./ika-cost`.
...
async fn create_txes(
    ...
    //
    // Transfer Whole Ika Coin and Transfer Portion of Ika Coin
    //
    let whole_ika_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_ika(None, IkaAddress::default())
            .build();
    let partial_ika_coin_tx =
        TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
            .transfer_ika(Some(10), IkaAddress::default())
            .build();
    ret.insert(
        CommonTransactionCosts::TransferWholeIkaCoin,
        whole_ika_coin_tx,
    );
    ret.insert(
        CommonTransactionCosts::TransferPortionIkaCoin,
        partial_ika_coin_tx,
    );
    ...
    let whole_coin_tx = TestTransactionBuilder::new(sender, gas_objects.pop().unwrap(), gas_price)
        .transfer(gas_objects.pop().unwrap(), IkaAddress::default())
        .build();
