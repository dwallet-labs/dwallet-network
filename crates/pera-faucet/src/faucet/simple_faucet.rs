...
use pera_json_rpc_types::{
    OwnedObjectRef, PeraObjectDataOptions, PeraTransactionBlockEffectsAPI,
...
use pera_types::{
    base_types::{ObjectID, PeraAddress, TransactionDigest},
    gas_coin::GasCoin,
...
pub struct SimpleFaucet {
    ...
    request_producer: Sender<(Uuid, PeraAddress, Vec<u64>)>,
...
impl SimpleFaucet {
    ...
    async fn sign_and_execute_txn(
        ...
        for_batch: bool,
    ) -> Result<PeraTransactionBlockResponse, FaucetError> {
        let signature = self
            .wallet
            .config
            .keystore
            .sign_secure(&self.active_address, &tx_data, Intent::pera_transaction())
            .map_err(FaucetError::internal)?;
        ...
        info!(
            ...
            ?coin_id,
            ?uuid,
            "PayPera transaction in faucet."
        );

        match timeout(
            Duration::from_secs(300),
            self.execute_pay_pera_txn_with_retries(&tx, coin_id, recipient, uuid),
        )
        .await
        {
            Err(elapsed) => {
                warn!(
                    ...
                    ?coin_id,
                    ?uuid,
                    "Failed to execute PayPera transactions in faucet after {elapsed}. Coin will \
                     not be reused."
                );
    ...
    async fn transfer_gases(
        ...
        match gas_coin_response {
            GasCoinResponse::ValidGasCoin(coin_id) => {
                let tx_data = self
                    .build_pay_pera_txn(coin_id, self.active_address, recipient, amounts, gas_cost)
                    .await
                    .map_err(FaucetError::internal)?;
    ...
    async fn execute_pay_pera_txn_with_retries(
        &self,
        tx: &Transaction,
        coin_id: ObjectID,
        recipient: PeraAddress,
        ...
        loop {
            ...
            info!(
                ...
                ?uuid,
                ?retry_delay,
                "PayPera transaction in faucet failed, previous error: {:?}",
                &res,
            );
    ...
    async fn build_pay_pera_txn(
        ...
        client
            .transaction_builder()
            .pay_pera(signer, vec![coin_id], recipients, amounts.to_vec(), budget)
            .await
            .map_err(|e| {
                anyhow::anyhow!(
                    "Failed to build PayPera transaction for coin {:?}, with err {:?}",
                    coin_id,
                    e
                )
            })
    ...
    async fn check_and_map_transfer_gas_result(
        &self,
        res: PeraTransactionBlockResponse,
        ...
        if created.len() != number_of_coins {
            return Err(FaucetError::CoinAmountTransferredIncorrect(format!(
                "PayPera Transaction should create exact {:?} new coins, but got {:?}",
    ...
    async fn build_batch_pay_pera_txn(
        &self,
        coin_id: ObjectID,
        batch_requests: Vec<(Uuid, PeraAddress, Vec<u64>)>,
        signer: PeraAddress,
        budget: u64,
    ) -> Result<TransactionData, anyhow::Error> {
    ...
    async fn check_and_map_batch_transfer_gas_result(
        &self,
        res: PeraTransactionBlockResponse,
        requests: Vec<(Uuid, PeraAddress, Vec<u64>)>,
    ) -> Result<(), FaucetError> {
        // Grab the list of created coins and turn it into a map of destination PeraAddress to Vec<Coins>
        let created = res
            .effects
            .ok_or_else(|| {
                FaucetError::ParseTransactionResponseError(format!(
        ...
        let mut address_coins_map: HashMap<PeraAddress, Vec<OwnedObjectRef>> = HashMap::new();
        ...
        // Assert that the number of times a pera_address occurs is the number of times the coins
        // come up in the vector.
        let mut request_count: HashMap<PeraAddress, u64> = HashMap::new();
        ...
        for (uuid, addy, amounts) in requests {
            let number_of_coins = amounts.len();
            // Get or insert pera_address into request count
...
impl Faucet for SimpleFaucet {
    async fn send(
        ...
        info!(uuid = ?id, ?recipient, ?digest, "PayPera txn succeeded");
...
pub async fn batch_gather(
    request_consumer: &mut Receiver<(Uuid, PeraAddress, Vec<u64>)>,
    requests: &mut Vec<(Uuid, PeraAddress, Vec<u64>)>,
    batch_request_size: u64,
) -> Result<(), FaucetError> {
...
pub async fn batch_transfer_gases(
    ...
    let total_pera_needed: u64 = requests.iter().flat_map(|(_, _, amounts)| amounts).sum();
    // This loop is utilized to grab a coin that is large enough for the request
    loop {
        let gas_coin_response = faucet
            .prepare_gas_coin(total_pera_needed + gas_cost, uuid, true)
            .await;

        match gas_coin_response {
            GasCoinResponse::ValidGasCoin(coin_id) => {
                let tx_data = faucet
                    .build_batch_pay_pera_txn(
                        coin_id,
                        requests.clone(),
                ...
                // pera address, and instead just fill it with the ZERO address.
                let recipient = PeraAddress::ZERO;
                {
                    // Register the intention to send this transaction before we send it, so that if
                    // faucet fails or we give up before we get a definite response, we have a
                    // chance to retry later.
...
mod tests {
    ...
    async fn execute_tx(
        ...
        tx_data: TransactionData,
    ) -> Result<PeraTransactionBlockEffects, anyhow::Error> {
        ...
        if let Some(effects) = result_effects {
            if matches!(effects.status(), PeraExecutionStatus::Failure { .. }) {
                Err(anyhow!(
            ...
            Err(anyhow!(
                "Effects from PeraTransactionBlockResult should not be empty"
            ))
        }
    }
    ...
    async fn test_batch_transfer_interface() {
        ...
        let target_addresses: Vec<PeraAddress> = (0..5)
            .map(|_| PeraAddress::random_for_testing_only())
            .collect();
    ...
    async fn test_clear_wal() {
        ...
        let tx_data = faucet
            .build_pay_pera_txn(coin_id, faucet_address, recipient, &[100], 200_000_000)
            .await
            .map_err(FaucetError::internal)
            .unwrap();
    ...
    async fn test_insufficient_balance_will_retry_success() {
        ...
        let destination_address = PeraAddress::random_for_testing_only();
        // Transfer all valid gases away except for 1
        for gas in gas_coins.iter().take(gas_coins.len() - 1) {
            let tx_data = client
                .transaction_builder()
                .transfer_pera(address, gas.0, gas_budget, destination_address, None)
                .await
                .unwrap();
            execute_tx(&mut context, tx_data).await.unwrap();
        }
    ...
    async fn test_amounts_transferred_on_batch() {
        ...
        let target_addresses: Vec<PeraAddress> = (0..2)
            .map(|_| PeraAddress::random_for_testing_only())
            .collect();

        // Send 2 coins of 1 pera each. We
        let coins_sent = 2;
