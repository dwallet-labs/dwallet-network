...
/// Persistent log of transactions paying out pera from the faucet, keyed by the coin serving the
...
mod tests {
    ...
    fn random_request(coin: ObjectRef) -> (PeraAddress, TransactionData) {
        let gas_price = 1;
        let send = PeraAddress::random_for_testing_only();
        let recv = PeraAddress::random_for_testing_only();
        (
            recv,
