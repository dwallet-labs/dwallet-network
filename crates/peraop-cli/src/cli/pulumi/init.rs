...
impl ProjectType {
    pub fn create_project(&self, use_kms: &bool, project_name: Option<String>) -> Result<()> {
        // make sure we're in peraops
        let peraops_path = ensure_in_peraops_repo()?;
        info!("perapop path: {}", peraops_path);
        ...
        let project_subdir = match self {
            ...
            Self::Basic => Text::new("project subdir:")
                .with_initial_value(&format!("{}/pulumi/", peraops_path))
...
fn ensure_in_peraops_repo() -> Result<String> {
    let remote_stdout = run_cmd(
        vec!["git", "config", "--get", "remote.origin.url"],
        Some(CommandOptions::new(false, false)),
    )
    .context("run this command within the pera-operations repository")?
    .stdout;
    let raw_path = String::from_utf8_lossy(&remote_stdout);
    let in_peraops = raw_path.trim().contains("pera-operations");
    if !in_peraops {
        Err(anyhow!(
            "please run this command from within the pera-operations repository"
        ))
    } else {
...
fn get_pulumi_dir() -> Result<PathBuf> {
    let peraops_dir_stdout = run_cmd(
        vec!["git", "rev-parse", "--show-toplevel"],
        Some(CommandOptions::new(false, false)),
    )
    .context("run this command from within the pera-operations repository")?
    .stdout;
    let peraops_dir = PathBuf::from(String::from_utf8_lossy(&peraops_dir_stdout).trim());
    Ok(peraops_dir.join("pulumi"))
}
