    ...
    S: WriteStore + Send + Sync + 'static,
{
    async fn push_checkpoint_message(
        ...
        let should_sync = highest_verified_checkpoint.map(|c| *checkpoint.sequence_number() >  c.sequence_number).unwrap_or(true);
...
impl<S> tower::layer::Layer<S>
for CheckpointMessageDownloadLimitLayer {
