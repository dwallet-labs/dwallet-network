...
struct PeerStateSyncInfo {
    ...
    dwallet_height: Option<DWalletCheckpointSequenceNumber>,
    system_height: Option<DWalletCheckpointSequenceNumber>,
}
...
impl PeerHeights {
    ...
    pub fn highest_known_checkpoint_sequence_number(
        ...
        self.peers
            .values()
            .filter_map(|info| info.on_same_chain_as_us.then_some(info.dwallet_height))
            .max()?
    }
...
impl Iterator for PeerBalancer {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while !self.peers.is_empty() {
            ...
            if info.dwallet_height >= Some(requested_checkpoint) {
