...
use ika_types::{
    ...
    messages_checkpoint::{
        CertifiedCheckpointMessage, CheckpointSequenceNumber,
...
impl Handle {
    ...
    pub async fn send_checkpoint(&self, checkpoint: VerifiedCheckpointMessage) {
        self.sender
            .send(StateSyncMessage::VerifiedCheckpointMessage(Box::new(checkpoint)))
            .await
            .unwrap()
    }

    /// Subscribe to the stream of checkpoints that have been fully synchronized and downloaded.
    pub fn subscribe_to_synced_checkpoints(&self) -> broadcast::Receiver<VerifiedCheckpointMessage> {
...
impl PeerHeights {
    ...
    pub fn get_checkpoint_by_digest(&self, digest: &CheckpointMessageDigest) -> Option<&CertifiedCheckpointMessage> {
    ...
    S: WriteStore + Clone + Send + Sync + 'static,
{
    ...
    pub async fn start(mut self) {
        ...
        let task = sync_checkpoint_messages_from_archive(
    ...
    fn handle_checkpoint_from_consensus(&mut self, checkpoint: Box<VerifiedCheckpointMessage>) {
        ...
        let next_sequence_number = latest_checkpoint_sequence_number.map(|s| s.checked_add(1).expect("exhausted u64")).unwrap_or(0);
    ...
    let info = {
        ...
        if let Some(info) = maybe_info {
            ...
            let info = match response {
                Ok(GetChainIdentifierResponse { chain_identifier }) => {
                    PeerStateSyncInfo {
                        chain_identifier,
    ...
    S: WriteStore,
{
    ...
    let mut current = store
        .get_highest_verified_checkpoint()
        .expect("store operation should not fail");
    let current_sequence_number = current
        .as_ref()
    ...
    while let Some((maybe_checkpoint, next, maybe_peer_id)) = request_stream.next().await {
        ...
        let checkpoint = maybe_checkpoint.map(VerifiedCheckpointMessage::new_unchecked).ok_or_else(|| {
