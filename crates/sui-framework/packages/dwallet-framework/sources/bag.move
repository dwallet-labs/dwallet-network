...
/// A bag is a heterogeneous map-like collection. The collection is similar to `dwallet::table` in that
...
/// assert!(&bag1 != &bag2, 0);
/// ```
/// At it's core, `dwallet::bag` is a wrapper around `UID` that allows for access to
/// `dwallet::dynamic_field` while preventing accidentally stranding field values. A `UID` can be
...
module dwallet::bag {
    ...
    /// Aborts with `dwallet::dynamic_field::EFieldAlreadyExists` if the bag already has an entry with
    /// that key `k: K`.
    ...
    /// Aborts with `dwallet::dynamic_field::EFieldDoesNotExist` if the bag does not have an entry with
    /// that key `k: K`.
    /// Aborts with `dwallet::dynamic_field::EFieldTypeMismatch` if the bag has an entry for the key, but
