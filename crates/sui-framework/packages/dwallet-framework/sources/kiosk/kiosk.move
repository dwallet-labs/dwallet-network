...
module dwallet::kiosk {
    ...
    // - `lock_internal`
    // - `uid_mut_internal`
    friend dwallet::kiosk_extension;
    ...
    entry fun default(ctx: &mut TxContext) {
        ...
        dwallet::transfer::transfer(cap, sender(ctx));
        dwallet::transfer::share_object(kiosk);
    }
    ...
    public fun purchase<T: key + store>(
        self: &mut Kiosk, id: ID, payment: Coin<DWLT>
    ): (T, TransferRequest<T>) {
    ...
    public fun purchase_with_cap<T: key + store>(
        self: &mut Kiosk, purchase_cap: PurchaseCap<T>, payment: Coin<DWLT>
    ): (T, TransferRequest<T>) {
    ...
    public fun profits_mut(self: &mut Kiosk, cap: &KioskOwnerCap): &mut Balance<DWLT> {
        assert!(has_access(self, cap), ENotOwner);
        &mut self.profits
    }
