...
module dwallet_system::validator_set {
    ...
    use dwallet_system::validator::{Self, Validator, staking_pool_id, sui_address};
    use dwallet_system::validator_cap::{Self, UnverifiedValidatorOperationCap, ValidatorOperationCap};
    use dwallet_system::staking_pool::{Self, PoolTokenExchangeRate, StakedSui, pool_id};
    use dwallet::object::{Self, ID};
    use dwallet::priority_queue as pq;
    ...
    use dwallet_system::voting_power;
    ...
    #[test_only]
    friend dwallet_system::validator_set_tests;

    #[test_only]
    friend dwallet_system::stake_tests;
    ...
    /// Called by `dwallet_system` to add a new validator candidate.
    ...
    /// Called by `dwallet_system` to remove a validator candidate, and move them to `inactive_validators`.
    ...
    /// Called by `dwallet_system` to add a new validator to `pending_active_validators`, which will be
    ...
    /// Called by `dwallet_system`, to remove a validator.
    ...
    /// Called by `dwallet_system`, to add a new stake to the validator.
    ...
    /// Called by `dwallet_system`, to withdraw some share of a stake from the validator. The share to withdraw
    ...
    public(friend) fun advance_epoch(
        self: &mut ValidatorSet,
        computation_reward: &mut Balance<DWLT>,
        storage_fund_reward: &mut Balance<DWLT>,
    ...
    /// Called by `dwallet_system` to derive reference gas price for the new epoch.
    ...
    fun distribute_reward(
        ...
        staking_rewards: &mut Balance<DWLT>,
        storage_fund_reward: &mut Balance<DWLT>,
        ctx: &mut TxContext
    ) {
        let length = vector::length(validators);
