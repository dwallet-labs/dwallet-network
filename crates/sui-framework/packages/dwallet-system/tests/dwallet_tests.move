...
module dwallet_system::dwallet_tests {
    ...
    public fun test_save_encypted_user_share() {
        ...
        test_scenario::next_tx(&mut scenario, mock_sender_address);
        {
            ...
            dwallet::save_encrypted_user_share(&mut encrypted_user_shares, &encrypted_user_share, ctx);
            assert!(
    ...
    #[test]
   #[expected_failure(abort_code = EInvalidParametes)]
   public fun test_save_encypted_with_different_encryption_key_failuer() {
       let (sender, scenario) = set_up();
       test_scenario::next_tx(&mut scenario, sender);
       {
           let ctx = test_scenario::ctx(&mut scenario);
