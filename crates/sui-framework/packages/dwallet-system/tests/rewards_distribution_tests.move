...
#[test_only]
module dwallet_system::rewards_distribution_tests {
    ...
    fun test_stake_subsidy() {
        set_up_dwallet_system_state_with_big_amounts();
        let scenario_val = test_scenario::begin(VALIDATOR_ADDR_1);
        let scenario = &mut scenario_val;
    ...
    fun test_everyone_slashed() {
        ...
        assert_eq(dwallet_system::get_storage_fund_total_balance(&mut system_state), 4000 * MIST_PER_SUI);

        // The entire 1000 SUI of storage rewards should go to the object rebate portion of the storage fund.
        assert_eq(dwallet_system::get_storage_fund_object_rebates(&mut system_state), 1000 * MIST_PER_SUI);
    ...
    fun test_mul_rewards_withdraws_at_same_epoch() {
        ...
        assert_eq(dwallet_system::validator_stake_amount(&mut system_state, VALIDATOR_ADDR_1), 140 * 23 * MIST_PER_SUI);
        test_scenario::return_shared(system_state);
        ...
        assert_eq(dwallet_system::validator_stake_amount(&mut system_state, VALIDATOR_ADDR_1), 140 * MIST_PER_SUI);
        test_scenario::return_shared(system_state);
        test_scenario::end(scenario_val);
    }
    ...
    fun test_uncapped_rewards() {
        ...
        while (i < num_validators) {
            ...
            assert_eq(dwallet_system::validator_stake_amount(&mut system_state, addr), (962 + i * 4) * MIST_PER_SUI);
            i = i + 1;
        };
        test_scenario::return_shared(system_state);
        test_scenario::end(scenario_val);
    ...
    fun set_up_dwallet_system_state_with_big_amounts() {
        let scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        let ctx = test_scenario::ctx(scenario);
        ...
        create_dwallet_system_state_for_testing(validators, 1000000000, 0, ctx);
        test_scenario::end(scenario_val);
    }
    ...
    fun set_commission_rate_and_advance_epoch(addr: address, commission_rate: u64, scenario: &mut Scenario) {
        ...
        let system_state = test_scenario::take_shared<DWalletSystemState>(scenario);
        let ctx = test_scenario::ctx(scenario);
        dwallet_system::request_set_commission_rate(&mut system_state, commission_rate, ctx);
        test_scenario::return_shared(system_state);
        governance_test_utils::advance_epoch(scenario);
    }
    ...
    fun report_validator(reporter: address, reportee: address, scenario: &mut Scenario) {
        ...
        dwallet_system::report_validator(&mut system_state, &cap, reportee);
