...
#[test_only]
module dwallet_system::stake_tests {
    use dwallet::coin;
    use dwallet::test_scenario;
    use dwallet_system::dwallet_system::{Self, DWalletSystemState};
    use dwallet_system::staking_pool::{Self, StakedSui, PoolTokenExchangeRate};
    ...
    use dwallet_system::governance_test_utils::{
        Self,
        add_validator,
        ...
        create_dwallet_system_state_for_testing,
        stake_with,
        remove_validator,
    ...
    fun test_add_remove_stake_flow() {
        ...
        test_scenario::next_tx(scenario, STAKER_ADDR_1);
        {
            ...
            dwallet_system::request_add_stake(
                system_state_mut_ref, coin::mint_for_testing(60 * MIST_PER_SUI, ctx), VALIDATOR_ADDR_1, ctx);

            assert!(dwallet_system::validator_stake_amount(system_state_mut_ref, VALIDATOR_ADDR_1) == 100 * MIST_PER_SUI, 101);
            assert!(dwallet_system::validator_stake_amount(system_state_mut_ref, VALIDATOR_ADDR_2) == 100 * MIST_PER_SUI, 102);
        ...
        test_scenario::next_tx(scenario, STAKER_ADDR_1);
        {
            ...
            assert!(dwallet_system::validator_stake_amount(system_state_mut_ref, VALIDATOR_ADDR_1) == 160 * MIST_PER_SUI, 103);
            assert!(dwallet_system::validator_stake_amount(system_state_mut_ref, VALIDATOR_ADDR_2) == 100 * MIST_PER_SUI, 104);

            let ctx = test_scenario::ctx(scenario);

            // Unstake from VALIDATOR_ADDR_1
            dwallet_system::request_withdraw_stake(system_state_mut_ref, staked_sui, ctx);

            assert!(dwallet_system::validator_stake_amount(system_state_mut_ref, VALIDATOR_ADDR_1) == 160 * MIST_PER_SUI, 107);
            test_scenario::return_shared(system_state);
        };
        ...
        {
            let system_state = test_scenario::take_shared<DWalletSystemState>(scenario);
            assert!(dwallet_system::validator_stake_amount(&mut system_state, VALIDATOR_ADDR_1) == 100 * MIST_PER_SUI, 107);
            test_scenario::return_shared(system_state);
        };
        test_scenario::end(scenario_val);
    }
    ...
    fun test_remove_stake_post_active_flow(should_distribute_rewards: bool) {
        set_up_dwallet_system_state_with_storage_fund();
        let scenario_val = test_scenario::begin(VALIDATOR_ADDR_1);
        let scenario = &mut scenario_val;
        ...
        test_scenario::next_tx(scenario, STAKER_ADDR_1);
        {
            ...
            assert!(!validator_set::is_active_validator_by_sui_address(
                        dwallet_system::validators(system_state_mut_ref),
                        VALIDATOR_ADDR_1
                    ), 0);
    ...
    fun test_staking_pool_exchange_rate_getter() {
        ...
        let system_state = test_scenario::take_shared<DWalletSystemState>(scenario);
        let rates = dwallet_system::pool_exchange_rates(&mut system_state, &pool_id);
    ...
    fun set_up_dwallet_system_state() {
        let scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        let ctx = test_scenario::ctx(scenario);
        ...
        create_dwallet_system_state_for_testing(validators, 0, 0, ctx);
        test_scenario::end(scenario_val);
    }

    fun set_up_dwallet_system_state_with_storage_fund() {
        let scenario_val = test_scenario::begin(@0x0);
        let scenario = &mut scenario_val;
        let ctx = test_scenario::ctx(scenario);
        ...
        create_dwallet_system_state_for_testing(validators, 300, 100, ctx);
        test_scenario::end(scenario_val);
    }
}
