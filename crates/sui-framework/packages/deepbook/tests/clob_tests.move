...
module deepbook::clob_test {
    ...
    use dwallet::coin::{Self, mint_for_testing, burn_for_testing};
    use dwallet::dwlt::DWLT;
    use dwallet::test_scenario::{Self as test, Scenario, next_tx, ctx, end, TransactionEffects};
    ...
    fun get_market_price_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);{
            let pool = test::take_shared<Pool<DWLT, USD>>(&test);
            let (bid_price, ask_price) = clob::get_market_price<DWLT, USD>(&pool);
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::test_increase_user_available_balance<DWLT>(base_custodian, account_cap_user, alice_deposit_WSUI);
    ...
    fun test_deposit_withdraw_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            clob::deposit_base(&mut pool, mint_for_testing<DWLT>(alice_deposit_WSUI, ctx(&mut test)), &account_cap);
    ...
    fun test_clean_up_expired_orders_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::test_increase_user_available_balance<DWLT>(base_custodian, account_cap_user, bob_deposit_WSUI);
        ...
        next_tx(&mut test, owner);
        {
            ...
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_owner_alice, 10000, 0);
        ...
        next_tx(&mut test, owner);
        {
            ...
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_owner_bob, 20000, 0);
    ...
    fun test_partial_fill_and_cancel_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::deposit(
                base_custodian,
                mint_for_testing<DWLT>(1000 * 100000000, ctx(&mut test)),
                account_cap_user
            );
            custodian::deposit(
                quote_custodian,
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap = test::take_from_sender<AccountCap>(&test);
            let clock = test::take_shared<Clock>(&test);
            clob::place_limit_order<DWLT, USD>(
                &mut pool,
                CLIENT_ID_ALICE,
                5 * FLOAT_SCALING,
                200 * 100000000,
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 0, 1000 * 100000000);
            test::return_shared(pool);
            test::return_to_sender<AccountCap>(&test, account_cap);
        };
        next_tx(&mut test, bob);
        {
            ...
            custodian::deposit(
                base_custodian,
                mint_for_testing<DWLT>(300 * 100000000, ctx(&mut test)),
                account_cap_user
            );
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 300 * 100000000, 0);
            test::return_shared(pool);
            test::return_to_address<AccountCap>(bob, account_cap);
        };
        ...
        next_tx(&mut test, bob);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 0, 0);
        ...
        next_tx(&mut test, alice);
        {
            ...
            clob::cancel_order<DWLT, USD>(&mut pool, 2, &account_cap);
            {
                let open_orders = vector::empty<Order>();
                vector::push_back(
                    &mut open_orders,
    ...
    fun test_full_transaction_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::deposit(base_custodian, mint_for_testing<DWLT>(1000, ctx(&mut test)), account_cap_user);
        ...
        next_tx(&mut test, bob);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let (coin1, coin2) = clob::place_market_order<DWLT, USD>(&mut pool, &account_cap, CLIENT_ID_BOB, 600,
                false,
                mint_for_testing<DWLT>(600, ctx(&mut test)),
                mint_for_testing<USD>(0, ctx(&mut test)),
                &clock,
                ctx(&mut test));
            assert!(coin::value<DWLT>(&coin1) == 0, 0);
        ...
        next_tx(&mut test, bob);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let (coin1, coin2) =clob::place_market_order<DWLT, USD>(&mut pool, &account_cap, CLIENT_ID_BOB, 100,
                false,
                mint_for_testing<DWLT>(600, ctx(&mut test)),
                mint_for_testing<USD>(0, ctx(&mut test)),
                &clock,
                ctx(&mut test));
            assert!(coin::value<DWLT>(&coin1) == 500, 0);
    ...
    fun test_place_limit_order_fill_or_kill_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            let account_cap = test::take_from_sender<AccountCap>(&test);
            let clock = test::take_shared<Clock>(&test);
            let (base_quantity_filled, quote_quantity_filled, is_placed, order_id) = clob::place_limit_order<DWLT, USD>(
                &mut pool,
                CLIENT_ID_BOB,
                4 * FLOAT_SCALING,
                400,
        ...
        next_tx(&mut test, bob);
        {
            ...
            let  (base_avail, base_locked, quote_avail, quote_locked) = account_balance<DWLT, USD>(&pool, &account_cap);
    ...
    fun test_absorb_all_liquidity_bid_side_with_customized_tick_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let (coin1, coin2) = clob::place_market_order<DWLT, USD>(&mut pool, &account_cap, CLIENT_ID_BOB, 2000, false,
                mint_for_testing<DWLT>(2000, ctx(&mut test)),
                mint_for_testing<USD>(0, ctx(&mut test)),
                &clock,
                ctx(&mut test));
            assert!(coin::value<DWLT>(&coin1) == 500, 0);
    ...
    fun test_absorb_all_liquidity_ask_side_with_customized_tick_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::deposit(base_custodian, mint_for_testing<DWLT>(10000, ctx(&mut test)), account_cap_user);
            custodian::deposit(quote_custodian, mint_for_testing<USD>(10000, ctx(&mut test)), account_cap_user);
            test::return_shared(pool);
            test::return_to_sender<AccountCap>(&test, account_cap);
        };
        ...
        next_tx(&mut test, bob);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let (coin1, coin2) = clob::place_market_order<DWLT, USD>(&mut pool, &account_cap, CLIENT_ID_BOB, 5000, true,
                mint_for_testing<DWLT>(10000, ctx(&mut test)),
                mint_for_testing<USD>(10000, ctx(&mut test)),
                &clock,
                ctx(&mut test));
            assert!(coin::value<DWLT>(&coin1) == 12000, 0);
    ...
    fun test_swap_exact_base_for_quote_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            let (base_coin, quote_coin, _) = clob::swap_exact_base_for_quote(
                ...
                &account_cap,
                1500,
                mint_for_testing<DWLT>(1500, ctx(&mut test)),
                mint_for_testing<USD>(0,  ctx(&mut test)),
                &clock,
                ctx(&mut test)
            );
    ...
    fun test_pool_with_small_fee_example_(test: Scenario): TransactionEffects {
        ...
        // Buys some sui from alice
        {
            ...
            let (base_coin, quote_coin, _) = clob::swap_exact_base_for_quote(
                ...
                &account_cap,
                100000,
                mint_for_testing<DWLT>(100000000, ctx(&mut test)),
                mint_for_testing<USD>(0,  ctx(&mut test)),
                &clock,
                ctx(&mut test)
            );
    ...
    fun test_cancel_and_remove_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 0, 10);
        ...
        next_tx(&mut test, alice);
        {
            ...
            let (base_custodian, quote_custodian) = clob::borrow_custodian(&pool);
            custodian::assert_user_balance<USD>(quote_custodian, account_cap_user, 35 + 10 + 15, 65 - 10 - 15);
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 10, 0);
            test::return_shared(pool);
            test::return_to_address<AccountCap>(alice, account_cap);
        };
        end(test)
    ...
    fun test_inject_and_match_taker_bid_with_quote_quantity_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            let pool = test::take_shared<Pool<DWLT, USD>>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, bob);
            let (base_quantity_filled, quote_quantity_filled) = clob::test_match_bid_with_quote_quantity<DWLT, USD>(&mut pool, &account_cap, CLIENT_ID_BOB, 500, MAX_PRICE, 0);
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 8000, 2000);
            let (next_bid_order_id, next_ask_order_id, _, _) = clob::get_pool_stat(&pool);
            assert!(next_bid_order_id == clob::order_id_for_test(1, true), 0);
            assert!(next_ask_order_id == clob::order_id_for_test(3, false), 0);
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user_alice, 8000, 500 + 5);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
    ...
    fun test_inject_and_match_taker_bid_with_quote_quantity_zero_lot_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user_alice, 8000, 2000);
            {
                let open_orders = vector::empty<Order>();
                vector::push_back(
                    &mut open_orders,
    ...
    fun test_inject_and_match_taker_bid_with_quote_quantity_partial_lot_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user_alice, 8000, 500 + 10);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
                clob::check_empty_tick_level(asks, 2 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_match_taker_bid_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user_alice, 8000, 500);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
                clob::check_empty_tick_level(asks, 2 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_match_taker_bid_with_maker_order_not_fully_filled_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            let (base_custodian, quote_custodian) = clob::borrow_custodian(&pool);
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user_alice, 8000, 750);
    ...
    fun test_inject_and_match_taker_ask_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 0, 10000);
            let (next_bid_order_id, next_ask_order_id, _, _) = clob::get_pool_stat(&pool);
            assert!(next_bid_order_id == clob::order_id_for_test(3, true), 0);
            assert!(next_ask_order_id == clob::order_id_for_test(1, false), 0);
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 1500, 10000);
            {
                let (_, _, bids, _) = get_pool_stat(&pool);
                clob::check_empty_tick_level(bids, 5 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_match_taker_bid_with_quote_quantity_and_expiration_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 8500, 5);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
                clob::check_empty_tick_level(asks, 2 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_match_taker_bid_with_expiration_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 8500, 0);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
                clob::check_empty_tick_level(asks, 2 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_price_limit_affected_match_taker_bid_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 85, 15);
            let (next_bid_order_id, next_ask_order_id, _, _) = clob::get_pool_stat(&pool);
            assert!(next_bid_order_id == clob::order_id_for_test(1, true), 0);
            assert!(next_ask_order_id == clob::order_id_for_test(3, false), 0);
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 85, 0);
            {
                let (_, _, _, asks) = get_pool_stat(&pool);
                clob::check_empty_tick_level(asks, 2 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_price_limit_affected_match_taker_ask_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, bob);
        {
            ...
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 5, 10);
            {
                let (_, _, bids, _) = get_pool_stat(&pool);
                clob::check_empty_tick_level(bids, 5 * FLOAT_SCALING);
            };
    ...
    fun test_inject_and_price_limit_affected_match_taker_ask_returned_pool_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            let wrapped_pool = test::take_shared<WrappedPool<DWLT, USD>>(&test);
            let pool = borrow_mut_pool<DWLT, USD>(&mut wrapped_pool);
            let account_cap = test::take_from_address<AccountCap>(&test, alice);
            let account_cap_user = account_owner(&account_cap);
    ...
    fun test_place_market_sell_order_with_skipping_self_matching_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, alice);
            let (coin1, coin2) =clob::place_market_order<DWLT, USD>(&mut pool, &account_cap, CLIENT_ID_ALICE, 600,
                false,
                mint_for_testing<DWLT>(600, ctx(&mut test)),
                mint_for_testing<USD>(0, ctx(&mut test)),
                &clock,
                ctx(&mut test));
            assert!(coin::value<DWLT>(&coin1) == 600, 0);
    ...
    fun test_place_market_buy_order_with_skipping_self_matching_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_mut_custodian(&mut pool);
            custodian::deposit(base_custodian, mint_for_testing<DWLT>(1500, ctx(&mut test)), account_cap_user);
            custodian::deposit(quote_custodian, mint_for_testing<USD>(10000, ctx(&mut test)), account_cap_user);
            test::return_shared(pool);
            test::return_to_sender<AccountCap>(&test, account_cap);
        };
        ...
        next_tx(&mut test, alice);
        {
            ...
            let clock = test::take_shared<Clock>(&test);
            let account_cap = test::take_from_address<AccountCap>(&test, alice);
            let (coin1, coin2) =clob::place_market_order<DWLT, USD>(&mut pool, &account_cap, CLIENT_ID_ALICE, 1,
                true,
                mint_for_testing<DWLT>(0, ctx(&mut test)),
    ...
    fun test_place_limit_order_with_skipping_self_matching_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, _) = clob::borrow_mut_custodian(&mut pool);
            custodian::deposit(base_custodian, mint_for_testing<DWLT>(400, ctx(&mut test)), account_cap_user);
            test::return_shared(pool);
            test::return_to_sender<AccountCap>(&test, account_cap);
        };
    ...
    fun test_swap_exact_quote_for_base_with_skipping_self_matching_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            let account_cap_user = account_owner(&account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_custodian(&pool);
            custodian::assert_user_balance<DWLT>(base_custodian, account_cap_user, 10000, 0);
            custodian::assert_user_balance<USD>(quote_custodian, account_cap_user, 0, 10000);
    ...
    fun test_inject_and_match_taker_ask_with_skipping_self_matching_(test: Scenario): TransactionEffects {
        ...
        next_tx(&mut test, alice);
        {
            ...
            custodian::assert_user_balance<DWLT>(
                base_custodian,
                account_cap_user,
                0,
                10000
    ...
    fun test_cancel_order_no_rounding(): TransactionEffects {
        ...
        // Sells some USDC to alice
        {
            ...
            let (base_coin, quote_coin) = clob::place_market_order<DWLT, USD>(
                &mut pool,
                &account_cap,
                CLIENT_ID_BOB,
                100000,
            ...
            let (base_coin, quote_coin) = clob::place_market_order<DWLT, USD>(
                ...
                100000,
                false,
                mint_for_testing<DWLT>(100000, ctx(&mut test)),
                mint_for_testing<USD>(0, ctx(&mut test)),
                &clock,
                ctx(&mut test)
            );
        ...
        // Check cancel all orders returns all funds
        {
            ...
            let account_cap = test::take_from_address<AccountCap>(&test, alice);
            // cancel all order
            clob::cancel_all_orders<DWLT, USD>(&mut pool, &account_cap);
            let (base_custodian, quote_custodian) = clob::borrow_custodian(&pool);
            clob::check_balance_invariants_for_account(&account_cap, quote_custodian, base_custodian, &pool);
            test::return_shared(pool);
            test::return_to_address<AccountCap>(alice, account_cap);
