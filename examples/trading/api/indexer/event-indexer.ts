...
import { EventId, IkaClient, IkaEvent, IkaEventFilter } from '@ika-io/ika/client';

import { CONFIG } from '../config';
import { prisma } from '../db';
import { getClient } from '../ika-utils';
import { handleEscrowObjects } from './escrow-handler';
import { handleLockObjects } from './locked-handler';

type IkaEventsCursor = EventId | null | undefined;

type EventExecutionResult = {
	cursor: IkaEventsCursor;
	hasNextPage: boolean;
};
...
type EventTracker = {
	...
	type: string;
	filter: IkaEventFilter;
	callback: (events: IkaEvent[], type: string) => any;
};
...
const runEventJob = async (client: IkaClient, tracker: EventTracker, cursor: IkaEventsCursor) => {
