...
Unit tests for a Move package can be run with the [`pera move test` command](./packages.md).
...
```bash
$ pera move new test_example; cd test_example 
```
...
You can run a specific test, or a set of tests with `pera move test <str>`. This
...
runtime and gas used for each test. You can additionally add `csv` (`pera move
