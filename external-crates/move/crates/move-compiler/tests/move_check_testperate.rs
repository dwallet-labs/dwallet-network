...
use move_compiler::{
    ...
    pera_mode, Compiler, PASS_PARSER,
};
...
const PERA_MODE_DIR: &str = "pera_mode";
...
fn default_testing_addresses(flavor: Flavor) -> BTreeMap<String, NumericalAddress> {
    ...
    if flavor == Flavor::Pera {
        mapping.extend([("pera", "0x2"), ("pera_system", "0x3")]);
    }
    mapping
        .into_iter()
...
fn move_check_testperate(path: &Path) -> datatest_stable::Result<()> {
    let path_contains = |s| path.components().any(|c| c.as_os_str() == s);
    let lint = path_contains(LINTER_DIR);
    let flavor = if path_contains(PERA_MODE_DIR) {
        Flavor::Pera
    } else {
        Flavor::default()
    };
    ...
    testperate(path, config, lint)
}

fn testperate(path: &Path, mut config: PackageConfig, lint: bool) -> datatest_stable::Result<()> {
...
// Runs all tests under the test/testperate directory.
pub fn run_test(
    path: &Path,
    exp_path: &Path,
    out_path: &Path,
...
pub fn run_test_inner(
    ...
    if flavor == Flavor::Pera {
        let (prefix, filters) = pera_mode::linters::known_filters();
        compiler = compiler.add_custom_known_filters(prefix, filters);
        if lint {
            compiler = compiler.add_visitors(pera_mode::linters::linter_visitors(LintLevel::All))
        }
    }
...
datatest_stable::harness!(move_check_testperate, "tests/", r".*\.move$");
