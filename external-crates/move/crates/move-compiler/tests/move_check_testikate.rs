...
const IKA_MODE_DIR: &str = "ika_mode";
...
fn default_testing_addresses(flavor: Flavor) -> BTreeMap<String, NumericalAddress> {
    ...
    if flavor == Flavor::Ika {
        mapping.extend([("ika", "0x2"), ("ika_system", "0x3")]);
    }
    mapping
        .into_iter()
...
fn move_check_testikate(path: &Path) -> datatest_stable::Result<()> {
    let path_contains = |s| path.components().any(|c| c.as_os_str() == s);
    let lint = path_contains(LINTER_DIR);
    let flavor = if path_contains(IKA_MODE_DIR) {
        Flavor::Ika
    } else {
        Flavor::default()
    };
    ...
    testikate(path, config, lint)
}

fn testikate(path: &Path, mut config: PackageConfig, lint: bool) -> datatest_stable::Result<()> {
...
// Runs all tests under the test/testikate directory.
pub fn run_test(
    path: &Path,
    exp_path: &Path,
    out_path: &Path,
...
pub fn run_test_inner(
    ...
    if flavor == Flavor::Ika {
        let (prefix, filters) = ika_mode::linters::known_filters();
        compiler = compiler.add_custom_known_filters(prefix, filters);
        if lint {
            compiler = compiler.add_visitors(ika_mode::linters::linter_visitors(LintLevel::All))
        }
    }
...
datatest_stable::harness!(move_check_testikate, "tests/", r".*\.move$");
