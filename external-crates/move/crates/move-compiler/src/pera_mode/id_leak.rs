...
use crate::{
    ...
    pera_mode::{OBJECT_NEW, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT},
};
...
use super::{
    ...
    PERA_ADDR_NAME, PERA_CLOCK_CREATE, PERA_SYSTEM_ADDR_NAME, PERA_SYSTEM_CREATE,
    PERA_SYSTEM_MODULE_NAME, UID_TYPE_NAME,
};

pub const FRESH_ID_FUNCTIONS: &[(Symbol, Symbol, Symbol)] = &[
    (PERA_ADDR_NAME, OBJECT_MODULE_NAME, OBJECT_NEW),
    (PERA_ADDR_NAME, OBJECT_MODULE_NAME, OBJECT_NEW_UID_FROM_HASH),
    (PERA_ADDR_NAME, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT),
];
pub const FUNCTIONS_TO_SKIP: &[(Symbol, Symbol, Symbol)] = &[
    (
        PERA_SYSTEM_ADDR_NAME,
        PERA_SYSTEM_MODULE_NAME,
        PERA_SYSTEM_CREATE,
    ),
    (PERA_ADDR_NAME, CLOCK_MODULE_NAME, PERA_CLOCK_CREATE),
    (
        PERA_ADDR_NAME,
    ...
    (PERA_ADDR_NAME, DENY_LIST_MODULE_NAME, DENY_LIST_CREATE),
...
impl<'a> SimpleAbsInt for IDLeakVerifierAI<'a> {
    ...
    fn exp_custom(
        ...
        if !matches!(first_value, Value::FreshID(_)) {
            ...
            let uid_msg = format!(
                "The UID must come directly from {pera}::{object}::{new}. \
                Or for tests, it can come from {pera}::{ts}::{ts_new}",
                pera = PERA_ADDR_NAME,
...
fn value_for_ty(loc: &Loc, sp!(_, t): &SingleType) -> Value {
    if t.is_apply(PERA_ADDR_NAME, OBJECT_MODULE_NAME, UID_TYPE_NAME)
        .is_some()
    {
        Value::NotFresh(*loc)
    } else {
