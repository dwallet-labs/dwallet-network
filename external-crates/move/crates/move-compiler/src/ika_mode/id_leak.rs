...
use crate::{
    ...
    ika_mode::{
        ...
        OBJECT_NEW_UID_FROM_HASH, RANDOMNESS_MODULE_NAME, RANDOMNESS_STATE_CREATE, IKA_ADDR_NAME,
        IKA_ADDR_VALUE, IKA_CLOCK_CREATE, IKA_SYSTEM_ADDR_VALUE, IKA_SYSTEM_CREATE,
        IKA_SYSTEM_MODULE_NAME, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT, UID_TYPE_NAME,
    },
};
...
pub const FRESH_ID_FUNCTIONS: &[(AccountAddress, Symbol, Symbol)] = &[
    (IKA_ADDR_VALUE, OBJECT_MODULE_NAME, OBJECT_NEW),
    (IKA_ADDR_VALUE, OBJECT_MODULE_NAME, OBJECT_NEW_UID_FROM_HASH),
    (IKA_ADDR_VALUE, TEST_SCENARIO_MODULE_NAME, TS_NEW_OBJECT),
];
pub const FUNCTIONS_TO_SKIP: &[(AccountAddress, Symbol, Symbol)] = &[
    (
        IKA_SYSTEM_ADDR_VALUE,
        IKA_SYSTEM_MODULE_NAME,
        IKA_SYSTEM_CREATE,
    ),
    (IKA_ADDR_VALUE, CLOCK_MODULE_NAME, IKA_CLOCK_CREATE),
    (
        IKA_ADDR_VALUE,
    ...
    (IKA_ADDR_VALUE, DENY_LIST_MODULE_NAME, DENY_LIST_CREATE),
...
impl<'a> SimpleAbsInt for IDLeakVerifierAI<'a> {
    ...
    fn exp_custom(
        ...
        if !matches!(first_value, Value::FreshID(_)) {
            ...
            let uid_msg = format!(
                "The UID must come directly from {ika}::{object}::{new}. \
                Or for tests, it can come from {ika}::{ts}::{ts_new}",
                ika = IKA_ADDR_NAME,
...
fn value_for_ty(loc: &Loc, sp!(_, t): &SingleType) -> Value {
    if t.is_apply(&IKA_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME)
        .is_some()
    {
        Value::NotFresh(*loc)
    } else {
