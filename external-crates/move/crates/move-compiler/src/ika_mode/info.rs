...
//! ProgramInfo extension for Ika Flavor
//! Contains information that may be expensive to compute and is needed only for Ika
...
use crate::{
    ...
    ika_mode::{
        OBJECT_MODULE_NAME, IKA_ADDR_VALUE, TRANSFER_FUNCTION_NAME, TRANSFER_MODULE_NAME,
        UID_TYPE_NAME,
    },
...
pub enum TransferKind {
    ...
    PublicTransfer(Loc),
    /// transferred within the module to an address vis `ika::transfer::transfer`
    PrivateTransfer(Loc),
}
...
impl IkaInfo {
    pub fn new(
        ...
        assert!(info.ika_flavor_info.is_none());
    ...
    fn visit_ty(
        ...
        match ty_ {
            ...
            N::Type_::Apply(_, sp!(_, tn_), _)
                if tn_.is(&IKA_ADDR_VALUE, OBJECT_MODULE_NAME, UID_TYPE_NAME) =>
            {
