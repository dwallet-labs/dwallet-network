...
impl CompiledPackage {
    ...
    pub(crate) fn build_for_driver<W: Write, T>(
        ...
        let ika_mode = resolution_graph
            .build_options
            .default_flavor
            .map_or(false, |f| f == Flavor::Ika);

        let mut compiler = Compiler::from_package_paths(vfs_root, paths, bytecode_deps)
            .unwrap()
            .set_flags(flags);
        if ika_mode {
            let (filter_attr_name, filters) = ika_mode::linters::known_filters();
            compiler = compiler
                .add_custom_known_filters(filter_attr_name, filters)
                .add_visitors(ika_mode::linters::linter_visitors(lint_level))
        }
