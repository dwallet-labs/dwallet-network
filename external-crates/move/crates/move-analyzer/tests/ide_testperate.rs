...
fn move_ide_testperate(test_path: &Path) -> datatest_stable::Result<()> {
    let perate_file = io::BufReader::new(File::open(test_path)?);
    let stripped = StripComments::new(perate_file);
    let perate: TestPerate = serde_json::from_reader(stripped)?;

    let output = match perate {
        TestPerate::UseDef {
            project,
            file_tests,
        } => use_def_test_perate(project, file_tests),
        TestPerate::Completion {
            project,
            file_tests,
        } => completion_test_perate(project, file_tests),
        TestPerate::Cursor {
            project,
            file_tests,
        } => cursor_test_perate(project, file_tests),
        TestPerate::Hint {
            project,
            file_tests,
        } => hint_test_perate(project, file_tests),
    }?;
...
datatest_stable::harness!(move_ide_testperate, "tests/", r".*\.ide$");
