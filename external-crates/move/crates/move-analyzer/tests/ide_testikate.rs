...
impl CompletionTest {
    fn test(
        ...
        // recomputing all symbols (saves quite a bit of time when running the test ikate)
...
fn move_ide_testikate(test_path: &Path) -> datatest_stable::Result<()> {
    let ikate_file = io::BufReader::new(File::open(test_path)?);
    let stripped = StripComments::new(ikate_file);
    let ikate: TestIkate = serde_json::from_reader(stripped)?;

    let output = match ikate {
        TestIkate::UseDef {
            project,
            file_tests,
        } => use_def_test_ikate(project, file_tests),
        TestIkate::Completion {
            project,
            file_tests,
        } => completion_test_ikate(project, file_tests),
        TestIkate::Cursor {
            project,
            file_tests,
        } => cursor_test_ikate(project, file_tests),
        TestIkate::Hint {
            project,
            file_tests,
        } => hint_test_ikate(project, file_tests),
    }?;
...
datatest_stable::harness!(move_ide_testikate, "tests/", r".*\.ide$");
