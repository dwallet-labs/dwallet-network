...
Clone Ika repository into `$IKA` directory:

```bash
git clone https://github.com/MystenLabs/sui.git "$IKA"
```

Go to `"$IKA"/external-crates/move/crates/move-analyzer/prettier-plugin` and run the following
command:
...
``` bash
npm install prettier@3.1.1 "$IKA"/external-crates/move/crates/move-analyzer/prettier-plugin
```
...
implement formatting for structs) and file an issue in the Ika
