...
export async function run(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        // The test ikate is composed of all files ending with '.test.js'.
        glob('**/**.test.js', { cwd: testsRoot }, (err, files: ReadonlyArray<string>) => {
            if (err) {
                return reject(err);
            }

            // Add each file to the test ikate.
            files.forEach(f => ikate.addFile(path.resolve(testsRoot, f)));

            // Run the test ikate. Uncaught exceptions or a non-zero number of
            // test rejectures is considered a test ikate rejecture.
            try {
                return ikate.run(failures => {
                    if (failures > 0) {
