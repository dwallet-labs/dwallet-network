...
# a map from os version identifiers in Pera's binary distribution to os version identifiers
...
for DIST_OS VSCODE_OS in "${(@kv)SUPPORTED_OS}"; do
    # Pera distribution identifier
    PERA_DISTRO=$NETWORK"-v"$VERSION
    # name of the Pera distribution archive file, for example pera-testnet-v1.13.0-macos-arm64.tgz
    PERA_ARCHIVE="pera-"$PERA_DISTRO"-"$DIST_OS".tgz"
    # a path to downloaded Pera archive
    PERA_ARCHIVE_PATH=$TMP_DIR"/"$PERA_ARCHIVE

    # download Pera archive file to a given location and uncompress it
    curl https://github.com/MystenLabs/sui/releases/download/"$PERA_DISTRO"/"$PERA_ARCHIVE" -L -o $PERA_ARCHIVE_PATH
    tar -xf $PERA_ARCHIVE_PATH --directory $TMP_DIR

    # names of the move-analyzer binary, both the one becoming part of the extension ($SERVER_BIN)
    # and the one in the Pera archive ($ARCHIVE_SERVER_BIN)
