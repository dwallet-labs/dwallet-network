...
use ika_types::{error::ExecutionError, IKA_FRAMEWORK_ADDRESS};
...
/// All transfer functions (the functions in `ika::transfer`) are "private" in that they are
...
pub fn verify_module(module: &CompiledModule) -> Result<(), ExecutionError> {
    if *module.address() == IKA_FRAMEWORK_ADDRESS
        && module.name() == IdentStr::new(TEST_SCENARIO_MODULE_NAME).unwrap()
    {
        // exclude test_module which is a test-only module in the Ika framework which "emulates"
        // transactional execution and needs to allow test code to bypass private generics
        return Ok(());
    }
    // do not need to check the ika::transfer module itself
...
fn verify_function(view: &CompiledModule, fdef: &FunctionDefinition) -> Result<(), String> {
    ...
    for instr in &code.code {
        if let Bytecode::CallGeneric(finst_idx) = instr {
            ...
            if ident == (IKA_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
                verify_private_transfer(view, fhandle, type_arguments)?
            } else if ident == (IKA_FRAMEWORK_ADDRESS, EVENT_MODULE) {
...
fn verify_private_transfer(
    ...
    if addr_module(view, self_handle) == (IKA_FRAMEWORK_ADDRESS, TRANSFER_MODULE) {
        return Ok(());
    }
    ...
    if !is_defined_in_current_module(view, type_arg) {
        return Err(format!(
            "Invalid call to '{ika}::transfer::{f}' on an object of type '{t}'. \
            ...
            instead, i.e. '{ika}::transfer::public_{f}'",
            ika = IKA_FRAMEWORK_ADDRESS,
            f = fident,
