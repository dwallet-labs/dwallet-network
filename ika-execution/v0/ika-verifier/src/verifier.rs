...
pub fn ika_verify_module_metered_check_timeout_only(
    ...
    if let Err(error) = ika_verify_module_metered(config, module, fn_info_map, meter) {
        if matches!(
            error.kind(),
...
pub fn ika_verify_module_unmetered(
    ...
    ika_verify_module_metered(config, module, fn_info_map, &mut DummyMeter).inspect_err(|err| {
