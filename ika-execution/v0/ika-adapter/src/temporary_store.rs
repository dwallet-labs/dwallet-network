...
use ika_types::execution::{DynamicallyLoadedObjectMetadata, ExecutionResults, SharedInput};
...
use ika_types::storage::{BackingStore, DeleteKindWithOldVersion, DenyListResult, PackageObject};
use ika_types::ika_system_state::{get_ika_system_state_wrapper, AdvanceEpochParams};
use ika_types::{
    base_types::{
        ObjectDigest, ObjectID, ObjectRef, SequenceNumber, IkaAddress, TransactionDigest,
        VersionDigest,
    },
...
impl<'backing> TemporaryStore<'backing> {
    ...
    ///
    /// 3. all IKA in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// amount of IKA. We need these information for conservation check.
    pub fn check_ika_conserved(
        ...
        for (id, input, output) in self.get_modified_objects() {
            if let Some((version, storage_rebate)) = input {
                ...
                if do_expensive_checks {
                    total_input_ika += self.get_input_ika(&id, version, layout_resolver)?;
                }
            }
        ...
        if do_expensive_checks {
            ...
            if total_input_ika != total_output_ika {
                ...
                format!("IKA conservation failed: input={}, output={}, this transaction either mints or burns IKA",
                total_input_ika,
                total_output_ika))
            );
            }
        ...
        // all IKA in storage rebate fields of input objects should flow either to the transaction storage rebate, or the non-refundable
        // storage rebate pool
        if total_input_rebate != gas_summary.storage_rebate + gas_summary.non_refundable_storage_fee
        {
            ...
            /*return Err(ExecutionError::invariant_violation(
                format!("IKA conservation failed--{} IKA in storage rebate field of input objects, {} IKA in tx storage rebate or tx non-refundable storage rebate",
                total_input_rebate,
                gas_summary.non_refundable_storage_fee))
            );*/
        }

        // all IKA charged for storage should flow into the storage rebate field of some output object
        if gas_summary.storage_cost != total_output_rebate {
            // TODO: re-enable once we fix the edge case with OOG, gas smashing, and storage rebate
            /*return Err(ExecutionError::invariant_violation(
                format!("IKA conservation failed--{} IKA charged for storage, {} IKA in storage rebate field of output objects",
...
impl<'backing> Storage for TemporaryStore<'backing> {
    ...
    fn record_execution_results(&mut self, results: ExecutionResults) {
        let ExecutionResults::V1(results) = results else {
            panic!("ExecutionResults::V1 expected in ika-execution v0");
        };
    ...
    fn check_coin_deny_list(
        ...
        unreachable!("Coin denylist v2 is not supported in ika-execution v0");
    }
}
