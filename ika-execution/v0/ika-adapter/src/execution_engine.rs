...
mod checked {
    ...
    use ika_types::storage::WriteKind;
    #[cfg(msim)]
    ...
    use ika_types::{
        base_types::{ObjectRef, IkaAddress, TransactionDigest, TxContext},
        object::{Object, ObjectInner},
        ika_system_state::{ADVANCE_EPOCH_FUNCTION_NAME, IKA_SYSTEM_MODULE_NAME},
        IKA_FRAMEWORK_ADDRESS,
    };
    use ika_types::{IKA_FRAMEWORK_PACKAGE_ID, IKA_SYSTEM_PACKAGE_ID};
    ...
    fn execute_transaction<Mode: ExecutionMode>(
        ...
        if !is_genesis_tx && !Mode::allow_arbitrary_values() {
            ...
            let conservation_result = {
                ...
                temporary_store.check_ika_conserved(
                    &cost_summary,
            ...
            if let Err(conservation_err) = conservation_result {
                ...
                let mut layout_resolver =
                    TypeLayoutResolver::new(move_vm, Box::new(&*temporary_store));
                if let Err(recovery_err) = temporary_store.check_ika_conserved(
                    &cost_summary,
                    advance_epoch_gas_summary,
                    &mut layout_resolver,
                    enable_expensive_checks,
