...
mod checked {
    ...
    use ika_types::execution_status::CommandArgumentError;
    use ika_types::storage::PackageObject;
    use ika_types::{
        balance::Balance,
        base_types::{MoveObjectType, ObjectID, SequenceNumber, IkaAddress, TxContext},
        coin::Coin,
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        pub fn new(
            ...
            // TODO remove this
            let linkage = LinkageView::new(Box::new(state_view.as_ika_resolver()), init_linkage);
            ...
            // exist. Plus, Ika Move does not use these changes or events
    ...
    // Create a new Session ikatable for resolving type and type operations rather than execution
