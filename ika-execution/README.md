...
The `ika-execution` crate is responsible for abstracting access to the
...
- The adapter, that integrates Move into Ika.
...
- `./ika-execution`, (latest version and cuts/copies of ika-specific
  crates).
...
code must be via `ika-execution`, and not by directly depending a
constituent crate.
...
and elsewhere, **default to accessing execution via `ika-execution`.**
...
`ika-execution tests::test_encapsulation` is a test that detects
...
`./crates/ika-types` are shared across all versions.  Changes to
...
## `ika-execution/src/lib.rs`

The entry-point to the execution crate -- `ika-execution/src/lib.rs`
...
- `ika-execution/src/lib.template.rs` -- a template file with
