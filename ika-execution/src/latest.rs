...
use ika_adapter_latest::adapter::{new_move_vm, run_metered_move_bytecode_verifier};
use ika_adapter_latest::execution_engine::{
    execute_genesis_state_update, execute_transaction_to_effects,
};
use ika_adapter_latest::type_layout_resolver::TypeLayoutResolver;
use ika_move_natives_latest::all_natives;
use ika_types::storage::BackingStore;
use ika_verifier_latest::meter::IkaVerifierMeter;

use crate::executor;
use crate::verifier;
use ika_adapter_latest::execution_mode;
...
impl<'m> verifier::Verifier for Verifier<'m> {
    fn meter(&self, config: MeterConfig) -> Box<dyn Meter> {
        Box::new(IkaVerifierMeter::new(config))
    }
