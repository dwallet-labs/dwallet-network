...
#[test]
/// Make sure that all accesses to execution layer crates in the `ika-node` and `ika-replay` crates
/// go via the `ika-execution` crate (in other words, the `ika-execution` crate dominates execution
/// layer crates in the dependency graphs of `ika-node` and `ika-replay`).
///
...
fn test_encapsulation() {
    ...
    let mut exec_crates: BTreeSet<_> = packages.normal_deps("ika-execution").collect();

    // Remove the crates that the execution layer depends on but which are not directly part of the
    // execution layer -- these don't need to be accessed exclusively via `ika-execution`.
    exec_crates.remove("ika-protocol-config");
    exec_crates.remove("ika-types");
    ...
    for root in ["ika-node", "ika-replay"] {
        let mut graph = packages.graph(root);

        // If we can still create a path from `root` to an execution crate after removing these
        // nodes then we know that we can potential bypass "ika-execution".
        graph.remove_node("ika-execution");
    ...
    panic!(
        "protocol-sensitive binaries depend on execution crates outside of 'ika-execution', e.g.:\n\
         \n  {}\n\
         \n\
         This can cause execution to fork by not respecting the execution layer version set in the \
         protocol config.  Fix this by depending on these crates via 'ika-execution'.\n\
         \n\
         P.S. if you believe one of these crates should not be part of 'ika-execution' then update \
         the test to exclude this crate.",
        examples.join("\n  "),
    );
}

/// Parse `cargo metadata` for the `ika` repo.
fn cargo_metadata() -> cargo_metadata::Result<Metadata> {
    let ika_execution = PathBuf::from(env!("CARGO_MANIFEST_DIR"));

    MetadataCommand::new()
        .manifest_path(ika_execution.join("../Cargo.toml"))
        .current_dir(ika_execution.join(".."))
        .no_deps()
        .exec()
}
