...
use ika_types::{
    ...
    coin::Coin,
    error::{ExecutionError, ExecutionErrorKind, IkaError},
...
pub trait IkaResolver: ResourceResolver<Error = IkaError> + BackingPackageStore {
    fn as_backing_package_store(&self) -> &dyn BackingPackageStore;
}

impl<T> IkaResolver for T
where
    T: ResourceResolver<Error = IkaError>,
...
pub trait ExecutionState: StorageView + IkaResolver {
    fn as_ika_resolver(&self) -> &dyn IkaResolver;
...
impl TryFromValue for IkaAddress {
    fn try_from_value(value: Value) -> Result<Self, CommandArgumentError> {
