...
use ika_types::base_types::VersionDigest;
use ika_types::committee::EpochId;
use ika_types::digests::ObjectDigest;
use ika_types::effects::{TransactionEffects, TransactionEvents};
use ika_types::execution::{
    DynamicallyLoadedObjectMetadata, ExecutionResults, ExecutionResultsV2, SharedInput,
};
use ika_types::execution_config_utils::to_binary_config;
use ika_types::execution_status::ExecutionStatus;
use ika_types::inner_temporary_store::InnerTemporaryStore;
use ika_types::layout_resolver::LayoutResolver;
use ika_types::storage::{BackingStore, DenyListResult, PackageObject};
use ika_types::ika_system_state::{get_ika_system_state_wrapper, AdvanceEpochParams};
use ika_types::{
    base_types::{ObjectID, ObjectRef, SequenceNumber, IkaAddress, TransactionDigest},
    effects::EffectsObjectChange,
    error::{ExecutionError, IkaError, IkaResult},
    fp_bail,
    gas::GasCostSummary,
    object::Owner,
    object::{Data, Object},
...
use ika_types::{is_system_package, IKA_SYSTEM_STATE_OBJECT_ID};
...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn into_effects_v1(
        ...
        let updated_gas_object_info = if let Some(coin_id) = gas_charger.gas_coin() {
            ...
            (
                ...
                Owner::AddressOwner(IkaAddress::default()),
            )
        };
    ...
    pub fn conserve_unmetered_storage_rebate(&mut self, unmetered_storage_rebate: u64) {
        ...
        let mut system_state_wrapper = self
            .read_object(&IKA_SYSTEM_STATE_OBJECT_ID)
        ...
        // If not, we would be dropping IKA on the floor by overriding it.
...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn get_objects_to_authenticate(
    ...
    ) -> IkaResult<(Vec<ObjectID>, HashSet<ObjectID>)> {
        ...
        for id in &self.execution_results.modified_objects {
            ...
            match &old_obj.owner {
                ...
                Owner::Immutable => {
                    assert!(is_epoch_change, "Immutable objects cannot be written, except for Ika Framework/Move stdlib upgrades at epoch change boundaries");
...
impl<'backing> TemporaryStore<'backing> {
    ...
    /// `IkaGasStatus` `storage_rebate` and `storage_gas_units` track the transaction
...
impl<'backing> TemporaryStore<'backing> {
    pub fn advance_epoch_safe_mode(
        ...
        let wrapper = get_ika_system_state_wrapper(self.store.as_object_store())
...
impl<'backing> TemporaryStore<'backing> {
    fn get_input_ika(
        ...
        if let Some(obj) = self.input_objects.get(id) {
            ...
            obj.get_total_ika(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input IKA in IKA conservation checking for input with \
                         type {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
            ...
            let Ok(Some(obj)) = self.store.get_object_by_key(id, expected_version) else {
                invariant_violation!(
                    "Failed looking up dynamic field {id} in IKA conservation checking"
                );
            };
            obj.get_total_ika(layout_resolver).map_err(|e| {
                make_invariant_violation!(
                    "Failed looking up input IKA in IKA conservation checking for type \
                         {:?}: {e:#?}",
                    obj.struct_tag(),
                )
            })
    ...
    /// Check that this transaction neither creates nor destroys IKA. This should hold for all txes
    ...
    ///
    /// 1. all IKA in storage rebate fields of input objects should flow either to the transaction
    ///    storage rebate, or the transaction non-refundable storage rebate
    /// 2. all IKA charged for storage should flow into the storage rebate field of some output
    ///    object
    ///
    ...
    pub fn check_ika_conserved(
        ...
        // total amount of IKA in storage rebate of input objects
        let mut total_input_rebate = 0;
        // total amount of IKA in storage rebate of output objects
        ...
        if gas_summary.storage_cost == 0 {
                    ...
                    + gas_summary.non_refundable_storage_fee
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "IKA conservation failed -- no storage charges in gas summary \
            ...
            // all IKA in storage rebate fields of input objects should flow either to
            ...
            {
                return Err(ExecutionError::invariant_violation(format!(
                    "IKA conservation failed -- {} IKA in storage rebate field of input objects, \
                        {} IKA in tx storage rebate or tx non-refundable storage rebate",
                    total_input_rebate, gas_summary.non_refundable_storage_fee,
                )));
            }

            // all IKA charged for storage should flow into the storage rebate field
            // of some output object
            if gas_summary.storage_cost != total_output_rebate {
                return Err(ExecutionError::invariant_violation(format!(
                    "IKA conservation failed -- {} IKA charged for storage, \
                        {} IKA in storage rebate field of output objects",
    ...
    /// Check that this transaction neither creates nor destroys IKA.
    /// This more expensive check will check a third invariant on top of the 2 performed
    /// by `check_ika_conserved` above:
    ///
    /// * all IKA in input objects (including coins etc in the Move part of an object) should flow
    ...
    /// advance epoch transaction would mint `epoch_fees` amount of IKA, and burn `epoch_rebates`
    /// amount of IKA. We need these information for this check.
    pub fn check_ika_conserved_expensive(
        &self,
        gas_summary: &GasCostSummary,
        advance_epoch_gas_summary: Option<(u64, u64)>,
        layout_resolver: &mut impl LayoutResolver,
    ) -> Result<(), ExecutionError> {
        // total amount of IKA in input objects, including both coins and storage rebates
        let mut total_input_ika = 0;
        // total amount of IKA in output objects, including both coins and storage rebates
        let mut total_output_ika = 0;
        for (id, input, output) in self.get_modified_objects() {
            if let Some(input) = input {
                total_input_ika += self.get_input_ika(&id, input.version, layout_resolver)?;
            }
            if let Some(object) = output {
                total_output_ika += object.get_total_ika(layout_resolver).map_err(|e| {
                    make_invariant_violation!(
                        "Failed looking up output IKA in IKA conservation checking for \
                         mutated type {:?}: {e:#?}",
                        object.struct_tag(),
                    )
                })?;
        ...
        total_output_ika += gas_summary.computation_cost + gas_summary.non_refundable_storage_fee;
        if let Some((epoch_fees, epoch_rebates)) = advance_epoch_gas_summary {
            total_input_ika += epoch_fees;
            total_output_ika += epoch_rebates;
        }
        if total_input_ika != total_output_ika {
            return Err(ExecutionError::invariant_violation(format!(
                "IKA conservation failed: input={}, output={}, \
                    this transaction either mints or burns IKA",
                total_input_ika, total_output_ika,
            )));
        }
        Ok(())
    }
...
impl<'backing> Storage for TemporaryStore<'backing> {
    ...
    fn record_execution_results(&mut self, results: ExecutionResults) {
        let ExecutionResults::V2(results) = results else {
            panic!("ExecutionResults::V2 expected in ika-execution v1 and above");
        };
    ...
    fn check_coin_deny_list(
        ...
        unreachable!("Coin denylist v2 is not supported in ika-execution v1");
    }
}
    ...
    fn get_resource(
        ...
        let object = match self.read_object(&ObjectID::from(*address)) {
            ...
            None => match self.read_object(&ObjectID::from(*address)) {
                ...
                Some(x) => {
                    if !x.is_immutable() {
                        fp_bail!(IkaError::ExecutionInvariantViolation);
                    }
                    x
                }
            },
