...
use crate::execution_value::IkaResolver;
use move_core_types::{
...
pub struct LinkageView<'state> {
    ...
    resolver: Box<dyn IkaResolver + 'state>,
...
impl<'state> LinkageView<'state> {
    pub fn new(resolver: Box<dyn IkaResolver + 'state>) -> Self {
        Self {
            resolver,
            linkage_info: None,
    ...
    pub(crate) fn relocate(&self, module_id: &ModuleId) -> Result<ModuleId, IkaError> {
