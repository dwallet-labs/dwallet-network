...
use ika_types::{error::ExecutionError, move_package::FnInfoMap};
...
pub fn ika_verify_module_metered(
...
/// Runs the Ika verifier and checks if the error counts as a Ika verifier timeout
/// NOTE: this function only check if the verifier error is a timeout
/// All other errors are ignored
pub fn ika_verify_module_metered_check_timeout_only(
    module: &CompiledModule,
    fn_info_map: &FnInfoMap,
    meter: &mut (impl Meter + ?Sized),
    verifier_config: &VerifierConfig,
) -> Result<(), ExecutionError> {
    // Checks if the error counts as a Ika verifier timeout
    if let Err(error) = ika_verify_module_metered(module, fn_info_map, meter, verifier_config) {
        if matches!(
            error.kind(),
            ika_types::execution_status::ExecutionFailureStatus::IkaMoveVerificationTimedout
        ) {
            return Err(error);
        }
    }
...
pub fn ika_verify_module_unmetered(
    module: &CompiledModule,
    fn_info_map: &FnInfoMap,
    verifier_config: &VerifierConfig,
) -> Result<(), ExecutionError> {
    ika_verify_module_metered(module, fn_info_map, &mut DummyMeter, verifier_config).inspect_err(
        |err| {
