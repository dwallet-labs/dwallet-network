...
pub fn verify_module(
    ...
    // In Ika's framework code there is an exception to the one-time witness type rule - we have a
    // IKA type in the ika module but it is instantiated outside of the module initializer (in fact,
    // the module has no initializer). The reason for it is that the IKA coin is only instantiated
    ...
    if ModuleId::new(IKA_FRAMEWORK_ADDRESS, ident_str!("ika").to_owned()) == self_id {
        return Ok(());
    }
