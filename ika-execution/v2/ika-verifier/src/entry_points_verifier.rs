...
use ika_types::randomness_state::is_mutable_random;
use ika_types::{
...
fn verify_param_type(
    ...
    // Only `ika::ika_system` is allowed to expose entry functions that accept a mutable clock
    // parameter.
    ...
    // Only `ika::ika_system` is allowed to expose entry functions that accept a mutable Random
    // parameter.
