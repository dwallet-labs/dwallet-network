...
use ika_types::{MOVE_STDLIB_ADDRESS, IKA_FRAMEWORK_ADDRESS, IKA_SYSTEM_ADDRESS};
...
pub fn all_natives(silent: bool) -> NativeFunctionTable {
    let ika_framework_natives: &[(&str, &str, NativeFunction)] = &[
    ...
    let ika_framework_natives_iter =
        ika_framework_natives
            .iter()
            .cloned()
    ...
    let ika_system_natives: &[(&str, &str, NativeFunction)] = &[(
        "validator",
    ...
    ika_system_natives
        ...
        .chain(ika_framework_natives_iter)
        ...
        _ => {
            return Err(
                PartialVMError::new(StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR)
                    .with_message("Ika verifier guarantees this is a struct".to_string()),
            )
        }
    };
