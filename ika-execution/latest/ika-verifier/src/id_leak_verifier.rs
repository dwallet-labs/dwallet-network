...
//! Objects whose struct type has key ability represent Ika objects.
//! They have unique IDs that should never be reused. This verifier makes
//! sure that the id field of Ika objects never get leaked.
//! Unpack is the only bytecode that could extract the id field out of
//! a Ika object. From there, we track the flow of the value and make
...
use ika_types::bridge::BRIDGE_MODULE_NAME;
...
use ika_types::{
    ...
    ika_system_state::IKA_SYSTEM_MODULE_NAME,
    BRIDGE_ADDRESS, IKA_FRAMEWORK_ADDRESS, IKA_SYSTEM_ADDRESS,
};
...
const IKA_SYSTEM_CREATE: FunctionIdent = (
    &IKA_SYSTEM_ADDRESS,
    IKA_SYSTEM_MODULE_NAME,
    ident_str!("create"),
);
const IKA_CLOCK_CREATE: FunctionIdent = (
    &IKA_FRAMEWORK_ADDRESS,
    CLOCK_MODULE_NAME,
    ident_str!("create"),
);
const IKA_AUTHENTICATOR_STATE_CREATE: FunctionIdent = (
    &IKA_FRAMEWORK_ADDRESS,
    AUTHENTICATOR_STATE_MODULE_NAME,
    ident_str!("create"),
);
const IKA_RANDOMNESS_STATE_CREATE: FunctionIdent = (
    &IKA_FRAMEWORK_ADDRESS,
    RANDOMNESS_MODULE_NAME,
    ident_str!("create"),
);
const IKA_DENY_LIST_CREATE: FunctionIdent = (
    &IKA_FRAMEWORK_ADDRESS,
    DENY_LIST_MODULE,
    DENY_LIST_CREATE_FUNC,
);

const IKA_BRIDGE_CREATE: FunctionIdent =
...
const FUNCTIONS_TO_SKIP: &[FunctionIdent] = &[
    IKA_SYSTEM_CREATE,
    IKA_CLOCK_CREATE,
    IKA_AUTHENTICATOR_STATE_CREATE,
    IKA_RANDOMNESS_STATE_CREATE,
    IKA_DENY_LIST_CREATE,
    IKA_BRIDGE_CREATE,
];
    ...
    if handle.abilities.has_key() && last_value != AbstractValue::Fresh {
        ...
        let msg = format!(
            "Invalid object creation in {cur_package}::{cur_module}::{cur_function}. \
                ...
                The UID must come directly from ika::{}::{}. \
                Or for tests, it can come from ika::{}::{}",
