...
mod checked {
    ...
    use ika_verifier::check_for_verifier_timeout;
    use tracing::instrument;

    use ika_move_natives::{object_runtime::ObjectRuntime, NativesCostTable};
    use ika_protocol_config::ProtocolConfig;
    use ika_types::{
        base_types::*,
        error::ExecutionError,
        error::{ExecutionErrorKind, IkaError},
        execution_config_utils::to_binary_config,
        metrics::LimitsMetrics,
        storage::ChildObjectResolver,
    };
    use ika_verifier::verifier::ika_verify_module_metered_check_timeout_only;

    pub fn new_move_vm(
        natives: NativeFunctionTable,
        protocol_config: &ProtocolConfig,
        _enable_profiler: Option<PathBuf>,
    ) -> Result<MoveVM, IkaError> {
        ...
        .map_err(|_| IkaError::ExecutionInvariantViolation)
    }
    ...
    /// Run both the Move verifier and the Ika verifier, checking just for timeouts. Returns Ok(())
    ...
    fn verify_module_timeout_only(
        ...
        if let Err(e) = verify_module_with_config_metered(verifier_config, module, meter) {
            ...
            if check_for_verifier_timeout(&e.major_status()) {
                return Err(IkaError::ModuleVerificationFailure {
                    error: format!("Verification timed out: {}", e),
                });
            }
        } else if let Err(err) = ika_verify_module_metered_check_timeout_only(
            module,
            &BTreeMap::new(),
            meter,
            verifier_config,
