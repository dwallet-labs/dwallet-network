...
#[ika_macros::with_checked_arithmetic]
mod checked {
    ...
    use ika_move_natives::all_natives;
    use ika_protocol_config::{check_limit_by_meter, LimitThresholdCrossed, ProtocolConfig};
    use ika_types::authenticator_state::{
    ...
    use ika_types::bridge::BRIDGE_COMMITTEE_MINIMAL_VOTING_POWER;
    use ika_types::bridge::{
        BridgeChainId, BRIDGE_CREATE_FUNCTION_NAME, BRIDGE_INIT_COMMITTEE_FUNCTION_NAME,
        BRIDGE_MODULE_NAME,
    };
    use ika_types::clock::{CLOCK_MODULE_NAME, CONSENSUS_COMMIT_PROLOGUE_FUNCTION_NAME};
    ...
    use ika_types::error::{ExecutionError, ExecutionErrorKind};
    use ika_types::execution::is_certificate_denied;
    use ika_types::execution_config_utils::to_binary_config;
    use ika_types::execution_status::{CongestedObjects, ExecutionStatus};
    ...
    #[cfg(msim)]
    use ika_types::ika_system_state::advance_epoch_result_injection::maybe_modify_result;
    use ika_types::ika_system_state::{AdvanceEpochParams, ADVANCE_EPOCH_SAFE_MODE_FUNCTION_NAME};
    ...
    use ika_types::transaction::{CheckedInputObjects, RandomnessStateUpdate};
    use ika_types::{
        base_types::{ObjectID, ObjectRef, IkaAddress, TransactionDigest, TxContext},
        object::{Object, ObjectInner},
        ika_system_state::{ADVANCE_EPOCH_FUNCTION_NAME, IKA_SYSTEM_MODULE_NAME},
        IKA_AUTHENTICATOR_STATE_OBJECT_ID, IKA_FRAMEWORK_ADDRESS, IKA_FRAMEWORK_PACKAGE_ID,
    ...
    pub fn execute_transaction_to_effects<Mode: ExecutionMode>(
        ...
        let status = if let Err(error) = &execution_result {
            ...
            match error.kind() {
                ...
                K::IkaMoveVerificationError | K::VMVerificationOrDeserializationError => {
                    #[skip_checked_arithmetic]
                    tracing::debug!(
                        kind = ?error.kind(),
                        tx_digest = ?transaction_digest,
    ...
    fn execute_transaction<Mode: ExecutionMode>(
        ...
        // information provided to check_ika_conserved, because we mint rewards, and burn
    ...
    fn run_conservation_checks<Mode: ExecutionMode>(
        ...
        let mut result: std::result::Result<(), ika_types::error::ExecutionError> = Ok(());
        if !is_genesis_tx && !Mode::skip_conservation_checks() {
            // ensure that this transaction did not create or destroy IKA, try to recover if the check fails
            let conservation_result = {
                temporary_store
                    .check_ika_conserved(simple_conservation_checks, cost_summary)
                    .and_then(|()| {
                        if enable_expensive_checks {
                            // ensure that this transaction did not create or destroy IKA, try to recover if the check fails
                            let mut layout_resolver =
                                TypeLayoutResolver::new(move_vm, Box::new(&*temporary_store));
                            temporary_store.check_ika_conserved_expensive(
                                cost_summary,
                                advance_epoch_gas_summary,
                                &mut layout_resolver,
                            )
            ...
            if let Err(conservation_err) = conservation_result {
                ...
                if let Err(recovery_err) = {
                    ...
                    // we will create or destroy IKA otherwise
                    panic!(
                        "IKA conservation fail in tx block {}: {}\nGas status is {}\nTx was ",
                        tx_ctx.digest(),
                        recovery_err,
                        gas_charger.summary()
                    )
        ...
        } // else, we're in the genesis transaction which mints the IKA supply, and hence does not satisfy IKA conservation, or
            ...
            TransactionKind::Genesis(GenesisTransaction { objects }) => {
                ...
                for genesis_object in objects {
                    match genesis_object {
                        ika_types::transaction::GenesisObject::RawObject { data, owner } => {
    ...
    pub fn construct_advance_epoch_pt(
        ...
        let storage_rebates = builder.programmable_move_call(
            IKA_SYSTEM_PACKAGE_ID,
            IKA_SYSTEM_MODULE_NAME.to_owned(),
