...
mod checked {
    ...
    use ika_move_natives::object_runtime::{
        self, get_all_uids, max_event_error, LoadedRuntimeObject, ObjectRuntime, RuntimeResults,
    };
    use ika_protocol_config::ProtocolConfig;
    use ika_types::execution::ExecutionResults;
    use ika_types::storage::{DenyListResult, PackageObject};
    use ika_types::{
        balance::Balance,
        base_types::{MoveObjectType, ObjectID, IkaAddress, TxContext},
        coin::Coin,
    ...
    use ika_types::{error::command_argument_error, execution_status::CommandArgumentError};
    ...
    pub struct ExecutionContext<'vm, 'state, 'a> {
        ...
        additional_transfers: Vec<(/* new owner */ IkaAddress, ObjectValue)>,
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        ...
        where
            'a: 'state,
        {
            let mut linkage_view = LinkageView::new(Box::new(state_view.as_ika_resolver()));
        ...
        pub fn convert_type_argument_error(&self, idx: usize, error: VMError) -> ExecutionError {
            use move_core_types::vm_status::StatusCode;
            use ika_types::execution_status::TypeArgumentError;
        ...
        pub(crate) fn execute_function_bypass_visibility(
            ...
            let mut data_store = IkaDataStore::new(&self.linkage_view, &self.new_packages);
    ...
    pub fn load_type_from_struct(
        ...
        let data_store = IkaDataStore::new(linkage_view, new_packages);
    ...
    pub(crate) struct IkaDataStore<'state, 'a> {
        linkage_view: &'a LinkageView<'state>,
        new_packages: &'a [MovePackage],
    }

    impl<'state, 'a> IkaDataStore<'state, 'a> {
        pub(crate) fn new(
            linkage_view: &'a LinkageView<'state>,
            new_packages: &'a [MovePackage],
        ) -> Self {
    ...
    impl<'state, 'a> DataStore for IkaDataStore<'state, 'a> {
