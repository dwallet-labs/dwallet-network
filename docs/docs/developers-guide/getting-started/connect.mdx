...
You can use our test network to experiment with the current version of dWallet running on that network.
Alternatively, you can spin up a local dWallet network for [local dWallet network](./local-network.mdx) for local development.

You can request test DWLT tokens through your [local faucet](./local-network.mdx#use-the-local-faucet).
These tokens have no financial value. Please note that there is no faucet service for Mainnet.
...
After installation, request DWLT test tokens by using cURL to request tokens from your [local faucet](./local-network.mdx#use-the-local-faucet).
...
Refer to [Connect to a custom RPC endpoint](#connect-to-a-custom-rpc-endpoint) to add a new environment alias or to switch the currently active network.
