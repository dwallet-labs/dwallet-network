...
4. Open a terminal or console on your system at the location you want to save the repository locally.
Type `git clone `followed by the URL you copied in the previous step, and press `Enter`.
5. Type `cd dwallet-network` to change the active directory to `dwallet-network`.
