...
(the one you cloned during the [dWallet Enviornment Setup](./dwallet-network-environment.mdx)):
...
[Set up a local dWallet Network account](#show-the-current-active-address) section.
You can use this address with the local faucet.

To request test coins from the local faucet, run the following cURL command:
...
To connect the live dWallet Network Explorer to your local network, open the following URL:
http://explorer.alpha.testnet.dwallet.cloud/.

For the most recent updates to the dWallet Network Explorer,
install and run it from your local clone of the dWallet Network repository.
