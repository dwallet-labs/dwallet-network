...
The following example uses the [Ika TypeScript SDK](https://sdk.mystenlabs.com/typescript) to connect via websocket to listen to events.

```javascript
const local_client = new IkaClient({ url: 'https://fullnode.mainnet.ika.io:443/' });
...
deepbook_client.ikaClient.subscribeEvent({
