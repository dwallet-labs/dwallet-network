...
Browser extension wallets built for Ika use the [Wallet standard](https://github.com/wallet-standard/wallet-standard/). This is a cross-chain standard that defines how dApps can automatically discover and interact with wallets.
...
```tsx
import { IKA_DEVNET_CHAIN, Wallet } from '@mysten/wallet-standard';
...
Features are standard methods consumers can use to interact with a wallet. To be listed in the Ika
...
- `ika:signPersonalMessage` - Use to prompt the user to sign a personal message and return the
  message signature back to the dApp. Use this to verify the user’s public key.
- `ika:signTransaction` - Use to prompt the user to sign a transaction and return the serialized
  transaction and signature back to the dApp. This method does not submit the transaction for
  execution.
- `ika:signAndExecuteTransaction` - Use to prompt the user to sign a transaction, then submit it
  for execution to the blockchain.
- `ika:reportTransactionEffects` - Use to report the effects of a transaction executed in the dApp
  to the wallet. This allows the wallet to update its internal state to reflect the changes the transaction makes.
- `ika:signTransactionBlock` - The previous version of `ika:signTransaction`. Still
  implemented for compatibility with dApps that have not updated to the new feature.
- `ika:signAndExecuteTransactionBlock` - The previous version of `ika:signAndExecuteTransaction`. Still implemented for compatibility with dApps that have not updated to the new feature.
...
import {
  ...
  IkaReportTransactionEffectsMethod
} from "@mysten/wallet-standard";

class YourWallet implements Wallet {
	/* ... existing code from above ... */

  get features(): ConnectFeature & EventsFeature & IkaFeatures {
    return {
      "standard:connect": {
        version: "1.0.0",
		...
		return someWalletAccounts.map(
			(walletAccount) =>
				...
				new ReadonlyWalletAccount({
					address: walletAccount.ikaAddress,
					publicKey: walletAccount.pubkey,
					// The Ika chains that your wallet supports.
					chains: [IKA_DEVNET_CHAIN],
...
The Wallet standard has been updated from its original design to better support changes in the Ika ecosystem. For example, the GraphQL service was introduced after Mainnet launched. The `ika:signAndExecuteTransactionBlock` feature is closely tied to the JSON RPC options and data structures, so its continued maintenance becomes increasingly difficult as the GraphQL service becomes more ubiquitous.

Consequently, the Wallet standard introduced the `ika:signAndExecuteTransaction` feature. The features of this method are more useful, regardless of which API you use to execute transactions. This usefulness comes at the expense
of flexibility in what `ika:signAndExecuteTransaction` returns.

To solve this problem, use the `ika:signTransaction` feature to sign transactions, and
...
To mitigate this, wallets can use the `ika:reportTransactionEffects` feature so that dApps can report
...
The `@ika-io/ika/transaction` SDK exports the `SerialTransactionExecutor` class, which you can use
...
Using the combination of `ika:signTransaction` and `ika:reportTransactionEffects`, dApps can use
...
Construct the transaction separately with the `@ika-io/ika` library and then sign it with the private key of the user. Use the `ika:signTransaction` feature to achieve this:

```tsx
wallet.features[ika:signTransaction].singTransaction(<Transaction>, <WalletAccount>);
```
...
To execute the transaction, use `IkaClient` from `@ika-io/ika`:

```tsx
const client: IkaClient
...
wallet.features['ika:reportTransactionEffects'].reportTransactionEffects(
...
Many wallets abstract the above flow into one feature: `ika:signAndExecuteTransaction`. The required arguments for this feature are the raw transaction and the options with the desired information to be included in the response:
