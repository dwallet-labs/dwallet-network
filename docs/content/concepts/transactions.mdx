...
All updates to the Ika database happen via transactions. This topic describes the transaction types supported by Ika and explains how their execution changes the ledger.  There are only two kinds of transactions on Ika:
...
All Ika transactions have the following common metadata:

- **Sender address:** The [address](guides/developer/getting-started/get-address.mdx) of the user sending this transaction.
- **Gas input:** An object reference pointing to the object that will be used to pay for this transaction's execution and storage. This object must be owned by the user and must be of type `ika::coin::Coin<IKA>` (i.e., the Ika native currency).
...
- **Epoch:** The Ika epoch this transaction is intended for.
...
Here's an example showing how objects and transactions are connected to each other in Ika.

In the following example there are two objects:
- Object A is a coin of type IKA with a total balance of 5 IKA
- Object B with 2 IKA coins that belongs to John

Tom decides to send 1 IKA coin to Alice. In this case, Object A is the input to this transaction and 1 IKA coin is debited from this object. The output of the transaction is two objects: 
- Object A with 4 IKA coins that still belongs to Tom
- new created Object C with 1 IKA coin that belongs now to Alice

At the same time, John decides to send 2 IKA coins to Anna. Because the relationship between objects and transactions is written in a directed acyclic graph (DAG), and both transactions interact with different objects, this transaction executes in parallel with the transaction that sends coins from Tom to Alice. This transaction changes only the owner of Object B from John to Anna.

After receiving 2 IKA coins, Anna sent them immediately to Tom. Now Tom has 6 IKA coins (4 from Object A and 2 from Object B).

Finally, Tom sends all of his IKA coins to John. For this transaction, the input is actually two objects (Object A and Object B). Object B is destroyed, and its value is added to Object A. As a result, the transaction's output is only Object A with a value of 6 IKA.
...
Ika has some limits on transactions and the data used in them, such as a maximum size and number of objects used. You can find these limits in the [`ika-protocol-config` crate](https://github.com/MystenLabs/sui/blob/main/crates/ika-protocol-config/src/lib.rs) of the Ika repo. The limits are defined in the `ProtocolConfig` struct and values set in the `get_for_version_impl` function.
