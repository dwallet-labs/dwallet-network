---
title: Understand Ika Security
description: Ika is designed to provide very high security guarantees to asset owners. Assets on Ika can be used only by their owners, according to the logic pre-defined by smart contracts that can be audited, and that the network will be available to process them correctly despite some of the validators operating Ika not following the protocol correctly (fault tolerance).
---

This page provides an overview of the major guarantees Ika provides in terms of security.

Ika asset owners and smart contract designers can start learning here about the mechanisms available to secure their assets, and the assurances Ika provides for them. Smart contract designers can also learn about the overall Ika security architecture to ensure the asset types they design leverage Ika to provide a secure experience to the asset holders.

## Security features {#security-features}

Ika is designed to provide very high security guarantees to asset owners. Assets on Ika can be used only by their owners, according to the logic pre-defined by smart contracts that can be audited, and that the network will be available to process them correctly despite some of the validators operating Ika not following the protocol correctly (fault tolerance).

The security features of the Ika system ensure a number of properties:
...
- The Ika system operates through a protocol between a set of independent validators. Yet all its security properties are preserved when a small subset of the validators do not follow the protocol.
- All operations in Ika can be audited to ensure any assets have been correctly processed. This implies all operations on Ika are visible to all, and users may wish to use multiple different addresses to protect their privacy.
- Validators are determined periodically through users of Ika locking and delegating IKA tokens to one or more validators.

## Security architecture {#security-architecture}

The Ika system is operated by a set of validators that process transactions. They implement the Ika protocol that allows them to reach agreement on valid transactions submitted and processed in the system.

The agreement protocols Ika uses tolerate a fraction of validators not following the Ika protocol correctly, through the use of Byzantine fault tolerant broadcast and consensus. Specifically, each validator has some voting power, assigned to it through the process of users staking / voting for them using their IKA tokens. Ika maintains all its security properties if over 2/3 of the stake is assigned to validators that follow the protocol. However, a number of auditing properties are maintained even if more validators are faulty.

### Addresses and ownership {#addresses-and-ownership}

A Ika transaction is valid and can proceed only if the owner of all owned assets it operates on digitally signs it with their private signature key. This signature key can be kept private by the user and not be shared with anyone else. As a result, it is not feasible for any other party to operate on an owned asset of a user undetected, even if all validators do not follow the protocol.

A private signature key also corresponds to a public address on the Ika network that can be used to send a user assets or allow smart contracts to define custom access control logic. A user may have one or more addresses corresponding to multiple signature keys for convenience or privacy reasons. An address does not need any pre-registration, and sending an asset to an address automatically creates this address on the network. However, this means that users should be careful to check the recipient address of transfers, or parties involved in any other operations, as sending assets to an incorrect address may have irrevocable effects.

### Smart contracts define asset types and their logic {#smart-contracts}

All assets have a type that is defined within a Ika Smart Contract. Ika provides a few system contracts, such as those used to manage the IKA native token, yet also allows anyone to write and submit custom smart contracts. A transaction on an asset type can call operations defined in only the smart contract that defined the asset type, and is constrained by the logic in the contract.

For this reason, users are encouraged to operate on their assets using smart contracts they trust, that they or others they trust have audited, and understand the logic they define for operations on their assets. Ika smart contracts are defined as immutable assets to allow third parties to audit them and also prevent their modification to increase assurance.
