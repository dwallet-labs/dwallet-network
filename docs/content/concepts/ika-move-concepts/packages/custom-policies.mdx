...
To address the security risk of single-key upgrade ownership poses while still providing the opportunity to [upgrade live packages](./upgrade.mdx), Ika offers _custom upgrade policies_. These policies protect `UpgradeCap` access and issue `UpgradeTicket` objects that authorize upgrades on a case-by-case basis.

## Compatibility {#compatibility}

Ika comes with a set of built-in package compatibility policies, listed here from most to least strict:
...
You can change the current policy by calling one of the functions in `ika::package` (`only_additive_upgrades`, `only_dep_upgrades`, `make_immutable`) on the package's `UpgradeCap` and a policy can become only more restrictive. For example, after you call `ika::package::only_dep_upgrades` to restrict the policy to become additive, calling `ika::package::only_additive_upgrades` on the `UpgradeCap` of the same package results in an error.
...
While step 2 is a built-in command, steps 1 and 3 are implemented as Move functions. The Ika framework provides their most basic implementation:
...
These are the functions that `ika client upgrade` calls for authorization and commit. Custom upgrade policies work by guarding
...
bytecode or source code for ahead of time. Ika calculates the digest as follows:
...
Refer to the [implementation for digest calculation](https://github.com/MystenLabs/sui/blob/d8cb153d886d54752763fbdab631b062da7d894b/crates/ika-types/src/move_package.rs#L232-L251) for more information, but in most cases, you can rely on the Move toolchain to output the digest as part of the build, when passing the `--dump-bytecode-as-base64` flag:

```bash
ika move build --dump-bytecode-as-base64
...
The `UpgradeReceipt` is proof that the `Upgrade` command ran successfully, and Ika added the new package to the set of created
...
After Ika creates an `UpgradeReceipt`, you must use it to update its `UpgradeCap` within the same transaction (you cannot store it for later, drop it, or burn it), or the transaction fails.
...
The signature of a custom `authorize_upgrade` can be different from the signature of `ika::package::authorize_upgrade` as long as it returns an `UpgradeTicket`.

Finally, provide implementations of `commit_upgrade` and `make_immutable` that delegate to their respective functions in `ika::package`:
...
module policy::day_of_week {
    ...
    fun week_day(ctx: &TxContext): u8 {
        let days_since_unix_epoch =
            ika::tx_context::epoch_timestamp_ms(ctx) / MS_IN_DAY;
        // The unix epoch (1st Jan 1970) was a Thursday so shift days
        // since the epoch by 3 so that 0 = Monday.
        ((days_since_unix_epoch + 3) % 7 as u8)
    }
...
Use the `ika client publish` command to publish the policy.
...
----- Transaction Data ----
Transaction Signature: [Signature(Ed25519IkaSignature(Ed25519IkaSignature([0, 251, 96, 164, 70, 48, 195, 251, 181, 82, 206, 254, 167, 84, 165, 40, 29, 254, 102, 165, 152, 81, 244, 203, 199, 97, 33, 107, 29, 95, 120, 212, 34, 19, 233, 109, 179, 72, 246, 219, 23, 254, 108, 222, 210, 250, 166, 172, 208, 133, 108, 252, 36, 165, 71, 97, 210, 206, 144, 138, 237, 169, 15, 218, 13, 92, 225, 85, 204, 230, 61, 45, 147, 106, 193, 13, 195, 116, 230, 99, 61, 161, 251, 251, 68, 154, 46, 172, 143, 122, 101, 212, 120, 80, 164, 214, 54])))]
Transaction Kind : Programmable
Inputs: [Pure(IkaPureValue { value_type: Some(Address), value: "<SENDER>" })]
Commands: [
...
Array [
    Object {
        ...
        "objectType": String("0x2::coin::Coin<0x2::ika::IKA>"),
        "objectId": String("<GAS>"),
        "version": String("6"),
...
Following best practices, use the Ika Client CLI to call `ika::package::make_immutable` on the `UpgradeCap` to make the policy immutable.

```bash
ika client call --gas-budget 10000000 \
    --package 0x2 \
    --module 'package' \
...
----- Transaction Data ----
Transaction Signature: [Signature(Ed25519IkaSignature(Ed25519IkaSignature([0, 123, 97, 9, 252, 127, 238, 10, 88, 175, 157, 155, 98, 11, 23, 234, 52, 167, 230, 45, 218, 171, 31, 174, 87, 107, 174, 117, 236, 65, 117, 18, 42, 74, 56, 149, 82, 107, 216, 199, 223, 142, 135, 165, 200, 80, 151, 32, 110, 75, 133, 128, 150, 66, 13, 40, 173, 228, 211, 94, 222, 201, 248, 221, 10, 92, 225, 85, 204, 230, 61, 45, 147, 106, 193, 13, 195, 116, 230, 99, 61, 161, 251, 251, 68, 154, 46, 172, 143, 122, 101, 212, 120, 80, 164, 214, 54])))]
Transaction Kind : Programmable
...
If you don't have a package available, use the `ika move new` command to create the template for a new package called `example`.

```bash
ika move new example
```
...
Open a terminal or console to the root of your Node.js project. Run the following command to add the Ika TypeScript SDK as a dependency:
...
- `IKA`: the location of the `ika` CLI binary.
...
Next, add boilerplate code to get the signer key pair for the currently active address in the Ika Client CLI:
...
const sender = execSync(`${IKA} client active-address`, { encoding: 'utf8' }).trim();
const signer = (() => {
...
const { modules, dependencies } = JSON.parse(
	execSync(`${IKA} move build --dump-bytecode-as-base64 --path ${packagePath}`, {
		encoding: 'utf-8',
	}),
);
```
...
import { getFullnodeUrl, IkaClient } from '@ika-io/ika/client';
import { Ed25519Keypair } from '@ika-io/ika/keypairs/ed25519';
import { Transaction, UpgradePolicy } from '@ika-io/ika/transactions';
import { fromBase64 } from '@ika-io/ika/utils';
...
Use the Ika Client CLI to test the upgraded package (the package ID is **different** from the original version of your example package):
