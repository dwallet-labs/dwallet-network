...
Objects in Ika can have different types of ownership, with two broad categories: immutable objects and mutable objects. An immutable object is an object that can't be mutated, transferred, or deleted. Immutable objects have no owner, so anyone can use them.

## Create immutable object

To convert an object into an immutable object, call the `public_freeze_object` function from the [transfer module](https://github.com/MystenLabs/sui/blob/main/crates/ika-framework/packages/ika-framework/sources/transfer.move):
...
After an object becomes immutable, the rules of who can use this object in Ika Move calls change:
You can only pass an immutable object as a read-only, immutable reference to Ika Move entry functions as `&T`.
...
```shell
export ADDR=`ika client active-address`

ika client objects $ADDR
```

Publish the `ColorObject` code on-chain using the Ika Client CLI:

{@include: ../../snippets/info-gas-budget.mdx}

```shell
ika client publish $ROOT/examples/move/color_object --gas-budget <GAS-AMOUNT>
```

Set the package object ID to the `$PACKAGE` environment variable, if you have it set. Then create a new `ColorObject`:

```shell
ika client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "create" --args 0 255 0
```
...
```shell
ika client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "freeze_object" --args \"$OBJECT\"
```
...
```shell
ika client call --gas-budget <GAS-AMOUNT> --package $PACKAGE --module "color_object" --function "update" --args \"$OBJECT\" 0 0 0
```
