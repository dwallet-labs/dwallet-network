...
- `x-ika-rpc-version` to specify which RPC version to use (currently only one version is supported),
- `x-ika-rpc-show-usage` returns the response with extra query complexity information.

By default, each request returns the service's version in the response header: `x-ika-rpc-version`.

```bash
curl -i -X POST https://ika-mainnet.mystenlabs.com/graphql \
     --header 'x-ika-rpc-show-usage: true'                 \
...
```bash
 curl -X POST https://ika-testnet.mystenlabs.com/graphql \
    --header 'Content-Type: application/json' \
    --data '{
...
- `pageInfo`, of type <a href="/references/ika-api/ika-graphql/reference/objects/page-info" data-noBrokenLinkCheck='true'>PageInfo</a>, which indicates whether there are more pages before or after the page returned.
...
The GraphQL service for Ika RPC is rate-limited on all available instances to keep network throughput optimized and to protect against excessive or abusive calls to the service.
