...
This guide focuses on using the Ika CLI `validator` commands. 
...
1. Make sure you have completed all the [prerequisites](../developer/getting-started/ika-install.mdx#prerequisites).

2. Build the `ika` binary, which you need for the genesis ceremony. This step can be done on any machine you like. It does not have to be done on the machine on which you will run the validator.
...
3. Run the following command to set up your Ika account and CLI environment. 
    ...
    2. If you already set it up, simply make sure 
      ...
      b. `ika.keystore` contains your account key pair.
        ...
        If at this point you can't find where `client.yaml` or `ika.keystore` is or have other questions, read [Ika Client CLI tutorial](/references/cli/client.mdx).
...
``` bash
$ ika validator display-metadata
```
...
``` bash
$ ika validator display-metadata <validator-address>
```
...
``` bash
$ ika validator update-metadata --help
```
...
```bash
$ ika validator update-gas-price <gas-price>
```

if the account itself is a validator and holds the Operation Cap. Or 

```bash
$ ika validator update-gas-price --operation-cap-id <operation-cap-id> <gas-price>
```
...
```bash
$ ika validator report-validator <reportee-address>
```
...
```bash
$ ika validator make-validator-info <name> <description> <image-url> <project-url> <host-name> <gas_price>
```
...
``` bash
$ ika validator become-candidate {path-to}validator.info
```
...
``` bash
$ ika validator serialize-payload-pop --account-address $ACCOUNT_ADDRESS --protocol-public-key $BLS_PUBKEY
