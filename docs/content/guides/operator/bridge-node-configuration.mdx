---
title: Ika Bridge Validator Node Configuration
sidebar_label: Ika Bridge Node Configuration
---
...
To set up and run a Bridge Node, you need to install `ika` and `ika-bridge-cli`. You can install them using one of the following options: 

```sh
# install from tip of `main`
$ cargo install --locked --git "https://github.com/MystenLabs/sui.git" ika ika-bridge-cli
# install with a commit sha
$ cargo install --locked --git "https://github.com/MystenLabs/sui.git" --rev {SHA} ika ika-bridge-cli
```
...
- A REST API URL where the Bridge Node listens to and serves requests. Example: `https://bridge.example-ika-validator.io:443`. Make sure the port is correct and the URL does not contain any invalid characters, like quotes for example.

To create a `BridgeAuthorityKey`, run
```sh
$ ika-bridge-cli create-bridge-validator-key <PATH-TO-WRITE>
```
...
After you have both authority key file and REST API URL ready, you can register them by using Ika CLI:

```sh
$ ika validator register-bridge-committee --bridge-authority-key-path <BRIDGE-AUTHORITY-KEY-PATH> --bridge-authority-url <BRIDGE-AUTHORITY-URL>
```
...
```sh
$ ika client execute-signed-tx
```

#### Update metadata (before committee is finalized)

Both key and URL are changeable **before the committee is finalized**. If you wish to update metadata, simply rerun `ika validator register-bridge-committee`.
...
```sh
$ ika-bridge-cli view-bridge-registration --ika-rpc-url {IKA-FULLNODE-URL}
```
...
```sh
$ ika validator update-bridge-committee-node-url
```
...
Use `ika-bridge-cli` command to create a template. If you want to run `BridgeClient` (see the following section), pass `--run-client` as a parameter.

```sh
$ ika-bridge-cli create-bridge-node-config-template {PATH}
$ ika-bridge-cli create-bridge-node-config-template --run-client {PATH}
```
...
| `bridge-authority-key-path` | The path to the Bridge Validator key, generated from `ika-bridge-cli create-bridge-validator-key` command referenced previously. |
...
| `ika:ika-rpc-url` | Ika RPC URL. |
| `ika:ika-bridge-chain-id` | `0` for Ika Mainnet, `1` for Ika Testnet. |
...
| `metrics:push-url` | The url of the remote Ika metrics pipeline (for example, `https://metrics-proxy.[testnet_OR_mainnet].ika.io:8443/publish/metrics`). See the [metrics push section](#metrics-push) that follows for more details. | 
...
| `ika:bridge-client-key-path` | The file path of Bridge Client key. This key can be generated with `ika-bridge-cli create-bridge-client-key` as previously shown. When `run-client` is true but you do not provide `ika:bridge-client-key-path`, it defaults to use the Bridge Validator key to submit transactions on Ika. This is not recommended for the sake of key separation. | 

### Bridge Client

`BridgeClient` orchestrates bridge transfer requests. It is **optional** to run for a `BridgeNode`. `BridgeClient` submits transaction on the Ika network. Thus when it's enabled, you need a Ika account key with enough IKA balance.
...
```sh
ika-bridge-cli create-bridge-client-key <PATH_TO_BRIDGE_CLIENT_KEY>
```
This prints the newly created Ika Address. Then we need to fund this address with some IKA for operations.
...
- Use `cargo install`.
    ```sh
    $ cargo install --locked --git "https://github.com/MystenLabs/sui.git" --branch {BRANCH-NAME} ika-bridge
    # OR
    $ cargo install --locked --git "https://github.com/MystenLabs/sui.git" --rev {SHA-NAME} ika-bridge
    ```
- Compile from source code
    ...
    $ cargo build --release --bin ika-bridge
    ```
- Use `curl`/`wget` pre-built binaries (for Linux/AMD64 only).
    ```
    curl https://ika-releases.s3.us-east-1.amazonaws.com/{SHA}/ika-bridge -o ika-bridge
    ```
- Use pre-built Docker image. Pull from Docker Hub: `ika-io/ika-tools:{SHA}`


### Run Bridge Node

Running Bridge Node is similar to running a Ika node using systemd or Ansible. The command to start the Bridge Node is:

```sh
$ RUST_LOG=info,ika_bridge=debug ika-bridge --config-path {BRIDGE-NODE-CONFIG-PATH}
```
...
You can find a full list of Bridge Node metrics and their descriptions in the [`ika-bridge` crate](https://github.com/MystenLabs/sui/blob/main/crates/ika-bridge/src/metrics.rs).
