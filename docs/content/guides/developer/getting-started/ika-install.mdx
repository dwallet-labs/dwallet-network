---
title: Install Ika
description: Install the Ika framework and required prerequisites on your system, including the Ika command line interface to interact with the Ika network.
---

The quickest way to install Ika is using the binaries delivered with every release. If you require more control over the install process, you can install from source. To take advantage of containerization, you can utilize the Docker images in the `docker` folder of the ika repository.

## Supported operating systems {#supported-operating-systems}

Ika supports the following operating systems:
...
If you use this method to install Ika, you are all set. The quick install is ikatable for most use cases. The remaining installation methods are for those wanting more control over the installation process.
...
Each Ika release provides a set of binaries for several operating systems. You can download these binaries from GitHub and use them to install Ika.
...
1. In the right pane, find the **Releases** section.
    ...
    ![Ika releases in GitHub](./images/releases.png)
...
1. Extract all files from the .tgz file into the preferred location on your system. These instructions assume you extract the files into a `ika` folder at the user root of your system for demonstration purposes. Replace references to this location in subsequent steps if you choose a different directory.
1. Navigate to the expanded folder. You should have the following extracted files:

    {@include: ../../../snippets/binaries-file-list.mdx}

1. Add the folder containing the extracted files to your `PATH` variable. To do so, you can update your `~/.bashrc` to include the location of the Ika binaries. If using the suggested location, you type `export PATH=$PATH:~/ika` and press Enter.
...
1. Click the release tagged **Latest** to open the release's page.
1. In the **Assets** section of the release, select the .tgz compressed file that corresponds to your operating system.
1. Extract all files from the .tgz file into the preferred location on your system. These instructions assume you extract the files into a `ika` folder at the user root of your system. Replace references to this location in subsequent steps if you choose a different directory.
1. Navigate to the expanded folder. You should have the following extracted files: 
    
    {@include: ../../../snippets/binaries-file-list.mdx}

1. Add the folder containing the extracted files to your `PATH` variable. To do so, you can update your `~/.zshrc` or `~/.bashrc` to include the location of the Ika binaries. If using the suggested location, you type `export PATH=$PATH:~/ika` and press Enter.
1. Start a new terminal session or type `source ~/.zshrc` (or `.bashrc`) to load the new `PATH` value.
1. If running the binaries for the first time, you might receive an error from MacOS that prevents the binaries from running. If you receive this error, close the dialog and type `xattr -d com.apple.quarantine ~/ika/*` in your terminal and press Enter (be sure to adjust the path if different). 
...
1. Click the release tagged **Latest** to open the release's page.
1. In the **Assets** section of the release, select the .tgz compressed file that corresponds to your operating system.
1. Extract all files from the .tgz file into the preferred location on your system. These instructions assume you extract the files into a `ika` folder at the root of your C drive. Replace references to this location in subsequent steps if you choose a different directory.
...
1. In the Edit environment variable window, click **New** and add the path to your expanded folder. Using the example path, this would be `C:\ika`.
1. Click **OK**.
...
Running binaries other than `ika` might require installing prerequisites itemized in the following section. 

:::

#### Confirm the installation {#confirm-the-installation}

To confirm that Ika installed correctly, type `ika --version` in your console or terminal and press Enter. The response should provide the Ika version installed. If the console or terminal responds with a command not found error, make sure the full path to your Ika binaries is included in your `PATH` variable.

## Install from Cargo {#install-ika-binaries-from-source}

Run the following command to install Ika binaries from the `testnet` branch:

```shell
cargo install --locked --git https://github.com/MystenLabs/sui.git --branch testnet ika --features tracing
```

Enabling the `tracing` feature is important as it adds Move test coverage and debugger support in the Ika CLI. Without it these two features will not be able to be used.
...
The command installs Ika components in the `~/.cargo/bin` folder.

## Upgrade from Cargo

If you previously installed the Ika binaries, you can update them to the most recent release with the same command you used to install them (changing `testnet` to the desired branch):
...
Follow the instructions in this topic to install the Rust crates (packages) required to interact with Ika networks, including the Ika CLI.

To install Ika from source, you first need to install its [prerequisites](#prerequisites) for your operating system. After installing the supporting technologies, you can install [Ika binaries from source](#install-ika-binaries-from-source).

You can also download the [source code](/references/contribute/ika-environment.mdx) to have local access to files.

#### Prerequisites {#prerequisites}

Your system needs the following prerequisites available to successfully install Ika.

#### Rust and Cargo {#rust-and-cargo}

Ika requires Rust and Cargo (Rust's package manager) on all supported operating systems. The suggested method to install Rust is with `rustup` using cURL.
...
Ika uses the latest version of Cargo to build and manage dependencies. See the [Cargo installation](https://doc.rust-lang.org/cargo/getting-started/installation.html) page on the Rust website for more information.
...
Ika requires the following additional tools on computers running Windows.
...
**Known issue** - The `ika console` command does not work in PowerShell.

</TabItem>

</Tabs>

## Using Ika from command line

With Ika installed, you can interact with Ika networks using the Ika CLI. For more details, see the [Ika CLI](/references/cli.mdx) reference.

## Next steps {#next-steps}

Now that you have Ika installed, it's time to start developing. Check out the following topics to start working with Ika:

- Read about the [Ika CLI](/references/cli.mdx), the most straightforward way to start exploring Ika networks.
