...
Ika includes support for the Move testing framework. Using the framework, you can write unit tests that analyze Move code much like test frameworks for other languages, such as the built-in Rust testing framework or the JUnit framework for Java.

An individual Move unit test is encapsulated in a public function that has no parameters, no return values, and has the `#[test]` annotation. The testing framework executes such functions when you call the `ika move test` command from the package root (`my_move_package` directory as per the current running example):
...
Use a filter string to run only a matching subset of the unit tests. With a filter string provided, the `ika move test` checks the fully qualified (`<address>::<module_name>::<fn_name>`) name for a match.
...
## Ika-specific testing

The previous testing example uses Move but isn't specific to Ika beyond using some Ika packages, such as `ika::tx_context` and `ika::transfer`. While this style of testing is already useful for writing Move code for Ika, you might also want to test additional Ika-specific features. In particular, a Move call in Ika is encapsulated in a Ika transaction, and you might want to test interactions between different transactions within a single test (for example, one transaction creating an object and the other one transferring it).

Ika-specific testing is supported through the `test_scenario` module that provides Ika-related testing functionality otherwise unavailable in pure Move and its testing framework.

The `test_scenario` module provides a scenario that emulates a series of Ika transactions, each with a potentially different user executing them. A test using this module typically starts the first transaction using the `test_scenario::begin` function. This function takes an address of the user executing the transaction as its argument and returns an instance of the `Scenario` struct representing a scenario.

An instance of the `Scenario` struct contains a per-address object pool emulating Ika object storage, with helper functions provided to manipulate objects in the pool. After the first transaction finishes, subsequent test transactions start with the `test_scenario::next_tx` function. This function takes an instance of the `Scenario` struct representing the current scenario and an address of a user as arguments.

Update your `example.move` file to include a function callable from Ika that implements `sword` creation. With this in place, you can then add a multi-transaction test that uses the `test_scenario` module to test these new capabilities. Put this functions after the accessors (Part 5 in comments).

{@inject: examples/move/first_package/sources/example.move#fun=sword_create noComments}

The code of the new functions uses struct creation and Ika-internal modules (`tx_context`) in a way similar to what you have seen in the previous sections. The important part is for the function to have correct signatures.
...
- No return values
- Private visibility
- Optionally, the parameter list starts by accepting the module's [one-time witness by value](../../../concepts/ika-move-concepts/one-time-witness.mdx)
...
While the `ika move` command does not support publishing explicitly, you can still test module initializers using the testing framework by dedicating the first transaction to executing the initializer function.
...
- [Publish a Package](./publish.mdx): Continue the example by publishing your package to the Ika network.
- [Package Upgrades](../../../concepts/ika-move-concepts/packages.mdx): Upgrading packages published on the Ika network.
