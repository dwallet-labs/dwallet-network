...
Before you can call functions in a Move package (beyond an emulated Ika execution scenario), that package must be available on the Ika network. When you publish a package, you are actually creating an immutable Ika object on the network that anyone can access.

To publish your package to the Ika network, use the `publish` CLI command in the root of your package. Use the `--gas-budget` flag to set a value for the maximum amount of gas the transaction can cost. If the cost of the transaction is more than the budget you set, the transaction fails and your package doesn't publish. 

{@include: ../../../snippets/info-gas-budget.mdx}

```shell
$ ika client publish --gas-budget 5000000
```
...
Your currently active address now has three objects (or more, if you had objects prior to this example). Assuming you are using a new address, running the `ika objects` command reveals what those objects are.
...
Now that the package is on chain, you can call its functions to interact with the package. You can use the `ika client call` command to make individual calls to package functions, or you can construct more advanced blocks of transactions using the `ika client ptb` command. The `ptb` part of the command stands for [programmable transaction blocks](../../../concepts/transactions/prog-txn-blocks.mdx). In basic terms, PTBs allow you to group commands together in a single transaction for more efficient and cost-effective network activity.

```mermaid
flowchart TB
    my_module["my_module::new_sword(&Forge, strength, magic)"]
    
    Ika_client["Ika client"]
...
After the transaction executes, you can check the status of the `Sword` object by using the `ika client objects` command again. Provided you used your address as the `<TO-ADDRESS>`, you should now see a total of four objects:
...
Congratulations! You have successfully published a package to the Ika network and modified the blockchain state by using a programmable transaction block. 
...
- [Ika Client PTB CLI](../../../references/cli/ptb.mdx): The `client ptb` command allows you to specify the transactions for execution in a programmable transaction block directly from your CLI or through bash scripts.
