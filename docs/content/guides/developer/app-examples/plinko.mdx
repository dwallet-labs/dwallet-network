...
sequenceDiagram
    ...
    participant Blockchain as Ika Blockchain
...
The `plinko::plinko` module combines various Ika blockchain features, such as coin handling, event emissions, and dynamic object fields, to create on-chain Plinko games.
...
public fun start_game(counter: &mut Counter, num_balls: u64, coin: Coin<IKA>, house_data: &mut HouseData, ctx: &mut TxContext): ID {
...
public fun finish_game(game_id: ID, bls_sig: vector<u8>, house_data: &mut HouseData, num_balls: u64, ctx: &mut TxContext): (u64, address, vector<u8>) {
    ...
    let stake_per_ball = stake.value<IKA>() / num_balls;
    ...
    // Divide by 100 to adjust for multiplier scale and IKA units
...
let payout_coin: Coin<IKA> = coin::take(payout_balance_mut, total_funds_amount, ctx);
...
fun internal_start_game(counter: &mut Counter, num_balls: u64, coin: Coin<IKA>, house_data: &HouseData, fee_bp: u16, ctx: &mut TxContext): (ID, Game) {
...
public fun initialize_house_data(house_cap: HouseCap, coin: Coin<IKA>, public_key: vector<u8>, multiplier: vector<u64>, ctx: &mut TxContext) {
    ...
    let mut house_data = HouseData {
        ...
        house: ctx.sender(),
        public_key,
        max_stake: 10_000_000_000, // 10 IKA = 10^9.
