...
description: An app that performs atomic swaps on Ika. Atomic swaps are similar to escrows but without requiring a trusted third party.
effort: large
---

{@include: ../../../snippets/app-examples-swap-source.mdx}

This guide demonstrates how to make an app that performs atomic swaps on Ika. Atomic swaps are similar to escrows but without requiring a trusted third party.
...
- [Installed the latest version of Ika](../getting-started/ika-install.mdx).
...
- Read the basics of [shared versus owned objects](../ika-101/shared-owned.mdx).
...
- You have the latest version of Ika installed. If you run `ika --version` in your terminal or console, it responds with the currently installed version.
- Your active environment is pointing to the expected network. Run `ika client active-env` to make sure. If you receive a warning about a client and server API version mismatch, update Ika using the version in the relevant branch (`mainnet`, `testnet`, `devent`) of the Ika repo.
- Your active address has IKA. Run `ika client balance` in your terminal or console. If there is no balance, [acquire IKA](../getting-started/get-coins.mdx) from the faucet (not available in Mainnet).
...
In this part of the guide, you write the Move contracts that perform the trustless swaps. The guide describes how to create the package from scratch, but you can use a fork or copy of the example code in the Ika repo to follow along instead. See [Write a Move Package](../first-app/write-package.mdx) to learn more about package structure and how to use the Ika CLI to scaffold a new project.
...
If you are targeting a network other than Testnet, be sure to update the `rev` value for the Ika dependency.
...
The test starts with a helper function for creating an object, it doesn't matter what kind of object it is, as long as it has the `store` ability. The test uses `Coin<IKA>`, because it comes with a `#[test_only]` function for minting:
...
- The `test_scenario` module is used to provide access to a `&mut TxContext` in the test (necessary for creating new objects). Tests that don't need to simulate multiple transactions but still need access to a `TxContext` can use `ika::tx_context::dummy` to create a test context instead.
...
From your `escrow` folder, run `ika move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass:
...
- Concepts: [Shared Object Deletion](https://blog.ika.io/ephemeral-shared-objects/)
...
`Locked` and `Key` objects are typically owned by the transaction sender, and so can be queried through the Ika RPC, but `Escrow` objects are shared, and it is useful to be able to query them by their sender and recipient (so that users can see the trades they have offered and received).
...
{@inject: examples/trading/contracts/escrow/sources/shared.move#use=ika::event}
...
- Guide: [Using Events](../ika-101/using-events.mdx)

:::

:::checkpoint

You now have `shared.move` and `locked.move` files in your `sources` folder. From the parent `escrow` folder, run `ika move test` in your terminal or console. If successful, you get a response similar to the following that confirms the package builds and your tests pass: 
...
- Reference: [JSON-RPC](https://docs.ika.io/ika-api-ref)
...
3. Deploy the Ika package

  <details>
    <summary>
      Deployment instructions
    </summary>
    {@include: ../../../snippets/initialize-ika-client-cli.mdx}

  Next, configure the Ika CLI to use `testnet` as the active environment. 
  ...
  Before being able to publish your package to Testnet, you need Testnet IKA tokens. To get some, run the following command: 

  ```bash
  ika client faucet
  ```

  For other ways to get IKA in your Testnet account, see [Get IKA Tokens](/guides/developer/getting-started/get-coins).

  Now that you have an account with some Testnet IKA, you can deploy your contracts. 

  There are some helper functions to publish the smart contracts so you can create some demo data (for Testnet). The helper function to publish the smart contrqcts expects built smart contracts in both the `escrow` and `demo` directories. Run `ika move build` in both directories, if necessary. Be sure to update the Ika dependency in the manifest to point to the correct source based on your environment. 
...
- Tooling: [Ika Typescript SDK](https://sdk.mystenlabs.com/typescript). For basic usage on how to interact with Ika with TypeScript.
- Tooling: [Ika dApp Kit](https://sdk.mystenlabs.com/dapp-kit). To learn basic building blocks for developing a dApp in the Ika ecosystem with React.js.
- Tooling: [`@mysten/dapp`](https://sdk.mystenlabs.com/dapp-kit/create-dapp). This is used within this project to quickly scaffold a React-based Ika dApp.
...
<details>
  <summary>
    `IkaObjectDisplay.tsx`
  </summary>
  {@inject: examples/trading/frontend/src/components/IkaObjectDisplay.tsx}
</details>
...
Note that `dApp Kit` provides a set of hooks for making query and mutation calls to the Ika blockchain. These hooks are thin wrappers around query and mutation hooks from `@tanstack/react-query`.
...
Fetch the owned objects directly from Ika blockchain using the `useIkaClientInfiniteQuery()` hook from `dApp Kit`. This hook is a thin wrapper around Ika blockchain RPC calls, reference the documentation to learn more about these [RPC hooks](https://sdk.mystenlabs.com/dapp-kit/rpc-hooks). Basically, supply the RPC endpoint you want to execute, in this case it's the [`getOwnedObjects` endpoint](https://docs.ika.io/ika-api-ref#ikax_getownedobjects). Supply the connected wallet account as the `owner`. The returned data is stored inside the cache at query key `getOwnedObjects`. In a future step you invalidate this cache after a mutation succeeds, so the data will be re-fetched automatically.
...
Use the `useIkaClient()` hook from `dApp Kit` to retrieve the Ika client instance configured in `src/main.tsx`. The `useSignTransaction()` function is another hook from `dApp kit` that helps to sign the transaction block using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction block, use the `executeTransaction()` on the Ika client instance of the Ika TypeScript SDK.
...
As previously mentioned, this example uses `@tanstack/react-query` to query, cache, and mutate server state. Server state is data only available on remote servers, and the only way to retrieve or update this data is by interacting with these remote servers. In this case, it could be from an API or directly from Ika blockchain RPC.

When you execute a transaction call to mutate data on the Ika blockchain, use the `useMutation()` hook. The `useMutation()` hook accepts several inputs, however, you only need two of them for this example. The first parameter, `mutationFn`, accepts the function to execute the main mutating logic, while the second parameter, `onSuccess`, is a callback that runs when the mutating logic succeeds.
...
This component uses tanstack's `useInfiniteQuery` instead of `useIkaClientInfiniteQuery` since the data is being fetched from the example's API rather than Ika. 
