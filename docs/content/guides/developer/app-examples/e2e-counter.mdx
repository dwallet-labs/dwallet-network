...
This example walks you through building a basic distributed counter app, covering the full end-to-end flow of building your Ika Move module and connecting it to your React Ika dApp. The app allows users to create counters that anyone can increment, but only the owner can reset. 
...
Learn more about taking [object references as input](../../../concepts/ika-move-concepts.mdx#entry-points)
...
Your smart contract is complete. You should be able to run the `ika move build` command from `react-e2e-counter/move/counter` and receive a response similar to the following:
```sh
...
BUILDING counter
```
You always run `ika move build` at the same level as your `Move.toml` file. After a successful build, you now have a `build` folder inside `react-e2e-counter/move/counter`.
...
The `create` function now creates a new Ika `Transaction` and calls the `create` function from your Move module. The PTB is then signed and executed via the `useSignAndExecuteTransaction` hook. The `onCreated` callback is called with the new counter's ID when the transaction is successful.
