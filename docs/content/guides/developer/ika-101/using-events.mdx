...
The Ika network stores countless objects on chain where Move code can perform actions using those objects. Tracking this activity is often desired, for example, to discover how many times a module mints an NFT or to tally the amount of IKA in transactions that a smart contract generates.

To support activity monitoring, Move provides a structure to emit events on the Ika network. You can then leverage a custom indexer to process checkpoint data that includes events that have been emitted. See the [custom indexer](../advanced/custom-indexer.mdx) topic in the Advanced section to learn how to stream checkpoints and filter events continuously. 

If you don't want to run a custom indexer, you can poll the Ika network to query for emitted events instead. This approach typically includes a database to store the data retrieved from these calls. The [Poll events](#poll-events) section provides an example of using this method.

## Move event structure

An event object in Ika consists of the following attributes:
...
- `sender`: The Ika network address that triggered the event.
...
To create an event in your Move modules, add the `ika::event` dependency.
...
The Ika RPC provides a [queryEvents](/ika-api-ref#ikax_queryEvents) method to query on-chain packages and return available events. As an example, the following `curl` command queries the Deepbook package on Mainnet for a specific type of event:

```sh
curl -X POST https://fullnode.mainnet.ika.io:443 \
	...
	"result": {
		"data": [
			...
			},
			{
				...
				"transactionModule": "deepbook_utils",
				"sender": "0x8b35e67a519fffa11a9c74f169228ff1aa085f3a3d57710af08baab8c02211b9",
				"type": "0xdee9::clob_v2::OrderFilled<0x2::ika::IKA, 0x5d4b302506645c37ff133b98c4b50a5ae14841659738d6d733d59d0d217a93bf::coin::COIN>",
				"parsedJson": {
...
The TypeScript SDK provides a wrapper for the `ikax_queryEvents` method: [`client.queryEvents`](https://sdk.mystenlabs.com/typedoc/classes/_mysten_ika.client.IkaClient.html#queryEvents).
...
The [Ika by Example](https://github.com/gdanezis/ika-by-example/blob/main/src/05_reading_events/bin/main.rs) repo on GitHub contains a code sample that demonstrates how to query events using the `query_events` function. The package that `PACKAGE_ID_CONST` points to exists on Mainnet, so you can test this code using Cargo. To do so, clone the `ika-by-example` repo locally and follow the [Example 05 directions](https://github.com/gdanezis/ika-by-example/tree/main/src/05_reading_events). 

```rust
use ika_sdk::{rpc_types::EventFilter, types::Identifier, IkaClientBuilder};

const PACKAGE_ID_CONST: &str = "0x279525274aa623ef31a25ad90e3b99f27c8dbbad636a6454918855c81d625abc";

#[tokio::main]
async fn main() -> Result<(), anyhow::Error> {
    let ika_mainnet = IkaClientBuilder::default()
        .build("https://fullnode.mainnet.ika.io:443")
        .await?;

    let events = ika_mainnet
        .event_api()
        .query_events(
...
You can use GraphQL to query events instead of JSON RPC. The following example queries are in the [`ika-graphql-rpc` crate](https://github.com/MystenLabs/sui/tree/main/crates/ika-graphql-rpc/examples/event_connection) in the Ika repo. 

<details>
<summary>
Event connection
</summary>
{@inject: crates/ika-graphql-rpc/examples/event_connection/event_connection.graphql}
</details>
...
{@inject: crates/ika-graphql-rpc/examples/event_connection/filter_by_sender.graphql}
</details>
...
{@inject: crates/ika-graphql-rpc/examples/event_connection/filter_by_emitting_package_module_and_event_type.graphql}
</details>

The [TypeScript SDK](https://sdk.mystenlabs.com/typedoc/modules/_mysten_ika.graphql.html) provides functionality to interact with the Ika GraphQL service.
...
- Incorporate a [custom indexer](../advanced/custom-indexer.mdx) to take advantage of Ika's micro-data ingestion framework.
- Poll the Ika network on a schedule to query events.
...
To monitor events, you need a database to store checkpoint data. The [Trustless Swap](../app-examples/trustless-swap.mdx) example uses a Prisma database to store checkpoint data from the Ika network. The database is populated from polling the network to retrieve emitted events. 
