...
description: Use the borrow module in the Ika framework to include objects by reference in your programmable transaction blocks.
---

Everything on the Ika blockchain is an object. When you develop Move packages for the Ika network, you're typically manipulating or using on-chain objects in some way through functionality available in the Ika API. For most API functions, you provide an object by reference.

References are a key construct when programming in Move and on Ika. Most of the functionality available in the Ika API takes objects by reference.
