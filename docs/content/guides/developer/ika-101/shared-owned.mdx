...
description: On Ika, you can use shared objects, owned objects, or both in your transactions. Weigh the benefits of each to decide the best approach for your project.
---

Objects on Ika can be shared (accessible for reads and writes by any transaction) or owned (accessible for reads and writes by transactions signed by their owner).  Many applications can be built using a solution that either uses shared objects or only owned objects, with trade-offs for each that need to be weighed.
...
For more information on the types of objects that Ika supports, see [Object Ownership](/concepts/object-ownership.mdx).
