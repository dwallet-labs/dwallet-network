...
Please, provide any feedback or report any issues you encounter via [GitHub](https://github.com/MystenLabs/sui/issues/new/choose), [Discord](https://discord.gg/Ika), or [Telegram](https://t.me/IkaTokenNetwork).
...
1. Delete your existing `Move.lock` file (if one exists) to make sure you're using the newest `ika-framework` version.
1. Perform one of the following:
   - Run `ika move migrate` in the root of your Move project. See [Automatic migration](#automatic-migration).
...
See the getting started guide on [Move IDEs and plugins](../../../references/contribute/ika-environment.mdx#move-ides-and-plugins) for more information.
...
```move
use ika::{balance, coin::{Self, Coin}};
```
...
Move 2024 revises how paths and namespaces work compared to legacy Move, toward easing `enum` aliasing in the future. Consider the following snippet from a test annotation in the `ika_system` library:
...
Legacy Move would always treat a three-part name as an address(`ika_system`), module(`validator_set`), and module member (`EInvalidCap`). Move 2024 respects scope for `use`, so `ika_system` in the attribute resolves to the module, producing a name resolution error overall.
