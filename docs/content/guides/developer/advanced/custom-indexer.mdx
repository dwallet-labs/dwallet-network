...
description: You can build custom indexers using the Ika micro-data ingestion framework. To create an indexer, you subscribe to a checkpoint stream with full checkpoint content. Establishing a custom indexer helps improve latency, allows pruning the data of your Ika Full node, and provides efficient assemblage of checkpoint data.
---

You can build custom indexers using the Ika micro-data ingestion framework. To create an indexer, you subscribe to a checkpoint stream with full checkpoint content. This stream can be one of the publicly available streams from Mysten Labs, one that you set up in your local environment, or a combination of the two.  

Establishing a custom indexer helps improve latency, allows pruning the data of your Ika Full node, and provides efficient assemblage of checkpoint data.
...
- Testnet: `https://checkpoints.testnet.ika.io`
- Mainnet: `https://checkpoints.mainnet.ika.io`

The checkpoint files are stored in the following format: `https://checkpoints.testnet.ika.io/<checkpoint_id>.chk`. You can download the checkpoint file by sending an HTTP GET request to the relevant URL. Try it yourself for checkpoint 1 at [https://checkpoints.testnet.ika.io/1.chk](https://checkpoints.testnet.ika.io/1.chk).
...
The Ika data ingestion framework provides a helper function to quickly bootstrap an indexer workflow.

{@inject: examples/custom-indexer/rust/remote_reader.rs}

This is ikatable for setups with a single ingestion pipeline where progress tracking is managed outside of the framework.
...
To enable, add the following to your [Full node configuration](../../operator/ika-full-node.mdx) file:
...
executor.run(
    ...
    Some("https://checkpoints.testnet.ika.io".to_string()), // Remote Checkpoint Store
...
Find the following source code in the [Ika repo](https://github.com/mystenlabs/sui/tree/main/examples/custom-indexer/rust). 
...
- [Ika internal example](https://github.com/MystenLabs/sui/tree/main/crates/ika-data-ingestion/src/): Ika data ingestion daemon that runs internal pipelines.
- [Production example](https://github.com/MystenLabs/sui/tree/main/crates/ikans-indexer/src): Ika Name Service custom indexer.
- [Using Events](../ika-101/using-events.mdx): Events in Ika enable you to monitor on-chain activity in near-real time when coupled with a custom indexer.
