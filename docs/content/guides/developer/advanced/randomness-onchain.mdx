...
`Random` has a reserved address `0x8`. See <a href="/references/framework/ika-framework/random" data-noBrokenLinkCheck='true'>random.move</a> for the Move APIs for accessing randomness on Ika.
...
entry fun insecure_play(r: &Random, payment: Coin<IKA>, ...) {
  ...
...
module games::dice {
  ...
  public fun play_dice(guess: u8, fee: Coin<IKA>, r: &Random, ctx: &mut TxContext): Ticket {
    // Pay for the turn
...
For example, consider the `entry play_dice(guess: u8, fee: Coin<IKA>, r: &Random, ctx: &mut TxContext): Ticket { … }` function defined earlier, the attacker can publish the function
...
To protect against PTB-based composition attacks, Ika rejects PTBs that have commands that are not `TransferObjects` or `MergeCoins` following a `MoveCall` command that uses `Random` as an input.
...
- [Ika Framework Reference](../../../references/framework)
  - <a href="/references/framework/ika-framework/random" data-noBrokenLinkCheck="true">
    	random.move
    </a>
- [Raffle example](https://github.com/MystenLabs/sui/blob/main/examples/move/random/raffles)
- [Ika Client CLI](../../../references/cli/client)
