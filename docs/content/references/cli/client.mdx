---
title: Ika Client CLI
description: The Ika Client CLI provides command-level access to interact with the Ika network.
---

The Ika CLI `client` command provides command-level access to interact with the Ika network. Typical uses for `ika client` include publishing Move smart contracts, getting the information of an object, executing transactions, or managing addresses.

{@include: ../../snippets/cli-check-install.mdx}

## Commands

The following list itemizes all the available subcommands for the `ika client` command.

```
Usage: ika client [OPTIONS] [COMMAND]
...
Commands:
  ...
  envs                        List all Ika environments
  ...
  new-env                     Add new Ika environment
  ...
  pay-all-ika                 Pay all residual IKA coins to the recipient with input coins, after deducting the gas cost. The input coins also include the coin for gas payment, so no extra gas coin is required
  pay-ika                     Pay IKA coins to recipients following following specified amounts, with input coins. Length of recipients must be the same as that of amounts. The input coins also include the coin for
  ...
  transfer-ika                Transfer IKA, and pay gas with the same IKA coin object. If amount is specified, only the amount is transferred; otherwise the entire object is transferred
...
Append the `--json` flag to commands to format responses in JSON instead of the more human-friendly default Ika CLI output. This can be useful for extremely large datasets, for example, as those results can have a troublesome display on smaller screens. In these cases, the `--json` flag is useful.
...
Use the `ika client envs` command to find the network environments set up in the CLI. The information for these environments is also stored in the client.yaml file in the Ika configuration directory (`~/.ika/ika_config`).
...
Use `client new-env` to add details for a new network environment. This example creates an environment pointer to Ika Mainnet. Setting the `alias` value makes referencing the environment less prone to typographical errors. After running this command, Ika updates your client.yaml file in `~/.ika/ika_config` with the new information.

```
$ ika client new-env --alias=mainnet --rpc https://fullnode.mainnet.ika.io:443

Added new Ika env [mainnet] to config.
```

### Set current environment

Use the `ika client switch` command to change the current network. This example switches the current network to `mainnet`.

```shell
$ ika client switch --env mainnet

Active environment switched to [mainnet]
```

If you run `ika client envs` after this command, you see the asterisk in the `active` column on the `mainnet` row of the table.

### Get current active address

Use the `ika client active-address` command to reveal the current address. The CLI uses the current active address to execute address-specific CLI commands (like `ika client objects`) when you don't provide them with a Ika address value.

```shell
$ ika client active-address
0x514692f08249c3e9951234ce29074695840422564bff85e424b56de462913e0d
```

### Request a IKA coin from faucet

If you use one of the standard public RPCs (for example, `fullnode.testnet.ika.io:443`), you can use the `faucet` command to request gas coins. If you use a custom faucet service, then pass in the URL to the faucet using the `--url` option.

```shell
$ ika client faucet
Request successful. It can take up to 1 minute to get the coin. Run ika client gas to check your gas coins.
```

### Get objects owned by an address

Use `ika client objects` to list summary information about the objects the current active address owns. You can provide a Ika address value to the command to list objects for a particular address. This example lists objects for the current active address.

```
$ ika client objects 0x36df11369cf00ecf0be68d6ba965b0abe2e883bc5245911e3a29ebfa0aaf6b69
...
Use `ika client object <OBJECT-ID>` to list object information for the ID you provide. This example displays the information for a Coin object.

```
$ ika client object 0xfffbb30ccb631f15f6cd36700589fc9c31cb04af28a95f3ed26d62daf3acb57f
╭───────────────┬─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╮
│ objectId  	  │  0xfffbb30ccb631f15f6cd36700589fc9c31cb04af28a95f3ed26d62daf3acb57f                                             	  │
│ version   	  │  33363559                                                                                                       	  │
│ digest    	  │  3FzvXJVVVcXb9H6dEXdARaY9EmxXyyNFduet3X4eYV4x                                                                   	  │
│ objType   	  │  0x2::coin::Coin<0x2::ika::IKA>                                                                                 	  │
...
Use the `ika client dynamic-field <DYNAMIC-FIELD-ID>` command to list the details of the dynamic field with the ID you provide.

```
$ ika client dynamic-field 0x5
╭─────────────┬───────────────────────────────────────────────────────────────────────────────────────────╮
│ hasNextPage │  false                                                                                	  │
...
│         	  │ │ │ bcsName	   │  LQM2cdzDY3                                                      	  │ │ │
│         	  │ │ │ type   	   │  DynamicField                                                    	  │ │ │
│         	  │ │ │ objectType │  0x3::ika_system_state_inner::IkaSystemStateInnerV2              	  │ │ │
...
In this example, let's see how to transfer IKA or transfer an object from one address to another. First of all, there two main commands for sending IKA or transferring objects: `pay` and `transfer`. 
Both `pay` and `transfer` have a few sister commands: `pay-ika`, `pay-all-ika`, `transfer-ika`. 

The differences between these commands are:
- commands that end in `-ika` deal with Ika's native coin, and they use the input coints to pay for gas and for transferring IKA or the object.
...
- `pay-all-ika` is a special case of `pay-ika` that offers a way to transfer the entire balance after smashing.
- `transfer-ika` is a legacy command and has been entirely superseded by `pay-ika` or `pay-all-ika` depending on whether an amount is specified or not.
...
```
ika client gas hungry-spodumene
╭────────────────────────────────────────────────────────────────────┬────────────────────┬──────────────────╮
│ gasCoinId                                                          │ nikaBalance (NIKA) │ ikaBalance (IKA) │
├────────────────────────────────────────────────────────────────────┼────────────────────┼──────────────────┤
...
You want to send 0.5 IKA to `eloquent-amber`. Given that you have a few gas coins, you can use `pay`. If only one gas coin exists, then you need to use `transfer-ika` or `pay-ika`, or you would need to split the coin first to have another coin to use for paying gas. In this case, let's use the `pay-ika` command as you do not need to provide a separate gas coin to be used for the gas fees. In the command below, you set the recipient to be `eloquent-amber`, which coin to use to transfer IKA from, and the amount of IKA to transfer.

{@include: ../../snippets/info-gas-budget.mdx}

```
ika client pay-ika --recipients eloquent-amber --input-coins 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --amounts 500000000 --gas-budget 5000000
```
...
│  └──                                                                                              │
│ Gas Cost Summary:                                                                                 │
│    Storage Cost: 1976000 NIKA                                                                     │
│    Computation Cost: 1000000 NIKA                                                                 │
│    Storage Rebate: 978120 NIKA                                                                    │
│    Non-refundable Storage Fee: 9880 NIKA                                                          │
│                                                                                                   │
...
│  │ Sender: 0x0fe375fff0ee40d20c54a7f2478b9b5c7eaa3625b7611f9661ec5faefb4a6fea                    │
│  │ Owner: Account Address ( 0x3d78abc33ccd173c521b4f9e78b21ea2c989960b631732c539efaa38bda30976 ) │
│  │ ObjectType: 0x2::coin::Coin<0x2::ika::IKA>                                                    │
│  │ Version: 3                                                                                    │
│  │ Digest: Fq3maqq3pShgKw58Ssm8cS2j1pspfhanedVK2MMxfQWQ                                          │
│  └──                                                                                             │
│ Mutated Objects:                                                                                 │
...
Check the gas for the sender and the recipient's coins using `ika client gas <address>`. The sender now has 199.49 IKA for that gas coin that was used. 0.5 IKA was transferred, and the remaining 0.01 IKA paid the gas fees.
```
ika client gas
╭────────────────────────────────────────────────────────────────────┬────────────────────┬──────────────────╮
│ gasCoinId                                                          │ nikaBalance (NIKA) │ ikaBalance (IKA) │
...
If you want to transfer the whole object, you can use `ika client pay-all-ika` or `ika client transfer-ika` (without passing the amount): 
```
ika client pay-ika --recipient eloquent-amber --input-coins 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --gas-budget 5000000
or
ika client transfer-ika --to eloquent-amber --ika-coin-object-id 0xc9b447fff18f13fa035e028534b8344d5fc8a8760248fad10155e78f44dc3a52 --gas-budget 5000000
```
...
Use the `ika client replay-transaction --tx-digest <TRANSACTION-DIGEST>` to re-execute a transaction locally and show the transaction effects.
...
```
$ ika client replay-transaction --tx-digest 51MzJP2Uesvza8vXGpPCGbfLrY6UCfdvdoErN1z4oXPW
╭───────────────────────────────────────────────────────────────────────────────────────────────────╮
│ Transaction Effects                                                                               │
├───────────────────────────────────────────────────────────────────────────────────────────────────┤
...
Use `ika client replay-batch --path <FILEPATH>` to replay several transactions listed in a newline-separated file. This
will verify that all transactions local execution results match the effects on-chain.

### Profile a transaction

Use the `ika client profile-transaction --tx-digest <TRANSACTION-DIGEST>` command to re-execute a transaction locally
...
To enable the profiler, you must either install or build the Ika Client binary locally with the `--features tracing` flag if it has not already been built with that feature enabled.
...
One of the main uses of the `ika client` command is to publish smart contracts on the Ika network. This example switches the current environment to the Devnet network, then builds, tests, and publishes one of the existing Move examples available in the Ika repository: [ika/examples/move](https://github.com/MystenLabs/sui/tree/main/examples/move)

This example also makes use of `ika move` commands. To learn more about those commands, see [Ika Move CLI](./move.mdx).

1. Open a terminal or console to the root of your local Ika repository and navigate to the `move_tutorial` example.
    ```shell
    cd examples/move
    cd first_package
    ```
1. Switch to the Devnet network. This command uses an alias, so the `devnet` value might be different for you, depending on the alias name set in your configuration (use `ika client envs` for a list of your defined networks and their aliases).
    ```shell
    ika client switch --env devnet
    ```
1. Use `ika move build` to build the package. You must run this command at the same level as the package manifest file ([Move.toml](https://move-book.com/concepts/manifest.html)). The console responds with the status of the build.
    ```shell
    ika move build
    ...
    ```
1. Use `ika move test` to run the unit tests. The console responds with updates of its progress.
    ```shell
    ika move test
    INCLUDING DEPENDENCY Ika
    INCLUDING DEPENDENCY MoveStdlib
    ...
    Test result: OK. Total tests: 2; passed: 2; failed: 0
     ```
1. Use the `ika client verify-bytecode-meter` to check if the module passes the bytecode meter. The console responds with the maximum allowed values, as well as the amount the package uses.
    ```shell
    $ ika client verify-bytecode-meter
    ...
    ╰────────┴────────────┴────────────╯
    ```
1. Use `ika client gas` to verify that the active address has a gas coin for paying gas. In the case of this example, the console responds with the information that the address is coinless.
    ```shell
    $ ika client gas
    No gas coins are owned by this address
    ```
1. If you need coins, use `ika client faucet` (not available for Mainnet). For more information on getting gas tokens, see [Get Ika Tokens](/guides/developer/getting-started/get-coins.mdx).
    ```shell
    $ ika client faucet
    ```
1. Use `ika client gas` to verify the current active address received the coins.
    ```shell
    $ ika client gas
    ╭────────────────────────────────────────────────────────────────────┬─────────────╮
    ...
    ╰────────────────────────────────────────────────────────────────────┴─────────────╯
    ```
1. Use `ika client publish` to publish the package, being sure to set an appropriate value for the `gas-budget` flag. The console responds with the details of the publish. You can use `ika client object <OBJECT-ID>` to check the details of any of the objects from the process.
    ```shell
    $ ika client publish --gas-budget 100000000 .
...
Each command has its own help section. For example, `ika client call --help` displays the following prompt:

```
Call Move function

Usage: ika client call [OPTIONS] --package <PACKAGE> --module <MODULE> --function <FUNCTION> --gas-budget <GAS_BUDGET>
...
Options:
  	...
  	--serialize-unsigned-transaction  Instead of executing the transaction, serialize the bcs bytes of the unsigned transaction data (TransactionData) using base64 encoding, and print out
                                    	the string <TX_BYTES>. The string can be used to execute transaction with `ika client execute-signed-tx --tx-bytes <TX_BYTES>`.
  	--serialize-signed-transaction	Instead of executing the transaction, serialize the bcs bytes of the signed transaction data (SenderSignedData) using base64 encoding, and print out the
                                    	string <SIGNED_TX_BYTES>. The string can be used to execute transaction with `ika client execute-combined-signed-tx --signed-tx-bytes <SIGNED_TX_BYTES>`.
  	--json                        	Return command outputs in json format
  -h, --help                        	Print help
```
