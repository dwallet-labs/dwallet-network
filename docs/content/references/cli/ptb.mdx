---
title: Ika Client PTB CLI
description: The Ika Client PTB CLI enables a user to construct a PTB and execute it from the command line or a file.
---
...
The following list itemizes all the available args for the `ika client ptb` command. Use the `--help` for a long help version that includes some examples on how to use this command.
...
Options:
      ...
      --gas-budget <NIKA>
          An optional gas budget for this PTB (in NIKA). If gas budget is not provided, the tool will first perform a dry run to estimate the gas cost, and then it will execute the transaction. Please note that this incurs a small
...
```bash
ika client ptb --move-call "$PKG::m::f" '"Hello, world"' --gas-budget 10000000 --preview
```
...
```bash
ika client ptb --transfer-objects [ARRAY_OF_OBJECTS] @0x02a212de6a9dfa3a69e22387acfbafbb1a9e591bd9d636e7895dcfc8de05f331 --gas-coin @0x00002819ee07a66e53800495ccf5eeade8a02054a2e0827546c70e4b226f0495
```
...
CLI PTB uses name resolution for common packages like `ika`, `std`, `deepbook`, so you can use them directly instead of their addresses: `0x2`, `0x1`, or `0xdee9`.

:::

In the second case, if a previous command outputs some result, you can bound it to a variable for later access. Let's see an example where you want a new coin with 1000 NIKA, which you can achieve by using the `split-coins` command. After you do that, you want to transfer the new coin to another address. Without the `--assign` argument, you couldn't instruct the CLI to transfer that new coin object as you would not have a way to refer to it.
...
When needing to execute a Move call, use the `--move-call` transaction to call a specific function from a package. The CLI PTB supports name resolution for common packages like `ika`, `std`, `deepbook`, so you can use both `0x1::option::is_none` as well as `std::option::is_none` for passing the function name.
...
Publishing a package is one of the most important commands you need when working with Ika. While the CLI has a standalone `publish` command, PTBs also support publishing and upgrading packages. One main difference is that with `ika client ptb`, you must explicitly transfer the `UpgradeCap` object that is returned when creating a package, or destroy it with a call to [`make_immutable`](/concepts/ika-move-concepts/packages.mdx). Here is an example on how to publish a Move project on chain using the `ika client ptb` command. It makes a call to the `ika::tx_context::sender` to acquire the sender and assigns the result of that call to the `sender` variable, and then calls the publish command. The result of `publish` is bounded to `upgrade_cap` variable, and then this object is transferred to the sender.
...
--move-call ika::coin::destroy_zero<ika::ika::IKA> zcoins.0
--move-call ika::coin::destroy_zero<ika::ika::IKA> zcoins.1
