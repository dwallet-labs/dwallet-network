---
title: Ika Move CLI
description: The Ika CLI move command provides commands for working with Move source code directly from a terminal or console.
---

The Ika CLI `move` command provides several commands for working with Move source code. A typical usage of `ika move` is to compile and test the Move code, or to generate a new Move project by using `ika move new project_name`, which creates the needed directories and the `Move.toml` file.

{@include: ../../snippets/cli-check-install.mdx}

## Commands

Typing `ika move --help` into your terminal or console displays the following information on available commands.

```
Usage: ika move [OPTIONS] <COMMAND>
...
To create a new Move project that automatically adds the necessary dependencies in a `Move.toml` file, run `ika move new [<PROJECT-NAME>]`.

```shell
$ ika move new smart_contract_test
...
Use `ika move build` at the root of your Move project to build the package.
...
Use `ika move test` to run the tests in a Move package.
...
To get the a summary of the test coverage, you must first run the `ika move test --coverage` command, and then the `ika move coverage summary --test` to get a summary of the test coverage in the example project.

```shell
$ ika move test --coverage
INCLUDING DEPENDENCY Ika
INCLUDING DEPENDENCY MoveStdlib
...
$ ika move coverage summary --test
+-------------------------+
...
Each command has its own help section. For example `ika move build –help` displays the following prompt:

```shell
$ ika move build --help
Usage: ika move build [OPTIONS]
