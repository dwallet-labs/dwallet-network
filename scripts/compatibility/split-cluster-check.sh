...
if [ -f "$WORKING_DIR/pera-node-release" ] && [ -f "$WORKING_DIR/pera-release" ] && [ -f "$WORKING_DIR/pera-node-candidate" ]; then
  echo "Binaries already built, skipping build"
else
  echo "Building pera-node and pera at $RELEASE_COMMIT"

  # remember current commit
  CURRENT_COMMIT=$(git rev-parse HEAD)

  git checkout $RELEASE_COMMIT || exit 1
  cargo build --bin pera-node --bin pera || exit 1
  cp ./target/debug/pera-node "$WORKING_DIR/pera-node-release"
  cp ./target/debug/pera "$WORKING_DIR/pera-release"

  echo "Building pera-node at $RELEASE_CANDIDATE_COMMIT"
  git checkout $RELEASE_CANDIDATE_COMMIT || exit 1
  cargo build --bin pera-node || exit 1
  cp ./target/debug/pera-node "$WORKING_DIR/pera-node-candidate"

  echo "returning to $CURRENT_COMMIT"
  git checkout $CURRENT_COMMIT || exit 1
fi

export PERA_CONFIG_DIR="$WORKING_DIR/config"
rm -rf "$PERA_CONFIG_DIR"

"$WORKING_DIR/pera-release" genesis --epoch-duration-ms 20000
...
while IFS= read -r -d '' file; do
  CONFIGS+=("$file")
done < <(find "$PERA_CONFIG_DIR" -name "127.0.0.1*.yaml" -print0)

export RUST_LOG=pera=debug,info

# 2 release nodes
"$WORKING_DIR/pera-node-release" --config-path "${CONFIGS[0]}" > "$LOG_DIR/node-0.log" 2>&1 &
"$WORKING_DIR/pera-node-release" --config-path "${CONFIGS[1]}" > "$LOG_DIR/node-1.log" 2>&1 &

# 2 candidate nodes
"$WORKING_DIR/pera-node-candidate" --config-path "${CONFIGS[2]}" > "$LOG_DIR/node-2.log" 2>&1 &
"$WORKING_DIR/pera-node-candidate" --config-path "${CONFIGS[3]}" > "$LOG_DIR/node-3.log" 2>&1 &

# and a fullnode
"$WORKING_DIR/pera-node-release" --config-path "$PERA_CONFIG_DIR/fullnode.yaml" > "$LOG_DIR/fullnode.log" 2>&1 &
