...
echo "pera-test-validator binary has been deprecated in favor of pera start, which is a more powerful command that allows you to start the local network with more options.
This script offers backward compatibiltiy, but ideally, you should migrate to pera start instead. Use pera start --help to see all the flags and options. 

To recreate the exact basic functionality of pera-test-validator, you must use the following options:
...
# Basic command that replicates the command line arguments of pera-test-validator
cmd="pera start --with-faucet --force-regenesis"
