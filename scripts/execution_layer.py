...
def parse_args():
    ...
    generate_lib = subparsers.add_parser(
        "generate-lib",
        help=(
            "Generate `pera-execution/src/lib.rs` based on the current set of "
...
def do_cut(args):
    ...
    update_toml(args.feature, Path() / "pera-execution" / "Cargo.toml")
    generate_impls(args.feature, impl_module)

    with open(Path() / "pera-execution" / "src" / "lib.rs", mode="w") as lib:
        generate_lib(lib)
...
def do_generate_lib(args):
    if args.dry_run:
        ...
        lib_path = Path() / "pera-execution" / "src" / "lib.rs"
...
def cut_command(f):
    ...
    return [
        ...
        *["-d", f"pera-execution/latest:pera-execution/{f}:-latest"],
        *["-d", f"external-crates/move:external-crates/move/move-execution/{f}"],
        *["-p", "pera-adapter-latest"],
        *["-p", "pera-move-natives-latest"],
        *["-p", "pera-verifier-latest"],
...
def cut_directories(f):
    ...
    pera_base = Path() / "pera-execution"
    external = Path() / "external-crates"

    crates = [
        pera_base / f / "pera-adapter",
        pera_base / f / "pera-move-natives",
        pera_base / f / "pera-verifier",
    ]
...
def impl(feature):
    ...
    return Path() / "pera-execution" / "src" / (feature.replace("-", "_") + ".rs")
...
def update_toml(feature, toml_path):
    """Add dependencies for 'feature' to pera-execution's manifest."""
...
def generate_impls(feature, copy):
    ...
    orig = Path() / "pera-execution" / "src" / "latest.rs"
...
def generate_lib(output_file: TextIO):
    ...
    Generates the contents of pera-execution/src/lib.rs to assign a numeric
    ...
    template_path = Path() / "pera-execution" / "src" / "lib.template.rs"
    cuts = discover_cuts()
...
# Modules in `pera-execution` that don't count as "cuts" (they are
...
def discover_cuts():
    ...
    Finds all modules within the `pera-execution` crate that count as
