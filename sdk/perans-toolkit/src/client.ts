...
import type { DataFields, NameObject, NetworkType, PeraNSContract } from './types/objects.js';
...
class PeransClient {
	private peraClient: PeraClient;
	contractObjects: PeraNSContract | undefined;
	networkType: NetworkType | undefined;

	constructor(
		peraClient: PeraClient,
		options?: {
			contractObjects?: PeraNSContract;
			networkType?: NetworkType;
		},
	) {
		if (!peraClient) {
			throw new Error('PeraClient must be specified.');
		}
		this.peraClient = peraClient;
		this.contractObjects = options?.contractObjects;
		this.networkType = options?.networkType;
	}

	async getPeransContractObjects() {
		if ((this.contractObjects as PeraNSContract)?.packageId) return;
		...
		try {
			...
			throw new Error(`Error getting PeraNS contract objects, ${(error as Error).message}`);
		}
	...
	protected async getDynamicFieldObject(
		...
		const dynamicFieldObject = await this.peraClient.getDynamicFieldObject({
	...
	protected async getNameData(dataObjectId: string, fields: DataFields[] = []) {
		...
		const { data: dynamicFields } = await this.peraClient.getDynamicFields({
	...
	async getNameObject(
		...
		await this.getPeransContractObjects();

		const registryResponse = await this.getDynamicFieldObject(
			(this.contractObjects as PeraNSContract).registry,
		...
		if (nameObject.nftId && (includeAvatar || options?.showOwner)) {
			const [owner, avatarNft] = await Promise.all([
				getOwner(this.peraClient, nameObject.nftId),
				includeAvatar ? getAvatar(this.peraClient, nameObject.avatar) : Promise.resolve(null),
			]);
	 ...
	 *
	 * @param domain a domain name ends with `.pera`
	 */
