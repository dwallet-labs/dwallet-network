...
import { PeransClient } from '../src';

const domainName = 'test.pera';
const walletAddress = '0xfce343a643991c592c4f1a9ee415a7889293f694ab8828f78e3c81d11c9530c6';

describe('PeraNS Client', () => {
	const client = new PeransClient(new PeraClient({ url: getFullnodeUrl('testnet') }), {
		networkType: 'testnet',
		contractObjects: {
			...
			perans: '0x4acaf19db12fafce1943bbd44c7f794e1d81d00aeb63617096e5caa39499ba88',
		},
	});

	const nonExistingDomain = walletAddress + '.pera';
	const nonExistingWalletAddress = walletAddress.substring(0, walletAddress.length - 4) + '0000';

	beforeEach(async () => {
		await client.getPeransContractObjects();
	});
	...
	describe('getNameObject', () => {
		it('returns related data of the name', async () => {
			expect(
				...
				// avatar: 'https://api-testnet.perafrens.pera.io/perafrens/0x4e3ba002444df6c6774f41833f881d351533728d585343c58cca1fec1fef74ef/svg',
