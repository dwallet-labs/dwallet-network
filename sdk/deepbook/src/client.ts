...
import type { OrderArguments, PaginatedEvents, PaginationArguments } from '@ika-io/ika/client';
import { getFullnodeUrl, IkaClient } from '@ika-io/ika/client';
import type { Argument, TransactionObjectInput, TransactionResult } from '@ika-io/ika/transactions';
...
import {
	...
	MODULE_CLOB,
	MODULE_CUSTODIAN,
	NORMALIZED_IKA_COIN_TYPE,
	ORDER_DEFAULT_EXPIRATION_IN_MS,
	PACKAGE_ID,
} from './utils/index.js';

const DUMMY_ADDRESS = normalizeIkaAddress('0x0');

export class DeepBookClient {
	#poolTypeArgsCache: Map<string, string[]> = new Map();
	/**
	 *
	 * @param ikaClient connection to fullnode
	 ...
	 */
	constructor(
		public ikaClient: IkaClient = new IkaClient({ url: getFullnodeUrl('testnet') }),
	 ...
	 * @param coinId the coin used for the deposit. You can omit this argument if you are depositing IKA, in which case
	...
	async deposit(
		...
		const hasIka =
			baseAsset === NORMALIZED_IKA_COIN_TYPE || quoteAsset === NORMALIZED_IKA_COIN_TYPE;

		if (coinId === undefined && !hasIka) {
			throw new Error('coinId must be specified if neither baseAsset nor quoteAsset is IKA');
		}
		...
		const coinType = coinId ? await this.getCoinType(coinId) : NORMALIZED_IKA_COIN_TYPE;
	...
	async getAllPools(
		...
		const resp = await this.ikaClient.queryEvents({
	...
	async getPoolInfo(poolId: string): Promise<PoolSummary> {
		const resp = await this.ikaClient.getObject({
			id: poolId,
	...
	async getOrderStatus(
		...
		const results = (
			await this.ikaClient.devInspectTransactionBlock({
				transactionBlock: tx,
	...
	async getUserPosition(
		...
		tx.moveCall({
			...
			arguments: [tx.object(normalizeIkaObjectId(poolId)), tx.object(cap)],
		});
	...
	async getLevel2BookStatus(
		...
		const results = await this.ikaClient.devInspectTransactionBlock({
			transactionBlock: tx,
			sender: this.currentAddress,
		});
	...
	#checkAccountCap(accountCap: string | undefined = undefined): string {
		...
		return normalizeIkaObjectId(cap);
	}
	...
	#checkAddress(recipientAddress: string): string {
		...
		return normalizeIkaAddress(recipientAddress);
	}
	...
	public async getCoinType(coinId: string) {
		...
		if (
			parsed?.address === NORMALIZED_IKA_COIN_TYPE.split('::')[0] &&
			parsed.module === 'coin' &&
			parsed.name === 'Coin' &&
