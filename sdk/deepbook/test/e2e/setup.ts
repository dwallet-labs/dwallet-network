...
import { FLOAT_SCALING_FACTOR, NORMALIZED_IKA_COIN_TYPE } from '../../src/utils/index.js';
...
export async function publishPackage(packagePath: string, toolbox?: TestToolbox) {
	...
	const { modules, dependencies } = JSON.parse(
		execSync(
			`${IKA_BIN} move move --client.config ${toolbox.configPath} build --dump-bytecode-as-base64 --path ${packagePath} --install-dir ${tmpobj.name}`,
			{ encoding: 'utf-8' },
		),
	);
...
export async function setupPool(toolbox: TestToolbox): Promise<PoolSummary> {
	...
	const quoteAsset = NORMALIZED_IKA_COIN_TYPE;
...
export async function setupDeepbookAccount(toolbox: TestToolbox): Promise<string> {
	...
	const accountCap = ((resp.objectChanges?.filter(
		(a) => a.type === 'created',
	) as IkaObjectChangeCreated[]) ?? [])[0].objectId;
	return accountCap;
}
