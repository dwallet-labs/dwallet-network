...
- **`client`** (optional) - The `@ika-io/ika` client used to fetch data to construct the link. If
  not provided, a default client will be used.
- **`network`** (optional) - The Ika network that the link will be created for. Defaults to
  `mainnet`.
...
#### Adding IKA to the link

You can add IKA to the link by calling `link.addClaimableNIka()`. This method takes the following
params:

- **`amount`** (required) - The amount of NIKA (the base unit of IKA) to add to the link.

#### Adding non-IKA coins to the link

You can add non-IKA coins to the link by calling `link.addClaimableBalance()`. This method takes the
following params:

- **`coinType`** (required) - The coin type of the coin to add to the link (e.g. `0x2::ika::IKA`).
...
`stashed_address` query parameter to the URL, containing the Ika address of the user that claimed
