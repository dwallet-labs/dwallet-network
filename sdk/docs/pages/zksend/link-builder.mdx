...
- **`client`** (optional) - The `@pera-io/pera` client used to fetch data to construct the link. If
...
#### Adding PERA to the link

You can add PERA to the link by calling `link.addClaimableNPera()`. This method takes the following
params:

- **`amount`** (required) - The amount of NPERA (the base unit of PERA) to add to the link.

#### Adding non-PERA coins to the link

You can add non-PERA coins to the link by calling `link.addClaimableBalance()`. This method takes the
following params:

- **`coinType`** (required) - The coin type of the coin to add to the link (e.g. `0x2::pera::PERA`).
...
`stashed_address` query parameter to the URL, containing the Pera address of the user that claimed
