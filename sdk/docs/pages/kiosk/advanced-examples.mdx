...
const bullsharkType = `${packageId}::ikafrens::IkaFren<${packageId}::bullshark::Bullshark>`;
// a constant for capy's type.
const capyType = `${packageId}::ikafrens::IkaFren<${packageId}::capy::Capy>`;
...
This example demonstrates how to mint a IkaFren.
...
async function mintFren(address: string) {
	...
	tx.moveCall({
		target: `${packageId}::ikafrens::mint_app::mint`,
