...
const bullsharkType = `${packageId}::perafrens::PeraFren<${packageId}::bullshark::Bullshark>`;
// a constant for capy's type.
const capyType = `${packageId}::perafrens::PeraFren<${packageId}::capy::Capy>`;
...
This example demonstrates how to mint a PeraFren.
...
async function mintFren(address: string) {
	...
	tx.moveCall({
		target: `${packageId}::perafrens::mint_app::mint`,
