# Network Interactions with IkaClient

The Ika TypeScript SDK provides a `IkaClient` class to connect to a network's JSON-RPC server. Use
`IkaClient` for all JSON-RPC operations.

## Connecting to a Ika network

To establish a connection to a network, import `IkaClient` from `@ika-io/ika/client` and pass the
relevant URL to the `url` parameter. The following example establishes a connection to Testnet and
requests IKA from that network's faucet.
...
const client = new IkaClient({ url: rpcUrl });
...
For local development, you can run `cargo run --bin ika -- start --with-faucet --force-regenesis` to
spin up a local network with a local validator, a Full node, and a faucet server. Refer to
[the Local Network guide](https://docs.ika.io/guides/developer/getting-started/local-network) for
more information.

## Manually calling unsupported RPC methods

You can use `IkaClient` to call any RPC method the node you're connecting to exposes. Most RPC
methods are built into `IkaClient`, but you can use `call` to leverage any methods available in the
RPC.
...
// asynchronously call ikax_getCommitteeInfo
const committeeInfo = await client.call('ikax_getCommitteeInfo', []);
```

For a full list of available RPC methods, see the
[RPC documentation](https://docs.ika.io/references/ika-api).

## Customizing the transport

The `IkaClient` uses a `Transport` class to manage connections to the RPC node. The default
`IkaHTTPTransport` makes both JSON RPC requests, as well as websocket requests for subscriptions.
...
`IkaClient` exposes a number of RPC methods that return paginated results. These methods return a
...
In addition to the RPC methods mentioned above, `IkaClient` also exposes some methods for working
with Transactions.
