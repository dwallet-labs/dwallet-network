# Network Interactions with PeraClient

The Pera TypeScript SDK provides a `PeraClient` class to connect to a network's JSON-RPC server. Use
`PeraClient` for all JSON-RPC operations.

## Connecting to a Pera network

To establish a connection to a network, import `PeraClient` from `@pera-io/pera/client` and pass the
relevant URL to the `url` parameter. The following example establishes a connection to Testnet and
requests PERA from that network's faucet.
...
const client = new PeraClient({ url: rpcUrl });
...
For local development, you can run `cargo run --bin pera -- start --with-faucet --force-regenesis` to
spin up a local network with a local validator, a Full node, and a faucet server. Refer to
[the Local Network guide](https://docs.pera.io/guides/developer/getting-started/local-network) for
more information.

## Manually calling unsupported RPC methods

You can use `PeraClient` to call any RPC method the node you're connecting to exposes. Most RPC
methods are built into `PeraClient`, but you can use `call` to leverage any methods available in the
RPC.
...
// asynchronously call perax_getCommitteeInfo
const committeeInfo = await client.call('perax_getCommitteeInfo', []);
```

For a full list of available RPC methods, see the
[RPC documentation](https://docs.pera.io/references/pera-api).

## Customizing the transport

The `PeraClient` uses a `Transport` class to manage connections to the RPC node. The default
`PeraHTTPTransport` makes both JSON RPC requests, as well as websocket requests for subscriptions.
...
`PeraClient` exposes a number of RPC methods that return paginated results. These methods return a
...
In addition to the RPC methods mentioned above, `PeraClient` also exposes some methods for working
with Transactions.
