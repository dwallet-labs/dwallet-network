# Paying for Ika Transactions with Gas Coins

With Programmable Transactions, you can use the gas payment coin to construct coins with a set
balance using `splitCoin`. This is useful for Ika payments, and avoids the need for up-front coin
...
[by-reference](https://docs.ika.io/guides/developer/ika-101/simulating-refs). Practically speaking,
...
By default, the gas price is set to the computation price per unit size of the network. You can also explicitly
...
**Note:** The gas budget is represented in Ika, and should take the gas price of the transaction
into account.
