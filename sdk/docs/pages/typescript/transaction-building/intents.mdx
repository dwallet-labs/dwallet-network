...
The `CoinWithBalance` intent makes it easy to get a coin with a specific balance. For PERA, this has
...
This approach works well for PERA, but can't be used for other coin types. The CoinWithBalance intent
...
```typescript
import { coinWithBalance, Transaction } from '@pera-io/pera/transactions';

const tx = new Transaction();

// Setting the sender is required for the CoinWithBalance intent to resolve coins when not using the gas coin
tx.setSender(keypair.toPeraAddress());

tx.transferObjects(
	[
		// Create a PERA coin (balance is in NPERA)
...
transfer PERA that does not use the gas coin, you can set the `useGasCoin` option to `false`:
...
coinWithBalance intent may use all the PERA coins, leaving no coins to use for gas.
