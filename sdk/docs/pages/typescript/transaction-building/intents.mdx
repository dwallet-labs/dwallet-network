...
The `CoinWithBalance` intent makes it easy to get a coin with a specific balance. For IKA, this has
...
This approach works well for IKA, but can't be used for other coin types. The CoinWithBalance intent
...
```typescript
import { coinWithBalance, Transaction } from '@ika-io/ika/transactions';

const tx = new Transaction();

// Setting the sender is required for the CoinWithBalance intent to resolve coins when not using the gas coin
tx.setSender(keypair.toIkaAddress());

tx.transferObjects(
	[
		// Create a IKA coin (balance is in NIKA)
...
transfer IKA that does not use the gas coin, you can set the `useGasCoin` option to `false`:
...
coinWithBalance intent may use all the IKA coins, leaving no coins to use for gas.
