# Pera Programmable Transaction Basics

This example starts by constructing a transaction to send PERA. To construct transactions, import the
...
tx.transferObjects([coin], '0xSomePeraAddress');
```
...
// procure a list of some Pera transfers to make
...
[the PeraClient docs for more details](/typescript/pera-client#arguments)). These options will cause
...
Pera supports following transactions:
...
tx.transferObjects([coin], tx.pure.address('0xSomePeraAddress'));
tx.transferObjects([coin], tx.pure(bcs.Address.serialize('0xSomePeraAddress')));
```
...
When building a transaction, Pera expects all objects to be fully resolved, including the object
...
In most cases, building requires your PeraClient to fully resolve input values.
