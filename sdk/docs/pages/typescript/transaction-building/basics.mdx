# Ika Programmable Transaction Basics

This example starts by constructing a transaction to send IKA. To construct transactions, import the
...
tx.transferObjects([coin], '0xSomeIkaAddress');
```
...
[the IkaClient docs for more details](/typescript/ika-client#arguments)). These options will cause
...
Ika supports following transactions:
...
tx.transferObjects([coin], tx.pure.address('0xSomeIkaAddress'));
tx.transferObjects([coin], tx.pure(bcs.Address.serialize('0xSomeIkaAddress')));
```
...
When building a transaction, Ika expects all objects to be fully resolved, including the object
...
In most cases, building requires your IkaClient to fully resolve input values.
