...
The `@ika-io/ika/bcs` package extends `@mysten/bcs` with Ika specific scheme definitions.

To learn more about using BCS see the [BCS documentation](/bcs).

the `bcs` export of `@ika-io/ika/bcs` contains all the same exports as `bcs` from `@mysten/bcs` plus
