...
To fund transactions, the `SerialTransactionExecutor` will select all of the senders PERA coins for
...
- `client`: An instance of `PeraClient` used to execute transactions.
...
import { SerialTransactionExecutor } from '@pera-io/pera/transactions';
...
<Callout type="warning">
	Using PeraClient or wallets to execute additional transactions while `ParallelTransactionExecutor`
...
- `coinBatchSize`: The maximum number of new coins to create when refilling the gas pool
  (default 20)
- `initialCoinBalance`: The balance of new coins created for the gas pool in NPERA (default
  `200_000_000n`),
...
```ts
import { getFullnodeUrl, PeraClient } from '@pera-io/pera/client';
import { ParallelTransactionExecutor } from '@pera-io/pera/transactions';
