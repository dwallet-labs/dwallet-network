...
To fund transactions, the `SerialTransactionExecutor` will select all of the senders IKA coins for
...
- `client`: An instance of `IkaClient` used to execute transactions.
...
import { SerialTransactionExecutor } from '@ika-io/ika/transactions';
...
<Callout type="warning">
	Using IkaClient or wallets to execute additional transactions while `ParallelTransactionExecutor`
...
- `coinBatchSize`: The maximum number of new coins to create when refilling the gas pool
  (default 20)
- `initialCoinBalance`: The balance of new coins created for the gas pool in NIKA (default
  `200_000_000n`),
...
```ts
import { getFullnodeUrl, IkaClient } from '@ika-io/ika/client';
import { ParallelTransactionExecutor } from '@ika-io/ika/transactions';
