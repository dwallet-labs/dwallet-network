...
- [A new GraphQL Client for the newly released Ika GraphQL API](../graphql)
...
The majority of changes in this release are in the `@ika-io/ika` package, but the changes to API
...
## Changes to `@ika-io/ika`

As part of the 1.0 release the `@ika-io/ika.js` package has been renamed to `@ika-io/ika`

To upgrade to the new version, uninstall the old package and install the new one:

```sh npm2yarn
npm uninstall @ika-io/ika.js
...
```diff
- import { IkaClient } from '@ika-io/ika.js'
+ import { IkaClient } from '@ika-io/ika'
```
...
coming to other parts of the IKA ecosystem in the future.
...
- `ikaClient.waitForTransactionBlock({ transactionBlock })` ->
  `ikaClient.waitForTransaction({ transaction })`
- `ikaClient.signAndExecuteTransactionBlock({ transactionBlock })` ->
  `ikaClient.signAndExecuteTransaction({ transaction })`

There are a few methods on IkaClient that retain the `TransactionBlock` name because they correspond
to specific JSON RPC calls. The following methods are unchanged:

- `ikaClient.getTransactionBlock`
- `ikaClient.queryTransactionBlocks`
- `ikaClient.executeTransactionBlock`
- `ikaClient.dryRunTransactionBlock`
- `ikaClient.devInspectTransactionBlock`
...
```diff
- import { Transactions } from '@ika-io/ika/transactions'
+ import { Commands } from '@ika-io/ika/transactions'
...
- [`TransactionObjectArgument`](/typedoc/types/_mysten_ika.transactions.TransactionObjectArgument.html)
  shape changed
- [`TransactionResult`](/typedoc/types/_mysten_ika.transactions.TransactionResult.html) shape
  changed
- [`TransactionObjectInput`](/typedoc/types/_mysten_ika.transactions.TransactionObjectInput.html)
  shape changed
- [`TransactionArgument`](/typedoc/types/_mysten_ika.transactions.TransactionArgument.html) shape
  changed
- [`TransactionBlockInput`](/typedoc/types/_mysten_ika.transactions.TransactionBlockInput.html)
  shape changed
...
The `publicKeyFromIkaBytes` has been moved to `@ika-io/ika/verify`

```diff
- import { publicKeyFromIkaBytes } from '@ika-io/ika/multisig'
...
`ikaClient.executeTransactionBlock` for returning additional data.
...
+ 		await ikaClient.executeTransactionBlock({
...
`showRawEffects` to true if using `ikaClient.executeTransactionBlock`, or by returning an object
...
[wallet-standard documentation](https://docs.ika.io/standards/wallet-standard).

The old `ika:signTransactionBlock` and `ika:signAndExecuteTransactionBlock` methods have been
deprecated in favor of the new `ika:signTransaction` and `ika:signAndExecuteTransaction` methods,
