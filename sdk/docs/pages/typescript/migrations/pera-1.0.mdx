...
- [A new GraphQL Client for the newly released Pera GraphQL API](../graphql)
...
The majority of changes in this release are in the `@pera-io/pera` package, but the changes to API
...
## Changes to `@pera-io/pera`

As part of the 1.0 release the `@pera-io/pera.js` package has been renamed to `@pera-io/pera`

To upgrade to the new version, uninstall the old package and install the new one:

```sh npm2yarn
npm uninstall @pera-io/pera.js
...
```diff
- import { PeraClient } from '@pera-io/pera.js'
+ import { PeraClient } from '@pera-io/pera'
```
...
coming to other parts of the PERA ecosystem in the future.
...
- `peraClient.waitForTransactionBlock({ transactionBlock })` ->
  `peraClient.waitForTransaction({ transaction })`
- `peraClient.signAndExecuteTransactionBlock({ transactionBlock })` ->
  `peraClient.signAndExecuteTransaction({ transaction })`

There are a few methods on PeraClient that retain the `TransactionBlock` name because they correspond
to specific JSON RPC calls. The following methods are unchanged:

- `peraClient.getTransactionBlock`
- `peraClient.queryTransactionBlocks`
- `peraClient.executeTransactionBlock`
- `peraClient.dryRunTransactionBlock`
- `peraClient.devInspectTransactionBlock`
...
```diff
- import { Transactions } from '@pera-io/pera/transactions'
+ import { Commands } from '@pera-io/pera/transactions'
...
- [`TransactionObjectArgument`](/typedoc/types/_mysten_pera.transactions.TransactionObjectArgument.html)
  shape changed
- [`TransactionResult`](/typedoc/types/_mysten_pera.transactions.TransactionResult.html) shape
  changed
- [`TransactionObjectInput`](/typedoc/types/_mysten_pera.transactions.TransactionObjectInput.html)
  shape changed
- [`TransactionArgument`](/typedoc/types/_mysten_pera.transactions.TransactionArgument.html) shape
  changed
- [`TransactionBlockInput`](/typedoc/types/_mysten_pera.transactions.TransactionBlockInput.html)
  shape changed
...
The `publicKeyFromPeraBytes` has been moved to `@pera-io/pera/verify`

```diff
- import { publicKeyFromPeraBytes } from '@pera-io/pera/multisig'
...
`peraClient.executeTransactionBlock` for returning additional data.
...
`showRawEffects` to true if using `peraClient.executeTransactionBlock`, or by returning an object
...
[wallet-standard documentation](https://docs.pera.io/standards/wallet-standard).

The old `pera:signTransactionBlock` and `pera:signAndExecuteTransactionBlock` methods have been
deprecated in favor of the new `pera:signTransaction` and `pera:signAndExecuteTransaction` methods,
