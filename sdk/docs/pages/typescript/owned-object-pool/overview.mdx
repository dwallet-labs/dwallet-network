# Ika Owned Object Pools Overview
...
scaling or concurrency for a service that executes transactions on Ika in the natural way results in
...
[gas smashing](https://docs.ika.io/concepts/transactions/gas-smashing) and the Ika TypeScript SDK's
default coin selection logic, which uses all the `0x2::coin::Coin<0x2::ika::IKA>` objects owned by
...
The basic concept of IkaOOP is to provide a `ExecutorServiceHandler` to use multiple worker pools
...
As an example, assume that you need to execute 10 transactions that transfer 100 NIKA each to a
fixed recipient.

Before you can run this example, you need to already have at least one coin of type
`0x2::coin::Coin<0x2::ika::IKA>` in your wallet for each transaction that you need to execute in
parallel (in our case 10 coins). Each `Coin<IKA>` should have enough balance to execute each
transaction. If you need IKA for a test network, you can
[use a faucet](https://docs.ika.io/guides/developer/getting-started/get-coins) to mint some.
...
```ts
import { ExecutorServiceHandler } from 'ikaoop';
...
![IkaOOP processing flow](/ika-exec-handler-flowchart.png)
