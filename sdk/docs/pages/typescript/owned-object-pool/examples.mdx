...
These examples demonstrate some possible use cases for the Ika Owned Object Pool library.

## Use case 1: Parallel coin transfers service—Multiple Coins

Assume that you have a service that needs to make payments of size 50000000 NIKA to multiple
...
import type { IkaObjectRef, IkaObjectResponse } from '@ika-io/ika/client/';
import { Transaction } from '@ika-io/ika/transactions';

const client = new IkaClient({
	url: 'https://fullnode.testnet.ika.io:443',
});
...
async function splitCoinAndTransferToSelf(
	...
	tx.transferObjects(
		...
		tx.pure(getKeyPair(yourAddressSecretKey).getPublicKey().toIkaAddress()),
	);
	tx.setGasPayment([toIkaObjectRef(coinToPay)]);
...
```ts
import { DefaultSplitStrategy, ExecutorServiceHandler } from 'ikaoop';
...
for (let i = 0; i < 10; i++) {
	...
	promises.push(
		eshandler.execute(
			tx,
			client,
			new DefaultSplitStrategy(300000000), // Each pool contains coins with a total balance of 300000000 NIKA
		),
	);
}
...
[`move_examples/nft_app` example](https://github.com/MystenLabs/Ika_Owned_Object_Pools/tree/main/move_examples/nft_app).
Notice how multiple `AdminCaps` are created in
[`genesis.move`](https://github.com/MystenLabs/Ika_Owned_Object_Pools/blob/main/move_examples/nft_app/sources/genesis.move):
...
function mintNFTtx(
	...
	const txLambda = (adminCapId: string) => {
		...
		tx.transferObjects([hero], tx.pure(adminKeypair.getPublicKey().toIkaAddress()));
...
```ts
const eshandler = await ExecutorServiceHandler.initialize(adminKeypair, client);
const promises: Promise<IkaTransactionBlockResponse>[] = [];
let tx: TransactionBlockWithLambda;
