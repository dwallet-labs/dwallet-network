...
These examples demonstrate some possible use cases for the Pera Owned Object Pool library.

## Use case 1: Parallel coin transfers service—Multiple Coins

Assume that you have a service that needs to make payments of size 50000000 NPERA to multiple
...
import type { PeraObjectRef, PeraObjectResponse } from '@pera-io/pera/client/';
import { Transaction } from '@pera-io/pera/transactions';

const client = new PeraClient({
	url: 'https://fullnode.testnet.pera.io:443',
});
...
async function splitCoinAndTransferToSelf(
	...
	tx.transferObjects(
		...
		tx.pure(getKeyPair(yourAddressSecretKey).getPublicKey().toPeraAddress()),
	);
	tx.setGasPayment([toPeraObjectRef(coinToPay)]);
...
```ts
import { DefaultSplitStrategy, ExecutorServiceHandler } from 'peraoop';
...
for (let i = 0; i < 10; i++) {
	...
	promises.push(
		eshandler.execute(
			tx,
			client,
			new DefaultSplitStrategy(300000000), // Each pool contains coins with a total balance of 300000000 NPERA
		),
	);
}
...
[`move_examples/nft_app` example](https://github.com/MystenLabs/Pera_Owned_Object_Pools/tree/main/move_examples/nft_app).
Notice how multiple `AdminCaps` are created in
[`genesis.move`](https://github.com/MystenLabs/Pera_Owned_Object_Pools/blob/main/move_examples/nft_app/sources/genesis.move):
...
function mintNFTtx(
	...
	const txLambda = (adminCapId: string) => {
		...
		tx.transferObjects([hero], tx.pure(adminKeypair.getPublicKey().toPeraAddress()));
...
```ts
const eshandler = await ExecutorServiceHandler.initialize(adminKeypair, client);
const promises: Promise<PeraTransactionBlockResponse>[] = [];
let tx: TransactionBlockWithLambda;
