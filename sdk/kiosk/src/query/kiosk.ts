...
export async function getOwnedKiosks(
	...
	const filter: IkaObjectDataFilter = {
		MatchAny: [
			{
	...
	const kioskIdList = data?.map((x: IkaObjectResponse) => {
	...
	const filteredData = data.filter((x) => 'data' in x).map((x) => x.data) as IkaObjectData[];
