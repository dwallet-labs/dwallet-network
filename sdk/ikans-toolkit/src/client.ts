...
import type { DataFields, NameObject, NetworkType, IkaNSContract } from './types/objects.js';
...
class IkansClient {
	...
	contractObjects: IkaNSContract | undefined;
	networkType: NetworkType | undefined;

	constructor(
		ikaClient: IkaClient,
		options?: {
			contractObjects?: IkaNSContract;
			networkType?: NetworkType;
		},
	) {
		if (!ikaClient) {
			throw new Error('IkaClient must be specified.');
		}
	...
	async getIkansContractObjects() {
		if ((this.contractObjects as IkaNSContract)?.packageId) return;
		...
		try {
			...
			throw new Error(`Error getting IkaNS contract objects, ${(error as Error).message}`);
		}
	...
	protected async getDynamicFieldObject(
		...
		const dynamicFieldObject = await this.ikaClient.getDynamicFieldObject({
	...
	protected async getNameData(dataObjectId: string, fields: DataFields[] = []) {
		...
		const { data: dynamicFields } = await this.ikaClient.getDynamicFields({
	...
	async getNameObject(
		...
		await this.getIkansContractObjects();

		const registryResponse = await this.getDynamicFieldObject(
			(this.contractObjects as IkaNSContract).registry,
		...
		if (nameObject.nftId && (includeAvatar || options?.showOwner)) {
			const [owner, avatarNft] = await Promise.all([
				getOwner(this.ikaClient, nameObject.nftId),
				includeAvatar ? getAvatar(this.ikaClient, nameObject.avatar) : Promise.resolve(null),
			]);
	 ...
	 *
	 * @param domain a domain name ends with `.ika`
	 */
