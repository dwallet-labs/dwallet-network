...
describe('Object id/Address/Transaction digest validation', () => {
	...
	it('Test all functions with invalid Ika Address', async () => {
		//empty id
	...
	it('Test all functions with invalid Object Id', async () => {
		//empty id
		expect(toolbox.client.getObject({ id: '' })).rejects.toThrowError(/Invalid Ika Object id/);
	...
	it('Validates tx.pure.address and tx.pure.id', async () => {
		...
		expect(() => tx.pure.address('')).toThrowError(/Invalid Ika address/);
		expect(() => tx.pure.id('')).toThrowError(/Invalid Ika address/);
	});
});
