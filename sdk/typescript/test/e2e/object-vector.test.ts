...
import { PERA_FRAMEWORK_ADDRESS } from '../../src/utils';
...
describe('Test Move call with a vector of objects as input', () => {
	...
	beforeEach(async () => {
		toolbox = await setup();
		const packagePath =
			__dirname + '/../../../../crates/pera-core/src/unit_tests/data/entry_point_vector';
	...
	it('Test regular arg mixed with object vector arg', async () => {
		...
		tx.moveCall({
			target: `${PERA_FRAMEWORK_ADDRESS}::pay::join_vec`,
