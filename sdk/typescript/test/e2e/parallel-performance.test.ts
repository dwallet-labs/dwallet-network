...
describe('Parallel Performance Tests', () => {
	...
	it(`should run DKG ${DEFAULT_PARALLEL_CONFIG.parallelCount} times in parallel`, async () => {
	...
	it(`should run DKG once and Presign ${DEFAULT_PARALLEL_CONFIG.parallelCount} times in parallel`, async () => {
	...
	it(`should run DKG once, Presign once, and Sign ${DEFAULT_PARALLEL_CONFIG.parallelCount} times in parallel`, async () => {
	...
	it(`should run full flow (DKG + Presign + Sign) ${DEFAULT_PARALLEL_CONFIG.parallelCount} times in parallel`, async () => {
		...
		const successCount = metrics.filter((m) => m.success).length;
		expect(successCount).toBeGreaterThan(0);
	}, DEFAULT_PARALLEL_CONFIG.timeout * 2); // Double timeout for full flow

	it('should run comprehensive performance comparison', async () => {
		...
		const dkgMetrics = dkgResults.map((result, index) => {
			if (result.status === 'fulfilled') {
		...
		if (dkgMetrics.length > 0) {
			...
			const presignMetrics = presignResults.map((result, index) => {
				if (result.status === 'fulfilled') {
					allMetrics.push(result.value.metrics);
					return result.value;
				}
		...
		operationTypes.forEach(opType => {
			...
			if (opMetrics.length > 0) {
				...
				console.log(`${opType.replace('-Comparison', '').padEnd(8)}: avg=${avgDuration.toFixed(0)}ms, min=${minDuration}ms, max=${maxDuration}ms`);
			}
		});
