...
import { normalizePeraAddress, PERA_TYPE_ARG } from '../../src/utils';
...
describe('Object Reading API', () => {
	...
	it('Get Object', async () => {
		...
		objectInfos.forEach((objectInfo) => {
			expect(objectInfo.data?.type).to.equal('0x2::coin::Coin<0x2::pera::PERA>');
		});
	});
	...
	it('handles trying to get non-existent old objects', async () => {
		const res = await toolbox.client.tryGetPastObject({
			id: normalizePeraAddress('0x9999'),
			version: 0,
		});
	...
	it('can find old versions of objects', async () => {
		...
		tx.transferObjects([tx.gas], normalizePeraAddress('0x2'));
