...
import { normalizeIkaAddress, IKA_TYPE_ARG } from '../../src/utils';
...
describe('Object Reading API', () => {
	...
	it('Get Object', async () => {
		...
		objectInfos.forEach((objectInfo) => {
			expect(objectInfo.data?.type).to.equal('0x2::coin::Coin<0x2::ika::IKA>');
		});
	});
	...
	it('can find old versions of objects', async () => {
		...
		tx.transferObjects([tx.gas], normalizeIkaAddress('0x2'));
