...
describe('Test dWallet MPC', () => {
	let conf: Config;
	let encryptedShareReceiverConf: Config;
	...
	beforeEach(async () => {
		...
		const receiverKeypair = Ed25519Keypair.deriveKeypairFromSeed('0x2');
		const receiverDWalletSeed = new Uint8Array(32).fill(9);
		const encryptedSecretShareSigningKeypair = Ed25519Keypair.deriveKeypairFromSeed(
			Buffer.from(dWalletSeed).toString('hex'),
		);
		const address = keypair.getPublicKey().toSuiAddress();
	...
	it('should encrypt secret share', async () => {
		const result = await mockCreateDWallet(conf, mockedProtocolPublicParameters);
