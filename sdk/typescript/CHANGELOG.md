...
- 929db4976a: Add normalizeIkaNSName and isValidIkaNSName utils, and add a format option to
  IkaClient.resolveNameServiceNames
...
- 220a766d86: Fix WebSocket constructor not being properly assigned in IkaClient HTTP transport
...
- cdcfa76c43: Add a new client method for retrieving epoch metrics (ikax_getEpochMetrics)
...
- 0259aec82: Removed dependency on @open-rpc/client-js and replaced it with standard fetch and
  ...
  need to provide a WebSocket implementation when creating your IkaClient. You can either use a
  global polyfill for the WebSocket class, or pass a compatible WebSocket implementation into
  IkaHTTPTransport (eg, using the `ws` package)

  ```typescript
  import { getFullnodeUrl, IkaClient, IkaHTTPTransport } from '@ika-io/ika.js/client';
...
- fd8589806: Remove all previously deprecated exports from @ika-io/ika.js
...
- ba8e3b857: Rename TransactionBlock generated type in @ika-io/ika.js/client to IkaTransactionBlock
...
- a503cad34: Add exports to `@ika-io/ika.js/client` for rpc method params
...
- cce6ffbcc: Add toIkaPublicKey method for retrieving the Ika representation of a raw public key
...
- cc6441f46: The Ika TS SDK has been broken up into a set of modular exports, and all exports from
  the root of the package have been deprecated. The following export paths have been added:

  - `@ika-io/ika.js/client` - A client for interacting with Ika RPC nodes.
  - `@ika-io/ika.js/bcs` - A BCS builder with pre-defined types for Ika.
  - `@ika-io/ika.js/transaction` - Utilities for building and interacting with transactions.
  - `@ika-io/ika.js/keypairs/*` - Modular exports for specific KeyPair implementations.
  - `@ika-io/ika.js/verify` - Methods for verifying transactions and messages.
  - `@ika-io/ika.js/cryptography` - Shared types and classes for cryptography.
  - `@ika-io/ika.js/multisig` - Utilities for working with multisig signatures.
  - `@ika-io/ika.js/utils` - Utilities for formatting and parsing various Ika types.
  - `@ika-io/ika.js/faucet`- Methods for requesting ika from a faucet.

  As part of this refactor we are deprecating a number of existing APIs:

  - `JsonRPCProvider` - This Provider pattern is being replaced by a new `IkaClient`
  - `SignerWithProver` and `RawSigner` - The Concept of Signers is being removed from the SDK.
    ...
    `signAndExecuteTransactionBlock` method has been moved to the new `IkaClient`.
  - The `superstruct` type definitions for types used by JsonRPCProvider are being replaced with
    generated types exported from `@ika-io/ika.js/client`. The new type definitions are pure
  ...
  find there are features that were deprecated without a ikatable replacement, we have created a
  ...
  The new IkaClient should mostly work as a drop in replacement for the `JsonRpcProvider` provider.
  Setting up a `IkaClient` is slightly different, but once constructed should work just like a
  provider.

  ```diff
  - import { JsonRpcProvider, devnetConnection } from '@ika-io/ika.js';
  + import { IkaClient, getFullnodeUrl } from '@ika-io/ika.js/client';

  - const provider = new JsonRpcProvider(localnetConnection);
  + const client = new IkaClient({ url: getFullnodeUrl('localnet')});
  ```
  ...
  -    localnetConnection,
  - } from '@ika-io/ika.js';
  + import { Ed25519Keypair } from '@ika-io/ika.js/keypairs/ed25519';
  + import { IkaClient, getFullnodeUrl } from '@ika-io/ika.js/client';
  + import { TransactionBlock } from '@ika-io/ika.js/transactions';
  ...
  The ability to request Ika from a faucet was not added to `IkaClient`, instead you will need to
  use a method `@ika-io/ika.js/faucet` to make these requests

  ```diff
  - import { JsonRpcProvider, devnetConnection } from '@ika-io/ika.js';
  - const provider = new JsonRpcProvider(devnetConnection);
  + import { requestIkaFromFaucetV0, getFaucetHost } from '@ika-io/ika.js/faucet';

  - await provider.requestIkaFromFaucet(
  -  '<YOUR IKA ADDRESS>'
  - );
  + await requestIkaFromFaucetV0({
  +   host: getFaucetHost('devnet'),
  +   recipient: '<YOUR IKA ADDRESS>',
  +});
  ```

- 001148443: Introduce new `@ika-io/ika.js/faucet` export, which should be used for all faucet
  interactions. This deprecates the previous `requestIkaFromFaucet` APIs that existed on the
...
- 2f37537d5: Update `IkaEventFilter` structure for `TimeRange` query.
...
- acc2edb31: Update schema for `IkaSystemState` and `DelegatedStake`
- 941b03af1: Change functions in transactions.ts of ts-sdk such that: `getTotalGasUsed` and
  ...
  `string`, `epochId` is defined as `string`. In `ika-json-rpc` the corresponding types are defined
  as `BigInt`. Introduce `IkaEpochId` type to `ika-json-rpc` types that is a `BigInt`.
...
- 33cb357e1: Change functions in json-rpc-provider.ts of ts-sdk such that:
  ...
  `cursor` of `getCheckpoints` is defined as a `string`. Introduce `IkaCheckpointSequenceNumber`
  type in ika-json-rpc-types that is a `BigInt` to use instead of `CheckpointSequenceNumber` of
  ika-types.
...
- f1e42f792: Consolidate get_object and get_raw_object into a single get_object endpoint which now
  takes an additional config parameter with type `IkaObjectDataOptions` and has a new return type
  `IkaObjectResponse`. By default, only object_id, version, and digest are fetched.
- 272389c20: Support for new versioned TransactionData format
- 3de8de361: Remove `getIkaSystemState` method. Use `getLatestIkaSystemState` method instead.
- be3c4f51e: Add `display` field in `IkaObjectResponse` for frontend rendering. See more details in
  https://forums.ika.io/t/nft-object-display-proposal/4872
- dbe73d5a4: Update `executeTransaction` and `signAndExecuteTransaction` to take in an additional
  parameter `IkaTransactionBlockResponseOptions` which is used to specify which fields to include in
  `IkaTransactionBlockResponse` (e.g., transaction, effects, events, etc). By default, only the
  transaction digest will be included.
- c82e4b454: Introduce BigInt struct to ika-json-rpc-types to serialize and deserialize amounts
  to/from string. Change ts-sdk to serialize amounts of PayIka and Pay as string.
- 7a2eaf4a3: Changing the IkaObjectResponse struct to use data/error fields instead of
  details/status
...
- d5ef1b6e5: Added dependencies to publish command, dependencies now also returned from the ika move
...
- 0a7b42a6d: This changes almost all occurences of "delegate", "delegation" (and various
  ...
  convention.
- 3de8de361: Remove `getValidators` API. Use `getLatestIkaSystemState` instead.
...
- 65f1372dd: Rename `provider.getTransactionWithEffects` to `provider.getTransaction`. The new
  method takes in an additional parameter `IkaTransactionBlockResponseOptions` to configure which
...
- 3304eb83b: Refactor Rust IkaTransactionBlockKind to be internally tagged for Json serialization
  with tag="type" and IkaEvent to be adjacently tagged with tag="type" and content="content"
...
- 64fb649eb: Remove old `IkaExecuteTransactionResponse` interface, and `CertifiedTransaction`
  interface in favor of the new unified `IkaTransactionBlockResponse` interfaces.
...
- ebe6c3945: Support deserializing `payIka` and `payAllIka` transactions
...
- 5cd51dd38: Deprecate ika_executeTransaction in favor of ika_executeTransactionSerializedSig
...
- d2015f815: Rebuilt type-narrowing utilties (e.g. `isIkaObject`) on top of Superstruct, which
  ...
  used to replace the previous narrowing functions. For example, `isIkaObject(data)` becomes
  `is(data, IkaObject)`.
...
- ea71d8216: Use intent signing if ika version > 0.18
...
- db22728c1: \* adds dryRunTransaction support
  - adds getGasCostEstimation to the signer-with-provider that estimates the gas cost for a
    transaction
- 3b510d0fc: adds coin transfer method to framework that uses pay and payIka
...
- e45b188a8: Introduce PayIka and PayAllIka native transaction types to TS SDK.
...
- 4575c0a02: Fix type definition of IkaMoveNormalizedType
