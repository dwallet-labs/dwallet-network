...
import { MOVE_STDLIB_ADDRESS, IKA_FRAMEWORK_ADDRESS } from '../utils/index.js';
...
export function isTxContext(param: OpenMoveTypeSignature): boolean {
	...
	return (
		!!struct &&
		normalizeIkaAddress(struct.package) === normalizeIkaAddress('0x2') &&
...
export function getPureBcsSchema(typeSignature: OpenMoveTypeSignatureBody): BcsType<any> | null {
	...
	if ('datatype' in typeSignature) {
		const pkg = normalizeIkaAddress(typeSignature.datatype.package);

		if (pkg === normalizeIkaAddress(MOVE_STDLIB_ADDRESS)) {
			if (
		...
		if (
			pkg === normalizeIkaAddress(IKA_FRAMEWORK_ADDRESS) &&
