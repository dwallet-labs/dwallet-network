...
export async function executeSignTransaction(tx: Transaction, conf: Config) {
	const extractSignResult = (result: SuiTransactionBlockResponse) => {
		...
		return getObjectWithType(conf, startSessionEvent.event_data.sign_id, isReadySignObject);
	};

	const signObj = await executeTransactionWithTiming(
		conf,
		tx,
		`Sign: ${conf.suiClientKeypair.toSuiAddress()}`,
		extractSignResult,
	);
...
export async function signWithImportedDWallet(
	...
	const { tx, messageApproval } = await approveImportedDWalletMessageTX(
		conf,
		dwalletCapID,
		hash,
		message,
...
export async function verifySignWithPartialUserSignatures(
	...
	const [verifiedPartialUserSignatureCap] = tx.moveCall({
		...
		arguments: [dwalletStateArg, tx.object(unverifiedPartialUserSignatureCapID)],
	});
...
export async function completeFutureSign(
	...
	return await executeSignTransaction(tx, conf);
}
