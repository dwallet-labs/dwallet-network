...
export const PERA_PRIVATE_KEY_PREFIX = 'peraprivkey';
...
export abstract class Signer {
	...
	toPeraAddress(): string {
		return this.getPublicKey().toPeraAddress();
	}
 ...
 * 33-byte Bech32 encoded string starting with `peraprivkey`, and
 * parse out the signature scheme and the private key in bytes.
 */
export function decodePeraPrivateKey(value: string): ParsedKeypair {
	const { prefix, words } = bech32.decode(value);
	if (prefix !== PERA_PRIVATE_KEY_PREFIX) {
...
/**
 * This returns a Bech32 encoded string starting with `peraprivkey`,
 * encoding 33-byte `flag || bytes` for the given the 32-byte private
 * key and its signature scheme.
 */
export function encodePeraPrivateKey(bytes: Uint8Array, scheme: SignatureScheme): string {
	...
	return bech32.encode(PERA_PRIVATE_KEY_PREFIX, bech32.toWords(privKeyBytes));
}
