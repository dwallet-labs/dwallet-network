...
import { PeraGraphQLClient } from '../graphql/client.js';
...
export class ZkLoginPublicIdentifier extends PublicKey {
	#data: Uint8Array;
	#client?: PeraGraphQLClient;

	/**
	 * Create a new ZkLoginPublicIdentifier object
	 * @param value zkLogin public identifier as buffer or base-64 encoded string
	 */
	constructor(value: PublicKeyInitData, { client }: { client?: PeraGraphQLClient } = {}) {
		super();
	...
	/**
	 * Return the Pera address associated with this ZkLogin public identifier
	 */
	flag(): number {
	...
	verifyPersonalMessage(message: Uint8Array, signature: Uint8Array | string): Promise<boolean> {
		...
		const address = new ZkLoginPublicIdentifier(parsedSignature.publicKey).toPeraAddress();
...
export function toZkLoginPublicIdentifier(
	addressSeed: bigint,
	iss: string,
	options?: { client?: PeraGraphQLClient },
...
async function graphqlVerifyZkLoginSignature({
	...
	client = new PeraGraphQLClient({
		url: 'https://pera-mainnet.mystenlabs.com/graphql',
	}),
}: {
	address: string;
	bytes: string;
