...
import type {
	...
	IkaMoveNormalizedType,
} from '@ika-io/ika/client';
import { normalizeIkaAddress, parseStructTag } from '@ika-io/ika/utils';
...
export function mapNormalizedType(type: OpenMoveTypeSignatureBody): IkaMoveNormalizedType {
	switch (type) {
		case 'address':
			return 'Address';
		case 'bool':
			...
			fn.typeParameters?.map((param) => ({
				abilities:
					param.constraints?.map(
						(constraint) =>
							`${constraint[0]}${constraint.slice(1).toLowerCase()}` as IkaMoveAbility,
					) ?? [],
			})) ?? [],
		...
		abilities: {
			abilities:
				struct.abilities?.map(
					(ability) => `${ability[0]}${ability.slice(1).toLowerCase()}` as IkaMoveAbility,
				) ?? [],
		},
		fields:
...
export function mapNormalizedMoveModule(
	...
	const exposedFunctions: Record<string, IkaMoveNormalizedFunction> = {};
	const structs: Record<string, IkaMoveNormalizedStruct> = {};
...
export function moveDataToRpcContent(data: MoveData, layout: MoveTypeLayout): MoveValue {
	...
	if ('UID' in data) {
		return {
			id: normalizeIkaAddress(data.UID.map((byte) => byte.toString(16).padStart(2, '0')).join('')),
		};
	}

	if ('ID' in data) {
		return normalizeIkaAddress(data.ID.map((byte) => byte.toString(16).padStart(2, '0')).join(''));
	}
	...
	if ('Struct' in data) {
		...
		// https://github.com/MystenLabs/sui/blob/5849f6845a3ab9fdb4c17523994adad461478a4c/crates/ika-json-rpc-types/src/ika_move.rs#L481
