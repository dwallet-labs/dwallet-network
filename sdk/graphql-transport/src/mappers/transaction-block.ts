...
import type {
	...
	PeraObjectChange,
	PeraTransaction,
	PeraTransactionBlock,
	PeraTransactionBlockKind,
...
function mapObjectChanges(
	transactionBlock: Rpc_Transaction_FieldsFragment,
	effects: PeraTransactionBlockResponse['effects'],
) {
	const changes: PeraObjectChange[] = [];
...
function mapTransactionInput(input: typeof bcs.CallArg.$inferType): PeraCallArg {
	if (input.Pure) {
		return {
			type: 'pure',
...
function mapTransaction(transaction: typeof bcs.Command.$inferType): PeraTransaction {
...
function mapTransactionArgument(arg: typeof bcs.Argument.$inferType): PeraArgument {
	switch (arg.$kind) {
		case 'GasCoin': {
			return 'GasCoin';
		}
...
const ADDRESS_ZERO = normalizePeraAddress('0x0');

export function mapEffects(data: string): PeraTransactionBlockResponse['effects'] {
