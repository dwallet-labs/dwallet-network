...
import type { ObjectOwner, PeraObjectChange, PeraTransactionBlockResponse } from '@pera-io/pera/client';
...
function getObjectOwnerFromObjectChange(objectChange: PeraObjectChange, isSent: boolean) {
	if (isSent) {
...
function isObjectOwner(objectChange: PeraObjectChange, address: string, isSent: boolean) {
...
export function ownedAfterChange(
	objectChange: PeraObjectChange,
	address: string,
): objectChange is Extract<PeraObjectChange, { type: 'created' | 'transferred' | 'mutated' }> {
...
export function isOwner(owner: ObjectOwner, address: string): owner is { AddressOwner: string } {
	return (
		...
		normalizePeraAddress(owner.AddressOwner) === address
	);
}
