...
import type { Keypair, Signer } from '@pera-io/pera/cryptography';
import { Ed25519Keypair } from '@pera-io/pera/keypairs/ed25519';
import type { TransactionObjectArgument, TransactionObjectInput } from '@pera-io/pera/transactions';
import { Transaction } from '@pera-io/pera/transactions';
import { normalizeStructTag, normalizePeraAddress, PERA_TYPE_ARG, toB64 } from '@pera-io/pera/utils';
...
export class ZkSendLinkBuilder {
	...
	constructor({
		...
		this.#client = client;
		this.sender = normalizePeraAddress(sender);
	...
	addClaimableNPera(amount: bigint) {
		this.addClaimableBalance(PERA_COIN_TYPE, amount);
	}
	...
	getLink(): string {
		...
		link.hash = `${this.#contract ? '$' : ''}${toB64(
			decodePeraPrivateKey(this.keypair.getSecretKey()).secretKey,
		)}`;
	...
	async #objectsToTransfer(tx: Transaction) {
		...
		for (const [coinType, amount] of this.balances) {
			if (coinType === PERA_COIN_TYPE) {
				const [pera] = tx.splitCoins(tx.gas, [amount]);
				refsWithType.push({
					ref: pera,
	...
	async #createSendTransactionWithoutContract({
		...
		const address = this.keypair.toPeraAddress();
	...
	async #estimateClaimGasFee(): Promise<bigint> {
		...
		tx.setGasPayment([]);
		tx.transferObjects([tx.gas], this.keypair.toPeraAddress());
	...
	static async createLinks({
		...
		const mergedCoins = new Map<string, TransactionObjectArgument>([
			[PERA_COIN_TYPE, transaction.gas],
		]);
		...
		for (const link of links) {
			const receiver = link.keypair.toPeraAddress();
		...
		for (const [coinType, merged] of mergedCoins) {
			...
			for (const [i, link] of linksWithCoin.entries()) {
				transaction.add(
					contract.add({
						arguments: [store, link.keypair.toPeraAddress(), splits[i]],
