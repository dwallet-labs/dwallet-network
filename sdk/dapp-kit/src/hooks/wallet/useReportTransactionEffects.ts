...
import type { PeraReportTransactionEffectsInput } from '@mysten/wallet-standard';
...
type UseReportTransactionEffectsArgs = Omit<
	PartialBy<PeraReportTransactionEffectsInput, 'account' | 'chain'>,
	'effects'
> & {
	...
	return useMutation({
		...
		mutationFn: async ({ effects, chain = currentWallet?.chains[0], account = currentAccount }) => {
			...
			const reportTransactionEffectsFeature =
				currentWallet.features['pera:reportTransactionEffects'];
