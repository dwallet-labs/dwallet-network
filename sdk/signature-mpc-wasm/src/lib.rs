...
pub fn initiate_dkg() -> Result<JsValue, JsErr> {
    ...
    let (
        commitment_to_secret_key_share,
...
#[wasm_bindgen]
pub fn finalize_dkg(decommitment_round_party_state: Vec<u8>, secret_key_share_encryption_and_proof: Vec<u8>) -> Result<JsValue, JsErr> {
    let decommitment_round_party_state: DKGDecommitmentRoundState<ProtocolContext> = bcs::from_bytes(&decommitment_round_party_state)?;
    let decommitment_round_party = decommitment_round_centralized_party_dkg(decommitment_round_party_state)?;
    ...
    let value = FinalizeDKGValue {
        public_key_share_decommitment_and_proof: bcs::to_bytes(&public_key_share_decommitment_and_proof)?,
...
pub fn initiate_presign(dkg_output: Vec<u8>, batch_size: usize) -> Result<JsValue, JsErr> {
    ...
    let (
        ...
        proof_verification_round_party,
...
#[wasm_bindgen]
pub fn finalize_presign(dkg_output: Vec<u8>, signature_nonce_shares_and_commitment_randomnesses: Vec<u8>, presign_output: Vec<u8>) -> Result<JsValue, JsErr> {
    let dkg_output: DKGCentralizedPartyOutput = bcs::from_bytes(&dkg_output)?;
    let presign_output: PresignDecentralizedPartyOutput<ProtocolContext> = bcs::from_bytes(&presign_output)?;
    let signature_nonce_shares_and_commitment_randomnesses: Vec<(Scalar, Scalar)> = bcs::from_bytes(&signature_nonce_shares_and_commitment_randomnesses)?;
    let commitment_round_party = finalize_centralized_party_presign( dkg_output.clone(), signature_nonce_shares_and_commitment_randomnesses)?;
...
pub fn initiate_sign(dkg_output: Vec<u8>, presigns: Vec<u8>, messages: Vec<u8>, hash: u8) -> Result<JsValue, JsErr> {
    ...
    let public_nonce_encrypted_partial_signature_and_proofs = bcs::to_bytes(&public_nonce_encrypted_partial_signature_and_proofs)?;

    Ok(serde_wasm_bindgen::to_value(&public_nonce_encrypted_partial_signature_and_proofs)?)
}
