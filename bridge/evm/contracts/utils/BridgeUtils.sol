...
/// @title BridgeUtils
/// @notice This library defines the message format and constants for the Pera native bridge. It also
...
library BridgeUtils {
    ...
    /// @notice Converts the provided token amount to the Pera decimal adjusted amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param peraDecimal The pera decimal value for the token.
    /// @param amount The ERC20 amount of the tokens to convert to Pera.
    /// @return Pera converted amount.
    function convertERC20ToPeraDecimal(uint8 erc20Decimal, uint8 peraDecimal, uint256 amount)
        internal
        pure
        returns (uint64)
    {
        if (erc20Decimal == peraDecimal) {
        ...
        require(erc20Decimal > peraDecimal, "BridgeUtils: Invalid Pera decimal");

        // Difference in decimal places
        uint256 factor = 10 ** (erc20Decimal - peraDecimal);
    ...
    /// @notice Converts the provided Pera decimal adjusted amount to the ERC20 token amount.
    /// @param erc20Decimal The erc20 decimal value for the token.
    /// @param peraDecimal The pera decimal value for the token.
    /// @param amount The Pera amount of the tokens to convert to ERC20.
    /// @return ERC20 converted amount.
    function convertPeraToERC20Decimal(uint8 erc20Decimal, uint8 peraDecimal, uint64 amount)
        internal
        pure
        returns (uint256)
    {
        if (peraDecimal == erc20Decimal) {
    ...
    ///     bytes 1-32   : sender address (as we only support Pera now, it has to be 32 bytes long)
        ...
        returns (TokenTransferPayload memory)
    {
        ...
        require(
            ...
            "BridgeUtils: Invalid sender address length, Pera address must be 32 bytes"
        );
    ...
    ///     byte m + 1       : number of pera decimals
    ///     bytes m + 2 -> i : pera decimals
    ...
    /// @return peraDecimals the Pera decimal places of the tokens to be added.
        ...
        )
    {
        ...
        peraDecimals = new uint8[](decimalCount);
        for (uint8 i; i < decimalCount; i++) {
            peraDecimals[i] = uint8(_payload[offset++]);
        }
