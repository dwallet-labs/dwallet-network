...
import "./interfaces/IPeraBridge.sol";
...
/// tokens. Bridge operations are managed by a committee of Pera validators that are responsible
...
contract PeraBridge is IPeraBridge, CommitteeUpgradeable, PausableUpgradeable {
    /* ========== STATE VARIABLES ========== */
    ...
    uint8 constant PERA_ADDRESS_LENGTH = 32;

    /* ========== INITIALIZER ========== */

    /// @notice Initializes the PeraBridge contract with the provided parameters.
        ...
        onlySupportedChain(message.chainID)
    {
        ...
        require(!isTransferProcessed[message.nonce], "PeraBridge: Message already processed");
        ...
        require(
            tokenTransferPayload.targetChain == config.chainID(), "PeraBridge: Invalid target chain"
        );

        // convert amount to ERC20 token decimals
        uint256 erc20AdjustedAmount = BridgeUtils.convertPeraToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(tokenTransferPayload.tokenID)).decimals(),
            config.tokenPeraDecimalOf(tokenTransferPayload.tokenID),
    ...
    /// @param recipientAddress The address on the Pera chain where the tokens will be sent.
    ...
    function bridgeERC20(
        ...
        require(
            recipientAddress.length == PERA_ADDRESS_LENGTH,
            "PeraBridge: Invalid recipient address length"
        );

        IBridgeConfig config = committee.config();

        require(config.isTokenSupported(tokenID), "PeraBridge: Unsupported token");
        ...
        require(
            ...
            "PeraBridge: Insufficient allowance"
        );
        ...
        // Adjust the amount
        uint64 peraAdjustedAmount = BridgeUtils.convertERC20ToPeraDecimal(
            IERC20Metadata(tokenAddress).decimals(),
            config.tokenPeraDecimalOf(tokenID),
            amountTransfered
        );
        ...
        onlySupportedChain(destinationChainID)
    {
        ...
        require(success, "PeraBridge: Failed to transfer ETH to vault");
    ...
    function _transferTokensFromVault(
        ...
        require(tokenAddress != address(0), "PeraBridge: Unsupported token");
    ...
    modifier limitNotExceeded(uint8 chainID, uint8 tokenID, uint256 amount) {
        require(
            ...
            "PeraBridge: Amount exceeds bridge limit"
        );
        _;
    }
    ...
    modifier onlySupportedChain(uint8 targetChainID) {
        require(
            ...
            "PeraBridge: Target chain not supported"
        );
        _;
    }
}
