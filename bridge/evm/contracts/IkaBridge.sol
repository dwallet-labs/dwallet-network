...
import "./interfaces/IIkaBridge.sol";
...
/// tokens. Bridge operations are managed by a committee of Ika validators that are responsible
...
contract IkaBridge is IIkaBridge, CommitteeUpgradeable, PausableUpgradeable {
    /* ========== STATE VARIABLES ========== */
    ...
    uint8 constant IKA_ADDRESS_LENGTH = 32;

    /* ========== INITIALIZER ========== */

    /// @notice Initializes the IkaBridge contract with the provided parameters.
        ...
        onlySupportedChain(message.chainID)
    {
        ...
        require(!isTransferProcessed[message.nonce], "IkaBridge: Message already processed");
        ...
        require(
            tokenTransferPayload.targetChain == config.chainID(), "IkaBridge: Invalid target chain"
        );

        // convert amount to ERC20 token decimals
        uint256 erc20AdjustedAmount = BridgeUtils.convertIkaToERC20Decimal(
            IERC20Metadata(config.tokenAddressOf(tokenTransferPayload.tokenID)).decimals(),
            config.tokenIkaDecimalOf(tokenTransferPayload.tokenID),
    ...
    /// @param recipientAddress The address on the Ika chain where the tokens will be sent.
    ...
    function bridgeERC20(
        ...
        require(
            recipientAddress.length == IKA_ADDRESS_LENGTH,
            "IkaBridge: Invalid recipient address length"
        );

        IBridgeConfig config = committee.config();

        require(config.isTokenSupported(tokenID), "IkaBridge: Unsupported token");
        ...
        require(
            ...
            "IkaBridge: Insufficient allowance"
        );
        ...
        // Adjust the amount
        uint64 ikaAdjustedAmount = BridgeUtils.convertERC20ToIkaDecimal(
            IERC20Metadata(tokenAddress).decimals(),
            config.tokenIkaDecimalOf(tokenID),
            amountTransfered
        );
        ...
        onlySupportedChain(destinationChainID)
    {
        ...
        require(success, "IkaBridge: Failed to transfer ETH to vault");
    ...
    function _transferTokensFromVault(
        ...
        require(tokenAddress != address(0), "IkaBridge: Unsupported token");
    ...
    modifier limitNotExceeded(uint8 chainID, uint8 tokenID, uint256 amount) {
        require(
            ...
            "IkaBridge: Amount exceeds bridge limit"
        );
        _;
    }
    ...
    modifier onlySupportedChain(uint8 targetChainID) {
        require(
            ...
            "IkaBridge: Target chain not supported"
        );
        _;
    }
}
