...
contract BridgeConfigTest is BridgeBaseTest {
    ...
    function testBridgeConfigInitialization() public {
        ...
        assertEq(config.tokenIkaDecimalOf(0), 9);
        assertEq(config.tokenIkaDecimalOf(1), 8);
        assertEq(config.tokenIkaDecimalOf(2), 8);
        assertEq(config.tokenIkaDecimalOf(3), 6);
        assertEq(config.tokenIkaDecimalOf(4), 6);
        assertEq(config.tokenPriceOf(0), IKA_PRICE);
    ...
    function testTokenIkaDecimalOf() public {
        assertEq(config.tokenIkaDecimalOf(1), 8);
    }
    ...
    function testAddTokensWithSignatures() public {
        ...
        assertEq(config.tokenIkaDecimalOf(10), 6);
    ...
    function testAddTokensIkaDecimalFailure() public {
        MockUSDC _newToken = new MockUSDC();
        ...
        // add token shoudl fail because the ika decimal is greater than the eth decimal
        vm.expectRevert(bytes("BridgeConfig: Invalid Ika decimal"));
        config.addTokensWithSignatures(signatures, message);
    }
    ...
    function testUpdateTokenPriceRegressionTestWithSigVerficiation() public {
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                ...
                (address(committee), chainID, supportedTokens, tokenPrices, tokenIds, ikaDecimals, _supportedDestinationChains)
            ),
            opts
        );
    ...
    function testAddTokensRegressionTest() public {
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                BridgeConfig.initialize,
                (address(committee), 12, supportedTokens, tokenPrices, tokenIds, ikaDecimals, _supportedDestinationChains)
            ),
            opts
        );
        config = BridgeConfig(_config);
        ...
        assertEq(ikaDecimals.length, 3);
        assertEq(ikaDecimals[0], 5);
        assertEq(ikaDecimals[1], 6);
        assertEq(ikaDecimals[2], 7);
        ...
        assertEq(config.tokenIkaDecimalOf(99), 5);
        assertEq(config.tokenIkaDecimalOf(100), 6);
        assertEq(config.tokenIkaDecimalOf(101), 7);
