...
import "../contracts/interfaces/IIkaBridge.sol";
import "./mocks/MockIkaBridgeV2.sol";

contract IkaBridgeTest is BridgeBaseTest, IIkaBridge {
    // This function is called before each unit test
    function setUp() public {
        setUpBridgeTest();
    }

    function testIkaBridgeInitialization() public {
    ...
    function testTransferBridgedTokensWithSignaturesTokenDailyLimitExceeded() public {
        ...
        vm.expectRevert(bytes("IkaBridge: Amount exceeds bridge limit"));
    ...
    function testTransferBridgedTokensWithSignaturesInvalidTargetChain() public {
        ...
        signatures[2] = getSignature(messageHash, committeeMemberPkC);
        signatures[3] = getSignature(messageHash, committeeMemberPkD);
        vm.expectRevert(bytes("IkaBridge: Target chain not supported"));
        bridge.transferBridgedTokensWithSignatures(signatures, message);
    }
    ...
    function testTransferBridgedTokensWithSignaturesInsufficientStakeAmount() public {
        ...
        BridgeUtils.TokenTransferPayload memory payload = BridgeUtils.TokenTransferPayload({
            ...
            // This is Ika amount (eth decimal 8)
            amount: 100_000_000
        });
        BridgeUtils.Message memory message = BridgeUtils.Message({
            messageType: BridgeUtils.TOKEN_TRANSFER,
    ...
    function testTransferWETHWithValidSignatures() public {
        ...
        vm.expectRevert(bytes("IkaBridge: Message already processed"));
        bridge.transferBridgedTokensWithSignatures(signatures, message);
    }
    ...
    function testBridgeERC20UnsupportedToken() public {
        vm.expectRevert(bytes("IkaBridge: Unsupported token"));
        bridge.bridgeERC20(
    ...
    function testBridgeERC20InsufficientAllowance() public {
        vm.expectRevert(bytes("IkaBridge: Insufficient allowance"));
        bridge.bridgeERC20(
            BridgeUtils.ETH,
            type(uint256).max,
    ...
    function testBridgeERC20InvalidRecipientAddress() public {
        vm.expectRevert(bytes("IkaBridge: Invalid recipient address length"));
        bridge.bridgeERC20(
            BridgeUtils.ETH,
            1 ether,
    ...
    function testBridgeEth() public {
        ...
        // assert emitted event
        vm.expectEmit(true, true, true, false);
        emit IIkaBridge.TokensDeposited(
            chainID,
            0, // nonce
            0, // destination chain id
            BridgeUtils.ETH,
    ...
    function testIkaBridgeInvalidERC20DecimalConversion() public {
        IERC20(wETH).approve(address(bridge), 10 ether);
    ...
    function testIkaBridgeInvalidEthDecimalConversion() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
    ...
    function testIkaBridgeInvalidERC20Transfer() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
        bridge.bridgeERC20(
            BridgeUtils.USDC,
            0,
    ...
    function testIkaBridgeInvalidETHTransfer() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
    ...
    function testTransferIkaToEthRegressionTest() public {
        ...
        tokenPrices[0] = 1 * USD_VALUE_MULTIPLIER; // IKA PRICE
        ...
        address _config = Upgrades.deployUUPSProxy(
            ...
            abi.encodeCall(
                ...
                (address(committee), 11, _supportedTokens, tokenPrices, tokenIds, ikaDecimals, _supportedDestinationChains)
            ),
            opts
        );
