...
import "../contracts/interfaces/IPeraBridge.sol";
import "./mocks/MockPeraBridgeV2.sol";

contract PeraBridgeTest is BridgeBaseTest, IPeraBridge {
    // This function is called before each unit test
    function setUp() public {
        setUpBridgeTest();
    }

    function testPeraBridgeInitialization() public {
    ...
    function testTransferBridgedTokensWithSignaturesTokenDailyLimitExceeded() public {
        ...
        vm.expectRevert(bytes("PeraBridge: Amount exceeds bridge limit"));
    ...
    function testTransferBridgedTokensWithSignaturesInvalidTargetChain() public {
        ...
        signatures[2] = getSignature(messageHash, committeeMemberPkC);
        signatures[3] = getSignature(messageHash, committeeMemberPkD);
        vm.expectRevert(bytes("PeraBridge: Target chain not supported"));
        bridge.transferBridgedTokensWithSignatures(signatures, message);
    }
    ...
    function testTransferBridgedTokensWithSignaturesInsufficientStakeAmount() public {
        ...
        BridgeUtils.TokenTransferPayload memory payload = BridgeUtils.TokenTransferPayload({
            ...
            // This is Pera amount (eth decimal 8)
            amount: 100_000_000
        });
        BridgeUtils.Message memory message = BridgeUtils.Message({
            messageType: BridgeUtils.TOKEN_TRANSFER,
    ...
    function testTransferWETHWithValidSignatures() public {
        ...
        vm.expectRevert(bytes("PeraBridge: Message already processed"));
        bridge.transferBridgedTokensWithSignatures(signatures, message);
    }
    ...
    function testBridgeERC20UnsupportedToken() public {
        vm.expectRevert(bytes("PeraBridge: Unsupported token"));
        bridge.bridgeERC20(
    ...
    function testBridgeERC20InsufficientAllowance() public {
        vm.expectRevert(bytes("PeraBridge: Insufficient allowance"));
        bridge.bridgeERC20(
            BridgeUtils.ETH,
            type(uint256).max,
    ...
    function testBridgeERC20InvalidRecipientAddress() public {
        vm.expectRevert(bytes("PeraBridge: Invalid recipient address length"));
        bridge.bridgeERC20(
            BridgeUtils.ETH,
            1 ether,
    ...
    function testBridgeEth() public {
        ...
        // assert emitted event
        vm.expectEmit(true, true, true, false);
        emit IPeraBridge.TokensDeposited(
            chainID,
            0, // nonce
            0, // destination chain id
            BridgeUtils.ETH,
    ...
    function testPeraBridgeInvalidERC20DecimalConversion() public {
        IERC20(wETH).approve(address(bridge), 10 ether);
    ...
    function testPeraBridgeInvalidEthDecimalConversion() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
    ...
    function testPeraBridgeInvalidERC20Transfer() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
        bridge.bridgeERC20(
            BridgeUtils.USDC,
            0,
    ...
    function testPeraBridgeInvalidETHTransfer() public {
        vm.expectRevert(bytes("BridgeUtils: Insufficient amount provided"));
    ...
    function testTransferPeraToEthRegressionTest() public {
        ...
        tokenPrices[0] = 1 * USD_VALUE_MULTIPLIER; // PERA PRICE
