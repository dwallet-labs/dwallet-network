...
async function getOriginByteKioskContents(address: string, client: PeraClient) {
...
async function getPeraKioskContents(address: string, kioskClient: KioskClient) {
...
export function useGetKioskContents(address?: string | null, disableOriginByteKiosk?: boolean) {
	const { client: peraClient, network } = usePeraClientContext();
	const kioskClient = useKioskClient();
	return useQuery({
		// eslint-disable-next-line @tanstack/query/exhaustive-deps
		queryKey: ['get-kiosk-contents', address, disableOriginByteKiosk, network, kioskClient.network],
		queryFn: async () => {
			const peraKiosks = await getPeraKioskContents(address!, kioskClient);
			const obKiosks = await getOriginByteKioskContents(address!, peraClient);
			return [...peraKiosks, ...obKiosks];
		},
		select(data) {
