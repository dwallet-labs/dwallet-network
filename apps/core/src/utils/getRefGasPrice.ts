...
import { PeraValidatorSummary } from '@pera-io/pera/client';
 ...
 */
export function getRefGasPrice(validators?: PeraValidatorSummary[]) {
	...
	const totalStaked = validators.reduce(
		(acc, cur) => acc + BigInt(cur.stakingPoolPeraBalance),
		BigInt(0),
	);
	...
	for (let i = 0; i < sortedByGasPrice.length; i++) {
		...
		const stake = BigInt(validator?.stakingPoolPeraBalance);
