...
test('staking', async ({ page, extensionUrl }) => {
	...
	await page.getByText(/Stake and Earn PERA/).click();
	...
	await expect(page.getByTestId(`stake-button-${STAKE_AMOUNT}-PERA`)).toBeVisible({
		timeout: TEST_TIMEOUT,
	});
	await page.getByTestId(`stake-button-${STAKE_AMOUNT}-PERA`).click();
