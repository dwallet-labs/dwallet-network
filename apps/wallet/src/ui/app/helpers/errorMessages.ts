 ...
 * operations on the Pera Ledger application
 */
export function getPeraApplicationErrorMessage(error: unknown) {
	if (error instanceof LockedDeviceError) {
		...
		if (error.statusCode === StatusCodes.INS_NOT_SUPPORTED) {
			...
			return 'Make sure the Pera app is open on your device.';
		}
	}
	return null;
}
