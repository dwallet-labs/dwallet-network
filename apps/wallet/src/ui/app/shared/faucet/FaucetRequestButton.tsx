...
import { FaucetRateLimitError, getFaucetHost } from '@pera-io/pera/faucet';
...
function FaucetRequestButton({ variant = 'primary', size = 'narrow' }: FaucetRequestButtonProps) {
	...
	const networkName = API_ENV_TO_INFO[network].name.replace(/pera\s*/gi, '');
	...
	return mutation.enabled ? (
		<Button
			...
			text={`Request ${networkName} PERA Tokens`}
		/>
	) : null;
}
