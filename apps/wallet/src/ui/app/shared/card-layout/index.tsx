...
import { Ika, ThumbUpFill32 } from '@mysten/icons';
...
export function CardLayout({ children, title, subtitle, headerCaption, icon }: CardLayoutProps) {
	return (
		<div className="flex flex-col flex-nowrap rounded-20 items-center bg-ika-lightest shadow-wallet-content p-7.5 pt-10 flex-grow w-full max-h-popup-height max-w-popup-width overflow-auto">
			{icon === 'success' ? (
			...
			{icon === 'ika' ? (
				<div className="flex flex-col flex-nowrap items-center justify-center rounded-full w-16 h-16 bg-ika mb-7">
					<Ika className="text-white text-4xl" />
				</div>
			) : null}
			{headerCaption ? (
