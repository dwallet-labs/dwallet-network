...
pub(crate) struct LocalProtocolConfig {
    ...
    pub(crate) signature_mpc_paillier_public_parameters: Option<String>,
}
...
impl LocalProtocolConfig {
    fn new(config: &ProtocolConfig) -> Self {
        Self {
            ...
            signature_mpc_paillier_public_parameters: config.signature_mpc_paillier_public_parameters().map(|s| s.to_string()),
        }
    }
}
