...
const KNOWN_TYPES = {
  ikafren:
    '0x80d7de9c4a56194087e0ba0bf59492aa8e6a5ee881606226930827085ddf2332::ikafrens::IkaFren<0x80d7de9c4a56194087e0ba0bf59492aa8e6a5ee881606226930827085ddf2332::capy::Capy>',
};
...
program
  .command('list')
  .description('list an item in the Kiosk for the specified amount of IKA')
  .argument('<item ID>', 'The ID of the item to list')
  .argument('<amount NIKA>', 'The amount of IKA to list the item for')
  .action(listItem);
...
async function newKiosk() {
  const sender = keypair.getPublicKey().toIkaAddress();
...
async function showInventory({ address, onlyDisplay, cursor, filter }) {
  const owner = address || keypair.getPublicKey().toIkaAddress();

  if (!isValidIkaAddress(owner)) {
    throw new Error(`Invalid IKA address: "${owner}"`);
  }
...
async function showKioskContents({ id, address }) {
  ...
  if (id) {
    if (!isValidIkaObjectId(id)) {
      throw new Error(`Invalid Kiosk ID: "${id}"`);
    }

    kioskId = id;
  } else {
    const sender = address || keypair.getPublicKey().toIkaAddress();

    if (!isValidIkaAddress(sender)) {
      throw new Error(`Invalid IKA address: "${sender}"`);
    }
  ...
  const tabledItems = items
    .map((item) => ({
      ...
      'price (IKA)': item.listing ? formatAmount(item.listing.price) : 'N/A',
    }))
...
async function placeItem(itemId) {
  const kioskCap = await findKioskCap().catch(() => null);
  const owner = keypair.getPublicKey().toIkaAddress();

  if (kioskCap === null) {
    throw new Error('No Kiosk found for sender; use `new` to create one');
  }

  if (!isValidIkaObjectId(itemId)) {
...
async function takeItem(itemId, { address }) {
  const cap = await findKioskCap().catch(() => null);
  const receiver = address || keypair.getPublicKey().toIkaAddress();

  if (!isValidIkaAddress(receiver)) {
...
/**
 * Command: `list`
 * Description: Lists an item in the Kiosk for the specified amount of IKA
 */
...
async function purchaseItem(itemId, opts) {
  ...
  if (inputKioskId && !isValidIkaObjectId(inputKioskId)) {
...
function formatAmount(amount) {
  ...
  if (amount <= NIKA_PER_IKA) {
    return Number(amount) / Number(NIKA_PER_IKA);
  }
